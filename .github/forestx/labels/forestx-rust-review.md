...
- Each crate in the Cargo workspace in `forestx-rs` has a specific purpose: make a note if you believe new code is not introduced in the correct crate.
- When possible, try to keep the `core` crate as small as possible. Non-core but shared logic is often a good candidate for `forestx-rs/common`.
...
- If the nature of the change seems to have a visual component (which is often the case for changes to `forestx-rs/tui`), recommend including a screenshot or video to demonstrate the change, if appropriate.
...
{FORESTX_ACTION_GITHUB_EVENT_PATH} contains the JSON that triggered this GitHub workflow. It contains the `base` and `head` refs that define this PR. Both refs are available locally.

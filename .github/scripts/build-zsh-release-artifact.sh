...
work_root="$(mktemp -d "${temp_root%/}/forestx-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/forestx-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${FORESTX_WRAPPER_LOG:?missing FORESTX_WRAPPER_LOG}"
printf '%s\n' "$@" > "$FORESTX_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
...
FORESTX_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" forestx-zsh)

#!/usr/bin/env python3
"""Mirror a Forestx GitHub Release to Cloudflare R2.
...
Objects are created under ``forestx/releases/<version>/`` with a validated upload
...
objects are verified, the same metadata advances ``forestx/channels/latest`` when
the release is marked latest and ``forestx/channels/prerelease`` for prereleases.
Stable releases also update the mutable ``forestx/install.sh`` and
``forestx/install.ps1`` bootstrap aliases from their verified versioned assets.
"""

...
"""Verify forestx-tui does not depend on or import forestx-core directly."""
...
TUI_ROOT = ROOT / "forestx-rs" / "tui"
TUI_MANIFEST = TUI_ROOT / "Cargo.toml"
FORBIDDEN_PACKAGE = "forestx-core"
FORBIDDEN_SOURCE_PATTERNS = (
    re.compile(r"\bforestx_core::"),
    re.compile(r"\buse\s+forestx_core\b"),
    re.compile(r"\bextern\s+crate\s+forestx_core\b"),
)
...
def main() -> int:
    ...
    print("forestx-tui must not depend on or import forestx-core directly.")
    print(
        "Use the app-server protocol/client boundary instead; temporary embedded "
        "startup gaps belong behind forestx_app_server_client::legacy_core."
    )
    print()
    ...
    for path in sorted(TUI_ROOT.glob("**/*.rs")):
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            if any(pattern.search(line) for pattern in FORBIDDEN_SOURCE_PATTERNS):
                failures.append(f"{relative_path(path)}:{line_number} imports `forestx_core`")
    return failures

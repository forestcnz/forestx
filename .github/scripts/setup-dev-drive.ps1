...
if ((Test-Path "D:\") -and (Test-DevDrive "D:")) {
    ...
    try {
        $VhdPath = Join-Path $env:RUNNER_TEMP "forestx-dev-drive.vhdx"
        $SizeBytes = 64GB
        ...
        $Volume = $Partition | Format-Volume -FileSystem ReFS -NewFileSystemLabel "ForestxDevDrive" -DevDrive -Confirm:$false -Force -ErrorAction Stop

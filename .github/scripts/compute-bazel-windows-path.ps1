...
publishes the result via `GITHUB_ENV` as `FORESTX_BAZEL_WINDOWS_PATH` so later
...
Write-Host 'Derived FORESTX_BAZEL_WINDOWS_PATH entries:'
foreach ($pathEntry in $stablePathEntries) {
  Write-Host "  $pathEntry"
}
"FORESTX_BAZEL_WINDOWS_PATH=$stablePath" | Out-File -FilePath $env:GITHUB_ENV -Encoding utf8 -Append

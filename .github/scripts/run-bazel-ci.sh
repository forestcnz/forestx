...
if [[ -n "${FORESTX_BAZEL_EXECUTION_LOG_COMPACT_DIR:-}" ]]; then
  post_config_bazel_args+=(
    "--execution_log_compact_file=${FORESTX_BAZEL_EXECUTION_LOG_COMPACT_DIR}/execution-log-${bazel_args[0]}-${GITHUB_JOB:-local}-$$.zst"
  )
fi
...
if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
  ...
  if [[ -z "${FORESTX_BAZEL_WINDOWS_PATH:-}" ]]; then
    echo "FORESTX_BAZEL_WINDOWS_PATH must be set for Windows Bazel CI." >&2
    exit 1
  fi

  if [[ $pass_windows_build_env -eq 1 ]]; then
    post_config_bazel_args+=(
      "--action_env=PATH=${FORESTX_BAZEL_WINDOWS_PATH}"
      "--host_action_env=PATH=${FORESTX_BAZEL_WINDOWS_PATH}"
    )
  elif [[ $windows_cross_compile -eq 1 ]]; then
    # Remote build actions run on Linux RBE workers. Give their shell snippets
    # a Linux PATH while preserving FORESTX_BAZEL_WINDOWS_PATH below for local
  ...
  post_config_bazel_args+=("--test_env=PATH=${FORESTX_BAZEL_WINDOWS_PATH}")
fi

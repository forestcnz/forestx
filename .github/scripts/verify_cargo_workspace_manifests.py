...
"""Verify that forestx-rs Cargo manifests follow workspace manifest policy.

Checks:
- Crates inherit `[workspace.package]` metadata.
- Crates opt into `[lints] workspace = true`.
- Crate names follow the forestx-rs directory naming conventions.
...
CARGO_RS_ROOT = ROOT / "forestx-rs"
WORKSPACE_PACKAGE_FIELDS = ("version", "edition", "license")
TOP_LEVEL_NAME_EXCEPTIONS = {
    "windows-sandbox-rs": "forestx-windows-sandbox",
}
UTILITY_NAME_EXCEPTIONS = {
    "path-utils": "forestx-utils-path",
}
MANIFEST_FEATURE_EXCEPTIONS = {
    "forestx-rs/code-mode/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
    "forestx-rs/v8-poc/Cargo.toml": {"sandbox": ("v8/v8_enable_sandbox",)},
}
...
def main() -> int:
    ...
    print(
        "Cargo manifests under forestx-rs must inherit workspace package metadata, "
    ...
    print(
        "Cargo only applies `forestx-rs/Cargo.toml` `[workspace.lints.clippy]` "
    ...
    print()
    print(
        "Package-name checks apply to `forestx-rs/<crate>/Cargo.toml` and "
        "`forestx-rs/utils/<crate>/Cargo.toml`."
    )
    print(
...
def expected_package_name(path: Path) -> str | None:
    ...
    if len(parts) == 2 and parts[1] == "Cargo.toml":
        ...
        return TOP_LEVEL_NAME_EXCEPTIONS.get(
            directory,
            directory if directory.startswith("forestx-") else f"forestx-{directory}",
        )
    if len(parts) == 3 and parts[0] == "utils" and parts[2] == "Cargo.toml":
        directory = parts[1]
        return UTILITY_NAME_EXCEPTIONS.get(directory, f"forestx-utils-{directory}")
    return None

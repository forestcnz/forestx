...
usage() {
  cat <<'EOF'
Usage: build-forestx-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-bin <path>] \
  [--zsh-manifest <path>] \
  [--forestx-command-runner-bin <path>] \
  [--forestx-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --forestx-command-runner-bin)
      resource_args+=(
        --forestx-command-runner-bin
        "${2:?--forestx-command-runner-bin requires a value}"
      )
    ...
    --forestx-windows-sandbox-setup-bin)
      resource_args+=(
        --forestx-windows-sandbox-setup-bin
        "${2:?--forestx-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="forestx"
    entrypoint="forestx"
    archive_stem="forestx-package"
    ;;
  app-server)
    variant="forestx-app-server"
    entrypoint="forestx-app-server"
    archive_stem="forestx-app-server-package"
    ;;
  *)
    echo "No Forestx package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/forestx-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/forestx-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/forestx-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--forestx-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--forestx-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_forestx_package.py"
  --target "$target"
  --variant "$variant"

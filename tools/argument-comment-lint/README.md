...
targeted package runs such as `just argument-comment-lint -p forestx-core`.
...
```bash
./tools/argument-comment-lint/run.py -p forestx-core
```

Run the lint against `forestx-rs` from the repo root:

```bash
just argument-comment-lint
bazel build --config=argument-comment-lint -- \
  $(./tools/argument-comment-lint/list-bazel-targets.sh)
./tools/argument-comment-lint/run-prebuilt-linter.py -p forestx-core
just argument-comment-lint -p forestx-core
```

If no package selection is provided, `just argument-comment-lint` now defaults
to the Bazel aspect path over `//forestx-rs/...`. The Python wrappers remain the
...
```bash
./tools/argument-comment-lint/run-prebuilt-linter.py -p forestx-core -- --all-targets
```

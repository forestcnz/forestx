...
class WrapperCommonTest(unittest.TestCase):
    def test_defaults_to_workspace_and_all_targets(self) -> None:
        ...
        final_args = wrapper_common.build_final_args(parsed, Path("/repo/forestx-rs/Cargo.toml"))

        self.assertEqual(
            final_args,
            [
                "--manifest-path",
                "/repo/forestx-rs/Cargo.toml",
                "--workspace",
                "--no-deps",
                "--",
                "--all-targets",
    ...
    def test_forwarded_cargo_args_keep_single_separator(self) -> None:
        parsed = wrapper_common.parse_wrapper_args(["-p", "forestx-core", "--", "--tests"])
        final_args = wrapper_common.build_final_args(parsed, Path("/repo/forestx-rs/Cargo.toml"))
    ...
    def test_fix_does_not_add_all_targets(self) -> None:
        parsed = wrapper_common.parse_wrapper_args(["--fix", "-p", "forestx-core"])
        final_args = wrapper_common.build_final_args(parsed, Path("/repo/forestx-rs/Cargo.toml"))

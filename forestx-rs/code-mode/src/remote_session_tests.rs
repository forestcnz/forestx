...
use forestx_code_mode_protocol::CodeModeSessionProvider;
use forestx_code_mode_protocol::ExecuteRequest;
use forestx_code_mode_protocol::FunctionCallOutputContentItem;
use forestx_code_mode_protocol::RuntimeResponse;
...
use forestx_code_mode_protocol::host::WireCellId;
...
fn missing_host_error_limits_the_displayed_path_to_512_bytes() {
    let executable = "forestx-code-mode-host-does-not-exist";
...
async fn websocket_provider_executes_over_shared_connector() {
    ...
    assert_eq!(
        ...
        RuntimeResponse::Result {
            cell_id: forestx_code_mode_protocol::CellId::new("1".to_string()),
...
async fn provider_returns_missing_host_error() {
    let provider = ProcessOwnedCodeModeSessionProvider::with_host_program(
        "forestx-code-mode-host-does-not-exist".into(),
    );
    ...
    assert!(error.contains("failed to spawn code-mode host forestx-code-mode-host-does-not-exist"));
}
...
async fn shutdown_before_open_does_not_spawn_the_host() {
    ...
    let error = session
        .execute(forestx_code_mode_protocol::ExecuteRequest {

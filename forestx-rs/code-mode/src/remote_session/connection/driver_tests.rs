...
use forestx_code_mode_protocol::CellId;
use forestx_code_mode_protocol::CodeModeNestedToolCall;
use forestx_code_mode_protocol::CodeModeSessionDelegate;
use forestx_code_mode_protocol::ExecuteRequest;
use forestx_code_mode_protocol::NotificationFuture;
use forestx_code_mode_protocol::ToolInvocationFuture;
use forestx_code_mode_protocol::WaitRequest;
use forestx_code_mode_protocol::host::ClientToHost;
use forestx_code_mode_protocol::host::DelegateRequest;
use forestx_code_mode_protocol::host::DelegateRequestId;
...
struct DriverHarness {
    ...
    outgoing_rx: mpsc::Receiver<forestx_code_mode_protocol::host::EncodedFrame>,
...
impl DriverHarness {
    ...
    async fn start_cell(
        ...
        request_id: i64,
        cell_id: &str,
    ) -> forestx_code_mode_protocol::StartedCell {
        let (response_tx, response_rx) = oneshot::channel();
        self.command_tx
            ...
            .send(DriverEvent::HostMessage(HostToClient::DelegateRequest {
                ...
                request: DelegateRequest::InvokeTool {
                    invocation: WireNestedToolCall {
                        ...
                        tool_kind: forestx_code_mode_protocol::CodeModeToolKind::Function.into(),
                        input: None,
                    },
                },
            }))
...
async fn deferred_delegates_follow_cell_readiness_and_cancellation() {
    ...
    assert_eq!(
        ...
        ClientToHost::DelegateResponse {
            ...
            result: WireResult::Ok {
                value: forestx_code_mode_protocol::host::DelegateResponse::NotificationDelivered,
            },
        }
    );
...
async fn terminate_closes_cell_without_waiting_for_delegate_cleanup() {
    ...
    assert_eq!(
        ...
        Ok(forestx_code_mode_protocol::WaitOutcome::LiveCell(
            forestx_code_mode_protocol::RuntimeResponse::Terminated {
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }
        ))
...
async fn remote_wait_accepts_durations_longer_than_five_minutes() {
    ...
    assert_eq!(
        ...
        Ok(forestx_code_mode_protocol::WaitOutcome::LiveCell(
            forestx_code_mode_protocol::RuntimeResponse::Yielded {
                cell_id: CellId::new("1".to_string()),
                content_items: Vec::new(),
            }
        ))

    ...
    pub(super) fn handle_host_message(&mut self, message: HostToClient) -> bool {
        if self.should_defer_host_message(&message) {
            if self.deferred_host_messages.len()
                >= forestx_code_mode_protocol::host::MAX_PENDING_DELEGATE_CALLS
            {
                self.fail(
        ...
        match message {
            HostToClient::DelegateRequest {
                ...
                let cell_id = match request {
                    forestx_code_mode_protocol::host::DelegateRequest::InvokeTool { invocation } => {
                        &invocation.cell_id
                    }
                    forestx_code_mode_protocol::host::DelegateRequest::Notify { cell_id, .. } => {
                        cell_id
                    }
                };

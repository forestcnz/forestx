...
const FORESTX_AUTH_SECRETS_FILENAME: &str = "forestx_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// Forestx authentication credentials used by the CLI, TUI, app server, and other clients.
    ForestxAuth,
...
impl LocalSecretsBackend {
    pub fn new(forestx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.forestx_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(forestx_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let forestx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(forestx_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let forestx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let forestx_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        forestx_auth_backend.set(&scope, &name, "forestx-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            forestx_auth_backend.get(&scope, &name)?,
            Some("forestx-auth-value".to_string())
        );
        assert_eq!(
        ...
        assert!(!forestx_home.path().join("secrets").join("local.age").exists());
        Ok(())
    }
}

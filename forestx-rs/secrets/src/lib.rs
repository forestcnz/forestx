...
const KEYRING_SERVICE: &str = "forestx";
...
impl SecretsManager {
    pub fn new(forestx_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(forestx_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(forestx_home: &Path) -> String {

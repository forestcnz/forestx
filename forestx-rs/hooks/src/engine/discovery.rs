...
use forestx_config::HookStateToml;
...
use forestx_config::TomlValue;
use forestx_config::version_for_toml;
...
use forestx_protocol::protocol::HookTrustStatus;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project { dot_forestx_folder } => layer
            .hooks_config_folder()
            .unwrap_or_else(|| dot_forestx_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: forestx_protocol::protocol::HookEventName,
        ...
        for (handler_index, handler) in group.hooks.iter().cloned().enumerate() {
            match handler {
                HookHandlerConfig::Command {
                    ...
                    if r#async && event_name != forestx_protocol::protocol::HookEventName::SessionEnd
                    {
                        warnings.push(format!(
                    ...
                    let additional_context_limit = if matches!(
                        event_name,
                        forestx_protocol::protocol::HookEventName::PreToolUse
                            | forestx_protocol::protocol::HookEventName::PostToolUse
                            | forestx_protocol::protocol::HookEventName::SessionStart
                            | forestx_protocol::protocol::HookEventName::UserPromptSubmit
                            | forestx_protocol::protocol::HookEventName::SubagentStart
                    ) {
                        additional_context_limit
                    } else {
...
fn normalize_command_hook(
    ...
    if event_name != forestx_protocol::protocol::HookEventName::SessionEnd {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/forestx/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.forestx/config.toml").abs();
        let dot_forestx_folder = test_path_buf("/tmp/worktree/.forestx").abs();

...
async fn loads_plugin_namespace_dependencies_and_policy() {
    ...
    fs::create_dir_all(root.path().join(".forestx-plugin")).expect("manifest dir");
...
"#,
    ...
    let outcome =
        load_environment_skills_from_root(LOCAL_FS.as_ref(), &root_uri, Some(Product::Forestx)).await;

...
use forestx_exec_server::ExecutorFileSystemFuture;
use forestx_exec_server::FileMetadata;
use forestx_exec_server::FileSystemReadStream;
use forestx_exec_server::FileSystemSandboxContext;
use forestx_exec_server::LOCAL_FS;
use forestx_exec_server::ReadDirectoryEntry;
use forestx_exec_server::RemoveOptions;
use forestx_exec_server::WalkOptions;
use forestx_exec_server::WalkOutcome;
...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".forestx";
...
async fn load_user_skills_for_test(forestx_home: &TempDir) -> SkillLoadOutcome {
    load_skills_for_test([local_skill_root(
        &forestx_home.path().join(SKILLS_DIR_NAME),
        SkillScope::User,
    )])
    .await
}
...
fn write_skill(forestx_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&forestx_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(forestx_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &forestx_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
fn write_plugin_manifest(plugin_root: &Path, contents: &str) {
    let manifest_path = plugin_root.join(".forestx-plugin/plugin.json");
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&forestx_home, "demo", "dep-skill", "from json");
...
"#,
    ...
    let outcome = load_user_skills_for_test(&forestx_home).await;
...
async fn loads_skill_interface_metadata_from_yaml() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&forestx_home, "demo", "ui-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_from_yaml() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&forestx_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&forestx_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&forestx_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::Forestx, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(forestx_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &forestx_home.path().join("skills/shared"));
...
async fn ignores_symlinked_skill_file_for_user_scope() {
    ...
    let skill_dir = forestx_home.path().join("skills/demo");
    fs::create_dir_all(&skill_dir).unwrap();
...
async fn does_not_loop_on_symlink_cycle_for_user_scope() {
    ...
    // Create a cycle:
    //   $FORESTX_HOME/skills/cycle/loop -> $FORESTX_HOME/skills/cycle
    let cycle_dir = forestx_home.path().join("skills/cycle");
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = forestx_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = forestx_home.path().join("skills");
    let outcome = load_skills_from_roots(
        [SkillRoot {
...
async fn loads_valid_skill() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&forestx_home, "demo", "demo-skill", "does things\ncarefully");
    let outcome = load_user_skills_for_test(&forestx_home).await;
    assert!(
        outcome.errors.is_empty(),
        "unexpected errors: {:?}",
...
async fn skips_hidden_and_invalid() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = forestx_home.path().join("skills/.hidden");
    ...
    let invalid_dir = forestx_home.path().join("skills/invalid");
...
async fn preserves_overlong_descriptions() {
    ...
    write_skill(&forestx_home, "max-len", "max-len", &max_desc);
    ...
    write_skill(&forestx_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_user_skills_for_test(&forestx_home).await;
    assert!(
        outcome.errors.is_empty(),
        "unexpected errors: {:?}",
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&forestx_home, "user", "dupe-skill", "from user");
    let repo_skills_root = repo_dir
        .path()
        .join(REPO_ROOT_CONFIG_DIR_NAME)
        .join(SKILLS_DIR_NAME);
    ...
    let outcome = load_skills_for_test([
        ...
        local_skill_root(&forestx_home.path().join(SKILLS_DIR_NAME), SkillScope::User),
    ])
    .await;
    assert!(
        outcome.errors.is_empty(),
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_forestx_dirs() {
    let repo_dir = tempfile::tempdir().expect("tempdir");
...
async fn loads_skills_from_system_scoped_root() {
    ...
    let skill_path = write_system_skill(&forestx_home, "system", "system-skill", "from system");
    let system_root = forestx_home.path().join("skills/.system");

...
use forestx_core_skills::SkillMetadata;
...
fn user_layer(forestx_home: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
        ConfigLayerSource::User {
            file: forestx_home.join("config.toml"),
            profile: None,
        },
        empty_config(),
    )
...
fn project_layer(dot_forestx_folder: &AbsolutePathBuf) -> ConfigLayerEntry {
    ConfigLayerEntry::new(
...
fn layer_roots_preserve_scope_precedence_and_disabled_projects() {
    ...
    let system_folder = absolute(temp_dir.path().join("etc/forestx"));
    let home_folder = absolute(temp_dir.path().join("home"));
    let user_folder = home_folder.join("forestx");
    let project_folder = absolute(temp_dir.path().join("repo/.forestx"));
    let nested_project_folder = absolute(temp_dir.path().join("repo/nested/.forestx"));
    let config_stack = stack(vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_forestx_folder: project_folder.clone(),
            },
            empty_config(),
            "untrusted project",
        ),
...
async fn resolved_project_layer_loads_skill_without_git_marker() {
    ...
    let dot_forestx = workspace.join(".forestx");
    let skill_root = dot_forestx.join("skills");
    fs::create_dir_all(&workspace).expect("create workspace");
    let skill_path = write_skill(&skill_root, "local", "local-skill");
    let config_stack = stack(vec![project_layer(&dot_forestx)]);
...
async fn resolved_project_layer_loads_skill_when_cwd_is_file() {
    ...
    let dot_forestx = repository.join(".forestx");
    let skill_root = dot_forestx.join("skills");
...
async fn resolved_config_and_repo_roots_preserve_order_and_dedupe_paths_not_names() {
    let temp_dir = TempDir::new().expect("temp dir");
    let home_folder = absolute(temp_dir.path().join("home"));
    let forestx_home = home_folder.join("forestx");
    let system_folder = absolute(temp_dir.path().join("etc/forestx"));
    let repository = absolute(temp_dir.path().join("repo"));
    ...
    let project_dot_forestx = repository.join(".forestx");
    let nested_project_dot_forestx = repository.join("nested/.forestx");
    let user_skills = forestx_home.join("skills");
    let root_project_skill = write_skill(
        &project_dot_forestx.join("skills"),
        "root-duplicate",
        "duplicate-skill",
    );
    let nested_project_skill = write_skill(
        &nested_project_dot_forestx.join("skills"),
        "nested-duplicate",
        "duplicate-skill",
    );
    ...
    let system_skill = write_skill(&forestx_home.join("skills/.system"), "system", "system-skill");
    ...
    let config_stack = stack(vec![
        ...
        user_layer(&forestx_home),
        project_layer(&project_dot_forestx),
        project_layer(&nested_project_dot_forestx),
    ]);

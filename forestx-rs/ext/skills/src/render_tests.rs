...
fn path_aliases_are_not_used_without_budget_pressure() {
    let root = "/Users/test/.forestx/plugins/cache/openai-curated/example/hash/skills";
    let catalog = SkillCatalog {
        entries: vec![
...
fn path_aliases_retain_every_skill_under_budget_pressure() {
    let root = "/Users/test/.forestx/plugins/cache/openai-curated/example/hash1234567890/skills-with-a-very-long-shared-prefix";
    let entries = (0..12)
        .map(|index| {
...
async fn host_alias_roots_follow_core_discovery_order() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parent = std::env::temp_dir().join(format!(
        "forestx-skills-extension-alias-order-{}-{unique}",
        std::process::id()
    ));
...
fn singleton_plugin_versions_share_the_marketplace_alias_root() {
    let github_root = "/Users/test/.forestx/plugins/cache/openai-curated/github/hash123/skills";
    let slack_root = "/Users/test/.forestx/plugins/cache/openai-curated/slack/hash456/skills";
    let entries = [
    ...
    assert_eq!(
        ...
        vec!["- `r0` = `/Users/test/.forestx/plugins/cache/openai-curated`".to_string()]
    );
    assert_eq!(

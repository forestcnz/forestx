...
use forestx_core_skills::loader::load_environment_skills_from_root;
use forestx_exec_server::CapabilityRootDiscoverRequest;
use forestx_exec_server::CapabilityRootsDiscoverParams;
use forestx_exec_server::LOCAL_FS;
use forestx_exec_server::discover_capability_roots;
...
async fn executor_bundle_parser_matches_the_existing_environment_loader() {
    ...
    let plugin_manifest = root.path().join(".forestx-plugin/plugin.json");
...
async fn executor_bundle_preserves_parent_namespace_and_manifest_precedence() {
    ...
    for (relative_path, name) in [
        (".forestx-plugin/plugin.json", "forestx-name"),
    ...
    assert!(
        discovery.namespace_manifests[0]
            .path
            .to_string()
            .ends_with("/.forestx-plugin/plugin.json")
    );
    assert_eq!(bundled.warnings, existing.warnings);
    assert_eq!(
        bundled
    ...
    assert_eq!(bundled.skills[0].metadata.name, "forestx-name:search");
}

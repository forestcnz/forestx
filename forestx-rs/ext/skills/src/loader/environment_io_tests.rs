...
use forestx_skills::EnvironmentSkillMetadata;
...
async fn loads_nearest_plugin_namespaces_without_reading_unused_sibling_manifests() {
    ...
    for (plugin_root, name) in [
        ...
        fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("manifest dir");
        fs::write(
    ...
    let mut expected_manifest_reads = [&outer_root, &inner_root]
        .into_iter()
        .map(|plugin_root| {
            PathUri::from_host_native_path(plugin_root.join(".forestx-plugin/plugin.json")).unwrap()
        })
        .collect::<Vec<_>>();
...
async fn reuses_walk_inventory_for_missing_skill_metadata() {
    ...
    let root = tempdir().expect("tempdir");
    let manifest_path = root.path().join(".forestx-plugin/plugin.json");

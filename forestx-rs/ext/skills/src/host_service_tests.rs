...
fn write_user_skill(forestx_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = forestx_home.path().join("skills").join(dir);
...
fn user_config_layer(forestx_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
...
fn config_stack(forestx_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(forestx_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(forestx_home, user_config_toml),
            ConfigLayerEntry::new(
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&forestx_home, "");
    let skills_service = HostSkillsService::new(
        forestx_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

    write_user_skill(&forestx_home, "a", "skill-a", "from a");
    let outcome1 =
    ...
    write_user_skill(&forestx_home, "b", "skill-b", "from b");
    let outcome2 =
        skills_for_config_with_stack(&skills_service, &cwd, &config_layer_stack, &[]).await;
...
async fn skills_for_cwd_loads_repo_and_user_roots_with_local_fs() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let repo_dot_forestx = cwd.path().join(".forestx");
    fs::create_dir_all(&repo_dot_forestx).expect("create repo config dir");

    write_user_skill(&forestx_home, "user", "user-skill", "from local user root");
    let repo_skill_dir = repo_dot_forestx.join("skills/repo");
    ...
    let config_layer_stack = ConfigLayerStack::new(
        vec![
            user_config_layer(&forestx_home, ""),
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_forestx_folder: repo_dot_forestx.abs(),
                },
    ...
    let other_file_system: Arc<dyn forestx_exec_server::ExecutorFileSystem> =
        Arc::new(forestx_exec_server::LocalFileSystem::unsandboxed());
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let forestx_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = forestx_home.path().join("skills/.system/bundled-skill");
    ...
    let config_layer_stack = config_stack(&forestx_home, "[skills.bundled]\nenabled = false\n");
    let skills_service = HostSkillsService::new(
        forestx_home.path().abs(),
...
async fn skills_for_cwd_uses_cached_result_until_force_reload() {
    ...
    write_user_skill(&forestx_home, "late", "late-skill", "added after cache");
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    ...
    let parent_stack = config_stack(&forestx_home, &disabled_skill_config);
    let child_stack =
        config_stack_with_session_flags(&forestx_home, &disabled_skill_config, &enabled_skill_config);
    let skills_service = HostSkillsService::new(
        forestx_home.path().abs(),
        /*bundled_skills_enabled*/ true,
    );

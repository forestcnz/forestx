...
use forestx_protocol::mcp::ResourceContent;
use url::Url;
...
const ORCHESTRATOR_SKILL_DISCOVERY_DURATION_METRIC: &str =
    "forestx.skills.orchestrator.discovery.duration_ms";
const ORCHESTRATOR_SKILL_DISCOVERY_RESOURCES_METRIC: &str =
    "forestx.skills.orchestrator.discovery.resources_total";
...
impl SkillProvider for OrchestratorSkillProvider {
    fn list(&self, query: SkillListQuery) -> SkillProviderFuture<'_, SkillCatalog> {
        Box::pin(async move {
            ...
            if !client.has_server(FORESTX_APPS_MCP_SERVER_NAME).await {
                return Ok(SkillCatalog::default());
            }

            let _discovery_timer =
                forestx_otel::start_global_timer(ORCHESTRATOR_SKILL_DISCOVERY_DURATION_METRIC, &[])
                    .ok();
            ...
            for _ in 0..MAX_RESOURCE_PAGES {
                let page = match tokio::time::timeout_at(
                    discovery_deadline,
                    client.list_resources(FORESTX_APPS_MCP_SERVER_NAME, cursor.clone()),
                )
                .await
                {
    ...
    fn read(&self, request: SkillReadRequest) -> SkillProviderFuture<'_, SkillReadResult> {
        Box::pin(async move {
            if request.authority
                != SkillAuthority::new(SkillSourceKind::Orchestrator, FORESTX_APPS_MCP_SERVER_NAME)
            {
            ...
            let result = tokio::time::timeout(
                ...
                client.read_resource(FORESTX_APPS_MCP_SERVER_NAME, request.resource.as_str()),
            )
            .await
            .map_err(|_| {
                SkillProviderError::new(format!(

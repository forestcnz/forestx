...
use forestx_core_skills::loader::load_environment_skills_from_root as load_legacy_environment_skills_from_root;
...
use forestx_skills_extension::ExecutorSkillProvider;
...
async fn skill_loading_and_reads_use_the_supplied_executor_file_system() {
    let test_root =
        std::env::temp_dir().join(format!("forestx-executor-skill-fs-{}", std::process::id()));
...
async fn executor_discovery_routes_produce_equivalent_catalog_metadata() {
    ...
    let test_root = std::env::temp_dir().join(format!(
        "forestx-executor-skill-parity-{}-{id}",
        std::process::id()
    ));
    let plugin_manifest = test_root.join(".forestx-plugin/plugin.json");
    ...
    let comparable_entries = |catalog: &forestx_skills_extension::catalog::SkillCatalog| {
        catalog
            .entries
            .iter()
            .map(|entry| {
...
async fn pre_discovered_executor_catalog_snapshot() {
    ...
    let manifest_dir = test_root.join(".forestx-plugin");
...
async fn direct_executor_discovery_preserves_hidden_nested_and_probed_metadata() {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "forestx-executor-skill-discovery-{}-{id}",
        std::process::id()
    ));
    let outer_manifest = test_root.join(".forestx-plugin/plugin.json");
    ...
    let inner_manifest = test_root.join("nested/.forestx-plugin/plugin.json");
...
fn create_local_skill_root(label: &str) -> io::Result<std::path::PathBuf> {
    let id = NEXT_TEST_ROOT_ID.fetch_add(1, Ordering::Relaxed);
    let test_root = std::env::temp_dir().join(format!(
        "forestx-executor-skill-{label}-{}-{id}",
        std::process::id()
    ));

...
use forestx_core_skills::HostSkillsSnapshot;
use forestx_core_skills::SkillLoadOutcome;
use forestx_core_skills::injection::InjectedHostSkillPrompts;
use forestx_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
use forestx_core_skills::loader::SkillRoot;
use forestx_core_skills::loader::load_skills_from_roots;
use forestx_exec_server::LOCAL_FS;
use forestx_extension_api::ConversationHistory;
...
use forestx_extension_api::ExtensionWarning;
use forestx_extension_api::NoopTurnItemEmitter;
use forestx_extension_api::PreviousWorldStateSection;
use forestx_extension_api::ThreadStartInput;
use forestx_extension_api::ToolCall;
use forestx_extension_api::ToolPayload;
use forestx_extension_api::TurnInputContext;
use forestx_extension_api::WorldStateContributionInput;
use forestx_models_manager::model_info::model_info_from_slug;
use forestx_otel::MetricsClient;
use forestx_otel::MetricsConfig;
use forestx_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use forestx_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use forestx_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use forestx_otel::THREAD_SKILLS_TRUNCATED_METRIC;
...
use forestx_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use forestx_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
use forestx_protocol::protocol::SessionSource;
use forestx_protocol::protocol::SkillScope;
use forestx_protocol::protocol::TruncationPolicy;
...
use forestx_skills_extension::HostSkillProvider;
use forestx_skills_extension::SkillProviders;
use forestx_skills_extension::SkillsExtensionConfig;
use forestx_skills_extension::catalog::SkillAuthority;
use forestx_skills_extension::catalog::SkillCatalog;
use forestx_skills_extension::catalog::SkillCatalogEntry;
use forestx_skills_extension::catalog::SkillPackageId;
use forestx_skills_extension::catalog::SkillProviderError;
use forestx_skills_extension::catalog::SkillReadResult;
use forestx_skills_extension::catalog::SkillResourceId;
use forestx_skills_extension::catalog::SkillSearchResult;
use forestx_skills_extension::catalog::SkillSourceKind;
use forestx_skills_extension::install;
use forestx_skills_extension::install_with_providers;
use forestx_skills_extension::install_with_providers_and_metrics;
use forestx_skills_extension::provider::SkillListQuery;
use forestx_skills_extension::provider::SkillProvider;
use forestx_skills_extension::provider::SkillProviderFuture;
use forestx_skills_extension::provider::SkillReadRequest;
use forestx_skills_extension::provider::SkillSearchRequest;
...
static NEXT_FORESTX_HOME_ID: AtomicUsize = AtomicUsize::new(0);
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    let forestx_home = test_forestx_home();
    let skill_path = forestx_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(forestx_home)?;
    Ok(())
}
...
async fn host_world_state_records_catalog_metrics_on_publish_and_change() -> TestResult {
    ...
    let skill_path = AbsolutePathBuf::try_from(test_forestx_home().join("skills/demo/SKILL.md"))?;
    let mut outcome = SkillLoadOutcome::default();
    outcome.skills.push(SkillMetadata {
        name: "demo".to_string(),
        description: "Demo skill.".to_string(),
    ...
    let sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: forestx_protocol::ThreadId::new(),
            turn_id: "turn-1",
            environments: &[],
    ...
    let second_skill_path =
        AbsolutePathBuf::try_from(test_forestx_home().join("skills/other/SKILL.md"))?;
    outcome.skills.push(SkillMetadata {
...
async fn shadow_selection_uses_host_catalog_when_instructions_are_disabled() -> TestResult {
    ...
    let catalog_entry_counts = snapshot
        ...
        .find(|metric| metric.name() == "forestx.skills.shadow_selection.catalog_entries")
...
async fn host_catalog_compacts_shared_paths_under_budget_pressure() -> TestResult {
    let test_root = test_forestx_home();
...
fn test_forestx_home() -> PathBuf {
    let id = NEXT_FORESTX_HOME_ID.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!(
        "forestx-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}

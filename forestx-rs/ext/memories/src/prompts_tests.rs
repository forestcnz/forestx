...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let forestx_home = AbsolutePathBuf::from_absolute_path(temp.path()).unwrap();
    let memories_dir = forestx_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(&forestx_home)
        .await
        .unwrap();

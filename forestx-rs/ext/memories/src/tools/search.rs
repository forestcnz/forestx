    ...
    B: MemoriesBackend,
{
    ...
    fn spec(&self) -> ToolSpec {
        memory_function_tool::<SearchArgs, SearchMemoriesResponse>(
            SEARCH_TOOL_NAME,
            "Search Forestx memory files for substring matches, optionally normalizing separators or requiring all query substrings on the same line or within a line window.",
        )
    }
    ...
    B: MemoriesBackend,
{
    async fn handle_call(
        &self,
        call: ToolCall,
    ) -> Result<Box<dyn forestx_extension_api::ToolOutput>, forestx_extension_api::FunctionCallError>
    {

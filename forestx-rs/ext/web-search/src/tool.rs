...
use forestx_api::SearchClient;
use forestx_api::SearchCommands;
use forestx_api::SearchQuery;
use forestx_api::SearchRequest;
use forestx_api::SearchSettings;
use forestx_core::X_FORESTX_TURN_METADATA_HEADER;
use forestx_core::web_search_action_detail;
use forestx_extension_api::ExtensionTurnItem;
...
use forestx_extension_api::parse_tool_input_schema_without_compaction;
use forestx_extension_items::ExtensionItem;
use forestx_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "forestx.web_search.results.payload_bytes";
...
impl ToolExecutor<ToolCall> for WebSearchTool {
    ...
    fn handle(&self, call: ToolCall) -> forestx_extension_api::ToolExecutorFuture<'_> {
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.forestx_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = forestx_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_FORESTX_TURN_METADATA_HEADER, header_value);
    }

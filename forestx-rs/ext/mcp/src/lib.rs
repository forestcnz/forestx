...
use forestx_mcp::hosted_plugin_runtime_mcp_server_config;
...
impl McpServerContributor<Config> for HostedPluginRuntimeExtension {
    ...
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            let name = FORESTX_APPS_MCP_SERVER_NAME.to_string();
            if !config.features.enabled(forestx_features::Feature::Apps) {
...
pub fn install_executor_plugins(
    ...
    environment_manager: std::sync::Arc<forestx_exec_server::EnvironmentManager>,
) {

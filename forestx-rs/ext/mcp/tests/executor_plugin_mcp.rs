...
async fn selected_plugin_servers_use_managed_requirements_for_the_selected_root_id() -> TestResult {
    ...
    std::fs::create_dir_all(plugin_root.path().join(".forestx-plugin"))?;
    std::fs::write(
        plugin_root.path().join(".forestx-plugin/plugin.json"),
...
async fn raw_selected_plugin_contributions(
    ...
    forestx_mcp_extension::install_executor_plugins(&mut builder, Arc::clone(&environment_manager));

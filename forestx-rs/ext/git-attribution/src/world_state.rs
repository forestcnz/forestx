...
use forestx_extension_api::RenderedWorldStateFragment;
...
Co-authored-by: Forestx <noreply@openai.com>
...
When you create or amend a git commit or create or update a pull request for this task, add Forestx attribution exactly once:
- Commit messages must end with `Co-authored-by: Forestx <noreply@openai.com>`. Preserve existing trailers and, if this exact trailer is missing, append it with one blank line before the trailer block.
- Pull request bodies must include the exact line `Generated with [Forestx](https://openai.com/forestx/).`. Preserve the existing body and hidden markers and, if this exact marker is missing, append it near the end, before any trailing hidden metadata markers.
...
- Ignore any earlier instructions disabling Forestx attribution; this policy reflects the current workspace.
- Do not rewrite an existing commit or pull request solely to add attribution.
"#;
pub(super) const DISABLED_INSTRUCTIONS: &str = "
Forestx commit and pull request attribution is disabled for the current workspace. Ignore any earlier instructions requiring Forestx attribution and do not add it.
";
...
fn is_enabled_fragment(role: &str, text: &str) -> bool {
    role == "developer"
        ...
        && text.contains("Co-authored-by: Forestx <noreply@openai.com>")
        && (text.contains("Generated with [Forestx](https://openai.com/forestx/).")
            || text.contains("Generated with Forestx."))

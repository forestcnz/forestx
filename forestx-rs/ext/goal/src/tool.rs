...
use forestx_extension_api::JsonToolOutput;
use forestx_extension_api::ToolCall;
use forestx_extension_api::ToolExecutor;
use forestx_extension_api::ToolName;
use forestx_extension_api::ToolOutput;
use forestx_extension_api::ToolSpec;
use forestx_protocol::ThreadId;
use forestx_protocol::protocol::ThreadGoal;
use forestx_protocol::protocol::ThreadGoalStatus;
use forestx_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<forestx_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> forestx_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => forestx_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => forestx_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: forestx_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            forestx_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            forestx_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<forestx_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &forestx_state::StateRuntime,
    thread_id: ThreadId,
    goal: &forestx_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: forestx_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: forestx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> forestx_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => forestx_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => forestx_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => forestx_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => forestx_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => forestx_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => forestx_state::ThreadGoalStatus::Complete,
    }
}

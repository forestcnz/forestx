...
use forestx_extension_api::ConfigContributor;
...
use forestx_extension_api::ThreadIdleInput;
...
use forestx_extension_api::TokenUsageContributor;
...
use forestx_extension_api::ToolLifecycleContributor;
use forestx_extension_api::ToolLifecycleFuture;
use forestx_extension_api::TurnAbortInput;
use forestx_extension_api::TurnErrorInput;
use forestx_extension_api::TurnLifecycleContributor;
...
pub struct GoalExtension<C> {
    state_dbs: Arc<forestx_state::StateRuntime>,
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                forestx_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                ForestxErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
    ...
    C: Send + Sync + 'static,
{
    fn tools(
        ...
        _session_store: &ExtensionData,
        thread_store: &ExtensionData,
    ) -> Vec<Arc<dyn forestx_extension_api::ToolExecutor<forestx_extension_api::ToolCall>>> {
        let Some(runtime) = goal_runtime_handle(thread_store) else {
            return Vec::new();
        };

...
pub struct PreviousGoalSnapshot {
    pub goal_id: String,
    pub status: forestx_state::ThreadGoalStatus,
    pub objective: String,
}

impl From<&forestx_state::ThreadGoal> for PreviousGoalSnapshot {
    fn from(goal: &forestx_state::ThreadGoal) -> Self {
        Self {
...
impl GoalRuntimeHandle {
    ...
    pub async fn apply_external_goal_set(
        ...
        match goal.status {
            ...
            forestx_state::ThreadGoalStatus::Paused
            | forestx_state::ThreadGoalStatus::Blocked
            | forestx_state::ThreadGoalStatus::UsageLimited
            | forestx_state::ThreadGoalStatus::Complete => {
                self.inner.accounting_state.clear_active_goal();
            }
        }
        Ok(())
    ...
    pub(crate) async fn stop_active_goal_for_turn(
        ...
        let (event_name, status) = match reason {
            ActiveGoalStopReason::TurnError => {
                ("turn-error", forestx_state::ThreadGoalStatus::Blocked)
            }
            ActiveGoalStopReason::UsageLimit => {
                ("usage-limit", forestx_state::ThreadGoalStatus::UsageLimited)
            }
        };
        self.account_active_goal_progress(
            turn_id,
        ...
        let can_stop = active_goal.status == forestx_state::ThreadGoalStatus::Active
            || (active_goal.status == forestx_state::ThreadGoalStatus::BudgetLimited
                && status == forestx_state::ThreadGoalStatus::UsageLimited);
        if !can_stop {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    pub async fn restore_after_resume(&self) -> Result<(), String> {
        ...
        let goal = self
            ...
            .map_err(|err| err.to_string())?;
        match goal {
            Some(goal) if goal.status == forestx_state::ThreadGoalStatus::Active => {
                self.inner
                    .accounting_state
    ...
    pub(crate) async fn continue_if_idle(&self) -> Result<(), String> {
        ...
        if goal.status != forestx_state::ThreadGoalStatus::Active {
            self.inner.accounting_state.clear_active_goal();
            return Ok(());
        }
    ...
    async fn account_idle_goal_progress(
        ...
        Ok(match outcome {
            ...
            forestx_state::GoalAccountingOutcome::Unchanged(_) => {
    ...
    ) -> Result<Option<forestx_state::ThreadGoalStatus>, String> {
        let goal = self
            .inner
            .state_dbs
            .thread_goals()

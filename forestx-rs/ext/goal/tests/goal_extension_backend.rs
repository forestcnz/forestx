...
use forestx_extension_api::FunctionCallError;
use forestx_extension_api::NoopTurnItemEmitter;
use forestx_extension_api::ThreadResumeInput;
use forestx_extension_api::ThreadStartInput;
use forestx_extension_api::ThreadStopInput;
use forestx_extension_api::ToolCall;
use forestx_extension_api::ToolCallOutcome;
use forestx_extension_api::ToolCallSource;
use forestx_extension_api::ToolExecutor;
use forestx_extension_api::ToolFinishInput;
use forestx_extension_api::ToolPayload;
use forestx_extension_api::TurnErrorInput;
use forestx_extension_api::TurnStartInput;
use forestx_extension_api::TurnStopInput;
use forestx_goal_extension::GoalObjectiveUpdate;
use forestx_goal_extension::GoalRuntimeHandle;
use forestx_goal_extension::GoalService;
use forestx_goal_extension::GoalSetRequest;
use forestx_goal_extension::GoalTokenBudgetUpdate;
use forestx_goal_extension::install_with_backend;
...
use forestx_protocol::protocol::ThreadGoalStatus;
...
async fn budget_limited_goal_keeps_accruing_until_turn_stop() -> anyhow::Result<()> {
    ...
    assert_eq!(forestx_state::ThreadGoalStatus::BudgetLimited, goal.status);
...
async fn turn_error_usage_limit_accounts_progress_and_clears_accounting() -> anyhow::Result<()> {
    ...
    harness
        ...
        .await;
    harness
        .notify_turn_error("turn-1", ForestxErrorInfo::UsageLimitExceeded)
        .await;
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(forestx_state::ThreadGoalStatus::UsageLimited, goal.status);
    assert_eq!(
        vec![
            CapturedGoalEvent {
...
async fn turn_error_blocks_goal() -> anyhow::Result<()> {
    ...
    harness
        .notify_turn_error("turn-1", ForestxErrorInfo::Other)
        .await;
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(forestx_state::ThreadGoalStatus::Blocked, goal.status);
    Ok(())
}
...
async fn usage_limit_plan_turn_does_not_stop_goal() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(forestx_state::ThreadGoalStatus::Active, goal.status);
...
async fn thread_resume_rehydrates_active_goal_idle_accounting() -> anyhow::Result<()> {
    ...
    runtime
        ...
        .replace_thread_goal(
            ...
            forestx_state::ThreadGoalStatus::Active,
            /*token_budget*/ None,
        )
        .await?;
    let harness = GoalExtensionHarness::new(runtime.clone(), thread_id).await?;
...
async fn installed_tools(
    runtime: Arc<forestx_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: forestx_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = forestx_extension_api::ToolName::plain(tool_name);
    ...
    async fn notify_turn_error(&self, turn_id: &str, error: ForestxErrorInfo) {
        let turn_store = ExtensionData::new(turn_id);
        for contributor in self.registry.turn_lifecycle_contributors() {
            contributor
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: forestx_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        forestx_turn_metadata: None,
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: forestx_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<forestx_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    forestx_state::StateRuntime::init(
        forestx_state::SqliteConfig::new_for_testing(tempdir.keep().as_path().abs()),
...
async fn seed_thread_metadata(
    runtime: &forestx_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = forestx_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .sqlite()
            .home()
...
fn protocol_status(status: forestx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
        forestx_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        forestx_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        forestx_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        forestx_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        forestx_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        forestx_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

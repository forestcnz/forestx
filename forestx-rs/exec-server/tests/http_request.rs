...
use forestx_exec_server_protocol::JSONRPCError;
...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
    ...
    assert_eq!(
        (
            ...
            captured.headers.get("x-forestx-test").map(String::as_str),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<forestx_exec_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,

...
async fn exec_server_reports_malformed_websocket_json_and_keeps_running() -> anyhow::Result<()> {
    ...
    assert_eq!(id, forestx_exec_server_protocol::RequestId::Integer(-1));
...
async fn exec_server_accepts_binary_websocket_json() -> anyhow::Result<()> {
    let mut server = exec_server().await?;
    let initialize_id = forestx_exec_server_protocol::RequestId::Integer(1);
    let initialize = JSONRPCMessage::Request(forestx_exec_server_protocol::JSONRPCRequest {

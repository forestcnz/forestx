...
async fn exec_server_replays_only_chatgpt_cloudflare_cookies() -> anyhow::Result<()> {
    ...
    let env = vec![
        (
            OsString::from("FORESTX_CA_CERTIFICATE"),
...
fn generate_tls_material() -> anyhow::Result<TlsMaterial> {
    ...
    ca_distinguished_name.push(DnType::CommonName, "Forestx affinity test CA");
    ...
    T: DeserializeOwned,
{
    let message = server
        .wait_for_event(|event| match event {
            ...
            | JSONRPCMessage::Error(forestx_exec_server_protocol::JSONRPCError { id, .. }) => {
                id == &request_id
            }
            _ => false,
        })

...
use forestx_exec_server::CAPABILITY_ROOTS_DISCOVER_METHOD;
use forestx_exec_server::CapabilityRootDiscovery;
use forestx_exec_server::CapabilityRootsDiscoverParams;
use forestx_exec_server::CapabilityRootsDiscoverResponse;
use forestx_exec_server::FileSystemSandboxContext;
use forestx_exec_server::InitializeParams;
use forestx_exec_server::InitializeResponse;
use forestx_exec_server_protocol::CapabilityRootDiscoverRequest;
...
async fn discovers_a_complete_capability_bundle_in_one_request() -> anyhow::Result<()> {
    ...
    write_file(
        &root.path().join(".forestx-plugin/plugin.json"),
        r#"{
  "name": "demo",
  ...
  "apps": "./config/apps.json"
    ...
    assert_eq!(
        ...
        root_uri.join(".forestx-plugin/plugin.json")?
    );

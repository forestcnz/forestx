...
const RELEASED_FORESTX_ENV_VAR: &str = "FORESTX_TEST_RELEASED_FORESTX";
const CURRENT_FORESTX_ENV_VAR: &str = "FORESTX_TEST_CURRENT_FORESTX";
const EXECUTOR_MARKER_ENV_VAR: &str = "FORESTX_EXECUTOR_VERSION_SKEW_MARKER";
...
fn version_skew_binaries() -> Result<Option<(PathBuf, PathBuf)>> {
    let Some(released) = std::env::var_os(RELEASED_FORESTX_ENV_VAR) else {
        return Ok(None);
    };
    let current = std::env::var_os(CURRENT_FORESTX_ENV_VAR).with_context(|| {
        format!("{CURRENT_FORESTX_ENV_VAR} must name the current Forestx binary")
    })?;
    let current = PathBuf::from(current);
    let released = PathBuf::from(released);
    anyhow::ensure!(
        current.is_file(),
        "current Forestx does not exist: {}",
        current.display()
    );
    anyhow::ensure!(
        released.is_file(),
        "released Forestx does not exist: {}",
        released.display()
    );
...
async fn assert_noise_version_skew(app_binary: &Path, executor_binary: &Path) -> Result<()> {
    ...
    let model = mock_model(forestx_home.path()).await?;
...
"#
    ...
    let mut executor = Command::new(executor_binary)
        ...
        .current_dir(forestx_home.path())
        .env("FORESTX_HOME", forestx_home.path())
        .env("FORESTX_API_KEY", REGISTRY_TOKEN)
    ...
    let mut app_server = Command::new(app_binary)
        ...
        .env(
            FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR,
            &registry_url,
        )
        .env(
            FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR,
            ENVIRONMENT_ID,
        )
        .env(FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR, REGISTRY_TOKEN)
        .env_remove(FORESTX_EXEC_SERVER_URL_ENV_VAR)
    ...
    assert_eq!(
        std::fs::read_to_string(forestx_home.path().join("version-skew-output.txt"))?,
        EXPECTED_OUTPUT
    );

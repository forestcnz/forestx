...
use forestx_exec_server::ExecBackend;
use forestx_exec_server::ExecOutputStream;
use forestx_exec_server::ExecParams;
use forestx_exec_server::ExecProcess;
use forestx_exec_server::ExecProcessEvent;
...
async fn remote_sandboxed_process_preserves_custom_arg0() -> Result<()> {
    if let Some(warning) = forestx_sandboxing::system_bwrap_warning(&PermissionProfile::read_only())
    {
    ...
    let session = context
        ...
        .start(ExecParams {
            ...
            argv: vec![
                "/bin/sh".to_string(),
                "-c".to_string(),
                "printf '%s' \"$0\"; if /bin/cat \"$FORESTX_TEST_DENIED_FILE\" >/dev/null 2>&1; then exit 42; fi"
                    .to_string(),
            ],
            cwd,
            env_policy: None,
            env: HashMap::from([
                ...
                (
                    "FORESTX_TEST_DENIED_FILE".to_string(),
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("forestx-linux-sandbox");

...
use forestx_test_binary_support::TestBinaryDispatchGuard;
use forestx_test_binary_support::TestBinaryDispatchMode;
use forestx_test_binary_support::configure_test_binary_dispatch;

pub(crate) mod exec_server;

pub(crate) const DELAYED_OUTPUT_AFTER_EXIT_PARENT_ARG: &str =
    "--forestx-test-delayed-output-after-exit-parent";
pub(crate) const SYSTEM_PROXY_REQUEST_URL_ENV: &str =
    "FORESTX_EXEC_SERVER_TEST_SYSTEM_PROXY_REQUEST_URL";
pub(crate) const SYSTEM_PROXY_URL_ENV: &str = "FORESTX_EXEC_SERVER_TEST_SYSTEM_PROXY_URL";

const FORESTX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";
const DELAYED_OUTPUT_AFTER_EXIT_CHILD_ARG: &str = "--forestx-test-delayed-output-after-exit-child";

#[ctor]
pub static TEST_BINARY_DISPATCH_GUARD: Option<TestBinaryDispatchGuard> = {
    let guard = configure_test_binary_dispatch("forestx-exec-server-tests", |exe_name, argv1| {
        if argv1 == Some(FORESTX_ARG0_EXEC_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(FORESTX_FS_HELPER_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
        if argv1 == Some(FORESTX_WINDOWS_SANDBOX_ARG1) {
            return TestBinaryDispatchMode::DispatchArg0Only;
        }
...
pub(crate) fn current_test_binary_helper_paths() -> anyhow::Result<(PathBuf, Option<PathBuf>)> {
    ...
    let forestx_linux_sandbox_exe = if cfg!(target_os = "linux") {
        TEST_BINARY_DISPATCH_GUARD
            .as_ref()
            .and_then(|guard| guard.paths().forestx_linux_sandbox_exe.clone())
            .or_else(|| Some(current_exe.clone()))
    } else {
        None
    };
    Ok((current_exe, forestx_linux_sandbox_exe))
}
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let http_client_factory = match (
        ...
        (Ok(request_url), Ok(proxy_url)) => {
            forestx_http_client::cache_system_proxy_route_for_test(&request_url, proxy_url);
    ...
    let exit_code = match runtime.block_on(forestx_exec_server::run_main_with_telemetry(
        &listen_url,
        runtime_paths,

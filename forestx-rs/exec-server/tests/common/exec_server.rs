...
use forestx_exec_server_protocol::JSONRPCNotification;
use forestx_exec_server_protocol::JSONRPCRequest;
...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestForestxHelperPaths,
    child: Child,
...
pub(crate) struct TestForestxHelperPaths {
    pub(crate) forestx_exe: PathBuf,
    pub(crate) forestx_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_forestx_helper_paths() -> anyhow::Result<TestForestxHelperPaths> {
    let (helper_binary, forestx_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestForestxHelperPaths {
        forestx_exe: helper_binary,
...
{
    let helper_paths = test_forestx_helper_paths()?;
    let forestx_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.forestx_exe);
    ...
    child.env("FORESTX_HOME", forestx_home.path());
    child.envs(env);

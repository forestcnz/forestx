...
use forestx_network_proxy::NetworkPolicyDecider;
use forestx_network_proxy::NetworkProxy;
use forestx_network_proxy::NetworkProxyHandle;
use forestx_network_proxy::NetworkProxyState;
use forestx_network_proxy::RemoteNetworkProxyLaunchConfig;
use forestx_network_proxy::is_managed_mitm_ca_trust_bundle_path;
#[cfg(target_os = "windows")]
use forestx_network_proxy::strip_managed_proxy_env;
...
use forestx_sandboxing::WindowsSandboxProxySettingsMode;
use forestx_sandboxing::WindowsSandboxSpawnRequest;
...
#[cfg(unix)]
use crate::FORESTX_ARG0_EXEC_HELPER_ARG1;
...
pub(crate) async fn prepare_exec_request(
    ...
    let sandbox_helper_paths = params
        .arg0
        .iter()
        .map(|_| runtime_paths.forestx_self_exe.clone())
        .collect::<Vec<_>>();
    ...
    let sandbox_helper_paths = {
        ...
        if !sandbox_helper_paths.contains(&runtime_paths.forestx_self_exe) {
            sandbox_helper_paths.push(runtime_paths.forestx_self_exe.clone());
        }
        sandbox_helper_paths.extend(runtime_paths.forestx_linux_sandbox_exe.iter().cloned());
    ...
    let (program, args) = params.arg0.as_ref().map_or_else(
        ...
        |arg0| {
            ...
            helper_args.push(FORESTX_ARG0_EXEC_HELPER_ARG1.to_string());
    ...
    let transform_request = SandboxDirectSpawnTransformRequest {
        ...
        transform: SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd,
            forestx_linux_sandbox_exe: runtime_paths.forestx_linux_sandbox_exe.as_deref(),

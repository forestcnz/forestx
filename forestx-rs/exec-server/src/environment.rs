...
use crate::environment_toml::environment_provider_from_forestx_home;
...
pub const FORESTX_EXEC_SERVER_URL_ENV_VAR: &str = "FORESTX_EXEC_SERVER_URL";
pub const FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR: &str =
    "FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL";
pub const FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR: &str =
    "FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID";
pub const FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR: &str =
    "FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN";
pub const FORESTX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR: &str =
    "FORESTX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID";
...
/// Owns the execution/filesystem environments available to the Forestx runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `FORESTX_EXEC_SERVER_URL` behavior
/// while configured construction accepts a provider-supplied snapshot.
///
/// Setting `FORESTX_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    ///
    /// If `FORESTX_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `FORESTX_EXEC_SERVER_URL` behavior.
    pub async fn prepare_from_forestx_home(
        forestx_home: impl AsRef<std::path::Path>,
        ...
        let source = if let Some(config) = noise_environment_config_from_env()? {
            ...
            let provider = environment_provider_from_forestx_home(forestx_home.as_ref())?;
            PreparedEnvironmentSource::Snapshot(provider.snapshot().await?)
        };
        Ok(PreparedEnvironmentManager { source })
    }

    /// Builds a manager from `FORESTX_HOME` with an explicit outbound HTTP policy.
    pub async fn from_forestx_home(
        forestx_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        http_client_factory: HttpClientFactory,
    ) -> Result<Self, ExecServerError> {
        Self::prepare_from_forestx_home(forestx_home)
            .await?
...
-> Result<Option<NoiseRendezvousEnvironmentConfig>, ExecServerError> {
    noise_environment_config_from_values(
        optional_environment_value(FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR),
        optional_environment_value(FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR),
        optional_environment_value(FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR),
        optional_environment_value(FORESTX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR),
    )
}
        ...
        match (registry_url, environment_id, auth_token) {
            ...
            _ => {
                return Err(ExecServerError::EnvironmentRegistryConfig(format!(
                    "Noise environment requires {FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR}, \
{FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR}, and \
{FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR}"
                )));
            }
        };
...
impl Environment {
    ...
    /// Builds an environment from the raw `FORESTX_EXEC_SERVER_URL` value and
...
mod tests {
        ...
        let environment = Environment::remote_with_transport(
            ExecServerTransportParams::StdioCommand {
                command: StdioExecServerCommand {
                    program: "forestx-missing-exec-server-for-test".to_string(),
                    args: Vec::new(),
                    env: HashMap::new(),
                    cwd: None,
                },
    ...
    async fn local_environment_passes_runtime_paths_to_exec_backend() {
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile_with_cwd(
            forestx_protocol::models::PermissionProfile::workspace_write(),
            sandbox_cwd.clone(),
        );
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = forestx_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            forestx_protocol::models::PermissionProfile::from_runtime_permissions(
                &forestx_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                forestx_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );

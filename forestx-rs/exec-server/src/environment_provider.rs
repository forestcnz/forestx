...
use crate::environment::FORESTX_EXEC_SERVER_URL_ENV_VAR;
use crate::environment::LOCAL_ENVIRONMENT_ID;
use crate::environment::REMOTE_ENVIRONMENT_ID;

/// Lists the remote environment transports available to Forestx.
///
...
/// Default provider backed by `FORESTX_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `FORESTX_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `FORESTX_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(FORESTX_EXEC_SERVER_URL_ENV_VAR).ok())
    }

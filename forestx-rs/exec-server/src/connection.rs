...
fn terminate_process_tree(child_process: &mut Child, process_group_id: Option<u32>) {
    ...
    #[cfg(unix)]
    if let Err(err) = forestx_utils_pty::process_group::terminate_process_group(process_group_id) {
...
fn kill_process_tree(child_process: &mut Child, process_group_id: Option<u32>) {
    ...
    #[cfg(unix)]
    if let Err(err) = forestx_utils_pty::process_group::kill_process_group(process_group_id) {

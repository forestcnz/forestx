...
const ENVIRONMENTS_TOML_FILE: &str = "environments.jsonc";
...
pub(crate) fn environment_provider_from_forestx_home(
    forestx_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = forestx_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "forestx exec-server --listen stdio".to_string(),
                    ]),
                    env: Some(HashMap::from([(
                        "FORESTX_LOG".to_string(),
                        "debug".to_string(),
                    )])),
                    ..Default::default()
                },
        ...
        let cases = [
            ...
            (
                EnvironmentToml {
                    id: "devbox".to_string(),
                    url: Some("ws://127.0.0.1:8765".to_string()),
                    program: Some("forestx".to_string()),
                    ..Default::default()
                },
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = forestx_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"{
  "default": "ssh-dev",
  ...
  "environments": [
    ...
    },
    {
      "id": "ssh-dev",
      "program": "ssh",
      "args": ["dev", "forestx exec-server --listen stdio"],
      "cwd": "/tmp",
      "env": { "FORESTX_LOG": "debug" }
    }
  ]
}"#,
        )
    ...
    fn load_environments_toml_rejects_unknown_fields() {
        ...
        let cases = [
            ...
            (
                r#"{
  "environments": [
    { "id": "devbox", "url": "ws://127.0.0.1:4512", "unknown": true }
  ]
}"#,
                "unknown field `unknown`",
            ),
    ...
    #[tokio::test]
    async fn environment_provider_from_forestx_home_uses_present_environments_file() {
        let forestx_home = tempdir().expect("tempdir");
        std::fs::write(
            forestx_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"{
  "default": "none",
  "include_local": false
}"#,
        ...
        let provider = environment_provider_from_forestx_home(forestx_home.path())
    ...
    #[tokio::test]
    async fn environment_provider_from_forestx_home_falls_back_when_file_is_missing() {
        let forestx_home = tempdir().expect("tempdir");

...
use forestx_exec_server_protocol::JSONRPCErrorError;
...
use forestx_sandboxing::SandboxDirectSpawnTransformRequest;
use forestx_sandboxing::SandboxExecRequest;
...
use crate::fs_helper::FORESTX_FS_HELPER_ARG1;
...
impl FileSystemSandboxRunner {
    ...
    fn sandbox_exec_request(
        ...
        let helper = &self.runtime_paths.forestx_self_exe;
        ...
        let command = SandboxCommand {
            ...
            args: vec![FORESTX_FS_HELPER_ARG1.to_string()],
            cwd: cwd.uri.clone(),
        ...
        sandbox_manager
            .transform_for_direct_spawn(SandboxDirectSpawnTransformRequest {
                ...
                transform: SandboxTransformRequest {
                    ...
                    forestx_linux_sandbox_exe: self
                        .runtime_paths
...
fn helper_read_roots(runtime_paths: &ExecServerRuntimePaths) -> Vec<AbsolutePathBuf> {
    ...
    for path in std::iter::once(runtime_paths.forestx_self_exe.as_path())
        .chain(runtime_paths.forestx_linux_sandbox_exe.as_deref())
    {
...
mod tests {
    ...
    fn helper_permissions_preserve_existing_writes() {
        let forestx_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(forestx_self_exe, /*forestx_linux_sandbox_exe*/ None)
    ...
    fn helper_env_preserves_path_for_system_bwrap_discovery_without_leaking_secrets() {
        ...
        assert_eq!(
            ...
            HashMap::from([
                ...
                ("TMPDIR".to_string(), "/tmp/forestx".to_string()),
    ...
    fn sandbox_exec_request_carries_helper_env() {
        ...
        let forestx_self_exe = std::env::current_exe().expect("current exe");
        let runtime_paths =
            ExecServerRuntimePaths::new(forestx_self_exe.clone(), Some(forestx_self_exe))
                .expect("runtime paths");
    ...
    fn sandbox_cwd_rejects_cwd_dependent_profile_without_context_cwd() {
        ...
        let sandbox_context =
            forestx_file_system::FileSystemSandboxContext::from_permission_profile(
    ...
    fn helper_permissions_include_linux_sandbox_alias_parent() {
        ...
        let forestx_self_exe = root.path().join("bin").join("forestx");
        let forestx_linux_sandbox_exe = root.path().join("aliases").join("forestx-linux-sandbox");
        let runtime_paths =
            ExecServerRuntimePaths::new(forestx_self_exe, Some(forestx_linux_sandbox_exe))
                .expect("runtime paths");
        let cwd = AbsolutePathBuf::from_absolute_path(std::env::temp_dir().as_path())
            .expect("absolute cwd");
        let mut policy = restricted_policy(Vec::new());
        let forestx_parent = AbsolutePathBuf::from_absolute_path(root.path().join("bin"))
            .expect("absolute forestx parent");
        ...
        assert!(policy.can_read_path_with_cwd(forestx_parent.as_path(), cwd.as_path()));
    ...
    fn sandbox_context_with_cwd(
        ...
        forestx_file_system::FileSystemSandboxContext::from_permission_profile_with_cwd(

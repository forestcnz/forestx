...
`forestx-exec-server` is the library backing `forestx exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`forestx-utils-pty`.

It provides:

- a CLI entrypoint: `forestx exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `forestx` binary owns hidden helper dispatch for sandboxed
filesystem operations and `forestx-linux-sandbox`.

## Transport

The server speaks the exec-specific `forestx-exec-server-protocol` message
...
must support it.
It uses the standard Forestx ChatGPT sign-in state; run `forestx login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `FORESTX_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; Forestx then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `FORESTX_API_KEY`;
Forestx sends it as a bearer token on the registration request. For example:

```sh
FORESTX_API_KEY="$OPENAI_API_KEY" \
forestx exec-server \
  --remote ... \
...
`forestx.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/forestx.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `forestx-utils-pty`.
...
hidden helper process launched from the top-level `forestx` executable and
...
`HttpClientFactory` to `run_main()`. The top-level `forestx exec-server` command
builds these paths from the `forestx` arg0 dispatch state and resolves its HTTP
client factory from the effective Forestx configuration.
...
provider from Forestx auth state before starting remote mode.

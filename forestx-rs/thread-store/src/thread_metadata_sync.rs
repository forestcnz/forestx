...
use forestx_protocol::protocol::user_message_preview;
...
impl ThreadMetadataSync {
    ...
    pub(crate) fn observe_appended_items(
        ...
        let affects_metadata = items
            .iter()
            .any(forestx_state::rollout_item_affects_thread_metadata);
...
mod tests {
    ...
    fn thread_settings_applied_updates_live_metadata() {
        ...
        assert_eq!(update.patch.model.as_deref(), Some("gpt-5.2-forestx"));
        assert_eq!(
    ...
    fn resume_preserves_existing_display_metadata_and_initializes_missing_title() {
        let thread_id = ThreadId::new();
        let mut metadata = forestx_state::ThreadMetadataBuilder::new(
            thread_id,
            Default::default(),
            Utc::now(),
            SessionSource::Exec,

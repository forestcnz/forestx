...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl forestx_rollout::RolloutConfigView) -> Self {
        Self {
...
impl LocalThreadStore {
    ...
    pub fn new(config: LocalThreadStoreConfig, state_db: Option<StateDbHandle>) -> Self {
        let writer_lock_coordinator = Arc::new(WriterLockCoordinator::new(&config.forestx_home));
        Self {
            config,
    ...
    async fn thread_history_db(&self) -> ThreadStoreResult<&sqlx::SqlitePool> {
        ...
        self.thread_history_db
            .get_or_try_init(|| async {
                forestx_state::open_thread_history_db(&self.config.sqlite).await
            })
            .await
...
mod tests {
    ...
    async fn read_thread_uses_live_writer_rollout_path_for_external_resume() {
        ...
        assert!(error.to_string().contains("must be in Forestx home"));
    }

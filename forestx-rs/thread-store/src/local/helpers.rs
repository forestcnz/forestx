...
use forestx_rollout::ThreadItem;
use forestx_rollout::find_thread_names_by_ids;
...
pub(super) fn rollout_path_is_archived(forestx_home: &Path, path: &Path) -> bool {
    path.starts_with(forestx_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = forestx_rollout::plain_rollout_path(item.path.as_path());
...
pub(super) async fn resolve_thread_names(
    ...
    if let Ok(legacy_names) =
        find_thread_names_by_ids(store.config.forestx_home.as_path(), &legacy_thread_ids).await
    {

...
use forestx_rollout::RolloutRecorderParams;
...
/// Separate Forestx and SQLite homes must work together across startup backfill,
...
async fn split_homes_support_backfill_listing_and_paginated_history() {
    ...
    let runtime = forestx_rollout::state_db::try_init(&rollout_config)
        .await
        .expect("backfill state from Forestx home");
    assert!(
        runtime
    ...
    for sqlite_path in [&state_db_path, &thread_history_db_path] {
        ...
        assert!(
            !forestx_home.join(filename).exists(),
            "SQLite database should not be created under Forestx home"
        );
    }
}
...
async fn paginated_live_append_materializes_turn_items_and_state() {
    ...
    let pool = forestx_state::open_thread_history_db(&forestx_state::SqliteConfig::new_for_testing(
        home.path().abs(),
    ))
    .await
...
async fn catch_up_rejects_invalid_complete_suffixes_without_advancing_state() {
    ...
    for (name, suffix) in cases {
        ...
        let pool = forestx_state::open_thread_history_db(
...
async fn projection_store(forestx_home: &Path) -> LocalThreadStore {
    let config = test_config(forestx_home);

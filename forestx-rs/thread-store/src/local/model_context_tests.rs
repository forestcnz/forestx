...
async fn loads_latest_checkpoint_with_required_turn_metadata() {
    ...
    let thread_id = forestx_protocol::ThreadId::from_string(&uuid.to_string()).expect("thread id");
...
async fn fork_context_excludes_items_after_frozen_cutoff() {
    ...
    let session_meta = forestx_rollout::read_session_meta_line(path.as_path())
        .await
...
async fn replays_nested_archived_lineage_from_frozen_prefix() {
    ...
    let child_meta = forestx_rollout::read_session_meta_line(child_path.as_path())
        .await
...
async fn assert_reverse_scan_matches_full_history(home: &Path, path: &Path) {
    let session_meta = forestx_rollout::read_session_meta_line(path)
        .await
...
fn completed_user_message(turn_id: &str, message: &str) -> RolloutItem {
    RolloutItem::EventMsg(EventMsg::ItemCompleted(ItemCompletedEvent {
        thread_id: forestx_protocol::ThreadId::from_string("00000000-0000-0000-0000-000000000000")

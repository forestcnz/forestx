...
use forestx_protocol::protocol::GitInfo;
...
use forestx_rollout::ARCHIVED_SESSIONS_SUBDIR;
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = forestx_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
async fn metadata_for_missing_sqlite_row(
    ...
    patch: &ThreadMetadataPatch,
) -> ThreadStoreResult<forestx_state::ThreadMetadata> {
...
async fn canonical_history_mode(
    ...
    let session_meta = match read_session_meta_line(rollout_path).await {
        ...
        Err(err) => {
            if forestx_rollout::existing_rollout_path(rollout_path)
                .await
                .is_none()
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    forestx_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(forestx_git_utils::GitSha::new),
        branch: branch.clone(),
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.forestx_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = forestx_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn section_moves_persist_in_sqlite_without_changing_the_rollout() {
        ...
        store
            .move_thread_to_section(MoveThreadToSectionParams {
                thread_id,
                section: Some(forestx_state::PINNED_THREAD_SECTION_ID.to_string()),
        ...
        assert_eq!(
            pinned.section,
            Some(forestx_state::ThreadSection {
        ...
        let pinned_page = store
            .list_threads(ListThreadsParams {
                ...
                cwd_filters: None,
                section: Some(Some(forestx_state::PINNED_THREAD_SECTION_ID.to_string())),
                archived: false,
                search_term: None,
    ...
    async fn paginated_name_updates_use_sqlite_without_rollout_writes() {
        ...
        assert_eq!(
            forestx_rollout::find_thread_name_by_id(home.path(), &thread_id)
                .await
                .expect("find thread name")
                .as_deref(),
    ...
    async fn update_thread_metadata_sets_git_info() {
            ...
            .update_thread_metadata(UpdateThreadMetadataParams {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some("https://github.com/openai/forestx".to_string())),
                    }),
                    ..Default::default()
                },
                include_archived: false,
        ...
        assert_eq!(
            git_info.repository_url.as_deref(),
            Some("https://github.com/openai/forestx")
        );
    }
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd = forestx_utils_path::normalize_for_path_comparison(workspace.as_path())

...
use forestx_rollout::RolloutReferenceIndex;
use forestx_rollout::SESSIONS_SUBDIR;
use forestx_rollout::find_archived_thread_path_by_id_str;
use forestx_rollout::find_thread_path_by_id_str;
use forestx_rollout::remove_thread_name_entries;
...
async fn scan_reference_index(
    ...
    RolloutReferenceIndex::scan(store.config.forestx_home.as_path())
        .await
...
fn referenced_thread_error(thread_id: forestx_protocol::ThreadId) -> ThreadStoreError {
...
async fn delete_thread_after_reference_check(
    ...
    remove_thread_name_entries(store.config.forestx_home.as_path(), thread_id)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn delete_rollout_file(
    ...
    let plain_path = forestx_rollout::plain_rollout_path(rollout_path);
...
fn delete_rollout_path(
    ...
    let canonical_rollout_path = scoped_rollout_path(
        store.config.forestx_home.join(SESSIONS_SUBDIR),
        rollout_path,
        "sessions",
    )
    .or_else(|_| {
        scoped_rollout_path(
            store.config.forestx_home.join(ARCHIVED_SESSIONS_SUBDIR),
            rollout_path,
            "archived sessions",
        )
    })
...
mod tests {
    ...
    async fn delete_thread_without_state_db_preserves_materialized_thread_history() {
        ...
        .expect("session file");
        let pool = forestx_state::open_thread_history_db(&config.sqlite)
            .await

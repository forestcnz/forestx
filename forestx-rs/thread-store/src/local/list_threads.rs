...
use chrono::DateTime;
use chrono::Utc;
use forestx_rollout::RolloutConfig;
use forestx_rollout::RolloutRecorder;
use forestx_rollout::parse_cursor;
use forestx_state::ThreadFilterOptions;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => forestx_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => forestx_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => forestx_rollout::ThreadSortKey::RecencyAt,
    ...
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => forestx_rollout::SortDirection::Asc,
        SortDirection::Desc => forestx_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        forestx_home: store.config.forestx_home.clone(),
        sqlite: store.config.sqlite.clone(),
        cwd: store.config.forestx_home.clone(),
...
async fn list_section_threads(
    ...
    let anchor = params
        .cursor
        .as_deref()
        .map(|cursor| -> ThreadStoreResult<forestx_state::Anchor> {
            ...
            let thread_id = forestx_protocol::ThreadId::from_string(thread_id).map_err(|_| {
                ThreadStoreError::InvalidRequest {
                    message: format!("invalid cursor: {cursor}"),
                }
            })?;
            Ok(forestx_state::Anchor {
                ts: timestamp,
                id: Some(thread_id),
            })
        })
    ...
    let normalized_cwd_filters = params.cwd_filters.as_ref().map(|filters| {
        filters
            .iter()
            .map(|cwd| forestx_rollout::state_db::normalize_cwd_for_state_db(cwd))
            .collect::<Vec<_>>()
    });
    let filters = ThreadFilterOptions {
        ...
        sort_key: forestx_state::SortKey::SectionPosition,
        sort_direction: match params.sort_direction {
            SortDirection::Asc => forestx_state::SortDirection::Asc,
            SortDirection::Desc => forestx_state::SortDirection::Desc,
        },
    ...
    let page = match params.relation_filter {
        Some(ThreadRelationFilter::DirectChildrenOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    forestx_state::ThreadRelationFilter::DirectChildrenOf(thread_id),
                    filters,
                )
                .await
        }
        Some(ThreadRelationFilter::DescendantsOf(thread_id)) => {
            state_db
                .list_threads_by_relation(
                    params.page_size,
                    forestx_state::ThreadRelationFilter::DescendantsOf(thread_id),
                    filters,
                )
                .await
        }
...
pub(super) async fn list_rollout_threads(
    state_db: Option<forestx_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&forestx_rollout::Cursor>,
    sort_key: forestx_rollout::ThreadSortKey,
    sort_direction: forestx_rollout::SortDirection,
) -> ThreadStoreResult<forestx_rollout::ThreadsPage> {
        ...
        let relation_filter = params
            ...
            .map(|relation_filter| match relation_filter {
                ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                    forestx_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
                }
                ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                    forestx_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
                }
            });
        let page = forestx_rollout::state_db::list_threads_db(
            state_db.as_deref(),
            &config.sqlite,
            params.page_size,
            cursor,
...
mod tests {
    ...
    use forestx_state::PINNED_THREAD_SECTION_ID;
    ...
    async fn section_listing_uses_sqlite_metadata_without_reading_rollouts() {
        let home = TempDir::new().expect("temp dir");
        let config = test_config(home.path());
        let state = forestx_state::StateRuntime::init(
            config.sqlite.clone(),
            config.default_model_provider_id.clone(),
        )
        .await
        ...
        for index in 0..3 {
            ...
            forestx_rollout::state_db::reconcile_rollout(
                Some(state.as_ref()),

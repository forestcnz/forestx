...
use forestx_rollout::find_archived_thread_path_by_id_str;
use forestx_rollout::find_thread_name_by_id;
use forestx_rollout::find_thread_path_by_id_str;
use forestx_rollout::read_session_meta_line;
use forestx_rollout::read_thread_item_from_rollout;
        ...
        && (params.include_archived
            || (metadata.archived_at.is_none()
                && !rollout_path_is_archived(
                    store.config.forestx_home.as_path(),
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if forestx_rollout::existing_rollout_path(path).await.is_none() {
        return false;
    }
...
async fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.forestx_home.join(rollout_path)
    } else {
        rollout_path
    };
    ...
    let Some(path) = forestx_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && forestx_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
        && (include_archived || !rollout_path_is_archived(store.config.forestx_home.as_path(), &path))
    {
        return Ok(Some(path));
    }
...
async fn read_thread_from_rollout_path(
    ...
    let archived = rollout_path_is_archived(store.config.forestx_home.as_path(), path.as_path());
    ...
    thread.rollout_path = Some(forestx_rollout::plain_rollout_path(path.as_path()));
    ...
    if thread.history_mode == ThreadHistoryMode::Legacy
        && let Ok(Some(name)) =
            find_thread_name_by_id(store.config.forestx_home.as_path(), &thread.thread_id).await
        && !name.trim().is_empty()
    {
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<forestx_protocol::protocol::RolloutItem>> {
...
pub(super) async fn stored_thread_from_sqlite_metadata(
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if forestx_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
...
pub(super) fn stored_thread_from_state_metadata(
    store: &LocalThreadStore,
    metadata: ThreadMetadata,
    parent_thread_id: Option<forestx_protocol::ThreadId>,
) -> StoredThread {
    ...
    let rollout_path = forestx_rollout::plain_rollout_path(metadata.rollout_path.as_path());
    ...
    match history_mode {
        ...
        ThreadHistoryMode::Legacy => {
            if let Some(title) = distinct_thread_metadata_title(metadata) {
                ...
                find_thread_name_by_id(store.config.forestx_home.as_path(), &metadata.id)
                    .await
                    .ok()
                    .flatten()
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = forestx_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under forestx home")
            .to_path_buf();
    ...
    async fn read_thread_by_rollout_path_preserves_legacy_name_and_sqlite_git_info() {
        ...
        let active_path =
            write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        let runtime = forestx_state::StateRuntime::init(
            config.sqlite.clone(),
        ...
        metadata.section = Some(forestx_state::ThreadSection {
            id: forestx_state::PINNED_THREAD_SECTION_ID.to_string(),
            name: forestx_state::PINNED_THREAD_SECTION_NAME.to_string(),
        });
        ...
        forestx_rollout::append_thread_name(home.path(), thread_id, "Latest index name")
            .await
        ...
        assert_eq!(
            ...
            Some(forestx_state::PINNED_THREAD_SECTION_ID)
        );
    ...
    async fn read_paginated_thread_prefers_sqlite_display_metadata() {
        ...
        .expect("session file");
        forestx_rollout::append_rollout_item_to_path(
            &rollout_path,
        ...
        runtime
            ...
            .await
            .expect("state db upsert should succeed");
        forestx_rollout::append_thread_name(home.path(), thread_id, "Stale index name")
            .await
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        forestx_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
            .expect("append legacy thread name");

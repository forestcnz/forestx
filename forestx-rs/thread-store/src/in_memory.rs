...
use forestx_rollout::persisted_rollout_items;
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        section: state
            ...
            .get(&thread_id)
            .cloned()
            .map(|id| forestx_state::ThreadSection {
                name: if id == forestx_state::PINNED_THREAD_SECTION_ID {
                    forestx_state::PINNED_THREAD_SECTION_NAME.to_string()
                } else {
                    id.clone()
                },
                id,
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<forestx_protocol::protocol::GitInfo> {
    ...
    Some(forestx_protocol::protocol::GitInfo {

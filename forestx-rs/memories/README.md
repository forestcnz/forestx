...
Runtime orchestration for Phase 1 and Phase 2 still lives in `forestx-core` under
`forestx-rs/core/src/memories/`.

## Crates

- `forestx-rs/memories/read` (`forestx-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `forestx-rs/memories/write` (`forestx-memories-write`) owns the write path:
...
- In `forestx`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.forestx/memories/.git` by `forestx-git-utils`

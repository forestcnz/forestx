...
use forestx_state::Stage1Output;
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            forestx_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
                context.counter(
            ...
            forestx_state::Phase2JobClaimOutcome::SkippedRetryUnavailable => {
                return Err("skipped_retry_unavailable");
            }
            forestx_state::Phase2JobClaimOutcome::SkippedCooldown => {
                return Err("skipped_cooldown");
            }
            forestx_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[forestx_state::Stage1Output],
        reason: &'static str,
    ) -> bool {
        context.counter(MEMORY_PHASE_TWO_JOBS, /*inc*/ 1, &[("status", reason)]);
        db.memories()
...
mod agent {
    ...
    pub(super) fn get_config(
        ...
        // Preserve the parent's explicit choice to skip Forestx-managed sandboxing.
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<forestx_state::Stage1Output>,
        memory_root: forestx_utils_absolute_path::AbsolutePathBuf,
        agent: SpawnedConsolidationAgent,
        phase_two_e2e_timer: Option<forestx_otel::Timer>,
    ) {
        let Some(db) = context.state_db() else {
            return;
        };
    ...
    async fn loop_agent(
        ...
        token: String,
        thread_id: ThreadId,
        thread: &forestx_core::ForestxThread,
    ) -> AgentStatus {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[forestx_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()

...
async fn rate_limits_check(auth_manager: &AuthManager, config: &Config) -> Option<bool> {
    ...
    let snapshot = snapshots
        .iter()
        .find(|s| s.limit_id.as_deref() == Some(crate::guard_limits::FORESTX_LIMIT_ID))
    ...
    if !allowed {
        info!(
            ...
            "skipping memories startup because Forestx rate limits are below the configured threshold"
        );
    }

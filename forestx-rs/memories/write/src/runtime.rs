...
use forestx_core::content_items_to_text;
use forestx_core::detached_memory_responses_metadata;
...
pub(crate) struct SpawnedConsolidationAgent {
    ...
    pub(crate) thread: Arc<ForestxThread>,
}
...
impl StageOneRequestContext {
    pub(crate) fn start_timer(&self, name: &str) -> Option<forestx_otel::Timer> {
...
fn build_session_telemetry(
    ...
    let auth_mode = auth.map(ForestxAuth::auth_mode).map(TelemetryAuthMode::from);
    let account_id = auth.and_then(ForestxAuth::get_account_id);
    let account_email = auth.and_then(ForestxAuth::get_account_email);
        ...
        while let Some(message) = stream.next().await.transpose()? {
            match message {
                ...
                ResponseEvent::OutputItemDone(item) => {
                    if result.is_empty()
                        && let forestx_protocol::models::ResponseItem::Message { content, .. } = item

...
use forestx_git_utils::diff_since_latest_init;
use forestx_git_utils::reset_git_repository;
use forestx_login::AuthManager;
use forestx_login::ForestxAuth;
use forestx_model_provider::ModelProvider;
use forestx_model_provider::ModelProviderFuture;
use forestx_model_provider::ProviderAccountResult;
...
use forestx_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_forestx(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by Forestx before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test = build_test_forestx_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_forestx_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.forestx_home);
...
async fn build_test_forestx(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestForestx> {
    build_test_forestx_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_forestx_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestForestx> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<forestx_state::StateRuntime>> {
    let db = forestx_state::StateRuntime::init(
        forestx_state::SqliteConfig::new_for_testing(home.path().abs()),
        "test-provider".into(),
    )
    .await?;
...
async fn trigger_memories_startup(test: &TestForestx) {
    let config_snapshot = test.forestx.config_snapshot().await;
...
) -> (Arc<MemoryStartupContext>, Arc<forestx_core::config::Config>) {
    let config_snapshot = test.forestx.config_snapshot().await;
    let mut config = test.config.clone();
    config
        .features
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> forestx_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(forestx_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &forestx_state::StateRuntime,
    forestx_home: &Path,
    ...
    let mut metadata_builder = forestx_state::ThreadMetadataBuilder::new(
        thread_id,
        forestx_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = forestx_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = forestx_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<forestx_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.forestx.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        forestx_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_forestx(test: &TestForestx) -> anyhow::Result<()> {
    test.forestx.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.forestx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}

...
use forestx_rollout::INTERACTIVE_SESSION_SOURCES;
use forestx_rollout::should_persist_response_item_for_memories;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<forestx_state::Stage1JobClaim>> {
    ...
    match state_db
        ...
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            forestx_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<forestx_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: forestx_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> forestx_protocol::error::Result<String> {
        let filtered = items
            .iter()
        ...
        let serialized = serde_json::to_string(&filtered).map_err(|err| {
            ForestxErr::InvalidRequest(format!("failed to serialize rollout memory: {err}"))
        })?;
            ...
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: forestx_protocol::models::FunctionCallOutputPayload {
                        body: forestx_protocol::models::FunctionCallOutputBody::Text(

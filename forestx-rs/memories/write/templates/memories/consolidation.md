...
The folder `{{ memory_root }}/` is a git repository managed by Forestx. Read
`{{ phase2_workspace_diff_file }}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by Forestx for
...
- Typical workflows and tools (how they like to work, how they use Forestx/agents, preferred formats)

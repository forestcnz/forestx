...
This repository uses Bazel to build the Rust workspace under `forestx-rs`.
...
- `rules_rs` imports third-party crates from `forestx-rs/Cargo.toml` and
  `forestx-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `forestx_rust_crate`, which wraps `rust_library`,
...
- Each crate in `forestx-rs/*/BUILD.bazel` typically uses `forestx_rust_crate` and
...
Forestx uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/forestx` | Yes | Yes |
| `workflow_dispatch` in `openai/forestx` | Yes | Yes |
| Same-repository pull request in `openai/forestx` | Yes | Yes |
| Fork pull request into `openai/forestx` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/forestx \
  ./.github/scripts/run_bazel_with_buildbuddy.py \
  build --config=ci-linux //forestx-rs/cli:forestx
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `openai/forestx`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `forestx_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `forestx_rust_crate` to configure it.

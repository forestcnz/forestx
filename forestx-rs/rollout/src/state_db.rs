...
pub use forestx_state::LogEntry;
use forestx_state::SqliteConfig;
...
pub type StateDbHandle = Arc<forestx_state::StateRuntime>;
...
pub async fn init(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    match try_init_with_roots(config.forestx_home, config.sqlite, config.model_provider_id).await {
...
pub async fn try_init(config: &impl RolloutConfigView) -> anyhow::Result<StateDbHandle> {
    let config = RolloutConfig::from_view(config);
    try_init_with_roots(config.forestx_home, config.sqlite, config.model_provider_id).await
}
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        forestx_state::StateRuntime::init(sqlite.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    forestx_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == forestx_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, forestx_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match forestx_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> forestx_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == forestx_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<forestx_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&forestx_state::StateRuntime>,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => forestx_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => forestx_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => forestx_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<forestx_state::ThreadRelationFilter>,
    archived: bool,
    section: Option<Option<&str>>,
    search_term: Option<&str>,
) -> Option<forestx_state::ThreadsPage> {
    let ctx = context?;
    if ctx.sqlite() != sqlite {
        warn!(
            "state db SQLite home mismatch: expected {}, got {}",
    ...
    if let Some(relation_filter) = relation_filter {
        let filters = forestx_state::ThreadFilterOptions {
    ...
    Some(forestx_state::ThreadsPage {
        items,

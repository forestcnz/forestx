...
pub async fn append_thread_name(
    ...
    append_session_index_entry(forestx_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(forestx_home);
...
fn session_index_path(forestx_home: &Path) -> PathBuf {
    forestx_home.join(SESSION_INDEX_FILE)
}

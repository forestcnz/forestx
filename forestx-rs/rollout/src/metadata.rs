...
use forestx_state::BackfillState;
use forestx_state::BackfillStats;
use forestx_state::BackfillStatus;
use forestx_state::DB_ERROR_METRIC;
use forestx_state::DB_METRIC_BACKFILL;
use forestx_state::DB_METRIC_BACKFILL_DURATION_MS;
use forestx_state::ExtractionOutcome;
use forestx_state::ThreadMetadataBuilder;
use forestx_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = forestx_otel::global();
    ...
    let sessions_root = forestx_home.join(SESSIONS_SUBDIR);
    let archived_root = forestx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(forestx_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(forestx_home: &Path, path: &Path) -> String {
    path.strip_prefix(forestx_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}

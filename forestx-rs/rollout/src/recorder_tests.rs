...
fn test_config(forestx_home: &Path) -> RolloutConfig {
    RolloutConfig {
        forestx_home: forestx_home.to_path_buf(),
        sqlite: forestx_state::SqliteConfig::new_for_testing(forestx_home.abs()),
...
async fn state_db_init_backfills_before_returning() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        forestx_state::BackfillStatus::Complete
    );
...
-> std::io::Result<()> {
    ...
    let valid_created_at = chrono::Utc
        ...
        .single()
        .expect("valid datetime");
    let mut valid_builder = forestx_state::ThreadMetadataBuilder::new(
        valid_thread_id,
        valid_path.clone(),
        valid_created_at,
        SessionSource::Cli,
...
fn fill_missing_thread_item_metadata_preserves_identity_and_prefers_state_git_fields() {
    ...
    let state_item = ThreadItem {
        ...
        section: Some(forestx_state::ThreadSection {

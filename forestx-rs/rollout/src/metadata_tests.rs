...
use forestx_state::BackfillStatus;
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let forestx_home = dir.path().to_path_buf();
    ...
    let runtime = forestx_state::StateRuntime::init(
        forestx_state::SqliteConfig::new_for_testing(forestx_home.as_path().abs()),
        "test-provider".to_string(),
    )
    .await
    .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(forestx_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), forestx_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let forestx_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = forestx_home.join(".");
    let rollout_path = write_rollout_in_sessions_with_cwd(
        forestx_home.as_path(),
        "2026-01-27T12-34-56",
        "2026-01-27T12:34:56Z",
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = forestx_home.join("sessions");

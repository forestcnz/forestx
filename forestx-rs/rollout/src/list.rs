...
use crate::state_db;
use forestx_file_search as file_search;
...
pub struct ThreadItem {
    ...
    pub section: Option<forestx_state::ThreadSection>,
...
impl From<forestx_state::Anchor> for Cursor {
    fn from(anchor: forestx_state::Anchor) -> Self {
        let ts = anchor
            .ts
            .timestamp_nanos_opt()
...
pub async fn get_threads(
    ...
    let root = forestx_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.forestx/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    subdir: &str,
    id_str: &str,
    state_db_ctx: Option<&forestx_state::StateRuntime>,
    ...
    let mut root = forestx_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(forestx_home, SESSIONS_SUBDIR, id_str, state_db_ctx).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(forestx_home, ARCHIVED_SESSIONS_SUBDIR, id_str, state_db_ctx)
        .await
}

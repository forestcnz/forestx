...
mod tests {
    ...
    async fn stage1_claim_skips_when_up_to_date() {
        let forestx_home = unique_temp_dir();
        let runtime = StateRuntime::init(
            crate::SqliteConfig::new_for_testing(forestx_home.as_path().abs()),
        ...
        let metadata = test_thread_metadata(&forestx_home, thread_id, forestx_home.join("a"));
        runtime
        ...
        let _ = tokio::fs::remove_dir_all(forestx_home).await;
    }
    ...
    async fn stage1_running_stale_can_be_stolen_but_fresh_running_is_skipped() {
        ...
        let owner_a = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let owner_b = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let cwd = forestx_home.join("workspace");
        runtime
            .upsert_thread(&test_thread_metadata(&forestx_home, thread_id, cwd))
            .await
            .expect("upsert thread");
    ...
    async fn claim_stage1_jobs_filters_by_age_idle_and_current_thread() {
        ...
        let mut current =
            test_thread_metadata(&forestx_home, current_thread_id, forestx_home.join("current"));
        ...
        let mut fresh =
            test_thread_metadata(&forestx_home, fresh_thread_id, forestx_home.join("fresh"));
        ...
        let mut just_under_idle = test_thread_metadata(
            ...
            forestx_home.join("just-under-idle"),
        );
        ...
        let mut eligible_idle = test_thread_metadata(
            ...
            forestx_home.join("eligible-idle"),
        );
        ...
        let mut old = test_thread_metadata(&forestx_home, old_thread_id, forestx_home.join("old"));
    ...
    async fn claim_stage1_jobs_bounds_state_scan_before_memory_probes() {
        ...
        let mut stale =
            test_thread_metadata(&forestx_home, stale_thread_id, forestx_home.join("stale"));
    ...
    async fn claim_stage1_jobs_skips_threads_without_enabled_memory() {
        ...
        let mut disabled =
            test_thread_metadata(&forestx_home, disabled_thread_id, forestx_home.join("disabled"));
        ...
        let mut enabled =
            test_thread_metadata(&forestx_home, enabled_thread_id, forestx_home.join("enabled"));
    ...
    async fn claim_stage1_jobs_enforces_global_running_cap() {
        ...
        for idx in 0..total_candidates {
            ...
            let mut metadata = test_thread_metadata(
                &forestx_home,
                thread_id,
                forestx_home.join(format!("thread-{idx}")),
            );
...
WHERE kind = ? AND job_key = ?
    ...
    async fn list_stage1_outputs_for_global_returns_latest_outputs() {
        ...
        runtime
            ...
            .expect("upsert thread a");
        let mut metadata_b =
            test_thread_metadata(&forestx_home, thread_id_b, forestx_home.join("workspace-b"));
        ...
        assert_eq!(outputs[0].cwd, forestx_home.join("workspace-b"));
        ...
        assert_eq!(outputs[1].cwd, forestx_home.join("workspace-a"));
    ...
    async fn list_stage1_outputs_for_global_skips_empty_payloads() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &forestx_home,
                thread_id_non_empty,
                forestx_home.join("workspace-non-empty"),
            ))
            .await
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &forestx_home,
                thread_id_empty,
                forestx_home.join("workspace-empty"),
            ))
            .await
...
VALUES (?, ?, ?, ?, ?)
        ...
        assert_eq!(outputs[0].cwd, forestx_home.join("workspace-non-empty"));
    ...
    async fn list_stage1_outputs_for_global_includes_paginated_and_skips_polluted_threads() {
        ...
        for (thread_id, workspace) in [
            ...
            let mut metadata =
                test_thread_metadata(&forestx_home, thread_id, forestx_home.join(workspace));
    ...
    async fn get_phase2_input_selection_returns_current_selected_rows() {
        ...
        assert_eq!(
            ...
            forestx_home.join(format!("rollout-{thread_id_c}.jsonl"))
        );
        ...
        for (thread_id, updated_at) in [(thread_id_enabled, 100), (thread_id_polluted, 101)] {
            runtime
                .upsert_thread(&test_thread_metadata(
                    &forestx_home,
                    thread_id,
                    forestx_home.join(thread_id.to_string()),
                ))
                .await
                .expect("upsert thread");

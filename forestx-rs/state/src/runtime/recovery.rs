//! Backup-and-rebuild support for Forestx runtime SQLite databases.
//!
//! Forestx keeps several independent runtime SQLite databases under one SQLite
...
/// Move one Forestx runtime SQLite database out of the way so that database can
    ...
    match tokio::fs::metadata(sqlite_home).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            ...
            Err(std::io::Error::other(format!(
                "no Forestx runtime database files were found to back up for {}",
                db_path.display()
            )))
        }
        Err(err) => Err(err),
...
async fn backup_sqlite_paths(
    ...
    if backups.is_empty() {
        ...
        return Err(std::io::Error::other(
            "no Forestx runtime database files were found to back up",
        ));
    }

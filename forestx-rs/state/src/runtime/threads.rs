...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .as_ref()
                .map(forestx_protocol::protocol::ThreadSource::as_str),
        )
...
mod tests {
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        ...
        let mut metadata = test_thread_metadata(&forestx_home, thread_id, forestx_home.clone());
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = forestx_home.join("first");
        let second_cwd = forestx_home.join("second");
        let other_cwd = forestx_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&forestx_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&forestx_home, thread_id, forestx_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(forestx_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:openai/forestx.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:openai/forestx.git")
        );
    }
    ...
    async fn upsert_thread_preserves_existing_git_fields_atomically() {
        ...
        metadata.git_branch = Some("sqlite-branch".to_string());
        metadata.git_origin_url = Some("git@example.com:openai/forestx.git".to_string());
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:openai/forestx.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&forestx_home, thread_id, forestx_home.clone());
        ...
        let mut fallback = test_thread_metadata(&forestx_home, thread_id, forestx_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&forestx_home, first_id, forestx_home.clone());
        first.updated_at = updated_at;
        first.recency_at = updated_at;
        let mut second = test_thread_metadata(&forestx_home, second_id, forestx_home.clone());
        ...
        let mut older = test_thread_metadata(&forestx_home, older_id, forestx_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            forestx_protocol::protocol::TokenCountEvent {
                info: Some(forestx_protocol::protocol::TokenUsageInfo {
                    total_token_usage: forestx_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: forestx_protocol::protocol::TokenUsage::default(),

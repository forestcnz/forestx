...
impl RuntimeDbSpec {
    fn path(self, forestx_home: &Path) -> PathBuf {
        forestx_home.join(self.filename)
    }
}
...
/// Resolved configuration shared by all Forestx SQLite connections.
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SqliteConfig {
    ...
    /// Open a writable Forestx SQLite database, creating it if necessary.
    ...
    /// Open an existing Forestx SQLite database without creating or modifying it.

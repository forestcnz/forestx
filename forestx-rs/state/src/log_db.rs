...
//!
//! # async fn example(state_db: std::sync::Arc<forestx_state::StateRuntime>) {
...
pub fn default_filter() -> Targets {
    Targets::new()
        ...
        .with_target("forestx_rmcp_client", LevelFilter::INFO)
        .with_target("forestx_otel.log_only", LevelFilter::OFF)
        .with_target("forestx_otel.trace_safe", LevelFilter::OFF)
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout Forestx environments.
...
mod tests {
    ...
    fn temp_forestx_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("forestx-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    async fn sqlite_feedback_logs_match_feedback_formatter_shape() {
        let forestx_home = temp_forestx_home();

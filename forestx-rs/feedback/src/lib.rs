...
/// Filename used for the redacted `forestx doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "forestx-doctor-report.json";
/// Filename used for the raw Forestx Apps MCP tools cache feedback attachment.
pub const FORESTX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "forestx-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const FORESTX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str = "forestx-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_forestx_api_key_present = tracing::field::debug(auth_env.forestx_api_key_env_present),
        auth_env_forestx_api_key_enabled = tracing::field::debug(auth_env.forestx_api_key_env_enabled),
...
#[derive(Clone)]
pub struct ForestxFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for ForestxFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl ForestxFeedback {
    pub fn new() -> Self {
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("forestx_api::responses_websocket_timing", LevelFilter::OFF)
                    .with_target("forestx_core::post_sampling_token_estimate", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `forestx-logs.log` and before path-backed rollout
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("forestx-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = ForestxFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_excludes_responses_websocket_timing_payloads() {
        let fb = ForestxFeedback::new();
        let _guard = tracing_subscriber::registry()
            .with(fb.logger_layer())
            .set_default();

        tracing::trace!(target: "forestx_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "forestx_feedback_test", "retained");
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("forestx-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = ForestxFeedback::new()
        ...
        let attachments_without_diagnostics = ForestxFeedback::new()
            .snapshot(/*session_id*/ None)
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("forestx-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("forestx-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = ForestxFeedback::new()
            .snapshot(/*session_id*/ None)

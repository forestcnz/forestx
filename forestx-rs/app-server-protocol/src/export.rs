...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("forestx_app_server_protocol.schemas.json"),
        &bundle,
    )?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("forestx_app_server_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    root.insert(
        "title".to_string(),
        Value::String("ForestxAppServerProtocol".into()),
    );
...
fn build_flat_v2_schema(bundle: &Value) -> Result<Value> {
    ...
    let title = root
        ...
        .unwrap_or("ForestxAppServerProtocol");
...
mod tests {
    ...
    fn schema_root() -> Result<PathBuf> {
        let typescript_index = forestx_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("forestx_schema_{}", Uuid::now_v7()));
    ...
    fn build_flat_v2_schema_keeps_shared_root_schemas_and_dependencies() -> Result<()> {
        let bundle = serde_json::json!({
            ...
            "title": "ForestxAppServerProtocol",
            "type": "object",
            "definitions": {
                "ClientRequest": {
                    "oneOf": [
        ...
        assert_eq!(
            flat_bundle["title"],
            serde_json::json!("ForestxAppServerProtocolV2")
        );
    ...
    fn experimental_type_fields_ts_filter_handles_interface_shape() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("forestx_ts_filter_{}", Uuid::now_v7()));
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("forestx_app_server_protocol.schemas.json"))?;
        ...
        let flat_v2_bundle_json =
            fs::read_to_string(output_dir.join("forestx_app_server_protocol.v2.schemas.json"))?;
        ...
        assert_eq!(
            flat_v2_bundle_json.contains("\"title\": \"ForestxAppServerProtocolV2\""),
            true
        );
        let flat_v2_bundle =
            read_json_value(&output_dir.join("forestx_app_server_protocol.v2.schemas.json"))?;

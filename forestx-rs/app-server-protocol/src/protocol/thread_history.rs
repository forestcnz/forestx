...
use forestx_extension_items::image_generation::ImageGenerationItem;
use forestx_protocol::items::parse_hook_prompt_message;
use forestx_protocol::models::MessagePhase;
use forestx_protocol::protocol::AgentReasoningEvent;
use forestx_protocol::protocol::AgentReasoningRawContentEvent;
...
use forestx_protocol::protocol::ContextCompactedEvent;
use forestx_protocol::protocol::DynamicToolCallResponseEvent;
use forestx_protocol::protocol::ErrorEvent;
use forestx_protocol::protocol::EventMsg;
use forestx_protocol::protocol::ExecCommandBeginEvent;
use forestx_protocol::protocol::ExecCommandEndEvent;
use forestx_protocol::protocol::GuardianAssessmentEvent;
use forestx_protocol::protocol::GuardianAssessmentStatus;
use forestx_protocol::protocol::ImageGenerationBeginEvent;
use forestx_protocol::protocol::ImageGenerationEndEvent;
use forestx_protocol::protocol::ItemCompletedEvent;
use forestx_protocol::protocol::ItemStartedEvent;
use forestx_protocol::protocol::McpToolCallBeginEvent;
use forestx_protocol::protocol::McpToolCallEndEvent;
use forestx_protocol::protocol::PatchApplyBeginEvent;
use forestx_protocol::protocol::PatchApplyEndEvent;
...
use forestx_protocol::protocol::ViewImageToolCallEvent;
use forestx_protocol::protocol::WebSearchBeginEvent;
use forestx_protocol::protocol::WebSearchEndEvent;
#[cfg(test)]
use forestx_protocol::review_format::REVIEW_FALLBACK_MESSAGE;
...
#[cfg(test)]
use forestx_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use forestx_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `forestx-rs/core/rollout/policy.rs`.
    ...
    fn handle_response_item(&mut self, item: &forestx_protocol::models::ResponseItem) {
        let forestx_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &forestx_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            forestx_protocol::items::TurnItem::EnteredReviewMode(_)
                | forestx_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            forestx_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            forestx_protocol::items::TurnItem::HookPrompt(_)
            | forestx_protocol::items::TurnItem::CommandExecution(_)
            | forestx_protocol::items::TurnItem::DynamicToolCall(_)
            | forestx_protocol::items::TurnItem::CollabAgentToolCall(_)
            | forestx_protocol::items::TurnItem::SubAgentActivity(_)
            | forestx_protocol::items::TurnItem::Extension(_)
            | forestx_protocol::items::TurnItem::EnteredReviewMode(_)
            | forestx_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            forestx_protocol::items::TurnItem::UserMessage(_)
            | forestx_protocol::items::TurnItem::AgentMessage(_)
            | forestx_protocol::items::TurnItem::Reasoning(_)
            | forestx_protocol::items::TurnItem::WebSearch(_)
            | forestx_protocol::items::TurnItem::ImageView(_)
            | forestx_protocol::items::TurnItem::ImageGeneration(_)
            | forestx_protocol::items::TurnItem::FileChange(_)
            | forestx_protocol::items::TurnItem::McpToolCall(_)
            | forestx_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &forestx_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &forestx_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &forestx_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &forestx_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &forestx_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &forestx_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &forestx_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &forestx_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &forestx_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &forestx_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &forestx_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &forestx_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &forestx_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &forestx_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                forestx_error_info: payload.forestx_error_info.clone().map(Into::into),
    ...
    fn handle_turn_complete(&mut self, payload: &TurnCompleteEvent) {
        let terminal_error = payload.error.as_ref().map(|error| V2TurnError {
            ...
            forestx_error_info: error.forestx_error_info.clone().map(Into::into),
            additional_details: None,
        });
...
fn convert_dynamic_tool_content_items(
    items: &[forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
            } => DynamicToolCallOutputContentItem::InputImage { image_url },
            forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputAudio {
                audio_url,
...
mod tests {
    ...
    use forestx_extension_items::ExtensionItem as CoreExtensionItem;
    use forestx_extension_items::sleep::SleepItem as CoreSleepItem;
    use forestx_protocol::ThreadId;
    use forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use forestx_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use forestx_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use forestx_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use forestx_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use forestx_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use forestx_protocol::items::TurnItem as CoreTurnItem;
    use forestx_protocol::items::UserMessageItem as CoreUserMessageItem;
    use forestx_protocol::items::build_hook_prompt_message;
    use forestx_protocol::mcp::CallToolResult;
    use forestx_protocol::models::ImageDetail;
    use forestx_protocol::models::MessagePhase as CoreMessagePhase;
    use forestx_protocol::models::WebSearchAction as CoreWebSearchAction;
    use forestx_protocol::parse_command::ParsedCommand;
    ...
    use forestx_protocol::protocol::CompactedItem;
    use forestx_protocol::protocol::DynamicToolCallResponseEvent;
    use forestx_protocol::protocol::EnteredReviewModeEvent;
    use forestx_protocol::protocol::ExecCommandBeginEvent;
    use forestx_protocol::protocol::ExecCommandEndEvent;
    use forestx_protocol::protocol::ExecCommandSource;
    use forestx_protocol::protocol::ExitedReviewModeEvent;
    use forestx_protocol::protocol::ItemStartedEvent;
    use forestx_protocol::protocol::McpInvocation;
    use forestx_protocol::protocol::McpToolCallEndEvent;
    use forestx_protocol::protocol::PatchApplyBeginEvent;
    use forestx_protocol::protocol::ReviewTarget;
    use forestx_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![forestx_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
        ...
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                action: CoreWebSearchAction::Search {
                    query: Some("forestx".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/forestx",
                })]),
            }),
            EventMsg::ExecCommandEnd(ExecCommandEndEvent {
                call_id: "exec-1".into(),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                forestx_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("forestx_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    forestx_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
        ...
        let events = vec![
            ...
            EventMsg::TurnComplete(TurnCompleteEvent {
                ...
                error: Some(ErrorEvent {
                    ...
                    forestx_error_info: Some(ForestxErrorInfo::ServerOverloaded),
                }),
                ...
                Turn {
                    ...
                    error: Some(TurnError {
                        message: "Selected model is at capacity. Please try a different model."
                            .into(),
                        forestx_error_info: Some(
                            crate::protocol::v2::ForestxErrorInfo::ServerOverloaded,
                        ),
                        additional_details: None,
                    }),
                    started_at: Some(10),
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(forestx_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(forestx_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: forestx_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(forestx_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                forestx_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                forestx_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                forestx_error_info: Some(ForestxErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                forestx_error_info: Some(ForestxErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                forestx_error_info: Some(
                    crate::protocol::v2::ForestxErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn turn_complete_with_embedded_error_marks_turn_failed() {
        ...
        assert_eq!(
            ...
            vec![Turn {
                ...
                error: Some(TurnError {
                    message: "Selected model is at capacity. Please try a different model.".into(),
                    forestx_error_info: Some(crate::protocol::v2::ForestxErrorInfo::ServerOverloaded),
                    additional_details: None,
                }),
                started_at: Some(10),
                completed_at: Some(20),
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(forestx_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(forestx_protocol::models::ResponseItem::Message {
                id: Some(forestx_protocol::ResponseItemId::with_suffix("msg", "1")),
                role: "user".into(),
                content: vec![forestx_protocol::models::ContentItem::InputText {

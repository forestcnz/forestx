...
use forestx_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves Forestx response handoffs to the client's explicit append calls instead of forwarding
    ...
    /// Routes any transcript tail remaining at session end through Forestx. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub forestx_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `forestx_responses_as_items`.
    /// Optional prefix added to automatic Forestx response items when `forestxResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub forestx_response_item_prefix: Option<String>,
    /// Selects how automatic Forestx responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub forestx_response_handoff_mode: Option<ForestxResponseHandoffMode>,
    ...
    /// Set to false to start without Forestx's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
    ...
    /// Developer instructions given to the backing Forestx model when this realtime session starts.
    #[ts(optional = nullable)]
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to the backing Forestx model when this realtime session ends.
    #[ts(optional = nullable)]

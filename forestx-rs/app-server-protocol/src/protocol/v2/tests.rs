...
use forestx_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use forestx_protocol::config_types::MultiAgentMode;
use forestx_protocol::items::AgentMessageContent;
use forestx_protocol::items::AgentMessageItem;
use forestx_protocol::items::CollabAgentTool as CoreCollabAgentTool;
use forestx_protocol::items::CollabAgentToolCallItem;
use forestx_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use forestx_protocol::items::CommandExecutionItem;
...
use forestx_protocol::items::FileChangeItem;
use forestx_protocol::items::ImageViewItem;
use forestx_protocol::items::McpToolCallItem;
use forestx_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use forestx_protocol::items::ReasoningItem;
...
use forestx_protocol::items::WebSearchItem as CoreWebSearchItem;
use forestx_protocol::mcp::CallToolResult;
use forestx_protocol::mcp::McpServerInfo;
use forestx_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use forestx_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use forestx_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use forestx_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use forestx_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use forestx_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use forestx_protocol::protocol::AgentStatus as CoreAgentStatus;
use forestx_protocol::protocol::AskForApproval as CoreAskForApproval;
use forestx_protocol::protocol::ConversationTextRole;
use forestx_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use forestx_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use forestx_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use forestx_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use forestx_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use forestx_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: forestx_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        forestx_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        forestx_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn config_requirements_read_accepts_foreign_path_uris() {
    let response: ConfigRequirementsReadResponse = serde_json::from_value(json!({
        "requirements": {
            "sqliteHome": "file:///C:/Users/alice/.forestx/state",
            "logDir": "file:///C:/Users/alice/.forestx/logs",
            "modelCatalogJson": "file:///C:/Users/alice/.forestx/models.json"
        }
    }))
    ...
    assert_eq!(
        ...
        Some(PathUri::parse("file:///C:/Users/alice/.forestx/state").expect("valid URI"))
    );
    assert_eq!(
        requirements.log_dir,
        Some(PathUri::parse("file:///C:/Users/alice/.forestx/logs").expect("valid URI"))
    );
    assert_eq!(
        requirements.model_catalog_json,
        Some(PathUri::parse("file:///C:/Users/alice/.forestx/models.json").expect("valid URI"))
    );
}
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        forestx_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.forestx/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![forestx_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let dynamic_tool_call_item = TurnItem::DynamicToolCall(DynamicToolCallItem {
        ...
        content_items: Some(vec![
            forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text: "ok".to_string(),
            },
    ...
    let sender_thread_id = forestx_protocol::ThreadId::default();
    let receiver_thread_id = forestx_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: forestx_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            forestx_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(forestx_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@forestx-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "forestx-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@forestx-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "forestx-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn forestx_error_info_serializes_http_status_code_in_camel_case() {
    let value = ForestxErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn forestx_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(ForestxErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn forestx_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = ForestxErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(forestx_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(forestx_protocol::config_types::CollaborationMode {
            mode: forestx_protocol::config_types::ModeKind::Plan,
...
fn realtime_start_deserializes_client_handoff_channel_prefixes() {
    let params = serde_json::from_value::<ThreadRealtimeStartParams>(json!({
        "threadId": "thread_123",
        "outputModality": "audio",
        "forestxResponseHandoffChannelPrefixes": {
            "analysis": ["[THINKING]"],
    ...
    assert_eq!(
        params.forestx_response_handoff_channel_prefixes,
        Some(BTreeMap::from([

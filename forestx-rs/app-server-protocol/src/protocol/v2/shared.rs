...
use forestx_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use forestx_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use forestx_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
#[cfg(test)]
...
/// This translation layer make sure that we expose forestx error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `forestxErrorInfo` variant.
...
impl From<CoreForestxErrorInfo> for ForestxErrorInfo {
    fn from(value: CoreForestxErrorInfo) -> Self {
        match value {
            CoreForestxErrorInfo::ContextWindowExceeded => ForestxErrorInfo::ContextWindowExceeded,
            CoreForestxErrorInfo::SessionBudgetExceeded => ForestxErrorInfo::SessionBudgetExceeded,
            CoreForestxErrorInfo::UsageLimitExceeded => ForestxErrorInfo::UsageLimitExceeded,
            CoreForestxErrorInfo::ServerOverloaded => ForestxErrorInfo::ServerOverloaded,
            CoreForestxErrorInfo::CyberPolicy => ForestxErrorInfo::CyberPolicy,
            CoreForestxErrorInfo::HttpConnectionFailed { http_status_code } => {
                ForestxErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreForestxErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                ForestxErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreForestxErrorInfo::InternalServerError => ForestxErrorInfo::InternalServerError,
            CoreForestxErrorInfo::Unauthorized => ForestxErrorInfo::Unauthorized,
            CoreForestxErrorInfo::BadRequest => ForestxErrorInfo::BadRequest,
            CoreForestxErrorInfo::ThreadRollbackFailed => ForestxErrorInfo::ThreadRollbackFailed,
            CoreForestxErrorInfo::SandboxError => ForestxErrorInfo::SandboxError,
            CoreForestxErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                ForestxErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreForestxErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                ForestxErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreForestxErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                ForestxErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreForestxErrorInfo::Other => ForestxErrorInfo::Other,
        }
    }
}

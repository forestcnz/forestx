...
fn write_schema_fixtures_from_env() -> Result<()> {
    let schema_root = std::env::var_os("FORESTX_APP_SERVER_SCHEMA_ROOT")
        .map(PathBuf::from)
        .context("FORESTX_APP_SERVER_SCHEMA_ROOT must be set")?;
    let prettier = std::env::var_os("FORESTX_APP_SERVER_SCHEMA_PRETTIER").map(PathBuf::from);
    let experimental = std::env::var("FORESTX_APP_SERVER_SCHEMA_EXPERIMENTAL")
        .context("FORESTX_APP_SERVER_SCHEMA_EXPERIMENTAL must be set")?;
    let experimental_api = match experimental.as_str() {
        "0" => false,
        "1" => true,
        value => {
            anyhow::bail!("FORESTX_APP_SERVER_SCHEMA_EXPERIMENTAL must be 0 or 1, got {value:?}")
        }
    };
...
fn schema_root() -> Result<PathBuf> {
    ...
    let json_bundle =
        forestx_utils_cargo_bin::find_resource!("schema/json/forestx_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/forestx_app_server_protocol.schemas.json")?;
    anyhow::ensure!(

...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=FORESTX_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=FORESTX_SKIP_BWRAP_BUILD");
    ...
    if target_os != "linux" || env::var_os("FORESTX_SKIP_BWRAP_BUILD").is_some() {
        return;
    }
...
fn try_build_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built for Forestx"
"#,
    )
...
///
/// Priority:
/// 1. `FORESTX_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `forestx-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("FORESTX_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "FORESTX_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set FORESTX_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under forestx-rs/vendor.",
        vendor_dir.display()
    ))
}

...
#[cfg(unix)]
use crate::managed_install::resolved_managed_forestx_bin;
...
#[cfg(unix)]
const INSTALL_URL: &str = "https://chatgpt.com/forestx/install.sh";
...
async fn update_once(
    ...
    let managed_forestx_bin = resolved_managed_forestx_bin(&daemon.managed_forestx_bin).await?;
    let managed_identity = executable_identity(&managed_forestx_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_forestx_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_forestx_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_forestx_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed Forestx binary {}",
            managed_forestx_bin.display()
        )
    })
}
...
async fn install_latest_standalone(http: &RouteAwareClientPool) -> Result<()> {
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone Forestx updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone Forestx updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone Forestx updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone Forestx updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone Forestx updater exited with status {status}")
    }
}
...
async fn fetch_installer_script(http: &impl InstallerHttp) -> Result<Vec<u8>> {
    match http.get(INSTALL_URL).await? {
        ...
        InstallerResponse::Unsuccessful { status } => {
            anyhow::bail!("standalone Forestx updater request failed with status {status}")
        }
    }
}
...
impl InstallerHttp for RouteAwareClientPool {
    async fn get(&self, url: &str) -> Result<InstallerResponse> {
        let response = RouteAwareClientPool::get(self, url)
            .send()
            .await
            .context("failed to fetch standalone Forestx updater")?;
        ...
        let body = response
            .bytes()
            .await
            .context("failed to read standalone Forestx updater")?
            .to_vec();

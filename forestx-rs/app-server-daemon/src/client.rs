...
const CLIENT_NAME: &str = "forestx_app_server_daemon";
...
mod tests {
    ...
    #[test]
    fn parses_version_from_forestx_user_agent() {
        assert_eq!(
            parse_version_from_user_agent(
                "forestx_app_server_daemon/1.2.3 (Linux 6.8.0; x86_64) forestx_cli_rs/1.2.3",
            )
            .expect("version"),
            "1.2.3"
        );
    ...
    fn rejects_user_agent_without_version() {
        assert!(parse_version_from_user_agent("forestx_app_server_daemon").is_err());
    }
}

...
use forestx_app_server_transport::app_server_control_socket_path;
use forestx_utils_home_dir::find_forestx_home;
use managed_install::managed_forestx_bin;
#[cfg(unix)]
use managed_install::managed_forestx_version;
use serde::Serialize;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_forestx_path: PathBuf,
    pub managed_forestx_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "forestx app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_forestx_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let forestx_home = find_forestx_home().context("failed to resolve FORESTX_HOME")?;
        let socket_path = app_server_control_socket_path(forestx_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = forestx_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_forestx_bin: managed_forestx_bin(forestx_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_forestx_bin()?;
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by forestx app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_forestx_version(managed_forestx_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_forestx_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_forestx_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_forestx_version = self
            .managed_forestx_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_forestx_version}",
            self.managed_forestx_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_forestx_version = self.managed_forestx_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_forestx_path: self.managed_forestx_bin.clone(),
            managed_forestx_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_forestx_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_forestx_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_forestx_bin(&self) -> Result<()> {
        if self.managed_forestx_bin.is_file() {
            return Ok(());
        }

        let managed_forestx_path = self.managed_forestx_bin.display();
        Err(anyhow!(
            "managed standalone Forestx install not found at {managed_forestx_path}\n\n\
             This command requires the standalone install managed by the Forestx installer, because \
    ...
    #[cfg(unix)]
    async fn managed_forestx_version_best_effort(&self) -> Option<String> {
        managed_forestx_version(&self.managed_forestx_bin).await.ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_forestx_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            forestx_bin: managed_forestx_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_forestx_path: "forestx".into(),
            managed_forestx_version: Some("1.2.3".to_string()),
            socket_path: "forestx.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedForestxPath": "forestx",
                "managedForestxVersion": "1.2.3",
                "socketPath": "forestx.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_forestx_bin: temp_dir.path().join("missing-forestx"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_forestx_bin.display(),
                stderr_log.display()
            )
        );
    }

...
use anyhow::bail;
#[cfg(unix)]
use forestx_app_server_transport::REMOTE_CONTROL_DISABLED_ENV_VAR;
...
impl PidBackend {
    pub(crate) fn new(forestx_bin: PathBuf, pid_file: PathBuf, remote_control_enabled: bool) -> Self {
    ...
    pub(crate) fn new_update_loop(forestx_bin: PathBuf, pid_file: PathBuf) -> Self {
        let lock_file = pid_file.with_extension("pid.lock");
        Self {
            forestx_bin,
            pid_file,
    ...
    pub(crate) async fn start(&self) -> Result<Option<u32>> {
        ...
        let mut command = Command::new(&self.forestx_bin);

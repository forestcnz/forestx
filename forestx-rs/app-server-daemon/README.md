# forestx-app-server-daemon

> `forestx-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`forestx-app-server-daemon` backs the machine-readable `forestx app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Forestx instances launched over SSH, including fresh developer
...
```sh
forestx app-server daemon start
forestx app-server daemon restart
forestx app-server daemon enable-remote-control
forestx app-server daemon disable-remote-control
forestx app-server daemon stop
forestx app-server daemon version
forestx app-server daemon bootstrap --remote-control
```
...
$HOME/.forestx/packages/standalone/current/forestx app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `FORESTX_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes Forestx is installed through `install.sh` and always launches
the standalone managed binary under `FORESTX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `FORESTX_HOME/packages/standalone/current/forestx` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `FORESTX_HOME/packages/standalone/current/forestx` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `forestx remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `FORESTX_HOME`, so a concurrent
...
The daemon stores its local state under `FORESTX_HOME/app-server-daemon/`:

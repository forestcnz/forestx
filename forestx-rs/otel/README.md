...
`forestx-otel` is the OpenTelemetry integration crate for Forestx. It provides:

- Provider wiring for log/trace/metric exporters (`forestx_otel::OtelProvider`
  and `forestx_otel::provider`).
- Session-scoped business event emission via `forestx_otel::SessionTelemetry`.
- Low-level metrics APIs via `forestx_otel::metrics`.
- Trace-context helpers via `forestx_otel::trace_context` and crate-root re-exports.
...
```rust
use forestx_otel::config::OtelExporter;
use forestx_otel::config::OtelHttpProtocol;
use forestx_otel::config::OtelSettings;
use forestx_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "forestx-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    forestx_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, Forestx
...
Forestx-specific session events. Rich session/business events should go through
...
`forestx-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Forestx-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use forestx_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("forestx.session_started", 1, &[("source", "tui")])?;
metrics.histogram("forestx.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("forestx.turns", 1, &[("model", "gpt-5.1")])?;

...
impl OtelProvider {
    ...
    pub fn forestx_export_filter(meta: &tracing::Metadata<'_>) -> bool {
...
mod tests {
    ...
    fn log_export_target_excludes_trace_safe_events() {
        assert!(is_log_export_target("forestx_otel.log_only"));
        assert!(is_log_export_target("forestx_otel.network_proxy"));
        assert!(!is_log_export_target("forestx_otel.trace_safe"));
        assert!(!is_log_export_target("forestx_otel.trace_safe.debug"));
    }

    #[test]
    fn trace_export_target_only_includes_trace_safe_prefix() {
        assert!(is_trace_safe_target("forestx_otel.trace_safe"));
        assert!(is_trace_safe_target("forestx_otel.trace_safe.summary"));
        assert!(!is_trace_safe_target("forestx_otel.log_only"));
        assert!(!is_trace_safe_target("forestx_otel.network_proxy"));
    }
    ...
    fn statsig_runtime_only_metrics_are_not_exported() -> Result<(), Box<dyn Error>> {
        ...
        names.sort_unstable();
        names.dedup();
        assert_eq!(names, vec!["forestx.turns"]);
    ...
    fn test_otel_settings() -> OtelSettings {
        OtelSettings {
            ...
            service_name: "forestx-test".to_string(),

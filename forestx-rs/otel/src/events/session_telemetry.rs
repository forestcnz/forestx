...
pub struct AuthEnvTelemetryMetadata {
    ...
    pub forestx_api_key_env_present: bool,
    pub forestx_api_key_env_enabled: bool,
...
impl SessionTelemetry {
    ...
    pub fn record_startup_phase(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.startup_phase",
                startup.phase = phase,
    ...
    pub fn record_plugin_install_elicitation_sent(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.plugin_install_elicitation_sent",
    ...
    pub fn record_plugin_install_suggestion(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.plugin_install_suggestion",
                plugin_install.tool_type = tool_type,
                plugin_install.tool_id = tool_id,
                plugin_install.tool_name = tool_name,
    ...
    pub fn record_responses(&self, handle_responses_span: &Span, event: &ResponseEvent) {
        ...
        match event {
            ...
            ResponseEvent::Completed {
                ...
                handle_responses_span.record(
                    "forestx.usage.reasoning_output_tokens",
                    token_usage.reasoning_output_tokens,
                );
                handle_responses_span.record("forestx.usage.total_tokens", token_usage.total_tokens);
            }
            _ => {}
        }
    }
    ...
    pub fn conversation_starts(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.conversation_starts",
                provider_name = %provider_name,
                auth.env_openai_api_key_present = self.metadata.auth_env.openai_api_key_env_present,
                auth.env_forestx_api_key_present = self.metadata.auth_env.forestx_api_key_env_present,
                auth.env_forestx_api_key_enabled = self.metadata.auth_env.forestx_api_key_env_enabled,
    ...
    pub fn record_websocket_connect(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.websocket_connect",
                duration_ms = %duration.as_millis(),
                http.response.status_code = status,
                success = success_str,
                error.message = error,
    ...
    pub fn record_websocket_request(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.websocket_request",
                duration_ms = %duration.as_millis(),
                success = success_str,
                error.message = error,
                auth.env_openai_api_key_present = self.metadata.auth_env.openai_api_key_env_present,
    ...
    pub fn record_auth_recovery(
        ...
        log_and_trace_event!(
            self,
            common: {
                event.name = "forestx.auth_recovery",
                auth.mode = mode,
                auth.step = step,
    ...
    pub fn user_prompt(&self, items: &[UserInput]) {
        ...
        log_event!(
            self,
            event.name = "forestx.user_prompt",
    ...
    pub fn tool_decision(
        ...
        log_event!(
            self,
            event.name = "forestx.tool_decision",
            tool_name = %tool_name,
            call_id = %call_id,
    ...
    pub fn sandbox_outcome(
        ...
        log_event!(
            self,
            event.name = "forestx.sandbox_outcome",
            tool_name = %tool_name,
            call_id = %call_id,
            outcome = %outcome,
    ...
    pub fn log_tool_failed(&self, tool_name: &str, error: &str) {
        log_event!(
            self,
            event.name = "forestx.tool_result",
            tool_name = %tool_name,

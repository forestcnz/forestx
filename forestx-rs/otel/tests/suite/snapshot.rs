...
fn snapshot_collects_metrics_without_shutdown() -> Result<()> {
    ...
    let metric = find_metric(&snapshot, "forestx.tool.call").expect("counter metric missing");
...
fn observable_gauge_is_collected_on_every_delta_snapshot() -> Result<()> {
    let exporter = InMemoryMetricExporter::default();
    let config = MetricsConfig::in_memory("test", "forestx-cli", env!("CARGO_PKG_VERSION"), exporter)
        .with_runtime_reader();
    let metrics = MetricsClient::new(config)?;
    ...
    for snapshot in [metrics.snapshot()?, metrics.snapshot()?] {
        let gauge = find_metric(&snapshot, "forestx.active").expect("gauge metric missing");

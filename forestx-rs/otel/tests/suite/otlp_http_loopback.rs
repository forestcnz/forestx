...
use forestx_otel::OtelExporter;
use forestx_otel::OtelHttpProtocol;
use forestx_otel::OtelProvider;
use forestx_otel::OtelSettings;
use forestx_otel::Result;
...
fn otlp_http_exporter_sends_metrics_to_collector() -> Result<()> {
    ...
    metrics.counter("forestx.turns", /*inc*/ 1, &[("source", "test")])?;
    metrics.counter("forestx.api_request", /*inc*/ 1, &[("status", "200")])?;
    metrics.record_duration(
        "forestx.api_request.duration_ms",
        Duration::from_millis(100),
        &[("status", "200")],
    )?;
    metrics.counter("forestx.tool.call", /*inc*/ 1, &[("tool", "test")])?;
    metrics.record_duration(
        "forestx.tool.call.duration_ms",
        Duration::from_millis(42),
        &[("tool", "test")],
    )?;
    metrics.gauge_with_description(
        "forestx.active",
        "Number of active Forestx operations.",
        /*value*/ 1,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "forestx.test.log_exported",
    ...
    let body = String::from_utf8_lossy(&request.body);
    assert!(
        body.contains("forestx.test.log_exported"),
...
-> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let propagated_trace = tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::event!(
            ...
            tracing::Level::INFO,
            event.name = "forestx.test.trace_event",
    ...
    assert!(
        body.contains("forestx.test.trace_event"),

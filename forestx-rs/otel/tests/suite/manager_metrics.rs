...
use forestx_otel::PLUGIN_INSTALL_ELICITATION_SENT_METRIC;
use forestx_otel::PLUGIN_INSTALL_SUGGESTION_METRIC;
...
fn manager_attaches_metadata_tags_to_metrics() -> Result<()> {
    let (metrics, exporter) = build_metrics_with_defaults(&[("service", "forestx-cli")])?;
    ...
    let metric =
        find_metric(&resource_metrics, "forestx.session_started").expect("counter metric missing");
...
fn manager_attaches_optional_service_name_tag() -> Result<()> {
    ...
    manager.counter("forestx.session_started", /*inc*/ 1, &[]);
    manager.shutdown_metrics()?;

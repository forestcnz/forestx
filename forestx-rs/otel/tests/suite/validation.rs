...
fn counter_rejects_invalid_tag_key() -> Result<()> {
    ...
    let err = metrics
        .counter("forestx.turns", /*inc*/ 1, &[("bad key", "value")])
        .unwrap_err();
    assert!(matches!(
        err,
        MetricsError::InvalidTagComponent { label, value }
...
fn counter_rejects_negative_increment() -> Result<()> {
    let metrics = build_in_memory_client()?;
    let err = metrics.counter("forestx.turns", /*inc*/ -1, &[]).unwrap_err();
    assert!(matches!(
        err,
        MetricsError::NegativeCounterIncrement { name, inc } if name == "forestx.turns" && inc == -1
    ));
    metrics.shutdown()?;
    Ok(())
}

...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "forestx-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "forestx.turns",
        "Total number of Forestx turns.",
        /*inc*/ 1,
        &[("model", "gpt-5.1"), ("env", "dev")],
    )?;
    metrics.histogram(
        "forestx.tool_latency",
        /*value*/ 25,
        &[("tool", "shell")],
    )?;
    ...
    let counter = find_metric(&resource_metrics, "forestx.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of Forestx turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "forestx-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "forestx.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "forestx.tool_latency")
            ...
            .expect("forestx.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "forestx.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active Forestx operations.");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "forestx.alpha").expect("forestx.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "forestx.beta").expect("forestx.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("forestx.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("forestx.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;

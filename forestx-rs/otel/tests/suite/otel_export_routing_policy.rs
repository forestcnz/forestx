...
use forestx_otel::AuthEnvTelemetryMetadata;
...
fn auth_env_metadata() -> AuthEnvTelemetryMetadata {
    AuthEnvTelemetryMetadata {
        ...
        forestx_api_key_env_present: false,
        forestx_api_key_env_enabled: true,
...
fn otel_export_routing_policy_routes_user_prompt_log_and_trace_events() {
    ...
    assert!(
        logs.iter()
            .all(|log| { log.record.target().map(Cow::as_ref) == Some("forestx_otel.log_only") })
    );

    let prompt_log = find_log_by_event_name(&logs, "forestx.user_prompt");
    ...
    let prompt_trace_event = find_span_event_by_name_attr(span_events, "forestx.user_prompt");
...
fn otel_export_routing_policy_routes_tool_result_log_and_trace_events() {
    ...
    let tool_log = find_log_by_event_name(&logs, "forestx.tool_result");
    ...
    let tool_trace_event = find_span_event_by_name_attr(span_events, "forestx.tool_result");
...
fn otel_export_routing_policy_routes_auth_recovery_log_and_trace_events() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let recovery_log = find_log_by_event_name(&logs, "forestx.auth_recovery");
    ...
    let recovery_trace_event = find_span_event_by_name_attr(span_events, "forestx.auth_recovery");
...
fn otel_export_routing_policy_routes_api_request_auth_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let conversation_log = find_log_by_event_name(&logs, "forestx.conversation_starts");
    ...
    let request_log = find_log_by_event_name(&logs, "forestx.api_request");
    ...
    assert_eq!(
        request_log_attrs
            .get("auth.env_forestx_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
    ...
    let conversation_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "forestx.conversation_starts");
    ...
    let request_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "forestx.api_request");
...
fn otel_export_routing_policy_routes_websocket_connect_auth_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let connect_log = find_log_by_event_name(&logs, "forestx.websocket_connect");
    ...
    let connect_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "forestx.websocket_connect");
...
fn otel_export_routing_policy_routes_websocket_request_transport_observability() {
    ...
    let logs = log_exporter.get_emitted_logs().expect("log export");
    let request_log = find_log_by_event_name(&logs, "forestx.websocket_request");
    let request_log_attrs = log_attributes(&request_log.record);
    assert_eq!(
        request_log_attrs
            .get("auth.connection_reused")
    ...
    let spans = span_exporter.get_finished_spans().expect("span export");
    let request_trace_event =
        find_span_event_by_name_attr(&spans[0].events.events, "forestx.websocket_request");
    let request_trace_attrs = span_event_attributes(request_trace_event);
    assert_eq!(
        request_trace_attrs
            .get("auth.connection_reused")

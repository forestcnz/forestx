...
fn auth_identity(auth: &ForestxAuth) -> (Option<String>, Option<String>) {
    (auth.get_chatgpt_user_id(), auth.get_account_id())
}

fn cloud_config_eligible_auth(auth: &ForestxAuth) -> bool {
    ...
    C: BundleClient + 'static,
{
    pub(crate) fn new(
        ...
        let forestx_home = AbsolutePathBuf::resolve_path_against_base(forestx_home, "/");
        Self {
            auth_manager,
            client,
            cache: CloudConfigBundleCache::new(forestx_home.clone()),
            forestx_home,
            timeout,
        }
    }
    ...
    pub(crate) async fn load_startup_bundle_with_timeout(
        ...
        let _timer =
            forestx_otel::start_global_timer("forestx.cloud_config_bundle.fetch.duration_ms", &[]);
    ...
    async fn load_valid_cached_bundle(
        ...
        match self.cache.load(chatgpt_user_id, account_id).await {
            Ok(signed_payload) => {
                if let Err(err) = validate_bundle(&signed_payload.bundle, &self.forestx_home) {
                    tracing::warn!(
    ...
    async fn validate_and_cache_remote_bundle(
        ...
        if let Err(err) = validate_bundle(&bundle, &self.forestx_home) {
            emit_fetch_final_metric(
                trigger,
                "error",
                "invalid_bundle",

...
use forestx_backend_client::ConfigBundleResponse;
use forestx_backend_client::DeliveredTomlFragment;
use forestx_config::AbsolutePathBuf;
use forestx_config::CloudConfigFragment;
use forestx_config::CloudConfigTomlBundle;
use forestx_config::CloudRequirementsFragment;
use forestx_config::CloudRequirementsTomlBundle;
...
fn write_auth_json(forestx_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(forestx_home.join("auth.json"), serde_json::to_string(&value)?)?;
    Ok(())
}

fn create_test_cache(forestx_home: &Path) -> CloudConfigBundleCache {
    CloudConfigBundleCache::new(AbsolutePathBuf::resolve_path_against_base(forestx_home, "/"))
}
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(ForestxAuth::AgentIdentity(
...
impl BundleClient for StaticBundleClient {
    async fn get_bundle(&self, _auth: &ForestxAuth) -> Result<CloudConfigBundle, BundleRequestError> {
...
impl BundleClient for TokenBundleClient {
    async fn get_bundle(&self, auth: &ForestxAuth) -> Result<CloudConfigBundle, BundleRequestError> {
        self.request_count.fetch_add(1, Ordering::SeqCst);
        if matches!(
...
struct TestExternalChatgptAuth {
    current: RwLock<ForestxAuth>,
    refreshed: ForestxAuth,
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let forestx_home = tempdir().expect("tempdir");
    let cache = create_test_cache(forestx_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = ForestxAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = ForestxAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(forestx_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            forestx_backend_client::DeliveredRequirementsToml {

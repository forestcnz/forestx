...
fn bound_executed_tool_calls_for_prompt_with_priority(
    ...
    let omitted_call_reservation = serde_json::to_vec(&serde_json::json!({
        "_forestx_executed_tool_call_truncated": ExecutedToolCallTruncation {
...
pub enum ExecutedToolCallArguments {
    ...
    Truncated {
        #[serde(rename = "_forestx_executed_tool_call_truncated")]
        truncation: ExecutedToolCallTruncation,
    },
}

/// A model-attempted Forestx tool invocation captured at the shared runtime boundary.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, JsonSchema, TS)]
...
impl ExecutedToolCall {
    ...
    pub fn new(name: String, arguments: serde_json::Value) -> Self {
        let arguments = if arguments
            .as_object()
            .is_some_and(|object| object.contains_key("_forestx_executed_tool_call_truncated"))
        {
            serde_json::json!({ "_forestx_executed_tool_call_raw": arguments })
        } else {
            arguments
        };
        Self {

...
//! `schemars`) so they can be embedded in Forestx's own protocol structures.
...
/// MCP extensions supplied by the client that created a Forestx session.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `forestx-protocol` so other crates can convert

//! Defines the protocol for a Forestx session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Forestx:";
...
pub struct ConversationStartParams {
    /// Whether Forestx response handoffs are managed through explicit client append calls.
    ...
    /// Whether to route any remaining transcript tail through Forestx when the session ends.
    ...
    /// Sends automatic Forestx responses as realtime conversation items instead of handoff appends.
    pub forestx_responses_as_items: bool,
    /// Optional prefix added to automatic Forestx response items when `forestx_responses_as_items` is set.
    pub forestx_response_item_prefix: Option<String>,
    /// Selects how automatic Forestx handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub forestx_response_handoff_mode: ForestxResponseHandoffMode,
    /// Optional client-selected BEM prefixes keyed by `analysis`, `commentary`, and `final`.
    pub forestx_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append Forestx's startup context to the realtime backend prompt.
    ...
    /// Developer instructions given to Forestx when this realtime session starts.
    pub realtime_start_instructions: Option<String>,
    /// Developer instructions given to Forestx when this realtime session ends.
...
pub enum Op {
    ...
    /// Request Forestx to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down forestx instance.
    Shutdown,
...
/// running the command proposed by Forestx.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.forestx`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum ForestxResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// Forestx errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum ForestxErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub forestx_error_info: Option<ForestxErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.forestx_error_info
            .as_ref()
            .is_none_or(ForestxErrorInfo::affects_turn_status)
    }
}
...
pub struct TokenUsage {
    ...
    #[schemars(skip)]
    #[ts(skip)]
    pub forestx_rollout_budget_units: Option<serde_json::Number>,
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Forestx),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older Forestx
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(forestx_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                forestx_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_forestx() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("forestx".to_string()).restriction_product(),
            Some(Product::Forestx)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Forestx])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Forestx]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = forestx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            forestx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".forestx")).expect("create .forestx");
        let canonical_cwd = forestx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_forestx = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".forestx"))
            .expect("canonical .forestx");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_forestx.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_forestx = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".forestx"))
            .expect("canonical .forestx");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_forestx.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            forestx_error_info: Some(ForestxErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }

...
use chrono::TimeZone;
use chrono::Utc;
use forestx_http_client::HttpResponse;
...
#[test]
fn forestx_err_debug_preserves_legacy_shape() {
    let actual = [
        ForestxErr::Timeout,
        ForestxErr::Stream("disconnected".to_string()),
        ForestxErr::Stream("retry later".to_string()).with_retry_delay(Duration::from_secs(2)),
        ForestxErr::InternalServerError.with_retry_delay(Duration::from_secs(3)),
    ]
    .map(|err| format!("{err:?}"));
...
fn retryability_preserves_error_details_distinctions() {
    let errors = [
        (ForestxErr::ServerOverloaded, false),
        (
            ForestxErr::RetryLimit(RetryLimitReachedError {
        ...
        (
            ForestxErr::UnexpectedStatus(UnexpectedResponseError {
                status: StatusCode::TOO_MANY_REQUESTS,
                body: String::new(),
                user_message: None,
                url: None,
        ...
        (
            ForestxErrorDetails::ToolCollision("functions.update_plan".to_string()).into(),
            false,
        ),
        (ForestxErr::InternalServerError, true),
    ];
...
fn usage_limit_reached_error_formats_plus_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/forestx/settings/usage to purchase more credits or try again later."
    );
}
...
fn server_overloaded_maps_to_protocol() {
    let err = ForestxErr::ServerOverloaded;
    assert_eq!(
        err.to_forestx_protocol_error(),
        ForestxErrorInfo::ServerOverloaded
    );
}
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = ForestxErr::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    let err = ForestxErr::ResponseStreamFailed(ResponseStreamFailed {
        source,
    ...
    assert_eq!(
        event.forestx_error_info,
        Some(ForestxErrorInfo::ResponseStreamConnectionFailed {
...
fn usage_limit_reached_error_formats_free_plan() {
    ...
    assert_eq!(
        err.to_string(),
        "You've hit your usage limit. Upgrade to Plus to continue using Forestx (https://chatgpt.com/explore/plus), or try again later."
    );
}
...
fn usage_limit_reached_error_formats_pro_plan_with_reset() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Visit https://chatgpt.com/forestx/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_forestx_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
        let err = UsageLimitReachedError {
            ...
            promo_message: Some(
                "Visit https://chatgpt.com/forestx/settings/usage to purchase more credits"
                    .to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit for forestx_other. Switch to another model now, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn unexpected_status_prefers_error_message_when_present() {
    let err = UnexpectedResponseError {
        ...
        body: r#"{"error":{"message":"Workspace is not authorized in this region."},"status":401}"#
            .to_string(),
        user_message: None,
        url: Some("https://chatgpt.com/backend-api/forestx/responses".to_string()),
        cf_ray: None,
        request_id: Some("req-123".to_string()),
        identity_authorization_error: None,
        identity_error_code: None,
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: Workspace is not authorized in this region., url: https://chatgpt.com/backend-api/forestx/responses, request id: req-123"
        )
    );
}
...
fn unexpected_status_includes_cf_ray_and_request_id() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/forestx/responses, cf-ray: 9c81f9f18f2fa49d-LHR, request id: req-xyz"
        )
    );
}
...
fn unexpected_status_includes_identity_auth_details() {
    let err = UnexpectedResponseError {
        ...
        body: "plain text error".to_string(),
        user_message: None,
        url: Some("https://chatgpt.com/backend-api/forestx/models".to_string()),
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/forestx/models, cf-ray: cf-ray-auth-401-test, request id: req-auth, auth error: missing_authorization_header, auth error code: token_expired"
        )
    );
}
...
fn usage_limit_reached_includes_hours_and_minutes() {
    ...
    with_now_override(base, move || {
        ...
        let expected = format!(
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/forestx/settings/usage to purchase more credits or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}
...
fn usage_limit_reached_with_promo_message() {
    ...
    with_now_override(base, move || {
        ...
        let err = UsageLimitReachedError {
            ...
            rate_limits: Some(Box::new(rate_limit_snapshot())),
            promo_message: Some(
                "To continue using Forestx, start a free trial of <PLAN> today".to_string(),
            ),
            rate_limit_reached_type: None,
        };
        let expected = format!(
            "You've hit your usage limit. To continue using Forestx, start a free trial of <PLAN> today, or try again at {expected_time}."
        );
        assert_eq!(err.to_string(), expected);
    });
}

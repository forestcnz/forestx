...
use forestx_utils_image::PromptImageMode;
use forestx_utils_image::data_url_from_bytes;
use forestx_utils_image::load_for_prompt_bytes;
...
use forestx_utils_image::ImageProcessingError;
...
pub enum SandboxEnforcement {
    /// Forestx owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where Forestx owns sandbox construction.
...
fn local_media_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!("Forestx could not read the local {media_name} at `{path}`: {error}"),
    }
}
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Forestx cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
fn unsupported_audio_error_placeholder(path: &std::path::Path) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Forestx cannot attach audio at `{}`: unsupported audio format; use wav, mp3, m4a, webm, or ogg.",
            path.display()
        ),
    }
}
...
fn convert_mcp_content_to_items(
    ...
    const FORESTX_ENCRYPTED_CONTENT_META_KEY: &str = "forestx/encryptedContent";
    const FORESTX_IMAGE_DETAIL_META_KEY: &str = "forestx/imageDetail";
    ...
    for content in contents {
        let item = match serde_json::from_value::<McpContent>(content.clone()) {
            Ok(McpContent::Text { text, meta }) => {
                if meta
                    .as_ref()
                    .and_then(|meta| meta.get(FORESTX_ENCRYPTED_CONTENT_META_KEY))
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(FORESTX_IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into forestx-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__forestx_apps__gmail_get_recent_emails",
            "namespace": "mcp__forestx_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__forestx_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__forestx_apps__gmail".to_string()),
...
- ["a", "b", "c"]"#
        ...
        for i in 0..200 {
            exec_policy
                .add_prefix_rule(
                    ...
                    forestx_execpolicy::Decision::Allow,
                )
                .expect("add rule");
        }
    ...
    fn replaces_unreadable_local_audio_with_placeholder() {
        ...
        assert!(
            text.starts_with("Forestx could not read the local audio at `missing.wav`: "),
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__forestx_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Forestx cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {

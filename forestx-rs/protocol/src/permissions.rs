...
use forestx_utils_absolute_path::canonicalize_preserving_symlinks;
...
const PROTECTED_METADATA_FORESTX_PATH_NAME: &str = ".forestx";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_FORESTX_PATH_NAME,
];
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Forestx degrades to warn-and-ignore
    /// instead of failing to load. Forestx 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "forestx-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".forestx");
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_forestx*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_forestx*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_forestx*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_forestx = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_forestx)
                    .into_iter()
            ...
            // Example: if `<root>/.forestx -> <root>/decoy`, bwrap must still see
            // `<root>/.forestx`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.forestx` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_forestx: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .forestx read-only to the agent by
    ...
    let top_level_forestx = writable_root.join(PROTECTED_METADATA_FORESTX_PATH_NAME);
    if protect_missing_dot_forestx || top_level_forestx.as_path().is_dir() {
        subpaths.push(top_level_forestx);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "FORESTX_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_forestx() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_forestx = expected_root.join(".forestx");
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry::skip_missing_path(
                    FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".forestx".into())),
                    },
                    FileSystemAccessMode::Read,
                ),
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_forestx_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_forestx = expected_root.join(".forestx");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_forestx.clone(),
                },
                access: FileSystemAccessMode::Write,
                missing_path_behavior: None,
            },
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".forestx".to_string()),
            "explicit .forestx rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_forestx),
            "explicit .forestx rule should win over the default protected carveout"
        );
        assert!(
            policy.can_write_path_with_cwd(
                explicit_dot_forestx.join("config.toml").as_path(),
                cwd.path()
            )
        );
    }
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_forestx_config = cwd.path().join(".forestx").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_forestx_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_forestx_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_forestx*/ true,
            )
            .into_iter()
            .map(|path| {
                FileSystemSandboxEntry::skip_missing_path(
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".forestx/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let forestx_dir = real_root.join(".forestx");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&forestx_dir).expect("create .forestx");
        ...
        let expected_forestx = link_root.join(".forestx");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_forestx = expected_root.join(".forestx");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-forestx");
        let dot_forestx = root.join(".forestx");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_forestx).expect("create .forestx symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_forestx = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".forestx"),
        )
        .expect("absolute .forestx symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let forestx_dir = real_tmpdir.join(".forestx");

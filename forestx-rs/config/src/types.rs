//! Types used to define loaded and effective Forestx configuration values.
...
pub use forestx_protocol::config_types::ModeKind;
pub use forestx_protocol::config_types::Personality;
pub use forestx_protocol::config_types::ServiceTier;
...
pub enum ResumeCwdMode {
    /// Use the directory where Forestx was launched.
    Current,
...
/// Determine where Forestx should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in FORESTX_HOME/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in FORESTX_HOME.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Determine where Forestx should store and read MCP credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
...
pub struct AnalyticsConfigToml {
    /// When `false`, disables analytics across Forestx product surfaces in this profile.
...
pub struct FeedbackConfigToml {
    /// When `false`, disables the feedback flow across Forestx product surfaces.
    pub enabled: Option<bool>,
}
...
pub struct MemoriesToml {
    ...
    /// Minimum remaining percentage required in Forestx rate-limit windows before memory startup runs.
...
pub struct AppConfig {
    /// When `false`, Forestx does not surface this app.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
/// Fallback resize-reflow row cap when Forestx cannot identify a terminal-specific scrollback size.
...
pub struct Tui {
    ...
    /// Use `/theme` in the TUI or see `$FORESTX_HOME/themes` for custom themes.
    #[serde(default)]
    pub theme: Option<String>,

    /// Pet id to preselect in the terminal pet picker.
    ///
    /// Custom pet ids resolve against FORESTX_HOME/pets/<pet-id>/pet.json.
    #[serde(default)]
    ...
    /// transcript exceeds this cap. Omit to use Forestx's terminal-specific default. Set to `0` to
...
/// (primarily the Forestx IDE extension). NOTE: these are different from
...
pub struct Notice {
    ...
    /// Tracks whether the user opted out of Forestx-managed fast defaults.
    pub fast_default_opt_out: Option<bool>,
    /// Tracks whether the user opted out of the rate limit model switch reminder.
    pub hide_rate_limit_model_nudge: Option<bool>,
    /// Tracks whether the user has seen the model migration prompt
    pub hide_gpt5_1_migration_prompt: Option<bool>,
    /// Tracks whether the user has seen the gpt-5.1-forestx-max migration prompt
    #[serde(rename = "hide_gpt-5.1-forestx-max_migration_prompt")]
    pub hide_gpt_5_1_forestx_max_migration_prompt: Option<bool>,
...
pub struct PluginMcpServerConfig {
    /// When `false`, Forestx skips initializing this plugin MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,
...
pub struct MarketplaceConfig {
    /// Last time Forestx successfully added or refreshed this marketplace.
    #[serde(default)]
    pub last_updated: Option<String>,
    /// Git revision Forestx last successfully activated for this marketplace.
    #[serde(default)]

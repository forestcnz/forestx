...
use forestx_execpolicy::RuleRef;
use forestx_execpolicy::rule::PatternToken;
use forestx_execpolicy::rule::PrefixPattern;
use forestx_execpolicy::rule::PrefixRule;
...
pub enum RequirementsExecPolicyParseError {
    ...
    #[error(
        "rules prefix_rule at index {rule_index} has decision 'allow', which is not permitted in requirements.toml: Forestx merges these rules with other config and uses the most restrictive result (use 'prompt' or 'forbidden')"
    )]
...
impl RequirementsExecPolicyToml {
    ...
    /// representation used by `forestx-execpolicy`.

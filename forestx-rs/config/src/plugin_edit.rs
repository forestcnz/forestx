...
use forestx_utils_path::resolve_symlink_write_paths;
use forestx_utils_path::write_atomically;
use tokio::task;
...
pub async fn clear_user_plugin(forestx_home: &Path, plugin_key: String) -> std::io::Result<()> {
    apply_user_plugin_config_edits(forestx_home, vec![PluginConfigEdit::Clear { plugin_key }]).await
}
...
pub async fn apply_user_plugin_config_edits(
    ...
    task::spawn_blocking(move || apply_user_plugin_config_edits_blocking(&forestx_home, edits))
        .await
...
mod tests {
    ...
    async fn set_user_plugin_enabled_writes_plugin_entry() {
        ...
        let config = read_config(forestx_home.path());
...
"#,
        ...
        clear_user_plugin(forestx_home.path(), "demo@market".to_string())
            .await
            .unwrap();

        assert_eq!(
            fs::read_to_string(forestx_home.path().join(CONFIG_TOML_FILE)).unwrap(),
            ""
        );
    }
    ...
    async fn set_user_plugin_enabled_follows_config_symlink() {
        ...
        let forestx_home = TempDir::new().unwrap();
        let target_path = forestx_home.path().join("target_config.toml");
        symlink(&target_path, forestx_home.path().join(CONFIG_TOML_FILE)).unwrap();
...
enabled = true
    ...
    fn read_config(forestx_home: &Path) -> toml::Value {
        toml::from_str(&fs::read_to_string(forestx_home.join(CONFIG_TOML_FILE)).unwrap()).unwrap()
    }
}

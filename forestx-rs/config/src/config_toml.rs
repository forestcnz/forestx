//! Schema-heavy configuration TOML types used by Forestx.
...
use forestx_protocol::config_types::WebSearchToolConfig;
...
use forestx_utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.forestx/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Forestx will likely degrade model performance.
    ...
    /// file: Use a file in the Forestx home directory.
    ...
    /// Directory where Forestx stores the SQLite state DB.
    /// Defaults to `$FORESTX_SQLITE_HOME` when set. Otherwise uses `$FORESTX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Forestx writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$FORESTX_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned Forestx Apps MCP requests.
    ...
    /// directories for `.forestx` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different Forestx version.
    pub allow_forestx_version_mismatch: Option<bool>,
...
pub use forestx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use forestx_protocol::protocol::RealtimeVoice;

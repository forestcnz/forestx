...
impl LoaderOverrides {
    ...
    pub fn without_managed_config_for_tests() -> Self {
        let base = std::env::temp_dir().join("forestx-config-tests");
        Self {
            user_config_path: None,
            user_config_profile: None,
            managed_config_path: Some(base.join("managed_config.jsonc")),
            system_config_path: Some(base.join("config.toml")),
            system_requirements_path: Some(base.join("requirements.jsonc")),
    ...
    pub fn with_managed_config_path_for_tests(managed_config_path: PathBuf) -> Self {
        let system_requirements_path = managed_config_path.with_file_name("requirements.jsonc");
        Self {
            user_config_path: None,
            user_config_profile: None,
    ...
    pub fn user_config_path(&self, forestx_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.forestx/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ...
            ConfigLayerSource::Project { dot_forestx_folder } => Some(dot_forestx_folder.clone()),
    ...
    /// Returns the `.forestx/` folder that should be used for hook declarations.
    ///
...
impl ConfigLayerStack {
    ...
    /// the base `$FORESTX_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<()> {
    ...
    // user config.
    let mut previous_project_dot_forestx_folder: Option<&AbsolutePathBuf> = None;
    for layer in layers {
        if let ConfigLayerSource::Project {
            dot_forestx_folder: current_project_dot_forestx_folder,
        } = &layer.name
        {
            if let Some(previous) = previous_project_dot_forestx_folder {
                ...
                if previous == current_project_dot_forestx_folder
                    || !current_project_dot_forestx_folder
                        .as_path()
                        .ancestors()
            ...
            previous_project_dot_forestx_folder = Some(current_project_dot_forestx_folder);
        }
    }

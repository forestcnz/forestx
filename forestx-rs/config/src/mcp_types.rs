...
use forestx_protocol::config_types::ToolExposureSurface;
...
/// OAuth client settings used when Forestx launches an MCP OAuth flow.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema)]
#[schemars(deny_unknown_fields)]
...
/// Authentication flow Forestx attempts after resolving an HTTP MCP server's
...
pub struct McpServerConfig {
    ...
    /// Effective environment id for where Forestx should start this MCP server.
    pub environment_id: String,

    /// When `false`, Forestx skips initializing this MCP server.
    #[serde(default = "default_enabled")]
    pub enabled: bool,

    /// When `true`, `forestx exec` exits with an error if this MCP server fails to initialize.

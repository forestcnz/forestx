...
fn layer(id: &str, name: &str, contents: &str) -> RequirementsLayerEntry {
    // Test data is authored as TOML for readability; parse it here with the TOML
    // parser and hand the entry a pre-parsed value, so tests exercise the
    // composition logic rather than the (now JSONC) production parse path.
    let value = toml::from_str(contents).expect("test layer contents should parse as TOML");
...
fn layer_from(source: RequirementSource, contents: impl AsRef<str>) -> RequirementsLayerEntry {
    // Like `layer`, but for tests that supply their own `RequirementSource`.
    // Test data is TOML, parsed here so the entry bypasses the (now JSONC)
    // production parse path and exercises composition logic instead.
    let value =
        toml::from_str(contents.as_ref()).expect("test layer contents should parse as TOML");
    RequirementsLayerEntry::from_toml_value(source, value)
}

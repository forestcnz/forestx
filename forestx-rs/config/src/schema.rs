...
use forestx_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == forestx_features::Feature::Artifact {
            continue;
        }
        if feature.id == forestx_features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::CodeModeConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::CodeModeHost {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::CodeModeHostConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::NonPrefixedMcpToolNames {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::NonPrefixedMcpToolNamesConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::TokenBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::TokenBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::RolloutBudget {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::RolloutBudgetConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::CurrentTimeReminder {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::CurrentTimeReminderConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == forestx_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
        ...
        if feature.id == forestx_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<forestx_features::FeatureToml<
                    forestx_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
    ...
    validation.properties.insert(
        ...
        schema_gen.subschema_for::<forestx_features::ToolRegistryConfigToml>(),
    );

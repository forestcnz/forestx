...
#[cfg(unix)]
const FORESTX_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/forestx/managed_config.jsonc";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/forestx/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(forestx_home)),
    )?;
...
pub(super) async fn read_config_from_path(
    ...
    match fs.read_file_text(&path_uri, /*sandbox*/ None).await {
        Ok(contents) => match crate::jsonc::parse_jsonc_as_toml(&contents) {
            Ok(value) => {
                if strict_config {
            ...
            Err(err) => {
                tracing::error!("Failed to parse JSONC {}: {err}", path.as_path().display());
                Err(io::Error::new(
...
pub(super) fn managed_config_default_path(forestx_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = forestx_home;
        PathBuf::from(FORESTX_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        forestx_home.join("managed_config.jsonc")
    }
}

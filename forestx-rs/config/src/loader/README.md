# `forestx-config` loader

This module is the canonical place to **load and describe Forestx configuration layers** (user config, CLI/session overrides, cloud-managed config, managed config, and MDM-managed preferences) and to produce:
...
Exported from `forestx_config::loader`:

- `load_config_layers_state(fs, forestx_home, cwd_opt, cli_overrides, options, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`

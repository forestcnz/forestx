...
use forestx_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/forestx/config.jsonc";
...
///
/// - system    `/etc/forestx/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Forestx\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/forestx/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Forestx\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${FORESTX_HOME}/config.toml`
/// - profile   `${FORESTX_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.forestx/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.forestx/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/forestx/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(forestx_home)?;
        ...
        #[cfg(target_os = "macos")]
        {
            let managed_preferences_base_dir = AbsolutePathBuf::from_absolute_path(forestx_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, forestx_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(forestx_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, forestx_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/forestx/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(forestx_home)?;
    ...
    let toml_value = match fs.read_file_text(&toml_file_uri, /*sandbox*/ None).await {
        Ok(contents) => {
            ...
            let config: TomlValue =
                crate::jsonc::parse_jsonc_as_toml(&contents).map_err(|err| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                        format!(
                            "Error parsing JSONC config {}: {err}",
                            toml_file.as_path().display()
                        ),
                    )
                })?;
...
pub async fn load_requirements_toml(
        ...
        .await
    {
        Ok(contents) => {
            ...
            let value = crate::jsonc::parse_jsonc_as_toml(&contents).map_err(|err| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
                        "Error parsing JSONC requirements {}: {err}",
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/forestx/requirements.jsonc"))
}
...
#[cfg(windows)]
fn windows_forestx_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("Forestx")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_forestx_system_dir().join("requirements.jsonc");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_forestx_system_dir().join("config.jsonc");
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for Forestx to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".forestx"))
    }
}

fn project_layer_entry(
    dot_forestx_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_forestx_folder: dot_forestx_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_forestx_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let forestx_home_abs = AbsolutePathBuf::from_absolute_path(forestx_home)?;
    let forestx_home_normalized = normalize_path(forestx_home_abs.as_path())
        .unwrap_or_else(|_| forestx_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_forestx_abs = dir.join(".forestx");
        let dot_forestx_uri = PathUri::from_abs_path(&dot_forestx_abs);
        if !fs
            .get_metadata(&dot_forestx_uri, /*sandbox*/ None)
            .await
        ...
        let dot_forestx_normalized = normalize_path(dot_forestx_abs.as_path())
            .unwrap_or_else(|_| dot_forestx_abs.to_path_buf());
        if dot_forestx_abs == forestx_home_abs || dot_forestx_normalized == forestx_home_normalized
        {
            continue;
        }
        let config_file = dot_forestx_abs.join(CONFIG_TOML_FILE);
        let config_file_uri = PathUri::from_abs_path(&config_file);
        match fs.read_file_text(&config_file_uri, /*sandbox*/ None).await {
            Ok(contents) => {
                let config: TomlValue = match crate::jsonc::parse_jsonc_as_toml(&contents) {
                    Ok(config) => config,
                    Err(e) => {
                        if decision.is_trusted() {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_forestx_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_forestx_abs.as_path())?;
...
async fn merge_root_checkout_project_hooks(
    ...
    {
        Ok(contents) => {
            let parsed: TomlValue = match crate::jsonc::parse_jsonc_as_toml(&contents) {
                Ok(parsed) => parsed,
                Err(err) => {
                    if is_trusted {
                        return Err(io::Error::new(
...
/// from a file like `/etc/forestx/managed_config.toml` that has the same
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Forestx").join("config.jsonc"))
        );
    }
}

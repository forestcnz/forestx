...
//!
//! The history is stored at `~/.forestx/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.forestx`.
...
impl HistoryConfig {
    pub fn new(forestx_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            forestx_home: forestx_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.forestx_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.forestx/history.jsonl` and ensure the parent directory exists.

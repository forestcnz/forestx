...
async fn append_entry_trims_history_when_beyond_max_bytes() {
    ...
    let mut config = HistoryConfig::new(forestx_home.path(), &history);
    let conversation_id = "conversation-id";

    let entry_one = "a".repeat(200);
    let entry_two = "b".repeat(200);

    let history_path = forestx_home.path().join("history.jsonl");
    ...
    config = HistoryConfig::new(forestx_home.path(), &history);

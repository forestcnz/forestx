...
use forestx_tui::render_markdown_text;
...
fn rounded_enabled() -> bool {
    *ROUNDED.get_or_init(|| {
        std::env::var("FORESTX_TUI_ROUNDED")
            .ok()
            .map(|v| v == "1")
            .unwrap_or(true)
    })
...
fn render_task_item(_app: &App, t: &forestx_cloud_tasks_client::TaskSummary) -> ListItem<'static> {

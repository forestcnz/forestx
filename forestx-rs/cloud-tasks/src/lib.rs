...
use anyhow::anyhow;
use chrono::Utc;
use forestx_cloud_tasks_client::TaskStatus;
use forestx_git_utils::current_branch_name;
use forestx_git_utils::default_branch_name;
...
use forestx_login::default_client::get_forestx_user_agent;
...
struct ApplyJob {
    task_id: forestx_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn forestx_cloud_tasks_client::CloudBackend>,
    base_url: String,
...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    let use_mock = matches!(
        std::env::var("FORESTX_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );
    let base_url = std::env::var("FORESTX_CLOUD_TASKS_BASE_URL")
    ...
    if use_mock {
        ...
        return Ok(BackendContext {
            backend: Arc::new(forestx_cloud_tasks_mock_client::MockClient),
            base_url,
    ...
    let mut http = forestx_cloud_tasks_client::HttpClient::new(base_url.clone(), http_client_factory)
        .with_user_agent(ua);
    ...
    let auth = match auth {
        ...
        None => {
            eprintln!(
                "Not signed in. Please run 'forestx login' to sign in with ChatGPT, then re-run 'forestx cloud'."
            );
            std::process::exit(1);
        }
    };
    ...
    let auth_provider = forestx_model_provider::auth_provider_from_auth(&auth);
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("forestx_cloud_tasks_exec").await?;
    ...
    let created = forestx_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `forestx cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `forestx cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
fn parse_task_id(raw: &str) -> anyhow::Result<forestx_cloud_tasks_client::TaskId> {
    ...
    Ok(forestx_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    backend: &dyn forestx_cloud_tasks_client::CloudBackend,
    task_id: &forestx_cloud_tasks_client::TaskId,
) -> anyhow::Result<Vec<AttemptDiffData>> {
    let text =
        forestx_cloud_tasks_client::CloudBackend::get_task_text(backend, task_id.clone()).await?;
    let mut attempts = Vec::new();
    if let Some(diff) =
        forestx_cloud_tasks_client::CloudBackend::get_task_diff(backend, task_id.clone()).await?
    {
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = forestx_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &forestx_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &forestx_cloud_tasks_client::TaskSummary,
...
fn format_task_list_lines(
    tasks: &[forestx_cloud_tasks_client::TaskSummary],
    base_url: &str,
    now: chrono::DateTime<Utc>,
    colorize: bool,
) -> Vec<String> {
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    let ctx = init_backend("forestx_cloud_tasks_status").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let summary =
        forestx_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    let ctx = init_backend("forestx_cloud_tasks_list").await?;
    let env_filter = if let Some(env) = args.environment {
        Some(resolve_environment_id(&ctx, &env).await?)
    } else {
        None
    };
    let page = forestx_cloud_tasks_client::CloudBackend::list_tasks(
        &*ctx.backend,
        env_filter.as_deref(),
        Some(args.limit),
    ...
    if let Some(cursor) = page.cursor {
        let command = format!("forestx cloud list --cursor='{cursor}'");
        if colorize {
            println!(
...
async fn run_diff_command(args: crate::cli::DiffCommand) -> anyhow::Result<()> {
    let ctx = init_backend("forestx_cloud_tasks_diff").await?;
    let task_id = parse_task_id(&args.task_id)?;
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    let ctx = init_backend("forestx_cloud_tasks_apply").await?;
    let task_id = parse_task_id(&args.task_id)?;
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = forestx_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
    ...
    if !matches!(
        outcome.status,
        forestx_cloud_tasks_client::ApplyStatus::Success
    ) {
        std::process::exit(1);
    }
    Ok(())
...
fn level_from_status(status: forestx_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        forestx_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        forestx_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        forestx_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = forestx_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = forestx_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
/// Entry point for the `forestx cloud` subcommand.
pub async fn run_main(cli: Cli, _forestx_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext {
        ...
        base_url,
        environment_http,
    } = init_backend("forestx_cloud_tasks_tui").await?;
    ...
    let force_internal = matches!(
        std::env::var("FORESTX_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx
                            .send(Instant::now() + forestx_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match forestx_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, forestx_cloud_tasks_client::ApplyStatus::Success) {
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let forestx_tui::ComposerAction::Submitted(text) =
                                        page.composer.input(key)
                                    {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = forestx_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, /*qa_mode*/ false, best_of_n).await;
                                    ...
                                    if page.composer.is_in_paste_burst() {
                                        let _ = frame_tx.send(
                                            Instant::now()
                                                + forestx_tui::ComposerInput::recommended_flush_delay(),
                                        );
                                    }
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match forestx_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match forestx_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = forestx_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match forestx_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
mod tests {
    ...
    use forestx_cloud_tasks_client::DiffSummary;
    use forestx_cloud_tasks_client::TaskId;
    use forestx_cloud_tasks_client::TaskStatus;
    use forestx_cloud_tasks_client::TaskSummary;
    use forestx_cloud_tasks_mock_client::MockClient;
    ...
    fn format_task_list_lines_formats_urls() {
        ...
        assert_eq!(
            lines,
            vec![
                "https://chatgpt.com/forestx/tasks/task_1".to_string(),
                "  [READY] Example task".to_string(),
                "  Env  •  0s ago".to_string(),
                "  +5/-2 • 3 files".to_string(),
                String::new(),
                "https://chatgpt.com/forestx/tasks/task_2".to_string(),
                "  [PENDING] No diff task".to_string(),
                "  env-2  •  0s ago".to_string(),
                "  no diff".to_string(),
            ]
    ...
    async fn collect_attempt_diffs_includes_sibling_attempts() {
        ...
        let task_id = parse_task_id("https://chatgpt.com/forestx/tasks/T-1000").expect("id");
    ...
    fn parse_task_id_from_url_and_raw() {
        ...
        let url =
            parse_task_id("https://chatgpt.com/forestx/tasks/task_i_123456?foo=bar").expect("url id");

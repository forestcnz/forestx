...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = forestx_login::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("forestx_cloud_tasks_tui");
    let ua = forestx_login::default_client::get_forestx_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("forestx-cli")),
    );
    if let Some(am) = load_auth_manager(/*chatgpt_base_url*/ None).await.0
        && let Some(auth) = am.auth().await
        && auth.uses_forestx_backend()
    {
        headers.extend(forestx_model_provider::auth_provider_from_auth(&auth).to_auth_headers());
    }
    headers
}
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/forestx/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/forestx") {
        return format!("{root}/forestx/tasks/{task_id}");
    }
    if normalized.ends_with("/forestx") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/forestx/tasks/{task_id}")
}

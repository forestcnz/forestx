...
pub enum Command {
    /// Submit a new Forestx Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Forestx Cloud task.
    Status(StatusCommand),
    /// List Forestx Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Forestx Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Forestx Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Forestx Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `forestx cloud` to browse).
    ...
    /// Git branch to run in Forestx Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Forestx Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Forestx Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Forestx Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,

...
# Run a suite of smoke tests against the Windows sandbox via the Forestx CLI
...
def _resolve_forestx_cmd() -> List[str]:
    """Resolve the Forestx CLI to invoke `forestx sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Forestx.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "forestx.exe",
        ws_root / "target" / "release" / "forestx.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "forestx.exe",
            cargo_base / "release" / "forestx.exe",
        ])
    ...
    if shutil.which("forestx"):
        return ["forestx"]

    raise FileNotFoundError(
        "Forestx CLI not found. Build it first, e.g.\n"
        "  cargo build -p forestx-cli --release\n"
        "or for debug:\n"
        "  cargo build -p forestx-cli\n"
    )

FORESTX_CMD = _resolve_forestx_cmd()
print(FORESTX_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to forestx CLI overrides.
    ...
    argv = [*FORESTX_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"FORESTX_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\forestx_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.forestx artifacts) denied
    forestx_home = Path(os.environ["USERPROFILE"]) / ".forestx"
    cap_sid_target = forestx_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .forestx\\policy.json"], WS_ROOT)
    add("WS: .forestx cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .forestx policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > forestx_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://forestx-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(

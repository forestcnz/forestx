...
pub fn current_log_file_path_for_forestx_home(forestx_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(forestx_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_forestx_home_uses_sandbox_dir() {
        let forestx_home = Path::new("forestx-home");

        assert_eq!(
            current_log_file_path_for_forestx_home(forestx_home),
            current_log_file_path(&forestx_home.join(".sandbox"))
        );
    }
}

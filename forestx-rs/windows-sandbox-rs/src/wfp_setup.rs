...
const WFP_SETUP_SERVICE_NAME: &str = "forestx-windows-sandbox-setup";
const WFP_SETUP_SUCCESS_METRIC: &str = "forestx.windows_sandbox.wfp_setup_success";
const WFP_SETUP_FAILURE_METRIC: &str = "forestx.windows_sandbox.wfp_setup_failure";
...
fn build_wfp_metrics_provider(
    ...
    // The setup helper cannot call forestx-core's OTEL builder because core
...
fn emit_wfp_setup_metric(
    ...
    let Some(provider) = build_wfp_metrics_provider(forestx_home, otel)? else {
        return Ok(());
    };
    ...
    F: FnMut(&str),
{
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        emit_wfp_setup_metric(forestx_home, otel, metric)
    }));
    match result {
        Ok(Ok(())) => {}
    ...
    F: FnMut(&str),
{
    ...
    emit_wfp_setup_metric_safely(forestx_home, otel, offline_username, &metric, &mut log);
}

//! Internal `forestx.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const FORESTX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const FORESTX_HOME_FLAG: &str = "--forestx-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        FORESTX_WINDOWS_SANDBOX_ARG1.to_string(),
        FORESTX_HOME_FLAG.to_string(),
        forestx_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            forestx_home: request.forestx_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            FORESTX_HOME_FLAG => forestx_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let forestx_home = forestx_home.ok_or_else(|| anyhow!("missing required {FORESTX_HOME_FLAG}"))?;
    if !forestx_home.is_absolute() {
        bail!(
            "{FORESTX_HOME_FLAG} must be absolute: {}",

...
pub const OFFLINE_USERNAME: &str = "ForestxSandboxOffline";
pub const ONLINE_USERNAME: &str = "ForestxSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "forestx-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(forestx_home: &Path) -> PathBuf {
    forestx_home.join(".sandbox")
}

pub fn sandbox_bin_dir(forestx_home: &Path) -> PathBuf {
    forestx_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(forestx_home: &Path) -> PathBuf {
    forestx_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(forestx_home: &Path) -> PathBuf {
    sandbox_dir(forestx_home).join("setup_marker.json")
}

pub fn sandbox_users_path(forestx_home: &Path) -> PathBuf {
    sandbox_secrets_dir(forestx_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, forestx_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        forestx_home: request.forestx_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.forestx_home)
    })
}

fn run_setup_refresh_payload(b64: &str, forestx_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(forestx_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(forestx_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    cmd.arg(b64)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    let cwd = std::env::current_dir().unwrap_or_else(|_| forestx_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(forestx_home) {
        log_note(
...
fn gather_helper_read_roots(forestx_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(forestx_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(forestx_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, forestx_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<forestx_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "FORESTX_NETWORK_ALLOW_LOCAL_BINDING";
...
const WINDOWS_SANDBOX_PROXY_PORTS_ENV_KEY: &str = "FORESTX_WINDOWS_SANDBOX_PROXY_PORTS";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(forestx_home) {
...
fn verify_setup_completed(forestx_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(forestx_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, forestx_home)
    })
}
...
fn run_setup_exe_payload(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(forestx_home)?;
        if let Err(err) = clear_setup_error_report(forestx_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.forestx_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: forestx_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.forestx_home)
}
...
pub fn run_elevated_provisioning_setup(
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: forestx_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, forestx_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.forestx_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, forestx_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to FORESTX_HOME or anything under FORESTX_HOME/.sandbox,
    // FORESTX_HOME/.sandbox-bin, or FORESTX_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let forestx_home_key = canonical_path_key(forestx_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(forestx_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(forestx_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(forestx_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != forestx_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let forestx_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(forestx_home.path())
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("forestx.exe");
        let setup_exe = resources_dir.join("forestx-windows-sandbox-setup.exe");
        fs::write(&exe, b"forestx").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "FORESTX_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_forestx_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let forestx_home = user_profile.join("ForestxHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&forestx_home).expect("create forestx home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &forestx_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &forestx_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&forestx_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&forestx_home);
        fs::create_dir_all(&forestx_home).expect("create forestx home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_forestx_home = dunce::canonicalize(&forestx_home).expect("canonical forestx home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_forestx_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_forestx = extra_write_root.join(".forestx");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_forestx).expect("create extra .forestx");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_forestx).expect("canonical extra .forestx"),
                explicit_deny,
            ]
            .into_iter()

...
pub(crate) const RESOURCES_DIRNAME: &str = "forestx-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "forestx-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(forestx_home: &Path) -> PathBuf {
    sandbox_bin_dir(forestx_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, forestx_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(forestx_home: &Path, fallback_executable: &str) -> PathBuf {
    let source = match std::env::current_exe() {
        Ok(path) => path,
        Err(_) => return PathBuf::from(fallback_executable),
    };
    resolve_exe_for_launch(&source, forestx_home)
}

pub fn resolve_exe_for_launch(source: &Path, forestx_home: &Path) -> PathBuf {
    let Some(file_name) = source.file_name() else {
        return source.to_path_buf();
    };
    let destination = helper_bin_dir(forestx_home).join(file_name);
    match copy_from_source_if_needed(source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(forestx_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), forestx_home.display());
    ...
    let destination = helper_destination_for_source(kind, forestx_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(forestx_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let forestx_home = Path::new(r"C:\Users\example\.forestx");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.forestx\.sandbox-bin"),
            helper_bin_dir(forestx_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("forestx-command-runner.exe");
        fs::write(&runner_source, b"runner").expect("runner");
        let runner_suffix = helper_version_suffix(&runner_source).expect("runner suffix");
        let runner_destination = helper_bin_dir(&forestx_home).join(materialized_file_name(
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("forestx.exe");
        let helper = resources_dir.join("forestx-command-runner.exe");
        fs::write(&exe, b"forestx").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "forestx-command-runner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("forestx.exe");
        let package_helper = package_resources_dir.join("forestx-command-runner.exe");
        let bin_helper = bin_resources_dir.join("forestx-command-runner.exe");
        fs::write(&exe, b"forestx").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("forestx.exe");
        let sibling_helper = release_dir.join("forestx-command-runner.exe");
        let resource_helper = resources_dir.join("forestx-command-runner.exe");
        fs::write(&exe, b"forestx").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "forestx-command-runner-test-suffix.exe");
    }
}

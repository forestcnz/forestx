...
use super::FORESTX_HOME_FLAG;
use super::FORESTX_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "forestx.exe".to_string(),
            "--forestx-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.forestx"),
    );

    assert_eq!(args[0], FORESTX_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&FORESTX_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["forestx.exe", "--forestx-run-as-fs-helper"]
    );

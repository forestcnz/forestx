...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.forestx`)
...
pub fn cap_sid_file(forestx_home: &Path) -> PathBuf {
    forestx_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(forestx_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(forestx_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(forestx_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(forestx_home);
    let mut caps = load_or_create_cap_sids(forestx_home)?;
...
pub fn writable_root_cap_sid_for_path(forestx_home: &Path, root: &Path) -> Result<String> {
    let path = cap_sid_file(forestx_home);
    let mut caps = load_or_create_cap_sids(forestx_home)?;
...
pub fn workspace_write_cap_sid_for_root(
    ...
    if canonical_path_key(root) == canonical_path_key(cwd) {
        workspace_cap_sid_for_cwd(forestx_home, cwd)
    } else {
        writable_root_cap_sid_for_path(forestx_home, root)
    }
}
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&forestx_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&forestx_home, alt_spelling.as_path()).expect("second sid");
    ...
    fn write_roots_get_path_scoped_sids() {
        ...
        let extra_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &extra_root)
            .expect("extra root sid");

        assert_ne!(workspace_sid, extra_sid);
        assert_eq!(
            extra_sid,
            writable_root_cap_sid_for_path(&forestx_home, &extra_root).expect("extra root sid again")
        );

...
use anyhow::Context;
use anyhow::Result;
use forestx_utils_pty::JobObject;
use forestx_windows_sandbox::ConsoleMode;
use forestx_windows_sandbox::ErrorPayload;
use forestx_windows_sandbox::ErrorStage;
use forestx_windows_sandbox::ExitPayload;
use forestx_windows_sandbox::FramedMessage;
use forestx_windows_sandbox::IPC_PROTOCOL_VERSION;
use forestx_windows_sandbox::LocalSid;
use forestx_windows_sandbox::Message;
use forestx_windows_sandbox::OutputPayload;
use forestx_windows_sandbox::OutputStream;
use forestx_windows_sandbox::PipeSpawnHandles;
use forestx_windows_sandbox::ResizePayload;
use forestx_windows_sandbox::SpawnReady;
use forestx_windows_sandbox::SpawnRequest;
use forestx_windows_sandbox::StderrMode;
use forestx_windows_sandbox::StdinMode;
use forestx_windows_sandbox::WindowsSandboxTokenMode;
use forestx_windows_sandbox::allow_null_device;
use forestx_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use forestx_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use forestx_windows_sandbox::decode_bytes;
use forestx_windows_sandbox::encode_bytes;
use forestx_windows_sandbox::get_current_token_for_restriction;
use forestx_windows_sandbox::hide_current_user_profile_dir;
use forestx_windows_sandbox::log_note;
use forestx_windows_sandbox::read_frame;
use forestx_windows_sandbox::read_handle_loop;
use forestx_windows_sandbox::spawn_process_with_pipes;
use forestx_windows_sandbox::to_wide;
use forestx_windows_sandbox::token_mode_for_permission_profile;
use forestx_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with forestx_exec_server::FORESTX_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--forestx-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\ForestxSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<forestx_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.forestx_home.clone();
    hide_current_user_profile_dir(req.forestx_home.as_path());
    ...
    let (pi, job, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = forestx_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,

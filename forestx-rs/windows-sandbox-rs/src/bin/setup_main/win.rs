...
use forestx_otel::StatsigMetricsSettings;
use forestx_windows_sandbox::SETUP_VERSION;
use forestx_windows_sandbox::SetupErrorCode;
use forestx_windows_sandbox::SetupErrorReport;
use forestx_windows_sandbox::SetupFailure;
use forestx_windows_sandbox::add_deny_write_ace;
use forestx_windows_sandbox::convert_string_sid_to_sid;
use forestx_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use forestx_windows_sandbox::ensure_allow_write_aces;
use forestx_windows_sandbox::extract_setup_failure;
use forestx_windows_sandbox::hide_newly_created_users;
use forestx_windows_sandbox::install_wfp_filters;
use forestx_windows_sandbox::log_note;
use forestx_windows_sandbox::log_writer;
use forestx_windows_sandbox::path_mask_allows;
use forestx_windows_sandbox::path_write_aces_need_refresh;
use forestx_windows_sandbox::sandbox_bin_dir;
use forestx_windows_sandbox::sandbox_dir;
use forestx_windows_sandbox::sandbox_secrets_dir;
use forestx_windows_sandbox::string_from_sid_bytes;
use forestx_windows_sandbox::sync_persistent_deny_read_acls;
use forestx_windows_sandbox::to_wide;
use forestx_windows_sandbox::workspace_write_cap_sid_for_root;
use forestx_windows_sandbox::workspace_write_root_overlaps_path;
use forestx_windows_sandbox::write_setup_error_report;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(forestx_home) = std::env::var("FORESTX_HOME") {
            let sbx_dir = sandbox_dir(Path::new(&forestx_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.forestx_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.forestx_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if writes_setup_marker {
        prepare_setup_marker(&payload.forestx_home, &payload.real_user)?;
    }
    match payload.mode {
...
fn lock_persistent_sandbox_dirs(
    ...
    lock_sandbox_dir(
        &sandbox_dir(&payload.forestx_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_dir(&payload.forestx_home).display()
            ),
        ))
    })?;
    lock_sandbox_dir(
        &sandbox_secrets_dir(&payload.forestx_home),
        &payload.real_user,
        sandbox_group_sid,
        DENY_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_secrets_dir(&payload.forestx_home).display()
            ),
        ))
    })?;
    let legacy_users = sandbox_dir(&payload.forestx_home).join("sandbox_users.json");
...
fn lock_sandbox_bin_dir(
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.forestx_home),
        &payload.real_user,
        sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.forestx_home).display()
            ),
        ))
    })
}
...
fn run_setup_full(payload: &Payload, log: &mut dyn Write, sbx_dir: &Path) -> Result<()> {
    ...
    if refresh_only {
        setup_runtime_bin::ensure_forestx_app_runtime_paths_readable(
            sandbox_group_psid,
            &mut refresh_errors,
            log,
        )?;
    ...
    for root in &payload.write_roots {
        ...
        let root_cap_sid_str =
            workspace_write_cap_sid_for_root(&payload.forestx_home, &payload.command_cwd, root)?;
    ...
    for path in &payload.deny_write_paths {
        ...
        // legacy protected children such as `.git`, `.forestx`, and `.agents`.
        //
...
mod tests {
    ...
    use forestx_otel::StatsigMetricsSettings;
    use forestx_windows_sandbox::ensure_allow_mask_aces;
    use forestx_windows_sandbox::ensure_allow_write_aces;
    use forestx_windows_sandbox::load_or_create_cap_sids;
    use forestx_windows_sandbox::path_mask_allows;
    use forestx_windows_sandbox::path_write_aces_need_refresh;
    use forestx_windows_sandbox::workspace_write_cap_sid_for_root;
    ...
    fn payload_json() -> serde_json::Value {
        json!({
            ...
            "offline_username": "ForestxSandboxOffline",
            "online_username": "ForestxSandboxOnline",
            "forestx_home": "C:\\forestx-home",
    ...
    fn write_root_refresh_replaces_stale_delete_child_grant() {
        ...
        let forestx_home = temp.path().join("forestx-home");
        let workspace = temp.path().join("workspace");
        fs::create_dir_all(&forestx_home).expect("create forestx home");
        fs::create_dir_all(&workspace).expect("create workspace");

        let sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &workspace)
    ...
    fn write_root_refresh_checks_each_sid() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &workspace)
            .expect("workspace sid");
        let other_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &other_root)
    ...
    fn deny_path_under_active_root_uses_only_matching_root_sid() {
        ...
        let stale_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &stale_root)
            .expect("stale sid");
        let active_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &active_root)
            .expect("active sid");
        let workspace_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &workspace)
            .expect("workspace sid");
        let caps = load_or_create_cap_sids(&forestx_home).expect("load caps");
    ...
    fn deny_path_includes_nested_active_root_sid() {
        ...
        let forestx_home = temp.path().join("forestx-home");
        let workspace = temp.path().join("workspace");
        let protected_dir = workspace.join(".forestx");
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &workspace)
            .expect("workspace sid");
        let nested_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &nested_root)
            .expect("nested sid");

...
use forestx_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "ForestxSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Forestx sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(forestx_home);
...
pub(super) fn prepare_setup_marker(forestx_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(forestx_home).join("setup_marker.json");

...
fn connect_pipe_with_timeout(
    ...
    let mut connect_thread = Some(
        thread::Builder::new()
            .name(format!("forestx-runner-connect-{pipe_label}"))
            .spawn(move || {
...
pub(crate) fn spawn_runner_transport(
    ...
    let runner_exe = find_runner_exe(forestx_home, log_dir);
    let runner_cmdline = runner_exe
        .to_str()
        .map(str::to_owned)
        .unwrap_or_else(|| "forestx-command-runner.exe".to_string());
    ...
    if let Err(err) = connect_result {
        unsafe {
            ...
            // that child instead of leaking a stray `forestx-command-runner.exe`.
            if pi.hProcess != 0 {
    ...
    let startup_result = (|| -> Result<()> {
        ...
        // runner instead of leaving a stray `forestx-command-runner.exe` behind.

...
pub fn sandbox_setup_is_complete(forestx_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(forestx_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(forestx_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(forestx_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(forestx_home);
...
fn load_users(forestx_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(forestx_home);
...
fn remove_sandbox_users_file(forestx_home: &Path, reason: &str) -> Result<()> {
    let path = sandbox_users_path(forestx_home);
    debug_log(
...
fn select_identity(
    ...
    let _marker = match load_marker(forestx_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(forestx_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(forestx_home);
    let needed_read = read_roots_override
        .map(<[PathBuf]>::to_vec)
        .unwrap_or_else(|| gather_read_roots(command_cwd, permissions, env_map, forestx_home));
    ...
    let marker = load_marker(forestx_home)?;
    ...
    // NOTE: Do not add FORESTX_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match marker {
        Some(marker) if marker.version_matches() => {
                ...
                marker.request_mismatch_reason(network_identity, &desired_offline_proxy_settings)
            {
                ...
                let selected = select_identity(network_identity, forestx_home)?;
                if selected.is_none() {
                    setup_reason = Some(
    ...
    if identity.is_none() {
        ...
        identity = select_identity(network_identity, forestx_home)?;
    }
...
pub(crate) fn refresh_logon_sandbox_creds(
    ...
    remove_sandbox_users_file(forestx_home, "sandbox user login failed")?;
...
mod tests {
    ...
    fn remove_sandbox_users_file_deletes_existing_file() {
        ...
        let users_path = sandbox_users_path(forestx_home.path());
        ...
        remove_sandbox_users_file(forestx_home.path(), "stale creds").expect("remove users");

...
mod tests {
    ...
    #[test]
    fn denies_forestx_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let forestx_dir = command_cwd.join(".forestx");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&forestx_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&forestx_dir).unwrap(),

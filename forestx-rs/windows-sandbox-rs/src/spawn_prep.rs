...
use crate::sandbox_utils::ensure_forestx_home_exists;
...
use crate::workspace_acl::protect_workspace_forestx_dir;
use anyhow::Context;
use anyhow::Result;
...
fn prepare_spawn_context_common(
    ...
    ensure_forestx_home_exists(forestx_home)?;
    let sandbox_base = forestx_home.join(".sandbox");
...
pub(crate) fn prepare_legacy_session_security(
    ...
    let caps = load_or_create_cap_sids(forestx_home)?;
    let (h_token, readonly_sid, readonly_sid_str, write_root_sids) = unsafe {
        if uses_write_capabilities {
            let write_root_sids = root_capability_sids(forestx_home, cwd, capability_roots)?;
...
pub(crate) fn root_capability_sids(
    ...
    for root in roots {
        let sid_str = workspace_write_cap_sid_for_root(forestx_home, cwd, &root)?;
    ...
    unsafe {
                ...
                matching_root_capability(current_dir, acl_sids.write_root_sids)
        {
            ...
            if is_command_cwd_root(&workspace_sid.root, &canonical_cwd) {
                let _ = protect_workspace_forestx_dir(current_dir, workspace_sid.sid.as_ptr());
...
pub(crate) fn prepare_elevated_spawn_context_for_permissions(
    ...
    let sandbox_base = forestx_home.join(".sandbox");
    ensure_forestx_home_exists(&sandbox_base)?;
    ...
    let (psid_to_use, cap_sids) = if uses_write_capabilities {
        let cap_sids = root_capability_sids(forestx_home, cwd, effective_write_roots)?
            .into_iter()
...
mod tests {
    ...
    fn legacy_deny_path_includes_nested_active_root_sid() {
        ...
        let nested_sid = workspace_write_cap_sid_for_root(&forestx_home, &workspace, &nested_root)
            .expect("nested sid");
        let unrelated_sid =
            workspace_write_cap_sid_for_root(&forestx_home, &workspace, &unrelated_root)
    ...
    fn legacy_capability_roots_use_effective_write_roots() {
        ...
        let workspace = temp.path().join("workspace");
        let active_root = temp.path().join("active-root");
        let sandbox_root = forestx_home.join(".sandbox");
        std::fs::create_dir_all(&forestx_home).expect("create forestx home");
        std::fs::create_dir_all(&workspace).expect("create workspace");
        std::fs::create_dir_all(&active_root).expect("create active root");
        ...
        let writable_roots = vec![
            ...
            AbsolutePathBuf::try_from(forestx_home.as_path()).expect("forestx home"),
        ...
        let roots =
            legacy_session_capability_roots(&permissions, &workspace, &HashMap::new(), &forestx_home);
        ...
        assert!(!roots.contains(&dunce::canonicalize(&forestx_home).expect("forestx home")));

...
use forestx_apply_patch::FORESTX_CORE_APPLY_PATCH_ARG1;
#[cfg(unix)]
use forestx_exec_server::FORESTX_ARG0_EXEC_HELPER_ARG1;
use forestx_exec_server::FORESTX_FS_HELPER_ARG1;
use forestx_install_context::InstallContext;
use forestx_sandboxing::landlock::FORESTX_LINUX_SANDBOX_ARG0;
use forestx_utils_home_dir::find_forestx_home;
#[cfg(target_os = "windows")]
use forestx_windows_sandbox::FORESTX_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "forestx-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Forestx executable for child re-execs.
    ///
    ...
    /// instead of the real Forestx CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime.block_on(
            forestx_shell_escalation::run_shell_escalation_execve_wrapper(file, argv),
        );
        match exit_code {
    ...
    if exe_name == FORESTX_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        forestx_linux_sandbox::run_main();
    ...
    #[cfg(unix)]
    if argv1 == FORESTX_ARG0_EXEC_HELPER_ARG1 {
        forestx_exec_server::run_arg0_exec_helper_main();
    }
    if argv1 == FORESTX_FS_HELPER_ARG1 {
        forestx_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == FORESTX_WINDOWS_SANDBOX_ARG1 {
        forestx_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == FORESTX_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match forestx_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                match runtime.block_on(forestx_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    forestx_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!("Error: {FORESTX_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_forestx_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that Forestx will proceed successfully even if
...
/// While we want to deploy the Forestx CLI as a single executable for simplicity,
...
/// `forestx-linux-sandbox` we *directly* execute
/// [`forestx_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.forestx/.env` before creating any threads.
...
///     `forestx-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        forestx_self_exe: current_exe.clone(),
        forestx_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `forestx-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().forestx_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "FORESTX_";

/// Load env vars from ~/.forestx/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `FORESTX_`.
fn load_dotenv() {
    if let Ok(forestx_home) = find_forestx_home()
        && let Ok(iter) = dotenvy::from_path_iter(forestx_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `FORESTX_` keys.
...
///   with the hidden `--forestx-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of Forestx
/// CLI.
...
fn prepare_path_entry_for_forestx_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if forestx_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (forestx_home: {forestx_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a FORESTX_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = forestx_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {FORESTX_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        forestx_self_exe: std::env::current_exe().ok(),
        forestx_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(FORESTX_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("forestx-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(ForestxPackageLayout {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_forestx_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("forestx-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                forestx_self_exe: Some(PathBuf::from("/usr/bin/forestx")),
                forestx_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/forestx"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("forestx-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/forestx")),
            |paths| async move {

use forestx_config::HooksFile;
use forestx_utils_absolute_path::AbsolutePathBuf;
use forestx_utils_path_uri::PathConvention;
use forestx_utils_path_uri::PathUri;
use forestx_utils_plugins::AGENT_PLUGIN_MANIFEST_RELATIVE_PATH;
use forestx_utils_plugins::find_plugin_manifest_path;
...
pub type PluginManifest = forestx_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = forestx_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface = forestx_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    forestx_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = forestx_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = forestx_plugin::manifest::PluginManifest<PathUri>;
...
pub(crate) fn load_plugin_manifest_with_format(plugin_root: &Path) -> Option<LoadedPluginManifest> {
    ...
    let overlay = if is_agent_plugin {
        let overlay_path = plugin_root.join(".forestx-plugin/plugin.json");
...
fn resolve_raw_plugin_manifest(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = forestx_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(forestx_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: forestx_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<forestx_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| forestx_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty()).then_some(forestx_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => {
            Some(forestx_plugin::manifest::PluginManifestHooks::Inline(vec![
                *hooks,
            ]))
        }
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty())
            .then_some(forestx_plugin::manifest::PluginManifestHooks::Inline(hooks)),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<forestx_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(forestx_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(forestx_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use forestx_plugin::PluginProvider;
    use forestx_plugin::ResolvedPlugin;
    use forestx_plugin::manifest::PluginManifest as GenericPluginManifest;
    use forestx_plugin::manifest::PluginManifestHooks;
    use forestx_plugin::manifest::PluginManifestInterface;
    use forestx_plugin::manifest::PluginManifestMcpServers;
    use forestx_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {
    ...
    fn parse_uri_composer_icon(plugin_root: &PathUri, composer_icon: &str) -> Option<PathUri> {
        let manifest_path = plugin_root
            .join(".forestx-plugin/plugin.json")
            .expect("manifest URI");
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".forestx-plugin/plugin.json")).expect("manifest");

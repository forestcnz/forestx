...
const REMOTE_PLUGIN_INSTALL_METADATA_FILE: &str = ".forestx-remote-plugin-install.json";
...
impl PluginStore {
    pub fn new(forestx_home: PathBuf) -> Self {
        Self::try_new(forestx_home)
            .unwrap_or_else(|err| panic!("plugin cache root should be absolute: {err}"))
    }

    pub fn try_new(forestx_home: PathBuf) -> Result<Self, PluginStoreError> {
        let root = AbsolutePathBuf::from_absolute_path_checked(forestx_home.join(PLUGINS_CACHE_DIR))
            .map_err(|err| PluginStoreError::io("failed to resolve plugin cache root", err))?;
        let data_root =
            AbsolutePathBuf::from_absolute_path_checked(forestx_home.join(PLUGINS_DATA_DIR))
        ...
        let forestx_home = AbsolutePathBuf::from_absolute_path_checked(forestx_home)
            .map_err(|err| PluginStoreError::io("failed to resolve Forestx home", err))?;
...
fn plugin_manifest_for_source(
    ...
    match manifest {
        ...
        InstallManifest::Fallback(contents) => parse_plugin_manifest(
            source_path,
            &source_path.join(".forestx-plugin/plugin.json"),
            contents,
        )
...
fn replace_plugin_root_atomically(
    ...
    if let InstallManifest::Fallback(contents) = manifest {
        ...
        let manifest_path = staged_version_root.join(".forestx-plugin/plugin.json");

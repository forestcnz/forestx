...
fn git_command_sanitizes_ambient_repository_environment() {
    ...
    assert_eq!(
        ...
        [
            OsStr::new("-c"),
            OsStr::new(forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG),
        ]
    );
...
async fn ordinary_clone_rejects_tracked_embedded_bare_repository() {
    ...
    run_setup_git(
        ...
        &[
            "-c",
            "user.name=Forestx Tests",
            "-c",
            "user.email=forestx-tests@example.com",
            "commit",
            "--quiet",
            "-m",
    ...
    let apply_error = forestx_git_utils::apply_git_patch(&forestx_git_utils::ApplyGitRequest {
        cwd: nested.clone(),
        diff: String::new(),
        revert: false,
        preflight: true,
    ...
    assert!(forestx_git_utils::collect_git_info(&nested).await.is_none());
    assert!(forestx_git_utils::git_diff_to_remote(&nested).await.is_none());
    assert!(
        !marker.exists(),
...
fn write_curated_plugin_sha(forestx_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(forestx_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(forestx_home.join(".tmp")) else {
        return false;
    };
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.forestx-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_forestx_home_tmp_dir() {
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.forestx-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.forestx-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    run_git(
        ...
        &[
            "-c",
            "user.name=Forestx Test",
            "-c",
            "user.email=forestx@example.com",
            "commit",
            "-m",
            "init",
        ],
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.forestx-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.forestx-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.forestx-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.forestx-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.forestx-plugin/plugin.json"),
            options,
        )
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        .expect("write marketplace");
    writer
        .start_file("plugins/plugins/gmail/.forestx-plugin/plugin.json", options)
        .expect("start plugin manifest entry");
    writer
        .write_all(br#"{"name":"gmail"}"#)
        .expect("write plugin manifest");

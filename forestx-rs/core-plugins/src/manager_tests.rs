...
use forestx_config::McpServerOAuthConfig;
use forestx_config::McpServerToolConfig;
use forestx_config::RequirementSource;
use forestx_config::RequirementsLayerEntry;
use forestx_config::compose_requirements;
use forestx_config::types::McpServerTransportConfig;
use forestx_core_skills::PluginSkillSnapshots;
use forestx_login::ForestxAuth;
use forestx_plugin::AppDeclaration;
use forestx_plugin::PluginId;
use forestx_protocol::auth::AuthMode;
use forestx_protocol::protocol::HookEventName;
use forestx_protocol::protocol::Product;
use forestx_skills::SkillConfigRules;
use forestx_skills_extension::HostSkillsLoadInput;
use forestx_skills_extension::HostSkillsService;
use forestx_utils_absolute_path::AbsolutePathBuf;
use forestx_utils_absolute_path::test_support::PathBufExt;
use forestx_utils_plugins::SkillDiscoveryMode;
...
fn config_layer_stack_with_requirements(
    ...
    let config_file =
        AbsolutePathBuf::try_from(forestx_home.join(CONFIG_TOML_FILE)).expect("absolute config path");
...
fn plugins_config_input_with_requirements(
    ...
    PluginsConfigInput::new(
        config_layer_stack_with_requirements(forestx_home, user_config, requirements),
        String::new(),
...
async fn marketplace_policy_projection_disables_installed_plugin_and_invalidates_cache() {
    let forestx_home = TempDir::new().expect("create Forestx home");
    write_plugin(
        &forestx_home.path().join("plugins/cache/company"),
        "sample/local",
        "sample",
    );
    let user_config = r#"
...
"#,
    ...
    let manager = PluginsManager::new(forestx_home.path().to_path_buf());
...
async fn plugin_read_rejects_marketplace_blocked_by_requirements() {
    let forestx_home = TempDir::new().expect("create Forestx home");
    let marketplace_root = forestx_home.path().join("marketplace");
...
"#,
    ...
    let err = PluginsManager::new(forestx_home.path().to_path_buf())
...
fn marketplace_policy_filters_discovered_marketplaces_by_configured_name() {
    let forestx_home = TempDir::new().expect("create Forestx home");
    let repo_root = forestx_home.path().join("repo");
...
fn write_auth_projection_plugin(forestx_home: &Path, name: &str, include_app: bool) {
  ...
  "mcpServers": {{
    ...
    if include_app {
        write_auth_projection_app(forestx_home, name, name);
    }
}

fn write_auth_projection_app(forestx_home: &Path, plugin_name: &str, app_name: &str) {
    let plugin_root = forestx_home
        .join("plugins/cache")
        .join("test")
        .join(plugin_name)
...
async fn auth_projection_config(forestx_home: &Path) -> PluginsConfigInput {
...
"#
    .to_string();
    write_file(&forestx_home.join(CONFIG_TOML_FILE), &config_toml);
    load_config(forestx_home, forestx_home).await
}
...
async fn plugin_auth_projection_hides_apps_without_chatgpt_auth() {
    let forestx_home = TempDir::new().unwrap();
    write_auth_projection_plugin(forestx_home.path(), "sample", /*include_app*/ true);
    write_auth_projection_plugin(forestx_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(forestx_home.path()).await;
...
async fn plugin_auth_projection_keeps_non_conflicting_mcp_with_chatgpt_apps_route() {
    let forestx_home = TempDir::new().unwrap();
    write_auth_projection_plugin(forestx_home.path(), "sample", /*include_app*/ false);
    write_auth_projection_app(forestx_home.path(), "sample", "sample_app");
    write_auth_projection_plugin(forestx_home.path(), "docs", /*include_app*/ false);
    let config = auth_projection_config(forestx_home.path()).await;
    let manager = PluginsManager::new_with_options(
        forestx_home.path().to_path_buf(),
...
"#,
    ...
    let config = load_config(forestx_home.path(), forestx_home.path()).await;
    let manager = PluginsManager::new_with_options(
        forestx_home.path().to_path_buf(),
        Some(Product::Forestx),
        Some(AuthMode::Chatgpt),
...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".forestx-plugin")).unwrap();
...
fn init_git_repo(repo: &Path) {
    run_git(repo, &["init"]);
    run_git(repo, &["config", "user.email", "forestx-test@example.com"]);
    run_git(repo, &["config", "user.name", "Forestx Test"]);
    run_git(repo, &["add", "."]);
...
async fn load_plugins_from_config(
    ...
    write_file(&forestx_home.join(CONFIG_TOML_FILE), config_toml);
    let config = load_config(forestx_home, forestx_home).await;
    PluginsManager::new_with_options(forestx_home.to_path_buf(), Some(Product::Forestx), auth_mode)
        .plugins_for_config(&config)
        .await
}

async fn load_config(forestx_home: &Path, cwd: &Path) -> PluginsConfigInput {
    load_plugins_config_input(forestx_home, cwd).await
}
...
fn remote_installed_plugin_in_marketplace(
    ...
    RemoteInstalledPlugin {
        ...
        installed_at: None,
        enabled: true,
        install_policy: forestx_app_server_protocol::PluginInstallPolicy::Available,
        install_policy_source: None,
        must_show_installation_interstitial: None,
        auth_policy: forestx_app_server_protocol::PluginAuthPolicy::OnUse,
        availability: forestx_app_server_protocol::PluginAvailability::Available,
...
fn write_cached_plugin(forestx_home: &Path, marketplace_name: &str, plugin_name: &str) {
    write_plugin_with_version(
        &forestx_home
            .join("plugins/cache")
...
"#;
    let outcome =
        load_plugins_from_config(config_toml, forestx_home.path(), /*auth_mode*/ None).await;
...
async fn remote_installed_plugin_preserves_configured_mcp_server_policy() {
    ...
    write_cached_plugin(forestx_home.path(), "openai-curated-remote", "linear");
    write_file(
        &plugin_root.join(".mcp.json"),
        r#"{
  "mcpServers": {
...
async fn installed_plugin_telemetry_metadata_collects_capabilities() {
    let forestx_home = TempDir::new().unwrap();
    write_cached_plugin(forestx_home.path(), "test", "sample");
    let manager = PluginsManager::new(forestx_home.path().to_path_buf());
...
async fn installed_plugin_telemetry_metadata_resolves_persisted_remote_identity() {
    ...
    PluginStore::new(forestx_home.path().to_path_buf())
...
"#,
    ...
    write_cached_plugin(forestx_home.path(), "openai-curated", "linear");
    write_cached_plugin(forestx_home.path(), "openai-curated", "calendar");
    write_cached_plugin(forestx_home.path(), "openai-api-curated", "linear");
    write_cached_plugin(forestx_home.path(), "openai-curated-remote", "linear");
    write_cached_plugin(forestx_home.path(), "openai-curated-remote", "remote-only");
...
"#,
    ...
    let mut config = load_config(forestx_home.path(), forestx_home.path()).await;
    let manager = PluginsManager::new_with_options(
        forestx_home.path().to_path_buf(),
        Some(Product::Forestx),
        Some(AuthMode::Chatgpt),
...
async fn build_remote_installed_plugin_marketplaces_from_cache_uses_remote_metadata() {
    ...
    plugin.install_policy = forestx_app_server_protocol::PluginInstallPolicy::InstalledByDefault;
    plugin.auth_policy = forestx_app_server_protocol::PluginAuthPolicy::OnInstall;
    plugin.interface = Some(forestx_app_server_protocol::PluginInterface {
    ...
    assert_eq!(
        ...
        forestx_app_server_protocol::PluginInstallPolicy::InstalledByDefault
    );
    assert_eq!(
        plugin.auth_policy,
        forestx_app_server_protocol::PluginAuthPolicy::OnInstall
    );
...
async fn install_plugin_materializes_default_command_skills() {
    let forestx_home = TempDir::new().unwrap();
    let source_root = forestx_home.path().join("source/sample");

    write_file(
        &source_root.join(".forestx-plugin/plugin.json"),
        r#"{
  "name": "sample",
  "skills": "./custom-skills/"
    ...
    write_file(
        &source_root.join(".forestx-plugin/migrated-command-skills/undeclared-command/SKILL.md"),
        "---\nname: undeclared-command\ndescription: undeclared command\n---\n",
    );
    let result = PluginStore::new(forestx_home.path().to_path_buf())
        .install(
            source_root.abs(),
            PluginId::parse("sample@test").expect("plugin id should parse"),
        )
    ...
    let migrated_skill = result
        .installed_path
        .join(".forestx-plugin/migrated-command-skills/source-command-pr-review/SKILL.md");
    ...
    assert!(
        !result
            .installed_path
            .join(".forestx-plugin/migrated-command-skills/undeclared-command")
            .exists()
    );
    assert!(
        !result
            .installed_path
            .join(".forestx-plugin/migrated-command-skills/source-command-oversized")
            .exists()
    );
        ...
        vec![
            ...
            AbsolutePathBuf::from_absolute_path_checked(
                fs::canonicalize(result.installed_path.join(
                    ".forestx-plugin/migrated-command-skills/source-command-summarize/SKILL.md"
                ))
                .unwrap()
            )
            .unwrap()
...
fn install_plugin_ignores_invalid_commands_manifest_field() {
    ...
    assert!(
        !result
            .installed_path
            .join(".forestx-plugin/migrated-command-skills")
            .exists()
    );
}
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    let forestx_home = TempDir::new().unwrap();
    let plugin_a_root = forestx_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-a/local");
    let plugin_b_root = forestx_home
        .path()
        .join("plugins/cache")
        .join("test/plugin-b/local");

    write_file(
        &plugin_a_root.join(".forestx-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".forestx-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    let outcome =
        load_plugins_from_config(&config_toml, forestx_home.path(), Some(AuthMode::Chatgpt)).await;
...
fn capability_index_filters_inactive_and_zero_capability_plugins() {
    ...
    let plugin = |config_name: &str, dir_name: &str, manifest_name: &str| LoadedPlugin {
        ...
        manifest_description: None,
        root: AbsolutePathBuf::try_from(forestx_home.path().join(dir_name)).unwrap(),
        enabled: true,
        skill_roots: Vec::new(),
        skill_discovery_mode: SkillDiscoveryMode::Recursive,
        disabled_skill_paths: HashSet::new(),
    ...
    let outcome = PluginLoadOutcome::from_plugins(vec![
        LoadedPlugin {
            skill_roots: vec![forestx_home.path().join("skills-plugin/skills").abs()],
            has_enabled_skills: true,
        ...
        LoadedPlugin {
            enabled: false,
            skill_roots: vec![forestx_home.path().join("disabled-plugin/skills").abs()],
...
async fn load_plugins_returns_empty_when_feature_disabled() {
    ...
    let config = load_config(forestx_home.path(), forestx_home.path()).await;
    let outcome = PluginsManager::new(forestx_home.path().to_path_buf())
        .plugins_for_config(&config)
        .await;
...
async fn plugin_cache_ignores_unrelated_session_overrides() {
    ...
    let plugin_root = forestx_home
        ...
        .join("test/sample/local");
    write_plugin(
        forestx_home.path().join("plugins/cache/test").as_path(),
        "sample/local",
        "sample",
    );
    write_file(
  ...
  "mcpServers": {
    ...
    let user_file = forestx_home.path().join(CONFIG_TOML_FILE).abs();
...
async fn skill_snapshots_resolve_remote_plugin_identity_from_authoritative_source() {
    ...
    for (installed_plugins, expected_remote_plugin_id) in [
        ...
        let forestx_home = TempDir::new().unwrap();
        let forestx_home_abs = forestx_home.path().to_path_buf().abs();
        let plugin_root = forestx_home
            .path()
...
"#,
        ...
        let skills_service =
            HostSkillsService::new(forestx_home_abs, /*bundled_skills_enabled*/ false);
...
async fn strict_install_requires_allowed_local_marketplace_to_be_added_first() {
    let forestx_home = TempDir::new().expect("create Forestx home");
    let marketplace_root = forestx_home.path().join("company-marketplace");
    write_plugin(&marketplace_root, "sample", "sample");
    write_file(
        &marketplace_root.join(".agents/plugins/marketplace.json"),
        r#"{
...
"#
    ...
    let config = config_layer_stack_with_requirements(forestx_home.path(), "", &requirements);
    let marketplace_path =
        AbsolutePathBuf::try_from(marketplace_root.join(".agents/plugins/marketplace.json"))
            .expect("absolute marketplace path");
    let manager = PluginsManager::new(forestx_home.path().to_path_buf());
    ...
    assert!(!forestx_home.path().join(CONFIG_TOML_FILE).exists());
...
"#
    ...
    write_file(&forestx_home.path().join(CONFIG_TOML_FILE), &user_config);
    let config =
        config_layer_stack_with_requirements(forestx_home.path(), &user_config, &requirements);
    let outcome = manager
        .install_plugin(
            &config,
            PluginInstallRequest {
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".forestx-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".forestx-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        fs::read_to_string(
            installed_path
                .join(".forestx-plugin/migrated-command-skills/source-command-review/SKILL.md")
        )
        .unwrap(),
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".forestx-plugin/plugin.json").is_file());
}
...
#[tokio::test]
async fn read_plugin_for_config_filters_mcp_servers_for_forestx_backend_auth() {
    let tmp = tempfile::tempdir().unwrap();
    let repo_root = tmp.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).unwrap();
    fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.forestx-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".forestx-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".forestx/config.toml"),
        r#"[[skills.config]]
...
async fn read_plugin_for_config_installed_git_source_reads_from_cache_without_cloning() {
  ...
  "plugins": [
    ...
    write_file(
        &cached_plugin_root.join(".forestx-plugin/plugin.json"),
        r#"{
  "name": "toolkit",
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.forestx-plugin/plugin.json"),
...
"#,
    ...
    let featured_plugin_ids = manager
        .featured_plugin_ids_for_config(
            &config,
            Some(&ForestxAuth::create_dummy_chatgpt_auth_for_testing()),
        )
        .await
        .unwrap();
...
#[tokio::test]
async fn featured_plugin_ids_for_config_defaults_query_param_to_forestx() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    Mock::given(method("GET"))
        .and(path("/backend-api/plugins/featured"))
        .and(query_param("platform", "forestx"))
        .respond_with(ResponseTemplate::new(200).set_body_string(r#"["forestx-plugin"]"#))
        .mount(&server)
        .await;
    ...
    assert_eq!(featured_plugin_ids, vec!["forestx-plugin".to_string()]);
}
...
"#,
    ...
    Mock::given(method("GET"))
        ...
        .and(header("authorization", "Bearer Access Token"))
        .and(header("chatgpt-account-id", "account_id"))
        .and(header("OAI-Product-Sku", "forestx"))
        .respond_with(
...
#[test]
fn curated_plugin_ids_from_config_keys_reads_latest_forestx_home_user_config() {
    let tmp = tempfile::tempdir().unwrap();
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
...
"#,
    ...
    assert_eq!(
        configured_curated_plugin_ids_from_forestx_home(tmp.path())
            .into_iter()
...
async fn load_plugins_ignores_project_config_files() {
    let forestx_home = TempDir::new().unwrap();
    let project_root = forestx_home.path().join("project");
    let plugin_root = forestx_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    ...
    write_file(
        &project_root.join(".forestx/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_forestx_folder: AbsolutePathBuf::try_from(project_root.join(".forestx")).unwrap(),
            },
...
"#,
    ...
    for marketplace_name in [
        ...
        write_cached_plugin(forestx_home.path(), marketplace_name, "linear");
        write_file(
            &forestx_home
                .path()
                .join("plugins/cache")

...
use forestx_config::RemoveMarketplaceConfigOutcome;
use forestx_config::remove_user_marketplace_config;
...
pub async fn remove_marketplace(
    ...
    tokio::task::spawn_blocking(move || remove_marketplace_sync(forestx_home.as_path(), request))
        .await
        .map_err(|err| {
...
fn remove_marketplace_sync(
    ...
    let destination = marketplace_install_root(forestx_home).join(&marketplace_name);
    let config_outcome =
        remove_user_marketplace_config(forestx_home, &marketplace_name).map_err(|err| {
...
mod tests {
    ...
    fn remove_marketplace_sync_removes_config_and_installed_root() {
        ...
        .unwrap();
        let installed_root = marketplace_install_root(forestx_home.path()).join("debug");
        ...
        let config =
            fs::read_to_string(forestx_home.path().join(forestx_config::CONFIG_TOML_FILE)).unwrap();

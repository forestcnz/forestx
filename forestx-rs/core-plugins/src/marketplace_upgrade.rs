...
fn upgrade_configured_git_marketplace(
    ...
    activate_marketplace_root(&destination, staged_dir, || {
        ensure_configured_git_marketplace_unchanged(forestx_home, marketplace)?;
        record_user_marketplace(forestx_home, &marketplace.name, &update).map_err(|err| {
            format!(
...
fn ensure_configured_git_marketplace_unchanged(
    ...
    let current = read_configured_git_marketplace(forestx_home, &expected.name)?;
    match current {

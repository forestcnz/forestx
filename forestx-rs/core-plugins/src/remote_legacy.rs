...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    url.query_pairs_mut().append_pair(
        "platform",
        product.unwrap_or(Product::Forestx).to_app_platform(),
    );
    ...
    if let Some(auth) = auth.filter(|auth| auth.uses_forestx_backend()) {
        request =
            request.headers(forestx_model_provider::auth_provider_from_auth(auth).to_auth_headers());
    }
...
fn ensure_forestx_backend_auth(
    auth: Option<&ForestxAuth>,
) -> Result<&ForestxAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_forestx_backend_auth(auth)?;

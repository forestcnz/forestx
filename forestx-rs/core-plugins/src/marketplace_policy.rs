...
use forestx_config::MarketplaceAllowedSourceKind;
use forestx_config::MarketplaceAllowedSourceToml;
use forestx_config::RequirementSource;
use forestx_config::types::MarketplaceConfig;
use forestx_config::types::MarketplaceSourceType;
use forestx_config::types::PluginConfig;
use forestx_plugin::PluginId;
use forestx_utils_absolute_path::AbsolutePathBuf;
use forestx_utils_path::paths_match_after_normalization;
use regex::Regex;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) = managed_marketplace_name(forestx_home, marketplace_path, &root) {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(forestx_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn project_effective_user_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&user_config, &policy, forestx_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(forestx_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(user_config) = project_effective_user_config(config_layer_stack, forestx_home) else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(forestx_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if paths_match_after_normalization(
        ...
        curated_plugins_api_marketplace_path(forestx_home),
    ) {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if paths_match_after_normalization(root.as_path(), curated_plugins_repo_path(forestx_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(forestx_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(forestx_home, path)
}

fn managed_local_marketplace_name(forestx_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = forestx_home
    ...
    let runtime_root = dirs::cache_dir()?
        .join("forestx-runtimes/forestx-primary-runtime/plugins")

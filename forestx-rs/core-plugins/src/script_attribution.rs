...
use forestx_shell_command::bash::extract_bash_command;
use forestx_shell_command::bash::parse_shell_lc_plain_commands;
use forestx_shell_command::parse_command::is_pathish;
...
impl TrustedPluginRoots {
    pub fn from_plugin_load_outcome(loaded_plugins: &PluginLoadOutcome, forestx_home: &Path) -> Self {
        let Ok(store) = PluginStore::try_new(forestx_home.to_path_buf()) else {
            return Self::default();
        };
            ...
            .filter_map(|plugin| {
                ...
                let expected_root = match plugin_id.marketplace_name.as_str() {
                    ...
                    OPENAI_CURATED_MARKETPLACE_NAME | OPENAI_API_CURATED_MARKETPLACE_NAME => {
                        let curated_sha = read_curated_plugins_sha(forestx_home)?;

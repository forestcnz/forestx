...
use forestx_utils_plugins::AGENT_PLUGIN_SCHEMA_PREFIX;
use forestx_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
use forestx_utils_plugins::SUPPORTED_AGENT_PLUGIN_SCHEMA_URIS;
use serde::Deserialize;
use serde_json::Value as JsonValue;

const FORESTX_AGENT_PLUGIN_EXTENSION_NAMESPACE: &str = "com.openai";
...
pub(super) fn parse_agent_plugin_manifest_uri(
    ...
    let forestx_extension = object
        .get("extensions")
        .and_then(JsonValue::as_object)
        .and_then(|extensions| extensions.get(FORESTX_AGENT_PLUGIN_EXTENSION_NAMESPACE))
        .and_then(|extension| {
            if extension.is_object() {
                ...
                tracing::warn!(
                    path = %manifest_path,
                    namespace = FORESTX_AGENT_PLUGIN_EXTENSION_NAMESPACE,
    ...
    if let Some(extension_contents) = forestx_extension.as_deref() {
        apply_forestx_agent_plugin_extension(
            &mut resolved,
            plugin_root,
            manifest_path,
            extension_contents,
...
fn apply_forestx_agent_plugin_extension(

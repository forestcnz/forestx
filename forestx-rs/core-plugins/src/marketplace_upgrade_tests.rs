...
"#,
    ...
    assert_eq!(
        read_configured_git_marketplace(forestx_home.path(), "good")
...
fn one_upgrade_failure_does_not_block_another_marketplace() {
    ...
    let good_url = url::Url::from_directory_path(remote_repo.path())
        ...
        .to_string();
    let missing_url = url::Url::from_directory_path(forestx_home.path().join("missing-repository"))
...
"#
    ...
    std::fs::write(forestx_home.path().join(CONFIG_TOML_FILE), &config).expect("write config");
    let stack = config_layer_stack(forestx_home.path(), &config);
    ...
    assert_eq!(
        ...
        vec![
            AbsolutePathBuf::try_from(marketplace_install_root(forestx_home.path()).join("good"))
...
fn upgrade_uses_validated_source_for_git_operations() {
    ...
    let normalized_url = url::Url::from_directory_path(remote_repo.path())
        .expect("remote repository URL")
        .to_string();
    let raw_source = forestx_home.path().join("missing-raw-source");
...
"#
    ...
    let install_root = marketplace_install_root(forestx_home.path());
...
fn up_to_date_fast_path_validates_marketplace_name() {
    ...
    let missing_source = forestx_home.path().join("missing-source");
...
fn config_layer_stack(forestx_home: &Path, config: &str) -> ConfigLayerStack {
    let config_file =

  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new("local-plugin".to_string(), "forestx-curated".to_string())
                .unwrap(),
...
fn find_marketplace_plugin_supports_git_subdir_sources() {
  ...
  "plugins": [
    ...
    assert_eq!(
        resolved,
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new("remote-plugin".to_string(), "forestx-curated".to_string())
                .unwrap(),
...
fn find_marketplace_plugin_supports_npm_sources() {
  ...
  "plugins": [
    ...
    assert_eq!(
        resolved,
        ResolvedMarketplacePlugin {
            plugin_id: PluginId::new("npm-plugin".to_string(), "forestx-curated".to_string())
                .unwrap(),
            source: MarketplacePluginSource::Npm {
                package: "@acme/forestx-plugin".to_string(),
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `remote-plugin` was not found in marketplace `forestx-curated`"
        );
    }
}
...
fn find_marketplace_plugin_reports_missing_plugin() {
    ...
    fs::write(
        repo_root.join(".agents/plugins/marketplace.json"),
        r#"{"name":"forestx-curated","plugins":[]}"#,
    )
    .unwrap();
    ...
    assert_eq!(
        err.to_string(),
        "plugin `missing` was not found in marketplace `forestx-curated`"
    );
}
...
fn list_marketplaces_supports_repo_root_local_plugin_sources() {
    for path in [".", "./"] {
        ...
        fs::create_dir_all(repo_root.join(".git")).unwrap();
        fs::create_dir_all(repo_root.join(".agents/plugins")).unwrap();
        fs::create_dir_all(repo_root.join(".forestx-plugin")).unwrap();
        fs::write(
            repo_root.join(".agents/plugins/marketplace.json"),
            format!(
                r#"{{
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        .unwrap();
        fs::write(
            repo_root.join(".forestx-plugin/plugin.json"),
            r#"{
  "name":"repo-root-plugin",
  "interface": {
...
fn list_marketplaces_returns_home_and_repo_marketplaces() {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        vec![
            Marketplace {
                name: "forestx-curated".to_string(),
                path:
...
fn list_marketplaces_resolves_plugin_interface_paths_to_absolute() {
  ...
  "plugins": [
    {
      ...
      "policy": {
        ...
        "products": ["FORESTX", "CHATGPT", "ATLAS"]
      },
      "category": "Design"
    }
  ]
  ...
  "interface": {
    ...
    assert_eq!(
        ...
        Some(vec![Product::Forestx, Product::Chatgpt, Product::Atlas])
    );
    assert_eq!(
  ...
  "plugins": [
    {{
      ...
      "source": {{
        ...
        assert_eq!(
            err.to_string(),
            "plugin `local-plugin` was not found in marketplace `forestx-curated`"
        );
    }
}
...
fn find_installable_marketplace_plugin_rejects_disallowed_product() {
  ...
  "plugins": [
    ...
    assert_eq!(
        err.to_string(),
        "plugin `chatgpt-plugin` is not available for install in marketplace `forestx-curated`"
    );
}
...
fn find_marketplace_plugin_allows_missing_products_field() {
  ...
  "plugins": [
    ...
    assert_eq!(resolved.plugin_id.as_key(), "default-plugin@forestx-curated");
}
...
fn find_installable_marketplace_plugin_rejects_explicit_empty_products() {
  ...
  "plugins": [
    ...
    assert_eq!(
        err.to_string(),
        "plugin `disabled-plugin` is not available for install in marketplace `forestx-curated`"
    );
}

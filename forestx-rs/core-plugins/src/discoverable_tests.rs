...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_forestx_auth() {
    let forestx_home = tempdir().expect("tempdir should succeed");
    write_file(
...
"#,
    ...
    let curated_root = curated_plugins_repo_path(forestx_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack", "openai-developers"]);

    let plugins = load_plugins_config(forestx_home.path(), forestx_home.path()).await;
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = ForestxAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(forestx_home.path(), curated_root.as_path(), "teams").await;
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_forestx_auth() {
    let forestx_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(forestx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = forestx_home
        .path()
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(forestx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.forestx-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
"#
    ...
    install_marketplace_plugin(forestx_home.path(), curated_root.as_path(), "installed").await;
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.forestx-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(forestx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(forestx_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.forestx-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(logs.matches("gmail/.forestx-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("openai-developers/.forestx-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = forestx_home
        .path()
...
"#
    ...
    install_marketplace_plugin(forestx_home.path(), sales_marketplace_root.as_path(), "sales").await;
...
"#,
    ...
    let auth = ForestxAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(forestx_home.path(), forestx_home.path()).await;
...
async fn install_marketplace_plugin(forestx_home: &Path, marketplace_root: &Path, plugin_name: &str) {
    write_curated_plugin_sha_with(forestx_home, TEST_CURATED_PLUGIN_SHA);
    let config = load_plugins_config(forestx_home, marketplace_root).await;
    PluginsManager::new(forestx_home.to_path_buf())
        .install_plugin(

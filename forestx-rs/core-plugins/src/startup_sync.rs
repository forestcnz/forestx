...
use forestx_otel::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use forestx_otel::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use http::Method;
...
const CURATED_PLUGINS_FETCH_REF: &str = "refs/forestx/curated-sync";
...
// Keep this comfortably above a normal sync attempt so we do not race another Forestx process.
...
pub fn curated_plugins_repo_path(forestx_home: &Path) -> PathBuf {
    forestx_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub fn curated_plugins_api_marketplace_path(forestx_home: &Path) -> PathBuf {
    curated_plugins_repo_path(forestx_home).join(".agents/plugins/api_marketplace.json")
}

pub fn read_curated_plugins_sha(forestx_home: &Path) -> Option<String> {
    read_sha_file(curated_plugins_sha_path(forestx_home).as_path())
}

fn curated_plugins_sha_path(forestx_home: &Path) -> PathBuf {
    forestx_home.join(CURATED_PLUGINS_SHA_FILE)
}
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    let _file_guard = lock_curated_plugins_startup_sync(forestx_home)?;

    let git_sync_result = match git_binary {
        Some(git_binary) => sync_openai_plugins_repo_via_git(forestx_home, git_binary),
    ...
    match git_sync_result {
        ...
        Err(err) => {
            ...
            match sync_openai_plugins_repo_via_http(forestx_home, api_base_url, http_client_factory) {
                Ok(remote_sha) => {
                ...
                Err(http_err) => {
                    ...
                    if has_local_curated_plugins_snapshot(forestx_home) {
...
fn lock_curated_plugins_startup_sync(forestx_home: &Path) -> Result<File, String> {
    let lock_path = forestx_home.join(CURATED_PLUGINS_SYNC_LOCK_FILE);
...
fn sync_openai_plugins_repo_via_git(
    ...
    let repo_path = curated_plugins_repo_path(forestx_home);
    let sha_path = forestx_home.join(CURATED_PLUGINS_SHA_FILE);
...
fn sync_openai_plugins_repo_via_backup_archive(
    ...
    let repo_path = curated_plugins_repo_path(forestx_home);
    let sha_path = curated_plugins_sha_path(forestx_home);
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|err| format!("failed to create curated plugins sync runtime: {err}"))?;
...
pub fn has_local_curated_plugins_snapshot(forestx_home: &Path) -> bool {
    curated_plugins_repo_path(forestx_home)
        .join(".agents/plugins/marketplace.json")
        .is_file()
        && forestx_home.join(CURATED_PLUGINS_SHA_FILE).is_file()
}

...
use forestx_http_client::RouteAwareRequestError;
...
const FORESTX_PRODUCT_SKU: &str = "forestx";
...
pub(crate) async fn fetch_and_cache_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_directory_plugins(forestx_home, config, auth, scope, &plugins);
    Ok(())
}
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    fetch_and_cache_remote_plugin_catalog(forestx_home, config, auth, RemotePluginScope::Global).await
}
...
pub fn invalidate_cached_remote_plugin_catalog_scopes(
    ...
    for scope in scopes {
        catalog_cache::remove_cached_directory_plugins(forestx_home, config, auth, *scope);
    }
}
...
pub(crate) fn has_fresh_cached_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_directory_plugins(forestx_home, config, auth, scope).is_some_and(
        |cached| {
            matches!(
                cached.freshness,
...
pub(crate) fn cached_remote_plugin_catalog_scopes(
    ...
    RemotePluginScope::CATALOG_CACHE_SCOPES
        .into_iter()
        .filter(|scope| {
            catalog_cache::load_cached_directory_plugins(forestx_home, config, auth, *scope).is_some()
        })
        .collect()
}
...
pub async fn uninstall_remote_plugin(
    ...
    tokio::task::spawn_blocking(move || {
        remove_remote_plugin_cache(forestx_home, marketplace_name, plugin_name, legacy_plugin_id)
    })
    .await
    .map_err(|err| {
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(forestx_home.clone())
    ...
    let legacy_remote_plugin_cache_root = forestx_home
...
async fn fetch_directory_plugins_for_scope_with_cache(
    ...
    if cache_mode == RemotePluginCatalogCacheMode::PreferCache
        && let Some(forestx_home) = forestx_home
        && let Some(cached) =
            catalog_cache::load_cached_directory_plugins(forestx_home, config, auth, scope)
    {
...
fn ensure_chatgpt_auth(auth: Option<&ForestxAuth>) -> Result<&ForestxAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    request
        ...
        .headers(forestx_model_provider::auth_provider_from_auth(auth).to_auth_headers())
        .header(OAI_PRODUCT_SKU_HEADER, FORESTX_PRODUCT_SKU)
}

...
fn catalog_cache_paths_are_isolated_by_scope() {
    let forestx_home = Path::new("/tmp/forestx-home");
    ...
    .map(|scope| cache_path(forestx_home, &cache_key_for_scope(scope)));
...
fn global_catalog_cache_reuses_legacy_cache_file() {
    ...
    let auth = ForestxAuth::Headers(AuthHeaders::new(http::HeaderMap::new()));
    let legacy_cache_path = forestx_home
        .path()
    ...
    forestx_utils_path::write_atomically(&legacy_cache_path, &contents).expect("write legacy cache");

    let cached =
        load_cached_directory_plugins(forestx_home.path(), &config, &auth, RemotePluginScope::Global)
...
fn header_auth_does_not_cache_private_catalogs_without_a_stable_identity() {
    ...
    for scope in [RemotePluginScope::User, RemotePluginScope::Workspace] {
        ...
        let insecure_cache_path = cache_path(forestx_home.path(), &insecure_cache_key);

        write_cached_directory_plugins(forestx_home.path(), &config, &auth, scope, &[]);
        ...
        forestx_utils_path::write_atomically(&insecure_cache_path, &contents)
            .expect("write insecure cache");

        assert!(load_cached_directory_plugins(forestx_home.path(), &config, &auth, scope).is_none());
    }
}

...
pub(crate) fn load_cached_directory_plugins(
    ...
    let cache_path = cache_path(forestx_home, &cache_key);
...
pub(crate) fn write_cached_directory_plugins(
    ...
    let _ = forestx_utils_path::write_atomically(&cache_path, &contents);
}
...
fn cache_path(forestx_home: &Path, cache_key: &RemotePluginCatalogCacheKey) -> PathBuf {

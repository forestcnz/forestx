...
pub async fn list_remote_plugin_shares(
    ...
    let local_plugin_paths =
        local_paths::load_plugin_share_local_paths(forestx_home).map_err(|err| {
...
pub fn load_plugin_share_remote_ids_by_local_path(
    ...
    let local_paths = local_paths::load_plugin_share_local_paths(forestx_home)?;
    local_paths
        .into_iter()
...
pub async fn delete_remote_plugin_share(
    ...
    if let Err(err) = local_paths::remove_plugin_share_local_path(forestx_home, remote_plugin_id) {
        warn!(

...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&forestx_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(forestx_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
                info!(
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = forestx_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
        ...
        for entry in fs::read_dir(&marketplace_root).map_err(|err| {
            ...
            if is_remote_plugin_cache_mutation_in_flight(forestx_home, marketplace_name, &plugin_name)
            {
                continue;
            }
...
fn remote_plugin_cache_root(forestx_home: &Path) -> PathBuf {
    forestx_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(forestx_home.path()),
        };
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let forestx_home = tempfile::tempdir().expect("create forestx home");
        let cached_manifest = forestx_home
            .path()
        ...
        let metadata_path = PluginStore::new(forestx_home.path().to_path_buf())
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let forestx_home = tempfile::tempdir().expect("create forestx home");
        let created_by_me_cached_manifest = forestx_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = forestx_home
            .path()
            .join(PLUGINS_CACHE_DIR)

...
async fn save_remote_plugin_share_creates_workspace_plugin() {
    ...
    assert_eq!(
        local_paths::load_plugin_share_local_paths(forestx_home.path()).unwrap(),
    ...
    assert_eq!(
        archive_files
            .get(".forestx-plugin/plugin.json")
            .map(Vec::as_slice),
...
fn archive_plugin_for_upload_places_manifest_at_archive_root() {
    ...
    assert_eq!(
        ...
        vec![
            ".forestx-plugin/plugin.json".to_string(),
...
fn archive_plugin_for_upload_round_trips_through_plugin_bundle_archive_with_long_paths() {
    ...
    assert_eq!(
        fs::read_to_string(destination.path().join(".forestx-plugin/plugin.json")).unwrap(),
        r#"{"name":"demo-plugin"}"#
    );
    assert_eq!(
...
async fn list_remote_plugin_shares_fetches_created_workspace_plugins() {
    ...
    let local_plugin_path =
        AbsolutePathBuf::try_from(forestx_home.path().join("local-plugin")).unwrap();
    ...
    let result = list_remote_plugin_shares(&config, Some(&auth), forestx_home.path())
        .await
        .unwrap();
...
async fn delete_remote_plugin_share_deletes_workspace_plugin() {
    ...
    delete_remote_plugin_share(&config, Some(&auth), forestx_home.path(), "plugins_123")
        .await
        .unwrap();
    assert_eq!(
        local_paths::load_plugin_share_local_paths(forestx_home.path()).unwrap(),

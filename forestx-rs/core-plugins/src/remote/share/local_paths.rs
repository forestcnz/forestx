...
pub(crate) fn load_plugin_share_local_paths(
    ...
    read_plugin_share_local_paths(forestx_home)
}
...
pub(crate) fn record_plugin_share_local_path(
    ...
    let _guard = lock_plugin_share_local_paths()?;
    let mut mapping = read_plugin_share_local_paths_for_update(forestx_home)?;
    mapping.insert(remote_plugin_id.to_string(), plugin_path);
    write_plugin_share_local_paths(forestx_home, mapping)
}
...
fn read_plugin_share_local_paths(
    ...
    let path = plugin_share_local_paths_path(forestx_home);
...
fn read_plugin_share_local_paths_for_update(
    ...
    match read_plugin_share_local_paths(forestx_home) {
...
fn plugin_share_local_paths_path(forestx_home: &Path) -> std::path::PathBuf {
    forestx_home.join(PLUGIN_SHARE_LOCAL_PATHS_FILE)
}

...
const PERSONAL_MARKETPLACE_NAME: &str = "forestx-curated";
...
pub async fn checkout_remote_plugin_share(
    ...
    let local_paths = load_share_local_paths_for_checkout(forestx_home)?;
...
fn load_share_local_paths_for_checkout(
    ...
    match local_paths::load_plugin_share_local_paths(forestx_home) {
        Ok(paths) => Ok(paths),

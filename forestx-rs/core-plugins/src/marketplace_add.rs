...
pub async fn add_marketplace(
    ...
    tokio::task::spawn_blocking(move || {
        add_marketplace_sync(forestx_home.as_path(), &requirements, request)
    })
    .await
...
fn add_marketplace_sync(
    ...
    add_marketplace_sync_with_cloner(forestx_home, requirements, request, clone_git_source)
}
    ...
    F: Fn(&str, Option<&str>, &[String], &Path) -> Result<(), MarketplaceAddError>,
{
    ...
    let managed_marketplace_name =
        validate_marketplace_source_for_add(forestx_home, requirements, &source)
    ...
    let install_root = marketplace_install_root(forestx_home);
    ...
    if let Some(existing_root) =
        installed_marketplace_root_for_source(forestx_home, &install_root, &install_metadata)?
    {
        ...
        validate_marketplace_name_for_add(managed_marketplace_name, &marketplace_name)
            .map_err(MarketplaceAddError::InvalidRequest)?;
        record_added_marketplace_entry(forestx_home, &marketplace_name, &install_metadata)?;
    ...
    if let MarketplaceSource::Local { path } = &source {
        ...
        validate_marketplace_name_for_add(managed_marketplace_name, &marketplace_name)
            .map_err(MarketplaceAddError::InvalidRequest)?;
        if find_marketplace_root_by_name(forestx_home, &install_root, &marketplace_name)?.is_some() {
...
mod tests {
    ...
    fn add_marketplace_sync_installs_marketplace_and_updates_config() -> Result<()> {
        ...
        let config = fs::read_to_string(forestx_home.path().join(forestx_config::CONFIG_TOML_FILE))?;
...
"#,
    ...
    fn add_marketplace_sync_installs_local_directory_source_and_updates_config() -> Result<()> {
        ...
        assert!(!result.already_added);
        assert!(
            !marketplace_install_root(forestx_home.path())
                .join("debug")
                .exists()
        );
    ...
    fn write_marketplace_source(source: &Path, marker: &str) -> std::io::Result<()> {
        ...
        fs::create_dir_all(source.join("plugins/sample/.forestx-plugin"))?;
        fs::write(
  ...
  "plugins": [
    {
      ...
      "source": {
        ...
        fs::write(
            source.join("plugins/sample/.forestx-plugin/plugin.json"),
            r#"{"name":"sample"}"#,
        )?;

...
use forestx_exec_server::FileSystemResult;
...
async fn unavailable_environment_does_not_fall_back_to_host_filesystem() {
    ...
    write_manifest(&plugin_root, ".forestx-plugin/plugin.json", MANIFEST_CONTENTS);
    let provider =
...
async fn malformed_preferred_manifest_does_not_fall_through_to_alternate() {
    ...
    write_manifest(&plugin_root, ".forestx-plugin/plugin.json", "{not-json");
    write_manifest(
        &plugin_root,
    ...
    let expected_path =
        PathUri::from_host_native_path(plugin_root.join(".forestx-plugin/plugin.json"))
            .expect("manifest URI");
    let provider = ExecutorPluginProvider::new(Arc::new(EnvironmentManager::default_for_tests()));

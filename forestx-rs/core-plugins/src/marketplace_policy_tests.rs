...
fn config_layer_stack_with_user_config(
    ...
    let requirements =
        forestx_config::ConfigRequirements::try_from(with_sources).expect("normalize requirements");
...
fn strict_install_validates_configured_name_source_and_root() {
    ...
    let config_file = AbsolutePathBuf::try_from(forestx_home.path().join("config.toml"))
...
"#
    ...
    assert_eq!(
        policy.validate_install(&stack, forestx_home.path(), &configured_path, "company"),
        Ok(())
    );
    assert!(
        policy
            .validate_install(&stack, forestx_home.path(), &configured_path, "other")
            .expect_err("unconfigured name should fail")
            .contains("must be added to config")
    );
    assert!(
        policy
            .validate_install(&stack, forestx_home.path(), &other_path, "company")
...
"#,
    ...
    let marketplace_path = AbsolutePathBuf::try_from(
        marketplace_install_root(forestx_home.path()).join("debug/.agents/plugins/marketplace.json"),
    )
    .expect("absolute marketplace path");

    let err = MarketplacePolicy::from_requirements(stack.requirements())
        .validate_install(&stack, forestx_home.path(), &marketplace_path, "debug")
...
"#,
    ...
    let marketplace_path = AbsolutePathBuf::try_from(
        curated_plugins_repo_path(forestx_home.path()).join(".agents/plugins/marketplace.json"),
    )
    .expect("absolute marketplace path");
    let policy = MarketplacePolicy::from_requirements(stack.requirements());
...
"#,
    ...
    let expected_name =
        validate_marketplace_source_for_add(forestx_home.path(), stack.requirements(), &source)
...
"#,
    ...
    let projected =
        project_effective_user_config(&stack, forestx_home.path()).expect("project user config");
    assert_eq!(
    ...
    assert_eq!(
        configured_plugins_from_stack(&stack, forestx_home.path())
            .into_keys()
            .collect::<Vec<_>>(),
...
"#,
    ...
    let outcome = upgrade_configured_git_marketplaces(forestx_home.path(), &stack, Some("debug"));
    ...
    assert!(!marketplace_install_root(forestx_home.path()).exists());
}

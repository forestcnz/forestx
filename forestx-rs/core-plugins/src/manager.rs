...
use crate::loader::configured_curated_plugin_ids_from_forestx_home;
...
use forestx_config::clear_user_plugin;
use forestx_config::set_user_plugin_enabled;
use forestx_config::types::PluginConfig;
...
use forestx_core_skills::config_rules::skill_config_rules_from_stack;
use forestx_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
use forestx_hooks::plugin_hook_declarations;
use forestx_http_client::HttpClientFactory;
use forestx_login::AuthManager;
use forestx_login::ForestxAuth;
use forestx_model_provider::AMAZON_BEDROCK_PROVIDER_ID;
use forestx_plugin::AppConnectorId;
use forestx_plugin::PluginCapabilitySummary;
use forestx_plugin::PluginId;
use forestx_plugin::PluginIdError;
use forestx_plugin::PluginTelemetryMetadata;
use forestx_plugin::app_connector_ids_from_declarations;
use forestx_plugin::prompt_safe_plugin_description;
...
use forestx_skills::SkillMetadata;
use forestx_tools::DiscoverablePluginInfo;
...
use forestx_utils_plugins::PluginIdentity;
use forestx_utils_plugins::PluginSkillRoot;
...
impl RemoteCatalogCacheRefreshRequest {
    fn has_same_cache_identity(&self, other: &Self) -> bool {
        self.service_config == other.service_config
            && self.auth.as_ref().and_then(ForestxAuth::get_account_id)
                == other.auth.as_ref().and_then(ForestxAuth::get_account_id)
            && self.auth.as_ref().and_then(ForestxAuth::get_chatgpt_user_id)
                == other.auth.as_ref().and_then(ForestxAuth::get_chatgpt_user_id)
            && self.auth.as_ref().map(ForestxAuth::is_workspace_account)
                == other.auth.as_ref().map(ForestxAuth::is_workspace_account)
    }
}
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        ...
        account_id: auth.and_then(ForestxAuth::get_account_id),
        chatgpt_user_id: auth.and_then(ForestxAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(ForestxAuth::is_workspace_account),
    }
}
...
fn target_curated_marketplace(
    ...
    match auth_mode {
        Some(auth_mode) => {
            if auth_mode.uses_forestx_backend() {
...
impl PluginsManager {
    pub fn new(forestx_home: PathBuf) -> Self {
        Self::new_with_options(forestx_home, Some(Product::Forestx), /*auth_mode*/ None)
    }
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given FORESTX_HOME:
        ...
        // contents of that FORESTX_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single FORESTX_HOME is only used by one product.
        Self {
            forestx_home: forestx_home.clone(),
            store: PluginStore::new(forestx_home),
    ...
    fn remote_global_catalog_active(&self, config: &PluginsConfigInput) -> bool {
        config.remote_plugin_enabled && self.auth_mode().is_some_and(AuthMode::uses_forestx_backend)
    }
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_forestx_backend()) else {
            return Vec::new();
        };
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.forestx_home.as_path())
                    .ok_or_else(|| {
        ...
        let forestx_home = self.forestx_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(forestx_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.forestx_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(forestx_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let loaded_manifest =
            if forestx_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let skill_config_rules = forestx_core_skills::config_rules::skill_config_rules_from_stack(
            &config.config_layer_stack,
        );
    ...
    fn schedule_non_curated_plugin_cache_refresh(
        ...
        let mut configured_plugin_keys =
            configured_plugins_from_stack(&config.config_layer_stack, self.forestx_home.as_path())
                .into_keys()
                .collect::<Vec<_>>();
        configured_plugin_keys.sort_unstable();
    ...
    fn start_curated_repo_sync(
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(move || {
                match sync_openai_plugins_repo(forestx_home.as_path(), http_client_factory) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_forestx_home(forestx_home.as_path());
        ...
        let curated_marketplace_path = if include_openai_curated {
            match target_curated_marketplace(self.auth_mode(), &config.model_provider_id) {
                TargetCuratedMarketplace::OpenAi | TargetCuratedMarketplace::OpenAiWithRemote => {
                    let curated_repo_root = curated_plugins_repo_path(self.forestx_home.as_path());
                    curated_repo_root.is_dir().then_some(curated_repo_root)
                }
                TargetCuratedMarketplace::OpenAiApi => {
                    let api_marketplace_path =
                        curated_plugins_api_marketplace_path(self.forestx_home.as_path());

...
pub struct MarketplacePluginPolicy {
    ...
    // TODO: Surface or enforce product gating at the Forestx/plugin consumer boundary instead of
...
fn resolve_marketplace_plugin_entry(
    ...
    let manifest = match &source {
        MarketplacePluginSource::Local { path } => {
            if forestx_utils_plugins::find_plugin_manifest_path(path.as_path()).is_some() {

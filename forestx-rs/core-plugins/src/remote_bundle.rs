...
use forestx_utils_plugins::AgentPluginSchemaStatus;
use forestx_utils_plugins::agent_plugin_schema_status;
...
pub enum RemotePluginBundleInstallError {
    ...
    DownloadBody {
        url: String,
        #[source]
        source: forestx_http_client::HttpError,
    },
...
pub async fn download_and_install_remote_plugin_bundle(
    ...
    tokio::task::spawn_blocking(move || {
        install_remote_plugin_bundle(forestx_home, bundle, bundle_bytes)
    })
    .await
    .map_err(|err| {
...
fn install_remote_plugin_bundle(
    ...
    let staging_root = forestx_home.join(REMOTE_PLUGIN_INSTALL_STAGING_DIR);
    ...
    let store = PluginStore::try_new(forestx_home)?;
...
mod tests {
    ...
    fn install_persists_remote_plugin_install_metadata() {
        ...
        let store = PluginStore::new(forestx_home.path().to_path_buf());
        ...
        let metadata_path = store
            ...
            .join(".forestx-remote-plugin-install.json");
        assert_eq!(
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".forestx-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".forestx-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
    ...
    fn find_extracted_plugin_root_rejects_nested_plugin_root() {
        ...
        std::fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create manifest dir");
        std::fs::write(

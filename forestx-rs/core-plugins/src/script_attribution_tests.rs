...
use forestx_plugin::PluginLoadOutcome;
...
fn synced_plugin_root(forestx_home: &Path, marketplace: &str, plugin_name: &str) -> AbsolutePathBuf {
    let synced_root = curated_plugins_repo_path(forestx_home);
    match marketplace {
    ...
    let root = PluginStore::new(forestx_home.to_path_buf()).plugin_root(
        &plugin_id,
...
fn cached_remote_plugin_root(forestx_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    let root = PluginStore::new(forestx_home.to_path_buf()).plugin_root(&plugin_id, "1.2.3");
    fs::create_dir_all(root.as_path()).expect("create cached remote plugin root");
    root
}
fn installed_remote_plugin_root(forestx_home: &Path, plugin_name: &str) -> AbsolutePathBuf {
    let root = cached_remote_plugin_root(forestx_home, plugin_name);
    let plugin_id = PluginId::new(
        plugin_name.to_string(),
        REMOTE_GLOBAL_MARKETPLACE_NAME.to_string(),
    )
    .expect("plugin id");
    PluginStore::new(forestx_home.to_path_buf())
...
fn roots_for(forestx_home: &Path, plugins: Vec<LoadedPlugin>) -> TrustedPluginRoots {
    TrustedPluginRoots::from_plugin_load_outcome(
        &PluginLoadOutcome::from_plugins(plugins),
        forestx_home,
    )
}
fn assert_untrusted(forestx_home: &Path, config_name: &str, root: &Path) {
    assert!(
        roots_for(forestx_home, vec![loaded_plugin(config_name, root, ENABLED)])
            .roots
            .is_empty()
    );
}
...
fn only_emits_safe_normalized_relative_script_paths() {
    ...
    assert_eq!(
        normalized_relative_script_path(Path::new(
            "/home/user/.forestx/plugins/cache/openai-curated/sample/scripts/run.py"
        )),
        None
    );
}

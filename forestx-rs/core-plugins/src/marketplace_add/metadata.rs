...
pub(super) fn record_added_marketplace_entry(
    ...
    record_user_marketplace(forestx_home, marketplace_name, &update).map_err(|err| {
...
mod tests {
    ...
    fn installed_marketplace_root_for_source_propagates_config_read_errors() {
        ...
        let install_root = forestx_home.path().join("marketplaces");
    ...
    fn installed_marketplace_root_for_source_uses_local_source_root() {
        let forestx_home = TempDir::new().unwrap();
        let install_root = forestx_home.path().join("marketplaces");
        let source_root = forestx_home.path().join("source");
        ...
        let install_metadata = MarketplaceInstallMetadata::from_source(&source, &[]);
        record_added_marketplace_entry(forestx_home.path(), "debug", &install_metadata).unwrap();

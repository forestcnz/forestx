...
use forestx_config::types::PluginMcpServerConfig;
use forestx_connectors::parse_plugin_app_config;
use forestx_connectors::parse_plugin_app_config_value;
use forestx_core_skills::PluginSkillSnapshots;
use forestx_core_skills::config_rules::resolve_disabled_skill_paths;
...
use forestx_mcp::parse_agent_plugin_mcp_config;
use forestx_mcp::parse_plugin_mcp_config;
use forestx_plugin::AppDeclaration;
use forestx_plugin::LoadedPlugin;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.forestx_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(forestx_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(forestx_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = forestx_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized =
                materialize_marketplace_plugin_source(forestx_home, &source).map_err(|err| {
...
fn configured_plugins_from_forestx_home(
    forestx_home: &Path,
...
pub fn configured_curated_plugin_ids_from_forestx_home(forestx_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_forestx_home(
        forestx_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".forestx-plugin/plugin.json"));
...
pub async fn load_configured_plugin_mcp_servers(
    ...
    let configured_plugins = configured_plugins_from_stack(config_layer_stack, forestx_home);
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = forestx_home.join("plugins/.marketplace-plugin-source-staging");
...
fn run_git_output(args: &[&str], cwd: Option<&Path>) -> Result<String, String> {
    ...
    command
        .args(["-c", forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG])
        .args(args);

...
pub(crate) fn write_curated_plugin_sha_with(forestx_home: &Path, sha: &str) {
    write_file(&forestx_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) async fn load_plugins_config(forestx_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let forestx_home = AbsolutePathBuf::try_from(forestx_home).expect("forestx home should be absolute");

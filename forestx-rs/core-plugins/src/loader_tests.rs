...
use forestx_utils_plugins::AGENT_PLUGIN_SCHEMA_URI;
...
async fn legacy_manifest_can_point_at_root_mcp_json() {
    let temp_dir = TempDir::new().expect("tempdir");
    let plugin_root = temp_dir.path().join("plugin");
    fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create manifest directory");
    fs::write(
        plugin_root.join(".forestx-plugin/plugin.json"),
...
async fn hooks_only_scope_shares_plugin_resolution_without_loading_other_capabilities() {
  ...
  "hooks": {
    ...
    write_file(
        &disabled_root.join(".forestx-plugin/plugin.json"),
        r#"{"name":"disabled"}"#,
    );
    write_file(
    ...
    write_file(
        &malformed_root.join(".forestx-plugin/plugin.json"),
        "not valid json",
    );

    let warning_root = temp_dir.path().join("plugins/cache/test/warning/local");
    write_file(
        &warning_root.join(".forestx-plugin/plugin.json"),
        r#"{"name":"warning"}"#,
    );
...
fn write_manifest(plugin_root: &AbsolutePathBuf, manifest: &str) {
    fs::write(plugin_root.join(".forestx-plugin/plugin.json"), manifest).expect("write manifest");
}

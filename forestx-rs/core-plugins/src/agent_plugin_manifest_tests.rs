...
#[test]
fn legacy_forestx_overlay_keeps_portable_components_fixed() {
    let tmp = tempdir().expect("tempdir");
    let plugin_root = tmp.path().join("demo-plugin");
    write_agent_plugin_manifest(
        &plugin_root,
  ...
  "description": "Portable description""#,
    ...
    fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".forestx-plugin/plugin.json"),
        r#"{
  "name": "different-name",
  "version": "9.9.9",
  "description": "Forestx description",
  "skills": [],
  "mcpServers": null,
  "interface": {"displayName": "Forestx Demo"}
}"#,
    )
...
fn inline_openai_extension_precedes_legacy_overlay() {
  ...
  "extensions": {
    "com.openai": {
      "interface": {"displayName": "Inline Forestx"}
    }
  }"#,
    );
    fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create overlay dir");
    fs::write(
        plugin_root.join(".forestx-plugin/plugin.json"),
        r#"{"interface":{"displayName":"Legacy Forestx"}}"#,
    )
    .expect("write overlay");

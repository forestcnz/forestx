...
use crate::events::ForestxAcceptedLineFingerprintsEventParams;
use crate::events::ForestxAcceptedLineFingerprintsEventRequest;
use crate::events::ForestxAppMentionedEventRequest;
use crate::events::ForestxAppServerClientMetadata;
use crate::events::ForestxAppUsedEventRequest;
use crate::events::ForestxCommandExecutionEventParams;
use crate::events::ForestxCommandExecutionEventRequest;
use crate::events::ForestxCompactionEventRequest;
use crate::events::ForestxHookRunEventRequest;
use crate::events::ForestxOnboardingExternalAgentImportFailureEventRequest;
use crate::events::ForestxOnboardingExternalAgentImportFailureMetadata;
use crate::events::ForestxPluginEventRequest;
use crate::events::ForestxPluginInstallFailedEventRequest;
use crate::events::ForestxPluginInstallFailedMetadata;
use crate::events::ForestxPluginUsedEventRequest;
use crate::events::ForestxReviewEventParams;
use crate::events::ForestxReviewEventRequest;
use crate::events::ForestxRuntimeMetadata;
use crate::events::ForestxToolItemEventBase;
use crate::events::ForestxTurnEventRequest;
...
use crate::events::forestx_app_metadata;
use crate::events::forestx_hook_run_metadata;
use crate::events::forestx_plugin_metadata;
use crate::events::forestx_plugin_used_metadata;
...
use crate::facts::TurnForestxErrorFact;
...
use forestx_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use forestx_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use forestx_app_server_protocol::CollabAgentToolCallStatus;
use forestx_app_server_protocol::CommandAction;
use forestx_app_server_protocol::CommandExecutionApprovalDecision;
use forestx_app_server_protocol::CommandExecutionRequestApprovalParams;
use forestx_app_server_protocol::CommandExecutionRequestApprovalResponse;
use forestx_app_server_protocol::CommandExecutionSource;
use forestx_app_server_protocol::CommandExecutionStatus;
use forestx_app_server_protocol::DynamicToolCallStatus;
use forestx_app_server_protocol::GuardianApprovalReview;
use forestx_app_server_protocol::GuardianApprovalReviewAction;
use forestx_app_server_protocol::GuardianApprovalReviewStatus;
use forestx_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use forestx_app_server_protocol::ImageGenerationItem;
...
use forestx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use forestx_app_server_protocol::ItemStartedNotification;
use forestx_app_server_protocol::JSONRPCErrorError;
use forestx_app_server_protocol::McpToolCallAppContext;
use forestx_app_server_protocol::McpToolCallStatus;
use forestx_app_server_protocol::NonSteerableTurnKind;
use forestx_app_server_protocol::PatchApplyStatus;
use forestx_app_server_protocol::PermissionsRequestApprovalParams;
use forestx_app_server_protocol::RequestId;
use forestx_app_server_protocol::RequestPermissionProfile;
use forestx_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
use forestx_app_server_protocol::ServerNotification;
use forestx_app_server_protocol::ServerRequest;
use forestx_app_server_protocol::ServerResponse;
use forestx_app_server_protocol::SessionSource as AppServerSessionSource;
use forestx_app_server_protocol::SubAgentActivityKind;
...
use forestx_app_server_protocol::ThreadSource as AppServerThreadSource;
use forestx_app_server_protocol::ThreadStartResponse;
use forestx_app_server_protocol::ThreadStatus as AppServerThreadStatus;
use forestx_app_server_protocol::Turn;
use forestx_app_server_protocol::TurnCompletedNotification;
use forestx_app_server_protocol::TurnDiffUpdatedNotification;
use forestx_app_server_protocol::TurnError as AppServerTurnError;
...
use forestx_app_server_protocol::TurnStatus as AppServerTurnStatus;
use forestx_app_server_protocol::TurnSteerParams;
use forestx_app_server_protocol::TurnSteerResponse;
use forestx_app_server_protocol::UserInput;
use forestx_app_server_protocol::WebSearchItem;
use forestx_login::default_client::DEFAULT_ORIGINATOR;
use forestx_login::default_client::originator;
use forestx_plugin::AppConnectorId;
use forestx_plugin::PluginCapabilitySummary;
...
use forestx_protocol::config_types::ApprovalsReviewer;
use forestx_protocol::config_types::ModeKind;
use forestx_protocol::error::ForestxErr;
use forestx_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use forestx_protocol::models::PermissionProfile as CorePermissionProfile;
...
use forestx_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use forestx_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use forestx_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
...
const TEST_PRODUCT_CLIENT_ID: &str = "forestx_work_desktop";
...
fn sample_app_server_client_metadata() -> ForestxAppServerClientMetadata {
    ForestxAppServerClientMetadata {
...
fn sample_runtime_metadata() -> ForestxRuntimeMetadata {
    ForestxRuntimeMetadata {
        forestx_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(forestx_app_server_protocol::TurnStartResponse {
        turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    forestx_error_info: Option<forestx_app_server_protocol::ForestxErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: forestx_error_info.map(|forestx_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                forestx_error_info: Some(forestx_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                message: "cannot steer a review turn".to_string(),
                forestx_error_info: Some(ForestxErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "forestx-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "forestx-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "forestx-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "forestx-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        runtime: ForestxRuntimeMetadata {
            forestx_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(forestx_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: forestx_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.forestx/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".forestx/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.forestx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/forestx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.forestx/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(ForestxAppMentionedEventRequest {
        event_type: "forestx_app_mentioned",
        event_params: forestx_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(ForestxAppUsedEventRequest {
        event_type: "forestx_app_used",
        event_params: forestx_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        ForestxAcceptedLineFingerprintsEventRequest {
            event_type: "forestx_accepted_line_fingerprints",
            event_params: ForestxAcceptedLineFingerprintsEventParams {
                event_type: "forestx.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("forestx".to_string()),
                model_slug: Some("gpt-5.1-forestx".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "forestx",
                "model_slug": "gpt-5.1-forestx",
                "completed_at": 1710000000,
...
async fn analytics_flush_delivers_completed_turn_with_file_diff() {
    ...
    let capture_path = std::env::temp_dir().join(format!(
        "forestx-analytics-turn-flush-{}-{nonce}.jsonl",
        std::process::id()
    ));
    let auth_manager = forestx_login::AuthManager::from_auth_for_testing(
        forestx_login::ForestxAuth::create_dummy_chatgpt_auth_for_testing(),
    );
...
"
    ...
    let event_types = contents
        ...
        .collect::<Vec<_>>();
    assert!(event_types.iter().any(|event| event == "forestx_turn_event"));
    assert!(
        event_types
            .iter()
            .any(|event| event == "forestx_accepted_line_fingerprints")
    );
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(ForestxCompactionEventRequest {
        event_type: "forestx_compaction_event",
        event_params: crate::events::forestx_compaction_event_params(
            ForestxCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                forestx_error_kind: None,
                forestx_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "forestx-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "forestx_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "forestx_error_kind": null,
                "forestx_error_http_status_code": null,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(ForestxCommandExecutionEventRequest {
        event_type: "forestx_command_execution_event",
        event_params: ForestxCommandExecutionEventParams {
            base: ForestxToolItemEventBase {
                thread_id: "thread-1".to_string(),
                ...
                app_server_client: ForestxAppServerClientMetadata {
                    product_client_id: "forestx_tui".to_string(),
                    client_name: Some("forestx-tui".to_string()),
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(ForestxReviewEventRequest {
        event_type: "forestx_review_event",
        event_params: ForestxReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "forestx_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["forestx_rs_version"],
        "0.99.0"
    );
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        forestx_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                ForestxCompactionEvent {
                    ...
                    forestx_error_kind: Some(ForestxErrKind::ContextWindowExceeded),
                    forestx_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "forestx_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["forestx_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["forestx_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("forestx-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "forestx_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "forestx_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "forestx_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "forestx_review_event");
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            forked_from_thread_id: None,
            product_client_id: "forestx-tui".to_string(),
            client_name: "forestx-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        forestx_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        forestx_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        forestx_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_keep_thread_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        forestx_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.forestx_rs_version, "0.1.0");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(ForestxPluginUsedEventRequest {
        event_type: "forestx_plugin_used",
        event_params: forestx_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(ForestxPluginEventRequest {
        event_type: "forestx_plugin_installed",
        event_params: forestx_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(ForestxPluginInstallFailedEventRequest {
        event_type: "forestx_plugin_install_failed",
        event_params: ForestxPluginInstallFailedMetadata {
            plugin: forestx_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(ForestxPluginEventRequest {
        event_type: "forestx_plugin_installed",
        event_params: forestx_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    let tracking = test_tracking_context("thread-3", "turn-3");
    let event = TrackEventRequest::HookRun(ForestxHookRunEventRequest {
        event_type: "forestx_hook_run",
        event_params: forestx_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(forestx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(forestx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(forestx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(forestx_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(forestx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: forestx_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_includes_plugin_ids_for_plugin_skill_invocations() {
    ...
    let tracking = test_tracking_context("thread-1", "turn-1");
    let skill_path =
        PathBuf::from("/Users/abc/.forestx/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "forestx_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "forestx_app_mentioned");
    assert_eq!(payload[1]["event_type"], "forestx_app_used");
    assert_eq!(payload[2]["event_type"], "forestx_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "forestx_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "forestx_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "forestx_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        ForestxOnboardingExternalAgentImportFailureEventRequest {
            event_type: "forestx_onboarding_external_agent_import_failure",
            event_params: ForestxOnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
                provider_id: "test-provider-42".to_string(),
                item_type: "PLUGINS".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(ForestxTurnEventRequest {
        event_type: "forestx_turn_event",
        event_params: crate::events::ForestxTurnEventParams {
            thread_id: "thread-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "forestx_cli_rs",
                    "client_name": "forestx-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("forestx_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["forestx_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("forestx_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        vec![
            ("forestx_command_execution_event", "session-thread-2"),
            ("forestx_file_change_event", "session-thread-2"),
            ("forestx_mcp_tool_call_event", "session-thread-2"),
            ("forestx_dynamic_tool_call_event", "session-thread-2"),
            ("forestx_collab_agent_tool_call_event", "session-thread-2"),
            ("forestx_web_search_event", "session-thread-2"),
            ("forestx_image_generation_event", "session-thread-2"),
        ]
    );
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnForestxError(Box::new(
                TurnForestxErrorFact::from_forestx_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &ForestxErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(forestx_app_server_protocol::ForestxErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["forestx_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["forestx_error_http_status_code"],
        json!(null)
    );
}
...
async fn rejected_turn_interrupt_does_not_tag_interrupted_turn_event() {
    ...
    assert_eq!(payload["event_params"]["forestx_error_kind"], json!(null));
}

...
#[cfg(debug_assertions)]
use crate::events::ForestxPluginMetadata;
#[cfg(debug_assertions)]
use crate::events::ForestxPluginUsedEventRequest;
#[cfg(debug_assertions)]
use crate::events::ForestxPluginUsedMetadata;
#[cfg(debug_assertions)]
...
use forestx_app_server_protocol::CommandExecutionOutputDeltaNotification;
...
fn sample_mcp_tool_call_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::McpToolCall(ForestxMcpToolCallEventRequest {
        ...
        event_params: ForestxMcpToolCallEventParams {
            base: ForestxToolItemEventBase {
                ...
                app_server_client: ForestxAppServerClientMetadata {
                    product_client_id: "forestx_desktop".to_string(),
                    client_name: None,
                    client_version: None,
                ...
                runtime: ForestxRuntimeMetadata {
                    forestx_rs_version: "0.0.0".to_string(),
...
fn sample_plugin_used_track_event(thread_id: &str, plugin_id: Option<&str>) -> TrackEventRequest {
    TrackEventRequest::PluginUsed(ForestxPluginUsedEventRequest {
        event_type: "forestx_plugin_used",
        event_params: ForestxPluginUsedMetadata {
            plugin: ForestxPluginMetadata {
                plugin_id: plugin_id.map(str::to_string),
                remote_plugin_id: None,
                ...
                product_client_id: Some("forestx_desktop".to_string()),
            },
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "forestx-analytics-{name}-{}-{nonce}.jsonl",
        std::process::id()
    ))
}
...
fn analytics_destination_uses_http_without_capture_file() {
    ...
    assert_eq!(
        ...
        AnalyticsEventsDestination::Http {
            url: "https://chatgpt.com/backend-api/forestx/analytics-events/events".to_string()
        }
    );
}
...
async fn capture_file_writes_exact_serialized_request() {
    ...
    let auth = forestx_login::ForestxAuth::create_dummy_chatgpt_auth_for_testing();
...
async fn capture_file_writes_final_batches_as_separate_lines() {
    ...
    assert_eq!(
        ...
        "forestx_accepted_line_fingerprints"
    );
...
#[tokio::test]
#[cfg(debug_assertions)]
async fn api_key_auth_sends_only_plugin_events_to_forestx_backend() {
    ...
    let auth_manager = forestx_login::AuthManager::from_auth_for_testing(
        forestx_login::ForestxAuth::from_api_key("sk-test"),
    );
...
fn track_response_ignores_unserializable_thread_responses() {
    ...
    thread_start.cwd = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(

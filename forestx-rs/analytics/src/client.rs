...
impl AnalyticsEventsDestination {
    ...
    fn from_base_url_and_capture_file(base_url: String, capture_file: Option<PathBuf>) -> Self {
        ...
        Self::Http {
            url: format!("{base_url}/forestx/analytics-events/events"),
        }
    }
}
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::ForestxCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: ForestxGoalEvent) {
    ...
    pub fn track_turn_forestx_error(&self, fact: TurnForestxErrorFact) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::TurnForestxError(
            Box::new(fact),
        )));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_forestx_backend() {
        return;
    }
    if events.is_empty() {
        return;

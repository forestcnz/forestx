...
use crate::events::ForestxCollabAgentToolCallEventParams;
use crate::events::ForestxCollabAgentToolCallEventRequest;
...
use crate::events::ForestxDynamicToolCallEventParams;
use crate::events::ForestxDynamicToolCallEventRequest;
use crate::events::ForestxFileChangeEventParams;
use crate::events::ForestxFileChangeEventRequest;
use crate::events::ForestxGoalEventRequest;
use crate::events::ForestxHookRunEventRequest;
use crate::events::ForestxImageGenerationEventParams;
use crate::events::ForestxImageGenerationEventRequest;
use crate::events::ForestxMcpToolCallEventParams;
use crate::events::ForestxMcpToolCallEventRequest;
use crate::events::ForestxOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::ForestxOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::ForestxPluginInstallRequestedEventRequest;
...
use crate::events::ForestxTurnEventParams;
use crate::events::ForestxTurnEventRequest;
use crate::events::ForestxTurnSteerEventParams;
use crate::events::ForestxTurnSteerEventRequest;
use crate::events::ForestxWebSearchEventParams;
use crate::events::ForestxWebSearchEventRequest;
...
use crate::events::forestx_compaction_event_params;
use crate::events::forestx_goal_event_params;
use crate::events::forestx_hook_run_metadata;
use crate::events::forestx_plugin_install_requested_metadata;
...
use crate::facts::TurnForestxError;
...
use forestx_app_server_protocol::ClientResponse;
use forestx_app_server_protocol::ForestxErrorInfo;
use forestx_app_server_protocol::CollabAgentStatus;
...
use forestx_app_server_protocol::DynamicToolCallOutputContentItem;
...
use forestx_app_server_protocol::NetworkPolicyRuleAction;
use forestx_app_server_protocol::PatchApplyStatus;
use forestx_app_server_protocol::PatchChangeKind;
...
use forestx_app_server_protocol::WebSearchAction;
use forestx_git_utils::collect_git_info;
...
use forestx_protocol::items::is_safe_plugin_relative_path;
use forestx_protocol::models::PermissionProfile;
use forestx_protocol::protocol::SessionSource;
use forestx_protocol::protocol::SkillScope;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a ForestxCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a ForestxGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a forestx_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<ForestxErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    forestx_error: Option<TurnForestxError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnForestxError(input) => {
                    self.ingest_turn_forestx_error(*input);
                }
    ...
    fn ingest_code_mode_tool_call(
        ...
        match input {
            ...
            CodeModeToolCallFact::Completed {
                ...
                let event = TrackEventRequest::DynamicToolCall(ForestxDynamicToolCallEventRequest {
                    event_type: "forestx_dynamic_tool_call_event",
                    event_params: ForestxDynamicToolCallEventParams {
                        base,
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "forestx_guardian_review",
    ...
    fn ingest_turn_forestx_error(&mut self, input: TurnForestxErrorFact) {
        let TurnForestxErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.forestx_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = forestx_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(ForestxAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = forestx_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(ForestxAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(ForestxHookRunEventRequest {
            event_type: "forestx_hook_run",
            event_params: forestx_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(ForestxPluginUsedEventRequest {
            event_type: "forestx_plugin_used",
            event_params: forestx_plugin_used_metadata(&tracking, plugin),
        }));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            ForestxPluginInstallRequestedEventRequest {
                event_type: "forestx_plugin_install_requested",
                event_params: forestx_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = ForestxPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            ForestxOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "forestx_onboarding_external_agent_import_complete",
                event_params: ForestxOnboardingExternalAgentImportCompleteMetadata {
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.forestx_error_info),
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: forestx_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(&mut self, input: ForestxCompactionEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            ForestxCompactionEventRequest {
                event_type: "forestx_compaction_event",
                event_params: forestx_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: ForestxGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
            self.thread_context_or_warn(AnalyticsDropSite::goal(&input))
        else {
            return;
        };
        out.push(TrackEventRequest::Goal(Box::new(ForestxGoalEventRequest {
            event_type: "forestx_goal_event",
            event_params: forestx_goal_event_params(
                input,
                thread_metadata.session_id.clone(),
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: forestx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(ForestxTurnSteerEventRequest {
            event_type: "forestx_turn_steer_event",
            event_params: ForestxTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(ForestxReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(ForestxTurnEventRequest {
            event_type: "forestx_turn_event",
            event_params: forestx_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
...
fn tool_event_base_mut(event: &mut TrackEventRequest) -> Option<&mut ForestxToolItemEventBase> {
    match event {
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                ForestxCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(ForestxFileChangeEventRequest {
                event_type: "forestx_file_change_event",
                event_params: ForestxFileChangeEventParams {
                    base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                ForestxMcpToolCallEventRequest {
                    event_type: "forestx_mcp_tool_call_event",
                    event_params: ForestxMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                ForestxDynamicToolCallEventRequest {
                    event_type: "forestx_dynamic_tool_call_event",
                    event_params: ForestxDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                ForestxCollabAgentToolCallEventRequest {
                    event_type: "forestx_collab_agent_tool_call_event",
                    event_params: ForestxCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(ForestxWebSearchEventRequest {
                event_type: "forestx_web_search_event",
                event_params: ForestxWebSearchEventParams {
                    base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                ForestxImageGenerationEventRequest {
                    event_type: "forestx_image_generation_event",
                    event_params: ForestxImageGenerationEventParams {
                        base,
...
fn file_change_counts(changes: &[forestx_app_server_protocol::FileUpdateChange]) -> FileChangeCounts {
...
fn forestx_turn_event_params(
    app_server_client: ForestxAppServerClientMetadata,
    runtime: ForestxRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> ForestxTurnEventParams {
    let (
        Some(thread_id),
    ...
    let forestx_error = turn_state.forestx_error.as_ref();
    ForestxTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        forestx_error_kind: forestx_error.map(|error| error.kind),
        forestx_error_http_status_code: forestx_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: forestx_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        forestx_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        forestx_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        forestx_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        forestx_app_server_protocol::TurnStatus::InProgress => None,
    }
}
    ...
    fn command_execution_script_paths_reject_unsafe_values() {
        assert_eq!(
            safe_plugin_relative_script_path(
                ...
                Some("/home/user/.forestx/plugins/cache/openai-curated/sample/scripts/run.py"),
            ),
            None
        );
        assert_eq!(

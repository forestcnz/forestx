...
pub fn app_server_control_socket_path(forestx_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
pub fn app_server_startup_lock_path(forestx_home: &Path) -> std::io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(
        forestx_home
            .join(APP_SERVER_CONTROL_SOCKET_DIR_NAME)
            ...
            let socket_path = if raw_socket_path.is_empty() {
                let forestx_home = find_forestx_home().map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        ...
                        message: format!("failed to resolve FORESTX_HOME: {err}"),
                    }
                })?;
                app_server_control_socket_path(&forestx_home).map_err(|err| {
                    AppServerTransportParseError::InvalidUnixSocketPath {
                        listen_url: listen_url.to_string(),

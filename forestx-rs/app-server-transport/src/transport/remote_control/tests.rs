...
use forestx_app_server_protocol::RemoteControlConnectionStatus;
...
use forestx_app_server_protocol::RemoteControlStatusChangedNotification;
use forestx_app_server_protocol::ServerNotification;
use forestx_app_server_protocol::ServerNotificationEnvelope;
use forestx_config::types::AuthCredentialsStoreMode;
use forestx_core::test_support::auth_manager_from_auth;
use forestx_core::test_support::auth_manager_from_auth_with_home;
...
use forestx_login::save_auth;
use forestx_login::token_data::TokenData;
use forestx_login::token_data::parse_chatgpt_jwt_claims;
use forestx_protocol::auth::AuthMode;
use forestx_state::RemoteControlEnrollmentRecord;
...
fn remote_control_auth_manager() -> Arc<AuthManager> {
    auth_manager_from_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
}

fn remote_control_auth_manager_with_home(forestx_home: &TempDir) -> Arc<AuthManager> {
...
async fn remote_control_state_runtime(forestx_home: &TempDir) -> Arc<StateRuntime> {
    StateRuntime::init(
...
async fn plain_start_resolves_persisted_remote_control_preference() {
    ...
    let forestx_home = TempDir::new().expect("temp dir should create");
    let state_db = remote_control_state_runtime(&forestx_home).await;
...
async fn ephemeral_enable_preserves_durable_preference() {
    ...
    remote_handle.state_db = Some(remote_control_state_runtime(&forestx_home).await);
    remote_handle
        .desired_state_tx
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    forestx_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message = JSONRPCMessage::Request(forestx_app_server_protocol::JSONRPCRequest {
        id: forestx_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(forestx_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    let (remote_task, remote_handle) = start_remote_control(
        ...
        Some(remote_control_state_runtime(&forestx_home).await),
        remote_control_auth_manager(),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(forestx_app_server_protocol::JSONRPCRequest {
                    id: forestx_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-forestx-server-id"),
        Some(&"srv_e_test".to_string())
    );
    assert_eq!(
        handshake_request.headers.get("x-forestx-name"),
        Some(&base64::engine::general_purpose::STANDARD.encode(&expected_server_name))
    );
    assert_eq!(
        handshake_request.headers.get("x-forestx-protocol-version"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(forestx_app_server_protocol::JSONRPCRequest {
                id: forestx_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
    ...
    writer
        .send(QueuedOutgoingMessage::new(OutgoingMessage::Response(
            crate::outgoing_message::OutgoingResponse {
                id: forestx_app_server_protocol::RequestId::Integer(11),
                result: Box::new(
                    forestx_app_server_protocol::ClientResponsePayload::Initialize(
                        forestx_app_server_protocol::InitializeResponse {
                            user_agent: "forestx-test-agent".to_string(),
                            forestx_home: forestx_home.path().abs(),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "message": {
                "id": 11,
                "result": {
                    "userAgent": "forestx-test-agent",
                    "forestxHome": forestx_home.path(),
...
async fn remote_control_http_mode_refreshes_persisted_enrollment_before_connecting() {
    ...
    let (remote_task, _remote_handle) = start_remote_control(
        ...
        Some(state_db.clone()),
        remote_control_auth_manager_with_home(&forestx_home),
        transport_event_tx,
        shutdown_token.clone(),
        /*app_server_client_name_rx*/ None,
        RemoteControlStartupMode::EnabledEphemeral,
...
async fn remote_control_http_mode_reenrolls_after_explicit_missing_server_404() {
    ...
    assert_eq!(
        websocket_request.headers.get("x-forestx-server-id"),

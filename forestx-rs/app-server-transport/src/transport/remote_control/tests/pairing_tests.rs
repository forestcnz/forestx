...
async fn remote_control_handle_recovers_auth_before_refreshing_pairing() {
    ...
    let auth_manager = auth_manager_with_replacement(&forestx_home, "account_id").await;
    let remote_handle =
...
async fn pairing_auth_recovery_failure_publishes_cleared_server_token() {
    ...
    let forestx_home = TempDir::new().expect("temp dir should create");
    let auth_manager = auth_manager_with_replacement(&forestx_home, "different_account_id").await;
    let remote_handle =
        remote_control_handle_with_current_enrollment(&remote_control_url, auth_manager);
    remote_handle
        .current_enrollment

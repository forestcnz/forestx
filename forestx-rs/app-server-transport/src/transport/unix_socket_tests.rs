...
fn listen_unix_socket_accepts_absolute_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix:///tmp/forestx.sock"),
        Ok(AppServerTransport::UnixSocket {
            socket_path: absolute_path("/tmp/forestx.sock")
        })
    );
}
...
fn listen_unix_socket_accepts_relative_custom_path() {
    assert_eq!(
        AppServerTransport::from_listen_url("unix://forestx.sock"),
        Ok(AppServerTransport::UnixSocket {
...
fn default_control_socket_path() -> AbsolutePathBuf {
    ...
    app_server_control_socket_path(&forestx_home).expect("default control socket path")
}

...
/// removes legacy allow rules that newer forestx versions no longer offer.
///
/// this migration is intentionally one-shot. once complete, a marker in `forestx_home` prevents
/// policies saved by newer forestx versions from being removed on later startups.
pub async fn prefix_rule_migration(
    forestx_home: &Path,
    policy_path: &Path,
    banned_prefixes: &[&[&str]],
) -> io::Result<()> {
    let marker_path = forestx_home.join(MIGRATION_MARKER_FILENAME);
    if tokio::fs::try_exists(&marker_path).await? {
        return Ok(());
    }
    clean_rules_file(policy_path, banned_prefixes).await?;

    write_migration_marker(forestx_home, &marker_path).await?;
    Ok(())
}

// atomically writes the marker after creating forestx home when needed.
async fn write_migration_marker(forestx_home: &Path, marker_path: &Path) -> io::Result<()> {
    tokio::fs::create_dir_all(forestx_home).await?;
    let forestx_home = forestx_home.to_owned();
    let marker_path = marker_path.to_owned();
    tokio::task::spawn_blocking(move || {
        let mut marker = tempfile::NamedTempFile::new_in(forestx_home)?;

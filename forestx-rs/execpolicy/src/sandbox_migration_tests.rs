...
async fn removes_banned_allow_rules_once() {
    ...
    let policy_path = forestx_home.path().join("rules/default.rules");
...
"#,
    ...
    prefix_rule_migration(forestx_home.path(), &policy_path, BANNED_PREFIXES)
        .await
...
"#
    ...
    assert_eq!(
        std::fs::read_to_string(forestx_home.path().join(MIGRATION_MARKER_FILENAME))

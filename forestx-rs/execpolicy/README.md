...
- From the Forestx CLI, run `forestx execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
forestx execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
forestx execpolicy check \
...
```bash
cargo run -p forestx-execpolicy -- check --rules path/to/policy.rules git status
```

...
use forestx_protocol::auth::PlanType as AuthPlanType;
...
const AGENT_IDENTITY_JWT_AUDIENCE: &str = "forestx-app-server";
const AGENT_IDENTITY_JWT_ISSUER: &str = "https://chatgpt.com/forestx-backend/agent-identity";
...
const AGENT_IDENTITY_KEY_DERIVATION_CONTEXT: &[u8] = b"forestx-agent-identity-ed25519-v1";
...
impl ChatGptEnvironment {
    pub fn from_chatgpt_base_url(chatgpt_base_url: &str) -> Result<Self> {
        match chatgpt_base_url.trim_end_matches('/') {
            ...
            | "https://chatgpt.com/forestx"
            | "https://chatgpt.com/backend-api/forestx"
            | "https://chat.openai.com"
            | "https://chat.openai.com/backend-api"
            | "https://chat.openai.com/forestx"
            | "https://chat.openai.com/backend-api/forestx" => Ok(Self::Production),
            "https://chatgpt-staging.com"
            | "https://chatgpt-staging.com/backend-api"
            | "https://chatgpt-staging.com/forestx"
            | "https://chatgpt-staging.com/backend-api/forestx" => Ok(Self::Staging),
            _ => anyhow::bail!(
...
/// single Forestx run, while the agent runtime id and private key are the
...
pub fn build_abom(session_source: SessionSource) -> AgentBillOfMaterials {
    AgentBillOfMaterials {
        ...
        agent_harness_id: match &session_source {
            SessionSource::VSCode => "forestx-app".to_string(),
            SessionSource::Cli
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::Unknown => "forestx-cli".to_string(),
        },
...
mod tests {
    ...
    use forestx_protocol::auth::KnownPlan;
...
-----END PRIVATE KEY-----"#,
    ...
    fn chatgpt_environment_maps_known_urls_to_authapi() -> anyhow::Result<()> {
        assert_eq!(
            ChatGptEnvironment::from_chatgpt_base_url("https://chatgpt.com/backend-api/forestx")?,
    ...
    fn agent_identity_jwks_url_uses_jwt_issuer_base_url() {
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/forestx"),
            "http://localhost:8080/api/forestx/agent-identities/jwks"
        );
        assert_eq!(
            agent_identity_jwks_url("http://localhost:8080/api/forestx/"),
            "http://localhost:8080/api/forestx/agent-identities/jwks"

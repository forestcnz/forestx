use super::*;
use forestx_agent_extension::AgentInvocation;
use forestx_agent_extension::AgentRun;
use forestx_agent_extension::AgentRunner;
...
use forestx_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use forestx_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
use forestx_skills::system_cache_root_dir;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<forestx_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = forestx_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input {
            ...
            forestx_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<forestx_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(forestx_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(forestx_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        if has_any_overrides {
            thread
                .preview_thread_settings_overrides(ForestxThreadSettingsOverrides {
        ...
        Ok(forestx_protocol::protocol::ThreadSettingsOverrides {
            environments,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != forestx_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            forestx_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            forestx_protocol::protocol::NonSteerableTurnKind::Compact => (
    ...
    async fn prepare_realtime_conversation_thread(
        ...
        request_id: &ConnectionRequestId,
        thread_id: &str,
    ) -> Result<Option<(ThreadId, Arc<ForestxThread>)>, JSONRPCErrorError> {
        ...
        self.submit_core_op(
            ...
            Op::RealtimeConversationStart(ConversationStartParams {
                ...
                flush_transcript_tail_on_session_end: params
                    ...
                    .unwrap_or(false),
                forestx_responses_as_items: params.forestx_responses_as_items.unwrap_or(false),
                forestx_response_item_prefix: params.forestx_response_item_prefix,
                forestx_response_handoff_mode: params.forestx_response_handoff_mode.unwrap_or_default(),
                forestx_response_handoff_channel_prefixes: params
    ...
    async fn start_inline_review(
        &self,
        request_id: &ConnectionRequestId,
        parent_thread: Arc<ForestxThread>,
    ...
    async fn start_detached_review(
        ...
        if matches!(
            ...
            forestx_protocol::protocol::ThreadHistoryMode::Paginated
        ) {
            return Err(invalid_request(
    ...
    async fn review_start_inner(
        ...
        match delivery.unwrap_or(ApiReviewDelivery::Inline).to_core() {
            ...
            CoreReviewDelivery::Detached => {
                let review_skill_path = system_cache_root_dir(&self.config.forestx_home)
                    .join("review-agent")
                    .join("SKILL.md");
                let prompt = format!(

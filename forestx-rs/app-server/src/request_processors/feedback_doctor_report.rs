...
//! the configured Forestx executable as a subprocess, accepts only valid JSON from
//! `forestx doctor --json`, derives a small set of Sentry tags, and otherwise
...
use forestx_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `forestx-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `forestx doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn Forestx, finish before the timeout, or parse JSON means the

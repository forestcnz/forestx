...
use forestx_app_server_protocol::BrowserUseRequirements;
use forestx_app_server_protocol::ClientResponsePayload;
use forestx_app_server_protocol::ComputerUseRequirements;
use forestx_app_server_protocol::ConfigBatchWriteParams;
use forestx_app_server_protocol::ConfigReadParams;
use forestx_app_server_protocol::ConfigReadResponse;
use forestx_app_server_protocol::ConfigRequirements;
use forestx_app_server_protocol::ConfigRequirementsReadResponse;
use forestx_app_server_protocol::ConfigValueWriteParams;
use forestx_app_server_protocol::ConfigWriteErrorCode;
use forestx_app_server_protocol::ConfigWriteResponse;
use forestx_app_server_protocol::ConfiguredHookHandler;
use forestx_app_server_protocol::ConfiguredHookMatcherGroup;
use forestx_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use forestx_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use forestx_app_server_protocol::FeedbackRequirements;
use forestx_app_server_protocol::JSONRPCErrorError;
use forestx_app_server_protocol::ManagedHooksRequirements;
use forestx_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use forestx_app_server_protocol::ModelsRequirements;
use forestx_app_server_protocol::NetworkDomainPermission;
use forestx_app_server_protocol::NetworkRequirements;
use forestx_app_server_protocol::NetworkUnixSocketPermission;
...
use forestx_config::HookEventsToml;
use forestx_config::HookHandlerConfig as CoreHookHandlerConfig;
use forestx_config::ManagedHooksRequirementsToml;
use forestx_config::MatcherGroup as CoreMatcherGroup;
use forestx_config::ResidencyRequirement as CoreResidencyRequirement;
use forestx_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use forestx_core::ThreadManager;
use forestx_features::canonical_feature_for_key;
use forestx_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<forestx_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = forestx_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ...
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(forestx_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(forestx_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            forestx_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            forestx_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: forestx_config::ComputerUseRequirementsToml,
...
fn map_browser_use_requirements_to_api(
    browser_use: forestx_config::BrowserUseRequirementsToml,
...
) -> forestx_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => forestx_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: forestx_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(forestx_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(forestx_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(forestx_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: forestx_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        forestx_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        forestx_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: forestx_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        forestx_config::NetworkUnixSocketPermissionToml::Allow => NetworkUnixSocketPermission::Allow,
        forestx_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use forestx_app_server_protocol::FeedbackRequirements;
    use forestx_app_server_protocol::WindowsSandboxSetupMode;
    use forestx_config::ComputerUseRequirementsToml;
    use forestx_config::ConfigRequirementsToml;
    use forestx_config::ModelsRequirementsToml;
    use forestx_config::NewThreadModelDefaultsToml;
    use forestx_config::WindowsRequirementsToml;

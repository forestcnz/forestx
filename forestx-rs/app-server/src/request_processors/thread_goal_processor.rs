...
use forestx_goal_extension::GoalServiceError;
...
impl ThreadGoalRequestProcessor {
    ...
    async fn thread_goal_set_inner(
        ...
        let persist_result = match self.thread_manager.get_thread(thread_id).await {
            Ok(thread) => match thread.rollout_path() {
                Some(path) if forestx_rollout::existing_rollout_path(&path).await.is_none() => {
    ...
    async fn state_db_for_materialized_thread(
        ...
        if let Ok(thread) = self.thread_manager.get_thread(thread_id).await {
            ...
            forestx_rollout::find_thread_path_by_id_str(
    ...
    async fn reconcile_thread_goal_rollout(
        ...
        let rollout_path = match running_thread.as_ref() {
            ...
            None => forestx_rollout::find_thread_path_by_id_str(
                &self.config.forestx_home,
                &thread_id.to_string(),
                self.state_db.as_deref(),
            )
        ...
        if let Ok(Some(metadata)) = state_db.get_thread(thread_id).await
            && forestx_rollout::plain_rollout_path(metadata.rollout_path.as_path())
                == forestx_rollout::plain_rollout_path(rollout_path.as_path())
            && let Some(existing_path) =
                forestx_rollout::existing_rollout_path(metadata.rollout_path.as_path()).await
            && forestx_rollout::read_session_meta_line(existing_path.as_path())
                .await
...
pub(super) fn api_thread_goal_from_state(goal: forestx_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn api_thread_goal_status_from_state(status: forestx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {

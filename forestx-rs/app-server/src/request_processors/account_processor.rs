...
use forestx_login::LoginOnboardingEntrypoint;
use forestx_model_provider::is_supported_amazon_bedrock_region;
...
const LOGIN_ISSUER_OVERRIDE_ENV_VAR: &str = "FORESTX_APP_SERVER_LOGIN_ISSUER";
#[cfg(debug_assertions)]
const LOGIN_OPEN_APP_URL_OVERRIDE_ENV_VAR: &str = "FORESTX_APP_SERVER_DEV_OPEN_APP_URL";
...
impl AccountRequestProcessor {
    ...
    fn current_account_updated_notification(&self) -> AccountUpdatedNotification {
        ...
        AccountUpdatedNotification {
            auth_mode: auth
                .as_ref()
                .map(ForestxAuth::api_auth_mode)
                .map(auth_mode_to_api),
            plan_type: auth.as_ref().and_then(ForestxAuth::account_plan_type),
        }
    }
            ...
            LoginAccountParams::Chatgpt {
                ...
                let login_success_page = if use_hosted_login_success_page {
                    let app_brand = match app_brand.unwrap_or_default() {
                        LoginAppBrand::Forestx => LoginSuccessPageBrand::Forestx,
                        LoginAppBrand::Chatgpt => LoginSuccessPageBrand::Chatgpt,
                    };
                    LoginSuccessPage::Hosted {
                        url: FORESTX_OPEN_APP_URL.parse().map_err(|err| {
                            internal_error(format!("invalid Forestx open app URL: {err}"))
                        })?,
                        app_brand,
                    }
                } else {
                ...
                self.login_chatgpt_v2(request_id, forestx_streamlined_login, login_success_page)
                    .await;
            }
    ...
    async fn login_chatgpt_common(
        &self,
        forestx_streamlined_login: bool,
        ...
        let opts = {
                ...
                && !open_app_url.trim().is_empty()
            {
                *url = open_app_url
                    .parse()
                    .map_err(|err| internal_error(format!("invalid Forestx open app URL: {err}")))?;
            }
            opts
        };
    ...
    async fn login_chatgpt_v2(
        ...
        let result = self
            .login_chatgpt_response(forestx_streamlined_login, login_success_page)
            .await;
        self.outgoing.send_result(request_id, result).await;
    }
    ...
    async fn login_chatgpt_response(
        ...
        let opts = self
            .login_chatgpt_common(forestx_streamlined_login, login_success_page)
            .await?;
    ...
    async fn logout_common(&self) -> std::result::Result<Option<AuthMode>, JSONRPCErrorError> {
        let managed_bedrock_auth = matches!(
            self.auth_manager.auth_cached(),
            Some(ForestxAuth::BedrockApiKey(_))
        );
    ...
    async fn get_account_rate_limits_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "forestx account authentication required to read rate limits",
            ));
        };
        ...
        let response = response
            .map_err(|err| internal_error(format!("failed to fetch forestx rate limits: {err}")))?;
        if response.rate_limits.is_empty() {
            return Err(internal_error(
                "failed to fetch forestx rate limits: no snapshots returned",
            ));
        }
        ...
        let rate_limits_by_limit_id: HashMap<_, _> = response
            ...
            .map(|snapshot| {
                let limit_id = snapshot
                    .limit_id
                    .clone()
                    .unwrap_or_else(|| "forestx".to_string());
                (limit_id, snapshot)
            })
            .collect();
    ...
    async fn get_account_token_usage_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "forestx account authentication required to read token usage",
            ));
        };
    ...
    async fn get_workspace_messages_response(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "forestx account authentication required to read workspace messages",
            ));
        };
    ...
    async fn send_add_credits_nudge_email_inner(
        ...
        let Some(auth) = self.auth_manager.auth().await else {
            return Err(invalid_request(
                "forestx account authentication required to notify workspace owner",
            ));
        };
...
mod tests {
    use super::*;
    use forestx_backend_client::TokenUsageProfileDailyBucket;
    use forestx_backend_client::TokenUsageProfileStats;
    ...
    fn workspace_messages_feature_disabled_only_for_not_found() {
        ...
        for (status, expected) in cases {
            let err = BackendRequestError::UnexpectedStatus {
                ...
                url: "https://example.test/api/forestx/workspace-messages".to_string(),
                status,

...
mod thread_processor_behavior_tests {
    async fn forked_from_id_from_rollout(path: &Path) -> Option<String> {
        forestx_core::read_session_meta_line(path)
            .await
            .ok()
    ...
    use forestx_state::ThreadMetadataBuilder;
    ...
    fn dynamic_tool(
        ...
        match namespace {
            Some(namespace) => {
                DynamicToolSpec::Namespace(forestx_app_server_protocol::DynamicToolNamespaceSpec {
    ...
    fn validate_dynamic_tools_rejects_duplicate_name_in_same_namespace() {
        ...
        let tools = vec![DynamicToolSpec::Namespace(
            forestx_app_server_protocol::DynamicToolNamespaceSpec {
        ...
        assert!(err.contains("forestx_app"), "unexpected error: {err}");
        assert!(err.contains("my_tool"), "unexpected error: {err}");
    }
    ...
    fn validate_dynamic_tools_rejects_namespace_not_supported_by_responses() {
        ...
        assert!(err.contains("forestx.app"), "unexpected error: {err}");
        assert!(
            err.contains("Responses API") && err.contains("^[a-zA-Z0-9_-]+$"),
            "unexpected error: {err}"
        );
    ...
    fn summary_from_stored_thread_preserves_millisecond_precision() {
        ...
        let stored_thread = StoredThread {
            ...
            thread_source: Some(forestx_protocol::protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            agent_path: None,
            git_info: None,
    ...
    fn requested_permissions_trust_project_uses_permission_profile_intent() {
        ...
        let full_access_profile = forestx_protocol::models::PermissionProfile::Disabled;
        let workspace_write_profile = forestx_protocol::models::PermissionProfile::workspace_write();
        let read_only_profile = forestx_protocol::models::PermissionProfile::read_only();
    ...
    async fn derive_config_from_params_uses_session_thread_config_model_provider() -> Result<()> {
        ...
        let session_provider = ModelProviderInfo {
            ...
            base_url: Some("http://127.0.0.1:8061/api/forestx".to_string()),
            env_key: None,
            ...
            .load_with_overrides(
                Some(HashMap::from([
                    ...
                    (
                        ...
                        json!({
                            "name": "request",
                            "base_url": "http://127.0.0.1:9999/api/forestx",
                            "wire_api": "responses",
                        }),
                    ),
                ])),
    ...
    fn collect_resume_override_mismatches_includes_service_tier() {
        ...
        let config_snapshot = ThreadConfigSnapshot {
            ...
            approval_policy: forestx_protocol::protocol::AskForApproval::OnRequest,
            approvals_reviewer: forestx_protocol::config_types::ApprovalsReviewer::User,
            permission_profile: forestx_protocol::models::PermissionProfile::Disabled,
    ...
    fn test_thread_metadata(
        ...
        let mut builder = ThreadMetadataBuilder::new(
            ...
            Utc::now(),
            forestx_protocol::protocol::SessionSource::default(),
        );
    ...
    fn merge_persisted_resume_metadata_prefers_persisted_model_and_reasoning_effort() -> Result<()>
    {
        ...
        let persisted_metadata =
            test_thread_metadata(Some("gpt-5.1-forestx-max"), Some(ReasoningEffort::High))?;
        ...
        assert_eq!(
            ...
            Some("gpt-5.1-forestx-max".to_string())
        );
        assert_eq!(
    ...
    fn merge_persisted_resume_metadata_preserves_explicit_overrides() -> Result<()> {
        ...
        assert_eq!(typesafe_overrides.model, Some("gpt-5.2-forestx".to_string()));
    ...
    fn merge_persisted_resume_metadata_skips_persisted_values_when_model_overridden() -> Result<()>
    {
        let mut request_overrides = Some(HashMap::from([(
            "model".to_string(),
            serde_json::Value::String("gpt-5.2-forestx".to_string()),
        )]));
        let mut typesafe_overrides = ConfigOverrides::default();
        let persisted_metadata =
            test_thread_metadata(Some("gpt-5.1-forestx-max"), Some(ReasoningEffort::High))?;
    ...
    async fn read_summary_from_rollout_preserves_agent_nickname() -> Result<()> {
        ...
        let session_meta = SessionMeta {
            ...
            thread_source: Some(forestx_protocol::protocol::ThreadSource::Subagent),
    ...
    fn summary_from_state_db_metadata_preserves_agent_nickname() -> Result<()> {
        ...
        let summary = summary_from_state_db_metadata(
            ...
            "0.0.0".to_string(),
            source,
            Some(forestx_protocol::protocol::ThreadSource::Subagent),

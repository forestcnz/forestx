...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match forestx_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: forestx_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> forestx_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        forestx_protocol::protocol::SessionSource::SubAgent(
...
) -> Option<forestx_app_server_protocol::ActivePermissionProfile> {
...
pub(crate) fn thread_settings_from_core_snapshot(
    snapshot: forestx_protocol::protocol::ThreadSettingsSnapshot,
) -> ThreadSettings {
    let forestx_protocol::protocol::ThreadSettingsSnapshot {
        model,
        model_provider_id,
        service_tier,
        approval_policy,
    ...
    let sandbox_policy = forestx_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        &permission_profile,
        cwd.as_path(),
    )
    .into();

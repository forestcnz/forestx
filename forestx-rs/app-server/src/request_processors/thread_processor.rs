...
use forestx_app_server_protocol::SelectedCapabilityRoot;
use forestx_app_server_protocol::ThreadSection;
use forestx_app_server_protocol::ThreadSectionMoveParams;
use forestx_app_server_protocol::ThreadSectionMoveResponse;
...
const FORESTX_TUI_CLIENT_NAME: &str = "forestx-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: forestx_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                forestx_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = forestx_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(FORESTX_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<ForestxThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            forestx_home: self.config.forestx_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(forestx_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<forestx_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<forestx_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                forestx_core::config::set_project_trust_level(
                    &listener_task_context.forestx_home,
        ...
        if let Ok(Some(err)) =
            forestx_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(forestx_app_server_protocol::ThreadStartSource::Startup)
                {
                    forestx_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    forestx_app_server_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
            ...
            .map_err(|err| match err.details() {
                ...
                ForestxErrorDetails::UnsupportedOperation(message) => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<forestx_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<forestx_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(forestx_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(forestx_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            forestx_linux_sandbox_exe: self.arg0_paths.forestx_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = forestx_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.forestx_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.forestx_home.display()
                ))
            })?;
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                forestx_app_server_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    ) -> Result<forestx_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = forestx_thread.legacy_instruction_sources().await;
                ...
                thread.thread_source = forestx_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = forestx_thread.config_snapshot().await;
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, forestx_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(forestx_core::util::normalize_thread_name);
        let prepared_fork = if paginated_source {
            let boundary = match (last_turn_id.as_deref(), before_turn_id.as_deref()) {
                (Some(turn_id), None) => {
                    forestx_thread_store::ForkBoundary::ThroughTurn(turn_id.to_string())
                }
                (None, Some(turn_id)) => {
                    forestx_thread_store::ForkBoundary::BeforeTurn(turn_id.to_string())
                }
                (None, None) => forestx_thread_store::ForkBoundary::Latest,
                (Some(_), Some(_)) => unreachable!("fork boundaries are mutually exclusive"),
            };
            Some(
                self.thread_store
                    .prepare_fork(forestx_thread_store::PrepareForkParams {
        ...
        let NewThread {
            ...
            ForestxErrorDetails::Io(_) | ForestxErrorDetails::Json(_) => {
...
fn deserialize_stored_thread_item(
    item: forestx_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        forestx_error_info: error.forestx_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: ForestxErr) -> JSONRPCErrorError {
    match err.details() {
        ForestxErrorDetails::ThreadNotFound(thread_id) => {
            invalid_request(format!("thread not found: {thread_id}"))
        }
        ForestxErrorDetails::InvalidRequest(message) => invalid_request(message.clone()),
        ForestxErrorDetails::UnsupportedOperation(message) => method_not_found(message.clone()),
...
) -> (Thread, Option<forestx_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<forestx_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(forestx_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match forestx_core::parse_turn_item(item) {
                Some(forestx_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            forestx_protocol::config_types::SandboxMode::WorkspaceWrite
                | forestx_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &forestx_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        forestx_protocol::models::PermissionProfile::Disabled
        | forestx_protocol::models::PermissionProfile::External { .. } => true,
        forestx_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<forestx_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {

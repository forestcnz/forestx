use super::*;
use forestx_core::config::permission_profile_catalog;
...
) -> Vec<forestx_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            forestx_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    forestx_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| forestx_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[forestx_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[forestx_core::skills::SkillError],
) -> Vec<forestx_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| forestx_app_server_protocol::SkillErrorInfo {
...
impl CatalogRequestProcessor {
    ...
    async fn experimental_feature_list_response(
        ...
        let workspace_forestx_plugins_enabled = self
            ...
            .map(|spec| {
                ...
                ApiExperimentalFeature {
                    ...
                    enabled: config.features.enabled(spec.id)
                        && (workspace_forestx_plugins_enabled
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                forestx_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![forestx_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let (effective_skill_roots, plugin_skill_snapshots) =
                        if workspace_forestx_plugins_enabled {
                    ...
                    let skills_input = forestx_core::skills::HostSkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(forestx_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![forestx_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let workspace_forestx_plugins_enabled = self
                ...
                .await;
            let plugins_enabled =
                config.features.enabled(Feature::Plugins) && workspace_forestx_plugins_enabled;
            let plugin_hooks = if plugins_enabled {
                let plugins_input = config.plugins_config_input();
                let plugin_outcome = plugins_manager.plugins_for_config(&plugins_input).await;
                forestx_core_plugins::PluginHookLoadOutcome {
                ...
                forestx_core_plugins::PluginHookLoadOutcome::default()
            };
            let hooks = forestx_hooks::list_hooks(forestx_hooks::HooksConfig {
    ...
    async fn skills_config_write_response_inner(
        ...
        let edits = vec![edit];
        ConfigEditsBuilder::new(&self.config.forestx_home)
            .with_edits(edits)
            .apply()
            .await
            .map(|()| {

...
use forestx_connectors::ConnectorRuntimeTool;
use forestx_connectors::connector_runtime_context_key;
use forestx_connectors::connector_tool_is_synthetic;
use forestx_connectors::installed_connector_runtime;
...
use forestx_mcp::host_owned_forestx_apps_enabled;
use forestx_mcp::tool_is_model_visible;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "forestx.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "forestx.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "forestx.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "forestx.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.forestx_home.to_path_buf(), cache_key.clone());
            let snapshot = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    anyhow::ensure!(
                        !mcp_servers.is_empty(),
                        "host-owned MCP server '{FORESTX_APPS_MCP_SERVER_NAME}' is not enabled"
                    );
                    ...
                    let forestx_apps_auth_manager =
                        host_owned_forestx_apps_enabled(&mcp_config, auth.as_ref())
                    ...
                    let runtime = McpRuntime::new(McpRuntimeInput {
                        ...
                        forestx_apps_tools_cache_key: cache_key.clone(),
                        ...
                        .await
                    {
                        ...
                        Err(anyhow::anyhow!(
                            "failed to refresh tools for MCP server '{FORESTX_APPS_MCP_SERVER_NAME}'"
                        ))
                    };
...
fn record_apps_installed_metrics(
    metrics: &forestx_otel::MetricsClient,
    started_at: Instant,
    force_refresh: bool,

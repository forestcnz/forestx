...
use forestx_extension_api::AgentSpawnFuture;
use forestx_extension_api::AgentSpawner;
...
pub(crate) struct ThreadExtensionDependencies {
    ...
    pub(crate) executor_skill_provider: Arc<dyn forestx_skills_extension::SkillProvider>,
...
where
    S: AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = ForestxErr> + 'static,
{
    ...
    if let Some(state_db) = state_db {
        forestx_goal_extension::install_with_backend(
            &mut builder,
            state_db,
            analytics_events_client,
            forestx_otel::global(),
            thread_manager,
            goal_service,
            |config: &Config| config.features.enabled(forestx_features::Feature::Goals),
        );
    }
    ...
    forestx_guardian::install(&mut builder, guardian_agent_spawner);
    forestx_memories_extension::install(&mut builder, forestx_otel::global());
    forestx_mcp_extension::install(&mut builder);
    forestx_mcp_extension::install_executor_plugins(&mut builder, environment_manager);
    forestx_web_search_extension::install(&mut builder, auth_manager.clone());
    forestx_image_generation_extension::install(&mut builder, auth_manager, |config: &Config| {
    ...
    let skill_providers = forestx_skills_extension::SkillProviders::new()
        ...
        .with_orchestrator_provider(Arc::new(
            forestx_skills_extension::OrchestratorSkillProvider::new(),
        ))
...
) -> impl AgentSpawner<StartThreadOptions, Spawned = NewThread, Error = ForestxErr> {
    move |forked_from_thread_id: ThreadId,
          options: StartThreadOptions|
          -> AgentSpawnFuture<'static, NewThread, ForestxErr> {
        let thread_manager = thread_manager.clone();
        Box::pin(async move {
            let thread_manager = thread_manager.upgrade().ok_or_else(|| {
                ForestxErr::UnsupportedOperation("thread manager dropped".to_string())
            })?;
            thread_manager
...
#[cfg(test)]
mod tests {
    use forestx_protocol::protocol::ThreadGoal as CoreThreadGoal;

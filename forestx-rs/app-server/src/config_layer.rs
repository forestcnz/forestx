use forestx_app_server_protocol::ConfigLayer as ApiConfigLayer;
use forestx_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use forestx_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use forestx_config::ConfigLayer;
use forestx_config::ConfigLayerMetadata;
use forestx_config::ConfigLayerSource;

/// Converts a config-layer source owned by `forestx-config` into the app-server wire type owned by
/// `forestx-app-server-protocol`.
///
...
pub(crate) fn config_layer_source_to_api(source: ConfigLayerSource) -> ApiConfigLayerSource {
    match source {
        ...
        ConfigLayerSource::Project { dot_forestx_folder } => {
            ApiConfigLayerSource::Project { dot_forestx_folder }
        }
...
/// Converts config-layer metadata owned by `forestx-config` into the app-server wire type owned by
/// `forestx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `forestx-config` into the app-server wire type owned by
/// `forestx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion

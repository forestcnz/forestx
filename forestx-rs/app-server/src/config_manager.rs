...
use forestx_cloud_config::cloud_config_bundle_loader;
...
use forestx_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective Forestx configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn forestx_home(&self) -> &Path {
        self.forestx_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.forestx_home())
    }
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(forestx_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
        let mut config = Config::load_default_with_cli_overrides_for_forestx_home(
            self.forestx_home.clone(),
            self.current_cli_overrides(),
        )
        .await?;
        ...
        {
            let user_config_path = self.loader_overrides.user_config_path(self.forestx_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = forestx_core::config::ConfigBuilder::default()
            .forestx_home(self.forestx_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.forestx_self_exe = self.arg0_paths.forestx_self_exe.clone();
        config.forestx_linux_sandbox_exe = self.arg0_paths.forestx_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(forestx_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(forestx_home: PathBuf) -> Self {
        Self::new_for_tests(
            forestx_home,
            Vec::new(),

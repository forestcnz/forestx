...
use forestx_app_server_protocol::Config as ApiConfig;
...
use forestx_app_server_protocol::OverriddenMetadata;
use forestx_app_server_protocol::WriteStatus;
use forestx_config::CONFIG_TOML_FILE;
use forestx_config::ConfigLayerEntry;
use forestx_config::ConfigLayerMetadata;
use forestx_config::ConfigLayerSource;
use forestx_config::ConfigLayerStack;
use forestx_config::ConfigRequirementsToml;
use forestx_config::ShellEnvironmentPolicyFilterRepresentation;
use forestx_config::config_toml::ConfigToml;
use forestx_config::merge_toml_values;
use forestx_config::shell_environment_filter_entry;
use forestx_config::validate_shell_environment_policy_filter_config;
use forestx_core::config::deserialize_config_toml_with_base;
use forestx_core::config::edit::ConfigEdit;
use forestx_core::config::edit::ConfigEditsBuilder;
use forestx_core::config::validate_feature_requirements_for_config_toml;
use forestx_core::path_utils;
use forestx_core::path_utils::SymlinkWritePaths;
use forestx_core::path_utils::resolve_symlink_write_paths;
use forestx_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.forestx_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .forestx/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_forestx_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_forestx_folder.display(),
        ),

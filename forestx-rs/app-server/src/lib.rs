...
use forestx_code_mode::WebSocketCodeModeSessionProvider;
...
use forestx_analytics::AppServerRpcTransport;
...
use forestx_app_server_protocol::TextPosition as AppTextPosition;
use forestx_app_server_protocol::TextRange as AppTextRange;
use forestx_config::ConfigLayerSource;
use forestx_config::ConfigLoadError;
use forestx_config::TextRange as CoreTextRange;
use forestx_core::ExecPolicyError;
...
use forestx_rollout::state_db as rollout_state_db;
...
const SQLITE_RECOVERY_CONFIG_WARNING_SUMMARY: &str = "Forestx rebuilt its local database.";
...
const OTEL_SERVICE_NAME: &str = "forestx-app-server";
#[cfg(debug_assertions)]
const TEST_USER_CONFIG_FILE_ENV_VAR: &str = "FORESTX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.all_layers_low_to_high() {
        let ConfigLayerSource::Project { dot_forestx_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_forestx_folder.as_path().display().to_string(),
...
pub async fn run_main_with_transport_options(
    ...
    let otel = forestx_core::otel_init::build_provider(
        &config,
    ...
    forestx_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    forestx_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    let unix_socket_startup_lock = match &transport {
        AppServerTransport::UnixSocket { socket_path } => {
            let startup_lock_path = app_server_startup_lock_path(&forestx_home)?;
    ...
    let feedback = ForestxFeedback::new();
...
async fn init_sqlite_state_db_with_fresh_start_on_corruption(
    ...
    loop {
        ...
        if !forestx_state::is_sqlite_corruption_error(&err)
        ...
        emit_state_db_backup_warning(&format!(
            "Forestx local database at {} appears damaged. Moving it into a backup folder so the app server can rebuild it from saved data.",
            database_path.display()
        ));
        let backups = forestx_state::backup_runtime_db_for_fresh_start(database_path.as_path())
            .await
            .map_err(|backup_err| {
                anyhow::anyhow!(
        ...
        for backup in &backups {
            emit_state_db_backup_warning(&format!(
                "Moved damaged Forestx local database file {} to {}",
...
mod tests {
    ...
    fn debug_test_user_config_file_overrides_loader_path() {
        let path = std::env::temp_dir().join("forestx-app-server-test-config.toml");
        let loader_overrides = loader_overrides_with_test_user_config_file(

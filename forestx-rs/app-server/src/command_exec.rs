...
use forestx_app_server_protocol::CommandExecOutputDeltaNotification;
use forestx_app_server_protocol::CommandExecOutputStream;
use forestx_app_server_protocol::CommandExecResizeParams;
use forestx_app_server_protocol::CommandExecResizeResponse;
use forestx_app_server_protocol::CommandExecResponse;
use forestx_app_server_protocol::CommandExecTerminalSize;
use forestx_app_server_protocol::CommandExecTerminateParams;
use forestx_app_server_protocol::CommandExecTerminateResponse;
use forestx_app_server_protocol::CommandExecWriteParams;
use forestx_app_server_protocol::CommandExecWriteResponse;
use forestx_app_server_protocol::JSONRPCErrorError;
use forestx_app_server_protocol::ServerNotification;
use forestx_core::config::StartedNetworkProxy;
use forestx_core::exec::ExecExpiration;
use forestx_core::exec::ExecExpirationOutcome;
use forestx_core::exec::IO_DRAIN_TIMEOUT_MS;
use forestx_core::sandboxing::ExecRequest;
use forestx_protocol::exec_output::bytes_to_string_smart;
use forestx_sandboxing::SandboxType;
use forestx_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use forestx_utils_pty::ProcessHandle;
use forestx_utils_pty::SpawnedProcess;
use forestx_utils_pty::TerminalSize;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match forestx_core::sandboxing::execute_env(exec_request, /*stdout_stream*/ None)
                    .await
                {
                    Ok(output) => {
                        outgoing
        ...
        let spawned = if tty {
            forestx_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            forestx_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0, &[])
                .await
        } else {
            forestx_utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            forestx_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: forestx_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: forestx_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: forestx_app_server_protocol::RequestId::Integer(100),
        };
        let cwd = AbsolutePathBuf::current_dir().expect("current dir");
        ...
        let forestx_app_server_protocol::ClientResponsePayload::OneOffCommandExec(response) =
            *response.result
        else {
    ...
    async fn timeout_or_cancellation_reports_cancellation_without_timeout_exit_code() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: forestx_app_server_protocol::RequestId::Integer(101),
        };
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: forestx_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: forestx_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: forestx_app_server_protocol::RequestId::Integer(3),
        };

...
use forestx_app_server_protocol::experimental_required_message;
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) feedback: ForestxFeedback,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let thread_store = forestx_core::thread_store_from_config(config.as_ref(), state_db.clone());
        let environment_manager_for_requests = Arc::clone(&environment_manager);
        let environment_manager_for_extensions = Arc::clone(&environment_manager);
        let restriction_product = session_source.restriction_product();
        let executor_skill_provider: Arc<dyn forestx_skills_extension::SkillProvider> = Arc::new(
            forestx_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            let manager = ThreadManager::new(
                config.as_ref(),
                auth_manager.clone(),
                forestx_core::build_models_manager(config.as_ref(), auth_manager.clone()),
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                let forestx_request = deserialize_client_request(request);
                let result = match forestx_request {
                    Ok(forestx_request) => {
    ...
    async fn handle_client_request(
        ...
        if let ClientRequest::Initialize { request_id, params } = forestx_request {
    ...
    async fn dispatch_initialized_client_request(
        ...
        if let Some(reason) = forestx_request.experimental_reason()
        ...
        let serialization_scope = forestx_request.serialization_scope();
    ...
    async fn handle_initialized_client_request(
        ...
        let request_id = ConnectionRequestId {
            connection_id,
            request_id: forestx_request.id().clone(),
        };

        let result: Result<Option<ClientResponsePayload>, JSONRPCErrorError> = match forestx_request {

...
//!
//! # Relationship to `forestx-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `forestx-app-server-client`,
...
pub use forestx_rollout::StateDbHandle;
pub use forestx_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `forestx-app-server-client`, which adds worker-task buffering,
...
pub struct InProcessClientHandle {
    ...
    #[cfg(test)]
    _test_forestx_home: Option<tempfile::TempDir>,
}
...
async fn start_uninitialized(args: InProcessStartArgs) -> IoResult<InProcessClientHandle> {
    ...
    let installation_id = resolve_installation_id(&args.config.forestx_home).await?;
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        let auth_manager =
            AuthManager::shared_from_config(args.config.as_ref(), args.enable_forestx_api_key_env)
                .await;
        ...
        let mut config_manager = ConfigManager::new(
            args.config.forestx_home.to_path_buf(),
            args.cli_overrides,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config(forestx_home.path()).await);
        let state_db = forestx_rollout::state_db::try_init(config.as_ref())
            .await
        ...
        let args = InProcessStartArgs {
            ...
            initialize: InitializeParams {
                client_info: ClientInfo {
                    name: "forestx-in-process-test".to_string(),
                    title: None,
        ...
        client._test_forestx_home = Some(forestx_home);
        client
    }

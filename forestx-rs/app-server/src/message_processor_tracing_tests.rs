...
fn init_test_tracing() -> &'static TestTracing {
    ...
    TEST_TRACING.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("forestx-app-server-message-processor-tests");
...
impl TracingHarness {
    async fn new() -> Result<Self> {
        ...
        let config = Arc::new(build_test_config(forestx_home.path(), &server.uri()).await?);
...
async fn build_test_config(forestx_home: &Path, server_uri: &str) -> Result<Config> {
...
async fn build_test_processor(
    ...
    let auth_manager =
        AuthManager::shared_from_config(config.as_ref(), /*enable_forestx_api_key_env*/ false).await;
        ...
        match envelope {
            crate::outgoing_message::OutgoingEnvelope::ToConnection {
                ...
                if matches!(
                    ...
                    forestx_app_server_protocol::ServerNotification::ThreadStarted(_)
                ) {
                    return;
                }
            }
...
async fn turn_start_jsonrpc_span_parents_core_turn_spans() -> Result<()> {
    ...
    let spans = wait_for_exported_spans(harness.tracing, |spans| {
        spans.iter().any(|span| {
            ...
            span_attr(span, "forestx.op") == Some("user_input")
                && span.span_context.trace_id() == remote_trace_id
        })
    })
    .await;
    ...
    let core_turn_span =
        find_span_with_trace(&spans, remote_trace_id, "forestx.op=user_input", |span| {
            span_attr(span, "forestx.op") == Some("user_input")
        });

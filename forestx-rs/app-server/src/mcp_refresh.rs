...
mod tests {
    ...
    use forestx_config::ThreadConfigContext;
    use forestx_config::ThreadConfigLoadError;
    use forestx_config::ThreadConfigLoadErrorCode;
    ...
    use forestx_extension_api::NoopExtensionEventSink;
    ...
    async fn strict_refresh_reports_thread_planning_failures() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(forestx_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_config_preserves_thread_mcp_overrides() -> anyhow::Result<()> {
        ...
        let thread_config = initial_config_manager
            ...
            .await?;
        let thread = thread_manager
            .start_thread(forestx_core::StartThreadOptions::new(thread_config))
            .await?
            .thread;
        std::fs::write(
            temp_dir.path().join(forestx_config::CONFIG_TOML_FILE),
...
"#,
        ...
        actual.remove(forestx_mcp::FORESTX_APPS_MCP_SERVER_NAME);
...
"#,
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                forestx_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        analytics_events_client: forestx_analytics::AnalyticsEventsClient::disabled(),
                        thread_manager: thread_manager.clone(),
                        goal_service: Arc::new(forestx_goal_extension::GoalService::new()),
                ...
                Arc::new(ForestxHomeUserInstructionsProvider::new(
                    good_config.forestx_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                forestx_core::local_agent_graph_store_from_state_db(Some(&state_db)),
        ...
        thread_manager
            .start_thread(forestx_core::StartThreadOptions::new(good_config))
            .await?;
        thread_manager
            .start_thread(forestx_core::StartThreadOptions::new(bad_config))
            .await?;
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> forestx_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {

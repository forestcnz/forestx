...
fn create_config_toml(forestx_home: &Path) -> std::io::Result<()> {
...
fn create_config_toml_forced_login(forestx_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = forestx_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(forestx_home.path())?;
...
async fn personal_access_token_without_email_supports_auth_status_and_account_read() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("OPENAI_API_KEY", None),
            ("FORESTX_ACCESS_TOKEN", Some("at-test-token")),
            ("FORESTX_AUTHAPI_BASE_URL", Some(authapi_base_url.as_str())),
        ])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let forestx_home = TempDir::new()?;
    create_config_toml_custom_provider(forestx_home.path(), /*requires_openai_auth*/ false)?;
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let forestx_home = TempDir::new()?;
    create_config_toml_forced_login(forestx_home.path(), "chatgpt")?;

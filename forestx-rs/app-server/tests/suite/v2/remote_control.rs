...
use forestx_app_server::AppServerRuntimeOptions;
use forestx_app_server::AppServerTransport;
use forestx_app_server::AppServerWebsocketAuthSettings;
use forestx_app_server::PluginStartupTasks;
use forestx_app_server::RemoteControlStartupMode;
use forestx_app_server::run_main_with_transport_options;
...
use forestx_app_server_protocol::RemoteControlClient;
use forestx_app_server_protocol::RemoteControlClientsListOrder;
use forestx_app_server_protocol::RemoteControlClientsListParams;
use forestx_app_server_protocol::RemoteControlClientsListResponse;
use forestx_app_server_protocol::RemoteControlClientsRevokeParams;
use forestx_app_server_protocol::RemoteControlClientsRevokeResponse;
use forestx_app_server_protocol::RemoteControlConnectionStatus;
use forestx_app_server_protocol::RemoteControlDisableResponse;
use forestx_app_server_protocol::RemoteControlEnableResponse;
use forestx_app_server_protocol::RemoteControlPairingStartParams;
use forestx_app_server_protocol::RemoteControlPairingStartResponse;
use forestx_app_server_protocol::RemoteControlPairingStatusParams;
use forestx_app_server_protocol::RemoteControlPairingStatusResponse;
use forestx_app_server_protocol::RemoteControlStatusChangedNotification;
use forestx_app_server_protocol::RemoteControlStatusReadResponse;
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = forestx_home.path().join("managed_config.toml");
    let socket_path = forestx_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _forestx_home_guard = EnvVarGuard::set("FORESTX_HOME", forestx_home.path().as_os_str());
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let listener = configured_remote_control_listener(forestx_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(forestx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(forestx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn stdio_eof_exits_with_remote_control_connection() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let mut backend = ConnectedRemoteControlBackend::start(forestx_home.path()).await?;
    let mut app_server = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(forestx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(forestx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized()
...
impl ConnectedRemoteControlBackend {
    async fn start(forestx_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(forestx_home).await?;
...
async fn configured_remote_control_listener(forestx_home: &std::path::Path) -> Result<TcpListener> {

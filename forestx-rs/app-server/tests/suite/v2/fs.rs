...
use forestx_app_server_protocol::FsChangedNotification;
use forestx_app_server_protocol::FsCopyParams;
use forestx_app_server_protocol::FsGetMetadataResponse;
use forestx_app_server_protocol::FsReadDirectoryEntry;
use forestx_app_server_protocol::FsReadFileResponse;
use forestx_app_server_protocol::FsUnwatchParams;
use forestx_app_server_protocol::FsWatchResponse;
...
async fn initialized_mcp(forestx_home: &TempDir) -> Result<TestAppServer> {
...
async fn fs_get_metadata_returns_only_used_fields() -> Result<()> {
    ...
    let file_path = forestx_home.path().join("note.txt");
    std::fs::write(&file_path, "hello")?;

    let mut mcp = initialized_mcp(&forestx_home).await?;
    let request_id = mcp
        .send_fs_get_metadata_request(forestx_app_server_protocol::FsGetMetadataParams {
...
async fn fs_methods_return_error_when_local_environment_is_disabled() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let absolute_file = forestx_home.path().join("absolute.txt");

    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .with_env_overrides(&[(FORESTX_EXEC_SERVER_URL_ENV_VAR, Some("none"))])
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;

    let read_id = mcp
        .send_fs_read_file_request(forestx_app_server_protocol::FsReadFileParams {
...
async fn fs_get_metadata_reports_symlink() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let file_path = forestx_home.path().join("note.txt");
    let symlink_path = forestx_home.path().join("note-link.txt");
    std::fs::write(&file_path, "hello")?;
...
async fn fs_methods_cover_current_fs_utils_surface() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let source_dir = forestx_home.path().join("source");
    let nested_dir = source_dir.join("nested");
    let source_file = source_dir.join("root.txt");
    let copied_dir = forestx_home.path().join("copied");
    let copy_file_path = forestx_home.path().join("copy.txt");
    let nested_file = nested_dir.join("note.txt");

    let mut mcp = initialized_mcp(&forestx_home).await?;

    let create_directory_request_id = mcp
        .send_fs_create_directory_request(forestx_app_server_protocol::FsCreateDirectoryParams {
    ...
    let read_directory_request_id = mcp
        .send_fs_read_directory_request(forestx_app_server_protocol::FsReadDirectoryParams {
    ...
    .await??;
    let mut entries =
        to_response::<forestx_app_server_protocol::FsReadDirectoryResponse>(readdir_response)?
            .entries;
    ...
    let remove_request_id = mcp
        .send_fs_remove_request(forestx_app_server_protocol::FsRemoveParams {
...
async fn fs_write_file_accepts_base64_bytes() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let file_path = forestx_home.path().join("blob.bin");
...
async fn fs_copy_rejects_directory_without_recursive() -> Result<()> {
    ...
    let request_id = mcp
        .send_fs_copy_request(FsCopyParams {
            ...
            destination_path: absolute_path(forestx_home.path().join("dest")),
            recursive: false,
        })
        .await?;
    let error = timeout(
...
async fn fs_copy_rejects_standalone_fifo_source() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let fifo_path = forestx_home.path().join("named-pipe");
    let output = Command::new("mkfifo").arg(&fifo_path).output()?;
    if !output.status.success() {
        anyhow::bail!(
            "mkfifo failed: stdout={} stderr={}",
    ...
    let request_id = mcp
        .send_fs_copy_request(FsCopyParams {
            ...
            destination_path: absolute_path(forestx_home.path().join("copied")),
            recursive: false,
        })
        .await?;
    expect_error_message(
...
-> Result<()> {
    let forestx_home = TempDir::new()?;
    let git_dir = forestx_home.path().join("repo").join(".git");
    ...
    let watch_request_id = mcp
        .send_fs_watch_request(forestx_app_server_protocol::FsWatchParams {

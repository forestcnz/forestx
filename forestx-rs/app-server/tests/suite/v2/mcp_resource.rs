...
use forestx_app_server_protocol::McpResourceContent;
...
const TEST_RESOURCE_URI: &str = "test://forestx/resource";
const TEST_BLOB_RESOURCE_URI: &str = "test://forestx/resource.bin";
const TEST_RESOURCE_BLOB: &str = "YmluYXJ5LXJlc291cmNl";
const TEST_RESOURCE_TEXT: &str = "Resource body from the MCP server.";
const TEST_ELICITATION_RESOURCE_URI: &str = "test://forestx/elicitation";
...
async fn mcp_resource_read_returns_resource_contents() -> Result<()> {
    ...
    let (_forestx_home, mut mcp) = start_resource_test_app_server(
        &apps_server_url,
...
async fn orchestrator_skill_can_read_referenced_resource_without_an_executor() -> Result<()> {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "warnings": ["Orchestrator skill discovery stopped after 2 resource pages: failed to list orchestrator skill resources: resources/list failed for `forestx_apps`: Mcp error: -32603: simulated later-page failure"],
            "next_cursor": null,
        })
    );
...
async fn disabled_orchestrator_skills_do_not_expose_skills_namespace() -> Result<()> {
    ...
    let (apps_server_url, apps_server_calls, apps_server_handle) =
        start_resource_apps_mcp_server().await?;
    let responses_server_uri = responses_server.uri();
    let (_forestx_home, mut mcp) = start_resource_test_app_server_with_extra_config(
        &apps_server_url,
        &responses_server_uri,
        r#"
[orchestrator.skills]
...
async fn start_resource_test_app_server_with_extra_config(
    ...
    let builder = TestAppServer::builder().with_forestx_home(forestx_home.path());
...
-> Result<(String, Arc<ResourceAppsMcpCalls>, JoinHandle<()>)> {
    ...
    let router = Router::new().nest_service("/api/forestx/ps/mcp", mcp_service);

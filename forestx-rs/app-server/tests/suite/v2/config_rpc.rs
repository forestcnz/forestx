...
use forestx_app_server_protocol::AppConfig;
use forestx_app_server_protocol::AppToolApproval;
use forestx_app_server_protocol::ApprovalsReviewer;
use forestx_app_server_protocol::AppsConfig;
use forestx_app_server_protocol::AppsDefaultConfig;
use forestx_app_server_protocol::AskForApproval;
use forestx_app_server_protocol::ConfigBatchWriteParams;
use forestx_app_server_protocol::ConfigEdit;
use forestx_app_server_protocol::ConfigLayerSource;
...
use forestx_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use forestx_app_server_protocol::ToolsV2;
...
use forestx_protocol::config_types::WebSearchContextSize;
use forestx_protocol::config_types::WebSearchLocation;
...
fn write_config(forestx_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let forestx_home_path = forestx_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(forestx_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("forestx")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let forestx_home = TempDir::new()?;
    write_config(&forestx_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_forestx_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let forestx_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path = AbsolutePathBuf::resolve_path_against_base("config.toml", forestx_home);
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(forestx_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let forestx_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(forestx_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(forestx_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[forestx_app_server_protocol::ConfigLayer],

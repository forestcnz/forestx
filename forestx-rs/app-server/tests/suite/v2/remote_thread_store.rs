...
//! the temporary `forestx_home` must not contain rollout session files or sqlite
...
use forestx_app_server::in_process::InProcessClientHandle;
use forestx_app_server::in_process::InProcessServerEvent;
...
use forestx_app_server_protocol::ThreadSectionCreateParams;
use forestx_app_server_protocol::ThreadSectionDeleteParams;
use forestx_app_server_protocol::ThreadSectionListParams;
use forestx_app_server_protocol::ThreadSectionUpdateParams;
...
use forestx_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_section_operations_without_sqlite_return_method_not_found() -> Result<()> {
    ...
    create_config_toml_with_thread_store(forestx_home.path(), "http://127.0.0.1:1", &store_id)?;
    let _in_memory_store = InMemoryThreadStoreId { store_id };
    let client = start_in_process_server(forestx_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(forestx_home.path())?;
...
async fn thread_start_rejects_paginated_history_without_list_support() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(forestx_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let forestx_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under forestx_home. Disable them
    ...
    let mut client = start_in_process_server(forestx_home.path()).await?;
...
async fn start_in_process_server(forestx_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
            .forestx_home(forestx_home.to_path_buf())
            .fallback_cwd(Some(forestx_home.to_path_buf()))
            .loader_overrides(loader_overrides.clone())
            .build()
            .await?,
    );
...
fn assert_no_local_persistence_artifacts(forestx_home: &Path) -> Result<()> {
    ...
    // the isolated test forestx_home.
    assert!(
        !forestx_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !forestx_home.join("archived_sessions").exists(),
    ...
    let sqlite_artifacts = std::fs::read_dir(forestx_home)?
    ...
    let mut entries = forestx_home_entries(forestx_home)?;
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in forestx_home"
    );

    Ok(())
}

fn forestx_home_entries(forestx_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(forestx_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;

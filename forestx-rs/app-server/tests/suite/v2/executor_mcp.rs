...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let executor_config: forestx_config::types::McpServerConfig = serde_json::from_value(json!({
    ...
    let oauth_credentials_path = forestx_home.path().join(".credentials.json");
    std::fs::write(
        &oauth_credentials_path,
        serde_json::to_vec(&json!({"host": host_oauth_credential.clone()}))?,
    )?;
    let forestx_bin = toml::Value::String(
        forestx_utils_cargo_bin::cargo_bin("forestx")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".forestx-plugin"))?;
    std::fs::write(
        plugin.path().join(".forestx-plugin/plugin.json"),

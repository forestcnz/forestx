...
use forestx_protocol::mcp_approval_meta as approval_meta;
...
const TOOL_NAMESPACE: &str = "mcp__forestx_apps__calendar";
...
async fn openai_form_capability_follows_the_turn_starting_connection() -> Result<()> {
    ...
    write_config_toml(forestx_home.path(), &responses_server.uri(), &apps_server_url)?;
    write_chatgpt_auth(
        forestx_home.path(),
...
impl ElicitationRoundTripFixture {
    async fn start(scenario: ElicitationScenario) -> Result<Self> {
        ...
        if !requirements.is_empty() {
            std::fs::write(forestx_home.path().join("requirements.toml"), requirements)?;
        }
        write_chatgpt_auth(
            forestx_home.path(),
            ChatGptAuthFixture::new("chatgpt-token")
            ...
            ElicitationScenario::StandardForm | ElicitationScenario::Strict(_) => {
                if let ElicitationScenario::Strict(strict) = scenario {
                    ...
                    for index in 0..strict.review_outcomes().len().max(1) {
                        ...
                        let apps_meta = context.meta.0.0.get(MCP_TOOL_FORESTX_APPS_META_KEY);
                        let mut meta = MetaObject(
                            json!({
                                ...
                                (MCP_TOOL_FORESTX_APPS_META_KEY): apps_meta

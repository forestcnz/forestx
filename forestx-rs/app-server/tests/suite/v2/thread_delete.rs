...
async fn thread_delete_deletes_spawned_descendants() -> Result<()> {
    ...
    let parent_id = create_delete_test_rollout(forestx_home.path(), /*minute*/ 0, "parent")?;
    let child_id = create_delete_test_rollout(forestx_home.path(), /*minute*/ 1, "child")?;
    let grandchild_id =
        create_delete_test_rollout(forestx_home.path(), /*minute*/ 2, "grandchild")?;
...
async fn thread_delete_preflights_external_fork_references_for_spawned_subtrees() -> Result<()> {
    ...
    let parent_id = create_delete_test_rollout(forestx_home.path(), /*minute*/ 0, "parent")?;
    let child_id = create_delete_test_rollout(forestx_home.path(), /*minute*/ 1, "child")?;
    let external_id = create_delete_test_rollout(forestx_home.path(), /*minute*/ 2, "external")?;
    let parent_thread_id = ThreadId::from_string(&parent_id)?;
    let child_thread_id = ThreadId::from_string(&child_id)?;
    ...
    let state_db = StateRuntime::init(
        SqliteConfig::new_for_testing(forestx_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
    state_db
...
fn create_delete_test_rollout(forestx_home: &Path, minute: u8, preview: &str) -> Result<String> {
    create_fake_rollout(
        forestx_home,

...
use forestx_app_server_protocol::ThreadRealtimeAudioChunk;
...
use forestx_app_server_protocol::ThreadRealtimeInitialItem;
...
impl RealtimeE2eHarness {
    ...
    async fn start_webrtc_realtime(&mut self, offer_sdp: &str) -> Result<StartedWebrtcRealtime> {
        self.start_webrtc_realtime_with_forestx_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*forestx_responses_as_items*/ None,
            /*forestx_response_handoff_mode*/ None,
    ...
    async fn start_webrtc_realtime_with_forestx_response_items(
        &mut self,
        offer_sdp: &str,
        ...
        self.start_webrtc_realtime_with_forestx_response_routing(
            offer_sdp,
            /*client_managed_handoffs*/ None,
            /*forestx_responses_as_items*/ Some(true),
            /*forestx_response_handoff_mode*/ None,
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
    ...
    async fn start_webrtc_realtime_with_forestx_response_routing(
        &mut self,
        offer_sdp: &str,
        client_managed_handoffs: Option<bool>,
        forestx_responses_as_items: Option<bool>,
        forestx_response_handoff_mode: Option<ForestxResponseHandoffMode>,
        ...
        let start_request_id = self
            ...
            .send_thread_realtime_start_request(ThreadRealtimeStartParams {
                ...
                forestx_response_item_prefix: forestx_responses_as_items
                    .unwrap_or(false)
    ...
    async fn start_websocket_realtime(&mut self) -> Result<ThreadRealtimeStartedNotification> {
        self.start_websocket_realtime_with_forestx_responses_as_items(
            /*forestx_responses_as_items*/ None,
        )
        .await
    }

    async fn start_websocket_realtime_with_forestx_response_items(
        &mut self,
    ...
    async fn start_websocket_realtime_with_forestx_responses_as_items(
        &mut self,
        forestx_responses_as_items: Option<bool>,
    ) -> Result<ThreadRealtimeStartedNotification> {
        let start_request_id = self
...
async fn webrtc_v3_start_posts_live_session_and_joins_without_session_update() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_forestx_response_routing(
            "v=offer\r\n",
            /*client_managed_handoffs*/ None,
            /*forestx_responses_as_items*/ None,
            /*forestx_response_handoff_mode*/ None,
...
async fn webrtc_v1_client_managed_handoffs_disable_automatic_output() -> Result<()> {
    ...
    assert!(
        ...
        "automatic Forestx output should not reach realtime in client-managed handoff mode"
    );
...
#[tokio::test]
async fn webrtc_v1_ignores_forestx_response_handoff_mode() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let started = harness
        .start_webrtc_realtime_with_forestx_response_routing(
            ...
            /*client_managed_handoffs*/ None,
            /*forestx_responses_as_items*/ None,
            /*forestx_response_handoff_mode*/ Some(ForestxResponseHandoffMode::BemTags),
            /*delegation_ack_filler*/ None,
            RealtimeConversationVersion::V1,
        )
        .await?;
...
async fn webrtc_v1_handoff_request_delegates_context_and_manual_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_webrtc_realtime_with_forestx_response_items("v=offer\r\n")
        .await?;
    assert_eq!(started.started.version, RealtimeConversationVersion::V1);
    assert_call_create_multipart(
        harness.call_capture.single_request(),
...
async fn realtime_automatic_standalone_output_is_item_and_append_speaks() -> Result<()> {
    ...
    let started = harness
        .start_websocket_realtime_with_forestx_response_items()
        .await?;
    assert_eq!(started.version, RealtimeConversationVersion::V2);
    assert_eq!(
...
async fn websocket_v3_routes_handoffs_by_session_mode() -> Result<()> {
    ...
    for (mode, channel_prefixes, texts, expected_channels) in [
        ...
        (
            Some(ForestxResponseHandoffMode::Commentary),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
        ...
        (
            Some(ForestxResponseHandoffMode::BemTags),
            None,
            [
                "[ANALYSIS]silent context",
                "[COMMENTARY]still working",
...
async fn websocket_v2_background_agent_steering_ack_requests_response_create() -> Result<()> {
    ...
    // Phase 4: release the gated delegated turn. Forestx should then continue

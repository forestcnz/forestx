...
use forestx_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
use forestx_app_server_protocol::ExternalAgentConfigImportHistoriesReadResponse;
use forestx_app_server_protocol::ExternalAgentConfigImportHistoryRecordResponse;
use forestx_app_server_protocol::ExternalAgentConfigImportProgressNotification;
use forestx_app_server_protocol::ExternalAgentConfigImportResponse;
use forestx_app_server_protocol::ExternalAgentConfigMigrationItemType;
use forestx_app_server_protocol::ExternalAgentImportedConnectorCandidate;
use forestx_app_server_protocol::ExternalAgentImportedConnectorSource;
...
fn external_agent_home(forestx_home: &Path) -> PathBuf {
    forestx_home.join(concat!(".", "cla", "ude"))
}
...
fn secondary_external_agent_home(forestx_home: &Path) -> PathBuf {
    forestx_home.join(concat!(".", "cur", "sor"))
}
...
-> Result<()> {
    ...
    let source_home = external_agent_home(forestx_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(source_home.join("CLAUDE.md"), "project instructions")?;
    let home_dir = forestx_home.path().display().to_string();
...
async fn external_agent_config_migration_source_drives_detect_and_import() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(forestx_home.path());
    std::fs::create_dir_all(&source_home)?;
    ...
    assert!(
        std::fs::read_to_string(forestx_home.path().join("config.toml"))?
...
async fn external_agent_config_import_source_remains_attribution_only() -> Result<()> {
    ...
    assert_eq!(completed.item_type_results[0].failures, Vec::new());
    assert_eq!(
        std::fs::read_to_string(forestx_home.path().join("AGENTS.md"))?,
        "Forestx guidance"
    );
...
async fn external_agent_config_secondary_source_imports_session_and_plugin_end_to_end() -> Result<()>
{
    let forestx_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(forestx_home.path());
    let project_root = forestx_home.path().join("workspace with.dots_and-dashes");
    ...
    let configured_marketplace_root = forestx_home.path().join("configured-marketplace");
    ...
    std::fs::create_dir_all(configured_plugin_root.join(".forestx-plugin"))?;
    std::fs::write(
  ...
  "plugins": [{
    ...
    std::fs::write(
        configured_plugin_root.join(".forestx-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(forestx_config::AbsolutePathBuf::as_path),
...
-> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            ("FORESTX_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
    ...
    let state_db = forestx_state::StateRuntime::init(
        forestx_state::SqliteConfig::new_for_testing(sqlite_home.path().abs()),
        "mock_provider".into(),
    )
    .await?;
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let project_cwd = forestx_home.path().join("project-a");
    std::fs::create_dir_all(&source_memory)?;
    ...
    let expected_resources_root = forestx_home
        .path()
    ...
    let memory_diff = forestx_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    forestx_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                forestx_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                forestx_git_utils::GitBaselineChangeStatus::Deleted,
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    std::fs::write(forestx_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = forestx_home.path().display().to_string();
    let analytics_capture_file = forestx_home.path().join("analytics-events.jsonl");
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("HOME", Some(home_dir.as_str())),
            (
                "FORESTX_ANALYTICS_EVENTS_CAPTURE_FILE",
    ...
    let events = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if captured_events.iter().any(|event| {
                event["event_type"] == "forestx_onboarding_external_agent_import_complete"
    ...
    let event = events
        .iter()
        .find(|event| {
            event["event_type"] == "forestx_onboarding_external_agent_import_failure"
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    write_analytics_config(forestx_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let missing_session_path =
        external_agent_home(forestx_home.path()).join("projects/repo/missing.jsonl");
    let project_root = forestx_home.path().join("repo");
    let home_dir = forestx_home.path().display().to_string();
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "forestx_onboarding_external_agent_import_complete",
    )
    .await?;
    let event_params = &event["event_params"];
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "forestx_onboarding_external_agent_import_failure",
    )
    .await?;
    let event_params = &event["event_params"];
    assert_eq!(event_params["import_id"], serde_json::json!(import_id));
...
async fn external_agent_config_import_reinstalls_plugins_from_known_marketplaces() -> Result<()> {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        Some(vec![forestx_app_server_protocol::PluginsMigration {
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(forestx_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
    let manifest_dir = connector_metadata_root(forestx_home.path())
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(forestx_home.path())?;
    let mut config = std::fs::read_to_string(forestx_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
fn write_analytics_config(forestx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(

...
use forestx_app_server_protocol::GrantedPermissionProfile;
...
"#,
    ...
    let manifest_dir = plugin_dir.join(".forestx-plugin")?;
    ...
    #[cfg(unix)]
    if scenario == ExecutorSkillScenario::RestrictedDeniedReference {
        let external_reference_dir = forestx_home.path().join("external-reference");
    ...
    if scenario == ExecutorSkillScenario::RestrictedVisible && !auto_env.environment().is_remote() {
        let denied_skill_dir = forestx_home.path().join("denied-skill");

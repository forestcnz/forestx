...
async fn plugin_share_save_uploads_local_plugin() -> Result<()> {
    ...
    write_remote_plugin_config(forestx_home.path(), &format!("{}/backend-api", server.uri()))?;
    write_chatgpt_auth(
        forestx_home.path(),
    ...
    write_corrupt_plugin_share_local_path_mapping(forestx_home.path())?;
...
async fn plugin_share_checkout_adds_personal_marketplace_entry() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareCheckoutResponse {
            remote_plugin_id: "plugins_123".to_string(),
            plugin_id: "demo-plugin@forestx-curated".to_string(),
    ...
    let request_id = mcp
        .send_plugin_list_request(PluginListParams {
            ...
            marketplace_kinds: Some(vec![
                forestx_app_server_protocol::PluginListMarketplaceKind::Local,
            ]),
            force_refetch: false,
        })
        .await?;
    ...
    assert_eq!(response.marketplaces[0].name, "forestx-curated");
...
async fn plugin_share_update_targets_updates_share_targets() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: forestx_app_server_protocol::PluginShareDiscoverability::Unlisted,
        }
    );
    Ok(())
}
...
async fn plugin_share_update_targets_publishes_workspace_plugin() -> Result<()> {
    ...
    assert_eq!(
        ...
        PluginShareUpdateTargetsResponse {
            ...
            discoverability: forestx_app_server_protocol::PluginShareDiscoverability::Listed,
        }
    );
    Ok(())
}
...
async fn plugin_share_delete_removes_created_workspace_plugin() -> Result<()> {
    ...
    let local_plugin_path = AbsolutePathBuf::try_from(forestx_home.path().join("local-plugin"))?;
    write_plugin_share_local_path_mapping(forestx_home.path(), "plugins_123", &local_plugin_path)?;
...
fn write_remote_plugin_config(forestx_home: &Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
...
fn write_test_plugin(root: &Path, plugin_name: &str) -> std::io::Result<PathBuf> {
    ...
    write_file(
        &plugin_path.join(".forestx-plugin/plugin.json"),
...
fn write_corrupt_plugin_share_local_path_mapping(forestx_home: &Path) -> std::io::Result<()> {
    write_file(

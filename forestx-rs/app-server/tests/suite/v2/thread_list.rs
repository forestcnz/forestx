...
use forestx_protocol::protocol::SessionSource as CoreSessionSource;
...
use forestx_state::DirectionalThreadSpawnEdgeStatus;
...
async fn init_mcp(forestx_home: &Path) -> Result<TestAppServer> {
...
async fn thread_list_basic_empty() -> Result<()> {
    ...
    create_minimal_config(forestx_home.path())?;

    let mut mcp = init_mcp(forestx_home.path()).await?;
...
async fn thread_list_reports_system_error_idle_flag_after_failed_turn() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_runtime_config(forestx_home.path(), &server.uri())?;
    let mut mcp = init_mcp(forestx_home.path()).await?;
...
fn create_minimal_config(forestx_home: &std::path::Path) -> std::io::Result<()> {
...
fn create_runtime_config(forestx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    MockResponsesConfig::new(server_uri).write(forestx_home)
}
...
async fn thread_list_respects_cwd_filters() -> Result<()> {
    ...
    let first_target_cwd = forestx_home.path().join("first-target-cwd");
    let second_target_cwd = forestx_home.path().join("second-target-cwd");
    fs::create_dir_all(&first_target_cwd)?;
    fs::create_dir_all(&second_target_cwd)?;
    set_rollout_cwd(
        rollout_path(forestx_home.path(), "2025-01-02T10-00-00", &first_filtered_id).as_path(),
        &first_target_cwd,
    )?;
    set_rollout_cwd(
        rollout_path(
    ...
    let mut mcp = init_mcp(forestx_home.path()).await?;
    let request_id = mcp
        .send_thread_list_request(forestx_app_server_protocol::ThreadListParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
...
"#,
    ...
    state_db
        ...
        .await?;
    let rollout_config = forestx_rollout::RolloutConfig {
        forestx_home: forestx_home.path().to_path_buf(),
        sqlite: forestx_state::SqliteConfig::new_for_testing(forestx_home.path().abs()),
        cwd: forestx_home.path().to_path_buf(),
        model_provider_id: "mock_provider".to_string(),
        generate_memories: false,
    };
    let repaired_page = forestx_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        forestx_core::ThreadSortKey::CreatedAt,
        forestx_core::SortDirection::Desc,
        &[],
...
async fn thread_search_returns_content_matches() -> Result<()> {
    ...
    let mut mcp = init_mcp(forestx_home.path()).await?;
    let request_id = mcp
        .send_thread_search_request(forestx_app_server_protocol::ThreadSearchParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
    ...
    for thread_id in [&older_match, &newer_match] {
        let request_id = mcp
            .send_thread_section_move_request(ThreadSectionMoveParams {
                ...
                section_id: Some(forestx_state::PINNED_THREAD_SECTION_ID.to_string()),
...
"#,
    ...
    let stale_cwd = forestx_home.path().join("stale-cwd");
...
async fn thread_list_relation_filters_read_spawn_graph_from_state_db() -> Result<()> {
    ...
    for (thread_id, created_at, source, model_provider) in [
        ...
        let mut builder = forestx_state::ThreadMetadataBuilder::new(
            thread_id,
            forestx_home.path().join(format!("{thread_id}.jsonl")),
            created_at,
            source,
        );
        builder.model_provider = Some(model_provider.to_string());
        builder.cwd = forestx_home.path().to_path_buf();
...
async fn thread_list_reports_loaded_subagent_direct_input_capability() -> Result<()> {
    ...
    for (filename_ts, timestamp, version, capability, should_resume) in [
        ...
        let path = rollout_path(forestx_home.path(), filename_ts, &thread_id);
...
async fn thread_list_sort_updated_at_orders_by_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-01-01T10-00-00", &id_old).as_path(),
        "2025-01-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-01-01T11-00-00", &id_mid).as_path(),
        "2025-01-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-01-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_sort_recency_at_uses_state_db_order_with_provider_filter() -> Result<()> {
    ...
    forestx_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        forestx_core::ThreadSortKey::CreatedAt,
        forestx_core::SortDirection::Desc,
        forestx_core::INTERACTIVE_SESSION_SOURCES.as_slice(),
        /*model_providers*/ None,
        /*cwd_filters*/ None,
        "mock_provider",
        /*search_term*/ None,
...
async fn thread_list_updated_at_paginates_with_cursor() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T10-00-00", &id_a).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T11-00-00", &id_b).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T12-00-00", &id_c).as_path(),
...
async fn thread_list_backwards_cursor_can_seed_forward_delta_sync() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T10-00-00", &id_old).as_path(),
        "2025-02-02T00:00:00Z",
    )?;
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T11-00-00", &id_watermark).as_path(),
        "2025-02-03T00:00:00Z",
    )?;
    ...
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T12-00-00", &id_new).as_path(),
...
async fn thread_list_updated_at_uses_mtime() -> Result<()> {
    ...
    set_rollout_mtime(
        rollout_path(forestx_home.path(), "2025-02-01T10-00-00", &thread_id).as_path(),
...
async fn thread_list_archived_filter() -> Result<()> {
    ...
    let archived_source = rollout_path(forestx_home.path(), "2025-03-01T09-00-00", &archived_id);

...
use forestx_app_server_protocol::GitInfo;
...
use forestx_app_server_protocol::ThreadSectionListResponse;
...
use forestx_protocol::protocol::GitInfo as RolloutGitInfo;
use forestx_rollout::state_db::reconcile_rollout;
use forestx_state::PINNED_THREAD_SECTION_ID;
use forestx_state::PINNED_THREAD_SECTION_NAME;
...
async fn thread_section_move_pins_and_unpins_with_filtered_recency_pagination() -> Result<()> {
    ...
    for (filename_timestamp, timestamp, preview) in [
        ...
        reconcile_rollout(
            Some(&state_db),
            rollout_path(forestx_home.path(), filename_timestamp, &thread_id).as_path(),
            "mock_provider",
            /*builder*/ None,
            &[],
            /*archived_only*/ None,
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_stored_thread() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    mock_responses_config(&server.uri()).write(forestx_home.path())?;
    let _state_db = init_state_db(forestx_home.path()).await?;
...
async fn thread_metadata_update_repairs_loaded_thread_without_resetting_summary() -> Result<()> {
    ...
    let rollout_path = rollout_path(forestx_home.path(), "2025-01-06T08-30-00", &thread_id);
    reconcile_rollout(
        Some(&state_db),
...
async fn thread_metadata_update_repairs_missing_sqlite_row_for_archived_thread() -> Result<()> {
    ...
    let archived_source = rollout_path(forestx_home.path(), "2025-01-06T08-30-00", &thread_id);

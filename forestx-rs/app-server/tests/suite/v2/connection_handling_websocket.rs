...
async fn thread_start_routes_project_exec_policy_warning_to_requester() -> Result<()> {
    ...
    let rules_dir = project.path().join(".forestx/rules");
    std::fs::create_dir_all(&rules_dir)?;
    let rules_path = rules_dir.join("broken.rules");
    std::fs::write(&rules_path, "prefix_rule(")?;
    set_project_trust_level(forestx_home.path(), project.path(), TrustLevel::Trusted)?;
...
async fn websocket_transport_rejects_missing_and_invalid_capability_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let forestx_home = TempDir::new()?;
    let token_file = forestx_home.path().join("app-server-token");
    ...
    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(forestx_home.path(), "ws://0.0.0.0:0", &auth_args).await?;
...
async fn websocket_transport_verifies_signed_short_lived_bearer_tokens() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let forestx_home = TempDir::new()?;
    let shared_secret_file = forestx_home.path().join("app-server-signing-secret");
    ...
    let auth_args = vec![
        ...
        "--ws-issuer".to_string(),
        "forestx-enroller".to_string(),
        "--ws-audience".to_string(),
        "forestx-app-server".to_string(),
        "--ws-max-clock-skew-seconds".to_string(),
        "1".to_string(),
    ];

    let (mut process, bind_addr) =
        spawn_websocket_server_with_args(forestx_home.path(), "ws://127.0.0.1:0", &auth_args).await?;
    let expired_token = signed_bearer_token(
        shared_secret.as_bytes(),
        json!({
            "exp": OffsetDateTime::now_utc().unix_timestamp() - 30,
            "iss": "forestx-enroller",
...
async fn websocket_transport_rejects_unauthenticated_non_loopback_startup() -> Result<()> {
    ...
    let output =
        run_websocket_server_to_completion_with_args(forestx_home.path(), "ws://0.0.0.0:0", &[])
            .await?;
    assert!(
        !output.status.success(),
...
pub(super) async fn spawn_websocket_server(forestx_home: &Path) -> Result<(Child, SocketAddr)> {
    spawn_websocket_server_with_args(forestx_home, "ws://127.0.0.1:0", &[]).await
}
...
pub(super) async fn spawn_websocket_server_with_args(
    ...
    let program = forestx_utils_cargo_bin::cargo_bin("forestx-app-server")

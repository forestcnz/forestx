...
async fn thread_archive_requires_materialized_rollout() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(forestx_home.path(), &thread.id, /*state_db_ctx*/ None)
            .await?
            .is_none(),
    ...
    // Verify file moved.
    let archived_directory = forestx_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
...
async fn thread_archive_succeeds_when_descendant_archive_fails() -> Result<()> {
    ...
    let child_rollout_path =
        find_thread_path_by_id_str(forestx_home.path(), &child_id, /*state_db_ctx*/ None)
            .await?
            .expect("child rollout path");
    let archived_child_path = forestx_home
        .path()
...
async fn thread_archive_succeeds_when_spawned_descendant_is_missing() -> Result<()> {
    ...
    assert!(
        find_thread_path_by_id_str(forestx_home.path(), &parent_id, /*state_db_ctx*/ None)
            .await?
            .is_none(),

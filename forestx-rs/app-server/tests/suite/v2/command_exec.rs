...
async fn command_exec_without_streams_can_be_terminated() -> Result<()> {
    ...
    create_config_toml(forestx_home.path(), &server.uri(), "never")?;
...
async fn command_exec_env_overrides_merge_with_server_environment_and_support_unset() -> Result<()>
{
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "/bin/sh".to_string(),
                "-lc".to_string(),
                "printf '%s|%s|%s|%s' \"$COMMAND_EXEC_BASELINE\" \"$COMMAND_EXEC_EXTRA\" \"${RUST_LOG-unset}\" \"$FORESTX_HOME\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
    ...
    assert_eq!(
        ...
        CommandExecResponse {
            exit_code: 0,
            stdout: format!("request|added|unset|{}", forestx_home.path().display()),
            stderr: String::new(),
        }
    );
    ...
    let command_request_id = mcp
        .send_command_exec_request(CommandExecParams {
            command: vec![
                "sh".to_string(),
                "-lc".to_string(),
                "printf '%s' \"${FORESTX_NETWORK_PROXY_ACTIVE-unset}\"".to_string(),
            ],
            process_id: None,
            tty: false,
            stream_stdin: false,
...
async fn command_exec_permission_profile_does_not_reuse_default_network_proxy() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml(forestx_home.path(), &server.uri(), "never")?;
    insert_networked_permission_profile_config(forestx_home.path(), Some("networked"))?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
...
async fn command_exec_permission_profile_project_roots_use_command_cwd() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let forestx_home = TempDir::new()?;
    let command_dir = forestx_home.path().join("command-cwd");
...
"#,
    ...
    assert!(
        !forestx_home.path().join("parent.txt").exists(),
...
-> Result<()> {
    ...
    create_config_toml(forestx_home.path(), &server.uri(), "never")?;
    let marker = format!(
        "forestx-command-exec-marker-{}",
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
    );

    let (mut process, bind_addr) = spawn_websocket_server(forestx_home.path()).await?;
...
"#
    ...
    insert_command_exec_config(forestx_home, &inserted_config)?;
    Ok(())
}

fn insert_command_exec_config(forestx_home: &Path, inserted_config: &str) -> Result<()> {

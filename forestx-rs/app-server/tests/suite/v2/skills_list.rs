...
use forestx_app_server_protocol::SkillScope;
use forestx_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = forestx_home.join("plugins/cache/openai-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".forestx-plugin"))?;
    std::fs::write(
        plugin_root.join(".forestx-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
fn write_cached_local_curated_plugin_with_skill(forestx_home: &std::path::Path) -> Result<()> {
    let plugin_root = forestx_home.join("plugins/cache/openai-curated/google-calendar/local");
    std::fs::create_dir_all(plugin_root.join(".forestx-plugin"))?;
    std::fs::write(
        plugin_root.join(".forestx-plugin/plugin.json"),
...
async fn skills_list_runtime_enable_refreshes_shared_system_skill_cache() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    let stale_skill_path = forestx_home
        .path()
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(forestx_home.path())?;
    std::fs::write(
        forestx_home.path().join("config.toml"),
        format!(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(forestx_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_forestx_plugins_disabled() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&forestx_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace Forestx plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&forestx_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".forestx/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(forestx_home.path().join(&relative_cwd))?;
...
-> Result<()> {
    ...
    let skill_dir = cwd.path().join(".forestx/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),

...
async fn run_removal_session_end_test(operation: &str) -> Result<()> {
    ...
    let log_path = write_config_and_hook(forestx_home.path(), &server.uri())?;
...
fn write_config_and_hook(forestx_home: &Path, server_uri: &str) -> Result<std::path::PathBuf> {
    let log_path = forestx_home.join("session-end.jsonl");
    let script_path = forestx_home.join("session-end.py");
    std::fs::write(
        &script_path,
        format!(
            r#"import json
...
"#,
    ...
    MockResponsesConfig::new(server_uri)
        ...
        .enable_feature(Feature::ForestxHooks)

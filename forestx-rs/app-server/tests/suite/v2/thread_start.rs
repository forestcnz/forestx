...
use forestx_app_server_protocol::TextPosition;
use forestx_app_server_protocol::TextRange;
...
async fn thread_start_warns_for_exec_policy_parse_failure_after_initialize() -> Result<()> {
    ...
    let rules_dir = forestx_home.path().join("rules");
...
async fn thread_start_provider_model_fallback_ignores_dynamic_catalog() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml_without_approval_policy(forestx_home.path(), &server.uri())?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .build_initialized()
        .await?;
...
async fn thread_start_response_includes_loaded_instruction_sources() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml_without_approval_policy(forestx_home.path(), &server.uri())?;
    let global_agents_path = forestx_home.path().join("AGENTS.md");
...
async fn thread_start_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(forestx_home.path(), &server.uri(), &server.uri())?;
...
async fn thread_start_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml_with_required_broken_mcp(forestx_home.path(), &server.uri())?;
...
async fn thread_start_emits_mcp_server_status_updated_notifications() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml_with_optional_broken_mcp(forestx_home.path(), &server.uri())?;
...
"#,
    ...
    set_project_trust_level(forestx_home.path(), workspace.path(), TrustLevel::Trusted)?;
    let config_before = std::fs::read_to_string(forestx_home.path().join("config.toml"))?;
    ...
    let config_after = std::fs::read_to_string(forestx_home.path().join("config.toml"))?;
    assert_eq!(config_after, config_before);
...
fn create_config_toml_without_approval_policy(
    ...
    create_config_toml(forestx_home, server_uri, "sandbox_mode = \"read-only\"", "")
}

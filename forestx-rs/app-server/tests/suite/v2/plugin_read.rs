...
use forestx_app_server_protocol::AppInfo;
use forestx_app_server_protocol::AppMetadata;
...
use forestx_app_server_protocol::HookEventName;
...
use forestx_app_server_protocol::ScheduledTaskSchedule;
use forestx_app_server_protocol::ScheduledTaskSummary;
use forestx_app_server_protocol::ScheduledTaskWeekday;
use forestx_app_server_protocol::SkillInterface;
    ...
    let request_id = mcp
        .send_plugin_read_request(PluginReadParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                forestx_home.path().join("marketplace.json"),
            )?),
...
async fn plugin_read_rejects_invalid_remote_plugin_name() -> Result<()> {
    let forestx_home = TempDir::new()?;
    write_remote_plugin_catalog_config(forestx_home.path(), "https://example.invalid/backend-api/")?;
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
...
"#,
    ...
    write_installed_plugin(&forestx_home, "openai-curated", "demo-plugin")?;
...
async fn plugin_read_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    write_plugin_share_local_path_mapping(forestx_home.path(), "plugins_123", &plugin_path)?;
    Mock::given(method("GET"))
...
async fn plugin_read_falls_back_to_local_share_context_without_remote_auth() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    write_plugins_enabled_config(&forestx_home)?;
    write_plugin_marketplace(
        repo_root.path(),
        "forestx-curated",
        "demo-plugin",
...
async fn plugin_read_fails_on_malformed_share_mapping() -> Result<()> {
    ...
    write_plugin_source(repo_root.path(), "demo-plugin", &[])?;
    std::fs::create_dir_all(forestx_home.path().join(".tmp"))?;
    std::fs::write(
        forestx_home
            .path()
...
[plugins."demo-plugin@forestx-curated"]
enabled = true

[hooks.state."demo-plugin@forestx-curated:hooks/hooks.json:pre_tool_use:0:0"]
enabled = false
"#,
    )?;
    write_installed_plugin(&forestx_home, "forestx-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "forestx-curated");
    assert_eq!(response.plugin.marketplace_path, Some(marketplace_path));
    assert_eq!(response.plugin.summary.id, "demo-plugin@forestx-curated");
    assert_eq!(response.plugin.summary.name, "demo-plugin");
    assert_eq!(
        response.plugin.description.as_deref(),
    ...
    assert_eq!(
        ...
        vec![
            forestx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@forestx-curated:hooks/hooks.json:pre_tool_use:0:0".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            forestx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@forestx-curated:hooks/hooks.json:pre_tool_use:0:1".to_string(),
                event_name: HookEventName::PreToolUse,
            },
            forestx_app_server_protocol::PluginHookSummary {
                key: "demo-plugin@forestx-curated:hooks/hooks.json:session_start:0:0".to_string(),
...
async fn plugin_read_batches_large_app_metadata_requests() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    write_connectors_config(forestx_home.path(), &server_url)?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_stops_batching_after_app_metadata_failure() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    write_connectors_config(forestx_home.path(), &server.uri())?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn plugin_read_hides_apps_for_api_key_auth() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[
            ("FORESTX_ACCESS_TOKEN", None),
            ("FORESTX_API_KEY", None),
            ("OPENAI_API_KEY", None),
        ])
        .build_initialized_with_timeout(DEFAULT_TIMEOUT)
        .await?;
...
fn write_plugins_enabled_config(forestx_home: &TempDir) -> Result<()> {
    std::fs::write(
        forestx_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn batch_apps(
    ...
    let product_sku_ok = headers
        .get("oai-product-sku")
        .and_then(|value| value.to_str().ok())
        .is_some_and(|value| value == "forestx");
...
fn write_connectors_config(forestx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(

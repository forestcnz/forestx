...
impl CuratedMcpSyncFixture {
    async fn set_up() -> Result<Self> {
        ...
        let forestx_home = fixture_root.path().join("forestx-home");
        ...
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.email", "forestx-tests@openai.com"],
        )?;
        run_git(
            &real_git,
            &curated_repo,
            &["config", "user.name", "Forestx Tests"],
        )?;
...
fn write_plugin(
    ...
    std::fs::create_dir_all(root.join(".forestx-plugin"))?;
    std::fs::write(
        root.join(".forestx-plugin/plugin.json"),

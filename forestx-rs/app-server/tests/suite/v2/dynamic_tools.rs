...
use forestx_app_server_protocol::DynamicToolCallResponse;
use forestx_app_server_protocol::DynamicToolCallStatus;
use forestx_app_server_protocol::DynamicToolFunctionSpec;
use forestx_app_server_protocol::DynamicToolNamespaceSpec;
...
async fn dynamic_tool_call_round_trip_sends_text_content_items_to_model() -> Result<()> {
    ...
    let tool_namespace = "forestx_app";
...
async fn start_function_dynamic_tool_call(call_id: &str) -> Result<PendingDynamicToolCall> {
    ...
    let mut model_info =
        forestx_core::test_support::construct_model_info_offline("mock-model", &config);
    model_info.input_modalities.push(InputModality::Audio);
    write_models_cache_with_models(forestx_home.path(), vec![model_info])?;

...
const SERVER_MODEL: &str = "gpt-5.3-forestx";
...
async fn cyber_policy_response_emits_typed_error_notification_v2() -> Result<()> {
    ...
    assert_eq!(
        error,
        ErrorNotification {
            error: forestx_app_server_protocol::TurnError {
    ...
    loop {
        ...
        match notification.method.as_str() {
            "error" => {
                ...
                if payload.error.forestx_error_info == Some(ForestxErrorInfo::CyberPolicy) {
                    error = Some(payload);
                }
            }
            "model/rerouted" => {
...
fn create_config_toml(forestx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {

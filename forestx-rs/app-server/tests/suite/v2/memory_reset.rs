...
use forestx_state::Stage1JobClaimOutcome;
...
async fn memory_reset_clears_memory_files_and_rows_preserves_threads() -> Result<()> {
    ...
    let thread_id = seed_stage1_output(&state_db, forestx_home.path()).await?;
...
async fn seed_stage1_output(state_db: &Arc<StateRuntime>, forestx_home: &Path) -> Result<ThreadId> {
    let now = Utc::now();
    let thread_id = ThreadId::from_string(&Uuid::new_v4().to_string())?;
    let worker_id = ThreadId::from_string(&Uuid::new_v4().to_string())?;
    let mut builder = ThreadMetadataBuilder::new(
        thread_id,
        forestx_home.join("sessions").join("test.jsonl"),
        now,
        SessionSource::Cli,
    );

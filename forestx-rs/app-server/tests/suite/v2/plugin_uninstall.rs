...
async fn plugin_uninstall_removes_plugin_cache_and_config_entry() -> Result<()> {
    ...
    write_installed_plugin(&forestx_home, "debug", "sample-plugin")?;
    std::fs::write(
...
async fn plugin_uninstall_tracks_analytics_event() -> Result<()> {
    ...
    let payload = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST" && request.url.path() == "/forestx/analytics-events/events"
            }) {
    ...
    assert_eq!(
        ...
        json!({
            "events": [{
                "event_type": "forestx_plugin_uninstalled",
                "event_params": {
...
async fn plugin_uninstall_writes_remote_plugin_to_cloud_when_remote_plugin_enabled() -> Result<()> {
    ...
    let remote_plugin_cache_root = forestx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear");
    std::fs::create_dir_all(remote_plugin_cache_root.join("1.0.0/.forestx-plugin"))?;
    std::fs::write(
        remote_plugin_cache_root.join("1.0.0/.forestx-plugin/plugin.json"),
    ...
    let legacy_remote_plugin_cache_root = forestx_home.path().join(format!(
        "plugins/cache/openai-curated-remote/{REMOTE_PLUGIN_ID}"
    ));
    std::fs::create_dir_all(legacy_remote_plugin_cache_root.join("local/.forestx-plugin"))?;
...
async fn plugin_uninstall_uses_detail_scope_for_cache_namespace() -> Result<()> {
    ...
    let workspace_cache_root = forestx_home
        .path()
        .join("plugins/cache/workspace-directory/linear");
    std::fs::create_dir_all(workspace_cache_root.join("1.0.0/.forestx-plugin"))?;
    std::fs::write(
        workspace_cache_root.join("1.0.0/.forestx-plugin/plugin.json"),
        r#"{"name":"linear","version":"1.0.0"}"#,
    )?;
    let global_cache_root = forestx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear");
    std::fs::create_dir_all(global_cache_root.join("1.0.0/.forestx-plugin"))?;

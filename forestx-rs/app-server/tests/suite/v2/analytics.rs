...
use forestx_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut forestx_core::config::Config) {
...
pub(crate) async fn mount_analytics_capture(server: &MockServer, forestx_home: &Path) -> Result<()> {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "forestx_goal_event"
        ...
        loop {
            ...
            for request in &requests {
                if request.method != "POST"
                    || request.url.path() != "/forestx/analytics-events/events"
                {
                    continue;
                }
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("forestx_thread_initialized event should be present"))
}

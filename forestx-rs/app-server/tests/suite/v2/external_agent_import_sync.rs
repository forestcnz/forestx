...
const NATIVE_USER: &str = "native Forestx message";
const NATIVE_ASSISTANT: &str = "native Forestx answer";
...
impl ImportFixture {
    async fn new() -> Result<Self> {
        ...
        let app_server = start_app_server(forestx_home.path()).await?;
        Ok(Self {
    ...
    async fn restart(&mut self) -> Result<()> {
        ...
        self.app_server = start_app_server(self._forestx_home.path()).await?;
        Ok(())
    }
...
async fn start_app_server(forestx_home: &Path) -> Result<TestAppServer> {
    let home = forestx_home.display().to_string();
...
async fn cold_diverged_target_is_deferred_without_checkpoint() -> Result<()> {
    ...
    let mut fixture = ImportFixture::new().await?;
    MockResponsesConfig::new(&model.uri()).write(fixture._forestx_home.path())?;

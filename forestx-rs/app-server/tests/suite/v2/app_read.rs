...
use forestx_app_server_protocol::ConnectorMetadata;
...
async fn app_read_deduplicates_orders_partial_misses_and_reuses_cached_metadata() -> Result<()> {
    ...
    write_apps_config(forestx_home.path(), &server_url, Some("tpp"))?;
...
"#,
    ...
    write_plugin_app(forestx_home.path(), "alpha-z", "Alpha Z", "alpha")?;
    write_plugin_app(forestx_home.path(), "alpha-a", "Alpha A", "alpha")?;
    write_plugin_app(
        forestx_home.path(),
        "disabled",
        "Disabled Plugin",
...
async fn start_batch_server(state: BatchServerState) -> Result<(String, JoinHandle<()>)> {
    ...
    let router = Router::new()
        ...
        .route("/api/forestx/ps/mcp", any(unexpected_mcp_request))
        .with_state(state);

...
async fn process_spawn_returns_before_exit_and_emits_exit_notification() -> Result<()> {
    ...
    let (_server, mut mcp) = initialized_mcp(forestx_home.path()).await?;

    let process_handle = "one-shot-1".to_string();
    let probe_file = forestx_home.path().join("process-created");
    let release_file = forestx_home.path().join("process-release");
    ...
    let command = if cfg!(windows) {
        vec![
            ...
            "-Command".to_string(),
            concat!(
                "[IO.File]::WriteAllText($env:FORESTX_PROCESS_EXEC_PROBE_FILE, 'process'); ",
                "while (!(Test-Path -LiteralPath $env:FORESTX_PROCESS_EXEC_RELEASE_FILE)) { ",
        ...
        vec![
            ...
            "-c".to_string(),
            concat!(
                "printf process > \"$FORESTX_PROCESS_EXEC_PROBE_FILE\"; ",
                "while [ ! -e \"$FORESTX_PROCESS_EXEC_RELEASE_FILE\" ]; do sleep 0.05; done; ",
                "printf process-out; ",
    ...
    let env = HashMap::from([
        (
            "FORESTX_PROCESS_EXEC_PROBE_FILE".to_string(),
            Some(probe_file.display().to_string()),
        ),
        (
            "FORESTX_PROCESS_EXEC_RELEASE_FILE".to_string(),
    ...
    let spawn_request_id = mcp
        .send_process_spawn_request(ProcessSpawnParams {
            ...
            ..process_spawn_params(process_handle.clone(), forestx_home.path(), command)?
        })
        .await?;
...
async fn initialized_mcp(forestx_home: &Path) -> Result<(MockServer, TestAppServer)> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    create_config_toml(forestx_home, &server.uri(), "never")?;
    let mut mcp = TestAppServer::builder()

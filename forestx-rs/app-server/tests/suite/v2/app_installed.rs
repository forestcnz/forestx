...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let forestx_home = configured_forestx_home(fixture.base_url())?;
    let mut app_server = start_app_server(forestx_home.path()).await?;
...
fn configured_forestx_home(base_url: &str) -> Result<TempDir> {
...
impl InstalledAppsFixture {
    async fn start() -> Result<Self> {
        ...
        synthetic_link
            ...
            .0
            .insert("_forestx_apps".to_string(), json!({ "synthetic_link": true }));

...
async fn review_start_exec_approval_item_id_matches_command_execution_item() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": forestx_protocol::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
...
async fn review_start_with_detached_delivery_returns_new_thread_id() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml(forestx_home.path(), &server.uri())?;
    let colliding_skill_dir = forestx_home.path().join("skills/review-agent-collision");
    ...
    let canonical_forestx_home = std::fs::canonicalize(forestx_home.path())?.try_into()?;
    let review_skill_path = system_cache_root_dir(&canonical_forestx_home)
        .join("review-agent")
        .join("SKILL.md");

...
use forestx_app_server_protocol::HookTrustStatus;
use forestx_app_server_protocol::HooksListParams;
use forestx_app_server_protocol::HooksListResponse;
use forestx_app_server_protocol::JSONRPCResponse;
use forestx_app_server_protocol::LoginAccountResponse;
use forestx_app_server_protocol::PluginAuthPolicy;
use forestx_app_server_protocol::PluginDisabledReason;
use forestx_app_server_protocol::PluginInstallPolicy;
use forestx_app_server_protocol::PluginInstallPolicySource;
...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "FORESTX_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
const ALTERNATE_MARKETPLACE_RELATIVE_PATH: &str = ".claude-plugin/marketplace.json";
const ALTERNATE_PLUGIN_MANIFEST_RELATIVE_PATH: &str = ".claude-plugin/plugin.json";

fn write_plugins_enabled_config(forestx_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(forestx_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = forestx_home.path().to_string_lossy().into_owned();
...
async fn plugin_installed_includes_installed_plugins_and_explicit_install_suggestions() -> Result<()>
{
    ...
    write_installed_plugin(&forestx_home, "openai-curated", "linear")?;
    std::fs::write(
...
async fn plugin_installed_prefers_remote_curated_conflicts_when_remote_plugin_enabled() -> Result<()>
{
    ...
    write_openai_curated_marketplace(forestx_home.path(), &["linear", "calendar"])?;
    write_installed_plugin(&forestx_home, "openai-curated", "linear")?;
    write_installed_plugin(&forestx_home, "openai-curated", "calendar")?;
    std::fs::write(
        forestx_home.path().join("config.toml"),
        format!(
...
async fn plugin_installed_prefers_api_curated_conflicts_after_switching_to_api_auth() -> Result<()>
{
    let forestx_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_openai_api_curated_marketplace(forestx_home.path(), &["linear"])?;
    write_installed_plugin(&forestx_home, "openai-api-curated", "linear")?;
    let config = format!(
        r#"chatgpt_base_url = "{}/backend-api/"
...
"#,
    ...
    std::fs::write(forestx_home.path().join("config.toml"), &config)?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    std::fs::write(forestx_home.path().join("config.toml"), "invalid config")?;
    let request_id = app_server
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.forestx-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.forestx-plugin/plugin.json"),
...
#[tokio::test]
async fn plugin_list_returns_empty_when_workspace_forestx_plugins_disabled() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
...
#[tokio::test]
async fn plugin_list_reuses_cached_workspace_forestx_plugins_setting() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.forestx-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        forestx_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.forestx-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
        ...
        vec![PluginMarketplaceEntry {
            ...
            plugins: vec![
                PluginSummary {
                    ...
                    disabled_reason: None,
                    eligible_plan_types: None,
                    interface: Some(forestx_app_server_protocol::PluginInterface {
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let forestx_home = TempDir::new()?;
    std::fs::create_dir_all(forestx_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(forestx_home.path())?;
    std::fs::write(
        forestx_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "forestx-curated",
  "plugins": [
    {
...
async fn plugin_list_force_refetch_waits_for_same_path_local_plugin_upgrade() -> Result<()> {
    ...
    let source_manifest = marketplace_root
        .path()
        .join("sample-plugin/.forestx-plugin/plugin.json");
...
"#,
    ...
    let plugin_cache = forestx_home
        .path()
        .join("plugins/cache/sample-marketplace/sample-plugin");
    let installed_manifest = plugin_cache.join("1.1.0/.forestx-plugin/plugin.json");
    assert!(
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&forestx_home, "forestx-curated", "enabled-plugin")?;
    write_installed_plugin(&forestx_home, "forestx-curated", "disabled-plugin")?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "forestx-curated",
...
[plugins."enabled-plugin@forestx-curated"]
enabled = true

[plugins."disabled-plugin@forestx-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "forestx-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@forestx-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@forestx-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@forestx-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let forestx_home = TempDir::new()?;
    std::fs::create_dir_all(forestx_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&forestx_home, "forestx-curated", "shared-plugin")?;
    std::fs::write(
        forestx_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "forestx-curated",
...
[plugins."shared-plugin@forestx-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".forestx"))?;
    std::fs::write(
        workspace_enabled.path().join(".forestx/config.toml"),
        r#"[plugins."shared-plugin@forestx-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@forestx-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@forestx-curated");
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    let cached_plugin_root = forestx_home.path().join("plugins/cache/debug/toolkit/local");
    std::fs::create_dir_all(cached_plugin_root.join(".forestx-plugin"))?;
    std::fs::write(
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    let installed_path = forestx_home
        .path()
    ...
    wait_for_path_exists(&installed_path.join(".forestx-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".forestx-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    write_installed_plugin_with_version(&forestx_home, "openai-curated-remote", "linear", "1.0.0")?;
    write_installed_plugin_with_version(&forestx_home, "openai-curated-remote", "stale", "1.0.0")?;
    ...
    let old_path = forestx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.0.0");
    let new_path = forestx_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3");
    let stale_path = forestx_home
        .path()
    ...
    wait_for_path_exists(&new_path.join(".forestx-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".forestx-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
async fn plugin_list_includes_remote_marketplaces_when_remote_plugin_enabled() -> Result<()> {
    ...
    write_installed_plugin_with_version(&forestx_home, "openai-curated-remote", "linear", "1.2.3")?;
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        forestx_app_server_protocol::PluginAvailability::Available
    );
    assert_eq!(
    ...
    let cache_files = std::fs::read_dir(forestx_home.path().join("cache/remote_plugin_catalog"))?
...
async fn plugin_list_honors_global_remote_catalog_cache_ttl() -> Result<()> {
    ...
    wait_for_cached_remote_catalog_plugin_ids(forestx_home.path(), &[cached_remote_plugin_id])
        .await?;
    ...
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
    wait_for_cached_remote_catalog_plugin_ids(forestx_home.path(), &[refreshed_remote_plugin_id])
        .await?;
    sleep(Duration::from_millis(100)).await;
    wait_for_remote_plugin_request_count(&server, "/ps/plugins/list", /*expected_count*/ 1).await?;
...
async fn app_server_startup_skips_disabled_remote_plugin_catalog_scopes() -> Result<()> {
    ...
    write_remote_plugin_catalog_config(forestx_home.path(), &base_url)?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    write_remote_plugins_disabled_config_with_base_url(forestx_home.path(), &base_url)?;
    server.reset().await;
    mount_remote_plugin_list(&server, "GLOBAL", &global_body).await;
    mount_remote_plugin_list(&server, "USER", &user_body).await;
    mount_remote_plugin_list(&server, "WORKSPACE", &workspace_body).await;
...
-> Result<()> {
    ...
    write_openai_api_curated_marketplace(forestx_home.path(), &["api-plugin"])?;
    login_with_api_key(
        forestx_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
...
#[tokio::test]
async fn plugin_list_includes_api_curated_marketplace_for_bedrock_without_forestx_auth() -> Result<()>
{
    let forestx_home = TempDir::new()?;
    std::fs::write(
        forestx_home.path().join("config.toml"),
...
"#,
    ...
    write_openai_curated_marketplace(forestx_home.path(), &["chatgpt-plugin"])?;
    write_openai_api_curated_marketplace(forestx_home.path(), &["api-plugin"])?;
    ...
    assert!(!forestx_home.path().join("auth.json").exists());
    let api_curated_marketplace = response
        .marketplaces
        .iter()
        .find(|marketplace| marketplace.name == "openai-api-curated")
...
"#,
    ...
    wait_for_path_exists(
        &forestx_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.forestx-plugin/plugin.json",
        ),
    )
    .await?;
...
"#,
    ...
    write_remote_plugin_test_auth(forestx_home.path())?;
    ...
    wait_for_path_exists(
        &forestx_home
            .path()
            .join("plugins/cache/workspace-directory/no-hooks/1.2.3/.forestx-plugin/plugin.json"),
    )
    .await?;
...
async fn plugin_installed_hook_trust_write_failure_stays_untrusted() -> Result<()> {
    ...
    symlink(&config_target, forestx_home.path().join("config.toml"))?;
    write_remote_plugin_test_auth(forestx_home.path())?;
...
async fn assert_disabled_remote_plugin_metadata(
    ...
    assert_eq!(
        plugin.availability,
        forestx_app_server_protocol::PluginAvailability::DisabledByAdmin
    );
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let server = MockServer::start().await;
    write_plugin_sync_config(forestx_home.path(), &format!("{}/backend-api/", server.uri()))?;
    write_openai_curated_marketplace(forestx_home.path(), &["linear", "gmail"])?;
...
async fn wait_for_cached_remote_catalog_plugin_ids(
    ...
    timeout(DEFAULT_TIMEOUT, async {
        loop {
            let plugin_ids = cached_remote_catalog_plugin_ids(forestx_home)?;
...
fn cached_remote_catalog_plugin_ids(forestx_home: &std::path::Path) -> Result<Vec<String>> {
    let cache_dir = forestx_home.join("cache/remote_plugin_catalog");
    if !cache_dir.exists() {
        return Ok(Vec::new());
    }
...
fn write_installed_plugin(
    ...
    write_installed_plugin_with_version(forestx_home, marketplace_name, plugin_name, "local")
}
...
fn write_plugin_sync_config(forestx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        forestx_home.join("config.toml"),
        format!(
            r#"
...
fn write_remote_plugin_test_auth(forestx_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        forestx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_curated_marketplace(
    ...
    let curated_root = forestx_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.forestx-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(forestx_home.join(".tmp"))?;
    std::fs::write(
        forestx_home.join(".tmp/plugins.sha"),
...
fn write_plugin_share_local_path_mapping(
    ...
    std::fs::create_dir_all(forestx_home.join(".tmp"))?;
    std::fs::write(
        forestx_home.join(".tmp/plugin-share-local-paths-v1.json"),
        format!("{contents}\n"),
    )
}

...
use forestx_shell_command::shell_detect::ShellType;
...
fn write_mock_config(forestx_home: &Path, server_uri: &str) -> std::io::Result<()> {
...
async fn thread_start_reports_selected_environment_metadata() -> Result<()> {
    ...
    write_mock_config(forestx_home.path(), &server.uri())?;
    ...
    let ThreadStartResponse {
        ...
        .await?;
    let host_cwd = forestx_home.path().to_path_buf().abs().canonicalize()?;

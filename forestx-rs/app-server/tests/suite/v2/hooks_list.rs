...
use forestx_app_server_protocol::HookHandlerType;
use forestx_app_server_protocol::HookMetadata;
use forestx_app_server_protocol::HookSource;
...
fn command_hook_hash(
    ...
    let Ok(value) = forestx_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    forestx_config::version_for_toml(&value)
}

fn write_user_hook_config(forestx_home: &std::path::Path) -> Result<()> {
    std::fs::write(
...
fn write_plugin_hook_config(forestx_home: &std::path::Path, hooks_json: &str) -> Result<()> {
    let plugin_root = forestx_home.join("plugins/cache/test/demo/local");
...
fn write_project_hook_config(dot_forestx_folder: &std::path::Path, command: &str) -> Result<()> {
    std::fs::create_dir_all(dot_forestx_folder)?;
    std::fs::write(
        dot_forestx_folder.join("config.toml"),
        format!(
            r#"[features]
hooks = true
...
async fn hooks_list_shows_discovered_hook() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_user_hook_config(forestx_home.path())?;
...
async fn hooks_list_warms_plugin_capabilities_for_thread_start() -> Result<()> {
  ...
  "hooks": {
    ...
    let plugin_mcp_path = forestx_home
        .path()
...
async fn hooks_list_shows_plugin_hook_load_warnings() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let cwd = TempDir::new()?;
    write_plugin_hook_config(forestx_home.path(), "{ not-json")?;
...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".forestx"))?;
    std::fs::write(
        workspace.path().join(".forestx/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".forestx/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".forestx"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".forestx"), "echo worktree hook")?;
    set_project_trust_level(forestx_home.path(), &repo_root, TrustLevel::Trusted)?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".forestx/config.toml"))?;
    ...
    let write_id = mcp
        ...
        .await?;
    let _: forestx_app_server_protocol::ConfigWriteResponse =
...
async fn config_batch_write_updates_hook_trust_for_loaded_session() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    let hook_script_path = forestx_home.path().join("user_prompt_submit_hook.py");
    let hook_log_path = forestx_home.path().join("user_prompt_submit_hook_log.jsonl");
    std::fs::write(
        &hook_script_path,
        format!(
            r#"import json
...
"#,
    ...
    let hook_list_id = mcp
        .send_hooks_list_request(HooksListParams {
            cwds: vec![forestx_home.path().to_path_buf()],
        })
        .await?;
    let HooksListResponse { data } =

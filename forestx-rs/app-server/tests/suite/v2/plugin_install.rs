...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(forestx_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".forestx-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(forestx_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_forestx_plugins_disabled() -> Result<()> {
    let forestx_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("Forestx plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                forestx_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_skips_mcp_oauth_for_chatgpt_dual_surface_plugin() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    write_connectors_config(forestx_home.path(), &apps_server_url)?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
"#,
    ...
    let persisted_config = std::fs::read_to_string(forestx_home.path().join("config.toml"))?;
...
async fn plugin_install_skips_mcp_oauth_for_unowned_environment() -> Result<()> {
    ...
    let mut executor =
        tokio::process::Command::new(forestx_utils_cargo_bin::cargo_bin("exec-server")?)
    ...
    std::fs::write(
        forestx_home.path().join("environments.toml"),
        format!(
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/forestx/ps/mcp", mcp_service);
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/forestx/analytics-events/events"))
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST"
                    && request
                        .url
                        .path()
                        .ends_with("/forestx/analytics-events/events")
            }) {
...
fn configure_remote_plugin_test(forestx_home: &std::path::Path, server: &MockServer) -> Result<()> {
    write_remote_plugin_catalog_config(forestx_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        forestx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")

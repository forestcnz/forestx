...
fn init_marketplace_repo(root: &Path, marketplace_name: &str, marker: &str) -> Result<String> {
    run_git(root, &["init"])?;
    run_git(root, &["config", "user.email", "forestx@example.com"])?;
    run_git(root, &["config", "user.name", "Forestx Tests"])?;
...
fn disable_plugin_startup_tasks(forestx_home: &Path) -> Result<()> {
...
fn marketplace_install_root(forestx_home: &Path) -> std::path::PathBuf {
    forestx_home.join(INSTALLED_MARKETPLACES_DIR)
}

fn expected_installed_root(forestx_home: &Path, marketplace_name: &str) -> Result<AbsolutePathBuf> {
    AbsolutePathBuf::try_from(
        marketplace_install_root(&forestx_home.canonicalize()?).join(marketplace_name),
    )
...
async fn marketplace_upgrade_all_configured_git_marketplaces() -> Result<()> {
    ...
    disable_plugin_startup_tasks(forestx_home.path())?;
    ...
    let debug_root = expected_installed_root(forestx_home.path(), "debug")?;
    let tools_root = expected_installed_root(forestx_home.path(), "tools")?;

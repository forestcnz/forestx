...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_forestx_home = AbsolutePathBuf::try_from(forestx_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        forestx_home: response_forestx_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("forestx_vscode/"));
    assert_eq!(response_forestx_home, expected_forestx_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "forestx_app_server_daemon".to_string(),
            title: Some("Forestx App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("forestx_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_forestx_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let forestx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri())
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "forestx-backend".to_string(),
            title: Some("Forestx Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .without_auto_env()
        .with_env_overrides(&[(
            "FORESTX_INTERNAL_ORIGINATOR_OVERRIDE",
            Some("forestx_originator_via_env_var"),
        )])
        .build()
        .await?;
    ...
    assert!(user_agent.starts_with("forestx_originator_via_env_var/"));
    assert_eq!(response_forestx_home, expected_forestx_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .without_auto_env()
        .with_env_overrides(&[("FORESTX_INTERNAL_ORIGINATOR_OVERRIDE", None)])
        .build()
        .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let forestx_home = TempDir::new()?;
    let notify_file = forestx_home.path().join("notify.json");
    let notify_capture = cargo_bin("forestx-app-server-test-notify-capture")?;

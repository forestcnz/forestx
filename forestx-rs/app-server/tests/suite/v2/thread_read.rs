...
use forestx_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use forestx_thread_store::AppendThreadItemsParams;
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(forestx_home.path())?;
    let thread_id = forestx_protocol::ThreadId::default();
    let sqlite = forestx_state::SqliteConfig::new_for_testing(forestx_home.path().abs());
    let state_db =
        forestx_state::StateRuntime::init(sqlite.clone(), "mock_provider".to_string()).await?;
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: forestx_protocol::protocol::ThreadHistoryMode::Paginated,
            history_base: None,
            ...
            items: vec![
                ...
                paginated_completed_item(
                    ...
                    CoreTurnItem::UserMessage(UserMessageItem {
                        ...
                        client_id: None,
                        content: vec![
                            forestx_protocol::user_input::UserInput::Text {
                                text: "Nee".to_string(),
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let thread_id = forestx_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = forestx_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let thread_id = forestx_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    MockResponsesConfig::new("http://127.0.0.1:1")
        .with_root_config(&format!(
            r#"experimental_thread_store = {{ type = "in_memory", id = "{store_id}" }}"#
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(forestx_app_server_protocol::TurnsPage::from(expected_page))
    );
...
) -> anyhow::Result<forestx_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
fn turn_user_texts(turns: &[forestx_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[forestx_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()

...
use forestx_app_server_protocol::AttestationGenerateResponse;
...
async fn attestation_generate_round_trip_adds_header_to_responses_websocket_handshake() -> Result<()>
{
    ...
    let initialized = timeout(
        ...
        mcp.initialize_with_capabilities(
            ClientInfo {
                name: "forestx_desktop".to_string(),
                title: Some("Forestx Desktop".to_string()),
...
fn create_chatgpt_websocket_config(forestx_home: &Path, server_uri: &str) -> std::io::Result<()> {
    std::fs::write(

...
async fn assert_standalone_web_search_round_trips_output(
    ...
    let search_path = match provider {
        WebSearchProvider::ChatGpt => "/api/forestx/alpha/search",
    ...
    let config = match provider {
        WebSearchProvider::ChatGpt => config
            ...
            .with_provider_base_url(&format!("{}/api/forestx", server.uri()))
    ...
    let turn_metadata_header = search_request
        .headers
        .get("x-forestx-turn-metadata")
        .context("standalone search should include x-forestx-turn-metadata")?
        .to_str()
        .context("x-forestx-turn-metadata should be valid ASCII")?;
    let turn_metadata: Value = serde_json::from_str(turn_metadata_header)
        .context("x-forestx-turn-metadata should be valid JSON")?;

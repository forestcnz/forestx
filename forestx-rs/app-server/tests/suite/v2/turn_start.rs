...
use forestx_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use forestx_app_server::INVALID_PARAMS_ERROR_CODE;
use forestx_app_server_protocol::AdditionalContextEntry;
use forestx_app_server_protocol::AdditionalContextKind;
use forestx_app_server_protocol::ByteRange;
...
use forestx_app_server_protocol::FileChangePatchUpdatedNotification;
...
use forestx_app_server_protocol::TextElement;
...
use forestx_core::test_support::all_model_presets;
...
const TEST_ORIGINATOR: &str = "forestx_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two Forestx turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    MockResponsesConfig::new(&server.uri()).write(forestx_home.path())?;
    ...
    let image_path = forestx_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("Forestx VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        .enable_feature(Feature::Personality)
        .write(forestx_home.path())?;
    write_models_cache(forestx_home.path())?;
    let cache_path = forestx_home.path().join("models_cache.json");
    ...
    write_test_skill(forestx_home.path(), "alpha-skill")?;
    write_test_skill(forestx_home.path(), "beta-skill")?;

    let isolated_home = forestx_home.path().to_string_lossy();
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.path())
        .with_env_overrides(&[
...
async fn turn_start_emits_raw_response_completed_with_upstream_usage() -> Result<()> {
    ...
    .await??;
    let notification: forestx_app_server_protocol::ServerNotification = notification.try_into()?;
    let forestx_app_server_protocol::ServerNotification::RawResponseCompleted(notification) =
        notification
    else {
...
async fn turn_start_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let event = wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "forestx_turn_event").await?;
...
async fn code_mode_exec_emits_correlated_production_analytics() -> Result<()> {
    ...
    let event = wait_for_analytics_event(
        &server,
        DEFAULT_READ_TIMEOUT,
        "forestx_dynamic_tool_call_event",
    )
    .await?;
    assert_eq!(
        json!({
...
async fn turn_start_accepts_managed_network_profile_from_requirements() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(forestx_home.path())?;
    std::fs::write(
        forestx_home.path().join("requirements.toml"),
        r#"
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: forestx_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Forestx turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        forestx_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let ThreadStartResponse { thread, .. } = mcp
        .start_thread(ThreadStartParams {
            model: Some("exp-forestx-personality".to_string()),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let forestx_home = tmp.path().to_path_buf();
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(forestx_home.as_path())?;

    let mut mcp = TestAppServer::builder()
        .with_forestx_home(forestx_home.as_path())
        .build_initialized()
        .await?;
    ...
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            ...
            params: TurnStartParams {
                ...
                approval_policy: Some(forestx_app_server_protocol::AskForApproval::Never),
                sandbox_policy: Some(forestx_app_server_protocol::SandboxPolicy::DangerFullAccess),
                model: Some("mock-model".to_string()),
...
async fn turn_start_explicit_local_environment_updates_legacy_cwd_between_turns() -> Result<()> {
    ...
    let first_writable_root =
        forestx_utils_absolute_path::AbsolutePathBuf::try_from(first_cwd.clone())?;
    let _: TurnStartResponse = mcp
        .request(|request_id| ClientRequest::TurnStart {
            request_id,
            params: TurnStartParams {
                ...
                sandbox_policy: Some(forestx_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    MockResponsesConfig::new(&server.uri()).write(&forestx_home)?;
    std::fs::write(
        forestx_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![forestx_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .disable_feature(Feature::ShellSnapshot)
        .write(&forestx_home)?;
    write_models_cache(&forestx_home)?;
    let cache_path = forestx_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn direct_input_to_multi_agent_v2_subagent_is_rejected() -> Result<()> {
    ...
    let listed: forestx_app_server_protocol::ThreadListResponse = mcp
        .request(|request_id| ClientRequest::ThreadList {
            request_id,
            params: forestx_app_server_protocol::ThreadListParams {
                cursor: None,
                limit: Some(10),
                sort_key: None,
                sort_direction: None,
                ...
                source_kinds: Some(vec![
                    forestx_app_server_protocol::ThreadSourceKind::SubAgentThreadSpawn,
                ]),
                archived: None,
                section_id: None,
                cwd: None,
    ...
    assert!(matches!(
        &listed_child.source,
        forestx_app_server_protocol::SessionSource::SubAgent(
            forestx_protocol::protocol::SubAgentSource::ThreadSpawn {
                agent_path: Some(_),
                ..
            }
        )
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    MockResponsesConfig::new(&server.uri())
        ...
        .write(forestx_home.path())?;
    std::fs::write(
        forestx_home.path().join("custom-role.toml"),
...
async fn command_execution_notifications_include_trusted_plugin_id() -> Result<()> {
    ...
    let synced_root = forestx_home.path().join(".tmp/plugins");
    for path in [
        plugin_root.join(".forestx-plugin"),
        script_path
            .parent()
    ...
    std::fs::write(
        forestx_home.path().join(".tmp/plugins.sha"),
        format!("{curated_sha}\n"),
    )?;
    std::fs::write(
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(forestx_home.path().join("config.toml"))?;
...
fn write_test_skill(forestx_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = forestx_home.join("skills").join(name);

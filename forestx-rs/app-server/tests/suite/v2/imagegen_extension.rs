...
async fn standalone_image_generation_returns_saved_path_hint_to_model() -> Result<()> {
    ...
    create_config_toml(forestx_home.path(), &server.uri(), ImagegenTestMode::Direct)?;
    write_chatgpt_auth(
        forestx_home.path(),
    ...
    let image_request = server
        ...
        .into_iter()
        .find(|request| request.url.path() == "/api/forestx/images/generations")
...
async fn standalone_image_generation_failure_emits_terminal_item() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/forestx/images/generations"))
...
async fn standalone_image_edit_uses_attached_model_visible_image() -> Result<()> {
    ...
    let edit_request = run_image_edit_test(|forestx_home| {
        let image_path = forestx_home.join("attached.png");
...
async fn run_image_edit_test(
    ...
    let forestx_home = TempDir::new()?;
    let (arguments, input) = input(forestx_home.path())?;
    let response_mock = responses::mount_sse_sequence(
        &server,
        vec![
            responses::sse(vec![
    ...
    let image_request = requests
        .iter()
        .find(|request| request.url.path() == "/api/forestx/images/edits")
...
async fn mount_image_edit_response(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/api/forestx/images/edits"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "created": 1,
            "data": [{"b64_json": RESULT}],
        })))
...
fn create_config_toml(
    ...
    let mut config = MockResponsesConfig::new(server_uri)
        ...
        .with_provider_base_url(&format!("{server_uri}/api/forestx"))

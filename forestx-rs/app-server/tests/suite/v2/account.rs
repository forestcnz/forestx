...
use forestx_app_server_protocol::DesktopOnboardingEntrypoint;
...
use forestx_login::CLIENT_ID_OVERRIDE_ENV_VAR;
...
const LOGIN_ISSUER_ENV_VAR: &str = "FORESTX_APP_SERVER_LOGIN_ISSUER";
const LOGIN_OPEN_APP_URL_ENV_VAR: &str = "FORESTX_APP_SERVER_DEV_OPEN_APP_URL";
...
fn create_config_toml(forestx_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
fn read_config_toml(forestx_home: &Path) -> Result<toml::Value> {
    Ok(toml::from_str(&std::fs::read_to_string(
        forestx_home.join("config.toml"),
    )?)?)
}

fn load_file_auth(forestx_home: &Path) -> Result<Option<AuthDotJson>> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(forestx_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(
        !forestx_home.path().join("auth.json").exists(),
...
async fn logout_account_succeeds_when_config_reload_fails() -> Result<()> {
    ...
    assert_eq!(load_file_auth(forestx_home.path())?, None);
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(forestx_app_server_protocol::ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
        })
        .await?;
    let thread: forestx_app_server_protocol::ThreadStartResponse =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(thread_req)).await??;

    let turn_req = mcp
        .send_turn_start_request(forestx_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![forestx_app_server_protocol::UserInput::Text {
    ...
    .await?;
    let _: forestx_app_server_protocol::TurnStartResponse =
...
async fn login_amazon_bedrock_replaces_primary_auth_and_persists_provider() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let mut expected_config = read_config_toml(forestx_home.path())?;
    expected_config
        .as_table_mut()
    ...
    assert_eq!(
        load_file_auth(forestx_home.path())?,
        Some(AuthDotJson {
    ...
    assert_eq!(read_config_toml(forestx_home.path())?, expected_config);
...
async fn login_amazon_bedrock_rejects_non_bedrock_provider_override_without_changes() -> Result<()>
{
    ...
    let expected_auth = load_file_auth(forestx_home.path())?;
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    let expected_config = read_config_toml(forestx_home.path())?;
    ...
    assert_eq!(load_file_auth(forestx_home.path())?, expected_auth);
    assert_eq!(read_config_toml(forestx_home.path())?, expected_config);
...
async fn logout_aws_managed_bedrock_errors_without_changing_auth_or_config() -> Result<()> {
    let forestx_home = TempDir::new()?;
    create_config_toml(forestx_home.path(), aws_managed_bedrock_config())?;
    login_with_api_key(
        forestx_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
    ...
    assert_eq!(
        error.error.message,
        "cannot log out while Amazon Bedrock is using AWS-managed credentials; manage those credentials through AWS or switch model providers before logging out Forestx authentication"
    );
    assert_eq!(load_file_auth(forestx_home.path())?, expected_auth);
    assert_eq!(read_config_toml(forestx_home.path())?, expected_config);
    Ok(())
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/forestx/device"));
...
async fn login_account_chatgpt_redirects_to_hosted_success_page() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            (
                ...
                Some("http://localhost:3000/forestx/open-app"),
            ),
        ])
        .build_initialized_with_timeout(DEFAULT_READ_TIMEOUT)
        .await?;
    ...
    assert_eq!(
        ...
        "http://localhost:3000/forestx/open-app?source=login&app_brand=chatgpt"
    );

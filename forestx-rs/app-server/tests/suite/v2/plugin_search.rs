...
use forestx_app_server_protocol::PluginSearchResponse;
use forestx_app_server_protocol::PluginSearchScope;
...
async fn plugin_search_stitches_local_results_into_the_first_remote_page() -> Result<()> {
    ...
    write_remote_plugin_search_config(forestx_home.path(), &server, /*remote_plugin*/ true)?;
    write_chatgpt_search_auth(forestx_home.path())?;
...
async fn plugin_search_returns_local_matches_for_api_key_auth() -> Result<()> {
    for remote_plugin_enabled in [false, true] {
        let forestx_home = TempDir::new()?;
        let repo_root = TempDir::new()?;
        let curated_root = forestx_home.path().join(".tmp/plugins");
        let bundled_alpha_root = forestx_home
            .path()
        ...
        write_remote_plugin_search_config(forestx_home.path(), &server, remote_plugin_enabled)?;
        login_with_api_key(
            forestx_home.path(),
            "sk-test-key",
            AuthCredentialsStoreMode::File,
...
-> Result<()> {
    ...
    write_installed_plugin(forestx_home.path(), "openai-curated", "calendar")?;
    write_installed_plugin(forestx_home.path(), "personal-tools", "local-planner")?;
    write_installed_plugin(forestx_home.path(), "personal-tools", "calendar-local-only")?;
    std::fs::write(
        forestx_home.path().join("config.toml"),
        format!(
            r#"chatgpt_base_url = "{}/backend-api/"
...
fn write_local_marketplace(
    ...
    for plugin in plugins {
        let plugin_manifest = root.join("plugins").join(plugin.name).join(".forestx-plugin");
...
fn write_chatgpt_search_auth(forestx_home: &std::path::Path) -> Result<()> {
    write_chatgpt_auth(
        forestx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_installed_plugin(
    ...
    let plugin_manifest = forestx_home
        .join("plugins/cache")

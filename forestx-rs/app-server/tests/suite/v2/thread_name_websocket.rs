...
use forestx_core::find_thread_name_by_id;
...
async fn thread_name_updated_broadcasts_for_loaded_threads() -> Result<()> {
    ...
    let conversation_id = create_rollout(forestx_home.path(), "2025-01-05T12-00-00")?;
    ...
    let result = async {
        ...
        assert_legacy_thread_name(forestx_home.path(), &conversation_id, renamed).await?;
...
async fn thread_name_updated_broadcasts_for_not_loaded_threads() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    create_config_toml(forestx_home.path(), &server.uri(), "never")?;
    let conversation_id = create_rollout(forestx_home.path(), "2025-01-05T12-05-00")?;
...
fn create_rollout(forestx_home: &std::path::Path, filename_ts: &str) -> Result<String> {
...
async fn assert_legacy_thread_name(
    ...
    assert_eq!(
        find_thread_name_by_id(forestx_home, &thread_id)
            .await?
            .as_deref(),
        Some(expected_name)
    );

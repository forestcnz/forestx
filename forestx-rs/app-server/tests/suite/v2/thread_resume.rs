...
use chrono::Utc;
use forestx_app_server_protocol::ApprovalsReviewer;
...
use forestx_app_server_protocol::ThreadActiveFlag;
...
use forestx_app_server_protocol::ThreadStatusChangedNotification;
...
use forestx_core::ARCHIVED_SESSIONS_SUBDIR;
use forestx_features::Feature;
use forestx_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use forestx_protocol::user_input::ByteRange;
use forestx_protocol::user_input::TextElement;
use forestx_rollout::append_rollout_item_to_path;
use forestx_rollout::read_session_meta_line;
...
const FORESTX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Forestx, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_paginated_model_context_preserves_original_metadata() -> Result<()> {
    ...
    mock_responses_config(&server.uri()).write(forestx_home.path())?;
    ...
    let path = rollout_path(forestx_home.path(), "2025-01-05T12-00-00", &conversation_id);
...
async fn thread_resume_preserves_persisted_approvals_reviewer() -> Result<()> {
    ...
    let config_path = forestx_home.path().join("config.toml");
...
async fn thread_resume_preserves_goal_first_and_fork_approvals_reviewer() -> Result<()> {
    ...
    let (thread_id, fork_thread_id) = {
        ...
        let start_id = mcp
            .send_thread_start_request_with_auto_env(ThreadStartParams {
                model: Some("gpt-5.2-forestx".to_string()),
                approvals_reviewer: Some(ApprovalsReviewer::AutoReview),
                ..Default::default()
            })
            .await?;
...
-> Result<()> {
    ...
    assert_eq!(model, "gpt-5.2-forestx");
    ...
    let update_id = mcp
        .send_thread_settings_update_request(ThreadSettingsUpdateParams {
            ...
            collaboration_mode: Some(CollaborationMode {
                ...
                settings: Settings {
                    model: "gpt-5.2-forestx".to_string(),
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let forestx_home = TempDir::new()?;
    let forestx_home_path = normalized_existing_path(forestx_home.path())?;
    mock_responses_config(&server.uri()).write(&forestx_home_path)?;
    let config_path = forestx_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = forestx_home_path
        .as_path()
        .to_str()
        .expect("test forestx home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_forestx_home(&forestx_home_path)
        .without_managed_config()
        .with_env_overrides(&[("FORESTX_SQLITE_HOME", Some(sqlite_home))])
        .build_initialized()
        .await?;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    mock_responses_config(&server.uri())
        ...
        .write(forestx_home.path())?;
    mount_analytics_capture(&server, forestx_home.path()).await?;
...
async fn thread_resume_running_thread_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let start_id = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            ...
            service_name: Some("forestx_work_desktop".to_string()),
            ..Default::default()
        })
        .await?;
    let ThreadStartResponse { thread, .. } =
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "forestx_thread_initialized"
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(forestx_home, filename_ts, thread_id);
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in ["forestx_chatgpt_android_remote", "forestx_chatgpt_ios_remote"] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(forestx_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(forestx_home.path(), filename_ts, &conversation_id);
    let archived_dir = forestx_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "forestx unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            forestx_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    let get_id = mcp
        ...
        .await?;
    let get: forestx_app_server_protocol::ThreadGoalGetResponse =
...
async fn cold_paginated_resume_omits_usage_when_its_turn_is_ambiguous() -> Result<()> {
    ...
    let path = rollout_path(forestx_home.path(), filename_ts, &conversation_id);
    append_rollout_item_to_path(
        &path,
        &RolloutItem::EventMsg(EventMsg::TokenCount(TokenCountEvent {
            info: Some(TokenUsageInfo {
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(forestx_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    ...
    let repo_path = forestx_home.path().join("repo");
    ...
    let rollout_path = rollout_path(forestx_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let forestx_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(forestx_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(forestx_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let ThreadResumeResponse { cwd, .. } =
        timeout(DEFAULT_READ_TIMEOUT, mcp.read_response(resume_id)).await??;
    assert_eq!(cwd, AbsolutePathBuf::from_absolute_path(forestx_home.path())?);
...
async fn thread_resume_rejects_mismatched_path_for_running_thread_id() -> Result<()> {
    ...
    let stale_path = rollout_path(forestx_home.path(), "2025-01-01T00-00-00", &stale_thread_id);
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let forestx_home = tmp.path().join("forestx_home");
    std::fs::create_dir(&forestx_home)?;
...
"#;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![forestx_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(forestx_home.path(), "materialize").await?;
...
command = "forestx-definitely-not-a-real-binary"
required = true"#,
        )
        .write(forestx_home.path())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(forestx_home.path(), "seed history").await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(FORESTX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
async fn setup_rollout_fixture(forestx_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    mock_responses_config(server_uri).write(forestx_home)?;

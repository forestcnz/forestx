...
use forestx_app_server_protocol::CapabilityRootLocation;
...
async fn selected_capability_stack_tracks_environment_availability_and_resume() -> Result<()> {
    ...
    let mut app_server = TestAppServer::builder()
        .with_forestx_home(fixture.forestx_home.path())
    ...
    let mut exec_server =
        spawn_exec_server(fixture.forestx_home.path(), &fixture.exec_server_url).await?;
    ...
    exec_server = spawn_exec_server(fixture.forestx_home.path(), &fixture.exec_server_url).await?;
    add_environment(&mut app_server, &fixture.exec_server_url).await?;
    wait_for_selected_mcp_server(&mut app_server, &thread_id).await?;
...
fn selected_capability_fixture(
    ...
    let local_skill_dir = forestx_home.path().join("skills/local-deploy");
    ...
    let manifest_dir = plugin.path().join(".forestx-plugin");
...
fn assert_selected_plugin_tools_absent(request: &ResponsesRequest) {
    ...
    let connector = request
        .tool_by_name("mcp__forestx_apps__calendar", "connector_calendar")
...
async fn spawn_exec_server(forestx_home: &std::path::Path, url: &str) -> Result<Child> {
    let mut child = Command::new(forestx_utils_cargo_bin::cargo_bin("forestx")?)

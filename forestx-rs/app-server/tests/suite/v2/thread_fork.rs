...
use forestx_rollout::append_thread_name;
...
async fn thread_fork_creates_new_thread_and_emits_started() -> Result<()> {
    ...
    let original_path = forestx_home
        .path()
        .join("sessions")
        .join("2025")
        .join("01")
...
async fn thread_fork_defers_inherited_active_goal_until_next_turn() -> Result<()> {
    ...
    assert_eq!(
        forked_goal.status,
        forestx_state::ThreadGoalStatus::BudgetLimited
    );
    assert_eq!(
        state_db
            .thread_goals()
...
async fn thread_fork_inherits_explicit_source_name_from_session_index() -> Result<()> {
    ...
    append_thread_name(forestx_home.path(), source_thread_id, source_name).await?;
...
async fn thread_fork_can_cut_before_unfinished_stored_turn() -> Result<()> {
    ...
    let source_path = rollout_path(forestx_home.path(), filename_ts, &conversation_id);
...
async fn assert_thread_fork_freezes_active_paginated_turn_as_interrupted(
    ...
    config.write(forestx_home.path())?;
    ...
    let source_path = rollout_path(forestx_home.path(), "2025-01-05T12-00-00", &source_thread_id);
    ...
    assert!(matches!(
        ...
        [
            ...
            RolloutLine {
                item: RolloutItem::ResponseItem(forestx_protocol::models::ResponseItem::Message {
                    role,
                    ..
                }),
                ..
...
#[tokio::test]
async fn pathless_ephemeral_thread_rejects_forestx_home_path_after_reload() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let forestx_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(forestx_home.path())?;
    ...
    let resume_id = app_server
        .send_thread_resume_request(ThreadResumeParams {
            ...
            path: Some(forestx_home_path.clone()),
            ..Default::default()
        })
        .await?;

...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&forestx_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(forestx_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(forestx_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(command.starts_with(&command_packaged_zsh_path(&forestx_home).display().to_string()));
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(forestx_app_server_protocol::AskForApproval::UnlessTrusted),
    ...
    while target_decision_index < target_decisions.len() || !saw_parent_approval {
        ...
        let is_parent_approval = approval_command
            .contains(&command_packaged_zsh_path(&forestx_home).display().to_string())
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(forestx_home, zsh_path)?;
...
fn create_test_package_app_server(forestx_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = forestx_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(forestx_home);
    ...
    std::fs::write(package_dir.join("forestx-package.json"), "{}")?;

    let app_server = bin_dir.join("forestx-app-server");
    copy_with_permissions(
        &forestx_utils_cargo_bin::cargo_bin("forestx-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(forestx_home: &Path) -> PathBuf {
    forestx_home
        .join("test-package")
...
fn command_packaged_zsh_path(forestx_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(forestx_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = forestx_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("forestx-rs/app-server/tests/suite/zsh");

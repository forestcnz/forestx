...
use forestx_app_server_protocol::AppBranding;
...
use forestx_app_server_protocol::AppReview;
use forestx_app_server_protocol::AppScreenshot;
...
#[tokio::test]
async fn list_apps_returns_empty_when_workspace_forestx_plugins_disabled() -> Result<()> {
    let connectors = vec![AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: Some("Beta connector".to_string()),
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let forestx_home = TempDir::new()?;
    write_connectors_and_plugins_config(forestx_home.path(), &server_url)?;
    write_plugin_app_fixture(forestx_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        forestx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_connectors_and_plugins_config(forestx_home: &Path, base_url: &str) -> std::io::Result<()> {
    let config_toml = forestx_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
...
fn write_plugin_app_fixture(forestx_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {

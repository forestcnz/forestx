...
use forestx_app_server_protocol::ConfigReadParams;
use forestx_app_server_protocol::ConfigValueWriteParams;
...
use forestx_app_server_protocol::FsCopyParams;
use forestx_app_server_protocol::FsCreateDirectoryParams;
use forestx_app_server_protocol::FsGetMetadataParams;
use forestx_app_server_protocol::FsReadDirectoryParams;
use forestx_app_server_protocol::FsReadFileParams;
use forestx_app_server_protocol::FsRemoveParams;
use forestx_app_server_protocol::FsUnwatchParams;
use forestx_app_server_protocol::FsWatchParams;
use forestx_app_server_protocol::FsWriteFileParams;
...
use forestx_app_server_protocol::PluginSearchParams;
use forestx_app_server_protocol::PluginSkillReadParams;
use forestx_app_server_protocol::PluginUninstallParams;
use forestx_app_server_protocol::ProcessKillParams;
use forestx_app_server_protocol::ProcessSpawnParams;
use forestx_app_server_protocol::RemoteControlClientsListParams;
use forestx_app_server_protocol::RemoteControlClientsRevokeParams;
use forestx_app_server_protocol::RemoteControlPairingStartParams;
use forestx_app_server_protocol::RemoteControlPairingStatusParams;
...
use forestx_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use forestx_exec_server::FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use forestx_exec_server::FORESTX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use forestx_exec_server::FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use forestx_exec_server::FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use forestx_exec_server::FORESTX_EXEC_SERVER_URL_ENV_VAR;
...
use core_test_support::test_forestx::TestEnv;
use core_test_support::test_forestx::test_env;
...
pub struct TestAppServer {
    ...
    // removing an owned FORESTX_HOME that may still be its cwd on Windows.
    ...
    _owned_forestx_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "forestx-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "FORESTX_APP_SERVER_DISABLE_MANAGED_CONFIG";
#[cfg(windows)]
...
impl TestAppServer {
    /// Starts building a server with a temporary FORESTX_HOME and the standard
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            forestx_home.join("managed_config.toml"),
        );
        cmd.env_remove(FORESTX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("forestx-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: forestx_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing FORESTX_HOME instead of a temporary one.
    pub fn with_forestx_home(mut self, forestx_home: &Path) -> Self {
        self.forestx_home = Some(forestx_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary FORESTX_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (forestx_home, owned_forestx_home) = match forestx_home {
            Some(forestx_home) => (forestx_home, None),
            None => {
                let owned_forestx_home = TempDir::new()?;
                (
                    owned_forestx_home.path().to_path_buf(),
                    Some(owned_forestx_home),
                )
            }
        };
        let attribution_settings_server = if forestx_home.join("auth.json").is_file() {
            let config_path = forestx_home.join("config.toml");
        ...
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = forestx_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            forestx_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&forestx_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        FORESTX_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        FORESTX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        FORESTX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (FORESTX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(), None),
                    (
                        FORESTX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        let mut program = match program {
            ...
            None => forestx_utils_cargo_bin::cargo_bin("forestx-app-server")
                .context("should find binary for forestx-app-server")?,
        };
        let mut owned_install_dir = None;
        if !custom_program
            && forestx_utils_cargo_bin::runfiles_available()
            && let Ok(code_mode_host_program) =
                forestx_utils_cargo_bin::cargo_bin("forestx-code-mode-host")
        {
        ...
        app_server._owned_forestx_home = owned_forestx_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `forestx-app-server` child process.
        //

...
pub fn rollout_path(forestx_home: &Path, filename_ts: &str, thread_id: &str) -> PathBuf {
...
/// Create a minimal rollout file under `FORESTX_HOME/sessions/YYYY/MM/DD/`.
///
...
pub fn create_fake_rollout_with_token_usage(
    ...
    let file_path = rollout_path(forestx_home, filename_ts, &thread_id);
    let line = json!({
...
fn create_fake_rollout_with_source_and_parent_thread_id(
    ...
    let file_path = rollout_path(forestx_home, filename_ts, &uuid_str);
    let dir = file_path
        .parent()
...
pub fn create_fake_rollout_with_text_elements(
    ...
    let month = &filename_ts[5..7];
    let day = &filename_ts[8..10];
    let dir = forestx_home.join("sessions").join(year).join(month).join(day);
    fs::create_dir_all(&dir)?;

...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(forestx_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(forestx_home);
        command.env("FORESTX_HOME", forestx_home);
        #[cfg(target_os = "linux")]
        command.env(
            FORESTX_LINUX_SANDBOX_EXE_ENV_VAR,

...
`forestx app-server` is the interface Forestx uses to power rich interfaces such as the [Forestx VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `forestx app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).

Supported transports:

- stdio (`--stdio` or `--listen stdio://`, default): newline-delimited JSON (JSONL)
- websocket (`--listen ws://IP:PORT`): one JSON-RPC message per websocket text frame (**experimental / unsupported**)
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over `$FORESTX_HOME/app-server-control/app-server-control.sock` or a custom socket path, using the standard HTTP Upgrade handshake
...
The unix socket transport is intended for local app-server control-plane clients. `forestx app-server proxy`
opens exactly one raw stream connection to `$FORESTX_HOME/app-server-control/app-server-control.sock`
...
Currently, you can dump a TypeScript version of the schema using `forestx app-server generate-ts`, or a JSON Schema bundle via `forestx app-server generate-json-schema`. Each output is specific to the version of Forestx you used to run the command, so the generated artifacts are guaranteed to match that version.

```
forestx app-server generate-ts --out DIR
forestx app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and Forestx:

- **Thread**: A conversation between a user and the Forestx agent. Each thread contains multiple turns.
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `forestxHome` for the server's Forestx home directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
when a Forestx session is created by `thread/start`, `thread/resume`, or
`thread/fork`. Forestx advertises that profile in the downstream MCP
...
Applications building on top of `forestx app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new Forestx integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-Forestx
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "forestx_vscode",
      "title": "Forestx VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `memory/reset` — experimental; clear the current `FORESTX_HOME/memories` directory and reset persisted memory stage data in sqlite while preserving existing thread memory modes; returns `{}` on success.
...
- `thread/realtime/start` — start a thread-scoped realtime session (experimental); pass `outputModality: "text"` or `outputModality: "audio"` to choose model output, optionally pass `model` and `version` to override configured realtime selection for this session only, pass `includeStartupContext: false` to omit Forestx's generated startup context, and optionally pass `initialItems` to seed V3 with complete role-bearing text messages at session creation. Pass `realtimeStartInstructions` and `realtimeEndInstructions` to control the developer instructions given to the backing Forestx model when this session starts and ends. Version `"v1"` uses legacy Bidi `conversation.handoff.*`, `"v2"` uses the Realtime Voice API, and `"v3"` preserves V1 Forestx Voice behavior while using Frameless Bidi `delegation.*`. For V3 automatic Forestx text, `forestxResponseHandoffMode` accepts `"thinking"` (the default; all output uses channel-less thinking appends), `"commentary"` (all output uses the commentary channel), or `"bemTags"` (the raw BEM envelope selects the API channel: BEM `analysis` and `commentary` use `commentary`, while BEM `final` and unparsable output use `speakable`). The BEM envelope remains in the appended text for the frontend model to interpret. V1 and V2 ignore this setting. For V3, pass `delegationAckFiller: false` to suppress the Realtime API's delegation acknowledgement filler or `true` to restore it; omitting the field preserves the Realtime API's default. V1 and V2 ignore `delegationAckFiller`. V3 handoffs do not prepend the legacy `"Agent Final Message"` label. Pass `clientManagedHandoffs: true` to disable automatic Forestx response delivery so only the client's explicit append calls produce handoffs. Pass `forestxResponsesAsItems: true` to send automatic Forestx responses as realtime conversation items instead, and optionally pass `forestxResponseItemPrefix` to prepend experiment instructions to those items. Returns `{}` and streams `thread/realtime/*` notifications. Omit `transport` for the websocket transport, or pass `{ "type": "webrtc", "sdp": "..." }` to create a Bidi WebRTC session from a browser-generated SDP offer; the remote answer SDP is emitted as `thread/realtime/sdp`. Conversation `version: "v2"` requests remain unsupported for WebRTC.
...
- `process/spawn` — experimental; spawn a standalone process without the Forestx sandbox on the host where the app server is running; returns after the process starts and emits `process/outputDelta` and `process/exited` notifications.
...
Start a fresh thread when you need a new Forestx conversation.

```json
{ "method": "thread/start", "id": 10, "params": {
    // Optionally set config settings. If not specified, will use the user's
    // current config settings.
    "model": "gpt-5.1-forestx",
...
Experimental: use `memory/reset` to clear local memory artifacts and sqlite-backed memory stage data for the current Forestx home. This preserves existing thread memory modes; use `thread/memoryMode/set` separately when a thread's future memory eligibility should change.
...
Turns attach user input (text, images, or audio) to a thread and trigger Forestx generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.forestx/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
Then send `offer.sdp` to app-server. Core uses `experimental_realtime_ws_backend_prompt` for the backend instructions and the thread conversation id as the default Realtime API session identifier. This `realtimeSessionId` value refers to the upstream Realtime API session, not a Forestx session/thread-group id. The start response is `{}`; the remote answer SDP arrives later as `thread/realtime/sdp` and should be passed to `setRemoteDescription()`:
...
Omit `prompt` to use Forestx's default realtime backend prompt. Send `prompt: null` or
...
the backing Forestx model when the thread enters realtime mode, and
`realtimeEndInstructions` to provide its developer instructions when the session
ends. These instructions configure Forestx, not the realtime frontend model. They
...
field preserves Forestx's existing default instructions.
...
Pass `includeStartupContext: false` to skip Forestx's startup context for this
...
Pass `clientManagedHandoffs: true` to suppress automatic Forestx response handoffs
...
Pass `forestxResponsesAsItems: true` to inject automatic Forestx responses with
`conversation.item.create` instead of the protocol's default speakable output
path. When using that mode, `forestxResponseItemPrefix` can prepend short
experiment instructions to each automatic Forestx response item. Omit
`forestxResponsesAsItems`, or pass `false`, to preserve the default speakable
behavior. In V3, automatic handoffs default to
`forestxResponseHandoffMode: "thinking"`, which omits the context append `channel`
...
`forestxResponseHandoffChannelPrefixes` to override the accepted prefixes for
...
clients may continue to send the removed `forestxResponseHandoffPrefix` field; the
...
Use `review/start` to run Forestx’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions Forestx will run).
...
For an inline review, Forestx streams the usual `turn/started` notification followed by an `item/started`
...
- `processId` is optional for buffered execution. When omitted, Forestx generates an internal id for lifecycle tracking, but `tty`, `streamStdin`, and `streamStdoutStderr` must stay disabled and follow-up `command/exec/write` / `command/exec/terminate` calls are not available for that command.
...
Use `process/spawn` to start a standalone argv-based process without the Forestx sandbox on the host where the app server is running. The `process/*` API is experimental and requires `initialize.params.capabilities.experimentalApi: true`. The spawn response means the process has started and the `processHandle` is registered; completion is reported later through `process/exited`.
...
- `thread/realtime/started` — `{ threadId, realtimeSessionId }` once realtime starts for the thread (experimental). `realtimeSessionId` is the upstream Realtime API session identifier, not a Forestx session/thread-group id.
...
- `turn/completed` — `{ turn }` where `turn.status` is `completed`, `interrupted`, or `failed`; successful turns include their final agent message when available, and failures carry `{ error: { message, forestxErrorInfo?, additionalDetails? } }`.
...
- `contextCompaction` — `{id}` emitted when forestx compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when forestx compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
`error` event is emitted whenever the server hits an error mid-turn (for example, upstream model errors or quota limits). Carries the same `{ error: { message, forestxErrorInfo?, additionalDetails? } }` payload as `turn.status: "failed"` and may precede that terminal notification.

`forestxErrorInfo` maps to the `ForestxErrorInfo` enum. Common values:
...
When an upstream HTTP status is available (for example, from the Responses API or a provider), it is forwarded in `httpStatusCode` on the relevant `forestxErrorInfo` variant.

## Approvals

Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell Forestx whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
...
`forestx_approval_kind: "mcp_tool_call"` and may include `persist: "session"`,
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.forestx/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a Forestx skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a Forestx skill",
                "iconSmall": "icon.svg",
...
```json
{
  ...
  "id": 27,
  "params": {
    "path": "/Users/alice/.forestx/skills/skill-creator/SKILL.md",
    "name": null,
    "enabled": false
  }
}
...
For linked Git worktrees, project hook declarations come from the matching `.forestx/` folders in the root checkout rather than from divergent hook declarations stored only in the linked worktree. This keeps each repo on one authoritative project-hook definition and one trust state.
...
```json
{
  ...
  "result": {
    "data": [{
      "cwd": "/Users/me/project",
      "hooks": [{
        "key": "/Users/me/.forestx/config.toml:pre_tool_use:0:0",
        ...
        "sourcePath": "/Users/me/.forestx/config.toml",
        "source": "user",
        "pluginId": null,
        "displayOrder": 0,
        "enabled": true,
...
```json
{
  ...
  "params": {
    "edits": [{
      "keyPath": "hooks.state",
      "value": {
        "/Users/me/.forestx/config.toml:pre_tool_use:0:0": {
          "enabled": false
        }
      },
...
Forestx supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`), which also includes the current ChatGPT `planType` when available, and can be inferred from `account/read`. Self-serve Business ProLite accounts use the `self_serve_business_prolite` plan type; Enterprise automation accounts use `enterprise_cbp_automation`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): Forestx owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"` for the browser flow or `type: "chatgptDeviceCode"` for device code; Forestx persists tokens to disk and refreshes them automatically.
- **Forestx managed Amazon Bedrock auth (`amazonBedrock`, experimental)**: Caller supplies an Amazon Bedrock API key and region via `account/login/start` with `type: "amazonBedrock"`. The client must enable the `experimentalApi` initialization capability for Forestx-managed Amazon Bedrock login. Forestx replaces the current primary auth with the Bedrock credential and writes `model_provider = "amazon-bedrock"` to the user config.
- **Personal access token (`personalAccessToken`)**: Forestx uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `forestx login --with-access-token` or `FORESTX_ACCESS_TOKEN`.
...
{ "id": 1, "result": { "account": { "type": "amazonBedrock", "usesForestxManagedCredentials": false }, "requiresOpenaiAuth": false } }
```
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, Forestx can run without OpenAI credentials.
- Amazon Bedrock reports `usesForestxManagedCredentials: true` when it uses a Bedrock API key managed by Forestx. It reports `false` for external credential paths, including the AWS credential chain and configured command auth. This identifies whether Forestx-managed credentials are selected; it does not validate that the credential source can resolve credentials.
...
2. Open `authUrl` in a browser; the app-server hosts the local callback.
   ...
   organization setup to the hosted Forestx success page instead. When hosted login success is
   enabled, clients may set `appBrand` to `"forestx"` or `"chatgpt"` to select the matching hosted
   page artwork; omitted or `null` values default to `"forestx"`.
...
1. Start:
   ...
   { "id": 4, "result": { "type": "chatgptDeviceCode", "loginId": "<uuid>", "verificationUrl": "https://auth.openai.com/forestx/device", "userCode": "ABCD-1234" } }
   ```
...
When using a Forestx-managed Bedrock key, logout removes the key and clears `model_provider` if it is still set to `"amazon-bedrock"`. When using AWS-managed credentials, manage them through AWS or switch providers before logging out.
...
`forestx app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
forestx app-server generate-ts --out DIR --experimental
forestx app-server generate-json-schema --out DIR --experimental
```
...
5. Verify the protocol crate:
   ...
   ```bash
   just test -p forestx-app-server-protocol
   ```

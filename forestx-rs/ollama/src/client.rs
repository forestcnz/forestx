...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(forestx_core::spawn::FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
    ...
    async fn test_try_from_provider_preserves_outbound_proxy_policy() {
        ...
        forestx_http_client::cache_system_proxy_route_for_test(&request_url, proxy.uri());
    ...
    async fn test_try_from_provider_handles_invalid_custom_ca_by_proxy_policy() {
        const CHILD_POLICY_ENV: &str = "FORESTX_OLLAMA_INVALID_CA_TEST_POLICY";
        ...
        let Ok(policy_name) = std::env::var(CHILD_POLICY_ENV) else {
            ...
            for ca_env in ["FORESTX_CA_CERTIFICATE", "SSL_CERT_FILE"] {
                for policy_name in ["reqwest-default", "respect-system-proxy"] {
                    ...
                    .arg("--nocapture")
                    .env_remove("FORESTX_CA_CERTIFICATE")
        ...
        match outbound_proxy_policy {
            ...
            OutboundProxyPolicy::RespectSystemProxy => {
                ...
                let ca_env = if std::env::var_os("FORESTX_CA_CERTIFICATE").is_some() {

...
use forestx_core::exec::process_exec_tool_call;
...
fn forestx_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_forestx-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> forestx_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<forestx_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let forestx_linux_sandbox_exe = Some(forestx_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &forestx_protocol::exec_output::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(err) => match err.details() {
            ...
            ForestxErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => true,
...
fn expect_denied(
    result: Result<forestx_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> forestx_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let forestx_linux_sandbox_exe: Option<PathBuf> = Some(forestx_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_forestx_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_forestx = tmpdir.path().join(".forestx");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_forestx).expect("create .forestx");

    let git_target = dot_git.join("config");
    let forestx_target = dot_forestx.join("config.toml");
    ...
    let forestx_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", forestx_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".forestx write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(forestx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_forestx_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-forestx");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_forestx = tmpdir.path().join(".forestx");
    symlink(&decoy, &dot_forestx).expect("create .forestx symlink");
    ...
    let forestx_output = expect_denied(
        ...
        .await,
        ".forestx symlink replacement should be denied",
    );
    assert_ne!(forestx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_forestx_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    .await
    {
        Err(err) => match err.details() {
            ...
            details => panic!(".forestx symlink build failure should deny: {details:?}"),
        },
        Ok(output) => panic!(".forestx symlink build failure should deny: {output:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|forestx|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_forestx_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".forestx"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .forestx directory creation should be denied",
    );
    assert_ne!(mkdir_forestx_output.exit_code, 0);
    assert!(!subdir.join(".forestx").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = forestx_linux_sandbox_exe()
        .parent()

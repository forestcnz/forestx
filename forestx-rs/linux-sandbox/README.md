# forestx-linux-sandbox

This crate is responsible for producing:

- a `forestx-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Forestx CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `forestx-exec` CLI can check if its arg0 is `forestx-linux-sandbox` and, if so, execute as if it were `forestx-linux-sandbox`
  - this should also be true of the `forestx` multitool CLI

On Linux, Forestx prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `forestx-resources/bwrap` binary shipped
with Forestx.
Forestx also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Forestx surfaces the same startup warning
...
it cannot create the required user namespaces, so Forestx rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `forestx-resources/bwrap` path.
- If `bwrap` is missing, Forestx also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Forestx surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Forestx rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.forestx`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `forestx sandbox`; the host OS selects the sandbox backend.

...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/forestx-arg0-session/forestx-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
            ...
            "/proc".to_string(),
            "--argv0".to_string(),
            "forestx-linux-sandbox".to_string(),
            "--".to_string(),
            "/bin/true".to_string(),
        ]
    );
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/forestx-arg0-session/forestx-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--forestx-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
...
fn synthetic_mount_registry_root_is_unique_to_effective_user() {
    ...
    assert_eq!(
        ...
        std::env::temp_dir().join(format!(
            "forestx-bwrap-synthetic-mount-targets-{effective_uid}"
        ))
    );
}

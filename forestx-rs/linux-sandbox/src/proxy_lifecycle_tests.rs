...
fn cleanup_proxy_socket_dir_removes_bridge_artifacts() {
    ...
    let socket_dir = root.path().join("forestx-linux-sandbox-proxy-test");
...
fn parse_proxy_socket_dir_owner_pid_reads_owner_pid() {
    assert_eq!(
        parse_proxy_socket_dir_owner_pid("forestx-linux-sandbox-proxy-1234-0"),
        Some(1234)
    );
    assert_eq!(
        parse_proxy_socket_dir_owner_pid("forestx-linux-sandbox-proxy-1234-1000-0"),
        Some(1234)
    );
    assert_eq!(
        parse_proxy_socket_dir_owner_pid("forestx-linux-sandbox-proxy-x"),
        None
    );

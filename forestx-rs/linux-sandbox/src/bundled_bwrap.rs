...
fn legacy_candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    candidates.push(exe_dir.join("forestx-resources").join("bwrap"));
    if let Some(package_target_dir) = exe_dir.parent() {
        candidates.push(package_target_dir.join("forestx-resources").join("bwrap"));
    }
...
fn expected_sha256() -> Option<[u8; 32]> {
    ...
    *EXPECTED.get_or_init(|| {
        let raw_digest = option_env!("FORESTX_BWRAP_SHA256")?;
        let digest = parse_sha256_hex(raw_digest)
            .unwrap_or_else(|err| panic!("invalid FORESTX_BWRAP_SHA256 value: {err}"));
...
mod tests {
    ...
    fn finds_package_layout_bwrap_from_install_context() {
        ...
        let resources_dir = package_dir.join("forestx-resources");
    ...
    fn finds_legacy_standalone_bundled_bwrap_next_to_exe_resources() {
        let temp_dir = tempdir().expect("temp dir");
        let exe = temp_dir.path().join("forestx");
        let expected_bwrap = temp_dir.path().join("forestx-resources").join("bwrap");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let exe = target_dir.join("forestx").join("forestx");
        let expected_bwrap = target_dir.join("forestx-resources").join("bwrap");
        write_executable(&exe);
        write_executable(&expected_bwrap);

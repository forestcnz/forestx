...
//! - sensitive subpaths such as `.git`, `.agents`, and `.forestx` remain
...
use forestx_protocol::permissions::is_protected_metadata_name;
...
use forestx_protocol::protocol::FileSystemSpecialPath;
use forestx_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.forestx`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".forestx")
                {
                    return None;
                }
    ...
    for (search_root, globs) in patterns_by_search_root {
        for path in ripgrep_files(search_root.as_path(), &globs, max_depth)? {
            ...
            if expanded_paths.len() > MAX_UNREADABLE_GLOB_MATCHES {
                return Err(ForestxErr::Fatal(format!(
...
mod tests {
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_forestx = temp_dir.path().join("real-forestx");
        let logical_forestx = logical_home.join(".forestx");
        let real_memories = real_forestx.join("memories");
        let logical_memories = logical_forestx.join("memories");
        ...
        std::os::unix::fs::symlink(&real_forestx, &logical_forestx)
            .expect("create symlinked forestx home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".forestx"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_forestx = path_to_string(&temp_dir.path().join(".forestx"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_forestx));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_forestx)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.forestx"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.forestx"),
            ]
        );
        assert_eq!(
            args.args,

...
This crate implements the business logic for Forestx. It is designed to be used by the various Forestx UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //forestx-rs/core:core-all-wine-exec-test`.
...
Note that `forestx-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.forestx` read-only.
...
Expects the binary containing `forestx-core` to run the equivalent of `forestx sandbox` when `arg0` is `forestx-linux-sandbox`. See the `forestx-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `forestx-resources/bwrap`
binary shipped with Forestx and Forestx surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
Forestx also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so Forestx rejects sandboxed shell commands that would enter the
...
Expects the binary containing `forestx-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--forestx-run-as-apply-patch`. See the
`forestx-arg0` crate for details.

...
use forestx_connectors::merge::plugin_connector_to_app_info;
use forestx_connectors::metadata::connector_install_url;
use forestx_connectors::metadata::sanitize_name;
...
fn forestx_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{FORESTX_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| FORESTX_APPS_MCP_SERVER_NAME.to_string());
...
fn synthetic_links_are_exposed_to_the_agent_but_not_accessible_in_app_list() {
    let mut synthetic_tool = forestx_app_tool("gmail_batch_read_email", "gmail", Some("Gmail"), &[]);
    ...
    let tools = vec![
        synthetic_tool,
        forestx_app_tool("calendar_list_events", "calendar", Some("Calendar"), &[]),
    ];
...
fn accessible_connectors_from_mcp_tools_preserves_description() {
    let mcp_tools = vec![ToolInfo {
        ...
        callable_namespace: "mcp__forestx_apps__calendar".to_string(),
...
"#
        ...
        assert_eq!(
            mcp_approvals_reviewer(&config, FORESTX_APPS_MCP_SERVER_NAME, Some("calendar")),
            expected_app
        );
        assert_eq!(
            mcp_approvals_reviewer(&config, FORESTX_APPS_MCP_SERVER_NAME, Some("drive")),
            expected_default
        );
        assert_eq!(
...
"#,
    ...
    let plugins_manager = PluginsManager::new(config.forestx_home.to_path_buf());

...
fn foreign_agents_md_uses_environment_native_paths() {
    let (cwd, rendered_cwd) = if cfg!(windows) {
        (
            PathUri::parse("file:///forestx%20runtime").expect("POSIX cwd URI"),
            "/forestx runtime",
        )
    } else {
        (
            PathUri::parse("file:///C:/forestx%20runtime").expect("Windows cwd URI"),
            r"C:\forestx runtime",
        )
    };
...
async fn make_config(root: &TempDir, limit: usize, instructions: Option<&str>) -> TestConfig {
    ...
    let user_instructions = instructions.map(|text| UserInstructions {
        text: text.to_owned(),
        source: config.forestx_home.join(DEFAULT_AGENTS_MD_FILENAME),
    });
    TestConfig {
        config,
        user_instructions,
...
async fn project_root_markers_are_honored_for_agents_discovery() {
    ...
    fs::write(root.path().join(".forestx-root"), "").unwrap();
...
async fn project_layers_do_not_override_project_root_markers() {
    ...
    config.cwd = nested.abs();
    let project_layer = |dot_forestx_folder: AbsolutePathBuf, marker: &str| {
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![
            project_layer(root.path().join(".forestx").abs(), ".ignored-root-marker"),
            project_layer(config.cwd.join(".forestx"), ".ignored-nested-marker"),
        ],
...
async fn instruction_sources_include_global_before_agents_md_docs() {
    ...
    let global_agents = cfg.forestx_home.join(DEFAULT_AGENTS_MD_FILENAME);
    fs::create_dir_all(&cfg.forestx_home).unwrap();
...
fn create_skill(forestx_home: PathBuf, name: &str, description: &str) {
    let skill_dir = forestx_home.join(format!("skills/{name}"));

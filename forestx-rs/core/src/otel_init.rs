...
use forestx_config::types::OtelExporterKind as Kind;
use forestx_config::types::OtelHttpProtocol as Protocol;
...
use forestx_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = forestx_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = forestx_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);

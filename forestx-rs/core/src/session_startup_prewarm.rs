...
use forestx_otel::STARTUP_PREWARM_AGE_AT_FIRST_TURN_METRIC;
use forestx_otel::STARTUP_PREWARM_DURATION_METRIC;
...
pub(crate) struct SessionStartupPrewarmHandle {
    task: AbortOnDropHandle<ForestxResult<ModelClientSession>>,
    started_at: Instant,
    timeout: Duration,
}
...
impl SessionStartupPrewarmHandle {
    pub(crate) fn new(
        task: JoinHandle<ForestxResult<ModelClientSession>>,
        started_at: Instant,
        timeout: Duration,
    ) -> Self {
        Self {
    ...
    fn resolution_from_join_result(
        result: std::result::Result<ForestxResult<ModelClientSession>, tokio::task::JoinError>,
        started_at: Instant,
    ) -> SessionStartupPrewarmResolution {
        match result {
...
async fn schedule_startup_prewarm_inner(
...
) -> ForestxResult<ModelClientSession> {
    ...
    let responses_metadata = startup_turn_context
        ...
        .to_responses_metadata(
            ...
            window_id,
            ForestxResponsesRequestKind::Prewarm,
        );

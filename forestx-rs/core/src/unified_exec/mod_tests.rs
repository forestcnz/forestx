...
use forestx_exec_server::ExecProcess;
use forestx_exec_server::ExecProcessEventReceiver;
use forestx_exec_server::ExecProcessFuture;
use forestx_exec_server::ProcessId;
use forestx_exec_server::ProcessSignal;
use forestx_exec_server::ReadResponse;
use forestx_exec_server::StartedExecProcess;
use forestx_exec_server::WriteResponse;
use forestx_exec_server::WriteStatus;
...
use core_test_support::test_forestx::test_env as remote_test_env;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, forestx_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, forestx_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), forestx_exec_server::ExecServerError> {
...
async fn blocking_terminate_unified_process(
    ...
    Ok(Arc::new(
        UnifiedExecProcess::from_exec_server_started(StartedExecProcess {
            ...
            sandbox_type: Some(forestx_sandboxing::SandboxType::None),
        })
        .await?,
    ))
}
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export FORESTX_INTERACTIVE_SHELL_VAR=forestx\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $FORESTX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export FORESTX_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $FORESTX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = forestx_exec_server::Environment::default_for_tests();

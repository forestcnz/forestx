...
use forestx_protocol::protocol::TerminalInteractionEvent;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the Forestx sandbox network proxy.";
...
) -> forestx_exec_server::ExecEnvPolicy {
    let mut exclude = policy
        .exclude
        .iter()
        .map(std::string::ToString::to_string)
        .collect::<Vec<_>>();
    exclude.push(FORESTX_PERMISSION_PROFILE_ENV_VAR.to_string());
    let mut r#set = policy.r#set.clone();
    r#set.retain(|key, _| !key.eq_ignore_ascii_case(FORESTX_PERMISSION_PROFILE_ENV_VAR));
    forestx_exec_server::ExecEnvPolicy {
...
fn env_overlay_for_exec_server(
    ...
    request_env
        .iter()
        .filter(|(key, value)| {
            key.as_str() == FORESTX_PERMISSION_PROFILE_ENV_VAR
...
fn exec_server_env_for_request(
    ...
    Option<forestx_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    tty: bool,
) -> forestx_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::Forestx(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<forestx_network_proxy::RemoteNetworkProxyLaunchConfig>,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::Forestx(ForestxErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        let network_proxy_restricting_sid = {
            ...
            {
                if request.sandbox == forestx_sandboxing::SandboxType::WindowsRestrictedToken {
                    request
                        .network
                        .as_ref()
                        .map(|network| {
        ...
        let windows_sandbox =
            if request.sandbox == forestx_sandboxing::SandboxType::WindowsRestrictedToken {
                Some(forestx_sandboxing::WindowsSandboxSpawnRequest {
        ...
        let spawn_result = forestx_sandboxing::spawn_process(forestx_sandboxing::SpawnRequest {
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            FORESTX_THREAD_ID_ENV_VAR.to_string(),
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Forestx(err) => match err.details() {

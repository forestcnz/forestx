...
fn unified_exec_env_injects_defaults() {
    ...
    let expected = HashMap::from([
        ...
        ("FORESTX_CI".to_string(), "1".to_string()),
    ]);
...
fn env_overlay_for_exec_server_keeps_runtime_changes_only() {
    ...
    let request_env = HashMap::from([
        ...
        ("SHELL_SET".to_string(), "policy".to_string()),
        ("FORESTX_THREAD_ID".to_string(), "thread-1".to_string()),
        (
            FORESTX_PERMISSION_PROFILE_ENV_VAR.to_string(),
            "current-profile".to_string(),
        ),
        (
            "FORESTX_SANDBOX_NETWORK_DISABLED".to_string(),
            "1".to_string(),
        ),
    ]);
...
fn exec_env_policy_excludes_runtime_permission_profile() {
    let policy = ShellEnvironmentPolicy {
        r#set: HashMap::from([
            (
                "forestx_permission_profile".to_string(),
    ...
    assert_eq!(
        ...
        forestx_exec_server::ExecEnvPolicy {
            ...
            exclude: vec![FORESTX_PERMISSION_PROFILE_ENV_VAR.to_string()],
...
fn exec_server_params_use_path_uri_and_env_policy_overlay_contract() {
    let cwd: forestx_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let permission_profile = forestx_protocol::models::PermissionProfile::Disabled;
    ...
    let mut request = ExecRequest {
        ...
        env: HashMap::from([
            ...
            ("FORESTX_NETWORK_PROXY_ACTIVE".to_string(), "1".to_string()),
            (
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: forestx_exec_server::ExecEnvPolicy {
                inherit: forestx_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: forestx_sandboxing::SandboxType::None,
    ...
    let proxy_settings_mode = forestx_sandboxing::WindowsSandboxProxySettingsMode::Preserve;
    ...
    request.exec_server_sandbox = Some(
        forestx_exec_server::FileSystemSandboxContext::from_permission_profile(permission_profile),
    );
    ...
    assert_eq!(
        ...
        Some(forestx_sandboxing::WindowsSandboxProxySettingsMode::Preserve)
    );
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: forestx_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let forestx_protocol::protocol::EventMsg::ItemCompleted(completed_event) = event.msg else {
        panic!("expected ItemCompleted event");
    };
    let forestx_protocol::items::TurnItem::CommandExecution(item) = completed_event.item else {
        panic!("expected CommandExecution item");
    };
    assert_eq!(item.id, "call-unified-denied");
    assert_eq!(
        item.status,
        forestx_protocol::items::CommandExecutionStatus::Failed
    );
...
async fn pruning_does_not_evict_live_process_while_exited_process_is_finalizing() {
    let exited_process = Arc::new(
        crate::unified_exec::process_tests::remote_process(
            forestx_exec_server::WriteStatus::Accepted,
            /*terminate_error*/ None,
            forestx_sandboxing::SandboxType::None,
        )
        .await,
    );
    exited_process

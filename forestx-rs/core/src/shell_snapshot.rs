...
impl ShellSnapshot {
    ...
    async fn build_for_cwd(
        ...
        async {
            let timer = config
                .session_telemetry
                .start_timer("forestx.shell_snapshot.duration_ms", &[]);
            ...
            config
                .session_telemetry
                .counter("forestx.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_create(
        ...
        let temp_path = forestx_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.tmp-{nonce}"));

        // Clean the (unlikely) leaked snapshot files.
        let forestx_home = forestx_home.clone();
        let cleanup_session_id = session_id;
        tokio::spawn(async move {
            if let Err(err) =
                cleanup_stale_snapshots(&forestx_home, cleanup_session_id, state_db).await
            {
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path =
            find_thread_path_by_id_str(forestx_home, session_id, state_db.as_deref()).await?;

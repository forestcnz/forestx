...
use forestx_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(forestx_home: &Path) -> bool {
    forestx_windows_sandbox::sandbox_setup_is_complete(forestx_home)
}

#[cfg(not(target_os = "windows"))]
pub fn sandbox_setup_is_complete(_forestx_home: &Path) -> bool {
    false
}
...
pub fn run_elevated_setup(
    ...
    let permissions =
        forestx_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
            permission_profile,
            workspace_roots,
        )?;
    forestx_windows_sandbox::run_elevated_setup(
        forestx_windows_sandbox::SandboxSetupRequest {
        ...
        forestx_windows_sandbox::SetupRootOverrides::default(),
    )
}
...
) -> std::io::Result<forestx_windows_sandbox::WindowsSandboxProvisioningSettings> {
    let Some(network) = network.filter(|network| network.enabled()) else {
        return Ok(forestx_windows_sandbox::WindowsSandboxProvisioningSettings::default());
    };
    Ok(forestx_windows_sandbox::WindowsSandboxProvisioningSettings {
...
pub fn run_elevated_provisioning_setup(
    ...
    forestx_windows_sandbox::run_elevated_provisioning_setup(
        forestx_home,
        real_user,
...
pub fn run_legacy_setup_preflight(
    ...
    forestx_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        forestx_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    forestx_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
...
async fn run_windows_sandbox_setup_and_persist(
    ...
    let forestx_home = request.forestx_home;
    let setup_forestx_home = forestx_home.clone();

    let setup_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        match mode {
            WindowsSandboxSetupMode::Elevated => {
                if !sandbox_setup_is_complete(setup_forestx_home.as_path()) {
                    run_elevated_setup(
                        &permission_profile,
                        workspace_roots.as_slice(),
                        command_cwd.as_path(),
                        &env_map,
                        setup_forestx_home.as_path(),
                    )?;
                }
            }
    ...
    ConfigEditsBuilder::new(forestx_home.as_path())
...
fn emit_windows_sandbox_setup_success_metrics(
    ...
    let Some(metrics) = forestx_otel::global() else {
        return;
    };
    let mode_tag = windows_sandbox_setup_mode_tag(mode);
    let _ = metrics.record_duration(
        "forestx.windows_sandbox.setup_duration_ms",
        duration,
        &[
            ("result", "success"),
    ...
    let _ = metrics.counter(
        "forestx.windows_sandbox.setup_success",
        /*inc*/ 1,
...
fn emit_windows_sandbox_setup_failure_metrics(
    ...
    let _ = metrics.counter(
        "forestx.windows_sandbox.setup_failure",
        /*inc*/ 1,
        &[("originator", originator_tag), ("mode", mode_tag)],
    );
    ...
    if matches!(mode, WindowsSandboxSetupMode::Elevated) {
        #[cfg(target_os = "windows")]
        {
            ...
            if let Some(failure) = forestx_windows_sandbox::extract_setup_failure(_err) {
                code_tag = Some(failure.code.as_str().to_string());
                message_tag = Some(forestx_windows_sandbox::sanitize_setup_metric_tag_value(
                    &failure.message,
                ));
            }
            ...
            let metric_name =
                if forestx_windows_sandbox::extract_setup_failure(_err).is_some_and(|failure| {
                    matches!(
                        failure.code,
                        forestx_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
                    )
                }) {
                    "forestx.windows_sandbox.elevated_setup_canceled"
                } else {
                    "forestx.windows_sandbox.elevated_setup_failure"
                };
            let _ = metrics.counter(metric_name, /*inc*/ 1, &failure_tags);
        }
    } else {
        let _ = metrics.counter(
            "forestx.windows_sandbox.legacy_setup_preflight_failed",
            /*inc*/ 1,
            &[("originator", originator_tag)],
        );
    }

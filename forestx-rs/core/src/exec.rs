...
use forestx_protocol::error::SandboxErr;
...
use forestx_protocol::protocol::ExecCommandOutputDeltaEvent;
use forestx_protocol::protocol::ExecOutputStream;
use forestx_sandboxing::SandboxCommand;
use forestx_sandboxing::SandboxManager;
use forestx_sandboxing::SandboxTransformRequest;
use forestx_sandboxing::SandboxType;
use forestx_sandboxing::SandboxablePreference;
use forestx_sandboxing::WindowsSandboxFilesystemOverrides;
pub(crate) use forestx_sandboxing::is_likely_sandbox_denied;
#[cfg(test)]
use forestx_sandboxing::permission_profile_supports_windows_restricted_token_sandbox;
use forestx_sandboxing::record_filesystem_sandbox_violation;
use forestx_sandboxing::resolve_windows_elevated_filesystem_overrides;
use forestx_sandboxing::resolve_windows_restricted_token_filesystem_overrides;
#[cfg(test)]
use forestx_sandboxing::unsupported_windows_restricted_token_sandbox_reason;
use forestx_sandboxing::windows_sandbox_uses_elevated_backend;
...
use forestx_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: forestx_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: forestx_protocol::config_types::WindowsSandboxLevel,
...
fn network_proxy_environment_error(
    ...
    ForestxErr::Io(io::Error::other(format!(
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            forestx_linux_sandbox_exe: forestx_linux_sandbox_exe.as_deref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
    ...
    .map_err(ForestxErr::UnsupportedOperation)?;
    Ok(exec_req)
}
...
pub(crate) async fn execute_exec_request(
    ...
    let cwd = cwd
        .to_abs_path()
        .map_err(|err| ForestxErr::InvalidRequest(format!("invalid exec cwd: {err}")))?;
    ...
    let windows_sandbox_policy_cwd = windows_sandbox_policy_cwd
        .to_abs_path()
        .map_err(|err| ForestxErr::InvalidRequest(format!("invalid sandbox cwd: {err}")))?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        forestx_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = forestx_otel::global() {
        let _ = metrics.counter(
            "forestx.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_forestx_home;
    use forestx_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use forestx_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            forestx_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let forestx_home = find_forestx_home().map_err(|err| {
        ForestxErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve forestx_home: {err}"
        )))
    })?;
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                forestx_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
                    permission_profile: &permission_profile,
                    workspace_roots: workspace_roots.as_slice(),
                    forestx_home: forestx_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(ForestxErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, ForestxErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(ForestxErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(ForestxErr::Sandbox(SandboxErr::Timeout {
            ...
            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                ...
                return Err(ForestxErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
/// FORESTX_SANDBOX_NETWORK_DISABLED=1 is added to the environment of the spawned
/// process.
///
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        forestx_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                forestx_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }

...
/// Start an interactive sub-Forestx thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_forestx_thread_interactive(
    config: Config,
...
) -> Result<(Arc<Session>, SessionIo), ForestxErr> {
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_forestx_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                forestx_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                forestx_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                forestx_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            forestx_protocol::request_user_input::RequestUserInputAnswer {
...
) -> forestx_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = forestx_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(
                    approval_id,
                    forestx_protocol::protocol::ReviewDecision::Abort,
                )
                .await;
            forestx_protocol::protocol::ReviewDecision::Abort
        }
...
#[cfg(test)]
#[path = "forestx_delegate_tests.rs"]
mod tests;

...
use forestx_rollout_trace::CompactionCheckpointTracePayload;
...
async fn run_remote_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            ...
            attempt
                .track(
                    sess.as_ref(),
                    forestx_analytics::CompactionStatus::Interrupted,
                    Some(&error),
                    analytics_details,
                )
                .await;
    ...
    if result.is_ok() {
        ...
        if let PostCompactHookOutcome::Stopped = post_compact_outcome {
            attempt
                .track(sess.as_ref(), status, forestx_error, analytics_details)
                .await;
    ...
    match result {
        Ok(()) => Ok(()),
        Err(err) if matches!(err.details(), ForestxErrorDetails::TurnAborted) => Err(err),
        Err(err) => {
            sess.track_turn_forestx_error(turn_context, &err);
...
) -> ForestxResult<RemoteCompactionV2Output> {
...
mod tests {
    ...
    fn response_stream(events: Vec<ForestxResult<ResponseEvent>>) -> ResponseStream {

...
use forestx_analytics::CompactionStrategy;
use forestx_analytics::CompactionTrigger;
...
use forestx_utils_string::to_ascii_json_string;
use http::HeaderMap as ApiHeaderMap;
use http::HeaderValue;
use serde::Serialize;
use serde_json::Value;

use crate::client::X_FORESTX_INSTALLATION_ID_HEADER;
use crate::client::X_FORESTX_PARENT_THREAD_ID_HEADER;
use crate::client::X_FORESTX_TURN_METADATA_HEADER;
use crate::client::X_FORESTX_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    ...
    X_FORESTX_TURN_METADATA_HEADER,
...
pub(crate) enum ForestxResponsesRequestKind {
    Turn,
    Prewarm,
...
impl ForestxResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            ForestxResponsesRequestKind::Turn => ("turn", None),
            ForestxResponsesRequestKind::Prewarm => ("prewarm", None),
            ForestxResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            ForestxResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, ForestxResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of Forestx metadata sent to ResponsesAPI.
///
/// The full Forestx turn metadata blob is transported canonically as
/// `client_metadata["x-forestx-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct ForestxResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<ForestxResponsesRequestKind>,
...
impl ForestxResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_FORESTX_INSTALLATION_ID_HEADER.to_string(),
                self.installation_id.clone(),
            ),
            (SESSION_ID_KEY.to_string(), self.session_id.clone()),
            (THREAD_ID_KEY.to_string(), self.thread_id.clone()),
            (X_FORESTX_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_FORESTX_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_FORESTX_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_FORESTX_WINDOW_ID_HEADER, &self.window_id);
        // Direct x-forestx-turn-metadata is compatibility output. Keep the unbounded Code Mode
        // mapping in client_metadata only so HTTP and WebSocket headers remain bounded.
        if self.has_turn_metadata()
            && let Ok(turn_metadata_json) = to_ascii_json_string(&ForestxTurnMetadataPayload {
                code_mode_tool_names: None,
    ...
    fn turn_metadata_payload(&self) -> ForestxTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(ForestxResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(ForestxResponsesRequestKind::has_turn_identity);
        ForestxTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the Forestx turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved Forestx-owned keys are filtered when
...
#[derive(Serialize)]
struct ForestxTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]

...
use forestx_analytics::GuardianReviewDecision;
use forestx_analytics::GuardianReviewFailureReason;
use forestx_analytics::GuardianReviewTerminalStatus;
use forestx_analytics::GuardianReviewTrackContext;
use forestx_analytics::GuardianReviewedAction;
...
use forestx_protocol::protocol::GuardianAssessmentDecisionSource;
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: ForestxErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &forestx_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        forestx_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
...
pub(crate) fn spawn_approval_request_review(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("forestx-approval-review".to_string())
        .spawn(move || {
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset
                ...
                .iter()
                .any(|effort| effort.effort == forestx_protocol::openai_models::ReasoningEffort::Low),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| preset.effort == forestx_protocol::openai_models::ReasoningEffort::Low),
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    ForestxErrorInfo::ServerOverloaded
                        | ForestxErrorInfo::HttpConnectionFailed { .. }
                        | ForestxErrorInfo::ResponseStreamConnectionFailed { .. }
                        | ForestxErrorInfo::InternalServerError
                        | ForestxErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )

//! Bridges Apps SDK-style `openai/fileParams` metadata into Forestx's MCP flow.
//!
//! Strategy:
...
//!
//! The model-facing local-path schema is owned by `forestx-mcp` alongside MCP tool inventory, so this
//! module only handles uploading the files and rewriting the execution-time arguments.

use crate::session::session::Session;
use crate::session::step_context::StepContext;
use forestx_api::OPENAI_FILE_UPLOAD_LIMIT_BYTES;
use forestx_api::upload_openai_file;
...
async fn build_uploaded_argument_value(
    ...
    let Some(auth) = auth else {
        return Err("ChatGPT auth is required to upload files for Forestx Apps tools".to_string());
    };
    ...
    let upload_auth = forestx_model_provider::auth_provider_from_auth(auth);
...
mod tests {
    ...
    async fn openai_file_argument_rewrite_requires_declared_file_params() {
        ...
        let arguments = Some(serde_json::json!({
            "file": "/tmp/forestx-smoke-file.txt"
        }));

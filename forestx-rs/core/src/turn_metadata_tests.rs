...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: ForestxResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, ForestxResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        ForestxResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-forestx-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-forestx-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert!(json.get("x-forestx-installation-id").is_none());
    assert!(json.get("x-forestx-parent-thread-id").is_none());

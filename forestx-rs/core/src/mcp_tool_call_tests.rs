...
use forestx_config::types::AppConfig;
use forestx_config::types::AppToolConfig;
use forestx_config::types::AppToolsConfig;
use forestx_config::types::ApprovalsReviewer;
use forestx_config::types::AppsConfigToml;
...
fn approval_metadata(
    ...
    McpToolApprovalMetadata {
        ...
        forestx_apps_meta: None,
        openai_file_input_optional_fields: None,
    }
}

fn approval_config(turn_context: &TurnContext) -> forestx_mcp::McpConfig {
...
fn write_sample_plugin_mcp(forestx_home: &std::path::Path) {
    let plugin_root = forestx_home.join("plugins/cache/test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.forestx_home)
        .expect("create forestx home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.forestx_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = forestx_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.forestx_home,
        hook_list.hooks,
    );
...
#[test]
fn openai_file_params_are_only_honored_for_forestx_apps() {
    let params = HashMap::from([("file".to_string(), Vec::new())]);

    assert_eq!(
        openai_file_input_optional_fields_for_server(FORESTX_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        &turn_context,
        McpToolCallSpanFields {
            server_name: FORESTX_APPS_MCP_SERVER_NAME,
            tool_name: "calendar_search",
            call_id: "call-123",
            server_origin: Some("https://chatgpt.com/api/forestx/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("forestx.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("forestx.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("forestx.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("forestx.mcp.target.id=")
            && !invalid_logs.contains("forestx.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("forestx.mcp.target.id=")
            && !missing_logs.contains("forestx.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("forestx.mcp.target.id=")
            && !no_meta_logs.contains("forestx.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("forestx.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn forestx_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        FORESTX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_forestx_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        FORESTX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn forestx_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: FORESTX_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn forestx_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: FORESTX_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_FORESTX_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_FORESTX_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_forestx_apps_identity() {
    let mut metadata = approval_metadata(
    ...
    metadata.forestx_apps_meta = Some(
        serde_json::json!({
            "resource_uri": "/asdk_app_0123456789abcdef0123456789abcdef/link_fedcba9876543210fedcba9876543210/create_event",
        })
        .as_object()
        .cloned()
        .expect("_forestx_apps metadata should be an object"),
    );

    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(FORESTX_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
#[tokio::test]
async fn forestx_apps_tool_call_request_meta_includes_turn_metadata_and_forestx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_FORESTX_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_FORESTX_APPS_META_KEY: {
...
#[tokio::test]
async fn forestx_apps_tool_call_request_meta_includes_call_id_without_existing_forestx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
...
fn forestx_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn forestx_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
#[tokio::test]
async fn forestx_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let result = forestx_apps_auth_failure_result();
    let metadata = forestx_apps_auth_failure_metadata();

    let returned = maybe_request_forestx_apps_auth_elicitation(
        &session,
        &turn_context,
...
#[tokio::test]
async fn forestx_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn forestx_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
    let mutable_turn_context = Arc::get_mut(&mut turn_context).expect("single turn context ref");
...
#[tokio::test]
async fn forestx_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_forestx_apps_auth_elicitation(
                &session,
                &turn_context,
                turn_context.approval_policy(),
                "call_123",
    ...
    assert_eq!(request.server_name, FORESTX_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        forestx_protocol::mcp::RequestId::String("forestx_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        forestx_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            FORESTX_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("forestx_apps_auth_call_123".into()),
            ElicitationResponse {
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_forestx_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            FORESTX_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_forestx_app_tool_approval_writes_tool_override() {
    ...
    persist_forestx_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(forestx_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .forestx_home(forestx_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents = std::fs::read_to_string(forestx_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_forestx_dir = project_dir.path().join(".forestx");
    std::fs::create_dir_all(&project_forestx_dir).expect("create project .forestx dir");
    std::fs::write(
        project_forestx_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&forestx_home)
        .set_project_trust_level(
            project_dir.path(),
            forestx_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_forestx_dir.join(CONFIG_TOML_FILE))

...
use forestx_analytics::TurnProfile;
use forestx_otel::TURN_TTFM_DURATION_METRIC;
    ...
    match item {
        ...
        ResponseItem::Reasoning {
            ...
            summary.iter().any(|entry| match entry {
                forestx_protocol::models::ReasoningItemReasoningSummary::SummaryText { text } => {
                    !text.is_empty()
                }
            }) || content.as_ref().is_some_and(|entries| {
                entries.iter().any(|entry| match entry {
                    forestx_protocol::models::ReasoningItemContent::ReasoningText { text }
                    | forestx_protocol::models::ReasoningItemContent::Text { text } => {
                        !text.is_empty()
                    }
                })
            })

...
use forestx_execpolicy::AmendError;
use forestx_execpolicy::Decision;
use forestx_execpolicy::Error as ExecPolicyRuleError;
...
use forestx_execpolicy::blocking_append_allow_prefix_rule;
use forestx_execpolicy::blocking_append_network_rule;
...
use forestx_shell_command::is_dangerous_command::DangerousCommandMatch;
use forestx_shell_command::is_dangerous_command::dangerous_command_match;
use forestx_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            .config_layer_stack
            .layers_low_to_high()
            .filter_map(forestx_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: forestx_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(forestx_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &forestx_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            forestx_shell_command::is_dangerous_command::dangerous_powershell_words_match(command)
        }
    }
}
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            forestx_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
pub(crate) fn default_policy_path(forestx_home: &Path) -> PathBuf {
    forestx_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            forestx_shell_command::powershell::parse_powershell_command_into_plain_commands(command)
            && !commands.is_empty()
        {
            return ExecPolicyCommands {
                commands,
...
/// - Note: we only use this amendment when the command fails to run in sandbox and forestx prompts the user to run outside the sandbox

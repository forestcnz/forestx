...
pub(crate) struct SessionState {
    ...
    pub(crate) pending_session_start_sources: VecDeque<forestx_hooks::SessionStartSource>,
...
impl SessionState {
    ...
    pub(crate) fn queue_pending_session_start_source(
        &mut self,
        value: forestx_hooks::SessionStartSource,
    ) {
...
// missing values as the default `"forestx"` bucket.
fn merge_rate_limit_fields(
    previous: Option<&RateLimitSnapshot>,
    mut snapshot: RateLimitSnapshot,
) -> RateLimitSnapshot {
    if snapshot.limit_id.is_none() {
        snapshot.limit_id = Some("forestx".to_string());
    }

...
use forestx_mcp::ToolInfo as McpToolInfo;
...
pub(crate) fn append_mcp_tools(
    ...
    config: &Config,
    apps_enabled: bool,
    mcp_server_catalog: &forestx_mcp::ResolvedMcpCatalog,
    search_tool_enabled: bool,
    registry: &mut ToolRegistry,
) -> HashSet<ToolName> {
    // Keep regular MCP tools first; Apps tools also require connector and policy checks.
    let non_app_tools = filter_non_forestx_apps_mcp_tools_only(all_mcp_tools);
    let app_tools = apps_enabled
        .then(|| filter_forestx_apps_mcp_tools(all_mcp_tools, config))
        .into_iter()
        .flatten();
...
fn filter_non_forestx_apps_mcp_tools_only(
    mcp_tools: &[McpToolInfo],
) -> impl Iterator<Item = &McpToolInfo> + '_ {
    mcp_tools.iter().filter(|tool| {
        tool.server_name != FORESTX_APPS_MCP_SERVER_NAME && tool_is_model_visible(tool)
    })
}

fn filter_forestx_apps_mcp_tools<'a>(

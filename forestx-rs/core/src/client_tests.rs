...
use super::X_FORESTX_INSTALLATION_ID_HEADER;
use super::X_FORESTX_PARENT_THREAD_ID_HEADER;
use super::X_FORESTX_TURN_METADATA_HEADER;
use super::X_FORESTX_WINDOW_ID_HEADER;
use super::X_OPENAI_SUBAGENT_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use crate::responses_metadata::ForestxResponsesMetadata;
use crate::test_support::TestForestxResponsesRequestKind;
use crate::test_support::responses_metadata as test_responses_metadata;
use forestx_api::AgentIdentityTelemetry;
use forestx_api::ApiError;
use forestx_api::ResponseEvent;
...
use forestx_model_provider::BearerAuthProvider;
use forestx_model_provider::SharedModelProvider;
use forestx_model_provider::create_model_provider;
use forestx_model_provider_info::CHATGPT_FORESTX_BASE_URL;
...
use forestx_rollout_trace::ExecutionStatus;
...
use forestx_rollout_trace::RolloutTrace;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&forestx_home, server.uri()).await;
    ...
    let output = client
        .compact_conversation_history(
            ...
            CompactConversationRequestSettings {
                effort: None,
                summary: forestx_protocol::config_types::ReasoningSummary::None,
                service_tier: None,
            },
...
fn write_chatgpt_auth_json(forestx_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(forestx_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::ForestxTurnStarted {
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(forestx_protocol::ResponseItemId::with_suffix("msg", id)),
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    for (client_key, metadata_key, expected) in [
        ...
        (
            X_FORESTX_WINDOW_ID_HEADER,
            "window_id",
            expected_window_id.as_str(),
        ),
        (
            X_FORESTX_PARENT_THREAD_ID_HEADER,
            "parent_thread_id",
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart Forestx, url: {url}"
        )
    );
}
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(CHATGPT_FORESTX_BASE_URL.to_string())),
        )
    } else {
        (
            None,
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_forestx_responses() {
...
#[tokio::test]
async fn non_chatgpt_forestx_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =

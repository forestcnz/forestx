...
impl RolloutBudget {
    ...
    pub(crate) fn record_usage(&self, usage: &TokenUsage) -> ForestxResult<bool> {
        let Some(mut state) = self.lock() else {
            return Ok(false);
        };
        let units = if let Some(units) = usage.forestx_rollout_budget_units.as_ref() {
            let units = units.as_f64().unwrap_or(f64::NAN);
            if !units.is_finite() || units < 0.0 {
                return Err(ForestxErr::Fatal(
                    "response.completed usage.forestx_rollout_budget_units must be finite and non-negative"
                        .to_string(),
                ));
            }
            units

...
///
/// 1. The process was spawned by Forestx as part of a shell tool call.
/// 2. NetworkSandboxPolicy is restricted for the tool call.
///
/// We may try to have just one environment variable for all sandboxing
/// attributes, so this may change in the future.
pub const FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "FORESTX_SANDBOX_NETWORK_DISABLED";
...
pub const FORESTX_SANDBOX_ENV_VAR: &str = "FORESTX_SANDBOX";
...
/// `FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    // If this Forestx process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            if detach_from_tty {
                forestx_utils_pty::process_group::detach_from_tty()?;
            }
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                forestx_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }

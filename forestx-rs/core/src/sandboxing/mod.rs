...
command transformation live in the forestx-sandboxing crate; this module keeps
...
#[cfg(target_os = "macos")]
use crate::spawn::FORESTX_SANDBOX_ENV_VAR;
use crate::spawn::FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
pub use forestx_protocol::models::SandboxPermissions;
...
pub(crate) struct ExecServerEnvConfig {
    pub(crate) policy: forestx_exec_server::ExecEnvPolicy,
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(FORESTX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        }
        Self {
            command,
            cwd,
...
) -> forestx_protocol::error::Result<ExecToolCallOutput> {

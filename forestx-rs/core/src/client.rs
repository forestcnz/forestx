...
//!
//! `ModelClient` is intended to live for the lifetime of a Forestx session and holds the stable
...
//! per-turn state such as the `x-forestx-turn-state` token used for sticky routing.
//!
...
use forestx_api::AgentIdentityTelemetry;
use forestx_api::ApiError;
use forestx_api::AuthProvider;
use forestx_api::CompactClient as ApiCompactClient;
use forestx_api::CompactionInput as ApiCompactionInput;
use forestx_api::Compression;
use forestx_api::MemoriesClient as ApiMemoriesClient;
use forestx_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use forestx_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use forestx_api::Provider as ApiProvider;
use forestx_api::RawMemory as ApiRawMemory;
use forestx_api::RealtimeCallClient as ApiRealtimeCallClient;
use forestx_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use forestx_api::Reasoning;
use forestx_api::ReasoningContext;
use forestx_api::RequestTelemetry;
use forestx_api::ReqwestTransport;
use forestx_api::ResponseCreateWsRequest;
use forestx_api::ResponsesApiRequest;
use forestx_api::ResponsesClient as ApiResponsesClient;
use forestx_api::ResponsesOptions as ApiResponsesOptions;
use forestx_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use forestx_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use forestx_api::ResponsesWsRequest;
use forestx_api::SharedAuthProvider;
use forestx_api::SseTelemetry;
use forestx_api::StreamOptions;
use forestx_api::TransportError;
use forestx_api::WebsocketTelemetry;
use forestx_api::auth_header_telemetry;
use forestx_api::build_session_headers;
use forestx_api::create_text_param_for_request;
use forestx_api::response_create_client_metadata;
...
use forestx_login::RefreshTokenError;
use forestx_login::UnauthorizedRecovery;
use forestx_login::default_client::add_originator_header;
use forestx_login::default_client::create_client_for_route;
...
use forestx_protocol::config_types::Verbosity as VerbosityConfig;
...
use forestx_protocol::protocol::InternalSessionSource;
use forestx_protocol::protocol::SessionSource;
use forestx_protocol::protocol::W3cTraceContext;
use forestx_rollout_trace::CompactionTraceContext;
use forestx_rollout_trace::InferenceTraceAttempt;
use forestx_rollout_trace::InferenceTraceContext;
use forestx_tools::create_tools_json_for_responses_api;
use forestx_tools::create_tools_json_for_responses_lite;
use forestx_tools::create_tools_raw_json_for_responses_api;
...
use forestx_feedback::FeedbackRequestTags;
use forestx_feedback::emit_feedback_request_tags_with_auth_env;
use forestx_login::auth::AgentIdentityAuthPolicy;
use forestx_login::auth_env_telemetry::AuthEnvTelemetry;
use forestx_login::auth_env_telemetry::collect_auth_env_telemetry;
use forestx_model_provider::AgentIdentitySessionFallback;
use forestx_model_provider::ProviderAuthScope;
use forestx_model_provider::SharedModelProvider;
use forestx_model_provider::create_model_provider;
#[cfg(test)]
use forestx_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use forestx_protocol::error::Result;
use forestx_response_debug_context::extract_response_debug_context;
use forestx_response_debug_context::extract_response_debug_context_from_api_error;
use forestx_response_debug_context::telemetry_api_error_message;
use forestx_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_FORESTX_INSTALLATION_ID_HEADER: &str = "x-forestx-installation-id";
pub const X_FORESTX_TURN_STATE_HEADER: &str = "x-forestx-turn-state";
pub const X_FORESTX_TURN_METADATA_HEADER: &str = "x-forestx-turn-metadata";
pub const X_FORESTX_PARENT_THREAD_ID_HEADER: &str = "x-forestx-parent-thread-id";
pub const X_FORESTX_WINDOW_ID_HEADER: &str = "x-forestx-window-id";
...
const X_FORESTX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-forestx-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_forestx_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_FORESTX_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-forestx-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a Forestx session
...
/// - The `x-forestx-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Forestx turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-forestx-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-forestx-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Forestx session. Per-turn values
    ...
    pub fn new(
        ...
        let forestx_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.forestx_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), forestx_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &ForestxResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "forestx.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &ForestxResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(ForestxAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_FORESTX_INSTALLATION_ID_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: forestx_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: forestx_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &ForestxResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                forestx_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&ForestxAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(ForestxAuth::uses_forestx_backend)
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata.insert(X_FORESTX_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
            match self
    ...
    /// Permanently disables WebSockets for this Forestx session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest<'_>) {
    ...
    payload
        ...
        .insert(
            X_FORESTX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement Forestx-specific conventions:
///
/// - `x-forestx-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-forestx-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-forestx-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_FORESTX_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_FORESTX_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: forestx_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let forestx_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = forestx_api::ResponseStream {
        rx_event,
...
/// the mapped `ForestxErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: forestx_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a ForestxResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(ForestxErr::RefreshTokenFailed(failed))
            }

use crate::ForestxAppsToolsCache;
use crate::HostSkillsService;
use crate::agent::AgentControl;
use crate::attestation::AttestationProvider;
use crate::forestx_thread::ForestxThread;
...
use forestx_agent_graph_store::AgentGraphStore;
use forestx_agent_graph_store::LocalAgentGraphStore;
...
use forestx_code_mode::CodeModeSessionProvider;
use forestx_code_mode::DisabledCodeModeSessionProvider;
use forestx_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use forestx_extension_api::ExtensionRegistry;
...
use forestx_model_provider_info::OPENAI_PROVIDER_ID;
...
use forestx_thread_store::LoadThreadHistoryParams;
use forestx_thread_store::LocalThreadStore;
use forestx_thread_store::LocalThreadStoreConfig;
use forestx_thread_store::MoveThreadToSectionParams;
use forestx_thread_store::PreparedFork;
use forestx_thread_store::ReadThreadByRolloutPathParams;
use forestx_thread_store::ReadThreadParams;
use forestx_thread_store::StoredModelContext;
use forestx_thread_store::StoredThread;
use forestx_thread_store::ThreadMetadataPatch;
use forestx_thread_store::ThreadStore;
use forestx_thread_store::ThreadStoreError;
use forestx_thread_store::UpdateThreadMetadataParams;
...
struct TempForestxHomeGuard {
    path: PathBuf,
}

impl Drop for TempForestxHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created Forestx thread (formerly called a conversation), including the first event
...
pub struct ThreadManager {
    ...
    _test_forestx_home_guard: Option<TempForestxHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<forestx_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<ForestxThread>>>>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                forestx_rollout::spawn_rollout_compression_worker(config.forestx_home.to_path_buf());
            }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        forestx_apps_tools_cache: ForestxAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
    ...
    /// Construct with a dummy AuthManager containing the provided ForestxAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let forestx_home = std::env::temp_dir().join(format!(
            "forestx-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&forestx_home)
            .unwrap_or_else(|err| panic!("temp forestx home dir create failed: {err}"));
        ...
        manager._test_forestx_home_guard = Some(TempForestxHomeGuard { path: forestx_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided ForestxAuth and forestx home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let absolute_forestx_home = match AbsolutePathBuf::from_absolute_path_checked(&forestx_home) {
            Ok(forestx_home) => forestx_home,
            Err(err) => panic!("test forestx_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite: forestx_state::SqliteConfig::new_for_testing(absolute_forestx_home),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(forestx_home, /*config_model_catalog*/ None),
                environment_manager,
                starting_mcp_runtimes: std::sync::Mutex::new(Vec::new()),
                skills_service,
                plugins_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            if !environment_ids.insert(environment.environment_id.as_str()) {
                return Err(ForestxErr::InvalidRequest(format!(
            ...
            self.state
                ...
                .ok_or_else(|| {
                    ForestxErr::InvalidRequest(format!(
    ...
    pub async fn list_models(
        ...
        http_client_factory: forestx_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> ForestxResult<Arc<ForestxThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `ForestxThread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    ForestxErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    ForestxErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, options: StartThreadOptions) -> ForestxResult<NewThread> {
    ...
    /// as `Arc<ForestxThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<ForestxThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> ForestxResult<Arc<ForestxThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(ForestxErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => ForestxErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> ForestxResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<ForestxThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(FORESTX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn spawn_thread(&self, request: ThreadSpawnRequest) -> ForestxResult<NewThread> {
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(ForestxErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(ForestxThread::new(
                    session,
                    io,
        ...
        Err(ForestxErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> forestx_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return forestx_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(forestx_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> ForestxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => ForestxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => ForestxErr::InvalidRequest(message),
        err => ForestxErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> ForestxErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => ForestxErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => ForestxErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => ForestxErr::UnsupportedOperation(format!(

...
use forestx_git_utils::get_git_remote_urls_assume_git_repo;
use forestx_git_utils::get_git_repo_root;
use forestx_git_utils::get_has_changes;
use forestx_git_utils::get_head_commit_hash;
...
pub async fn detached_memory_responses_metadata(
    ...
    ForestxResponsesMetadata {
        request_kind: Some(ForestxResponsesRequestKind::Memory),
        subagent_header: subagent_header_value(session_source),
        sandbox: sandbox.map(ToString::to_string),
        workspaces: memory_workspaces(cwd).await,
        ..ForestxResponsesMetadata::new(installation_id, session_id, thread_id, window_id)
    }
}
...
impl TurnMetadataState {
    ...
    fn responses_metadata_template(&self) -> ForestxResponsesMetadata {
        ForestxResponsesMetadata {

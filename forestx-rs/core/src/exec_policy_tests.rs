...
use forestx_config::RequirementsExecPolicy;
...
fn config_stack_for_dot_forestx_folder(dot_forestx_folder: &Path) -> ConfigLayerStack {
    let dot_forestx_folder =
        AbsolutePathBuf::from_absolute_path(dot_forestx_folder).expect("absolute dot_forestx_folder");
...
async fn test_config() -> (TempDir, Config) {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .forestx_home(home.path().to_path_buf())
        .build()
        .await
...
async fn returns_empty_policy_when_no_policy_files_exist() {
    ...
    let config_stack = config_stack_for_dot_forestx_folder(temp_dir.path());
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        forestx_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(forestx_config::Sourced::new(
            forestx_config::RequirementsExecPolicy::new(requirements_exec_policy),
            forestx_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
    let dot_forestx_folder = AbsolutePathBuf::from_absolute_path(temp_dir.path())?;
    let layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project { dot_forestx_folder },
        TomlValue::Table(Default::default()),
    );
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(forestx_config::CONFIG_TOML_FILE))
...
async fn ignores_rules_from_untrusted_project_layers() -> anyhow::Result<()> {
    ...
    let project_dot_forestx_folder = AbsolutePathBuf::from_absolute_path(project_dir.path())?;
    let layers = vec![ConfigLayerEntry::new_disabled(
        ConfigLayerSource::Project {
            dot_forestx_folder: project_dot_forestx_folder,
        },
        TomlValue::Table(Default::default()),
        "marked untrusted",
    )];
...
async fn ignore_user_config_keeps_user_policy_files() -> std::io::Result<()> {
    let temp = tempdir()?;
    let forestx_home = temp.path().join("home_ignore_user_config");
    let rules_dir = forestx_home.join(RULES_DIR_NAME);
...
async fn request_rule_falls_back_when_prefix_rule_does_not_approve_all_commands() {
    let command = vec![
        "bash".to_string(),
        "-lc".to_string(),
        "cargo install cargo-insta && rm -rf /tmp/forestx".to_string(),
    ];
    let manager = ExecPolicyManager::default();
...
async fn append_execpolicy_amendment_updates_policy_and_file() {
    let forestx_home = tempdir().expect("create temp dir");
    let prefix = vec!["echo".to_string(), "hello".to_string()];
    let manager = ExecPolicyManager::default();

    manager
        .append_amendment_and_update(forestx_home.path(), &ExecPolicyAmendment::from(prefix))
        .await
    ...
    let contents = fs::read_to_string(default_policy_path(forestx_home.path()))
...
async fn append_execpolicy_amendment_rejects_empty_prefix() {
    ...
    let result = manager
        .append_amendment_and_update(forestx_home.path(), &ExecPolicyAmendment::from(vec![]))
        .await;
...
async fn exec_policies_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let forestx_home = temp.path().join("home_execpolicy_nested");
    let project_root = temp.path().join("project_execpolicy_nested");
    let nested = project_root.join("nested");
    let root_rules = project_root.join(".forestx").join(RULES_DIR_NAME);
    let nested_rules = nested.join(".forestx").join(RULES_DIR_NAME);

    fs::create_dir_all(&forestx_home)?;
...
async fn exec_policies_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let nested = project_root.join("nested");
    let rules_dir = project_root.join(".forestx").join(RULES_DIR_NAME);
    ...
    for (name, trust_entries, expected_decision) in cases {
        let forestx_home = temp.path().join(format!("home_execpolicy_{name}"));
        fs::create_dir_all(&forestx_home)?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_entries, expect_warning) in cases {
        let forestx_home = temp.path().join(format!("home_execpolicy_warning_{name}"));
        fs::create_dir_all(&forestx_home)?;
        write_project_trust_config(&forestx_home, &trust_entries).await?;

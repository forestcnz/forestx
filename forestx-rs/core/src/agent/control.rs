...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> ForestxThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> ForestxResult<String> {
        let state = self.upgrade()?;
        self.handle_thread_request_result(
            agent_id,
            &state,
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: ForestxResult<String>,
    ) -> ForestxResult<String> {
        if result
            .as_ref()
            .is_err_and(|err| matches!(err.details(), ForestxErrorDetails::InternalAgentDied))
        {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> ForestxResult<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or_else(|| ForestxErr::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(ForestxErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> ForestxResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> ForestxResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> ForestxResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> ForestxResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> ForestxResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::ForestxThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                forestx_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {

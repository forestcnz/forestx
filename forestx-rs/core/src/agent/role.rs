...
use forestx_config::loader::resolve_relative_paths_in_config_toml;
...
async fn load_role_layer_toml(
    ...
    let (role_config_toml, role_config_base) = if is_built_in {
        ...
        (role_config_toml, config.forestx_home.as_path())
    } else {
...
mod reload {
    ...
    fn reload_overrides(
        ...
        ConfigOverrides {
            ...
            psp: Some(config.psp),
            forestx_linux_sandbox_exe: config.forestx_linux_sandbox_exe.clone(),

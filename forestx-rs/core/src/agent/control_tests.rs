...
use forestx_protocol::protocol::ThreadSettingsAppliedEvent;
use forestx_protocol::protocol::ThreadSettingsSnapshot;
...
use forestx_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(forestx_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<ForestxThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<ForestxThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &ForestxThread) -> String {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<ForestxThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<ForestxThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(err) => match err.details() {
            ForestxErrorDetails::ThreadNotFound(id) => assert_eq!(*id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(
        err.details(),
        ForestxErrorDetails::ThreadNotFound(id) if *id == thread_id
    );
}
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(err) => match err.details() {
            ForestxErrorDetails::ThreadNotFound(id) => assert_eq!(*id, spawned_agent.thread_id),
            _ => panic!("expected ThreadNotFound, got {err:?}"),
        },
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        ForestxAuth::from_api_key("dummy"),
        harness.config.model_provider.clone(),
        harness.config.forestx_home.to_path_buf(),
        std::sync::Arc::new(forestx_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = forestx_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
...
async fn spawn_agent_respects_legacy_max_threads_alias() {
    ...
    let ForestxErrorDetails::AgentLimitReached {
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let ForestxErrorDetails::AgentLimitReached { max_threads } = err.details() else {
        panic!("expected AgentLimitReached");
    };
...
async fn spawn_thread_subagents_persist_parent_originator_across_new_and_truncated_fork() {
    ...
    assert_eq!(parent_originator, "forestx_work_desktop");

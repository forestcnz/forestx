...
/// This structure is used to add some limits on the multi-agent capabilities for Forestx. In
...
impl AgentRegistry {
    pub(crate) fn reserve_spawn_slot(
        ...
        if let Some(max_threads) = max_threads {
            if !self.try_increment_spawned(max_threads) {
                return Err(ForestxErr::new(ForestxErrorDetails::AgentLimitReached {
                    max_threads,
                }));
            }
        } else {
            ...
            if let Some(name) = available_names.choose(&mut rand::rng()) {
                ...
                if let Some(metrics) = forestx_otel::global() {
                    let _ = metrics.counter(
                        "forestx.multi_agent.nickname_pool_reset",
                        /*inc*/ 1,
                        &[],
                    );
                }
    ...
    fn reserve_agent_path(&self, agent_path: &AgentPath) -> Result<()> {
        ...
        match active_agents.agent_tree.entry(agent_path.to_string()) {
            Entry::Occupied(_) => Err(ForestxErr::UnsupportedOperation(format!(
...
impl SpawnReservation {
    pub(crate) fn reserve_agent_nickname_with_preference(
        ...
        let agent_nickname = self
            ...
            .ok_or_else(|| {
                ForestxErr::UnsupportedOperation("no available agent nicknames".to_string())
            })?;

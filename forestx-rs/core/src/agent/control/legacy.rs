...
impl AgentControl {
    ...
    pub(crate) async fn shutdown_live_agent(&self, agent_id: ThreadId) -> ForestxResult<String> {
    ...
    pub(crate) async fn close_agent(&self, agent_id: ThreadId) -> ForestxResult<String> {
        let state = self.upgrade()?;
        ...
        match state.get_thread(agent_id).await {
                ...
                if !thread.config_snapshot().await.ephemeral
                    ...
                    && let Err(err) = agent_graph_store
                        .set_thread_spawn_edge_status(
                            agent_id,
                            forestx_agent_graph_store::ThreadSpawnEdgeStatus::Closed,
                        )
                        .await
                {
            ...
            Err(err)
                if known_agent && matches!(err.details(), ForestxErrorDetails::ThreadNotFound(_)) =>
            {
            ...
            Err(err) if matches!(err.details(), ForestxErrorDetails::ThreadNotFound(_)) => {}
            Err(err) => {
        ...
        match Box::pin(self.shutdown_agent_tree(agent_id)).await {
            Err(err)
                if known_agent
                    && matches!(
                        err.details(),
                        ForestxErrorDetails::ThreadNotFound(_) | ForestxErrorDetails::InternalAgentDied
                    ) =>
            {
                Ok(String::new())
            }
    ...
    pub(crate) async fn shutdown_agent_tree(&self, agent_id: ThreadId) -> ForestxResult<String> {

...
) -> ForestxResult<Option<Vec<RolloutItem>>> {
    match history_mode {
...
impl AgentControl {
    ...
    pub(crate) async fn restore_v2_agent_metadata(
        ...
        let descendant_ids = match agent_graph_store
            .list_thread_spawn_descendants(
                root_thread_id,
                Some(forestx_agent_graph_store::ThreadSpawnEdgeStatus::Open),
            )
            .await
        {
        ...
        for thread_id in descendant_ids {
            ...
            let restore_result = async {
                ...
                let stored_agent_path = stored_thread
                    ...
                    .transpose()
                    .map_err(|err| {
                        ForestxErr::InvalidRequest(format!("invalid stored agent path: {err}"))
                    })?;
    ...
    pub(crate) async fn ensure_v2_agent_loaded(
        ...
        if self.state.agent_metadata_for_thread(thread_id).is_none() {
            return Err(ForestxErr::ThreadNotFound(thread_id));
        }
        ...
        let history = load_agent_model_context(&state, thread_id, stored_thread.history_mode)
            .await?
            .ok_or(ForestxErr::ThreadNotFound(thread_id))?;
        ...
        if let Some(role_name) = session_source.get_agent_role() {
            ...
            apply_role_to_config_for_multi_agent_v2(&mut config, Some(&role_name))
                .await
                .map_err(ForestxErr::InvalidRequest)?;
            config
                .permissions
                .approval_policy
                .set(runtime_approval_policy)
                .map_err(|err| {
                    ForestxErr::InvalidRequest(format!("approval_policy is invalid: {err}"))
                })?;
            ...
            config
                ...
                .map_err(|err| {
                    ForestxErr::InvalidRequest(format!("permission_profile is invalid: {err}"))
                })?;
        }
    ...
    async fn resume_single_agent_from_rollout(
        ...
        thread_id: ThreadId,
        session_source: SessionSource,
    ) -> ForestxResult<(ThreadId, MultiAgentVersion)> {
        let state = self.upgrade()?;
        let stored_thread = state
            .read_stored_thread(ReadThreadParams {
                thread_id,
        ...
        let resumed_agent_path = stored_thread
            ...
            .map(AgentPath::try_from)
            .transpose()
            .map_err(|err| ForestxErr::InvalidRequest(format!("invalid stored agent path: {err}")))?;

...
async fn residency_slot_reservation_unloads_oldest_idle_v2_agent() {
    ...
    config.forestx_home = temp_home.path().to_path_buf().try_into().unwrap();
    ...
    match manager.get_thread(first.thread_id).await {
        Err(err) => match err.details() {
            ForestxErrorDetails::ThreadNotFound(thread_id) => assert_eq!(*thread_id, first.thread_id),
...
async fn mark_thread_completed(thread: &ForestxThread) {
...
async fn mark_thread_interrupted(thread: &ForestxThread) {
    let turn = thread.session.new_default_turn().await;
    thread
        .session
        .send_event(
...
async fn clear_active_turn(thread: &ForestxThread) {

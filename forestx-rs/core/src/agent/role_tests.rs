...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.forestx_linux_sandbox_exe = Some(PathBuf::from("/tmp/forestx-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/forestx-execve-wrapper"));
    config.psp = true;
    let role_path = write_role_config(
        &home,
    ...
    assert_eq!(
        config.forestx_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/forestx-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/forestx-execve-wrapper"))
    );
    assert!(config.psp);
}
...
"#,
    ...
    let snapshot = skills_service
        .snapshot_for_config(
            &skills_input,
            Some(Arc::clone(&forestx_exec_server::LOCAL_FS)),
        )
        .await;

...
use forestx_rollout_trace::ToolCallRequester;
...
impl forestx_code_mode::CodeModeSessionProvider for MissingCellCodeModeSessionProvider {
    fn create_session<'a>(
        &'a self,
        _delegate: Arc<dyn forestx_code_mode::CodeModeSessionDelegate>,
    ) -> forestx_code_mode::CodeModeSessionProviderFuture<'a> {
        Box::pin(async {
            Ok(Arc::new(MissingCellCodeModeSession) as Arc<dyn forestx_code_mode::CodeModeSession>)
        })
    }
}
...
impl forestx_code_mode::CodeModeSession for MissingCellCodeModeSession {
    fn execute<'a>(
        &'a self,
        _request: forestx_code_mode::ExecuteRequest,
    ) -> forestx_code_mode::CodeModeSessionResultFuture<'a, forestx_code_mode::StartedCell> {
        Box::pin(async { Err("test session cannot execute cells".to_string()) })
    }

    fn wait<'a>(
        &'a self,
        request: forestx_code_mode::WaitRequest,
    ) -> forestx_code_mode::CodeModeSessionResultFuture<'a, forestx_code_mode::WaitOutcome> {
        self.terminate(request.cell_id)
    }

    fn terminate<'a>(
        &'a self,
        cell_id: forestx_code_mode::CellId,
    ) -> forestx_code_mode::CodeModeSessionResultFuture<'a, forestx_code_mode::WaitOutcome> {
        Box::pin(async move {
            Ok(forestx_code_mode::WaitOutcome::MissingCell(
                forestx_code_mode::RuntimeResponse::Result {
    ...
    fn shutdown<'a>(&'a self) -> forestx_code_mode::CodeModeSessionResultFuture<'a, ()> {
        Box::pin(async { Ok(()) })
    }
}
...
async fn dispatch_lifecycle_trace_records_direct_and_code_mode_requesters() -> anyhow::Result<()> {
    ...
    let registry = ToolRegistry::with_handler_for_test(Arc::new(TestHandler {
        tool_name: forestx_tools::ToolName::plain("test_tool"),
    }));
    ...
    let replayed = forestx_rollout_trace::replay_bundle(single_bundle_dir(temp.path())?)?;
    assert_eq!(
    ...
    let result = registry
        .dispatch_any_with_terminal_outcome(
            test_invocation_with_payload(
                ...
                turn,
                "incompatible-call",
                forestx_tools::ToolName::plain("test_tool"),
                ToolCallSource::Direct,
                ToolPayload::Custom {
                    input: "{}".to_string(),
                },
...
fn test_invocation(
    ...
    test_invocation_with_payload(
        ...
        turn,
        call_id,
        forestx_tools::ToolName::plain(tool_name),
        source,
...
fn attach_test_trace(session: &mut Session, turn: &TurnContext, root: &Path) -> anyhow::Result<()> {
    ...
    let rollout_thread_trace =
        forestx_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_forestx_turn_started(turn.sub_id.as_str());

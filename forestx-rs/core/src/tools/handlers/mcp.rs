...
use forestx_tools::ToolSearchSourceInfo;
use forestx_tools::ToolSpec;
use forestx_tools::agent_plugin_mcp_tool_to_responses_api_tool;
use forestx_tools::mcp_tool_to_responses_api_tool;
...
mod tests {
    ...
    async fn mcp_pre_tool_use_payload_uses_prefixed_tool_name_and_raw_args() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: forestx_tools::ToolName::namespaced("memory", "create_entities"),
    ...
    async fn mcp_pre_tool_use_payload_keeps_builtin_like_tool_names_namespaced() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: forestx_tools::ToolName::namespaced("mcp__foo", "exec_command"),
                source: ToolCallSource::Direct,
                payload,
            }),
            Some(PreToolUsePayload {
    ...
    async fn mcp_post_tool_use_payload_uses_prefixed_tool_name_args_and_result() {
        ...
        let output = McpToolOutput {
            result: forestx_protocol::mcp::CallToolResult {
                content: vec![json!({
                    "type": "text",
                    "text": "notes"
                })],
            ...
            truncation_policy: forestx_utils_output_truncation::TruncationPolicy::Bytes(1024),
        };
        let (session, turn) = make_session_and_context().await;
        let turn = Arc::new(turn);
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: forestx_tools::ToolName::namespaced("filesystem", "read_file"),
            source: ToolCallSource::Direct,
            payload,
        };
        assert_eq!(

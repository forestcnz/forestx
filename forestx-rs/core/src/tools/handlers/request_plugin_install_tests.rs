...
use forestx_core_plugins::startup_sync::curated_plugins_repo_path;
...
async fn verified_plugin_install_completed_requires_installed_plugin() {
    ...
    write_curated_plugin_sha(forestx_home.path());
    write_plugins_feature_config(forestx_home.path());

    let config = load_plugins_config(forestx_home.path()).await;
    ...
    let refreshed_config = load_plugins_config(forestx_home.path()).await;
    assert!(verified_plugin_install_completed(
        "sample@openai-curated",
        &refreshed_config,
        &plugins_manager,
...
async fn persist_disabled_install_request_writes_connector_config() {
    ...
    persist_disabled_install_request(&forestx_home.path().abs(), &tool)
        .await
        .expect("persist connector disable");

    let contents =
        std::fs::read_to_string(forestx_home.path().join(CONFIG_TOML_FILE)).expect("read config");

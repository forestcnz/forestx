...
pub(crate) fn collab_spawn_error(err: ForestxErr) -> FunctionCallError {
    match err.details() {
        ForestxErrorDetails::UnsupportedOperation(message) if message == "thread manager dropped" => {
...
pub(crate) fn collab_agent_error(agent_id: ThreadId, err: ForestxErr) -> FunctionCallError {
    match err.details() {
        ForestxErrorDetails::ThreadNotFound(id) => {

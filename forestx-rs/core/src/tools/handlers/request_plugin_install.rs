...
use forestx_analytics::PluginInstallRequestSource;
use forestx_analytics::PluginInstallRequested;
use forestx_analytics::PluginInstallRequestedPlugin;
...
use forestx_tools::DiscoverableToolAction;
use forestx_tools::DiscoverableToolType;
use forestx_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use forestx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use forestx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use forestx_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use forestx_tools::RequestPluginInstallArgs;
use forestx_tools::RequestPluginInstallResult;
use forestx_tools::ToolName;
use forestx_tools::ToolSpec;
use forestx_tools::all_requested_connectors_picked_up;
use forestx_tools::build_request_plugin_install_elicitation_request;
use forestx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use forestx_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("forestx-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in forestx-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.forestx_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    forestx_home: &forestx_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    tool: &DiscoverableTool,
    auth: Option<&forestx_login::ForestxAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match session.hard_refresh_latest_forestx_apps_tools().await {
        Ok(mcp_tools) => {
        ...
        Err(err) => {
            warn!(
                "failed to refresh forestx apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }

...
use forestx_tools::ConversationHistory;
use forestx_tools::ExtensionTurnItem;
use forestx_tools::ResponsesApiNamespaceTool;
use forestx_tools::ToolCall as ExtensionToolCall;
use forestx_tools::ToolEnvironment;
use forestx_tools::ToolName;
use forestx_tools::ToolSearchInfo;
use forestx_tools::ToolSpec;
use forestx_tools::TurnItemEmissionFuture;
use forestx_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn forestx_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn forestx_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let forestx_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use forestx_extension_items::web_search::WebSearchItem;
    use forestx_protocol::items::TurnItem;
    ...
    impl forestx_extension_api::ToolExecutor<forestx_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> forestx_tools::ToolName {
            forestx_tools::ToolName::plain("extension_echo")
        }
        ...
        fn spec(&self) -> forestx_tools::ToolSpec {
            forestx_tools::ToolSpec::Function(forestx_tools::ResponsesApiTool {
                ...
                strict: true,
                parameters: forestx_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(&self, _call: forestx_tools::ToolCall) -> forestx_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(forestx_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn forestx_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<forestx_tools::ToolCall>>>,
    }

    impl forestx_extension_api::ToolExecutor<forestx_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> forestx_tools::ToolName {
            forestx_tools::ToolName::plain("extension_echo")
        }
        ...
        fn handle(&self, call: forestx_tools::ToolCall) -> forestx_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: forestx_tools::ToolCall,
        ) -> Result<Box<dyn forestx_tools::ToolOutput>, forestx_tools::FunctionCallError> {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: forestx_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = forestx_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        forestx_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        forestx_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {

...
use forestx_protocol::protocol::FileSystemAccessMode;
use forestx_protocol::protocol::FileSystemPath;
use forestx_protocol::protocol::FileSystemSandboxEntry;
use forestx_protocol::protocol::FileSystemSandboxPolicy;
use forestx_protocol::protocol::InitialHistory;
use forestx_protocol::protocol::InterAgentCommunication;
use forestx_protocol::protocol::NetworkSandboxPolicy;
...
async fn install_role_with_model_override(turn: &mut TurnContext) -> String {
    ...
    tokio::fs::create_dir_all(&turn.config.forestx_home)
        .await
        .expect("forestx home should be created");
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    forestx_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: forestx_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
async fn spawn_agent_role_service_tier_falls_back_to_supported_parent_tier() {
    ...
    tokio::fs::create_dir_all(&turn.config.forestx_home)
        .await
        .expect("forestx home should be created");
    let role_config_path = turn.config.forestx_home.as_path().join("tiered-role.toml");
    tokio::fs::write(
        &role_config_path,
        r#"model = "gpt-5.4"
service_tier = "turbo"
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(ForestxAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    config.forestx_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    turn.config = Arc::new(config);

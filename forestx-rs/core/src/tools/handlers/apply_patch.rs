...
use forestx_apply_patch::ApplyPatchAction;
use forestx_apply_patch::ApplyPatchFileChange;
use forestx_apply_patch::Hunk;
use forestx_apply_patch::StreamingPatchParser;
...
use forestx_protocol::protocol::PatchApplyUpdatedEvent;
use forestx_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use forestx_sandboxing::policy_transforms::merge_permission_profiles;
use forestx_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn format_update_chunks_for_progress(chunks: &[forestx_apply_patch::UpdateFileChunk]) -> String {
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &forestx_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &AbsolutePathBuf,
...
async fn effective_patch_permissions(
    ...
    forestx_protocol::permissions::FileSystemSandboxPolicy,
)> {
...
fn patch_permissions_without_path_matching(
    ...
    (
        ...
        forestx_protocol::permissions::FileSystemSandboxPolicy::unrestricted(),
    )
}
...
impl ApplyPatchHandler {
    async fn handle_call(
        ...
        let args = match forestx_apply_patch::parse_patch(&patch_input) {
            Ok(args) => args,
            Err(parse_error) => {
        ...
        match forestx_apply_patch::verify_apply_patch_args(
            args,
        ...
        .await
        {
            forestx_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            forestx_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            forestx_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            forestx_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match forestx_apply_patch::maybe_parse_apply_patch_verified(command, cwd, fs, Some(&sandbox))
        .await
    {
        forestx_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            let tool_ctx = ToolCtx {
        ...
        forestx_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}

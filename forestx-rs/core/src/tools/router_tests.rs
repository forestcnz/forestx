...
use forestx_extension_api::ResponsesApiTool;
use forestx_extension_api::ToolCall as ExtensionToolCall;
...
impl forestx_extension_api::ToolContributor for ExtensionEchoContributor {
    fn tools(
        &self,
...
impl ToolExecutor<ExtensionToolCall> for ExtensionEchoExecutor {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: true,
                parameters: forestx_extension_api::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
    ...
    fn handle(&self, call: ExtensionToolCall) -> forestx_tools::ToolExecutorFuture<'_> {
...
impl ExtensionEchoExecutor {
    async fn handle_call(
        ...
        Ok(Box::new(forestx_tools::JsonToolOutput::new(json!({
            "arguments": arguments,
            "callId": call.call_id,
...
async fn build_tool_call_uses_namespace_for_registry_name() -> anyhow::Result<()> {
    ...
    let call = ToolRouter::build_tool_call(ResponseItem::FunctionCall {
        ...
        namespace: Some("mcp__forestx_apps__calendar".to_string()),
        arguments: "{}".to_string(),
    ...
    assert_eq!(
        call.tool_name,
        ToolName::namespaced("mcp__forestx_apps__calendar", tool_name)
    );
...
async fn specs_filter_deferred_dynamic_tools() -> anyhow::Result<()> {
    ...
    assert_eq!(
        namespace_function_names(&router.model_visible_specs(), "forestx_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        router.deferred_tool_namespaces(),
        BTreeMap::from([("forestx_app".to_string(), "Forestx app tools.".to_string())])
    );
...
fn mcp_runtime(tool_info: forestx_mcp::ToolInfo) -> RegisteredTool {
...
async fn extension_tool_executors_are_model_visible_and_dispatchable() -> anyhow::Result<()> {
    ...
    let router = test_tool_router(
        ...
        extension_tool_executors(
            &session,
            &forestx_extension_api::ExtensionData::new(turn.sub_id.clone()),
        ),
        &turn.dynamic_tools,
    );

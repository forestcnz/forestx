...
use forestx_code_mode::CodeModeSession;
...
use forestx_utils_audio::estimate_audio_token_count;
use forestx_utils_output_truncation::TruncationPolicy;
use forestx_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use forestx_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = forestx_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = forestx_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = forestx_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) fn take_unavailable_warning(&self, tool_mode: ToolMode) -> Option<String> {
        ...
        .then(|| {
            format!(
                "Code Mode is unavailable because {error}. {behavior}; enable `features.code_mode_host` and install `forestx-code-mode-host`."
            )
        })
    }
    ...
    pub(crate) async fn execute(
        &self,
        mut request: forestx_code_mode::ExecuteRequest,
    ) -> Result<forestx_code_mode::StartedCell, String> {
    ...
    ) -> Result<forestx_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &forestx_code_mode::CellId) {
...
async fn call_nested_tool(
    ...
    exec.session
        ...
        .track_code_mode_tool_call(forestx_analytics::CodeModeToolCallFact::ChildStarted {

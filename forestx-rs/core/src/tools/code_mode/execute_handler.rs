...
impl CodeModeExecuteHandler {
    ...
    async fn execute(
        ...
        let args =
            forestx_code_mode::parse_exec_source(&code).map_err(FunctionCallError::RespondToModel)?;
        let exec = ExecContext { session, turn };
        let enabled_tools =
            forestx_tools::collect_code_mode_tool_definitions(&self.nested_tool_specs);
        let started_at = std::time::Instant::now();
        let started_cell = exec
            .session
            .services
            .code_mode_service
            .execute(forestx_code_mode::ExecuteRequest {
        ...
        exec.session
            ...
            .track_code_mode_tool_call(forestx_analytics::CodeModeToolCallFact::CellStarted {
        ...
        if !matches!(response, forestx_code_mode::RuntimeResponse::Yielded { .. }) {
            ...
            exec.session
                .services
                .analytics_events_client
                .track_code_mode_tool_call(forestx_analytics::CodeModeToolCallFact::CellClosed {
                    thread_id: exec.session.thread_id.to_string(),
                    turn_id: exec.turn.sub_id.clone(),
                    cell_id: cell_id.to_string(),
                });

    ...
    async fn handle_call(
        ...
        let result = match payload {
                    ...
                    && tool_name.name.as_str() == WAIT_TOOL_NAME =>
            {
                ...
                let cell_id = forestx_code_mode::CellId::new(args.cell_id);
                let wait_response = if args.terminate {
                    ...
                    exec.session
                        .services
                        .code_mode_service
                        .wait(forestx_code_mode::WaitRequest {
                            cell_id,
                ...
                if let forestx_code_mode::WaitOutcome::LiveCell(response) = &wait_response {
                    let runtime_cell_id = match response {
                        forestx_code_mode::RuntimeResponse::Yielded { cell_id, .. }
                        | forestx_code_mode::RuntimeResponse::Terminated { cell_id, .. }
                        | forestx_code_mode::RuntimeResponse::Result { cell_id, .. } => cell_id,
                    };
                    ...
                    if !matches!(response, forestx_code_mode::RuntimeResponse::Yielded { .. }) {
                        exec.session
                            .services
                        ...
                        exec.session
                            ...
                            .track_code_mode_tool_call(
                                forestx_analytics::CodeModeToolCallFact::CellClosed {
        ...
        telemetry.finish(
            result
                .as_ref()
                .is_ok_and(forestx_tools::ToolOutput::success_for_logging),
        );
        result
    }
}

use forestx_code_mode::ImageDetail as CodeModeImageDetail;
...
pub(super) fn into_function_call_output_content_items(
    items: Vec<forestx_code_mode::FunctionCallOutputContentItem>,
...
impl IntoProtocol<FunctionCallOutputContentItem>
    for forestx_code_mode::FunctionCallOutputContentItem
{
    fn into_protocol(self) -> FunctionCallOutputContentItem {
        let value = self;
        match value {
            forestx_code_mode::FunctionCallOutputContentItem::InputText { text } => {
                FunctionCallOutputContentItem::InputText { text }
            }
            forestx_code_mode::FunctionCallOutputContentItem::InputImage { image_url, detail } => {
            ...
            forestx_code_mode::FunctionCallOutputContentItem::InputAudio { audio_url } => {

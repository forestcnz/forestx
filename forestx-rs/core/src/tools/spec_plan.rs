...
use forestx_tools::TOOL_SEARCH_TOOL_NAME;
use forestx_tools::ToolCall as ExtensionToolCall;
use forestx_tools::ToolEnvironmentMode;
use forestx_tools::ToolExecutor;
use forestx_tools::ToolExposures;
use forestx_tools::ToolName;
use forestx_tools::ToolSearchInfo;
use forestx_tools::ToolSpec;
use forestx_tools::UnifiedExecShellMode;
use forestx_tools::can_request_original_image_detail;
use forestx_tools::collect_code_mode_exec_prompt_tool_definitions;
use forestx_tools::collect_request_plugin_install_entries;
use forestx_tools::default_namespace_description;
use forestx_tools::request_user_input_available_modes;
use forestx_tools::shell_command_backend_for_features;
use forestx_tools::shell_type_for_model_and_features;
...
pub(crate) fn finalize_tool_router(
    ...
    if code_mode_enabled {
        for tool_name in [
            ToolName::plain(forestx_code_mode::PUBLIC_TOOL_NAME),
            ToolName::plain(forestx_code_mode::WAIT_TOOL_NAME),
        ] {
        ...
        && let Some(tool_name) = registry.first_collision()
    {
        ...
        return Err(ForestxErrorDetails::ToolCollision(name).into());
    }
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && forestx_code_mode::is_code_mode_nested_tool(spec.name())
        && code_mode_tool_names
            .get(&forestx_code_mode::normalize_code_mode_identifier(
                &forestx_tools::code_mode_name_for_tool_name(tool_name),
            ))
            .is_some_and(|winner| winner == tool_name)
    {
        forestx_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    provider.uses_openai_actor_authorization()
        || (provider.requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_forestx_backend))
}
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure.is_available_in_code_mode()
        && forestx_code_mode::is_code_mode_nested_tool(&forestx_tools::code_mode_name_for_tool_name(
            tool_name,
        ))
}
...
fn register_code_mode_executors(
    ...
    for tool in registry.entries() {
        ...
        if !forestx_code_mode::is_code_mode_nested_tool(&code_mode_name) {
            continue;
        }
        match code_mode_tool_names.entry(forestx_code_mode::normalize_code_mode_identifier(
            &code_mode_name,
        )) {
            Entry::Vacant(entry) => {
    ...
    let default_exec_yield_time_ms =
        default_exec_yield_time_override_ms(&turn_context.config.features)
            .unwrap_or(forestx_code_mode::DEFAULT_EXEC_YIELD_TIME_MS);
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, forestx_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| forestx_code_mode::ToolNamespaceDescription {
...
fn compare_code_mode_tools(
    left: &forestx_code_mode::ToolDefinition,
    right: &forestx_code_mode::ToolDefinition,
...
fn code_mode_namespace_name<'a>(
    tool: &forestx_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, forestx_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()

...
use forestx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_LUNA_MODEL_ID;
use forestx_model_provider_info::AMAZON_BEDROCK_GPT_5_6_SOL_MODEL_ID;
...
use forestx_protocol::openai_models::ApplyPatchToolType;
...
use forestx_protocol::openai_models::WebSearchToolType;
...
use forestx_tools::ResponsesApiTool;
use forestx_tools::ToolCall as ExtensionToolCall;
use forestx_tools::ToolExecutor;
use forestx_tools::ToolExposure;
...
fn zsh_fork_config_for_spec_plan_tests() -> forestx_tools::ZshForkConfig {
    let placeholder_exe = forestx_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(forestx_tools::ResponsesApiNamespace {
    ...
    fn handle(&self, _call: ExtensionToolCall) -> forestx_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(forestx_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: forestx_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    forestx_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = forestx_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(forestx_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    forestx_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        forestx_code_mode::PUBLIC_TOOL_NAME,
        forestx_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(forestx_code_mode::PUBLIC_TOOL_NAME) else {
    ...
    for (tool_name, guardian) in [
        ...
        for allow_login_shell in [false, true] {
            let plan = probe(|turn| {
                ...
                if guardian {
                    turn.session_source = forestx_protocol::protocol::SessionSource::SubAgent(
                        forestx_protocol::protocol::SubAgentSource::Other(
...
async fn shell_command_is_not_registered_without_a_single_local_environment() {
    let remote_environment = probe(|turn| {
        ...
        environment.environment = Arc::new(
            forestx_exec_server::Environment::create_for_tests(Some(
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if forestx_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            forestx_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
...
async fn environment_tools_follow_the_step_context() {
    ...
    turn.environments.environments.clear();
    let turn = Arc::new(turn);
    let mcp = Arc::new(forestx_mcp::McpBinding::empty(mcp_config_for_test(
        &turn.config,
    )));
...
async fn strict_tool_collisions_reject_external_and_synthetic_duplicates() {
    let cases = [
        ...
        (
            ...
            ToolPlanInputs {
                ...
                dynamic_tools: vec![dynamic_tool(
                    /*namespace*/ None,
                    forestx_tools::TOOL_SEARCH_TOOL_NAME,
                    /*defer_loading*/ false,
                )],
                ..ToolPlanInputs::default()
            },
    ...
    for (expected_name, inputs, code_mode_enabled, search_enabled) in cases {
        ...
        assert!(matches!(
            error.details(),
            ForestxErrorDetails::ToolCollision(name) if name == expected_name
        ));
        assert_eq!(
            error.to_string(),
...
async fn code_mode_uses_the_first_normalized_tool_identity() {
    for (code_mode_only, winner_exposure, shadow_is_deferred) in [
        ...
        assert!(!code_mode_tool_names.contains_key(forestx_tools::TOOL_SEARCH_TOOL_NAME));
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("forestx_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("forestx_app"),

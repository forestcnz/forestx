...
fn executed_tool_call_recorder_bounds_pending_calls_and_preserves_overflow() {
    ...
    for index in 0..MAX_PENDING_EXECUTED_TOOL_CALLS + 2 {
        recorder.record_tool_call(
            &ToolCall {
                tool_name: forestx_tools::ToolName::plain("direct_tool"),
...
fn executed_tool_call_recorder_bounds_retained_history_and_reports_omissions() {
    ...
    for index in 0..512 {
        ...
        recorder.record_tool_call(
            &ToolCall {
                tool_name: forestx_tools::ToolName::plain(format!("retained_tool_{index}")),
        ...
        forestx_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt);
    ...
    let omitted_calls = metadata
        .iter()
        .filter_map(|call| {
            call["arguments"]["_forestx_executed_tool_call_truncated"]["omitted_calls"].as_u64()
        })
        .sum::<u64>();

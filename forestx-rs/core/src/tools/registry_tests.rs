...
use forestx_protocol::DEFAULT_FUNCTION_NAMESPACE;
...
struct TestHandler {
    tool_name: forestx_tools::ToolName,
}

impl ToolExecutor<ToolInvocation> for TestHandler {
    fn tool_name(&self) -> forestx_tools::ToolName {
        self.tool_name.clone()
    }

    fn spec(&self) -> forestx_tools::ToolSpec {
        test_spec(&self.tool_name)
    }

    fn handle(&self, _invocation: ToolInvocation) -> forestx_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(
...
impl ToolExecutor<ToolInvocation> for ReadinessTestHandler {
    ...
    fn handle(&self, invocation: ToolInvocation) -> forestx_tools::ToolExecutorFuture<'_> {
...
fn test_spec(tool_name: &forestx_tools::ToolName) -> forestx_tools::ToolSpec {
    forestx_tools::ToolSpec::Function(forestx_tools::ResponsesApiTool {
        name: tool_name.name.clone(),
        description: "Test tool.".to_string(),
        strict: false,
        defer_loading: None,
        parameters: forestx_tools::JsonSchema::default(),
        output_schema: None,
    })
}
...
enum RecordedToolLifecycle {
    ...
    Finish {
        call_id: String,
        tool_name: forestx_tools::ToolName,
        outcome: forestx_extension_api::ToolCallOutcome,
    },
}
...
impl forestx_extension_api::ToolLifecycleContributor for ToolLifecycleRecorder {
    fn on_tool_start<'a>(
        &'a self,
        input: forestx_extension_api::ToolStartInput<'a>,
    ) -> forestx_extension_api::ToolLifecycleFuture<'a> {
    ...
    fn on_tool_finish<'a>(
        &'a self,
        input: forestx_extension_api::ToolFinishInput<'a>,
    ) -> forestx_extension_api::ToolLifecycleFuture<'a> {
        let records = Arc::clone(&self.records);
...
fn handler_normalizes_only_the_default_namespace() {
    let namespace = "mcp__forestx_apps__gmail";
    let tool_name = "gmail_get_recent_emails";
    let plain_name = forestx_tools::ToolName::plain(tool_name);
    let namespaced_name = forestx_tools::ToolName::namespaced(namespace, tool_name);
    ...
    let default_namespaced = registry.tool(&forestx_tools::ToolName::namespaced(
        DEFAULT_FUNCTION_NAMESPACE,
        tool_name,
    ));
    let empty_namespaced = registry.tool(&forestx_tools::ToolName::namespaced("", tool_name));
    let namespaced = registry.tool(&namespaced_name);
    let missing_namespaced = registry.tool(&forestx_tools::ToolName::namespaced(
        "mcp__forestx_apps__calendar",
        tool_name,
    ));
...
fn registry_rejects_default_namespace_alias_collisions() {
    let plain_name = forestx_tools::ToolName::plain("lookup");
    let namespaced_name = forestx_tools::ToolName::namespaced(DEFAULT_FUNCTION_NAMESPACE, "lookup");
...
fn registry_preserves_external_winners_and_trusted_synthetic_order() {
    ...
    let [first_name, second_name, synthetic_name] =
        ["first", "second", "synthetic"].map(forestx_tools::ToolName::plain);
...
fn reserved_shell_command_rejects_external_runtimes_without_a_builtin() {
    let handler = |tool_name| Arc::new(TestHandler { tool_name }) as Arc<dyn CoreToolRuntime>;
    let shell_command_name = forestx_tools::ToolName::plain("shell_command");
    let namespaced_shell_command_name =
        forestx_tools::ToolName::namespaced("client", "shell_command");
    ...
    assert!(
        !registry.register_external(handler(forestx_tools::ToolName::namespaced(
            DEFAULT_FUNCTION_NAMESPACE,
            "shell_command",
        )))
    );
...
fn registry_records_reserved_shell_command_when_a_matching_tool_exists() {
    let tool_name = forestx_tools::ToolName::plain("shell_command");
...
fn registry_allows_identical_names_in_different_namespaces() {
    let handler = |tool_name| Arc::new(TestHandler { tool_name }) as Arc<dyn CoreToolRuntime>;
    let mut registry = ToolRegistry::from_tools([handler(forestx_tools::ToolName::namespaced(
        "first", "lookup",
    ))]);
...
async fn readiness_selects_exact_tool_with_registry_owned_exposure() {
    ...
    let plain_name = forestx_tools::ToolName::plain("echo");
    let namespaced_name = forestx_tools::ToolName::namespaced("mcp__server__", "echo");
    assert!(
        TestHandler {
            tool_name: plain_name.clone(),
        }
    ...
    assert!(
        registry
            .tool(&forestx_tools::ToolName::namespaced("mcp__missing__", "echo"))
            .is_none()
    );
    assert_eq!(
        [
...
async fn function_tools_expose_default_hook_payloads_and_rewrites() -> anyhow::Result<()> {
    ...
    let tool_name = forestx_tools::ToolName::namespaced("functions.", "echo");
...
async fn function_hook_input_defaults_empty_arguments_to_object() {
    let (session, turn) = crate::session::tests::make_session_and_context().await;
    let tool_name = forestx_tools::ToolName::plain("echo");
    let handler = TestHandler {
        tool_name: tool_name.clone(),
    };
    let invocation = ToolInvocation {
...
async fn spawn_agent_function_tools_use_agent_matcher_alias() {
    ...
    let hook_payloads = [
        forestx_tools::ToolName::plain("spawn_agent"),
        forestx_tools::ToolName::namespaced(DEFAULT_FUNCTION_NAMESPACE, "spawn_agent"),
        forestx_tools::ToolName::namespaced(MULTI_AGENT_V1_NAMESPACE, "spawn_agent"),
    ]
    .into_iter()
    .map(|tool_name| {
        let handler = TestHandler {
...
fn post_tool_use_feedback_output_keeps_code_mode_result_typed() {
    let result = AnyToolResult {
        ...
        result: Box::new(PostToolUseFeedbackOutput {
            original: Box::new(forestx_tools::JsonToolOutput::new(
...
async fn dispatch_uses_canonical_tool_names_for_lifecycle_contributors() -> anyhow::Result<()> {
    ...
    let ok_tool = forestx_tools::ToolName::plain("ok_tool");
    let failing_tool = forestx_tools::ToolName::namespaced("extensions", "failing_tool");
    ...
    registry
        .dispatch_any_with_terminal_outcome(
            test_invocation(
                ...
                Arc::clone(&turn),
                "ok-call",
                forestx_tools::ToolName::namespaced(DEFAULT_FUNCTION_NAMESPACE, "ok_tool"),
            ),
    ...
    let expected = vec![
        ...
        RecordedToolLifecycle::Finish {
            ...
            outcome: forestx_extension_api::ToolCallOutcome::Completed { success: false },
        },
        RecordedToolLifecycle::Start {
        ...
        RecordedToolLifecycle::Finish {
            ...
            outcome: forestx_extension_api::ToolCallOutcome::Failed {

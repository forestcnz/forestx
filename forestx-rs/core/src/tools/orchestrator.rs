...
use forestx_otel::ToolDecisionSource;
...
impl ToolOrchestrator {
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let attempt_with_network_approval = SandboxAttempt {
            ...
            forestx_linux_sandbox_exe: attempt.forestx_linux_sandbox_exe,
        ...
        T: ToolRuntime<Rq, Out>,
    {
        ...
        let initial_attempt = SandboxAttempt {
            ...
            workspace_roots,
            forestx_linux_sandbox_exe: turn_ctx.config.forestx_linux_sandbox_exe.as_ref(),
            use_legacy_landlock,
        ...
        match first_result {
            ...
            Err(ToolError::Forestx(err)) => {
                let ForestxErrorDetails::Sandbox(SandboxErr::Denied {
                    output,
                    network_policy_decision,
                }) = err.details()
                else {
                    let err = ToolError::Forestx(err);
                ...
                if network_policy_decision.is_some() && network_approval_context.is_none() {
                    ...
                    return Err(ToolError::Forestx(err));
                }
                ...
                let retry_forestx_linux_sandbox_exe = if unsandboxed_allowed {
                    None
                } else {
                    turn_ctx.config.forestx_linux_sandbox_exe.as_ref()
                };
                let retry_attempt = SandboxAttempt {
                    ...
                    sandbox_cwd: &sandbox_policy_cwd,
                    workspace_roots,
                    forestx_linux_sandbox_exe: retry_forestx_linux_sandbox_exe,
                    use_legacy_landlock,
                    windows_sandbox_level: turn_ctx.windows_sandbox_level,
                    windows_sandbox_private_desktop: turn_ctx
                        .config
...
fn sandbox_outcome_from_tool_error(err: &ToolError) -> Option<&'static str> {
    match err {
        ToolError::Forestx(err) => match err.details() {
            ForestxErrorDetails::Sandbox(SandboxErr::Denied { .. }) => Some("denied"),
            ForestxErrorDetails::Sandbox(SandboxErr::Timeout { .. }) => Some("timed_out"),
            ForestxErrorDetails::Sandbox(SandboxErr::Signal(_)) => Some("signal"),
            _ => None,
        },

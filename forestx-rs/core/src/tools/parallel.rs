...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, ForestxErr>> {
        ...
        async move {
            match future.await {
                ...
                Err(FunctionCallError::Fatal(message)) => Err(ForestxErr::Fatal(message)),
    ...
    pub(crate) fn handle_tool_call_with_source(
        ...
        if self
            ...
            .config
            .features
            .enabled(forestx_features::Feature::ExecutedToolCallMetadata)
    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: forestx_protocol::models::FunctionCallOutputPayload {
                    body: forestx_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
    ...
    fn drop(&mut self) {
        ...
        macro_rules! log_tool_call {
            ($dispatch_duration_ms:expr, $handler_duration_ms:expr, $total_duration_ms:expr) => {
                info!(
                    event.name = "forestx.tool_call",
                    trace_id = %forestx_otel::current_span_trace_id().unwrap_or_default(),
...
mod tests {
    ...
    async fn cancellation_before_dispatch_admission_logs_dispatch_only_timing() -> anyhow::Result<()>
    {
        ...
        let tool_name = forestx_tools::ToolName::plain("test_tool");
        ...
        let timing_events = logs
            .lines()
            .filter(|line| line.contains("event.name=\"forestx.tool_call\""))
            .collect::<Vec<_>>();
        assert_eq!(
            timing_events.len(),
            1,
    ...
    impl forestx_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl forestx_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: forestx_extension_api::ToolFinishInput<'a>,
        ) -> forestx_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let mut builder =
            forestx_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = forestx_tools::ToolName::plain("cleanup_tool");

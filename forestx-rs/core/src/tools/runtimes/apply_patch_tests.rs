...
use forestx_sandboxing::policy_transforms::effective_network_sandbox_policy;
...
fn test_turn_environment(environment_id: &str) -> crate::session::turn_context::TurnEnvironment {
    crate::session::turn_context::TurnEnvironment::new(
        ...
        std::sync::Arc::new(forestx_exec_server::Environment::default_for_tests()),
...
async fn approval_action_preserves_patch_path_uris() {
    ...
    let request = ApplyPatchRequest {
        turn_environment: test_turn_environment(forestx_exec_server::LOCAL_ENVIRONMENT_ID),
        action,
...
async fn file_system_sandbox_context_preserves_executor_workspace_permissions() {
    ...
    assert_eq!(
        sandbox.cwd,
        Some(forestx_utils_path_uri::PathUri::from_abs_path(&path))
    );
    assert_eq!(

...
use forestx_shell_command::powershell::prefix_powershell_script_with_utf8;
    ...
    async fn run(
        ...
        let (env, managed_network_context, network_proxy_launch) = match managed_network {
            Some(network) if environment_is_remote => {
                let mut launch = network.remote_launch_config().await.map_err(|err| {
                    ToolError::Forestx(ForestxErr::Io(io::Error::other(err.to_string())))
                })?;
                ...
                if !launch.proxy.enabled {
                    ...
                    let environment_info =
                        req.turn_environment
                            ...
                            .await
                            .map_err(|err| {
                                ToolError::Forestx(ForestxErr::Io(io::Error::other(format!(
        ...
        if let UnifiedExecShellMode::ZshFork(zsh_fork_config) = &self.shell_mode {
            ...
            .map_err(|error| match error {
                ...
                error @ ToolError::Forestx(_) => error,
            })?;
...
mod tests {
    ...
    use forestx_tools::ZshForkConfig;

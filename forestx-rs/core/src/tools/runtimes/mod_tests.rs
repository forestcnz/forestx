...
#[cfg(target_os = "macos")]
use forestx_network_proxy::FORESTX_PROXY_GIT_SSH_COMMAND_MARKER;
use forestx_network_proxy::CUSTOM_CA_ENV_KEYS;
use forestx_network_proxy::ConfigReloader;
use forestx_network_proxy::ConfigReloaderFuture;
use forestx_network_proxy::ConfigState;
use forestx_network_proxy::NetworkProxy;
use forestx_network_proxy::NetworkProxyConfig;
use forestx_network_proxy::NetworkProxyConstraints;
use forestx_network_proxy::NetworkProxyState;
use forestx_network_proxy::PROXY_ACTIVE_ENV_KEY;
use forestx_network_proxy::PROXY_ENV_KEYS;
#[cfg(target_os = "macos")]
use forestx_network_proxy::PROXY_GIT_SSH_COMMAND_ENV_KEY;
...
async fn test_network_proxy() -> anyhow::Result<NetworkProxy> {
    let state = forestx_network_proxy::build_config_state(
    ...
    NetworkProxy::builder()
        ...
        .managed_by_forestx(/*managed_by_forestx*/ false)
...
fn runtime_path_prepends_records_runtime_path_prepend() {
    ...
    runtime_path_prepends.prepend(&mut env, PathBuf::from("/package/forestx-path").as_path());

    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/forestx-path:/usr/bin:/bin"),
        "runtime PATH prepend should update the live exec environment"
    );
    assert_eq!(
        runtime_path_prepends.entries,
        vec!["/package/forestx-path"],
...
fn runtime_path_prepends_drops_empty_path_entries() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        ":/usr/bin:/package/forestx-path::/bin:".to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
...
fn apply_zsh_fork_path_prepend_uses_shell_parent() {
    ...
    apply_zsh_fork_path_prepend(
        ...
        PathBuf::from("/package/forestx-resources/zsh/bin/zsh").as_path(),
    );

    let expected = "/package/forestx-resources/zsh/bin:/usr/bin:/bin";
    assert_eq!(env.get("PATH").map(String::as_str), Some(expected));
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
            entries: vec!["/package/forestx-resources/zsh/bin".to_string()]
        }
    );
}
...
fn apply_zsh_fork_path_prepend_moves_existing_shell_parent_to_front() {
    let mut env = HashMap::from([(
        "PATH".to_string(),
        "/usr/bin:/package/forestx-resources/zsh/bin:/bin:/package/forestx-resources/zsh/bin"
            .to_string(),
    )]);
    let mut runtime_path_prepends = RuntimePathPrepends::default();
    ...
    assert_eq!(
        env.get("PATH").map(String::as_str),
        Some("/package/forestx-resources/zsh/bin:/usr/bin:/bin")
    );
    assert_eq!(
        runtime_path_prepends,
        RuntimePathPrepends {
...
#[test]
fn explicit_escalation_keeps_user_proxy_env_without_forestx_marker() {
    let env = HashMap::from([
        (
...
#[test]
fn maybe_wrap_shell_lc_with_snapshot_restores_forestx_thread_id_from_env() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport FORESTX_THREAD_ID='parent-thread'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printf '%s' \"$FORESTX_THREAD_ID\"".to_string(),
    ];
    let rewritten = maybe_wrap_shell_lc_with_snapshot(
        &command,
        &session_shell,
        Some(&shell_snapshot),
        &HashMap::new(),
        &HashMap::from([("FORESTX_THREAD_ID".to_string(), "nested-thread".to_string())]),
        &RuntimePathPrepends::default(),
    );
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env("FORESTX_THREAD_ID", "nested-thread")
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_restores_permission_profile_from_env() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport FORESTX_PERMISSION_PROFILE='parent-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
        "/bin/bash".to_string(),
        "-lc".to_string(),
        "printenv FORESTX_PERMISSION_PROFILE".to_string(),
    ];
    let env = HashMap::from([(
        FORESTX_PERMISSION_PROFILE_ENV_VAR.to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env(FORESTX_PERMISSION_PROFILE_ENV_VAR, "current-profile")
        .output()
        .expect("run rewritten command");
...
fn maybe_wrap_shell_lc_with_snapshot_unsets_absent_permission_profile() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport FORESTX_PERMISSION_PROFILE='stale-profile'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove(FORESTX_PERMISSION_PROFILE_ENV_VAR)
        .output()
        .expect("run rewritten command");
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_refreshes_forestx_proxy_git_ssh_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{FORESTX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
    );
    let fresh_command = format!(
        "{FORESTX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:48081 %h %p'"
    );
    std::fs::write(
        &snapshot_path,
        format!(
...
#[cfg(target_os = "macos")]
#[test]
fn maybe_wrap_shell_lc_with_snapshot_clears_stale_forestx_git_ssh_command_without_live_command() {
    let dir = tempdir().expect("create temp dir");
    let snapshot_path = dir.path().join("snapshot.sh");
    let stale_command = format!(
        "{FORESTX_PROXY_GIT_SSH_COMMAND_MARKER}ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
...
fn run_snapshot_path_probe_with_runtime_path_prepend(
    ...
    let package_path_dir = dir.path().join("forestx-path");
...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport FORESTX_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let (session_shell, shell_snapshot) =
        shell_with_snapshot(ShellType::Bash, "/bin/bash", snapshot_path.abs());
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${FORESTX_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$FORESTX_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "FORESTX_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("FORESTX_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
    assert_eq!(String::from_utf8_lossy(&output.stdout), "unset");

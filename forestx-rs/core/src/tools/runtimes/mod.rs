...
*/
use crate::exec_env::FORESTX_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::FORESTX_THREAD_ID_ENV_VAR;
...
pub(crate) use forestx_network_proxy::is_managed_proxy_env_var;
pub(crate) use forestx_network_proxy::strip_managed_proxy_env;
...
/// PATH entries owned by Forestx runtime setup.
///
...
/// runtime-only vars like `FORESTX_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains Forestx-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [FORESTX_THREAD_ID_ENV_VAR, FORESTX_PERMISSION_PROFILE_ENV_VAR] {
    ...
    let (override_captures, override_exports) =
        build_override_exports(&override_env, &[FORESTX_PERMISSION_PROFILE_ENV_VAR]);
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__FORESTX_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__FORESTX_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__FORESTX_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__FORESTX_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_forestx_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_forestx_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", FORESTX_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__FORESTX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}

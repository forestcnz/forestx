...
use forestx_execpolicy::PolicyParser;
...
use forestx_sandboxing::policy_transforms::effective_permission_profile;
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(forestx_execpolicy::Policy::empty())),
        ...
        stopwatch: forestx_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = forestx_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = forestx_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.forestx_home)
        .context("recreate forestx home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .forestx_home
        .join(forestx_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command =
        forestx_shell_command::parse_command::shlex_join(&["/usr/bin/touch".to_string(), target_str]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(forestx_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        forestx_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        forestx_shell_escalation::EscalationDecision::Escalate(
            forestx_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, forestx_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        forestx_shell_escalation::EscalationDecision::Deny {

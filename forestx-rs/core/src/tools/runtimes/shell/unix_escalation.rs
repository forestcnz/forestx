...
use forestx_execpolicy::MatchOptions;
...
use forestx_protocol::protocol::NetworkPolicyRuleAction;
...
use forestx_shell_command::bash::parse_shell_lc_plain_commands;
use forestx_shell_command::bash::parse_shell_lc_single_command_prefix;
use forestx_shell_escalation::EscalateServer;
use forestx_shell_escalation::EscalationDecision;
use forestx_shell_escalation::EscalationExecution;
use forestx_shell_escalation::EscalationPermissions;
use forestx_shell_escalation::EscalationPolicy;
use forestx_shell_escalation::EscalationPolicyFuture;
use forestx_shell_escalation::EscalationSession;
use forestx_shell_escalation::ExecParams;
use forestx_shell_escalation::ExecResult;
use forestx_shell_escalation::PreparedExec;
use forestx_shell_escalation::ResolvedPermissionProfile;
use forestx_shell_escalation::ShellCommandExecutor;
use forestx_shell_escalation::ShellCommandExecutorFuture;
use forestx_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        forestx_linux_sandbox_exe: ctx.turn.config.forestx_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    forestx_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<forestx_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["FORESTX_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            forestx_linux_sandbox_exe: self.forestx_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Forestx(ForestxErr::Sandbox(SandboxErr::Timeout {
    ...
    if is_likely_sandbox_denied(sandbox, &output) {
        ...
        return Err(ToolError::Forestx(ForestxErr::Sandbox(SandboxErr::Denied {
            output: Box::new(output),

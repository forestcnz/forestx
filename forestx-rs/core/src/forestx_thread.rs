...
use forestx_exec_server::SelectedCapabilityRootsStatus;
...
use forestx_thread_store::ThreadStoreResult;
...
/// Explains why `ForestxThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
pub struct ForestxThreadSettingsOverrides {
...
/// (formerly called a conversation) in Forestx.
impl ForestxThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> ForestxResult<String> {
    ...
    pub async fn shutdown_and_wait(&self) -> ForestxResult<()> {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(forestx_extension_api::ThreadResumeInput {
    ...
    pub async fn submit_user_input_and_wait_for_admission(
        ...
        trace: Option<W3cTraceContext>,
        client_user_message_id: Option<String>,
    ) -> ForestxResult<UserMessageAdmission> {
        ...
        tokio::select! {
            biased;
            result = admission => result.map_err(|_| ForestxErr::InternalAgentDied)?,
            () = self.io.session_loop_termination.clone() => Err(ForestxErr::InternalAgentDied),
        }
    }
    ...
    /// callers that only hold a `ForestxThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: ForestxThreadSettingsOverrides,
    ...
    async fn thread_settings_update(
        ...
        let ForestxThreadSettingsOverrides {
            environments,
    ...
    pub async fn submit_with_id(&self, mut sub: Submission) -> ForestxResult<()> {
        sub.parent_turn_id = None;
        self.io.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> ForestxResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> ForestxResult<()> {
        if items.is_empty() {
            return Err(ForestxErr::InvalidRequest(
    ...
    ) -> (forestx_mcp::McpConfig, forestx_mcp::McpRuntimeContext) {
    ...
    pub async fn current_mcp_config_and_runtime_context(
        &self,
    ) -> (Arc<forestx_mcp::McpConfig>, forestx_mcp::McpRuntimeContext) {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> ForestxResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            ForestxErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> ForestxResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(ForestxErr::InvalidRequest(

...
use forestx_analytics::ForestxCompactionEvent;
...
use forestx_protocol::items::ContextCompactionItem;
...
use forestx_utils_output_truncation::approx_token_count;
use forestx_utils_output_truncation::truncate_text;
use futures::prelude::*;
use tracing::error;

pub use forestx_prompts::SUMMARIZATION_PROMPT;
pub use forestx_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = ForestxErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let forestx_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
        ...
        window_id,
        ForestxResponsesRequestKind::Compaction(compaction_metadata),
    );
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(err)
                if matches!(
                    err.details(),
                    ForestxErrorDetails::Interrupted | ForestxErrorDetails::TurnAborted
                ) =>
            {
                return Err(err);
            }
            Err(e) if matches!(e.details(), ForestxErrorDetails::SessionBudgetExceeded) => {
                sess.track_turn_forestx_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e) if matches!(e.details(), ForestxErrorDetails::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        forestx_error: Option<&ForestxErr>,
        ...
        sess.services
            ...
            .track_compaction(ForestxCompactionEvent {
                ...
                status,
                forestx_error_kind: forestx_error.map(Into::into),
                forestx_error_http_status_code: forestx_error
                    .and_then(ForestxErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &ForestxResult<T>) -> CompactionStatus {
    match result {

...
fn bash_snapshot_filters_invalid_exports() -> Result<()> {
    let output = Command::new("/bin/bash")
        ...
        .env("VALID_NAME", "ok")
        .env("PWD", "/tmp/stale")
        .env("NEXTEST_BIN_EXE_forestx-write-config-schema", "/path/to/bin")
        .env("BAD-NAME", "broken")
        .output()?;
    ...
    assert!(!stdout.contains("NEXTEST_BIN_EXE_forestx-write-config-schema"));
...
fn zsh_snapshot_restores_tied_path() -> Result<()> {
    ...
    let readonly_restored = Command::new("/bin/zsh")
        .arg("-f")
        .arg("-c")
        .arg("set -e; . \"$1\"; export PATH='/forestx-path':\"$PATH\"; print -r -- \"$PATH\"")
        .arg("zsh")
        .arg(&snapshot_path)
        .env_clear()
        .env("PATH", "/usr/bin:/bin")
...
async fn write_rollout_stub(forestx_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
    let dir = forestx_home
        .join("sessions")
        .join("2025")
        .join("01")
        .join("01");
...
async fn cleanup_stale_snapshots_removes_orphans_and_keeps_live() -> Result<()> {
    let dir = tempdir()?;
    let forestx_home = dir.path().abs();
    let snapshot_dir = forestx_home.join(SNAPSHOT_DIR);
    ...
    write_rollout_stub(&forestx_home, live_session).await?;
    fs::write(&live_snapshot, "live").await?;
    fs::write(&orphan_snapshot, "orphan").await?;
    fs::write(&invalid_snapshot, "invalid").await?;

    cleanup_stale_snapshots(&forestx_home, ThreadId::new(), /*state_db*/ None).await?;
...
async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&forestx_home, stale_session).await?;
...
async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
    ...
    let rollout_path = write_rollout_stub(&forestx_home, active_session).await?;
    fs::write(&active_snapshot, "active").await?;

    set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

    cleanup_stale_snapshots(&forestx_home, active_session, /*state_db*/ None).await?;

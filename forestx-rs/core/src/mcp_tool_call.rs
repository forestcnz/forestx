...
use forestx_analytics::AppInvocation;
use forestx_analytics::InvocationType;
use forestx_analytics::build_track_events_context;
use forestx_config::ConfigLayerSource;
use forestx_config::types::AppToolApproval;
use forestx_config::types::ApprovalsReviewer;
use forestx_connectors::AppToolPolicy;
use forestx_connectors::AppToolPolicyEvaluator;
use forestx_connectors::AppToolPolicyInput;
use forestx_features::Feature;
use forestx_hooks::PermissionRequestDecision;
use forestx_mcp::FORESTX_APPS_MCP_SERVER_NAME;
use forestx_mcp::MCP_TOOL_FORESTX_APPS_META_KEY;
use forestx_mcp::McpPermissionPromptAutoApproveContext;
use forestx_mcp::PreparedMcpCall;
use forestx_mcp::SandboxState;
use forestx_mcp::auth_elicitation_completed_result;
use forestx_mcp::build_auth_elicitation_plan;
use forestx_mcp::mcp_permission_prompt_is_auto_approved;
use forestx_protocol::approvals::ElicitationRequest;
use forestx_protocol::items::McpToolCallError;
use forestx_protocol::items::McpToolCallItem;
use forestx_protocol::items::McpToolCallStatus;
use forestx_protocol::items::TurnItem;
use forestx_protocol::mcp::CallToolResult;
use forestx_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use forestx_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use forestx_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use forestx_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use forestx_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use forestx_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use forestx_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use forestx_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use forestx_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use forestx_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use forestx_protocol::request_user_input::RequestUserInputAnswer;
use forestx_protocol::request_user_input::RequestUserInputArgs;
use forestx_protocol::request_user_input::RequestUserInputQuestion;
use forestx_protocol::request_user_input::RequestUserInputQuestionOption;
...
use forestx_utils_output_truncation::TruncationPolicy;
use forestx_utils_output_truncation::truncate_text;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "forestx/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "forestx.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "forestx.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == FORESTX_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == FORESTX_APPS_MCP_SERVER_NAME {
    ...
    if server == FORESTX_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
        let result = notify_mcp_tool_call_skip(
            sess.as_ref(),
            turn_context.as_ref(),
            &call_id,
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == FORESTX_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.forestx_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    let result = async {
        let result = async {
            ...
            Ok(maybe_request_forestx_apps_auth_elicitation(
                sess,
                turn_context,
    ...
    .await;
    maybe_track_forestx_app_used(sess, turn_context, &server, &metadata).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        forestx.mcp.target.id = Empty,
        forestx.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        forestx.mcp.error.code = Empty,
    );
...
async fn maybe_request_forestx_apps_auth_elicitation(
    sess: &Arc<Session>,
    turn_context: &TurnContext,
    ...
    if server != FORESTX_APPS_MCP_SERVER_NAME {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        forestx_connectors::metadata::connector_install_url(
    ...
    refresh_forestx_apps_after_connector_auth(sess, turn_context).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_forestx_apps_after_connector_auth(sess: &Arc<Session>, turn_context: &TurnContext) {
    let mcp_tools_result = sess.hard_refresh_latest_forestx_apps_tools().await;
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh Forestx Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        forestx_linux_sandbox_exe: prepared_call.config().forestx_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                forestx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == forestx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_forestx_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    metadata: &McpToolApprovalMetadata,
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    forestx_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
impl Session {
    async fn register_mcp_tool_approval_metadata(
        ...
        turn_state.lock().await.insert_mcp_tool_approval_metadata(
            call_id.to_string(),
            (invocation.server == FORESTX_APPS_MCP_SERVER_NAME).then(|| invocation.clone()),
            metadata,
        );
    }
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, forestx_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_FORESTX_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == FORESTX_APPS_MCP_SERVER_NAME {
        let mut forestx_apps_meta = metadata
            .and_then(|metadata| metadata.forestx_apps_meta.clone())
            .unwrap_or_default();
        forestx_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_FORESTX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(forestx_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__forestx_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == FORESTX_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) fn build_guardian_mcp_tool_review_request(
    ...
    GuardianApprovalRequest::McpToolCall {
        ...
        connected_account_email: (invocation.server == FORESTX_APPS_MCP_SERVER_NAME)
...
fn mcp_tool_metadata(prepared_call: &PreparedMcpCall) -> McpToolApprovalMetadata {
    ...
    let connector_description = (server == FORESTX_APPS_MCP_SERVER_NAME)
        .then(|| tool_info.namespace_description.clone())
        .flatten();

    let forestx_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
        .and_then(|meta| meta.get(MCP_TOOL_FORESTX_APPS_META_KEY))
        .and_then(serde_json::Value::as_object)
        .cloned();
    let connected_account_email = if server == FORESTX_APPS_MCP_SERVER_NAME {
        forestx_apps_meta
            .as_ref()
...
fn openai_file_input_optional_fields_for_server(
    ...
    (server == FORESTX_APPS_MCP_SERVER_NAME)
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == FORESTX_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_forestx_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_forestx_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, forestx_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;

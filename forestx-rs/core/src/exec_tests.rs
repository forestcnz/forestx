...
fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"FORESTX_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
    assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
}
...
fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"FORESTX_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
...
async fn exec_full_buffer_capture_ignores_expiration() -> Result<()> {
    ...
    let output = exec(
        ExecParams {
            command,
            cwd: forestx_utils_absolute_path::AbsolutePathBuf::current_dir()?,
            expiration: 1.into(),
...
async fn process_exec_tool_call_preserves_full_buffer_capture_policy() -> Result<()> {
    ...
    let cwd = forestx_utils_absolute_path::AbsolutePathBuf::current_dir()?;
...
fn windows_restricted_token_rejects_managed_root_write_profiles() {
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Special {
                value: forestx_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: forestx_protocol::permissions::FileSystemAccessMode::Write,
...
fn windows_elevated_allows_split_restricted_read_policies() {
    ...
    let docs = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Path { path: docs },
            access: forestx_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
        },
    ]);
    let permission_profile = PermissionProfile::from_runtime_permissions(
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Special {
                value: forestx_protocol::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: forestx_protocol::permissions::FileSystemAccessMode::Write,
        ...
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Path {
                path: forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: forestx_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Path { path: docs.clone() },
            access: forestx_protocol::permissions::FileSystemAccessMode::Read,
            missing_path_behavior: None,
        },
    ]);
    ...
    // `.forestx`, so the restricted-token overlay only needs the extra read-only
    // docs carveout.
...
fn windows_restricted_token_rejects_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Path { path: blocked },
...
fn windows_elevated_supports_split_write_read_carveouts() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            additional_deny_read_paths: vec![],
            additional_deny_write_paths: vec![
                forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_docs)
                    .expect("absolute docs"),
            ],
        }))
    );
...
fn windows_workspace_defaults_do_not_hide_explicit_metadata_carveouts() {
    ...
    for name in forestx_protocol::permissions::PROTECTED_METADATA_PATH_NAMES {
        let (mut explicit_policy, network_policy) = default_profile.to_runtime_permissions();
        explicit_policy
            .entries
            .push(forestx_protocol::permissions::FileSystemSandboxEntry {
                path: forestx_protocol::permissions::FileSystemPath::Special {
...
fn windows_elevated_supports_unreadable_split_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Path {
                path: forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&blocked)
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
                    expected_blocked.clone(),
                )
                .expect("absolute blocked"),
            ],
            additional_deny_write_paths: vec![
                forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(expected_blocked)
                    .expect("absolute blocked"),
            ],
        }))
    );
...
fn windows_elevated_supports_unreadable_globs() {
    ...
    assert_eq!(
        ...
        Ok(Some(WindowsSandboxFilesystemOverrides {
            ...
            write_roots_override: None,
            additional_deny_read_paths: vec![
                forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(secret)
...
fn windows_elevated_rejects_reopened_writable_descendants() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        forestx_protocol::permissions::FileSystemSandboxEntry {
            path: forestx_protocol::permissions::FileSystemPath::Path {
                path: forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&nested)
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = forestx_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
    ...
    assert_eq!(
        select_process_exec_tool_sandbox_type(
            ...
            forestx_protocol::config_types::WindowsSandboxLevel::Disabled,
...
async fn kill_child_process_group_kills_grandchildren_on_timeout() -> Result<()> {
    ...
    let params = ExecParams {
        ...
        network_environment_id: None,
        sandbox_permissions: SandboxPermissions::UseDefault,
        windows_sandbox_level: forestx_protocol::config_types::WindowsSandboxLevel::Disabled,
        windows_sandbox_private_desktop: false,
        justification: None,
        arg0: None,
    };

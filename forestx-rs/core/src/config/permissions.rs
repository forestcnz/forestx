...
use forestx_config::permissions_toml::NetworkUnixSocketPermissionToml;
use forestx_config::permissions_toml::NetworkUnixSocketPermissionsToml;
...
use forestx_features::NetworkProxyDomainPermissionToml;
use forestx_features::NetworkProxyModeToml;
use forestx_features::NetworkProxyUnixSocketPermissionToml;
use forestx_network_proxy::NetworkMode;
use forestx_network_proxy::NetworkProxyConfig;
#[cfg(test)]
use forestx_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
use forestx_protocol::permissions::project_roots_glob_pattern;
...
/// for Forestx to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_forestx_runtime(
    forestx_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(forestx_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Forestx versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Forestx. Filesystem access will remain restricted. Upgrade Forestx if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{subpath}` is not recognized by this version of Forestx and will be ignored. Upgrade Forestx if this path is required."
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Forestx and will be ignored. Upgrade Forestx if this path is required."
            ),
        },
    );
}

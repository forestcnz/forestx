...
pub(super) fn push_sqlite_home_env_override_warning(
    ...
    tracing::warn!(
        ?source,
        ?value,
        "`FORESTX_SQLITE_HOME` is overridden by an exact requirement for sqlite_home"
    );
    startup_warnings.push(format!(
        "Environment value for `$FORESTX_SQLITE_HOME` is overridden by the required `sqlite_home` value {value:?} from {source}."
    ));
}

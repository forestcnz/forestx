...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let forestx_home = temp_dir.path().join(".forestx");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = forestx_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("forestx-arg0-session");
    let sibling_arg0_dir = arg0_root.join("forestx-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("forestx-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&forestx_home)?,
    )
    .await?;
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        config.unix_sockets,
        Some(forestx_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        config.domains,
        Some(forestx_network_proxy::NetworkDomainPermissions {
            entries: vec![forestx_network_proxy::NetworkDomainPermissionEntry {
                pattern: "openai.com".to_string(),
                permission: forestx_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}

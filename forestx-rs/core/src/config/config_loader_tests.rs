...
use forestx_config::ConfigError;
use forestx_config::ConfigLayerEntry;
use forestx_config::ConfigLayerSource;
use forestx_config::ConfigLoadError;
use forestx_config::ConfigLoadOptions;
use forestx_config::ConfigRequirements;
use forestx_config::ConfigRequirementsToml;
use forestx_config::ConfigRequirementsWithSources;
use forestx_config::FilesystemDenyReadPattern;
use forestx_config::LoaderOverrides;
use forestx_config::RequirementSource;
use forestx_config::RequirementsLayerEntry;
use forestx_config::SessionThreadConfig;
use forestx_config::StaticThreadConfigLoader;
use forestx_config::ThreadConfigSource;
use forestx_config::compose_requirements;
use forestx_config::config_error_from_ignored_toml_fields;
use forestx_config::config_error_from_toml;
use forestx_config::config_error_from_typed_toml;
...
async fn write_project_hook_config(
    dot_forestx_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_forestx_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_forestx_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when FORESTX_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .forestx_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = forestx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        forestx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = forestx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = forestx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when FORESTX_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn managed_preferences_requirements_resolve_paths_against_forestx_home() -> anyhow::Result<()> {
    use base64::Engine;

    let tmp = tempdir()?;
    let forestx_home = tmp.path().join("forestx-home");
...
"#
    ...
    .await?;
    let expected_log_dir = AbsolutePathBuf::resolve_path_against_base("~/.forestx/logs", &forestx_home);
    let requirements = layers.requirements_toml();

    assert_eq!(
        requirements.sqlite_home.as_deref(),
        Some(forestx_home.join("state").as_path())
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(forestx_home.join("models.json").as_path())
    );
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![forestx_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(forestx_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(forestx_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn system_requirements_control_in_app_updates() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let forestx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&forestx_home).await?;
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.forestx".to_string(),
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(forestx_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_forestx_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let forestx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&forestx_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", forestx_home.display()))
                .expect("bundle requirements path should resolve against forestx_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let forestx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&forestx_home)
        .await
        .expect("create forestx home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            forestx_config::SandboxModeRequirement::ReadOnly,
            forestx_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    forestx_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".forestx")).await?;
    tokio::fs::create_dir_all(project_root.join(".forestx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".forestx").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".forestx").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            ConfigLayerSource::Project { dot_forestx_folder } => Some(dot_forestx_folder),
            _ => None,
        })
        .collect();
    assert_eq!(project_layers.len(), 2);
    assert_eq!(project_layers[0].as_path(), nested.join(".forestx").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".forestx").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".forestx")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".forestx")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".forestx"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".forestx"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".forestx"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".forestx"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(".forestx"))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_forestx_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    tokio::fs::write(project_root.join(".forestx").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".forestx").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".forestx").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".forestx").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let forestx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&forestx_home).await?;
    tokio::fs::write(forestx_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_forestx_exists_without_config_toml() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_forestx_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".forestx"))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
#[tokio::test]
async fn forestx_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let forestx_home = home_dir.join(".forestx");
    tokio::fs::create_dir_all(&forestx_home).await?;
    tokio::fs::write(
        forestx_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn forestx_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_forestx = project_root.join(".forestx");
    let nested_dot_forestx = nested.join(".forestx");

    tokio::fs::create_dir_all(&nested_dot_forestx).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_forestx.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_forestx).await?;
    make_config_for_test(
        &project_dot_forestx,
        &project_root,
        TrustLevel::Trusted,
    ...
    .await?;
    let user_config_path = project_dot_forestx.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_forestx_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_forestx)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let forestx_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&forestx_home_untrusted).await?;
    make_config_for_test(
        &forestx_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = forestx_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let forestx_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&forestx_home_unknown).await?;
    tokio::fs::write(
        forestx_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_forestx).await?;
    ...
    // `.forestx` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_forestx.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_forestx.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.forestx` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".forestx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".forestx").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let forestx_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&forestx_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let forestx_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&forestx_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &forestx_home,
...
mod requirements_exec_policy_tests {
    ...
    use forestx_config::ConfigRequirementsWithSources;
    use forestx_config::RequirementSource;
    use forestx_config::RequirementsExecPolicyDecisionToml;
    use forestx_config::RequirementsExecPolicyParseError;
    use forestx_config::RequirementsExecPolicyPatternTokenToml;
    use forestx_config::RequirementsExecPolicyPrefixRuleToml;
    use forestx_config::RequirementsExecPolicyToml;
    use forestx_execpolicy::Decision;
    use forestx_execpolicy::Evaluation;
    use forestx_execpolicy::RuleMatch;
    use forestx_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_forestx_folder_with_requirements(
        dot_forestx_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_forestx_folder = AbsolutePathBuf::from_absolute_path(dot_forestx_folder)
            .expect("absolute dot_forestx_folder");
        let layer = ConfigLayerEntry::new(
            ConfigLayerSource::Project { dot_forestx_folder },
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_forestx_folder_with_requirements(temp_dir.path(), requirements);

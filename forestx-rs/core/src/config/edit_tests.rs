...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(forestx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = forestx_home.join("config-link.toml");
    let config_path = forestx_home.join(CONFIG_TOML_FILE);
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let forestx_home = tmp.path();
    let link_a = forestx_home.join("a.toml");
    let link_b = forestx_home.join("b.toml");
    let config_path = forestx_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(forestx_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(forestx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
#[test]
fn blocking_set_hide_gpt_5_1_forestx_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let forestx_home = tmp.path();
    std::fs::write(
        forestx_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    apply_blocking(
        forestx_home,
        &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
            "hide_gpt-5.1-forestx-max_migration_prompt".to_string(),
            true,
        )],
    )
    .expect("persist");
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-forestx-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
client_id = \"eci-prd-pub-forestx-123\"
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(forestx_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !forestx_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(forestx_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(forestx_home.join(CONFIG_TOML_FILE)).expect("read config");

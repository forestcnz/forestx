...
pub fn apply_blocking(forestx_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(forestx_home: &Path) -> Self {
        Self::for_config_path(&forestx_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(forestx_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.forestx_home.join(CONFIG_TOML_FILE).to_path_buf());

...
use forestx_config::types::DEFAULT_OTEL_ENVIRONMENT;
use forestx_config::types::OtelConfig;
...
fn resolve_span_attributes(
    ...
    for (key, value) in span_attributes {
        ...
        if let Err(err) = forestx_otel::validate_span_attributes(&attribute) {
...
fn resolve_tracestate(
    ...
    for (member_key, fields) in tracestate {
        ...
        if let Err(err) = forestx_otel::validate_tracestate_member(&member_key, &fields) {
    ...
    if let Err(err) = forestx_otel::validate_tracestate_entries(&valid_entries) {
        push_invalid_config_warning("otel.tracestate", err, startup_warnings);
        return BTreeMap::new();
    }
...
fn resolve_tracestate_member_fields(
    ...
    for (field_key, value) in fields {
        ...
        if let Err(err) = forestx_otel::validate_tracestate_member(member_key, &field) {
            push_invalid_config_warning("otel.tracestate", err, startup_warnings);
            continue;
        }

...
fn config_schema_matches_fixture() {
    let fixture_path = forestx_utils_cargo_bin::find_resource!("config.schema.json")
...
Run `just write-config-schema` to overwrite with your changes.\n\n{diff}"
    ...
    // Make sure the version in the repo matches exactly: https://github.com/openai/forestx/pull/10977.

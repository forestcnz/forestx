...
use forestx_config::ConfigRequirements;
use forestx_config::ConfigRequirementsToml;
use forestx_config::ConstrainedWithSource;
use forestx_config::FeatureRequirementsToml;
use forestx_config::McpServerRequirement;
use forestx_config::PluginRequirementsToml;
use forestx_config::ProfileV2Name;
use forestx_config::ResidencyRequirement;
use forestx_config::SandboxModeRequirement;
use forestx_config::Sourced;
use forestx_config::ThreadConfigLoader;
use forestx_config::config_toml::ConfigLockfileToml;
use forestx_config::config_toml::ConfigToml;
use forestx_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use forestx_config::config_toml::ProjectConfig;
use forestx_config::config_toml::RealtimeAudioConfig;
use forestx_config::config_toml::RealtimeConfig;
use forestx_config::config_toml::ThreadStoreToml;
use forestx_config::config_toml::validate_model_providers;
use forestx_config::loader::load_config_layers_state;
use forestx_config::loader::project_trust_key;
use forestx_config::permissions_toml::PermissionsToml;
use forestx_config::sandbox_mode_requirement_for_permission_profile;
use forestx_config::types::ApprovalsReviewer;
use forestx_config::types::AuthCredentialsStoreMode;
use forestx_config::types::AuthKeyringBackendKind;
use forestx_config::types::History;
use forestx_config::types::McpServerConfig;
use forestx_config::types::McpServerDisabledReason;
use forestx_config::types::MemoriesConfig;
use forestx_config::types::ModelAvailabilityNuxConfig;
use forestx_config::types::Notice;
use forestx_config::types::OAuthCredentialsStoreMode;
use forestx_config::types::ResumeCwdMode;
use forestx_config::types::SessionPickerViewMode;
use forestx_config::types::ToolSuggestConfig;
use forestx_config::types::ToolSuggestDisabledTool;
use forestx_config::types::ToolSuggestDiscoverable;
use forestx_config::types::TuiKeymap;
use forestx_config::types::TuiNotificationSettings;
use forestx_config::types::TuiPetAnchor;
use forestx_config::types::UriBasedFileOpener;
use forestx_config::types::WindowsSandboxModeToml;
use forestx_core_plugins::PluginLoadOutcome;
use forestx_core_plugins::PluginsConfigInput;
use forestx_exec_server::ExecutorFileSystem;
use forestx_exec_server::LOCAL_FS;
use forestx_features::CodeModeConfigToml;
use forestx_features::CurrentTimeReminderConfigToml;
use forestx_features::CurrentTimeReminderDeliveryMode;
use forestx_features::CurrentTimeSource;
use forestx_features::Feature;
use forestx_features::FeatureConfigSource;
use forestx_features::FeatureOverrides;
use forestx_features::FeatureToml;
use forestx_features::Features;
use forestx_features::FeaturesToml;
use forestx_features::MultiAgentV2ConfigToml;
use forestx_features::NetworkProxyConfigToml;
use forestx_features::TokenBudgetConfigToml;
use forestx_features::TokenBudgetMode;
...
use forestx_install_context::InstallContext;
use forestx_login::AuthManagerConfig;
use forestx_login::AuthRouteConfig;
use forestx_mcp::McpConfig;
use forestx_mcp::McpPluginAttribution;
use forestx_mcp::McpProtocolMode;
use forestx_mcp::McpServerRegistration;
use forestx_mcp::ResolvedMcpCatalog;
use forestx_memories_read::memory_root;
use forestx_model_provider::ProviderCapabilities;
use forestx_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use forestx_model_provider_info::ModelProviderInfo;
use forestx_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use forestx_model_provider_info::built_in_model_providers;
use forestx_model_provider_info::merge_configured_model_providers;
use forestx_models_manager::ModelsManagerConfig;
use forestx_protocol::config_types::AltScreenMode;
use forestx_protocol::config_types::AutoCompactTokenLimitScope;
...
use forestx_protocol::config_types::SandboxMode;
use forestx_protocol::config_types::ServiceTier;
use forestx_protocol::config_types::ShellEnvironmentPolicy;
use forestx_protocol::config_types::TrustLevel;
use forestx_protocol::config_types::Verbosity;
use forestx_protocol::config_types::WebSearchConfig;
use forestx_protocol::config_types::WebSearchMode;
use forestx_protocol::config_types::WindowsSandboxLevel;
use forestx_protocol::models::ActivePermissionProfile;
use forestx_protocol::models::PermissionProfile;
use forestx_protocol::models::SandboxEnforcement;
use forestx_protocol::openai_models::ModelsResponse;
...
pub use forestx_thread_store::ExtraConfig;
use forestx_utils_absolute_path::AbsolutePathBuf;
use forestx_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_forestx_runtime;
...
pub use forestx_config::ConfigLoadOptions;
pub use forestx_config::Constrained;
pub use forestx_config::ConstraintError;
pub use forestx_config::ConstraintResult;
pub use forestx_config::LoaderOverrides;
pub use forestx_network_proxy::NetworkProxyAuditMetadata;
use forestx_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use forestx_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<AbsolutePathBuf> {
    let raw = std::env::var(forestx_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(forestx_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal Forestx-only writable roots are intentionally excluded.
...
// when Forestx is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<forestx_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Forestx will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Forestx
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.forestx/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Forestx"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Forestx '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Forestx home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Forestx can reach out to for tool calls.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Forestx unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/forestx/blob/main/forestx-rs/rmcp-client/src/oauth.rs#L2
    /// file: FORESTX_HOME/.credentials.json
    ///       This file will be readable to Forestx and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Forestx will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Forestx state (defaults to `~/.forestx` but can be
    /// overridden by the `FORESTX_HOME` environment variable).
    pub forestx_home: AbsolutePathBuf,

    /// Resolved configuration shared by all Forestx SQLite databases.
    pub sqlite: forestx_state::SqliteConfig,

    /// Directory where Forestx writes log files (defaults to `$FORESTX_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where Forestx writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores Forestx version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_forestx_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.forestx/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current Forestx executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub forestx_self_exe: Option<PathBuf>,

    /// Path to the `forestx-linux-sandbox` executable. This must be set if
    /// [`forestx_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `forestx-linux-sandbox`.
    pub forestx_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `forestx-execve-wrapper` executable used for shell
    ...
    /// Whether Forestx-owned clients should respect host system proxy settings.
    pub respect_system_proxy: bool,

    /// Process-only ChatGPT routing selection supplied when Forestx is launched.
    pub psp: bool,
    ...
    /// When `true`, checks for Forestx updates on startup and surfaces update prompts.
    /// Set to `false` only if your Forestx updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Forestx product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Forestx product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: forestx_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn forestx_home(&self) -> PathBuf {
        self.forestx_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn forestx_home(mut self, forestx_home: PathBuf) -> Self {
        self.forestx_home = Some(forestx_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let forestx_home = match forestx_home {
            Some(forestx_home) => AbsolutePathBuf::from_absolute_path(forestx_home)?,
            None => find_forestx_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&forestx_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = forestx_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    forestx_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(forestx_config::io_error_from_config_error(
        ...
        {
            let allow_forestx_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_forestx_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_forestx_version_mismatch = allow_forestx_version_mismatch;
...
impl Config {
    pub fn sqlite_config(&self) -> &forestx_state::SqliteConfig {
        &self.sqlite
    }
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &forestx_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            forestx_home: self.forestx_home.to_path_buf(),
            ...
            forestx_linux_sandbox_exe: self.forestx_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                forestx_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.forestx_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_forestx_home(
    ...
    /// Load a default configuration for a specific Forestx home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_forestx_home(
        ...
        let cli_layer = forestx_config::build_cli_overrides_layer(&cli_overrides);
        forestx_config::merge_toml_values(&mut merged, &cli_layer);
        let forestx_home = AbsolutePathBuf::from_absolute_path_checked(forestx_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &forestx_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `forestx exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `forestx_self_exe`, `forestx_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(forestx_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(forestx_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &forestx_config::NoopThreadConfigLoader,
    )
    .await?;

    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, forestx_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .forestx/ folders.
...
/// Patch `FORESTX_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(forestx_home: &Path, provider: &str) -> std::io::Result<()> {
    forestx_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &forestx_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            forestx_protocol::permissions::FileSystemSandboxEntry {
                path: forestx_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: forestx_protocol::permissions::FileSystemAccessMode::Deny,
            ...
            forestx_protocol::permissions::FileSystemSandboxEntry {
                path: forestx_protocol::permissions::FileSystemPath::Path { path },
                access: forestx_protocol::permissions::FileSystemAccessMode::Deny,
                missing_path_behavior: None,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&forestx_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&forestx_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(forestx_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| forestx_home.join("log").to_path_buf());
        ...
        let sqlite_home = cfg
            ...
            .cloned()
            .or(sqlite_home_env)
            .unwrap_or_else(|| forestx_home.clone());
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Forestx would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_forestx_runtime(
            &forestx_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            forestx_home,
            sqlite: forestx_state::SqliteConfig::from_sqlite_home(sqlite_home),
            log_dir,
            ...
            config_lock_allow_forestx_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_forestx_version_mismatch)
...
/// Returns the path to the Forestx configuration directory, which can be
/// specified by the `FORESTX_HOME` environment variable. If not set, defaults to
/// `~/.forestx`.
///
/// - If `FORESTX_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `FORESTX_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_forestx_home() -> std::io::Result<AbsolutePathBuf> {
    forestx_utils_home_dir::find_forestx_home()
}

/// Returns the path to the folder where Forestx logs are stored. Does not verify

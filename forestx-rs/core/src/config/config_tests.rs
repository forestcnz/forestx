...
use forestx_config::McpServerCommandMatcher;
use forestx_config::McpServerIdentity;
use forestx_config::McpServerRequirement;
use forestx_config::McpServerValueMatcher;
use forestx_config::ProfileV2Name;
use forestx_config::RequirementSource;
use forestx_config::Sourced;
use forestx_config::config_toml::AgentRoleToml;
use forestx_config::config_toml::AgentsToml;
use forestx_config::config_toml::AutoReviewToml;
use forestx_config::config_toml::ConfigToml;
use forestx_config::config_toml::ExperimentalRequestUserInput;
use forestx_config::config_toml::ProjectConfig;
use forestx_config::config_toml::RealtimeConfig;
use forestx_config::config_toml::RealtimeToml;
use forestx_config::config_toml::RealtimeTransport;
use forestx_config::config_toml::RealtimeWsMode;
use forestx_config::config_toml::RealtimeWsVersion;
use forestx_config::config_toml::ToolsToml;
...
use forestx_config::permissions_toml::NetworkDomainPermissionToml;
use forestx_config::permissions_toml::NetworkDomainPermissionsToml;
use forestx_config::permissions_toml::NetworkMitmActionToml;
use forestx_config::permissions_toml::NetworkMitmHookToml;
use forestx_config::permissions_toml::NetworkMitmToml;
...
use forestx_config::permissions_toml::WorkspaceRootsToml;
use forestx_config::types::AppToolApproval;
use forestx_config::types::ApprovalsReviewer;
use forestx_config::types::BundledSkillsConfig;
use forestx_config::types::FeedbackConfigToml;
use forestx_config::types::HistoryPersistence;
use forestx_config::types::McpServerEnvVar;
use forestx_config::types::McpServerOAuthConfig;
use forestx_config::types::McpServerToolConfig;
use forestx_config::types::McpServerTransportConfig;
use forestx_config::types::MemoriesConfig;
use forestx_config::types::MemoriesToml;
use forestx_config::types::ModelAvailabilityNuxConfig;
use forestx_config::types::Notice;
use forestx_config::types::NotificationCondition;
use forestx_config::types::NotificationMethod;
use forestx_config::types::Notifications;
use forestx_config::types::OtelConfigToml;
use forestx_config::types::OtelExporterKind;
use forestx_config::types::ResumeCwdMode;
use forestx_config::types::SandboxWorkspaceWrite;
use forestx_config::types::SessionPickerViewMode;
use forestx_config::types::SkillsConfig;
use forestx_config::types::ToolSuggestDisabledTool;
use forestx_config::types::ToolSuggestDiscoverableType;
use forestx_config::types::Tui;
...
use forestx_config::types::WindowsToml;
...
use forestx_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use forestx_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use forestx_model_provider_info::WireApi;
use forestx_models_manager::bundled_models_response;
use forestx_network_proxy::NetworkMode;
use forestx_protocol::config_types::ModelProviderAuthInfo;
...
use forestx_protocol::protocol::NetworkAccess;
use forestx_protocol::protocol::RealtimeVoice;
use forestx_protocol::protocol::SandboxPolicy;
use forestx_utils_path_uri::LegacyAppPathString;
...
use forestx_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: forestx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
async fn load_config_resolves_non_prefixed_mcp_tool_servers() -> std::io::Result<()> {
    ...
    for (config_contents, expected_servers, expected_prefix) in cases {
        ...
        let plugins_manager = PluginsManager::new(forestx_home.path().to_path_buf());
...
[features.code_mode]
enabled = true
excluded_tool_namespaces = ["mcp__forestx_apps", "multi_agent_v1"]
...
"#,
    ...
    assert_eq!(
        ...
        vec!["mcp__forestx_apps".to_string(), "multi_agent_v1".to_string()]
    );
    assert_eq!(
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        forestx_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        forestx_core_plugins::remote::RemotePluginServiceConfig::new(
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        forestx_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = forestx_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let forestx_home = temp_dir.path().join("forestx-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".forestx"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".forestx".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".forestx"), cwd.as_path()),
            "expected .forestx carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".forestx"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".forestx"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Forestx and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Forestx and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Forestx."
        )),
        "{:?}",
        config.startup_warnings
    );
                        ...
                        file_system_policy
                            ...
                            .contains(&FileSystemSandboxEntry {
                                ...
                                missing_path_behavior: Some(
                                    forestx_protocol::permissions::FileSystemSandboxEntryMissingPathBehavior::Skip,
                                ),
                            }),
...
fn filter_plugin_mcp_servers_without_allowlists_does_not_filter_any_plugin() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            forestx_config::PluginRequirementsToml { mcp_servers: None },
        )]),
        RequirementSource::LegacyManagedConfigTomlFromMdm,
    );
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, forestx_home.path());
    let project_dot_forestx =
        AbsolutePathBuf::resolve_path_against_base("project/.forestx", forestx_home.path());
    ...
    let requirements_toml = forestx_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = forestx_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_forestx_folder: project_dot_forestx.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(forestx_home.path().to_path_buf()),
            psp: Some(true),
            ..Default::default()
        },
        forestx_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_forestx_folder: project_dot_forestx,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let forestx_home = TempDir::new()?;
    let plugin_root = forestx_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".forestx-plugin"))?;
    std::fs::write(
        plugin_root.join(".forestx-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )?;
    std::fs::write(
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            forestx_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let default_zsh_path = forestx_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_forestx_home_for_workspace_write() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite.home(), forestx_home.path());
...
-> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let memories_root = forestx_home.path().join("memories");
    let writable_root = forestx_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let forestx_home = TempDir::new()?;
    let managed_path = forestx_home.path().join("managed_config.toml");
    let config_path = forestx_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), forestx_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(forestx_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let forestx_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(forestx_home.path());
    ...
    let loaded = load_global_mcp_servers(forestx_home.path()).await?;
...
async fn to_mcp_config_flows_mcp_2026_feature_from_config() -> std::io::Result<()> {
    ...
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(mcp_config.protocol_mode, forestx_mcp::McpProtocolMode::Legacy);

    let _ = config.features.enable(Feature::Mcp20260728);
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert_eq!(
        mcp_config.protocol_mode,
        forestx_mcp::McpProtocolMode::V20260728
    );
...
"#,
    ...
    let err = load_global_mcp_servers(forestx_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/forestx-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/forestx-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-forestx-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-forestx-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-forestx-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(forestx_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(forestx_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let forestx_home = TempDir::new()?;
    let base_config = forestx_home.path().join(CONFIG_TOML_FILE);
    let selected_config = forestx_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(forestx_home.path(), &profile_name),
        forestx_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn forestx_home(&self) -> AbsolutePathBuf {
        self.forestx_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let workspace = forestx_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, forestx_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        forestx_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let missing_path = forestx_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let role_config_path = forestx_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = forestx_config::ConfigLayerStack::new(
        vec![forestx_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: forestx_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        forestx_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".forestx").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let researcher_path = forestx_home.path().join("agents").join("researcher.toml");
    let reviewer_path = forestx_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = forestx_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let catalog_path = forestx_home.path().join("catalog.json");
...
"#;
    ...
    let forestx_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        forestx_home: forestx_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: forestx_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let forestx_home = fixture.forestx_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = forestx_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![forestx_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = forestx_config::RequirementSource::Unknown;
    ...
    let requirements = forestx_config::ConfigRequirements {
        web_search_mode: forestx_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        forestx_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/forestx4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/forestx3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/forestx2");
...
[projects."/Users/mbolin/code/forestx4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/forestx3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let forestx_home = temp_dir.path();
    let config_path = forestx_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(forestx_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(forestx_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(forestx_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(forestx_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(forestx_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let cwd = forestx_home.path().join("workspace");
    let outside_root = forestx_home.path().join("outside-write");
...
export_dir = "locks"
allow_forestx_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_forestx_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let lock_path = forestx_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
forestx_version = "older-version"
...
-> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let selected_config = forestx_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
async fn exact_requirements_apply_to_runtime_config() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let catalog_path = forestx_home.path().join("required-models.json");
    let mut catalog = bundled_models_response()
        .unwrap_or_else(|err| panic!("bundled models.json should parse: {err}"));
    catalog.models = catalog.models.into_iter().take(1).collect();
    std::fs::write(
...
"#,
    ...
    let required_sqlite_home = forestx_home.path().join("required-state");
    let required_log_dir = forestx_home.path().join("required-logs");
...
"#,
    ...
    let config = load_with_enterprise_requirement(&forestx_home, requirements).await?;
...
async fn absent_allow_login_shell_does_not_report_an_override() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let config = load_with_enterprise_requirement(&forestx_home, "allow_login_shell = false").await?;
...
fn sqlite_home_env_conflict_reports_an_override() -> std::io::Result<()> {
    let forestx_home = TempDir::new()?;
    let required = AbsolutePathBuf::try_from(forestx_home.path().join("required-state"))?;
    let environment = forestx_home.path().join("environment-state");
    ...
    assert_eq!(
        warnings,
        vec![format!(
            "Environment value for `$FORESTX_SQLITE_HOME` is overridden by the required `sqlite_home` value {required:?} from {}.",
            RequirementSource::Unknown
        )]
    );

...
use forestx_network_proxy::BlockedRequestObserver;
...
use forestx_network_proxy::NetworkDecision;
...
use forestx_network_proxy::host_and_port_from_network_addr;
#[cfg(any(target_os = "windows", test))]
use forestx_network_proxy::managed_proxy_ports;
use forestx_network_proxy::normalize_host;
use forestx_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    forestx_network_proxy::NetworkDomainPermission::Allow
                } else {
                    forestx_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }

...
use crate::forestx_thread::BackgroundTerminalInfo;
...
use forestx_analytics::TurnProfileFact;
use forestx_analytics::TurnTokenUsageFact;
use forestx_otel::SessionTelemetry;
use forestx_otel::TURN_E2E_DURATION_METRIC;
use forestx_otel::TURN_MEMORY_METRIC;
use forestx_otel::TURN_NETWORK_PROXY_METRIC;
use forestx_otel::TURN_TOKEN_USAGE_METRIC;
use forestx_otel::TURN_TOOL_CALL_METRIC;
...
const TASK_COMPACT_METRIC: &str = "forestx.task.compact";

pub(crate) type SessionTaskResult = ForestxResult<Option<String>>;
...
///
/// Implementations encapsulate a specific Forestx workflow (regular chat,
...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// [`ForestxErr::TurnAborted`] completes the task through the aborted-turn
    /// lifecycle instead.
    fn run(
        self: Arc<Self>,
        session: Arc<Session>,
...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let task_span = info_span!(
            ...
            forestx.turn.reasoning_effort = %reasoning_effort,
            forestx.turn.token_usage.input_tokens = field::Empty,
            forestx.turn.token_usage.cached_input_tokens = field::Empty,
            forestx.turn.token_usage.cache_write_input_tokens = field::Empty,
            forestx.turn.token_usage.non_cached_input_tokens = field::Empty,
            forestx.turn.token_usage.output_tokens = field::Empty,
            forestx.turn.token_usage.reasoning_output_tokens = field::Empty,
            forestx.turn.token_usage.total_tokens = field::Empty,
        );
                ...
                if let Err(err) = sess.flush_rollout().await {
                    ...
                    sess.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                            message: format!(
                                "Failed to save the conversation transcript; Forestx will continue retrying. Error: {err}"
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) fn has_outstanding_durable_sleep(&self) -> bool {
        self.services
            ...
            .get::<forestx_extension_items::sleep::SleepItem>()
            .is_some()
    }
    ...
    pub async fn on_task_finished(
        ...
        let (last_agent_message, abort_reason) = match task_result {
            ...
            Err(err) => {
                ...
                .await;
                self.track_turn_forestx_error(turn_context.as_ref(), &err);
                self.send_event(
                    turn_context.as_ref(),
        ...
        // Emit token usage metrics.
        {
            ...
            current_span.record(
                "forestx.turn.token_usage.input_tokens",
                turn_token_usage.input_tokens,
            );
            current_span.record(
                "forestx.turn.token_usage.cached_input_tokens",
                turn_token_usage.cached_input(),
            );
            current_span.record(
                "forestx.turn.token_usage.cache_write_input_tokens",
                turn_token_usage.cache_write_input_tokens,
            );
            current_span.record(
                "forestx.turn.token_usage.non_cached_input_tokens",
                turn_token_usage.non_cached_input(),
            );
            current_span.record(
                "forestx.turn.token_usage.output_tokens",
                turn_token_usage.output_tokens,
            );
            current_span.record(
                "forestx.turn.token_usage.reasoning_output_tokens",
                turn_token_usage.reasoning_output_tokens,
            );
            current_span.record(
                "forestx.turn.token_usage.total_tokens",

...
pub(crate) async fn execute_user_shell_command(
    ...
    session
        .services
        .session_telemetry
        .counter("forestx.task.user_shell", /*inc*/ 1, &[]);
    ...
    // feature does not need to project the selected environment cwd onto the Forestx host.
    let Ok(cwd) = turn_environment.cwd().to_abs_path() else {
        send_user_shell_error(
            &session,
            turn_context.as_ref(),
            "shell working directory is not native to the Forestx host",
        )
        .await;
        return;
    };

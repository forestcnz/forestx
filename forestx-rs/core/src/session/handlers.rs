...
use forestx_protocol::mcp::RequestId as ProtocolRequestId;
...
pub async fn resolve_elicitation(
    ...
    decision: forestx_protocol::approvals::ElicitationAction,
    content: Option<Value>,
    meta: Option<Value>,
) {
    let action = match decision {
        forestx_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
        forestx_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
        forestx_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
    };
    ...
    if let Err(err) = sess.flush_rollout().await {
        sess.send_event(
            ...
            EventMsg::Warning(WarningEvent {
                message: format!(
                    "Rolled the thread back, but failed to save the rollback marker. Forestx will continue retrying. Error: {err}"
                ),
            }),
        )
        .await;
...
async fn emit_thread_stop_lifecycle(sess: &Session) {
    for contributor in sess.services.extensions.thread_lifecycle_contributors() {
        contributor
            .on_thread_stop(forestx_extension_api::ThreadStopInput {
...
pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
    ...
    info!("Shutting down Forestx instance");
    ...
    sess.services.session_telemetry.counter(
        "forestx.conversation.turn.count",
    ...
    sess.services
        .rollout_thread_trace
        .record_ended(forestx_rollout_trace::RolloutStatus::Completed);
    true
}

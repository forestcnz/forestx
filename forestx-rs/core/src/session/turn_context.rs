...
use forestx_core_plugins::TrustedPluginRoots;
use forestx_core_skills::HostSkillsSnapshot;
use forestx_file_system::FileSystemSandboxContext;
...
pub struct TurnContext {
    ...
    pub(crate) extension_data: Arc<forestx_extension_api::ExtensionData>,
...
impl TurnContext {
    ...
    pub(crate) fn apps_enabled(&self) -> bool {
        let uses_forestx_backend = self
            .auth_manager
            .as_deref()
            .is_some_and(AuthManager::current_auth_uses_forestx_backend);
        self.config
            .features
            .apps_enabled_for_auth(uses_forestx_backend)
...
impl Session {
    ...
    pub(crate) fn make_turn_context(
        ...
        let unified_exec_shell_mode = UnifiedExecShellMode::for_session(
            forestx_tools::unified_exec_feature_mode_for_features(per_turn_config.features.get()),
        ...
        let extension_data = Arc::new(forestx_extension_api::ExtensionData::new(sub_id.clone()));
    ...
    pub(crate) async fn new_turn_with_sub_id(
    ...
    ) -> ForestxResult<Arc<TurnContext>> {
        let notify_config_contributors = !self.services.extensions.config_contributors().is_empty();
        let update_result: ForestxResult<_> = {
            ...
            match state.session_configuration.clone().apply(&updates) {
                ...
                Err(err) => Err(ForestxErr::InvalidRequest(err.to_string())),
            }
        };
        ...
        let (
            ...
            Err(err) => {
                ...
                .await;
                return Err(ForestxErr::InvalidRequest(message));
            }
        };
    ...
    async fn new_turn_context_from_configuration(
        ...
        let trusted_plugin_roots = TrustedPluginRoots::from_plugin_load_outcome(
            &plugin_outcome,
            per_turn_config.forestx_home.as_path(),
        );
    ...
    pub(crate) async fn maybe_emit_model_warnings_for_turn(&self, tc: &TurnContext) {
        ...
        if !tc.code_mode_available
            && matches!(
                ...
                forestx_protocol::openai_models::ToolMode::CodeMode
                    | forestx_protocol::openai_models::ToolMode::CodeModeOnly
            )

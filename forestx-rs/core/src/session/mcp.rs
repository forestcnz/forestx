...
use super::*;
use forestx_exec_server::ExecutorCapabilityDiscoveryCache;
use forestx_exec_server::ExecutorCapabilityDiscoverySnapshot;
use forestx_exec_server::FileSystemSandboxContext;
use forestx_exec_server::MAX_SELECTED_CAPABILITY_ROOTS;
use forestx_exec_server::ResolvedSelectedCapabilityRoot;
use forestx_mcp::FORESTX_APPS_MCP_SERVER_NAME;
use forestx_mcp::ElicitationReviewRequest;
use forestx_mcp::ElicitationReviewer;
use forestx_mcp::ElicitationReviewerHandle;
...
use forestx_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_ELICITATION_APPROVAL_KIND_KEY;
use forestx_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_ELICITATION_APPROVAL_KIND_MCP_TOOL_CALL;
use forestx_protocol::mcp_approval_meta::APPROVAL_KIND_TOOL_SUGGESTION as MCP_ELICITATION_APPROVAL_KIND_TOOL_SUGGESTION;
use forestx_protocol::mcp_approval_meta::APPROVALS_REVIEWER_KEY as MCP_ELICITATION_APPROVALS_REVIEWER_KEY;
use forestx_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_ELICITATION_CONNECTOR_DESCRIPTION_KEY;
use forestx_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_ELICITATION_CONNECTOR_ID_KEY;
use forestx_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_ELICITATION_CONNECTOR_NAME_KEY;
use forestx_protocol::mcp_approval_meta::PERSIST_KEY as MCP_ELICITATION_PERSIST_KEY;
use forestx_protocol::mcp_approval_meta::REQUEST_TYPE_APPROVAL_REQUEST as MCP_ELICITATION_REQUEST_TYPE_APPROVAL_REQUEST;
use forestx_protocol::mcp_approval_meta::REQUEST_TYPE_KEY as MCP_ELICITATION_REQUEST_TYPE_KEY;
use forestx_protocol::mcp_approval_meta::STRICT_AUTO_REVIEW_KEY as MCP_ELICITATION_STRICT_AUTO_REVIEW_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_ELICITATION_TOOL_DESCRIPTION_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_NAME_KEY as MCP_ELICITATION_TOOL_NAME_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_ELICITATION_TOOL_PARAMS_KEY;
use forestx_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_ELICITATION_TOOL_TITLE_KEY;
use forestx_rmcp_client::Elicitation;
...
impl Session {
    ...
    pub(crate) async fn runtime_mcp_servers(
        ...
        forestx_mcp::configured_mcp_servers(&self.runtime_mcp_config(config).await)
    }
    ...
    pub(crate) async fn refresh_mcp_if_dirty(self: &Arc<Self>) {
        ...
        loop {
            ...
            if self
                .services
                .plugins_manager
                .set_auth_mode(auth.as_ref().map(ForestxAuth::api_auth_mode))
                || !self
                    .services
                    .mcp_runtime
    ...
    pub(crate) async fn hard_refresh_latest_forestx_apps_tools(
        self: &Arc<Self>,
    ) -> anyhow::Result<Vec<forestx_mcp::ToolInfo>> {
        ...
        anyhow::ensure!(
            input.mcp_servers.contains_key(FORESTX_APPS_MCP_SERVER_NAME),
            "unknown MCP server '{FORESTX_APPS_MCP_SERVER_NAME}'"
        );
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        Arc::new(forestx_mcp::McpBinding::empty(config))
    }
    ...
    pub(crate) fn mcp_elicitation_lifecycle(&self) -> forestx_mcp::ElicitationLifecycle {
        self.mcp_elicitation_lifecycle_handle
            .get_or_init(|| {
                let elicitations = self.services.elicitations.clone();
                forestx_mcp::ElicitationLifecycle::new(move || elicitations.register())
            })
            .clone()
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        if self.services.mcp_runtime.elicitations_auto_deny() {
            return McpServerElicitationOutcome {
                response: Some(ElicitationResponse {
                    action: forestx_rmcp_client::ElicitationAction::Accept,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                forestx_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                forestx_protocol::mcp::RequestId::Integer(value)
            }
        };
...
async fn review_guardian_mcp_elicitation(
    ...
    if matches!(
        ...
        let trusted_guardian_request = if request.server_name == FORESTX_APPS_MCP_SERVER_NAME {
        ...
        let decision = crate::guardian::review_approval_request_with_cancel(
            ...
            crate::guardian::GuardianReviewOptions {
                ...
                approval_request_source: forestx_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    match approval_policy {
        AskForApproval::Never => {
            if forestx_mcp::mcp_permission_prompt_is_auto_approved(
                approval_policy,
                &mcp_config.permission_profile,
                forestx_mcp::McpPermissionPromptAutoApproveContext::default(),
            ) && matches!(
                &request.elicitation,
                Elicitation::Mcp(
...
fn plugin_install_elicitation_telemetry_metadata(
    ...
    let forestx_protocol::approvals::ElicitationRequest::Form {

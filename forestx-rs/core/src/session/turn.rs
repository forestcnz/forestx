...
use forestx_analytics::CompactionPhase;
use forestx_analytics::CompactionReason;
use forestx_analytics::InvocationType;
use forestx_analytics::TurnResolvedConfigFact;
...
use forestx_core_skills::injection::InjectedHostSkillPrompts;
use forestx_extension_api::ExtensionData;
use forestx_extension_api::TurnInputContext;
use forestx_extension_api::TurnInputEnvironment;
use forestx_features::Feature;
use forestx_file_system::FindUpErrorPolicy;
use forestx_file_system::find_nearest_ancestor_with_markers;
use forestx_login::ForestxAuth;
use forestx_model_provider::RemoteCompactionSupport;
...
use forestx_protocol::items::PlanItem;
...
use forestx_protocol::protocol::AgentMessageContentDeltaEvent;
use forestx_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use forestx_protocol::protocol::PlanDeltaEvent;
use forestx_protocol::protocol::RawResponseCompletedEvent;
use forestx_protocol::protocol::ReasoningContentDeltaEvent;
use forestx_protocol::protocol::ReasoningRawContentDeltaEvent;
use forestx_protocol::protocol::SafetyBufferingEvent;
...
use forestx_tools::DiscoverableTool;
use forestx_tools::ToolName;
use forestx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use forestx_utils_path_uri::PathUri;
use forestx_utils_stream_parser::AssistantTextChunk;
use forestx_utils_stream_parser::AssistantTextStreamParser;
use forestx_utils_stream_parser::ProposedPlanSegment;
use forestx_utils_stream_parser::extract_proposed_plan_text;
use forestx_utils_stream_parser::strip_citations;
...
const POST_SAMPLING_TOKEN_ESTIMATE_TARGET: &str = "forestx_core::post_sampling_token_estimate";
...
pub(crate) async fn run_turn(
    ...
    .await
    {
        if matches!(err.details(), ForestxErrorDetails::TurnAborted) {
            run_hooks_and_record_inputs(&sess, &turn_context, &input).await;
            return Err(err);
        }
        if matches!(err.details(), ForestxErrorDetails::ToolCollision(_)) {
            return Err(err);
        }
        let error = err.to_forestx_protocol_error();
        ...
        .await
    {
        ...
        Err(err) if matches!(err.details(), ForestxErrorDetails::TurnAborted) => {
            run_hooks_and_record_inputs(&sess, &turn_context, &input).await;
            return Err(err);
        }
    ...
    // Although from the perspective of forestx.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: ForestxResult<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                ForestxResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            ...
            Err(forestx_error)
                if matches!(
                    forestx_error.details(),
                    ForestxErrorDetails::InvalidImageRequest()
                ) =>
            {
                sess.track_turn_forestx_error(turn_context.as_ref(), &forestx_error);
                let error = ForestxErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    forestx_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_forestx_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_forestx_error(turn_context.as_ref(), &e);
...
async fn required_mcp_servers_for_input(
    ...
    let connector_slug_counts = if turn_context.apps_enabled() && !mentions.plain_names.is_empty() {
        ...
        build_connector_slug_counts(
            &forestx_connectors::merge::merge_plugin_connectors_with_accessible(
                connector_ids,
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = forestx_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
                .map(|connector_id| connector_id.0.clone()),
...
///
/// Returns `None` when the active authentication does not use the Forestx backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    previous_model: &str,
    cancellation_token: &CancellationToken,
) -> ForestxResult<Option<Arc<StepContext>>> {
    let uses_forestx_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(forestx_login::AuthManager::current_auth_uses_forestx_backend);
    if !uses_forestx_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = forestx_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<forestx_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &ForestxResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> ForestxResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        {
            forestx_protocol::models::bound_executed_tool_calls_for_prompt(&mut prompt_input);
        }
        ...
        .await
        {
            ...
            Err(err) => match err.details() {
                ...
                ForestxErrorDetails::UsageLimitReached(e) => {
...
pub(super) fn agent_message_text(item: &forestx_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            forestx_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: forestx_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(forestx_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &forestx_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, ForestxResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> ForestxResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> ForestxResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, ForestxResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: ForestxResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            forestx.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            forestx.usage.reasoning_output_tokens = field::Empty,
            forestx.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(forestx_async_utils::CancelErr::Cancelled) => {
                break Err(ForestxErr::TurnAborted);
            }
        };
                ...
                if analytics_tool_call_ids.len() < MAX_ANALYTICS_TOOL_CALL_IDS_PER_RESPONSE {
                    let call_id = match &item {
                        ...
                        | ResponseItem::ImageGenerationCall { id, .. } => {
                            id.as_ref().map(forestx_protocol::ResponseItemId::as_str)
                        }
                        _ => None,
                    };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![forestx_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
            ...
            ResponseEvent::Completed {
                ...
                sess.services
                    ...
                    .track_code_mode_tool_call(
                        forestx_analytics::CodeModeToolCallFact::SamplingResponseCompleted {
                            thread_id: sess.thread_id.to_string(),
                            turn_id: turn_context.sub_id.clone(),
    ...
    if cancellation_token.is_cancelled() {
        return Err(ForestxErr::TurnAborted);
    }

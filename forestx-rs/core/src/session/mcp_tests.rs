...
fn guardian_meta(tool_params: Option<Value>) -> Option<RequestMetaObject> {
    let mut value = json!({
        "forestx_approval_kind": "mcp_tool_call",
        "forestx_request_type": "approval_request",
...
fn plugin_install_elicitation_telemetry_metadata_requires_install_tool_suggestion() {
    let event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        ...
        id: forestx_protocol::mcp::RequestId::String("request-1".to_string()),
        request: forestx_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
    ...
    let enable_event = EventMsg::ElicitationRequest(ElicitationRequestEvent {
        turn_id: Some("turn-1".to_string()),
        server_name: "forestx_apps".to_string(),
        id: forestx_protocol::mcp::RequestId::String("request-2".to_string()),
        request: forestx_protocol::approvals::ElicitationRequest::Form {
            meta: Some(json!({
                "forestx_approval_kind": "tool_suggestion",
                "suggest_type": "enable",

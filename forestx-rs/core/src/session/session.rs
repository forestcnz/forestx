...
pub(crate) struct Session {
    ...
    pub(super) mcp_elicitation_reviewer_handle: OnceLock<forestx_mcp::ElicitationReviewerHandle>,
    pub(super) mcp_elicitation_lifecycle_handle: OnceLock<forestx_mcp::ElicitationLifecycle>,
...
pub(crate) struct SessionConfiguration {
    ...
    /// Directory containing all Forestx state for this session.
    pub(super) forestx_home: AbsolutePathBuf,
...
impl SessionConfiguration {
    ...
    pub(crate) fn forestx_home(&self) -> &AbsolutePathBuf {
        &self.forestx_home
    }
    ...
    pub(super) fn sandbox_policy(&self) -> SandboxPolicy {
        ...
        forestx_sandboxing::compatibility_sandbox_policy_for_permission_profile(
            &permission_profile,
            self.cwd(),
        )
    }
            ...
            if let Some(active_permission_profile) = next_configuration.active_permission_profile()
            {
                ...
                config.permissions.network = config
                    ...
                    .map_err(|err| ConstraintError::InvalidValue {
                        ...
                        requirement_source: forestx_config::RequirementSource::Unknown,
                    })?;
                config
                    .permissions
...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        config: Arc<Config>,
        user_instructions: Option<forestx_extension_api::UserInstructions>,
        ...
        code_mode_session_provider: Arc<dyn forestx_code_mode::CodeModeSessionProvider>,
        extensions: Arc<forestx_extension_api::ExtensionRegistry<crate::config::Config>>,
        ...
        windows_sandbox_proxy_settings_mode: forestx_sandboxing::WindowsSandboxProxySettingsMode,
        ...
        thread_extension_init.insert(forestx_extension_api::ThreadOriginator(
            session_configuration.originator.clone(),
        ));
        let mcp_thread_init = thread_extension_init.clone();
        let thread_extension_data = forestx_extension_api::ExtensionData::new_with_init(
            thread_id.to_string(),
        ...
        let session_result: anyhow::Result<Arc<Self>> = async {
            ...
            let config_path = config.forestx_home.join(CONFIG_TOML_FILE);
            ...
            let auth_mode = telemetry_auth
                .map(ForestxAuth::auth_mode)
                .map(TelemetryAuthMode::from);
            let account_id = telemetry_auth.and_then(ForestxAuth::get_account_id);
            let account_email = telemetry_auth.and_then(ForestxAuth::get_account_email);
            let originator = session_configuration.originator.clone();
            let terminal_type = user_agent();
            let session_model = session_configuration.collaboration_mode.model().to_string();
            let auth_env_telemetry = collect_auth_env_telemetry(
                session_configuration.provider.info(),
                auth_manager.forestx_api_key_env_enabled(),
            );
            ...
            let mcp_server_names =
                forestx_mcp::effective_mcp_servers(
            ...
            let session_extension_data =
                forestx_extension_api::ExtensionData::new(session_id.to_string());
            ...
            for contributor in extensions.thread_lifecycle_contributors() {
                contributor.on_thread_start(forestx_extension_api::ThreadStartInput {
                ...
                || mcp_auth_changes.has_changed().unwrap_or(false)
            {
                sess.services
                    .plugins_manager
                    .set_auth_mode(latest_auth.as_ref().map(ForestxAuth::api_auth_mode));
                sess.services
                    .mcp_manager
                    .runtime_config_for_step(
                        config.as_ref(),
            ...
            let session_start_source = match &initial_history {
                InitialHistory::Resumed(_) => forestx_hooks::SessionStartSource::Resume,
                InitialHistory::New | InitialHistory::Forked(_) => {
                    forestx_hooks::SessionStartSource::Startup
                }
                InitialHistory::Cleared => forestx_hooks::SessionStartSource::Clear,
            };

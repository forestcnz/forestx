...
use forestx_protocol::models::AdditionalPermissionProfile as PermissionProfile;
...
use core_test_support::forestx_linux_sandbox_exe_or_skip;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                forestx_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    config
        .permissions
        .set_permission_profile(forestx_protocol::models::PermissionProfile::Disabled)
    ...
    config.forestx_linux_sandbox_exe = forestx_linux_sandbox_exe_or_skip!();
    config
        .features
        .enable(Feature::GuardianApproval)
        .expect("test setup should allow enabling guardian approvals");
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        forestx_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: forestx_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    let forestx_home = tempdir().expect("create forestx home");
    ...
    config.config_layer_stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_forestx_folder: project_dir.path().abs(),
            },
    ...
    let thread_store = Arc::new(forestx_thread_store::LocalThreadStore::new(
        forestx_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let (session, io) = Session::spawn(SessionSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        code_mode_session_provider: Arc::new(forestx_code_mode::DisabledCodeModeSessionProvider),
        extensions: forestx_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: forestx_rollout_trace::ThreadTraceContext::disabled(),
        user_shell_override: None,
        parent_trace: None,
        environment_selections: Vec::new(),
        thread_extension_init: forestx_extension_api::ExtensionDataInit::default(),
        ...
        windows_sandbox_proxy_settings_mode:
            forestx_sandboxing::WindowsSandboxProxySettingsMode::Preserve,
    })
    .await

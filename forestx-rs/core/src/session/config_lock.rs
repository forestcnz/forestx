...
use forestx_config::config_toml::OrchestratorFeatureToml;
use forestx_config::config_toml::OrchestratorToml;
...
use forestx_features::RolloutBudgetConfigToml;
use forestx_features::TokenBudgetConfigToml;
use forestx_features::ToolRegistryConfigToml;
...
pub(crate) async fn validate_config_lock_if_configured(
    ...
    let options = ConfigLockReplayOptions {
        allow_forestx_version_mismatch: config.config_lock_allow_forestx_version_mismatch,
    };
...
/// Forestx actually ran with, not only the user-authored TOML inputs.
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        config.token_budget = Some(crate::config::TokenBudgetConfig {
            mode: forestx_features::TokenBudgetMode::Thread,
        ...
        for spec in forestx_features::FEATURES {
            assert_eq!(
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(TokenBudgetConfigToml {
                enabled: Some(true),
                mode: Some(forestx_features::TokenBudgetMode::Thread),
                reminder_threshold_tokens: Some(16_000),
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(CurrentTimeReminderConfigToml {
                ...
                clock_source: Some(forestx_features::CurrentTimeSource::System),
                delivery_mode: Some(forestx_features::CurrentTimeReminderDeliveryMode::AnyInference),
    ...
    async fn lock_contains_exact_managed_requirements() {
        ...
        let sqlite_home = forestx_home.path().join("managed-state");
        let log_dir = forestx_home.path().join("managed-logs");
        let catalog_path = forestx_home.path().join("managed-models.json");
...
"#,
    ...
    async fn lock_drops_unmanaged_model_catalog_input() {
        let forestx_home = tempfile::tempdir().expect("create temp dir");
        let catalog_path = forestx_home.path().join("user-models.json");
        write_model_catalog(&catalog_path);
    ...
    #[tokio::test]
    async fn lock_validation_rejects_forestx_version_mismatch_by_default() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.forestx_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock Forestx version mismatch"),
            "{message}"
        );
        assert!(
            message.contains("debug.config_lockfile.allow_forestx_version_mismatch=true"),
            "{message}"
        );
    }

    #[tokio::test]
    async fn lock_validation_can_ignore_forestx_version_mismatch() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.forestx_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");

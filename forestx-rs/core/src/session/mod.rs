...
use forestx_analytics::ImagePreparationFact;
use forestx_analytics::ImagePreparationMetadata;
use forestx_analytics::SubAgentThreadStartedInput;
use forestx_analytics::TurnForestxErrorFact;
use forestx_async_utils::OrCancelExt;
use forestx_connectors::connector_runtime_context_key;
use forestx_exec_server::Environment;
use forestx_exec_server::EnvironmentManager;
use forestx_execpolicy::prefix_rule_migration;
use forestx_extension_api::ExtensionDataInit;
use forestx_extension_api::LoadedUserInstructions;
use forestx_extension_api::PromptFragment;
use forestx_extension_api::PromptSlot;
use forestx_extension_api::TurnContextContributionInput;
use forestx_features::FEATURES;
use forestx_features::Feature;
use forestx_features::unstable_features_warning_event;
use forestx_hooks::Hooks;
use forestx_hooks::HooksConfig;
use forestx_login::AuthManager;
use forestx_login::ForestxAuth;
use forestx_login::auth_env_telemetry::collect_auth_env_telemetry;
use forestx_mcp::McpResourceClient;
use forestx_mcp::McpRuntime;
use forestx_mcp::McpRuntimeContext;
use forestx_mcp::McpRuntimeInput;
use forestx_models_manager::manager::RefreshStrategy;
use forestx_models_manager::manager::SharedModelsManager;
use forestx_network_proxy::NetworkProxy;
use forestx_network_proxy::NetworkProxyAuditMetadata;
use forestx_network_proxy::normalize_host;
use forestx_otel::current_span_trace_id;
use forestx_otel::current_span_w3c_trace_context;
use forestx_otel::set_parent_from_w3c_trace_context;
use forestx_protocol::SessionId;
use forestx_protocol::ThreadId;
use forestx_protocol::approvals::ElicitationRequest;
use forestx_protocol::approvals::ElicitationRequestEvent;
use forestx_protocol::approvals::ExecPolicyAmendment;
use forestx_protocol::approvals::NetworkPolicyAmendment;
use forestx_protocol::approvals::NetworkPolicyRuleAction;
...
use forestx_protocol::dynamic_tools::DynamicToolResponse;
use forestx_protocol::dynamic_tools::DynamicToolSpec;
use forestx_protocol::items::EnteredReviewModeItem;
use forestx_protocol::items::TurnItem;
use forestx_protocol::items::UserMessageItem;
...
use forestx_protocol::protocol::AdditionalContextEntry;
use forestx_protocol::protocol::FileChange;
use forestx_protocol::protocol::HasLegacyEvent;
use forestx_protocol::protocol::HistoryPosition;
...
use forestx_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
use forestx_protocol::protocol::MultiAgentVersion;
use forestx_protocol::protocol::RawResponseItemEvent;
...
use forestx_protocol::protocol::ThreadHistoryMode;
use forestx_protocol::protocol::ThreadSource;
use forestx_protocol::protocol::TurnAbortReason;
use forestx_protocol::protocol::TurnContextItem;
use forestx_protocol::protocol::TurnContextNetworkItem;
...
use forestx_protocol::protocol::WorldStateItem;
...
use forestx_protocol::request_permissions::RequestPermissionsArgs;
use forestx_protocol::request_permissions::RequestPermissionsEvent;
use forestx_protocol::request_permissions::RequestPermissionsResponse;
use forestx_protocol::request_user_input::RequestUserInputArgs;
use forestx_protocol::request_user_input::RequestUserInputResponse;
use forestx_rmcp_client::ElicitationResponse;
use forestx_rollout::state_db;
use forestx_rollout_trace::AgentResultTracePayload;
use forestx_rollout_trace::ThreadStartedTraceMetadata;
use forestx_rollout_trace::ThreadTraceContext;
use forestx_sandboxing::policy_transforms::intersect_permission_profiles;
use forestx_shell_command::parse_command::parse_command;
use forestx_terminal_detection::user_agent;
use forestx_thread_store::CreateThreadParams;
use forestx_thread_store::LiveThread;
use forestx_thread_store::LiveThreadInitGuard;
use forestx_thread_store::LocalThreadStore;
use forestx_thread_store::ReadThreadParams;
use forestx_thread_store::ResumeThreadParams;
use forestx_thread_store::ThreadPersistenceMetadata;
use forestx_thread_store::ThreadStore;
use forestx_utils_audio::prepare_response_items as prepare_audio_response_items;
...
use crate::forestx_thread::ThreadConfigSnapshot;
#[cfg(test)]
...
#[cfg(test)]
use forestx_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                forestx_error_info: Some(ForestxErrorInfo::BadRequest),
            },
...
use forestx_core_plugins::PluginCommandAttribution;
use forestx_core_plugins::PluginsManager;
use forestx_core_plugins::RecommendedPluginCandidatesInput;
use forestx_git_utils::get_git_repo_root;
use forestx_mcp::McpConfig;
use forestx_mcp::effective_mcp_servers;
use forestx_otel::SessionTelemetry;
use forestx_otel::THREAD_STARTED_METRIC;
...
use forestx_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use forestx_protocol::config_types::WindowsSandboxLevel;
use forestx_protocol::mcp::ClientMcpExtensions;
use forestx_protocol::models::LocalImagePreparation;
use forestx_protocol::models::ResponseInputItem;
use forestx_protocol::models::ResponseItem;
use forestx_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use forestx_protocol::protocol::ApplyPatchApprovalRequestEvent;
...
use forestx_protocol::protocol::DeprecationNoticeEvent;
use forestx_protocol::protocol::ErrorEvent;
...
use forestx_protocol::protocol::ModelRerouteEvent;
use forestx_protocol::protocol::ModelRerouteReason;
use forestx_protocol::protocol::ModelVerification;
use forestx_protocol::protocol::ModelVerificationEvent;
use forestx_protocol::protocol::NetworkApprovalContext;
...
use forestx_protocol::protocol::RequestUserInputEvent;
...
use forestx_protocol::protocol::SessionNetworkProxyRuntime;
use forestx_protocol::protocol::StreamErrorEvent;
...
use forestx_protocol::protocol::TurnModerationMetadataEvent;
use forestx_protocol::protocol::WarningEvent;
use forestx_protocol::user_input::UserInput;
use forestx_tools::UnifiedExecShellMode;
use forestx_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use forestx_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn forestx_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<forestx_extension_api::ExtensionRegistry<crate::config::Config>>,
    ...
    pub(crate) windows_sandbox_proxy_settings_mode:
        forestx_sandboxing::WindowsSandboxProxySettingsMode,
}
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/forestx/concepts/cyber-safety";

impl Session {
    /// Spawn and initialize a new session.
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> ForestxResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if forestx_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> ForestxResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            {
                let forestx_home = config.forestx_home.clone();
                let policy_path = default_policy_path(forestx_home.as_path());
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| ForestxErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let mut config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            forestx_models_manager::manager::RefreshStrategy::Offline
        } else {
            forestx_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.forestx_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> ForestxResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> ForestxResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| ForestxErr::InternalAgentDied)?;
        Ok(())
    }

    pub(crate) async fn shutdown_and_wait(&self) -> ForestxResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(err) if matches!(err.details(), ForestxErrorDetails::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> ForestxResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> ForestxResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-forestx-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &forestx_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn forestx_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn forestx_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn forestx_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.forestx_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Forestx performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(&self) -> Option<forestx_extension_api::UserInstructions> {
    ...
    /// Record a terminal ForestxErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_forestx_error(&self, turn_context: &TurnContext, error: &ForestxErr) {
        self.services
            .analytics_events_client
            .track_turn_forestx_error(TurnForestxErrorFact::from_forestx_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .forestx_error_info
                .as_ref()
                .is_some_and(ForestxErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_forestx_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &forestx_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| forestx_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => forestx_rollout::existing_rollout_path(&path).await.is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&forestx_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the Forestx host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                GuardianReviewOptions {
                    ...
                    approval_request_source:
                        forestx_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn capture_step_context_with_required_mcp_servers(
        ...
        let executor_capability_discovery = self
            ...
            .await;
        let extension_data = forestx_extension_api::ExtensionData::new(turn_context.sub_id.clone());
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-forestx, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(forestx_hooks::SessionStartSource::Compact);
        }
    }
            ...
            && turn_context.model_context_window().is_some()
        {
            ...
            developer_sections.push(
                crate::context::TokenBudgetContext::new(
                    ...
                    session_source
                        .get_agent_path()
                        .unwrap_or_else(forestx_protocol::AgentPath::root),
                    turn_context
                        .config
                        .token_budget
                        .as_ref()
    ...
    pub(crate) async fn notify_stream_error(
        ...
        forestx_error: ForestxErr,
    ) {
        let additional_details = forestx_error.to_string();
        let forestx_error_info = ForestxErrorInfo::ResponseStreamDisconnected {
            http_status_code: forestx_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<forestx_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::ForestxHooks),

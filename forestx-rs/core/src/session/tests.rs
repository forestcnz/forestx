...
use crate::forestx_thread::TryStartTurnIfIdleRejectionReason;
...
use forestx_config::NetworkConstraints;
use forestx_config::NetworkDomainPermissionToml;
use forestx_config::NetworkDomainPermissionsToml;
use forestx_config::RequirementSource;
use forestx_config::Sourced;
use forestx_config::loader::project_trust_key;
use forestx_config::types::McpServerConfig;
use forestx_config::types::McpServerTransportConfig;
use forestx_config::types::ToolSuggestDisabledTool;
use core_test_support::test_forestx::local_selections;

use forestx_features::Feature;
use forestx_http_client::ClientRouteClass;
use forestx_http_client::HttpClientFactory;
use forestx_http_client::OutboundProxyPolicy;
use forestx_http_client::RouteAwareClientPool;
use forestx_login::ForestxAuth;
use forestx_login::auth::AgentIdentityAuthPolicy;
...
use forestx_models_manager::model_info;
...
use forestx_protocol::ResponseItemId;
use forestx_protocol::SessionId;
use forestx_protocol::ThreadId;
use forestx_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use forestx_protocol::config_types::ServiceTier;
use forestx_protocol::config_types::TrustLevel;
use forestx_protocol::exec_output::ExecToolCallOutput;
use forestx_protocol::models::AgentMessageInputContent;
use forestx_protocol::models::FileSystemPermissions;
use forestx_protocol::models::FunctionCallOutputBody;
...
use forestx_protocol::models::SandboxEnforcement;
use forestx_protocol::openai_models::ModelServiceTier;
use forestx_protocol::permissions::FileSystemAccessMode;
use forestx_protocol::permissions::FileSystemPath;
use forestx_protocol::permissions::FileSystemSandboxEntry;
use forestx_protocol::permissions::FileSystemSandboxPolicy;
use forestx_protocol::permissions::FileSystemSpecialPath;
use forestx_protocol::protocol::NonSteerableTurnKind;
use forestx_protocol::protocol::SandboxPolicy;
use forestx_protocol::protocol::TurnEnvironmentSelections;
use forestx_protocol::request_permissions::PermissionGrantScope;
use forestx_protocol::request_permissions::RequestPermissionProfile;
...
use forestx_config::config_toml::ConfigToml;
use forestx_config::config_toml::ProjectConfig;
use forestx_config::permissions_toml::FilesystemPermissionToml;
use forestx_config::permissions_toml::FilesystemPermissionsToml;
use forestx_config::permissions_toml::NetworkToml;
use forestx_config::permissions_toml::PermissionProfileToml;
use forestx_config::permissions_toml::PermissionsToml;
use forestx_execpolicy::Decision;
use forestx_execpolicy::NetworkRuleProtocol;
use forestx_execpolicy::Policy;
use forestx_network_proxy::NetworkProxyConfig;
use forestx_otel::MetricsClient;
use forestx_otel::MetricsConfig;
use forestx_otel::TelemetryAuthMode;
use forestx_protocol::config_types::CollaborationMode;
use forestx_protocol::config_types::ModeKind;
use forestx_protocol::config_types::Settings;
use forestx_protocol::items::HookPromptFragment;
use forestx_protocol::items::build_hook_prompt_message;
use forestx_protocol::models::BaseInstructions;
use forestx_protocol::models::ContentItem;
use forestx_protocol::models::InternalChatMessageMetadataPassthrough;
use forestx_protocol::models::ResponseItem;
use forestx_protocol::protocol::AskForApproval;
use forestx_protocol::protocol::ForestxErrorInfo;
use forestx_protocol::protocol::CompactedItem;
use forestx_protocol::protocol::ConversationAudioParams;
use forestx_protocol::protocol::CreditsSnapshot;
use forestx_protocol::protocol::GranularApprovalConfig;
use forestx_protocol::protocol::InitialHistory;
use forestx_protocol::protocol::InterAgentCommunication;
use forestx_protocol::protocol::MultiAgentVersion;
use forestx_protocol::protocol::NetworkApprovalProtocol;
use forestx_protocol::protocol::RateLimitSnapshot;
use forestx_protocol::protocol::RateLimitWindow;
use forestx_protocol::protocol::RealtimeAudioFrame;
use forestx_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use forestx_protocol::protocol::RealtimeVoice;
...
use forestx_protocol::protocol::SessionMeta;
use forestx_protocol::protocol::SessionMetaLine;
use forestx_protocol::protocol::Submission;
use forestx_protocol::protocol::ThreadRolledBackEvent;
use forestx_protocol::protocol::ThreadSettingsOverrides;
use forestx_protocol::protocol::TokenCountEvent;
use forestx_protocol::protocol::TokenUsage;
use forestx_protocol::protocol::TokenUsageInfo;
use forestx_protocol::protocol::TurnAbortedEvent;
use forestx_protocol::protocol::TurnCompleteEvent;
use forestx_protocol::protocol::TurnStartedEvent;
use forestx_protocol::protocol::UserMessageEvent;
use forestx_protocol::protocol::W3cTraceContext;
use forestx_rmcp_client::ElicitationAction;
...
use core_test_support::test_forestx::local;
...
use forestx_protocol::mcp::CallToolResult as McpCallToolResult;
...
pub(crate) fn mcp_config_for_test(config: &crate::config::Config) -> Arc<forestx_mcp::McpConfig> {
    Arc::new(config.to_mcp_config_with_loaded_plugins(
        &forestx_core_plugins::PluginLoadOutcome::default(),
        std::iter::empty(),
    ))
}
...
impl StepContext {
    pub(crate) fn for_test(turn: Arc<TurnContext>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            mcp: Arc::new(forestx_mcp::McpBinding::empty(mcp_config_for_test(
                &turn.config,
            ))),
...
fn extension_metrics_preserve_session_metadata_tags() {
    ...
    let attributes = single_histogram_attributes(&snapshot, "forestx.test.extension");
    assert_eq!(
        attributes,
        BTreeMap::from([
            (
...
async fn world_state_extension_metrics_follow_turn_model_switch() {
    ...
    impl forestx_extension_api::ContextContributor for WorldStateMetricsRecorder {
        fn contribute_world_state<'a>(
            &'a self,
            input: forestx_extension_api::WorldStateContributionInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<
            'a,
            Vec<forestx_extension_api::WorldStateSectionContribution>,
        > {
            Box::pin(async move {
                input
                    .extension_metrics
                    .expect("turn metrics should be available")
                    .histogram("forestx.test.extension.turn", /*value*/ 1, &[]);
                Vec::new()
            })
        }
    }
    ...
    let mut builder = forestx_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
    ...
    let snapshot = metrics.snapshot().expect("metrics snapshot");
    let attributes = single_histogram_attributes(&snapshot, "forestx.test.extension.turn");
    assert_eq!(
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("forestx-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        forestx_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_forestx: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_forestx)?;
    std::fs::write(
        dot_forestx.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        forestx_home.join(forestx_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<forestx_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&forestx_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: forestx_hooks::StartHookTarget::SessionStart {
                source: forestx_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn forestx_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { forestx_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<TurnEnvironment> for ProbeToolRuntime {
        ...
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: forestx_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
                #[allow(deprecated)]
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> forestx_sandboxing::SandboxablePreference {
            forestx_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(forestx_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: forestx_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let forestx_home = session.forestx_home().await;
    std::fs::create_dir_all(&forestx_home).expect("create forestx home");
    let config_toml_path = forestx_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = forestx_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let forestx_home = session.forestx_home().await;
    std::fs::create_dir_all(&forestx_home).expect("create forestx home");
    let base_config_path = forestx_home.join(CONFIG_TOML_FILE);
    let profile_config_path = forestx_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(forestx_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            .features
            .enable(Feature::ForestxHooks)
            .expect("enable Forestx hooks");
    })
    .await?;
    let forestx_home = session.forestx_home().await;
    std::fs::create_dir_all(&forestx_home)?;
    let config_toml_path = forestx_home.join(CONFIG_TOML_FILE);
    let user_config: forestx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = forestx_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = forestx_hooks::list_hooks(forestx_hooks::HooksConfig {
        feature_enabled: true,
        ...
        ..forestx_hooks::HooksConfig::default()
    });
    assert_eq!(hook_list.hooks.len(), 1);
    assert_eq!(
        hook_list.hooks[0].trust_status,
        forestx_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: forestx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: forestx_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: forestx_config::MatcherGroup {
                matcher: None,
                hooks: vec![forestx_config::HookHandlerConfig::Command {
        ...
        let identity = forestx_config::TomlValue::try_from(identity)?;
        forestx_config::version_for_toml(&identity)
    };
...
async fn refresh_runtime_config_updates_runtime_refreshable_fields_and_keeps_session_static_settings()
 {
    ...
    std::fs::create_dir_all(&forestx_home).expect("create forestx home");
    std::fs::write(
        forestx_home.join(CONFIG_TOML_FILE),
        r#"[apps.calendar]
enabled = false
destructive_enabled = false
...
async fn refresh_mcp_config_replaces_managed_server_and_plugin_requirements() {
    ...
    next_config.mcp_servers = forestx_config::Constrained::normalized(
...
async fn subagent_activity_emits_matching_start_and_completion() {
    let (session, turn_context, rx) = make_session_and_context_with_rx().await;
    let item = forestx_protocol::items::SubAgentActivityItem {
        id: "activity-1".to_string(),
        kind: forestx_protocol::protocol::SubAgentActivityKind::Started,
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        ForestxAuth::from_api_key("Test API Key"),
        Vec::new(),
        |_| {},
    )
    .await;
...
async fn record_initial_history_seeds_token_info_from_rollout() {
    ...
    let info1 = TokenUsageInfo {
        total_token_usage: TokenUsage {
            ...
            total_tokens: 30,
            forestx_rollout_budget_units: None,
        },
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl forestx_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a forestx_extension_api::ExtensionData,
            thread_store: &'a forestx_extension_api::ExtensionData,
            turn_store: &'a forestx_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl forestx_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: forestx_extension_api::TurnStartInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl forestx_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: forestx_extension_api::TurnErrorInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: ForestxErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, ForestxErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl forestx_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &forestx_extension_api::ExtensionData,
            thread_store: &forestx_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: forestx_protocol::protocol::NetworkAccess::Restricted,
    };
    let permission_profile = PermissionProfile::External {
        network: NetworkSandboxPolicy::Restricted,
    };
    let expected_permission_profile = permission_profile.clone();
    let mut builder = test_forestx().with_config(move |config| {
        config
            .permissions
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_forestx().with_config(|config| {
        config.permissions.approval_policy =
            forestx_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.forestx.ensure_rollout_materialized().await;
    initial
        .forestx
        .flush_rollout()
        .await
    ...
    fork_config.permissions.approval_policy =
        forestx_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "forestx_core__forestx_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: forestx_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            forestx_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            forestx_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            forestx_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.forestx_error_info,
        Some(ForestxErrorInfo::ThreadRollbackFailed)
    );
    assert_eq!(
...
async fn set_rate_limits_retains_previous_credits() {
    let forestx_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(forestx_home.path()).await;
    ...
    let session_configuration = SessionConfiguration {
        ...
        forestx_home: config.forestx_home.clone(),
        thread_name: None,
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(forestx_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("forestx_other".to_string()),
        limit_name: Some("forestx_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(forestx_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("forestx".to_string()),
            limit_name: None,
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.forestx_error_info == Some(ForestxErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(forestx_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/forestx/analytics-events/events"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&server)
        .await;

    let auth_manager =
        AuthManager::from_auth_for_testing(ForestxAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("forestx-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "forestx_thread_initialized")
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = forestx_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        forestx_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let forestx_home = tempfile::tempdir().expect("create forestx home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        forestx_home.path().join(forestx_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let forestx_home = parent_config.forestx_home.clone();
    let skill_dir = forestx_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&forestx_exec_server::LOCAL_FS));
    ...
    let role_path = forestx_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let forestx_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(forestx_home.path()).await;
    config
        .features
    ...
    let auth_manager = AuthManager::from_auth_for_testing(ForestxAuth::from_api_key("Test API Key"));
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.forestx_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(forestx_code_mode::DisabledCodeModeSessionProvider),
        Arc::new(forestx_extension_api::ExtensionRegistryBuilder::new().build()),
        forestx_extension_api::ExtensionDataInit::default(),
        ClientMcpExtensions::default(),
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(forestx_thread_store::LocalThreadStore::new(
            forestx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        forestx_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
        /*external_time_provider*/ None,
        Some(config.multi_agent_version_from_features()),
        GitEnrichmentPolicy::Fresh,
        forestx_sandboxing::WindowsSandboxProxySettingsMode::Reconcile,
    )
    .await;
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(forestx_mcp::McpRuntime::empty(config.prefix_mcp_tool_names()));
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: forestx_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(forestx_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: forestx_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: forestx_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: forestx_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(forestx_thread_store::LocalThreadStore::new(
            forestx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .forestx_home(config.forestx_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(forestx_thread_store::LocalThreadStore::new(
            forestx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                forestx_state::StateRuntime::init(
                    config.sqlite.clone(),
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            forestx_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(forestx_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(forestx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            forestx_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(forestx_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = forestx_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(forestx_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    forestx_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(forestx_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: forestx_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(forestx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            thread_settings: forestx_protocol::protocol::ThreadSettingsOverrides {
                ...
                approvals_reviewer: Some(forestx_config::types::ApprovalsReviewer::AutoReview),
                sandbox_policy: Some(config.legacy_sandbox_policy()),
                summary: config.model_reasoning_summary,
                personality: config.personality,
                collaboration_mode: Some(forestx_protocol::config_types::CollaborationMode {
                    mode: forestx_protocol::config_types::ModeKind::Default,
                    settings: forestx_protocol::config_types::Settings {
    ...
    assert_eq!(
        ...
        forestx_config::types::ApprovalsReviewer::AutoReview
    );
    assert!(
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = forestx_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        forestx_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = forestx_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(forestx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn forestx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        forestx_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl forestx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadStopRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: forestx_extension_api::ThreadStopInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl forestx_extension_api::ThreadLifecycleContributor<crate::config::Config> for LifecycleRecorder {
        fn on_thread_stop<'a>(
            &'a self,
            input: forestx_extension_api::ThreadStopInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
    ...
    impl forestx_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: forestx_extension_api::TurnAbortInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(forestx_home).await;
...
async fn refresh_mcp_servers_uses_latest_state_for_existing_turns() {
    ...
    .expect("parse refreshed MCP servers");
    {
        ...
        config.mcp_oauth_credentials_store_mode =
            forestx_config::types::OAuthCredentialsStoreMode::Auto;
        config
            .features
    ...
    let configured_servers = forestx_mcp::configured_mcp_servers(new_step.mcp.config());
    assert_eq!(
        configured_servers.get("refreshed"),
        refreshed_mcp_servers.get("refreshed")
    );
    assert!(
        !forestx_mcp::configured_mcp_servers(old_step.mcp.config()).contains_key("refreshed"),
        "an already-bound step must keep its captured config"
    );
    assert!(
        forestx_mcp::configured_mcp_servers(rematerialized_old.config()).contains_key("refreshed"),
    ...
    assert!(
        forestx_mcp::configured_mcp_servers(current.config()).contains_key("refreshed"),
...
async fn mcp_elicitation_reviewer_uses_latest_runtime_authority() {
    ...
    let request = forestx_mcp::ElicitationReviewRequest {
        server_name: "browser-use".to_string(),
        request_id: rmcp::model::NumberOrString::Number(7),
        elicitation: forestx_rmcp_client::Elicitation::Mcp(
            rmcp::model::ElicitRequestParams::FormElicitationParams {
                meta: Some(rmcp::model::RequestMetaObject::from(
                    serde_json::Map::from_iter([
                        ("forestx_approval_kind".to_string(), json!("mcp_tool_call")),
                        ("forestx_request_type".to_string(), json!("approval_request")),
...
async fn mcp_refresh_updates_plugin_auth_mode_before_checking_pending_state() {
    let forestx_home = tempfile::tempdir().expect("create auth test directory");
    let (mut session, _turn_context) = make_session_and_context().await;
    session.services.auth_manager = AuthManager::from_auth_for_testing_with_home(
        ForestxAuth::from_api_key("old-api-key"),
        forestx_home.path().to_path_buf(),
    );
    let session = Arc::new(session);
    ...
    forestx_login::login_with_api_key(
        forestx_home.path(),
        "new-api-key",
        forestx_login::AuthCredentialsStoreMode::File,
        forestx_login::AuthKeyringBackendKind::default(),
    )
...
impl forestx_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: forestx_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<forestx_exec_server::NoiseRendezvousConnectBundle, forestx_exec_server::ExecServerError>,
    > {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let selected_root =
        |environment_id: &str, path: &str| forestx_protocol::capabilities::SelectedCapabilityRoot {
            id: "shared-root".to_string(),
            location: forestx_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    for selected_root in &selected_roots {
        let forestx_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
        ...
        registration
            .complete(Ok(forestx_exec_server::EnvironmentReadyInfo {
...
async fn step_context_keeps_its_mcp_runtime_for_tools() -> anyhow::Result<()> {
    ...
    let next_step = session
        ...
        .await
        .expect("a fresh cancellation token cannot be cancelled");
    assert!(forestx_mcp::configured_mcp_servers(next_step.mcp.config()).contains_key("newer"));
    ...
    let current = session
        ...
        .await
        .expect("refreshed runtime should be available");
    assert!(forestx_mcp::configured_mcp_servers(current.config()).contains_key("newer"));
...
impl forestx_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a forestx_extension_api::ExtensionData,
        thread_store: &'a forestx_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<forestx_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    forestx_extension_api::PromptFragment::developer_policy(
...
-> Arc<forestx_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = forestx_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl forestx_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: forestx_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Vec<forestx_extension_api::PromptFragment>> + Send + 'a>,
    > {
        Box::pin(async move {
...
async fn build_initial_context_adds_multi_agent_v2_subagent_usage_hint_as_developer_message() {
    ...
    config.token_budget = Some(crate::config::TokenBudgetConfig {
        mode: forestx_features::TokenBudgetMode::Name,
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<forestx_thread_store::InMemoryThreadStore> {
    let store = Arc::new(forestx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn forestx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &forestx_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
    ...
    impl forestx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            _input: forestx_extension_api::ThreadIdleInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = forestx_protocol::user_input::TextElement::new(
        forestx_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
    ...
    impl forestx_extension_api::ThreadLifecycleContributor<crate::config::Config> for ThreadIdleRecorder {
        fn on_thread_idle<'a>(
            &'a self,
            input: forestx_extension_api::ThreadIdleInput<'a>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(forestx_extension_api::ExtensionData::new(tc.sub_id.clone())),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use forestx_protocol::protocol::AskForApproval;
    use forestx_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: forestx_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: forestx_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let forestx_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_forestx = project_root.join(".forestx");
    let nested_dot_forestx = nested.join(".forestx");

    std::fs::create_dir_all(&forestx_home)?;
    std::fs::create_dir_all(&nested_dot_forestx)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_forestx)?;
    write_project_hooks(&nested_dot_forestx)?;
    write_project_trust_config(&forestx_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .forestx_home(forestx_home)
    ...
    let expected_source_path = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_forestx.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_forestx = project_root.join(".forestx");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_forestx)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let forestx_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&forestx_home)?;
        write_project_trust_config(&forestx_home, &trust_entries).await?;

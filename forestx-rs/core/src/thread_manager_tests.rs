...
use forestx_protocol::capabilities::CapabilityRootLocation;
use forestx_protocol::capabilities::SelectedCapabilityRoot;
...
async fn child_session_inherits_client_mcp_extensions() {
    ...
    config.forestx_home = temp_dir.path().join("forestx-home").abs();
    config.cwd = config.forestx_home.abs();
    std::fs::create_dir_all(&config.forestx_home).expect("create forestx home");
...
impl forestx_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: forestx_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> forestx_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<forestx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> forestx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<forestx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> forestx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
fn effective_originator_prefers_thread_scoped_sources_before_env_originator() {
    for (metrics_service_name, persisted_originator, inherited_originator, expected_originator) in [
        (
            Some("forestx_work_desktop"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "forestx_work_desktop",
        ),
        (
            Some("forestx_work_web"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "forestx_work_web",
        ),
        (
            Some("forestx_work_mobile"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "forestx_work_mobile",
        ),
        (
            Some("forestx_work_cca"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "forestx_work_cca",
        ),
...
async fn mcp_invalidation_refreshes_threads_that_are_still_starting() {
    ...
    impl forestx_extension_api::ThreadLifecycleContributor<Config> for BlockingThreadStartup {
        fn on_thread_start<'a>(
            &'a self,
            _input: forestx_extension_api::ThreadStartInput<'a, Config>,
    ...
    impl forestx_extension_api::McpServerContributor<Config> for BlockingThreadStartup {
        ...
        fn contribute<'a>(
            &'a self,
            _context: forestx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> forestx_extension_api::ExtensionFuture<'a, Vec<forestx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(ForestxAuth::from_api_key("dummy"));
    let manager = Arc::new(ThreadManager::new(
        &config,
        Arc::clone(&auth_manager),
        build_models_manager(&config, auth_manager),
        crate::ForestxAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(forestx_exec_server::EnvironmentManager::default_for_tests()),
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl forestx_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
        fn on_thread_start<'a>(
            &'a self,
            input: forestx_extension_api::ThreadStartInput<'a, Config>,
        ) -> forestx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
    ...
    impl forestx_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        fn id(&self) -> &'static str {
            "selected_root_test"
        }
        ...
        fn contribute<'a>(
            &'a self,
            context: forestx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> forestx_extension_api::ExtensionFuture<'a, Vec<forestx_extension_api::McpServerContribution>>
        {
            Box::pin(async move {
                ...
                let mut server = forestx_mcp::forestx_apps_mcp_server_config(
                ...
                vec![forestx_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = forestx_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread(StartThreadOptions {
            metrics_service_name: Some("forestx_work_desktop".to_string()),
            environments: Some(Vec::new()),
    ...
    let selected_servers = |config: &forestx_mcp::McpConfig| {
        forestx_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let forestx_apps_server = forestx_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(forestx_mcp::FORESTX_APPS_MCP_SERVER_NAME)
        .expect("Forestx Apps server should be configured");
    let forestx_apps_headers = match forestx_apps_server.transport {
        forestx_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => http_headers,
        forestx_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("Forestx Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        forestx_apps_headers
            .expect("Forestx Apps headers should be configured")
            .get("originator"),
        Some(&"forestx_work_desktop".to_string())
    );
}
...
#[tokio::test]
async fn explicit_installation_id_skips_forestx_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.forestx_home = temp_dir.path().join("forestx-home").abs();
    config.cwd = config.forestx_home.abs();
    ...
    assert!(!config.forestx_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.forestx_home.join("models_cache.json").exists());
}

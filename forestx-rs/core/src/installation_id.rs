...
pub async fn resolve_installation_id(forestx_home: &AbsolutePathBuf) -> Result<String> {
    let path = forestx_home.join(INSTALLATION_ID_FILENAME);
    fs::create_dir_all(forestx_home).await?;
...
mod tests {
    ...
    async fn resolve_installation_id_generates_and_persists_uuid() {
        let forestx_home = TempDir::new().expect("create temp dir");
        let forestx_home_abs = forestx_home.path().abs();
        let persisted_path = forestx_home.path().join(INSTALLATION_ID_FILENAME);

        let installation_id = resolve_installation_id(&forestx_home_abs)
            .await
    ...
    async fn resolve_installation_id_reuses_existing_uuid() {
        ...
        std::fs::write(
            forestx_home.path().join(INSTALLATION_ID_FILENAME),
            existing.clone(),
        )
        .expect("write installation id");

        let resolved = resolve_installation_id(&forestx_home_abs)
            .await
            .expect("resolve installation id");
    ...
    async fn resolve_installation_id_rewrites_invalid_file_contents() {
        ...
        assert_eq!(
            std::fs::read_to_string(forestx_home.path().join(INSTALLATION_ID_FILENAME))

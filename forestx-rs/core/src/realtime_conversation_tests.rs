...
async fn clears_active_handoff_explicitly() {
    ...
    let state = RealtimeHandoffState {
        ...
        forestx_response_handoff_channel_prefixes: Arc::new(BTreeMap::new()),
...
fn realtime_headers_include_only_non_default_originator() {
    let default_originator = forestx_login::default_client::originator();
    for (originator, expected_header) in [
        ("forestx_work_desktop", Some("forestx_work_desktop")),

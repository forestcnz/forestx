...
use forestx_core_plugins::remote::fetch_and_cache_global_remote_plugin_catalog;
...
async fn list_discoverable_plugins_with_auth(
    ...
    let plugins_manager = PluginsManager::new_with_options(
        ...
        auth.map(forestx_login::ForestxAuth::api_auth_mode),
    );
...
async fn list_tool_suggest_discoverable_plugins_includes_cached_remote_global_plugins() {
    ...
    write_file(
        &forestx_home.path().join(crate::config::CONFIG_TOML_FILE),
        r#"[features]
plugins = true
"#,
    );
    ...
    let mut config = load_plugins_config(forestx_home.path()).await;
...
"#,
    ...
    let mut config_with_disabled_remote_plugin = load_plugins_config(forestx_home.path()).await;

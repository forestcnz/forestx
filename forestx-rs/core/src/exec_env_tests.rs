...
fn inject_permission_profile_env_overrides_policy_value() {
    ...
    assert_eq!(
        env.get(FORESTX_PERMISSION_PROFILE_ENV_VAR)
            .map(String::as_str),
...
fn inject_permission_profile_env_removes_stale_value_without_active_profile() {
    ...
    assert_eq!(env.get(FORESTX_PERMISSION_PROFILE_ENV_VAR), None);
}
...
fn test_core_inherit_defaults_keep_sensitive_vars() {
    ...
    expected.insert(FORESTX_THREAD_ID_ENV_VAR.to_string(), thread_id.to_string());

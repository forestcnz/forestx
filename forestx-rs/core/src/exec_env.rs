...
pub use forestx_protocol::shell_environment::FORESTX_THREAD_ID_ENV_VAR;
...
pub const FORESTX_PERMISSION_PROFILE_ENV_VAR: &str = "FORESTX_PERMISSION_PROFILE";
...
///
/// `FORESTX_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub(crate) fn inject_permission_profile_env(
    ...
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(FORESTX_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(FORESTX_PERMISSION_PROFILE_ENV_VAR);
    }

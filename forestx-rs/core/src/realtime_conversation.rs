...
use forestx_api::RealtimeAudioFrame;
use forestx_api::RealtimeContextAppendChannel;
use forestx_api::RealtimeEvent;
use forestx_api::RealtimeEventParser;
use forestx_api::RealtimeSessionConfig;
use forestx_api::RealtimeSessionMode;
use forestx_api::RealtimeWebsocketClient;
use forestx_api::RealtimeWebsocketEvents;
use forestx_api::RealtimeWebsocketWriter;
use forestx_api::build_session_headers;
use forestx_api::map_api_error;
...
use forestx_login::read_openai_api_key_from_env;
...
use forestx_protocol::protocol::ForestxResponseHandoffMode;
...
use forestx_protocol::protocol::RealtimeConversationClosedEvent;
use forestx_protocol::protocol::RealtimeConversationRealtimeEvent;
use forestx_protocol::protocol::RealtimeConversationSdpEvent;
use forestx_protocol::protocol::RealtimeConversationStartedEvent;
use forestx_protocol::protocol::RealtimeHandoffRequested;
use forestx_protocol::protocol::RealtimeOutputModality;
use forestx_protocol::protocol::RealtimeTranscriptEntry;
use forestx_protocol::protocol::RealtimeVoice;
use forestx_protocol::protocol::RealtimeVoicesList;
use forestx_utils_output_truncation::approx_bytes_for_tokens;
use forestx_utils_string::approx_token_count;
use forestx_utils_string::take_bytes_at_char_boundary;
use http::HeaderMap;
...
const STANDALONE_HANDOFF_ID: &str = "forestx";
...
struct RealtimeHandoffState {
    ...
    forestx_responses_as_items: bool,
    forestx_response_item_prefix: Option<String>,
    forestx_response_handoff_mode: ForestxResponseHandoffMode,
    forestx_response_handoff_channel_prefixes: Arc<BTreeMap<String, Vec<String>>>,
...
impl RealtimeHandoffState {
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.forestx_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.forestx_response_handoff_mode == ForestxResponseHandoffMode::BemTags
    }
}
...
struct RealtimeStart {
    ...
    forestx_response_item_prefix: Option<String>,
    forestx_response_handoff_mode: ForestxResponseHandoffMode,
    forestx_response_handoff_channel_prefixes: Option<BTreeMap<String, Vec<String>>>,
...
impl RealtimeConversationManager {
    ...
    ) -> ForestxResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> ForestxResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            ...
            forestx_responses_as_items,
            forestx_response_item_prefix,
            forestx_response_handoff_mode,
            forestx_response_handoff_channel_prefixes,
        ...
        let handoff = RealtimeHandoffState {
            ...
            forestx_response_item_prefix,
            forestx_response_handoff_mode,
            forestx_response_handoff_channel_prefixes: Arc::new(
                forestx_response_handoff_channel_prefixes.unwrap_or_default(),
            ),
            session_kind,
            event_parser,
        };
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> ForestxResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(ForestxErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> ForestxResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| ForestxErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
    ...
    pub(crate) async fn handoff_out(
        ...
        let phase = if handoff.routes_handoff_by_bem() {
            match bem_message_phase(
                &output_text,
                &handoff.forestx_response_handoff_channel_prefixes,
            ) {
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.forestx_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.forestx_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> ForestxResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> ForestxResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.forestx_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> ForestxResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> ForestxResult<PreparedRealtimeConversationStart> {
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        forestx_responses_as_items: params.forestx_responses_as_items,
        forestx_response_item_prefix: params.forestx_response_item_prefix,
        forestx_response_handoff_mode: params.forestx_response_handoff_mode,
        forestx_response_handoff_channel_prefixes: params.forestx_response_handoff_channel_prefixes,
...
) -> ForestxResult<RealtimeSessionConfig> {
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> ForestxResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), ForestxErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(auth: Option<&ForestxAuth>, provider: &ModelProviderInfo) -> ForestxResult<String> {
    ...
    if let Some(api_key) = auth.and_then(ForestxAuth::api_key) {
...
fn realtime_request_headers(
    ...
    event_parser: RealtimeEventParser,
    originator: &str,
) -> ForestxResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            ForestxErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: ForestxResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        ForestxResponseHandoffMode::Thinking => None,
        ForestxResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        ForestxResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.forestx_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    forestx_error_info: ForestxErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,

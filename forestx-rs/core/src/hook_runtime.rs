...
use forestx_analytics::HookRunFact;
use forestx_analytics::build_track_events_context;
use forestx_hooks::PermissionRequestDecision;
use forestx_hooks::PermissionRequestOutcome;
use forestx_hooks::PermissionRequestRequest;
use forestx_hooks::PostToolUseOutcome;
use forestx_hooks::PostToolUseRequest;
use forestx_hooks::PreToolUseOutcome;
use forestx_hooks::PreToolUseRequest;
use forestx_hooks::SessionStartOutcome;
use forestx_hooks::StartHookTarget;
use forestx_hooks::StopHookTarget;
use forestx_hooks::StopOutcome;
use forestx_hooks::SubagentHookContext;
use forestx_hooks::UserPromptSubmitOutcome;
use forestx_hooks::UserPromptSubmitRequest;
use forestx_otel::HOOK_RUN_DURATION_METRIC;
use forestx_otel::HOOK_RUN_METRIC;
...
use forestx_protocol::protocol::HookCompletedEvent;
use forestx_protocol::protocol::HookEventName;
use forestx_protocol::protocol::HookRunStatus;
use forestx_protocol::protocol::HookRunSummary;
use forestx_protocol::protocol::HookSource;
use forestx_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = forestx_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = forestx_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = forestx_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = forestx_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(forestx_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: forestx_hooks::HookEvent::AfterAgent {
                event: forestx_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            forestx_hooks::HookResult::Success => continue,
            forestx_hooks::HookResult::FailedContinue(error) => (error, false),
            forestx_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(forestx_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (forestx_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use forestx_protocol::protocol::HookEventName;
    use forestx_protocol::protocol::HookExecutionMode;
    use forestx_protocol::protocol::HookHandlerType;
    use forestx_protocol::protocol::HookRunStatus;
    use forestx_protocol::protocol::HookScope;
    use forestx_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    forestx_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {

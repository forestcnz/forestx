...
use forestx_protocol::protocol::EnvironmentConnectionEvent;
...
impl TurnEnvironmentSnapshot {
    ...
    #[cfg(test)]
    pub(crate) fn primary_environment(&self) -> Option<Arc<forestx_exec_server::Environment>> {
        self.primary()
...
"#,
        ...
        let cwd = AbsolutePathBuf::current_dir().expect("cwd");
        let cwd_uri = PathUri::from_abs_path(&cwd);
        let manager = EnvironmentManager::from_forestx_home(
            temp_dir.path(),
            Some(test_runtime_paths()),

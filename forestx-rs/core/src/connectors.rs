...
pub use forestx_connectors::AppBranding;
pub use forestx_connectors::AppInfo;
pub use forestx_connectors::AppMetadata;
use forestx_connectors::ConnectorDirectoryCacheContext;
use forestx_connectors::ConnectorDirectoryCacheKey;
use forestx_connectors::apps_config_from_layer_stack;
...
use forestx_mcp::ToolInfo;
use forestx_mcp::ToolPluginProvenance;
use forestx_mcp::effective_mcp_servers;
use forestx_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = forestx_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        forestx_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config
        .features
        .apps_enabled_for_auth(auth.as_ref().is_some_and(ForestxAuth::uses_forestx_backend))
    {
        return Some(Vec::new());
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_forestx_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    mcp_servers.retain(|name, _| name == FORESTX_APPS_MCP_SERVER_NAME);
    ...
    let forestx_apps_auth_manager =
        forestx_mcp::host_owned_forestx_apps_enabled(&mcp_config, auth.as_ref())
    ...
    let mcp_runtime = McpRuntime::new(McpRuntimeInput {
        ...
        runtime_context,
        forestx_apps_tools_cache: mcp_manager.forestx_apps_tools_cache(),
        tool_catalog_cache: mcp_manager.tool_catalog_cache(),
        forestx_apps_tools_cache_key: connector_runtime_context_key(auth.as_ref()),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: auth.clone(),
        forestx_apps_auth_manager,
    ...
    let refreshed_tools = if force_refetch {
        match mcp_runtime
            .latest_hard_refresh_forestx_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{FORESTX_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let forestx_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(FORESTX_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_runtime
            .latest_wait_for_server_ready(FORESTX_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_runtime
                .latest_wait_for_server_ready(FORESTX_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if forestx_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let account_id = auth.and_then(ForestxAuth::get_account_id);
    let chatgpt_user_id = auth.and_then(ForestxAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(ForestxAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + forestx_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    forestx_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != FORESTX_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(forestx_connectors::accessible::AccessibleConnectorTool {
    ...
    forestx_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer_from_layers(
    config_layer_stack: &forestx_config::ConfigLayerStack,
    default_reviewer: ApprovalsReviewer,
    server_name: &str,
    connector_id: Option<&str>,
) -> ApprovalsReviewer {
    let app_reviewer = if server_name == FORESTX_APPS_MCP_SERVER_NAME {

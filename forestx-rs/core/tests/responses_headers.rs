...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let forestx_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(
        request.header("x-forestx-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-forestx-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-forestx-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-forestx-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-forestx-sandbox"), None);
}
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let test = test_forestx().build(&server).await.expect("build test forestx");
    ...
    let initial_header = first_request
        ...
        .expect("x-forestx-turn-metadata header should be present");
    let initial_parsed: serde_json::Value =
        serde_json::from_str(&initial_header).expect("x-forestx-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-forestx-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/forestx.git",
    ]);

...
enum TestForestxHome {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl TestForestxHome {
    fn path(&self) -> &Path {
        match self {
...
fn forestx_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestForestxHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep FORESTX_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let forestx_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&forestx_home)
            .with_context(|| format!("create stable test FORESTX_HOME {}", forestx_home.display()))?;
        return Ok(TestForestxHome::Persistent(forestx_home));
    }

    Ok(TestForestxHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("forestx-resources");
    ...
    for helper_name in ["forestx-windows-sandbox-setup", "forestx-command-runner"] {
        let helper = forestx_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let forestx_home =
        forestx_home_for_windows_sandbox_test("windows-restricted-token-deny-read-forestx-home")?;
...
async fn windows_elevated_does_not_create_missing_workspace_metadata() -> anyhow::Result<()> {
    let forestx_home =
        forestx_home_for_windows_sandbox_test("windows-elevated-missing-metadata-forestx-home")?;
    let _forestx_home_guard = EnvVarGuard::set("FORESTX_HOME", forestx_home.path().as_os_str());
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let forestx_home = forestx_home_for_windows_sandbox_test("windows-elevated-deny-read-forestx-home")?;
    let _forestx_home_guard = EnvVarGuard::set("FORESTX_HOME", forestx_home.path().as_os_str());
    stage_windows_sandbox_helpers()?;
    let workspace = TempDir::new()?;
    let cwd = dunce::canonicalize(workspace.path())?.abs();
    ...
    let public = cwd.join("public.txt");
    let setup_marker = forestx_home.path().join(".sandbox").join("setup_marker.json");
    ...
    assert!(
        sandbox_setup_is_complete(forestx_home.path()),

...
async fn rapid_mcp_refreshes_coalesce_to_the_latest_config() -> Result<()> {
    ...
    let test = core_test_support::test_forestx::test_forestx()
...
async fn out_of_band_resource_read_reconciles_the_published_mcp_runtime() -> Result<()> {
    ...
    let user_config_path = refresh_config.forestx_home.join("config.toml");
...
"#,
    ...
    let refreshed_servers = user_config
        .get("mcp_servers")
        .cloned()
        .map(HashMap::<String, forestx_config::types::McpServerConfig>::deserialize)
        .transpose()?
    ...
    test.forestx.refresh_runtime_config(refresh_config).await;
    test.forestx.submit(Op::RefreshMcpServers).await?;
    ...
    for extension_id in [None, Some("hosted_plugin_runtime"), Some("test-extension")] {
        ...
        if let Some(id) = extension_id {
            ...
            extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
                id,
                url: format!("{}/api/forestx/ps/mcp", apps_server.chatgpt_base_url),
            }));
            builder = builder.with_extensions(Arc::new(extensions.build()));
        }
        let test = builder.build_with_auto_env(&server).await?;
        let startup = wait_for_mcp_server(&test.forestx, FORESTX_APPS_MCP_SERVER_NAME).await;
        ...
        if extension_id == Some("test-extension") {
            ...
            assert!(
                ...
                "an extension named forestx_apps must not inherit the trusted Apps limit: {error}"
            );
            continue;
        }
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__forestx_apps__calendar_create_event(args:")
        }),
...
async fn initially_empty_deferred_tool_world_state_is_not_rendered_or_persisted() -> Result<()> {
    ...
    let mut builder = core_test_support::test_forestx::test_forestx()
        .with_config(enable_deferred_tool_world_state_without_agents);
    let test = builder.build_with_auto_env(&server).await?;
...
async fn deferred_tool_world_state_survives_resume_without_duplicate_updates() -> Result<()> {
    ...
    wait_for_mcp_server(&initial.forestx, FORESTX_APPS_MCP_SERVER_NAME).await?;
    initial
        .submit_turn("inspect deferred tools before resume")
        .await?;

    initial.forestx.ensure_rollout_materialized().await;
    initial.forestx.flush_rollout().await?;
    ...
    assert_eq!(
        persisted_tools,
        json!({
            "mcp__forestx_apps__calendar": "Plan events and manage your calendar."
        })
    );
    ...
    drop(initial);
    wait_for_mcp_server(&resumed.forestx, FORESTX_APPS_MCP_SERVER_NAME).await?;
    resumed
...
async fn apps_guidance_and_deferred_namespace_appear_after_recovery_within_a_turn() -> Result<()> {
    ...
    extensions.mcp_server_contributor(Arc::new(AppsMcpServerContributor {
        ...
        url: format!("{}/api/forestx/ps/mcp", gated_apps_server.chatgpt_base_url),
    }));
    ...
    test.forestx
        ...
        .await?;
    let EventMsg::RequestUserInput(request) = wait_for_event(&test.forestx, |event| {
    ...
    wait_for_event(&test.forestx, |event| {
        matches!(
            ...
            EventMsg::McpStartupUpdate(update)
                if update.server == FORESTX_APPS_MCP_SERVER_NAME
                    && matches!(update.status, forestx_protocol::protocol::McpStartupStatus::Ready)
        )
    })
    .await;
...
-> Result<()> {
    ...
    tokio::fs::remove_dir_all(test.forestx_home_path().join("cache/forestx_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    test.forestx.submit(Op::RefreshMcpServers).await?;
    test.forestx
        .submit(Op::UserInput {

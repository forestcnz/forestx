...
fn extract_log_field_handles_empty_bare_values() {
    let line = "event.name=\"forestx.tool_result\" mcp_server= mcp_server_origin=";
...
fn extract_log_field_does_not_confuse_similar_keys() {
    let line = "event.name=\"forestx.tool_result\" mcp_server_origin=stdio";
...
async fn responses_api_emits_api_request_event() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("forestx.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected forestx.api_request event".to_string()))
    });
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("forestx.sse_event")
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("forestx.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected forestx.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    let TestForestx { forestx, .. } = test_forestx().build(&server).await.unwrap();
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing forestx.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("forestx.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing forestx.sse_event".to_string()))
    });
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("handle_responses{")
                ...
                && line.contains("forestx.request.reasoning_effort=high")
                ...
                && line.contains("forestx.usage.reasoning_output_tokens=2")
                && line.contains("forestx.usage.total_tokens=9")
        }),
    ...
    assert!(
        logs.lines().any(|line| {
            line.contains("turn{otel.name=\"session_task.turn\"")
                && line.contains("forestx.turn.reasoning_effort=high")
                && line.contains("forestx.turn.token_usage.input_tokens=3")
                && line.contains("forestx.turn.token_usage.cached_input_tokens=1")
                && line.contains("forestx.turn.token_usage.cache_write_input_tokens=2")
                && line.contains("forestx.turn.token_usage.non_cached_input_tokens=2")
                && line.contains("forestx.turn.token_usage.output_tokens=5")
                && line.contains("forestx.turn.token_usage.reasoning_output_tokens=2")
                && line.contains("forestx.turn.token_usage.total_tokens=9")
        }),
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("forestx.tool_result") && line.contains("call_id=custom-tool-call")
            })
            .ok_or_else(|| "missing forestx.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("forestx.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing forestx.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_shell_command_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| line.contains("forestx.tool_result") && line.contains("call_id=shell-call"))
            .ok_or_else(|| "missing forestx.tool_result event".to_string())?;
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("forestx.tool_decision") && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing forestx.tool_decision event for {call_id}"))?;
...
fn sandbox_outcome_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("forestx.sandbox_outcome")
                    && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing forestx.sandbox_outcome event for {call_id}"))?;
...
async fn handle_shell_command_user_approved_records_tool_decision() {
    let server = start_mock_server().await;
    let command = touch_command("forestx-otel-approval-test");
    mount_sse_once(
        &server,
        sse(vec![
    ...
    let approval_event =
        wait_for_event(&forestx, |ev| matches!(ev, EventMsg::ExecApprovalRequest(_))).await;

...
async fn responses_turn_state_persists_within_turn_and_resets_after() -> Result<()> {
    ...
    let first_turn_id = parse_turn_id(requests[0].header("x-forestx-turn-metadata"))
        .expect("first request should include turn metadata turn_id");
    let second_turn_id = parse_turn_id(requests[1].header("x-forestx-turn-metadata"))
        .expect("follow-up request should include turn metadata turn_id");
    let third_turn_id = parse_turn_id(requests[2].header("x-forestx-turn-metadata"))

...
async fn mcp_calls_stay_bound_to_each_thread() -> anyhow::Result<()> {
    ...
    let calls = [
        (&fixture.forestx, "first-call", "first-runtime"),
        (&second_thread, "second-call", "second-runtime"),
        (&fixture.forestx, "first-again", "first-runtime"),
    ];
    ...
    fixture.forestx.shutdown_and_wait().await?;
...
async fn regular_mcp_definition_cache_preserves_live_session_state() -> anyhow::Result<()> {
    ...
    .await;
    fixture.forestx.submit(user_turn("use the echo tool")).await?;

...
const REQUESTED_MODEL: &str = "gpt-5.3-forestx";
const TRUSTED_ACCESS_FOR_CYBER_VERIFICATION: &str = "trusted_access_for_cyber";

const CYBER_POLICY_MESSAGE: &str =
    "This request has been flagged for potentially high-risk cyber activity.";

fn disabled_text_turn(test: &TestForestx, text: &str) -> Op {
...
async fn openai_model_header_mismatch_emits_warning_event() -> Result<()> {
    ...
    let mut builder = test_forestx().with_model(REQUESTED_MODEL);
    ...
    let reroute = wait_for_event(&test.forestx, |event| {
    ...
    let warning = wait_for_event(&test.forestx, |event| matches!(event, EventMsg::Warning(_))).await;
    ...
    let _ = wait_for_event(&test.forestx, |event| {
...
async fn cyber_policy_response_emits_typed_error_without_retry() -> Result<()> {
    ...
    let error = wait_for_event(&test.forestx, |event| matches!(event, EventMsg::Error(_))).await;
    let EventMsg::Error(error) = error else {
        panic!("expected error event");
    };
    assert_eq!(error.message, CYBER_POLICY_MESSAGE);
    assert_eq!(error.forestx_error_info, Some(ForestxErrorInfo::CyberPolicy));
...
async fn model_verification_emits_structured_event_without_reroute_or_warning() -> Result<()> {
    ...
    let mut builder = test_forestx().with_model(SERVER_MODEL);
    let test = builder.build(&server).await?;

use anyhow::Result;
use anyhow::anyhow;
use forestx_core::compact::SUMMARIZATION_PROMPT;
...
async fn assert_compaction_uses_turn_lifecycle_id(forestx: &std::sync::Arc<forestx_core::ForestxThread>) {
    ...
    while turn_completed_id.is_none() {
        let event = forestx.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Forestx.
    ...
    forestx.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Forestx to flush rollout entries before inspecting the file.
    forestx.submit(Op::Shutdown).await.unwrap();
...
async fn manual_pre_compact_block_decision_does_not_block_compaction() {
    ...
    forestx.submit(Op::Compact).await.expect("trigger compact");

    let completed = wait_for_event_match(&forestx, |ev| match ev {
    ...
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::Warning(_))).await;
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    let hook_inputs = read_hook_inputs(&test.forestx_home_path().join("pre_compact_block_log.jsonl"));
...
async fn compact_hooks_respect_matchers_and_post_runs_after_compaction() {
    ...
    let hook_inputs =
        read_hook_inputs(&test.forestx_home_path().join("post_compact_manual_log.jsonl"));
    assert_eq!(hook_inputs.len(), 1);
    let input = &hook_inputs[0];
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let forestx = builder.build(&server).await.unwrap().forestx;
    ...
    let first = wait_for_event_match(&forestx, |ev| match ev {
    ...
    let last = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        forestx_protocol::models::ResponseItem::Message {
            id: None,
        ...
        forestx_protocol::models::ResponseItem::Compaction {
            id: None,
    ...
    let mut resume_builder = test_forestx().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    });
    ...
    wait_for_event(&resumed.forestx, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let test = builder.build(&server).await.expect("build test forestx");
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.forestx).await;
...
async fn pre_sampling_compact_falls_back_from_retired_previous_model_after_rename() {
    ...
    let request_log = mount_response_sequence(
        ...
        vec![
            ...
            invalid_request_response(format!(
                "The '{retired_model}' model is not supported when using Forestx with a ChatGPT account."
            )),
            sse_response(sse(vec![
                json!({
                    "type": "response.output_item.done",
    ...
    let mut initial_builder = test_forestx()
        .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test forestx");
    ...
    let model_provider = openai_model_provider(&server);
    let mut resumed_builder = test_forestx()
        .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(retired_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.forestx).await;
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&forestx, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&forestx, |event| match event {
...
async fn manual_compact_twice_preserves_latest_user_messages() {
    ...
    assert_eq!(
        ...
        requests[1].header("x-forestx-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        requests[2].header("x-forestx-window-id").as_deref()
    );
    assert_ne!(
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let forestx = builder.build(&server).await.unwrap();

    forestx.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    forestx.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    forestx.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn manual_compaction_keeps_the_creation_time_global_instructions() -> Result<()> {
    ...
    assert_eq!(
        test.forestx.instruction_sources().await,
...
-> Result<()> {
    ...
    test.forestx.flush_rollout().await?;
    ...
    let rollout_path = test.forestx.rollout_path().expect("rollout path");
    ...
    test.forestx.submit(Op::Shutdown).await?;
    wait_for_event(&test.forestx, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    let resumed_cwd = test.config.cwd.clone();
    let mut resume_builder = test_forestx()
        .with_home(Arc::clone(&home))
        .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
    ...
    assert_eq!(
        resumed.forestx.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],

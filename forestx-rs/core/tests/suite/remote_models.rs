...
use forestx_models_manager::model_info::BASE_INSTRUCTIONS;
...
use forestx_protocol::openai_models::default_input_modalities;
...
const REMOTE_MODEL_SLUG: &str = "forestx-test";
...
async fn unknown_model_sends_builtin_instructions() -> Result<()> {
    ...
    .await;
    let mut builder = test_forestx().with_model("future-custom-model");
...
async fn remote_models_get_model_info_uses_longest_matching_prefix() -> Result<()> {
    ...
    let specific = ModelInfo {
        display_name: "GPT 5.3 Forestx".to_string(),
    ...
    let manager = forestx_core::test_support::models_manager_with_provider(
        forestx_home.path().to_path_buf(),
        forestx_core::test_support::auth_manager_from_auth(auth),
        provider,
    );
    ...
    let model_info = manager
        .get_model_info("gpt-5.3-forestx-test", &config.to_models_manager_config())
        .await;

    assert_eq!(model_info.slug, "gpt-5.3-forestx-test");
    assert_eq!(
...
async fn remote_models_config_context_window_override_clamps_to_max_context_window() -> Result<()> {
    ...
    let turn_started_event = wait_for_event(&forestx, |event| {
        matches!(
            event,
...
async fn remote_models_long_model_slug_is_sent_with_custom_reasoning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "gpt-5.3-forestx-test";
    let prefix_model = "gpt-5.3-forestx";
...
async fn namespaced_model_slug_uses_catalog_metadata_without_fallback_warning() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let requested_model = "custom/gpt-5.2-forestx";
    let response_mock = mount_sse_once(
        &server,
        sse(vec![ev_response_created("resp-1"), ev_completed("resp-1")]),
    )
    ...
    loop {
        let event = wait_for_event(&forestx, |_| true).await;
        match event {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "forestx-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "forestx-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        /*priority*/ 1,
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "forestx-auto-balanced")
...
fn bundled_default_model_slug() -> String {
    forestx_core::test_support::all_model_presets()
        .iter()

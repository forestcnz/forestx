...
use forestx_core::TurnInput;
use forestx_core::config::CurrentTimeReminderConfig;
use forestx_extension_items::ExtensionItem;
use forestx_extension_items::sleep::SleepItem;
...
async fn idle_response_items_include_pending_mailbox_in_first_request() -> anyhow::Result<()> {
    ...
    .await;
    let test = test_forestx().build_with_auto_env(&server).await?;

    submit_queue_only_agent_mail(test.forestx.as_ref(), "pending mailbox input").await;
    test.forestx
    ...
    wait_for_turn_complete(test.forestx.as_ref()).await;
...
async fn assert_idle_user_input_reaches_the_first_model_request(
    ...
    let user_message = core_test_support::wait_for_event_match(test.forestx.as_ref(), |event| {
...
async fn build_forestx(server: &StreamingSseServer) -> Arc<ForestxThread> {
    test_forestx()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming Forestx test session")
        .forestx
}

async fn submit_user_input(forestx: &ForestxThread, text: &str) {
    forestx
        .submit(Op::UserInput {
...
async fn submit_danger_full_access_user_turn(test: &TestForestx, text: &str) {
...
async fn steer_user_input(forestx: &ForestxThread, text: &str) {
    forestx
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn enqueue_queue_only_agent_mail(forestx: &ForestxThread, text: &str) {
    forestx
...
async fn submit_queue_only_agent_mail(forestx: &ForestxThread, text: &str) {
    enqueue_queue_only_agent_mail(forestx, text).await;
    forestx
        .submit(Op::RealtimeConversationListVoices)
        .await
        .expect("submit list-voices barrier");
    wait_for_event(forestx, |event| {
        matches!(event, EventMsg::RealtimeConversationListVoicesResponse(_))
    })
    .await;
}

async fn wait_for_reasoning_item_started(forestx: &ForestxThread) {
    wait_for_event(forestx, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(forestx: &ForestxThread, text: &str) {
...
async fn wait_for_turn_complete(forestx: &ForestxThread) {
    wait_for_event(forestx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(forestx: &ForestxThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(forestx, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(forestx: &ForestxThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(forestx, |event| {
        matches!(
            event,
...
impl forestx_extension_api::ThreadLifecycleContributor<forestx_core::config::Config>
    for SleepingRootExtension
{
    fn on_thread_start<'a>(
        &'a self,
        input: forestx_extension_api::ThreadStartInput<'a, forestx_core::config::Config>,
...
async fn queue_only_agent_mail_wakes_sleeping_root_and_persists_message() {
    ...
    let mut extensions =
        forestx_extension_api::ExtensionRegistryBuilder::<forestx_core::config::Config>::new();
    ...
    let forestx = test_forestx()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build Forestx test session")
        .forestx;

    enqueue_queue_only_agent_mail(&forestx, CHILD_MESSAGE).await;
    ...
    assert!(history.items.iter().any(|item| {
        matches!(
            item,
            RolloutItem::ResponseItem(forestx_protocol::models::ResponseItem::AgentMessage {
                content,
                ..
            }) if content.iter().any(|content| matches!(
                content,
                forestx_protocol::models::AgentMessageInputContent::InputText { text }
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    submit_user_input(&forestx, INITIAL_PROMPT).await;
    wait_for_event(&forestx, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&forestx, STEER_PROMPT).await;
    wait_for_turn_complete(&forestx).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&forestx, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&forestx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&forestx, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&forestx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&forestx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&forestx, "new mailbox input").await;
    wait_for_sleep_item_completed(&forestx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&forestx).await;
    ...
    forestx.submit(Op::Shutdown).await.expect("shutdown session");
    wait_for_event(&forestx, |event| matches!(event, EventMsg::ShutdownComplete)).await;

    let rollout_path = forestx.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let forestx = build_forestx(&server).await;

    submit_user_input(&forestx, "first prompt").await;

    wait_for_reasoning_item_started(&forestx).await;

    submit_queue_only_agent_mail(&forestx, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&forestx, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&forestx, "second prompt").await;
    wait_for_turn_complete(&forestx).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&forestx, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&forestx, "resumed old task").await;
    wait_for_turn_complete(&forestx).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&forestx, "processed steered prompt").await;
    wait_for_turn_complete(&forestx).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&forestx, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&forestx, "second prompt").await;
    let _ = gate_first_completed_tx.send(());

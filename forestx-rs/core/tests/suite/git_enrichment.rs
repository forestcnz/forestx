...
fn turn_metadata(request: &Value) -> Result<Value> {
    serde_json::from_str(
        request["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
...
async fn concurrent_turns_keep_distinct_worktree_and_repository_metadata() -> Result<()> {
    ...
    tokio::try_join!(
        test.forestx.submit(user_turn("inspect the main worktree")),
        worktree_thread
            .thread
    ...
    tokio::join!(
        wait_for_event(test.forestx.as_ref(), |event| matches!(
            event,

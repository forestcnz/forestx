...
use forestx_protocol::protocol::ExecPolicyAmendment;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestForestx) -> (PathBuf, String) {
        match self {
    ...
    async fn prepare(
        ...
        match self {
            ...
            ActionKind::ApplyPatchShell { target, content } => {
                ...
                // Bazel may need to launch the configured Forestx helper binary
...
impl Expectation {
    fn verify(&self, test: &TestForestx, result: &CommandResult) -> Result<()> {
        match self {
...
fn assert_active_workspace_permission_profile(test: &TestForestx) {
    assert_eq!(
...
async fn wait_for_completion_without_approval(test: &TestForestx) {
    let event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestForestx) {
...
async fn wait_for_spawned_thread(test: &TestForestx) -> Result<Arc<ForestxThread>> {
...
/// same sandbox with `forestx sandbox -P`. The nested invocation must also pass
...
printenv FORESTX_PERMISSION_PROFILE
touch {outside_path:?}
"#
        ),
    )?;
    ...
    let approval_policy = AskForApproval::OnRequest;
    let mut builder = test_forestx().with_home(home).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
    });
    let test = builder.build(&server).await?;
    ...
    assert_eq!(
        ...
        "the unsandboxed script should inherit FORESTX_PERMISSION_PROFILE from the shell command"
    );
    assert!(
        outside_path.exists(),
...
/// profile names before passing one to `forestx sandbox -P`.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[cfg(unix)]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Print an ssh command that recreates the current Forestx sandbox remotely."
    )
...
def main():
    args = parse_args()
    profile_name = os.environ.get("FORESTX_PERMISSION_PROFILE")
    if not profile_name:
        raise SystemExit("FORESTX_PERMISSION_PROFILE must not be empty")
    if profile_name not in ALLOWED_PROFILES:
        raise SystemExit("FORESTX_PERMISSION_PROFILE is not allowlisted")
...
"#
    ...
    assert_eq!(
        sandbox_argv.len(),
        9,
        "expected forestx sandbox ... bash -lc CMD"
    );
    assert_eq!(
        sandbox_argv[..8],
        [
        ...
        "remote_bash.py should use the allowlisted inherited profile and managed configuration to reconstruct the Forestx sandbox"
    );
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command =
        "touch /tmp/forestx-fallback-rule-test.txt && echo hello > /tmp/forestx-fallback-rule-test.txt";
    let event = shell_event_with_prefix_rule(
        call_id,
        command,
        /*timeout_ms*/ 1_000,
...
"#,
    ...
    let fetch_command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://forestx-network-test.invalid', timeout=30).read().decode(errors='replace'))""#
        .to_string();

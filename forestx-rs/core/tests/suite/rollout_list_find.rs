...
use chrono::Utc;
use forestx_core::RolloutRecorder;
use forestx_core::RolloutRecorderParams;
use forestx_core::config::ConfigBuilder;
use forestx_core::find_archived_thread_path_by_id_str;
...
fn write_minimal_rollout_with_id_in_subdir(forestx_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = forestx_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(forestx_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(forestx_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    builder.cwd = forestx_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_forestx_home_directory() {
    let repo = TempDir::new().unwrap();
    let forestx_home = repo.path().join(".forestx");
    std::fs::create_dir_all(&forestx_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".forestx/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&forestx_home, id);

    let found =
        find_thread_path_by_id_str(&forestx_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();

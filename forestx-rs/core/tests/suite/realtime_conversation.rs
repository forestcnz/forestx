...
use forestx_core::test_support::auth_manager_from_auth;
...
use forestx_protocol::protocol::RealtimeConversationVersion;
use forestx_protocol::protocol::RealtimeEvent;
use forestx_protocol::protocol::RealtimeNoopRequested;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from Forestx.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = forestx_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "FORESTX_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
async fn seed_recent_thread(
    ...
    let db = test.forestx.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = forestx_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.forestx, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.forestx, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.forestx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.forestx.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.forestx, |msg| match msg {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn conversation_webrtc_frameless_chatgpt_sends_forestx_headers_to_backend() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let capture = RealtimeCallRequestCapture::new();
    Mock::given(method("POST"))
        .and(path_regex(".*/backend-api/forestx/realtime/calls$"))
        .and(capture.clone())
        .respond_with(
    ...
    let backend_base_url = format!("{}/backend-api/forestx", server.uri());
    ...
    let created = wait_for_event_match(&test.forestx, |msg| match msg {
    ...
    assert_eq!(
        ...
        (
            "/backend-api/forestx/realtime/calls",
    ...
    test.forestx.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.forestx, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=forestx-realtime-call-boundary")
    );
    ...
    assert_eq!(
        body,
        format!(
            "--forestx-realtime-call-boundary\r\n\
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.forestx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.forestx, |msg| match msg {
    ...
    timeout(Duration::from_secs(10), async {
        ...
        loop {
            match test.forestx.next_event().await?.msg {
                EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
                    payload: RealtimeEvent::Error(message),
                }) => {
        ...
        test.forestx.wait_until_terminated().await;
        Ok::<(), anyhow::Error>(())
    })
    .await
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.forestx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn assert_transport_close_tail_flush(
    ...
    let mut builder = test_forestx().with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.forestx_error_info, Some(ForestxErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_start_preflight_failure_emits_realtime_error_only() -> Result<()> {
    ...
    let closed = timeout(Duration::from_millis(200), async {
        wait_for_event_match(&test.forestx, |msg| match msg {
            EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
            _ => None,
        })
        .await
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.forestx
        ...
        .await?;
    let _ = wait_for_event_match(&test.forestx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_forestx().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(ForestxAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
            ClientMcpExtensions::default(),
        )
        .await?;
    let forestx = resumed_thread.thread;
    ...
    forestx.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("forestx-rs/core"))?;
    ...
    assert!(startup_context.contains("forestx-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.forestx, |event| match event {
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.forestx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            forestx_response_item_prefix: None,
            forestx_response_handoff_mode:
                forestx_protocol::protocol::ForestxResponseHandoffMode::BemTags,
            forestx_response_handoff_channel_prefixes: Some(BTreeMap::from([
                (
...
async fn inbound_handoff_request_starts_turn() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.forestx.next_event().await?;
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_forestx().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;

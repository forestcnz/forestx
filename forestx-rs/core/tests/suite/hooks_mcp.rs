...
async fn pre_tool_use_blocks_mcp_tool_before_execution(
    ...
    let test = test_forestx()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.forestx, RMCP_SERVER).await?;
    ...
    let hook_inputs = read_hook_inputs(test.forestx_home_path(), "pre_tool_use_hook_log.jsonl")?;
...
async fn post_tool_use_records_mcp_tool_payload_and_context(
    ...
    let hook_inputs = read_hook_inputs(test.forestx_home_path(), "post_tool_use_hook_log.jsonl")?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(
        json!({
            "hook_event_name": hook_inputs[0]["hook_event_name"],

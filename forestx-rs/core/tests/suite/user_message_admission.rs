use forestx_core::UserMessageAdmission;
...
async fn user_message_admission_reports_started_and_steered_for_concurrent_submissions() {
    ...
    loop {
        let event = timeout(Duration::from_secs(10), forestx.next_event())
            .await
    ...
    let rollout_path = forestx.rollout_path().expect("user-message rollout path");
...
async fn user_message_admission_reports_invalid_settings_and_recovers() {
    ...
    let error = timeout(
        ...
        forestx.submit_user_input_and_wait_for_admission(
            invalid_input,
            /*trace*/ None,
    ...
    let error_event = loop {
        let event = timeout(Duration::from_secs(5), forestx.next_event())
            .await
    ...
    assert_eq!(
        ...
        Some(ForestxErrorInfo::BadRequest)
    );

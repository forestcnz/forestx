...
async fn refresh_models_on_models_etag_mismatch_and_avoid_duplicate_models_fetch() -> Result<()> {
    ...
    // 1) On spawn, Forestx fetches /models and stores the ETag.
    ...
    // 2) If the server sends a different X-Models-Etag on /responses, Forestx refreshes /models.
    ...
    // Second /responses request (tool output) includes the same X-Models-Etag; Forestx should not
    ...
    let refresh_req = refresh_models_mock
        ...
        .next()
        .expect("one request");
    // Ensure Forestx includes client_version on refresh. (This is a stable signal that we're using the /models client.)
    assert!(
        refresh_req
            .url
            .query_pairs()

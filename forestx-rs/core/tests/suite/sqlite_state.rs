...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db = test.forestx.state_db().expect("state db enabled");
    assert!(
        !rollout_path.exists(),
...
async fn resume_restores_dynamic_tools_from_rollout_with_sqlite_enabled() -> Result<()> {
    ...
    let mut resume_builder = test_forestx().with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_forestx()
        .with_pre_build_hook(move |forestx_home| {
            let rollout_path = forestx_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = test.config.sqlite.state_db_path();
    let rollout_path = test.config.forestx_home.join(&rollout_rel_path);
...
async fn mcp_call_marks_thread_memory_mode_polluted_when_configured() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.forestx, server_name).await?;
    let db = test.forestx.state_db().expect("state db enabled");
    let thread_id = test.session_configured.thread_id;
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = forestx_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = forestx_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };

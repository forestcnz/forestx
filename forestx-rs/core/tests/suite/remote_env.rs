...
use forestx_core::WaitForEnvironmentToolConfig;
use forestx_core::compact::SUMMARIZATION_PROMPT;
use forestx_core::config::Config;
use forestx_core::config::Constrained;
use forestx_exec_server::CopyOptions;
use forestx_exec_server::CreateDirectoryOptions;
use forestx_exec_server::EnvironmentReadyInfo;
use forestx_exec_server::ExecServerError;
use forestx_exec_server::FileSystemSandboxContext;
use forestx_exec_server::LOCAL_ENVIRONMENT_ID;
use forestx_exec_server::NoiseChannelPublicKey;
use forestx_exec_server::NoiseRendezvousConnectBundle;
use forestx_exec_server::NoiseRendezvousConnectProvider;
use forestx_exec_server::REMOTE_ENVIRONMENT_ID;
use forestx_exec_server::RemoveOptions;
use forestx_extension_api::ExtensionFuture;
use forestx_extension_api::ExtensionRegistryBuilder;
use forestx_extension_api::ThreadLifecycleContributor;
use forestx_extension_api::ThreadStartInput;
...
use forestx_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
fn test_forestx_with_wait_for_environment() -> TestForestxBuilder {
    let mut extensions = ExtensionRegistryBuilder::new();
    extensions.thread_lifecycle_contributor(Arc::new(WaitForEnvironmentTestExtension));
    test_forestx().with_extensions(Arc::new(extensions.build()))
}

async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestForestx> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = forestx_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_patch_approval(test: &TestForestx) {
    let event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/forestx-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\forestx-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
    ...
    for deferred_executor_enabled in [false, true] {
        for host_config_present in [false, true] {
            ...
            let builder = if host_config_present {
                test_forestx_with_wait_for_environment()
            } else {
                test_forestx()
            };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    test.forestx
        ...
        .await?;
    let request = wait_for_event_match(&test.forestx, |event| match event {
...
async fn deferred_executor_promotes_primary_environment_when_startup_completes() -> Result<()> {
    ...
    test.forestx.ensure_rollout_materialized().await;
    test.forestx.flush_rollout().await?;
    let rollout = fs::read_to_string(test.forestx.rollout_path().context("rollout path")?)?;
...
async fn deferred_executor_starts_noise_connection_after_registration() -> Result<()> {
    ...
    .await;
    let mut builder = test_forestx_with_wait_for_environment().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    .await;
    let mut builder = test_forestx_with_wait_for_environment()
        .with_exec_server_url(format!("ws://{}", listener.local_addr()?))
        .with_config(|config| {
            assert!(config.features.enable(Feature::DeferredExecutor).is_ok());
        });
    ...
    assert_eq!(test.forestx.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.forestx.ensure_rollout_materialized().await;
    test.forestx.flush_rollout().await?;
    let rollout_path = test.forestx.rollout_path().context("rollout path")?;
...
async fn remote_exec_materializes_target_roots_before_sandbox_selection() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/forestx-remote-target-roots-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/forestx-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/forestx-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/forestx-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/forestx-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/forestx-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_sandboxed_read_allows_readable_root() -> Result<()> {
    ...
    let allowed_dir = PathBuf::from(format!("/tmp/forestx-remote-readable-{}", std::process::id()));
...
async fn remote_test_env_sandboxed_read_rejects_symlink_parent_dotdot_escape() -> Result<()> {
    ...
    let root = PathBuf::from(format!("/tmp/forestx-remote-dotdot-{}", std::process::id()));
...
async fn remote_test_env_remove_removes_symlink_not_target() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/forestx-remote-remove-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");

...
async fn user_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let TestForestx { forestx, .. } = test_forestx().build(&server).await?;
    ...
    let started_item = wait_for_event_match(&forestx, |ev| match ev {
    ...
    .await;
    let completed_item = wait_for_event_match(&forestx, |ev| match ev {
    ...
    let legacy_message = wait_for_event_match(&forestx, |ev| match ev {
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let started = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
    ...
    let Some(forestx_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
...
async fn missing_streamed_reasoning_id_is_reused_for_completion() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let TestForestx { forestx, .. } = test_forestx().build_with_auto_env(&server).await?;
    ...
    let started_id = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.id.clone()),
    ...
    .await;
    let completed_id = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.id.clone()),
...
async fn web_search_item_is_emitted() -> anyhow::Result<()> {
    ...
    .await;
    let begin = wait_for_event_match(&forestx, |ev| match ev {
    ...
    forestx.flush_rollout().await?;
    let rollout_path = forestx.rollout_path().expect("paginated rollout path");
...
async fn agent_message_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let TestForestx {
        ...
        session_configured,
        ..
    } = test_forestx().build(&server).await?;
    ...
    let (started_turn_id, started_item) = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            turn_id,
            item: TurnItem::AgentMessage(item),
            ..
    ...
    let delta_event = wait_for_event_match(&forestx, |ev| match ev {
...
async fn plan_mode_emits_plan_item_from_proposed_plan_block() -> anyhow::Result<()> {
    ...
    let plan_delta = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::PlanDelta(event) => Some(event.clone()),
        _ => None,
    })
    .await;

    let plan_completed = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ItemCompleted(ItemCompletedEvent {
...
async fn plan_mode_strips_plan_from_agent_messages() -> anyhow::Result<()> {
    ...
    while plan_delta.is_none() || agent_item.is_none() || plan_item.is_none() {
        let ev = wait_for_event(&forestx, |_| true).await;
        match ev {
...
async fn reasoning_content_delta_has_item_metadata() -> anyhow::Result<()> {
    ...
    let reasoning_item = wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ItemStarted(ItemStartedEvent {
            item: TurnItem::Reasoning(item),
            ..
        }) => Some(item.clone()),
...
async fn sequential_cutoff_renders_done_summaries_for_active_reasoning_item() -> anyhow::Result<()>
{
    ...
    loop {
        match wait_for_event(&forestx, |_| true).await {

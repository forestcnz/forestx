...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestForestx { forestx, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&forestx, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://developers.openai.com/forestx/config-basic#feature-flags for details."
        ),
    );

...
async fn web_search_mode_cached_sets_external_web_access_false() {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("create test Forestx conversation");
...
async fn web_search_tool_config_from_config_toml_is_forwarded_to_request() {
    ...
    let home = Arc::new(tempfile::TempDir::new().expect("create forestx home"));
    std::fs::write(
...
"#,
    ...
    let mut builder = test_forestx().with_model("gpt-5.2").with_home(home);
    let test = builder
        .build(&server)
        .await
        .expect("create test Forestx conversation");

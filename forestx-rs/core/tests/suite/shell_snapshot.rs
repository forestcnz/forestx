...
const POLICY_PATH_FOR_TEST: &str = "/forestx/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/forestx/snapshot/path";
const SNAPSHOT_MARKER_VAR: &str = "FORESTX_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(forestx_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = forestx_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_forestx().with_config(move |config| {
    ...
    let forestx_home = test.home.path().to_path_buf();
    ...
    .await;
    let snapshot_path = wait_for_snapshot(&forestx_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&forestx, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&forestx, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.forestx_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestForestxHarness::with_builder(builder).await?;
    let forestx_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(forestx_home));
...
async fn shell_command_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    wait_for_event(&forestx, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let forestx_home = home.path().to_path_buf();

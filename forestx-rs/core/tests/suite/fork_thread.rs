...
use forestx_core::parse_turn_item;
...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    for text in ["first", "second", "third"] {
        forestx
            ...
            .await
            .unwrap();
        let _ = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    }

    // Request history from the base conversation to obtain rollout path.
    let base_path = forestx.rollout_path().expect("rollout path");
    ...
    let fork1_path = forestx_fork1.rollout_path().expect("rollout path");
    expected_after_first.push(thread_settings_applied_item(
        forestx_fork1.config_snapshot().await,
    ));
    ...
    let fork2_path = forestx_fork2.rollout_path().expect("rollout path");
    ...
    expected_after_second.push(thread_settings_applied_item(
        forestx_fork2.config_snapshot().await,
    ));
...
async fn assert_copied_fork_persists_inherited_history(history_mode: ThreadHistoryMode) {
    ...
    let mut builder = test_forestx().with_history_mode(history_mode);
    let test = builder.build(&server).await.expect("create conversation");
    let forestx = test.forestx.clone();
    let thread_manager = test.thread_manager.clone();
    ...
    let source_path = forestx.rollout_path().expect("source rollout path");
    let source_items = read_rollout_items(&source_path);
    let source_meta = forestx_rollout::read_session_meta_line(source_path.as_path())
        .await
    ...
    if matches!(history_mode, ThreadHistoryMode::Paginated) {
        ...
        let resumed_history = forestx_rollout::RolloutRecorder::get_rollout_history(&forked_path)
            .await
        ...
        let resumed = thread_manager
            .resume_thread_with_history(
                test.config.clone(),
                resumed_history,
                forestx_core::test_support::auth_manager_from_auth(
                    forestx_login::ForestxAuth::from_api_key("dummy"),
                ),
                /*parent_trace*/ None,
                ClientMcpExtensions::default(),
            )

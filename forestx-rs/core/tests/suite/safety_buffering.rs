...
async fn emits_safety_buffering_with_the_header_fallback_model() -> anyhow::Result<()> {
    ...
    mount_response_once(
        ...
        sse_response(sse(vec![created, ev_completed("resp-1")]))
            .insert_header("x-forestx-safety-buffering-enabled", "true")
            .insert_header("x-forestx-safety-buffering-faster-model", FASTER_MODEL),
    )
    .await;
    ...
    let event = wait_for_event_match(&test.forestx, |event| match event {

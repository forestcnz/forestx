...
async fn request_body_is_zstd_compressed_for_forestx_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/forestx/v1", server.uri());
    ...
    let forestx = builder.build(&server).await?.forestx;

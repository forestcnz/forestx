...
use forestx_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use forestx_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use forestx_core::ForestxResponsesMetadata;
use forestx_core::ModelClient;
use forestx_core::ModelClientSession;
use forestx_core::Prompt;
use forestx_core::ResponseEvent;
use forestx_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
use forestx_core::test_support::with_parent_turn;
...
use forestx_rollout_trace::ConversationPart;
use forestx_rollout_trace::InferenceTraceContext;
use forestx_rollout_trace::RawTraceEventPayload;
use forestx_rollout_trace::TraceWriter;
...
fn turn_metadata(harness: &WebsocketTestHarness, turn_id: Option<&str>) -> ForestxResponsesMetadata {
    responses_metadata(harness, turn_id, TestForestxResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestForestxResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> ForestxResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestForestxResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(forestx_login::default_client::get_forestx_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-forestx-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_FORESTX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-forestx-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::ForestxTurnStarted {
            forestx_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server.single_handshake().header("x-forestx-turn-metadata"),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-forestx-primary-used-percent": "100.0",
            "x-forestx-secondary-used-percent": "87.5",
            "x-forestx-primary-over-secondary-limit-percent": "95.0",
            "x-forestx-primary-window-minutes": "15",
            "x-forestx-secondary-window-minutes": "60",
            "x-forestx-credits-has-credits": "true",
            "x-forestx-credits-unlimited": "false",
            "x-forestx-credits-balance": "",
            "x-forestx-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket forestx");
    ...
    let token_event =
        wait_for_event(&test.forestx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.forestx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = forestx_core::test_support::construct_model_info_offline(MODEL, &config);
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "forestx-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )

use anyhow::Result;
use forestx_config::types::Personality;
...
fn read_only_user_turn(test: &TestForestx, items: Vec<UserInput>, model: String) -> Op {
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_forestx().with_model("gpt-5.2");
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-forestx-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_forestx().build(&server).await?;
...
async fn unsupported_configured_service_tier_warns_at_session_start() -> Result<()> {
    ...
    let warning = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
...
async fn thread_rollback_after_generated_image_drops_entire_image_turn_history() -> Result<()> {
    ...
    test.forestx
        ...
        .await?;
    wait_for_event(&test.forestx, |ev| {
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count

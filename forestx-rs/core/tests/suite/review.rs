...
use forestx_core::REVIEW_PROMPT;
use forestx_core::config::Config;
use forestx_core::config::Constrained;
use forestx_core::find_thread_path_by_id_str;
...
use forestx_protocol::openai_models::ModelTokenBudgetConfig;
...
use forestx_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use forestx_protocol::protocol::ReviewCodeLocation;
use forestx_protocol::protocol::ReviewFinding;
use forestx_protocol::protocol::ReviewLineRange;
use forestx_protocol::protocol::ReviewOutputEvent;
...
use forestx_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Forestx sandbox network restrictions.
    skip_if_no_network!();
    ...
    let forestx = new_conversation_for_server(&server, forestx_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&forestx, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&forestx, |ev| {
        matches!(
            ev,
    ...
    let entered = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&forestx, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&forestx, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let path = forestx.rollout_path().expect("rollout path");
    ...
    let _forestx_home_guard = forestx_home;
    server.verify().await;
}
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match forestx.next_event().await.expect("review event").msg {
    ...
    forestx.submit(Op::Interrupt).await.unwrap();
...
async fn review_op_with_plain_text_emits_review_fallback() {
    ...
    let _entered = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    let review = match closed {
...
async fn review_uses_updated_turn_permissions_and_approval_policy() {
    ...
    let test = test_forestx()
        ...
        .with_config(|config| {
            ...
            config.config_lock_export_dir = Some(config.forestx_home.join("review-config-locks"));
    ...
    let forestx = Arc::clone(&test.forestx);
...
async fn review_uses_custom_review_model_from_config() {
    ...
    // Wait for completion
    let _entered = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&forestx, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = forestx_home.path().join("resume.jsonl");
    {
        ...
        let user = forestx_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = forestx_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let forestx =
        resume_conversation_for_server(&server, forestx_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let forestx = new_conversation_for_server(&server, forestx_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, forestx_home, resume_path)
        .await

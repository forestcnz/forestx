...
use forestx_models_manager::client_version_to_whole;
...
const REMOTE_MODEL: &str = "forestx-test-ttl";
const VERSIONED_MODEL: &str = "forestx-test-versioned";
const MISSING_VERSION_MODEL: &str = "forestx-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "forestx-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let cache_path = config.forestx_home.join(CACHE_FILE);
    ...
    forestx
        .submit(Op::UserInput {
            ...
            thread_settings: forestx_protocol::protocol::ThreadSettingsOverrides {
                ...
                approval_policy: Some(forestx_protocol::protocol::AskForApproval::Never),
    ...
    let _ = wait_for_event(&forestx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn matching_models_etag_does_not_rewrite_recent_cache() -> Result<()> {
    ...
    let cache_path = test.config.forestx_home.join(CACHE_FILE);

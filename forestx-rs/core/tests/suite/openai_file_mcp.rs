...
async fn run_extract_turn(test: &TestForestx, server: &MockServer) -> Result<ResponseMock> {
...
async fn forestx_apps_file_params_omit_fields_absent_from_tool_schema() -> Result<()> {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn forestx_apps_file_params_pass_uploaded_file_to_post_tool_use_hook() -> Result<()> {
    let server = start_mock_server().await;
    let apps_server = AppsTestServer::mount(&server).await?;

...
async fn submit_user_turn(
    test: &core_test_support::test_forestx::TestForestx,
    prompt: &str,
...
"#;
    ...
    let mut migrated_builder = test_forestx().with_config(|config| {
        let policy_path = config.forestx_home.join("rules/default.rules");
    ...
    let migrated_policy_path = migrated.forestx_home_path().join("rules/default.rules");
    assert_eq!(fs::read_to_string(&migrated_policy_path)?, MIGRATED_POLICY);
    assert_eq!(
        fs::read_to_string(migrated.forestx_home_path().join(MIGRATION_MARKER_FILENAME))?,
        "v1\n"
    );

    let mut ignored_builder = test_forestx().with_config(|config| {
        let policy_path = config.forestx_home.join("rules/default.rules");
        fs::create_dir_all(policy_path.parent().expect("rules directory"))
            .expect("create rules directory");
        fs::write(policy_path, LEGACY_POLICY).expect("write legacy policy");
    ...
    let ignored_policy_path = ignored.forestx_home_path().join("rules/default.rules");
...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_forestx().with_config(|config| {
        let policy_path = config.forestx_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.forestx, |event| {
...
"#,
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::UnifiedExec)
                .expect("test config should allow feature update");
            let policy_path = config.forestx_home.join("rules").join("broken.rules");
            fs::create_dir_all(
                policy_path
                    .parent()
                    .expect("policy directory must have a parent"),

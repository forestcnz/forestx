...
async fn build_unified_exec_zsh_fork_test_or_skip<F>(
    ...
    pre_build_hook: F,
) -> Result<Option<(MockServer, TestForestx)>>
where
...
async fn approve_expected_exec(test: &TestForestx, expected_command: &str) -> Result<()> {
    let approval = expect_exec_approval(test, expected_command).await;
    approve_exec(test, approval.effective_approval_id()).await
}

async fn approve_exec(test: &TestForestx, approval_id: String) -> Result<()> {
    test.forestx

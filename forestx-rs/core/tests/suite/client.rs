...
use forestx_core::ModelClient;
use forestx_core::NewThread;
use forestx_core::Prompt;
use forestx_core::ResponseEvent;
use forestx_core::StartThreadOptions;
use forestx_core::ThreadManager;
use forestx_core::resolve_installation_id;
use forestx_core::thread_store_from_config;
use forestx_extension_api::empty_extension_registry;
use forestx_features::Feature;
use forestx_login::AuthKeyringBackendKind;
...
use forestx_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use forestx_protocol::models::DEFAULT_IMAGE_DETAIL;
...
use forestx_protocol::models::LocalShellAction;
use forestx_protocol::models::LocalShellExecAction;
use forestx_protocol::models::LocalShellStatus;
use forestx_protocol::models::MessagePhase;
use forestx_protocol::models::ReasoningItemContent;
use forestx_protocol::models::ReasoningItemReasoningSummary;
use forestx_protocol::models::ResponseItem;
use forestx_protocol::models::WebSearchAction;
use forestx_protocol::openai_models::InputModality;
...
use forestx_protocol::protocol::RolloutLine;
...
use core_test_support::TestForestxResponsesRequestKind;
...
use core_test_support::test_forestx::TestForestx;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> forestx_core::ForestxResponsesMetadata {
    ...
    test_responses_metadata(
        ...
        TestForestxResponsesRequestKind::Turn,
    )
}
...
fn assert_forestx_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-forestx-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-forestx-turn-metadata"]
        .as_str()
        .expect("missing x-forestx-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-forestx-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-forestx-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_forestx().build(&server).await.unwrap();
...
async fn non_openai_responses_requests_include_item_ids_without_passthrough_metadata() {
    ...
    forestx
        ...
        .await
        .unwrap();
    wait_for_event(&forestx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.forestx.submit(Op::Shutdown).await?;
    wait_for_event(&initial.forestx, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let forestx_home = Arc::new(TempDir::new()?);
    let mut builder = test_forestx();
    let first = builder
        .resume(&server, Arc::clone(&forestx_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.forestx.submit(Op::Shutdown).await?;
    wait_for_event(&first.forestx, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, forestx_home, session_path).await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    let test = test_forestx()
        .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
    ...
    test.forestx.submit(Op::Compact).await?;
    wait_for_event(&test.forestx, |event| {
...
/// Writes an `auth.json` into the provided `forestx_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        forestx_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: forestx_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = forestx_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = forestx_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![forestx_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = forestx_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Forestx to resume from our file
    let forestx_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_forestx()
        .with_home(forestx_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, forestx_home, session_path.clone())
        .await
        .expect("resume conversation");
    let forestx = test.forestx.clone();
    ...
    forestx
        ...
        .await
        .unwrap();
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let forestx_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_forestx().with_model("gpt-5.4");
    let test = builder
        .resume(&server, forestx_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_forestx().with_auth(ForestxAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.forestx_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_forestx_client_metadata(
        &request_body,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&forestx_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = forestx_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        forestx_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(ForestxAuth::from_api_key(
            "unused-api-key",
        ))),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &forestx_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_forestx()
        .with_auth(ForestxAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/forestx", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_forestx()
        .with_auth(create_dummy_forestx_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/forestx/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = ForestxAuth::from_auth_storage(
        forestx_home.path(),
        AuthCredentialsStoreMode::File,
        /*chatgpt_base_url*/ None,
        AuthKeyringBackendKind::default(),
        &forestx_login::test_support::transport_default_auth_route_config(),
    )
    .await
    .expect("Failed to load ForestxAuth")
    .expect("No ForestxAuth found in forestx_home");
    let auth_manager = forestx_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.forestx_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        forestx_core::build_models_manager(&config, auth_manager),
        forestx_core::ForestxAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(forestx_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(forestx_core::test_support::EmptyUserInstructionsProvider),
    ...
    let NewThread { thread: forestx, .. } = thread_manager
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__forestx_apps"),
        "did not expect forestx_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect forestx_apps resource calls: {resource_methods:?}"
    );
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestForestx { forestx, .. } = test_forestx()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestForestx { forestx, .. } = test_forestx().with_model("gpt-5.4").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    .await;
    let TestForestx { forestx, config, .. } = test_forestx().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_prefixed_item_ids() {
    ...
    let thread_id = ThreadId::new();
    let auth_manager =
        forestx_core::test_support::auth_manager_from_auth(ForestxAuth::from_api_key("Test API Key"));
    let session_telemetry = SessionTelemetry::new(
        thread_id,
        model.as_str(),
        model_info.slug.as_str(),
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-forestx-primary-used-percent", "12.5")
        .insert_header("x-forestx-secondary-used-percent", "40.0")
        .insert_header("x-forestx-primary-window-minutes", "10")
        .insert_header("x-forestx-secondary-window-minutes", "60")
        .insert_header("x-forestx-primary-reset-at", "1704069000")
        .insert_header("x-forestx-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_forestx()
        .with_auth(ForestxAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let forestx = builder
    ...
    wait_for_event(&forestx, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-forestx-primary-used-percent", "100.0")
        .insert_header("x-forestx-secondary-used-percent", "87.5")
        .insert_header("x-forestx-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-forestx-primary-window-minutes", "15")
        .insert_header("x-forestx-secondary-window-minutes", "60")
        .insert_header("x-forestx-credits-has-credits", "true")
        .insert_header("x-forestx-credits-unlimited", "false")
        .insert_header("x-forestx-credits-balance", "")
        .insert_header(
            "x-forestx-rate-limit-reached-type",
    ...
    let mut builder = test_forestx();
    let forestx_fixture = builder.build(&server).await?;
    let forestx = forestx_fixture.forestx.clone();
    ...
    let token_event = wait_for_event(&forestx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&forestx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&forestx, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = ForestxErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_forestx_auth() -> ForestxAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Forestx sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();

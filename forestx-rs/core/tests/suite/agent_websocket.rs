...
async fn websocket_model_switch_to_responses_lite_omits_top_level_tools() -> Result<()> {
    ...
    let non_lite_turn_metadata: Value = serde_json::from_str(
        non_lite_turn["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
    ...
    let lite_turn_metadata: Value = serde_json::from_str(
        lite_turn["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
    ...
    assert!(
        lite_turn["client_metadata"]
            .get("x-forestx-code-mode-tool-names")
            .is_none()
    );
    assert!(
        non_lite_turn
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_test_forestx_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_forestx().with_windows_cmd_shell();
...
async fn websocket_first_turn_uses_startup_prewarm_and_create() -> Result<()> {
    ...
    assert_eq!(
        ...
        warmup["client_metadata"]["x-forestx-window-id"].as_str()
    );
    assert!(
        turn["tools"]
            .as_array()
    ...
    let turn_metadata: Value = serde_json::from_str(
        turn["client_metadata"]["x-forestx-turn-metadata"]
            .as_str()
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn websocket_v2_test_forestx_shell_chain() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let mut builder = test_forestx().with_windows_cmd_shell().with_config(|config| {
        config
            .features

...
use forestx_core::config::AgentRoleConfig;
...
fn write_home_skill(forestx_home: &Path, dir: &str, name: &str, description: &str) -> Result<()> {
    let skill_dir = forestx_home.join("skills").join(dir);
...
async fn wait_for_spawned_thread_id(test: &TestForestx) -> Result<String> {
    let deadline = Instant::now() + Duration::from_secs(2);
    loop {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    configure_test: impl FnOnce(
        core_test_support::test_forestx::TestForestxBuilder,
    ) -> core_test_support::test_forestx::TestForestxBuilder,
) -> Result<(
    TestForestx,
    String,
    ...
    let mut builder = configure_test(test_forestx().with_config(|config| {
        config
            .features
...
async fn subagent_stop_replaces_stop_and_skips_internal_subagents() -> Result<()> {
    ...
    let stop_inputs = read_hook_log(test.forestx_home_path(), "stop_hook_log.jsonl")?;
    assert!(
        stop_inputs
            .iter()
    ...
    let subagent_stop_inputs_after_internal =
        read_hook_log(test.forestx_home_path(), "subagent_stop_hook_log.jsonl")?;
    assert_eq!(subagent_stop_inputs_after_internal, subagent_stop_inputs);

    let stop_inputs_after_internal = read_hook_log(test.forestx_home_path(), "stop_hook_log.jsonl")?;
...
async fn multi_agent_v2_spawn_sends_agent_message_to_child(
    ...
    let mut builder = test_forestx().with_model(parent_model).with_config(|config| {
        config
            .features
            .enable(Feature::Collab)
            .expect("test config should allow feature update");
...
async fn spawn_agent_role_overrides_requested_model_and_reasoning_settings() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.forestx_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
...
async fn spawn_agent_preserves_configured_defaults_through_unrelated_role() -> Result<()> {
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.forestx_home.join("instructions-only-role.toml");
    ...
    let test = test_forestx()
        .with_config(|config| {
            config
                ...
                .enable(Feature::Collab)
                .expect("test config should allow feature update");
            let role_path = config.forestx_home.join("model-only-role.toml");

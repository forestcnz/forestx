...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_forestx().with_workspace_setup(move |cwd, fs| async move {
    ...
    core_test_support::wait_for_event(test.forestx.as_ref(), |event| {
        matches!(event, forestx_protocol::protocol::EventMsg::TurnComplete(_))
    })
    .await;

...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(begin) = wait_for_event(&test.forestx, |event| {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.forestx, |event| {
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_forestx_apps"),
        Some(&json!({
            "call_id": "calendar-call-1",
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-forestx-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-forestx-turn-metadata/thread_id"),
        Some(&json!(test.session_configured.thread_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-forestx-turn-metadata/turn_id")
            .and_then(Value::as_str)
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-forestx-turn-metadata/model")
            .and_then(Value::as_str),
        Some("gpt-5.4")
    );
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-forestx-turn-metadata/reasoning_effort")
            .and_then(Value::as_str),
        Some(first_request_reasoning_effort)
    );
    let mcp_turn_started_at_unix_ms = apps_tool_call
        .pointer("/params/_meta/x-forestx-turn-metadata/turn_started_at_unix_ms")
...
async fn tool_search_returns_deferred_v1_multi_agent_tools() -> Result<()> {
    ...
    let mut builder = test_forestx().with_config(configure_search_capable_model);
    let test = builder.build(&server).await?;
    test.submit_turn_with_approval_and_permission_profile(

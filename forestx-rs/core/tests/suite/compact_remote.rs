...
use forestx_core::compact::SUMMARY_PREFIX;
use forestx_features::Feature;
use forestx_login::ForestxAuth;
use forestx_login::auth::AgentIdentityAuth;
use forestx_login::auth::AgentIdentityAuthRecord;
use forestx_login::auth::BedrockApiKeyAuth;
use forestx_model_provider_info::AMAZON_BEDROCK_GPT_5_5_MODEL_ID;
...
use forestx_protocol::account::PlanType as AccountPlanType;
use forestx_protocol::config_types::ServiceTier;
use forestx_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
use forestx_protocol::dynamic_tools::DynamicToolFunctionSpec;
use forestx_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use forestx_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use forestx_protocol::protocol::RealtimeConversationRealtimeEvent;
use forestx_protocol::protocol::RealtimeEvent;
use forestx_protocol::protocol::RealtimeOutputModality;
...
use core_test_support::test_forestx::TestForestxBuilder;
use core_test_support::test_forestx::TestForestxHarness;
use core_test_support::test_forestx::test_forestx as base_test_forestx;
...
const TURN_STATE_HEADER: &str = "x-forestx-turn-state";
...
fn test_forestx() -> TestForestxBuilder {
    base_test_forestx().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_forestx_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestForestxBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_forestx()
        .with_auth(ForestxAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(forestx: &forestx_core::ForestxThread) -> Result<()> {
    forestx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            client_managed_handoffs: false,
            delegation_ack_filler: None,
            flush_transcript_tail_on_session_end: false,
            forestx_responses_as_items: false,
            forestx_response_item_prefix: None,
            forestx_response_handoff_mode:
                forestx_protocol::protocol::ForestxResponseHandoffMode::Thinking,
            forestx_response_handoff_channel_prefixes: None,
            model: None,
    ...
    wait_for_event_match(forestx, |msg| match msg {
...
async fn close_realtime_conversation(forestx: &forestx_core::ForestxThread) -> Result<()> {
    forestx.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(forestx, |msg| match msg {
...
async fn wait_for_turn_complete(forestx: &forestx_core::ForestxThread) {
...
fn amazon_bedrock_test_forestx() -> TestForestxBuilder {
    let auth = ForestxAuth::BedrockApiKey(BedrockApiKeyAuth {
...
async fn amazon_bedrock_uses_remote_compaction_endpoint() -> Result<()> {
    ...
    let harness = TestForestxHarness::with_auto_env_builder(amazon_bedrock_test_forestx()).await?;
    ...
    harness.test().forestx.submit(Op::Compact).await?;
    wait_for_turn_complete(&harness.test().forestx).await;
...
async fn amazon_bedrock_automatic_compaction_uses_v1_endpoint_when_v2_is_enabled() -> Result<()> {
    ...
    let harness = TestForestxHarness::with_auto_env_builder(amazon_bedrock_test_forestx().with_config(
        |config| {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestForestxHarness::with_builder(
        test_forestx().with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let forestx = harness.test().forestx.clone();
    ...
    forestx
        ...
        .await?;
    wait_for_turn_complete(&forestx).await;

    forestx.submit(Op::Compact).await?;
    wait_for_turn_complete(&forestx).await;
    ...
    assert_eq!(
        ...
        compact_request.header("x-forestx-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-forestx-window-id").as_deref()
    );
    assert_ne!(
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestForestxHarness::with_builder(
        test_forestx().with_auth(ForestxAuth::AgentIdentity(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_forestx().with_auth(auth);
    ...
    let harness = TestForestxHarness::with_builder(builder).await?;
    let forestx = harness.test().forestx.clone();
    let image_url =
...
-> Result<()> {
    ...
    assert_remote_manual_compact_request_parity(
        ForestxAuth::from_api_key("dummy"),
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-forestx-beta-features")
            .as_deref()
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-forestx-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let mut builder = test_forestx().with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "forestx_app".to_string(),
        description: "Forestx app tools.".to_string(),
        tools: vec![
    ...
    let new_thread = test
        ...
        .await?;
    test.forestx = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "forestx_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "forestx_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_does_not_charge_inline_audio_payload_as_text() -> Result<()> {
    ...
    let responses_mock = responses::mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ...
                responses::ev_function_call_with_namespace(call_id, "forestx_app", tool_name, "{}"),
    ...
    test.forestx
        ...
        .await?;
    let EventMsg::DynamicToolCallRequest(request) = wait_for_event(&test.forestx, |event| {
    ...
    test.forestx
        ...
        .await?;
    wait_for_turn_complete(&test.forestx).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&forestx, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&forestx, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestForestxHarness::with_builder(
        test_forestx()
            .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_forestx = baseline_harness.test().forestx.clone();
    ...
    baseline_forestx
        ...
        .await?;
    wait_for_event(&baseline_forestx, |event| {
    ...
    baseline_forestx.submit(Op::Compact).await?;
    wait_for_event(&baseline_forestx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestForestxHarness::with_builder(
        test_forestx()
            .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_forestx = override_harness.test().forestx.clone();
    ...
    override_forestx
        ...
        .await?;
    wait_for_event(&override_forestx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_forestx.submit(Op::Compact).await?;
    wait_for_event(&override_forestx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = forestx.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    forestx.submit(Op::Shutdown).await?;
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.forestx.submit(Op::Compact).await?;
    wait_for_event(&initial.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
    ...
    wait_for_event(&initial.forestx, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.forestx
        ...
        .await?;
    wait_for_event(&test.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_forestx_builder(&realtime_server).with_config(|config| {
        config.model_auto_compact_token_limit = Some(200);
    });
    let test = builder.build(&server).await?;
    ...
    start_realtime_conversation(test.forestx.as_ref()).await?;
    ...
    close_realtime_conversation(test.forestx.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_forestx_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_forestx_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.forestx.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.forestx.as_ref()).await?;
    initial.forestx.submit(Op::Shutdown).await?;
    wait_for_event(&initial.forestx, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_forestx_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.forestx.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.forestx.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_forestx_builder(&realtime_server);
    let test = builder.build(&server).await?;
    ...
    for user in ["USER_ONE", "USER_TWO", "USER_THREE"] {
        if user == "USER_THREE" {
            core_test_support::submit_thread_settings(
                &forestx,
                forestx_protocol::protocol::ThreadSettingsOverrides {

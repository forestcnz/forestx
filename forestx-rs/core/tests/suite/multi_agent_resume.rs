...
fn configure_multi_agent_v2_with_role(
    config: &mut forestx_core::config::Config,
    ...
    let role_path = config.forestx_home.join("durable-worker-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    let mut initial_builder = test_forestx().with_config(move |config| {
    ...
    let worker_thread_id = loop {
        if let Some(thread_id) = initial_child_request
            ...
            .find_map(|request| {
                ...
                if body["client_metadata"]["x-forestx-parent-thread-id"] != json!(root_thread_id) {
                    return None;
                }
                body["client_metadata"]["thread_id"]
                    .as_str()
                    .and_then(|thread_id| forestx_protocol::ThreadId::from_string(thread_id).ok())
            })
        {
            break thread_id;
        }
    ...
    let body_for = |text: &str, thread: forestx_protocol::ThreadId| {
        requests
            .iter()
            .find_map(|request| {
    ...
    for (body, parent_thread, parent_turn) in [
        ...
        if let Some(parent_thread) = parent_thread {
            assert_eq!(
                body["client_metadata"]["x-forestx-parent-thread-id"],
                json!(parent_thread)
            );
        }

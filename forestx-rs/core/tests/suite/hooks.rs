...
use forestx_core::config::ThreadStoreConfig;
...
use forestx_plugin::PluginHookSource;
use forestx_plugin::PluginId;
use forestx_protocol::items::parse_hook_prompt_fragment;
...
fn trust_plugin_hooks(config: &mut Config, plugin_hook_sources: Vec<PluginHookSource>) {
    ...
    let listed = forestx_hooks::list_hooks(forestx_hooks::HooksConfig {
...
async fn stop_hook_can_block_multiple_times_in_same_turn() -> Result<()> {
    ...
    let hook_inputs = read_stop_hook_inputs(test.forestx_home_path())?;
...
async fn session_start_hook_sees_materialized_transcript_path() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(test.forestx_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
async fn session_start_runs_before_user_prompt_submit_on_first_turn() -> Result<()> {
    ...
    let hook_inputs = read_hook_order_inputs(test.forestx_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
...
async fn resumed_thread_runs_resume_then_compact_session_start_hooks() -> Result<()> {
    ...
    let hook_inputs = read_session_start_hook_inputs(resumed.forestx_home_path())?;
    assert_eq!(
        hook_inputs
            .iter()
            .filter_map(|input| input.get("source").and_then(Value::as_str))
...
async fn resumed_thread_keeps_stop_continuation_prompt_in_history() -> Result<()> {
    ...
    let mut resume_builder = test_forestx().with_config(trust_discovered_hooks);
    let resumed = resume_builder.restart(&server, &initial).await?;
...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    let hook_inputs = read_user_prompt_submit_hook_inputs(test.forestx_home_path())?;
...
async fn permission_request_hook_allow_bypasses_strict_auto_review() -> Result<()> {
    ...
    let request = wait_for_event(&test.forestx, |event| {
...
async fn permission_request_hook_allows_network_approval_without_prompt() -> Result<()> {
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); print('OK:' + opener.open('http://forestx-network-test.invalid', timeout=2).read().decode(errors='replace'))""#;
...
async fn permission_request_hook_denies_network_approval_with_custom_message() -> Result<()> {
    ...
    let command = r#"python3 -c "import urllib.request; opener = urllib.request.build_opener(urllib.request.ProxyHandler()); opener.open('http://forestx-network-test.invalid', timeout=2).read()""#;
    run_network_permission_hook_test(
        "deny",
        denial,
...
"#,
    ...
    let test = test_forestx()
        ...
        .with_config(move |config| {
            trust_discovered_hooks(config);
            config.approvals_reviewer = forestx_config::types::ApprovalsReviewer::AutoReview;
    ...
    if expected_denial.is_none() {
        ...
        assert!(
            timeout(
                ...
                wait_for_event(&test.forestx, |event| matches!(
                    event,
    ...
    assert_single_permission_request_hook_input(
        test.forestx_home_path(),
        command,
        Some("network-access http://forestx-network-test.invalid:80"),
    )?;
    let requests = responses.requests();
    assert_eq!(
        requests
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let hook_inputs = read_pre_tool_use_hook_inputs(test.forestx_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
...
"#
    ...
    let hook_inputs = read_post_tool_use_hook_inputs(test.forestx_home_path())?;
    assert_eq!(hook_inputs.len(), 1);
    assert_eq!(hook_inputs[0]["tool_input"]["command"], command);
    assert_eq!(
...
async fn plugin_pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    fs::create_dir_all(plugin_root.join(".forestx-plugin"))
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<forestx_config::HooksFile>(plugin_hooks_json)
...
async fn pre_tool_use_merges_hooks_json_and_config_toml() -> Result<()> {
    ...
    let json_hook_inputs = read_pre_tool_use_hook_inputs(test.forestx_home_path())?
        .into_iter()
        .map(|hook_input| {
            serde_json::json!({
...
async fn post_tool_use_blocks_when_exec_session_completes_via_write_stdin() -> Result<()> {
    ...
    let pre_hook_inputs = read_pre_tool_use_hook_inputs(test.forestx_home_path())?;
    ...
    let post_hook_inputs = read_post_tool_use_hook_inputs(test.forestx_home_path())?;

...
use forestx_core::config::TokenBudgetConfig;
...
use forestx_protocol::protocol::CONTEXT_WINDOW_CLOSE_TAG;
use forestx_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_CLOSE_TAG;
use forestx_protocol::protocol::CONTEXT_WINDOW_GUIDANCE_OPEN_TAG;
use forestx_protocol::protocol::CONTEXT_WINDOW_OPEN_TAG;
...
async fn assert_context_compaction_item_lifecycle(forestx: &std::sync::Arc<forestx_core::ForestxThread>) {
...
async fn token_budget_model_defaults_survive_config_lock_replay() -> Result<()> {
    ...
    let initial = test_forestx()
        ...
        .with_config(|config| {
            config.model_context_window = Some(CONFIGURED_CONTEXT_WINDOW);
            config.config_lock_export_dir = Some(config.forestx_home.join("memento-config-locks"));
            config
                .features
                .enable(Feature::TokenBudget)
                .expect("test config should allow token budget");
...
async fn token_budget_context_injects_plain_thread_hint_text() -> Result<()> {
    ...
    let test = test_forestx()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test.forestx, "notes").await?;
    let responses = mount_sse_sequence(
        &server,
        vec![sse(vec![
            ev_response_created("resp-1"),
...
async fn token_budget_context_uses_new_window_after_compaction() -> Result<()> {
    ...
    assert_context_compaction_item_lifecycle(&test.forestx).await;
...
async fn token_budget_compaction_runs_compact_hooks() -> Result<()> {
    ...
    let pre_compact = wait_for_event_match(&test.forestx, |event| match event {
    ...
    let post_compact = wait_for_event_match(&test.forestx, |event| match event {
        EventMsg::HookCompleted(completed)

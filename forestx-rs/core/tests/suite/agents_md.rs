...
use core_test_support::test_forestx::RecordingUserInstructionsProvider;
...
async fn agents_instructions(mut builder: TestForestxBuilder) -> Result<String> {
...
async fn submit_thread_turn(thread: &Arc<forestx_core::ForestxThread>, prompt: &str) -> Result<()> {
    thread
        .submit(Op::UserInput {
...
async fn agents_override_is_preferred_over_agents_md() -> Result<()> {
    let instructions =
        agents_instructions(test_forestx().with_workspace_setup(|cwd, fs| async move {
...
async fn loads_user_instructions_without_a_primary_environment() -> Result<()> {
    ...
    let provider = Arc::new(RecordingUserInstructionsProvider::new(Arc::new(
        ForestxHomeUserInstructionsProvider::new(AbsolutePathBuf::try_from(
...
async fn fresh_thread_composes_global_before_project_and_reports_sources() -> Result<()> {
    ...
    assert_eq!(test.forestx.instruction_sources().await, creation_sources);
...
async fn invalid_utf8_global_instructions_are_lossy() -> Result<()> {
    ...
    let mut builder = test_forestx().with_home(home);
    let test = builder.build(&server).await?;
...
async fn cold_resume_invalidates_deleted_legacy_agents_md_once() -> Result<()> {
    ...
    let mut initial_builder = test_forestx().with_home(Arc::clone(&home));
    let initial = initial_builder.build(&server).await?;

    // Assert the pre-resume thread reports the source used to create its snapshot.
    assert_eq!(
        initial.forestx.instruction_sources().await,
    ...
    let mut resume_builder = test_forestx().with_home(Arc::clone(&home));
...
async fn fork_injects_changed_agents_md_once() -> Result<()> {
    ...
    let mut builder = test_forestx().with_home(Arc::clone(&home));
    let parent = builder.build(&server).await?;

    // Assert the parent reports the source used to create its snapshot.
    assert_eq!(
        parent.forestx.instruction_sources().await,
        vec![PathUri::from_abs_path(&source)],
        "parent reports the creation-time global source"
    );
    parent.submit_turn("persist instructions").await?;
    parent.forestx.ensure_rollout_materialized().await;
    parent.forestx.flush_rollout().await?;
    let rollout_path = parent.forestx.rollout_path().expect("rollout path");
    ...
    fork_config.forestx_self_exe = parent.config.forestx_self_exe.clone();
    let forked = parent
        .thread_manager
        .fork_thread(

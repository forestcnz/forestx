...
use forestx_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "forestx-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://forestx-network-test.invalid:80";
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    test.forestx.submit(Op::Interrupt).await?;
    let mut saw_turn_aborted = false;
    let mut saw_guardian_aborted = false;
    while !saw_turn_aborted || !saw_guardian_aborted {
        let event = tokio::time::timeout(Duration::from_secs(5), test.forestx.next_event())
            .await
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert!(
        approval
            .call_id
            .starts_with("network#local#http#forestx-network-test.invalid#80#")
    );
    ...
    assert_eq!(
        ...
        Some("forestx-network-test.invalid is not in the allowed_domains")
    );
    test.forestx
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="forestx-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): forestx-network-test.invalid",
        )
    }));
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    .await;
    test.forestx.submit(Op::CleanBackgroundTerminals).await?;
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.forestx.list_background_terminals().await.is_empty() {
                break;
            }
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestForestx> {
...
async fn wait_for_completion_without_network_prompt(test: &TestForestx) {
...
async fn wait_for_turn_complete(test: &TestForestx) {
    wait_for_event(&test.forestx, |event| {

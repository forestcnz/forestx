...
fn skill_script_command(test: &TestForestx, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestForestx) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.forestx.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestForestx) {
    wait_for_event(test.forestx.as_ref(), |event| {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/forestx-zsh-fork-workspace-write-deny.txt";
    let workspace_write_profile = restrictive_workspace_write_profile();

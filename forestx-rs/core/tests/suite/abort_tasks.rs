...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    forestx.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&forestx, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_forestx()
        ...
        .await
        .unwrap();
    let forestx = Arc::clone(&fixture.forestx);

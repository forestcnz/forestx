...
const FIXED_CWD: &str = "/tmp/forestx_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> ForestxAuth {
        match self {
            AuthCase::ChatGpt => ForestxAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => ForestxAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&forestx, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().forestx_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestForestxHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness(mode: Mode, settings: RunSettings, hooks: bool) -> Result<TestForestxHarness> {
...
async fn build_harness_inner(
    ...
    TestForestxHarness::with_builder(builder.with_config(move |config| {
        config.cwd = forestx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(PathBuf::from(
            FIXED_CWD,
        ))
...
fn rollout_path(harness: &TestForestxHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    forestx: &forestx_core::ForestxThread,
    rollout_path: &Path,
...
async fn submit_user_input(forestx: &forestx_core::ForestxThread, items: Vec<UserInput>) -> Result<()> {
    forestx
        .submit(Op::UserInput {
            items,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<FORESTX_HOME>");
            search_start = start_index + "<FORESTX_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <FORESTX_HOME>/skills/.system/imagegen/SKILL.md and \
         <FORESTX_HOME>/skills/custom/SKILL.md"
    );
}

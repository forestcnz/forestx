...
use forestx_protocol::protocol::ElicitationAction;
...
fn set_calendar_approval_mode(config: &mut Config, approval_mode: AppToolApproval) {
    ...
    let user_config_path = config.forestx_home.join("config.toml").abs();
        ...
        .submit(Op::UserInput {
            ...
            thread_settings: forestx_protocol::protocol::ThreadSettingsOverrides {
                ...
                collaboration_mode: collaboration_mode.or({
                    Some(forestx_protocol::config_types::CollaborationMode {
...
async fn wait_for_mcp_tool_call_item(
    ...
    wait_for_event_match(&test.forestx, |event| {
...
async fn approved_mcp_tool_call_metadata_records_prior_user_input_request() -> Result<()> {
    ...
    let EventMsg::ElicitationRequest(request) = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
    ...
    assert_eq!(
        apps_tool_call
            .pointer("/params/_meta/x-forestx-turn-metadata/user_input_requested_during_turn"),
        Some(&json!(true))
    );
...
async fn apps_default_prompt_with_auto_review_routes_actual_mcp_approval_to_guardian() -> Result<()>
{
    ...
    let route_event = wait_for_event(&test.forestx, |event| {
        matches!(
            event,
            EventMsg::ElicitationRequest(_) | EventMsg::TurnComplete(_)
        )
...
async fn apps_default_writes_prompts_for_writes_but_not_reads() -> Result<()> {
    ...
    let EventMsg::McpToolCallBegin(read_begin) = wait_for_event(&test.forestx, |event| {
        matches!(event, EventMsg::McpToolCallBegin(_))
    })
    .await
    else {
    ...
    let next_route = wait_for_event(&test.forestx, |event| {
        matches!(
            event,

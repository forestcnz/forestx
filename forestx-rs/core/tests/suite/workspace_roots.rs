...
async fn workspace_roots_test(server: &MockServer) -> Result<TestForestx> {
...
fn outside_workspace_path(test: &TestForestx, file_name: &str) -> Result<PathUri> {
    let file_name = format!("forestx-workspace-roots-{}-{file_name}", std::process::id());
...
async fn submit_workspace_turn(test: &TestForestx, prompt: &str) -> Result<()> {
    test.submit_turn_with_permission_profile(prompt, workspace_roots_profile())
        .await
}

async fn read_file(test: &TestForestx, path: &PathUri) -> Result<String> {
    Ok(String::from_utf8(
        test.fs().read_file(path, /*sandbox*/ None).await?,
    )?)
}

async fn remove_files(test: &TestForestx, paths: &[&PathUri]) -> Result<()> {
    for path in paths {
        test.fs()
            .remove(
                path,
...
async fn workspace_roots_allow_patches_but_protect_metadata_directories() -> Result<()> {
    const PATCH_CONTENTS: &str = "workspace root patch access";
    const PROTECTED_METADATA_DIRECTORIES: [&str; 3] = [".git", ".agents", ".forestx"];

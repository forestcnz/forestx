...
use forestx_skills_extension::OrchestratorSkillProvider;
use forestx_skills_extension::SkillProvider;
use forestx_skills_extension::SkillProviderSource;
...
fn write_host_skills(forestx_home: &std::path::Path, skills: &[(&str, &str)]) -> Result<()> {
...
fn catalog_extensions(
    ...
    Arc<forestx_extension_api::ExtensionRegistry<Config>>,
...
fn configure_catalog_test(config: &mut Config) {
    ...
    let system_config_path = config.forestx_home.join("config.toml");
...
async fn rendered_catalogs_for_turns(
    ...
    if !host_skills.is_empty() {
        write_host_skills(forestx_home.path(), host_skills)?;
    }
    ...
    for _ in 0..turn_count {
        test.forestx
            ...
            .await?;
        loop {
            match core_test_support::wait_for_event(&test.forestx, |_| true).await {
    ...
    // while core warnings are delivered through the TestForestx event stream.
...
async fn explicit_only_orchestrator_skill_is_hidden_but_can_be_invoked() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path_regex("^/api/forestx/ps/mcp/?$"))
        .and(|request: &Request| {
    ...
    let test = builder.build_with_auto_env(&server).await?;
    wait_for_mcp_server(&test.forestx, FORESTX_APPS_MCP_SERVER_NAME).await?;
...
async fn production_turn_preserves_host_alias_root_order_across_turns() -> Result<()> {
    ...
    let long_parent = temp_parent
        .path()
        .join("forestx-home-with-long-shared-prefix-for-production-alias-order-test");
    std::fs::create_dir_all(&long_parent)?;
    let forestx_home = Arc::new(TempDir::new_in(&long_parent)?);
...
async fn production_turn_uses_provider_host_catalog_and_core_snapshot_injection() -> Result<()> {
    ...
    write_host_skills(forestx_home.path(), &[(skill_name, snapshot_description)])?;
    let skill_path = forestx_home
        .path()
        .join("skills")
        .join(skill_name)
        .join("SKILL.md");
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "forestx_app_mentioned")
                        .cloned()
                })
            })
        {
...
async fn production_turn_keeps_full_snapshot_host_skill_prompt() -> Result<()> {
    ...
    .await;
    let forestx_home = Arc::new(TempDir::new()?);
    let skill_dir = forestx_home.path().join("skills").join("long-host");
    std::fs::create_dir_all(&skill_dir)?;
...
async fn production_turn_fairly_shortens_extension_catalog_descriptions() -> Result<()> {
    ...
    assert_eq!(
        warning.message,
        "Skill descriptions were shortened to fit the skills context budget. Forestx can still see every skill, but some descriptions are shorter. Disable unused skills or plugins to leave more room for the rest."
    );

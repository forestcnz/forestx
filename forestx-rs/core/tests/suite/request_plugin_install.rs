...
use forestx_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__forestx_apps__calendar";
...
async fn start_gated_step_preparation(test: &TestForestx, server: &MockServer) -> Result<PathUri> {
...
async fn start_install_turn(test: &TestForestx, prompt: &str) -> Result<ElicitationRequestEvent> {
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, test.config.cwd.as_path());
    test.forestx
        .submit(Op::UserInput {
    ...
    Ok(wait_for_event_match(&test.forestx, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "forestx_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": forestx_login::default_client::originator().value,
            }
        })
    );
...
async fn run_remote_plugin_install_refresh_case(refreshed_tools: RefreshedAppsTools) -> Result<()> {
    ...
    drop(requests);
    test.forestx.refresh_runtime_config(test.config.clone()).await;

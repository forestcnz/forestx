...
//! `just test -p forestx-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let forestx_home = home.path().join(".forestx");
    ...
    let mut cmd = Command::new(forestx_utils_cargo_bin::cargo_bin("forestx-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn forestx-rs");

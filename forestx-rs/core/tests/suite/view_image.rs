...
fn disabled_user_turn(test: &TestForestx, items: Vec<UserInput>, model: String) -> Op {
...
async fn create_workspace_directory(test: &TestForestx, rel_path: &str) -> anyhow::Result<PathBuf> {
...
async fn view_image_tool_attaches_local_image() -> anyhow::Result<()> {
    ...
    wait_for_event_with_timeout(
        ...
        |event| match event {
            EventMsg::ItemStarted(event) => {
                if matches!(&event.item, forestx_protocol::items::TurnItem::ImageView(_)) {
    ...
    match item_started.expect("view image item started event emitted") {
        forestx_protocol::items::TurnItem::ImageView(item) => {

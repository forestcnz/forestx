...
use forestx_config::types::McpServerAuth;
...
use forestx_exec_server::HttpRedirectPolicy;
use forestx_exec_server::HttpRequestParams;
use forestx_features::Feature;
use forestx_http_client::HttpClientBuilder;
use forestx_login::ForestxAuth;
use forestx_mcp::FORESTX_APPS_MCP_SERVER_NAME;
use forestx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use forestx_utils_cargo_bin::cargo_bin;
...
fn read_only_user_turn(fixture: &TestForestx, text: impl Into<String>) -> Op {
...
fn auto_approved_user_turn(fixture: &TestForestx, text: impl Into<String>) -> Op {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        forestx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/forestx-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.forestx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.forestx, |ev| {
    ...
    wait_for_event(&fixture.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn mcp_namespace_instructions_are_preserved_without_hiding_tools() -> anyhow::Result<()> {
    ...
    let fixture = test_forestx()
        ...
        .await?;
    wait_for_mcp_server(&fixture.forestx, "bounded").await?;

    fixture
        .forestx
        .submit(read_only_user_turn(&fixture, "show the bounded MCP tools"))
        .await?;
    wait_for_event(&fixture.forestx, |event| {
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.forestx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn stdio_mcp_tool_names_respect_selected_servers() -> anyhow::Result<()> {
    ...
    let command = remote_aware_stdio_server_bin()?;
    let fixture = test_forestx()
        .with_pre_build_hook(move |forestx_home| {
            fs::write(
...
"#,
        ...
        .build_with_auto_env(&server)
        .await?;
    wait_for_mcp_server(&fixture.forestx, "history").await?;
...
-> anyhow::Result<()> {
    ...
    let startup = loop {
        let event = fixture.forestx.next_event().await?;
...
async fn apps_enabled_turn_skips_pending_optional_mcp_without_cached_tools() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            let event = fixture
                ...
                .next_event()
                .await
                .context("event stream ended before Forestx Apps became ready")?;
            if let EventMsg::McpStartupUpdate(update) = event.msg
                && update.server == FORESTX_APPS_MCP_SERVER_NAME
    ...
    .await
    .context("Forestx Apps should finish starting before the first turn")??;
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.forestx.shutdown_and_wait())
        .await
...
async fn interrupt_during_mcp_startup_preserves_user_input_in_history(
    ...
    fixture.forestx.submit(Op::Interrupt).await?;
    wait_for_event(&fixture.forestx, |event| {
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            forestx_linux_sandbox_exe: fixture.config.forestx_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.forestx, |ev| {
            matches!(
                ev,
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: forestx_protocol::openai_models::ReasoningEffort::Medium,
    ...
    fixture
        ...
        .list_models(
            ...
            forestx_core::test_support::default_http_client_factory(),
        )
        .await;
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that Forestx should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: Forestx can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Forestx will call the MCP echo tool
    ...
    // Phase 3: configure Forestx with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert Forestx begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.forestx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_forestx()
        .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.forestx, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    let untrusted_mcp_url = format!("{}/api/forestx/ps/mcp", untrusted_apps.chatgpt_base_url);
    ...
    wait_for_mcp_server(&fixture.forestx, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/forestx/ps/mcp")
        .filter_map(|request| {
...
/// This test writes to a fallback credentials file in FORESTX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire FORESTX_HOME through the code.
#[test]
#[serial(forestx_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so Forestx will call the OAuth-backed
    ...
    // Phase 3: seed an isolated FORESTX_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _forestx_home_guard = EnvVarGuard::set("FORESTX_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure Forestx with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert Forestx begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.forestx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the Forestx fixture.
...
fn write_fallback_oauth_tokens(
    ...
    forestx_rmcp_client::save_oauth_tokens(
        server_name,
        &tokens,
        OAuthCredentialsStoreMode::File,
        forestx_config::types::AuthKeyringBackendKind::default(),
    )
}

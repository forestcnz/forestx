...
async fn responses_api_parent_and_subagent_requests_include_identity_headers() -> Result<()> {
    ...
    let parent_window_id = parent
        ...
        .ok_or_else(|| anyhow!("parent request missing x-forestx-window-id"))?;
    let child_window_id = child
        .header("x-forestx-window-id")
        .ok_or_else(|| anyhow!("child request missing x-forestx-window-id"))?;
    ...
    assert_eq!(
        child.header("x-forestx-parent-thread-id").as_deref(),
        Some(parent_thread_id)
    );
    let child_turn_metadata: serde_json::Value = serde_json::from_str(
        &child
            .header("x-forestx-turn-metadata")
            .ok_or_else(|| anyhow!("child request missing x-forestx-turn-metadata"))?,
    )?;
...
async fn submit_turn_with_timeout(test: &TestForestx, prompt: &str) -> Result<()> {

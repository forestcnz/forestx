...
async fn submit_turn(test: &TestForestx, permission_profile: PermissionProfile) -> Result<String> {
...
await tools.exec_command({
  ...
  justification: "test command approval",
    ...
    .await?;
    let approval = wait_for_event_match(&harness.test.forestx, |event| match event {
...
await tools.request_permissions({
  ...
  permissions: { network: { enabled: true } },
    ...
    .await?;
    let request = wait_for_event(&harness.test.forestx, |event| {
        matches!(
            event,

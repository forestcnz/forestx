...
use forestx_protocol::protocol::ExecCommandStatus;
use forestx_protocol::protocol::Op;
use forestx_protocol::user_input::UserInput;
use forestx_utils_output_truncation::approx_tokens_from_byte_count;
...
async fn wait_for_raw_unified_exec_output(
    ...
    let content = wait_for_event_match(&test.forestx, |event| match event {
...
async fn exec_command_hides_and_rejects_login_when_disabled() -> Result<()> {
    ...
    let builder = test_forestx().with_model("gpt-5.4").with_config(|config| {
...
async fn unified_exec_intercepts_apply_patch_exec_command() -> Result<()> {
    ...
    let builder = test_forestx().with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config
            .features
            .enable(Feature::UnifiedExec)
    ...
    wait_for_event(&forestx, |event| match event {
...
async fn unified_exec_rejects_justification_without_sandbox_permissions() -> Result<()> {
    ...
    let mut builder = test_forestx().with_model("gpt-5.2").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
        config
            .features
            .enable(Feature::UnifiedExec)
...
async fn unified_exec_emits_exec_command_begin_event() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&test.forestx, |msg| match msg {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    let end_event = wait_for_event_match(&test.forestx, |msg| match msg {
...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    let event = wait_for_event_match(&test.forestx, |msg| match msg {
        EventMsg::ExecCommandEnd(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn unified_exec_full_lifecycle_with_background_end_event() -> Result<()> {
    ...
    loop {
        let msg = wait_for_event(&test.forestx, |_| true).await;
        match msg {
...
async fn unified_exec_network_denial_emits_failed_background_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, time, urllib.parse; time.sleep(0.3); proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://forestx-network-denied.invalid/ HTTP/1.1\\r\\nHost: forestx-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024); time.sleep(5)\"",
        "yield_time_ms": 50,
    });
    let response_mock =
...
async fn unified_exec_short_lived_network_denial_emits_failed_end_event() -> Result<()> {
    ...
    let args = json!({
        "cmd": "python3 -c \"import os, socket, urllib.parse; proxy = urllib.parse.urlparse(os.environ['HTTP_PROXY']); sock = socket.create_connection((proxy.hostname, proxy.port), timeout=2); sock.sendall(b'GET http://forestx-short-network-denied.invalid/ HTTP/1.1\\r\\nHost: forestx-short-network-denied.invalid\\r\\n\\r\\n'); sock.recv(1024)\"",
        "yield_time_ms": 1000,
    });
    let response_mock =
        mount_unified_exec_network_denial_responses(&server, call_id, &args).await?;
...
) -> Result<(TestForestx, PermissionProfile)> {
...
async fn wait_for_unified_exec_end(
...
) -> (forestx_protocol::protocol::ExecCommandEndEvent, bool) {
    ...
    let end_event = loop {
        ...
        let event = tokio::time::timeout(remaining, test.forestx.next_event())
            .await
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let mut task_completed = false;
    loop {
        let event_msg = wait_for_event(&test.forestx, |_| true).await;
        match event_msg {
...
async fn exec_command_clamps_model_requested_max_output_tokens_to_policy() -> Result<()> {
    ...
    let mut builder = test_forestx().with_model("gpt-5.4").with_config(|config| {
        config.use_experimental_unified_exec_tool = true;
...
async fn write_stdin_returns_exit_metadata_and_clears_session() -> Result<()> {
    ...
    let mut turn_completed = false;
    loop {
        let event = wait_for_event(&test.forestx, |_| true).await;
        match event {
...
async fn unified_exec_keeps_long_running_session_after_turn_end() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&forestx, |msg| match msg {
...
async fn unified_exec_interrupt_preserves_long_running_session() -> Result<()> {
    ...
    let _begin_event = wait_for_event_match(&forestx, |msg| match msg {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    ...
    forestx.submit(Op::Interrupt).await?;
    wait_for_event(&forestx, |event| matches!(event, EventMsg::TurnAborted(_))).await;
    ...
    forestx.submit(Op::CleanBackgroundTerminals).await?;
    wait_for_process_exit(&pid).await?;
...
async fn unified_exec_timeout_and_followup_poll() -> Result<()> {
    ...
    loop {
        let event = test.forestx.next_event().await.expect("event");
...
"#
    ...
    let end_event = wait_for_event_match(&test.forestx, |event| match event {

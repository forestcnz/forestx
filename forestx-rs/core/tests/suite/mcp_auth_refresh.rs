...
use forestx_mcp::ForestxAppsToolsCache;
use forestx_mcp::EffectiveMcpServer;
use forestx_mcp::McpRuntime;
use forestx_mcp::McpRuntimeContext;
use forestx_mcp::McpRuntimeInput;
use forestx_mcp::McpToolCatalogCache;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let expected_auth = ForestxAuth::from_external_chatgpt_tokens(
        "header.e30.first",
        "test-account",
    ...
    let mut hosted_plugin_runtime_config = forestx_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let plugins_manager = forestx_core_plugins::PluginsManager::new(home.path().to_path_buf());
    ...
    let runtime = McpRuntime::new(McpRuntimeInput {
        ...
        forestx_apps_tools_cache: ForestxAppsToolsCache::default(),
        tool_catalog_cache: McpToolCatalogCache::default(),
        forestx_apps_tools_cache_key: forestx_mcp::forestx_apps_tools_cache_key(Some(&expected_auth)),
        client_mcp_extensions: ClientMcpExtensions::default(),
        auth: Some(expected_auth.clone()),
        forestx_apps_auth_manager: Some(Arc::clone(&auth_manager)),
    ...
    auth_manager
        .set_external_auth(Arc::new(StaticExternalAuth(
            ForestxAuth::from_external_chatgpt_tokens(
                "header.e30.reloaded",
                "test-account",
                /*chatgpt_plan_type*/ None,
            )?,
    ...
    // so this tool call only sees the new token if the Forestx Apps provider
    ...
    let tool_call_request = requests
        ...
        .expect("Forestx Apps should receive a tool call");
    assert_eq!(
        tool_call_request
            .headers
            .get("authorization")

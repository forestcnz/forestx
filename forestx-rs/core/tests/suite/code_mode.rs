...
use forestx_extension_api::ToolContributor;
...
use forestx_protocol::openai_models::ToolMode;
...
use forestx_tools::FunctionCallError;
use forestx_tools::JsonToolOutput;
use forestx_tools::ResponsesApiNamespace;
use forestx_tools::ResponsesApiNamespaceTool;
use forestx_tools::ToolCall;
use forestx_tools::ToolExecutor;
use forestx_tools::ToolExecutorFuture;
use forestx_tools::ToolName;
use forestx_tools::ToolOutput;
use forestx_tools::ToolPayload;
use forestx_tools::ToolSpec;
use forestx_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestForestx, ResponseMock)> {
...
async fn run_code_mode_turn_with_config(
    ...
    run_code_mode_turn_with_model_and_config(server, prompt, code, "test-gpt-5.1-forestx", configure)
        .await
}
...
async fn run_code_mode_turn_with_model_and_config(
    ...
    let builder = test_forestx().with_model(model).with_config(move |config| {
...
async fn run_code_mode_turn_with_builder(
    ...
    prompt: &str,
    code: &str,
    mut builder: TestForestxBuilder,
) -> Result<(TestForestx, ResponseMock)> {
...
async fn run_unavailable_code_mode_turn(
    ...
    loop {
        match wait_for_event(&test.forestx, |_| true).await {
...
async fn missing_process_host_falls_back_to_direct_tools_and_warns_once() -> Result<()> {
    ...
    let mut builder = test_forestx()
        .with_model("test-gpt-5.1-forestx")
        .with_code_mode_host_program("forestx-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
    ...
    assert!(
        first_warnings.iter().any(|warning| {
            warning.contains("Code Mode is unavailable")
                && warning.contains("forestx-code-mode-host-does-not-exist")
        }),
...
async fn missing_process_host_keeps_code_mode_only_and_fails_closed() -> Result<()> {
    ...
    assert!(
        output.contains("forestx-code-mode-host-does-not-exist"),
...
"#,
    ...
    let auth = ForestxAuth::from_api_key("dummy");
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-forestx")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-forestx").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let mut builder = test_forestx().with_model(model).with_config(move |config| {
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.forestx, "rmcp").await?;
...
async fn code_mode_exec_holds_captured_result_during_elicitation() -> Result<()> {
    ...
    assert_eq!(
        test.forestx.increment_out_of_band_elicitation_count().await?,
        1
    );
    assert_eq!(
        test.forestx.increment_out_of_band_elicitation_count().await?,
    ...
    let release_elicitation = async {
        ...
        assert_eq!(
            test.forestx.decrement_out_of_band_elicitation_count().await?,
            1
        );
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__forestx_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__forestx_apps__calendar_timezone_option_99",
...
async fn code_mode_wait_timeout_reconnects_on_next_exec() -> Result<()> {
    ...
    test.forestx
        ...
        .await?;
    wait_for_event_match(&test.forestx, |event| match event {
...
"#,
        ...
        test.forestx
            .submit(Op::UserInput {
                ...
                thread_settings: forestx_protocol::protocol::ThreadSettingsOverrides {
                    environments: Some(forestx_protocol::protocol::TurnEnvironmentSelections::new(
                        cwd,
                        Vec::new(),
                    )),
        ...
        let turn_id = wait_for_event_match(&test.forestx, |event| match event {
        ...
        test.forestx
            ...
            .await?;
        wait_for_event(&test.forestx, |event| match event {
        ...
        if use_responses_lite {
            let metadata: Value = serde_json::from_str(
                first_body["client_metadata"]["x-forestx-turn-metadata"]
                    .as_str()
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "forestx_app__hidden_dynamic_tool");
const out = await tools.forestx_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
    description: tool?.description ?? null,
...
"#;
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("forestx_app"));
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String("forestx_app__hidden_dynamic_tool".to_string()))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .and_then(Value::as_str)
            .is_some_and(|description| {
                description.contains("Forestx app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("forestx_app__hidden_dynamic_tool(args:")
            })
    );

...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    forestx
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&forestx, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_command_without_local_environment_emits_error() -> anyhow::Result<()> {
    ...
    let EventMsg::Error(error) =
        wait_for_event(&test.forestx, |event| matches!(event, EventMsg::Error(_))).await
    else {
        unreachable!()
    };
    assert_eq!(error.message, "shell is unavailable in this session");
    assert_eq!(error.forestx_error_info, None);
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _begin = wait_for_event_match(forestx, |ev| match ev {
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.forestx, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.forestx.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_forestx::test_forestx().with_config(move |config| {
        config
            .features
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:FORESTX_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${FORESTX_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.forestx, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.forestx, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.forestx, |ev| match ev {
    ...
    let _ = wait_for_event(&test.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_forestx::test_forestx().with_config(|config| {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:FORESTX_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command =
        r#"sh -c "printf '%s' \"${FORESTX_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();
    ...
    let ExecCommandEndEvent {
        ...
        stderr,
        ..
    } = wait_for_event_match(&test.forestx, |ev| match ev {
        EventMsg::ExecCommandEnd(event) => Some(event.clone()),
        _ => None,
    })
    .await;
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_forestx::test_forestx();
    let test = builder
        .with_config(|config| {

...
use core_test_support::test_forestx::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestForestxHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestForestxBuilder) -> TestForestxBuilder,
) -> Result<TestForestxHarness> {
    let builder = configure(test_forestx());
    // Box harness construction so apply_patch_cli tests do not inline the
    // full test-thread startup path into each test future.
    Box::pin(TestForestxHarness::with_auto_env_builder(builder)).await
}

async fn submit_without_wait(harness: &TestForestxHarness, prompt: &str) -> Result<()> {
...
async fn apply_patch_cli_uses_forestx_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let forestx_linux_sandbox_exe = harness
        .test()
        .config
        .forestx_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include forestx-linux-sandbox helper");
    assert_eq!(
        forestx_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(FORESTX_LINUX_SANDBOX_ARG0),
    );
...
async fn apply_patch_custom_tool_streaming_emits_updated_changes() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(&forestx_protocol::protocol::FileChange::Add {
            content: String::new(),
        })
    );
    assert_eq!(

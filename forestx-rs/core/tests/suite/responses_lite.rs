...
const RESPONSES_LITE_HEADER: &str = "x-openai-internal-forestx-responses-lite";

fn responses_extensions(auth: &ForestxAuth) -> Arc<ExtensionRegistry<Config>> {
...
fn configure_image_capable_model(model_info: &mut forestx_protocol::openai_models::ModelInfo) {
...
async fn responses_lite_uses_input_items_for_instructions_and_tools() -> Result<()> {
    ...
    let turn_metadata: Value = serde_json::from_str(
        client_metadata["x-forestx-turn-metadata"]
            .as_str()
    ...
    assert!(!client_metadata.contains_key("x-forestx-code-mode-tool-names"));
...
async fn responses_lite_prepares_images() -> Result<()> {
    ...
    let mut builder = test_forestx().with_model_info_override("gpt-5.4", |model_info| {
        model_info.use_responses_lite = true;

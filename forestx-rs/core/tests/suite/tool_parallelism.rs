...
async fn run_turn(test: &TestForestx, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestForestx, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_forestx_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestForestx> {
    let mut builder = test_forestx().with_model("test-gpt-5.1-forestx");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_forestx_with_test_tool(&server).await?;

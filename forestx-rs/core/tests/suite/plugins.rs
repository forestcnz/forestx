...
use forestx_core_plugins::store::PluginStore;
...
use forestx_skills_extension::SkillsExtensionConfig;
use forestx_skills_extension::install;
...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__forestx_apps__google_calendar";
...
async fn build_analytics_plugin_test_forestx(
    server: &MockServer,
    forestx_home: Arc<TempDir>,
) -> Result<TestForestx> {
    let chatgpt_base_url = server.uri();
    let mut builder = test_forestx()
        .with_home(forestx_home)
...
async fn build_apps_enabled_plugin_test_forestx(
    server: &MockServer,
    forestx_home: Arc<TempDir>,
...
async fn persisted_remote_plugin_command_attribution_flows_through_turn_context() -> Result<()> {
    ...
    skip_if_remote!(
        Ok(()),
        "remote plugin attribution fixture uses a local Forestx home cache"
    );

    let server = start_mock_server().await;
    let forestx_home = Arc::new(TempDir::new()?);
    let script_path = write_remote_plugin_script_and_config(forestx_home.as_ref());
    ...
    let mut builder = test_forestx()
        .with_home(Arc::clone(&forestx_home))
        .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model("gpt-5.2");
    let test_forestx = builder.build_with_auto_env(&server).await?;
    let forestx = Arc::clone(&test_forestx.forestx);
    let cwd = test_forestx.config.cwd.clone();
    let session_model = test_forestx.session_configured.model.clone();
    let (sandbox_policy, permission_profile) =
        turn_permission_fields(PermissionProfile::Disabled, cwd.as_path());
    forestx
        .submit(Op::UserInput {
            items: vec![forestx_protocol::user_input::UserInput::Text {
    ...
    let begin = wait_for_event_match(&forestx, |event| match event {
        EventMsg::ExecCommandBegin(event) if event.call_id == call_id => Some(event.clone()),
        _ => None,
    })
    .await;
    let end = wait_for_event_match(&forestx, |event| match event {
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let forestx_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(forestx_home.as_ref());
    write_plugin_app_plugin(forestx_home.as_ref());
    let mut builder = test_forestx()
        .with_home(Arc::clone(&forestx_home))
    ...
    let test_forestx = builder.build(&server).await?;
    let forestx = Arc::clone(&test_forestx.forestx);
...
async fn agent_plugin_skills_use_shared_catalog_and_direct_child_discovery() -> Result<()> {
    ...
    .await;
    let forestx_home = Arc::new(TempDir::new()?);
    let skill_path = std::fs::canonicalize(write_agent_plugin_skill_plugin(forestx_home.as_ref()))?;
    let test_forestx = test_forestx()
        .with_home(Arc::clone(&forestx_home))
        .with_extensions(skills_extensions())
        .build(&server)
        .await?;
    ...
    test_forestx
        ...
        .await?;
    let warning = wait_for_event(&test_forestx.forestx, |ev| {
        matches!(
            ev,
    ...
    .await;
    wait_for_event(&test_forestx.forestx, |ev| {
...
async fn legacy_plugin_skill_prompt_remains_complete() -> Result<()> {
    ...
    .await;
    let forestx_home = Arc::new(TempDir::new()?);
    let skill_path = write_plugin_skill_plugin(forestx_home.as_ref());
...
async fn agent_plugin_root_mcp_stdio_tool_round_trip_expands_reserved_paths() -> Result<()> {
    ...
    .await;
    let forestx_home = Arc::new(TempDir::new()?);
    write_agent_plugin_skill_plugin(forestx_home.as_ref());
    ...
    let test_forestx = test_forestx()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&test_forestx.forestx, "agent").await?;
    let data_root = std::fs::read_dir(forestx_home.path().join("plugins/data/agent-plugins"))?
        .next()
    ...
    test_forestx
        ...
        .await?;
    let end = wait_for_event(&test_forestx.forestx, |event| {
        matches!(event, EventMsg::McpToolCallEnd(_))
    })
    .await;
    wait_for_event(&test_forestx.forestx, |event| {
...
async fn agent_turns_route_curated_plugin_skills_after_auth_switch() -> Result<()> {
    ...
    const FIXTURES: &[Fixture] = &[
        ...
        Fixture {
            ...
            target_auth: TargetAuth::NoForestxAuth,
            target_model_provider_id: AMAZON_BEDROCK_PROVIDER_ID,
...
"#,
        ...
        let mut builder = test_forestx()
            ...
            .with_auth(ForestxAuth::create_dummy_chatgpt_auth_for_testing());
        let test_forestx = builder.build_with_auto_env(&server).await?;
        let plugins_manager = test_forestx.thread_manager.plugins_manager();
        let skills_service = test_forestx.thread_manager.skills_service();
        ...
        match fixture.target_auth {
            ...
            TargetAuth::NoForestxAuth => {
                test_forestx.thread_manager.auth_manager().logout().await?;
                assert_eq!(
                    test_forestx.thread_manager.auth_manager().get_api_auth_mode(),
                    None
                );
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins(
    ...
    write_plugin_skill_plugin(forestx_home.as_ref());
    write_plugin_mcp_plugin(forestx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(forestx_home.as_ref());
    ...
    let test_forestx =
        build_apps_enabled_plugin_test_forestx(&server, forestx_home, apps_server.chatgpt_base_url)
            .await?;
    let forestx = Arc::clone(&test_forestx.forestx);
    wait_for_mcp_server(&forestx, FORESTX_APPS_MCP_SERVER_NAME).await?;

    forestx
        .submit(Op::UserInput {
            items: vec![forestx_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(forestx_home.as_ref());
    write_plugin_mcp_plugin(forestx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(forestx_home.as_ref(), "sample_app");

    let test_forestx =
        build_apps_enabled_plugin_test_forestx(&server, forestx_home, apps_server.chatgpt_base_url)
            .await?;
    let forestx = Arc::clone(&test_forestx.forestx);
    wait_for_mcp_server(&forestx, "sample").await?;
...
async fn explicitly_requested_mcp_waits_for_startup(request: ExplicitMcpRequest) -> Result<()> {
    ...
    let skill_path = std::fs::canonicalize(write_plugin_skill_plugin(forestx_home.as_ref()))?;
    write_plugin_mcp_plugin(forestx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(forestx_home.as_ref());
    let initialize_barrier = block_plugin_mcp_startup(forestx_home.as_ref(), &rmcp_test_server_bin);
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let forestx_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(forestx_home.as_ref());
    let test_forestx = build_analytics_plugin_test_forestx(&server, forestx_home).await?;
    let forestx = Arc::clone(&test_forestx.forestx);
    ...
    let event = wait_for_analytics_event(&server, "forestx_plugin_used").await;
    ...
    assert_eq!(
        ...
        serde_json::json!(forestx_login::default_client::originator().value)
    );
...
async fn explicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let forestx_home = Arc::new(TempDir::new()?);
    let skill_path = std::fs::canonicalize(write_remote_plugin_skill_plugin(forestx_home.as_ref()))?;
    persist_sample_remote_plugin_id(forestx_home.as_ref());
    let test_forestx = build_analytics_plugin_test_forestx(&server, forestx_home).await?;
    let forestx = Arc::clone(&test_forestx.forestx);
...
async fn implicit_plugin_skill_invocation_tracks_remote_plugin_id() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let forestx_home = Arc::new(TempDir::new()?);
    let skill_path = write_remote_plugin_skill_plugin(forestx_home.as_ref());
    persist_sample_remote_plugin_id(forestx_home.as_ref());
...
async fn wait_for_analytics_event(server: &MockServer, event_type: &str) -> serde_json::Value {
    ...
    loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/forestx/analytics-events/events")
            .find_map(|request| {

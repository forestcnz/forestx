use anyhow::Result;
use forestx_core::config::RolloutBudgetConfig;
                ...
                json!({
                    ...
                    "response": {
                        ...
                        "usage": {
                            ...
                            "total_tokens": 75,
                            "forestx_rollout_budget_units": rollout_budget_units
                        }
                    }
                }),
            ]),
...
async fn invalid_provider_rollout_budget_units_fail_without_retry() -> Result<()> {
    ...
    completed["response"]["usage"]["forestx_rollout_budget_units"] = json!(-1.0);
    let responses = mount_sse_sequence(
        &server,
    ...
    assert_eq!(
        error.message,
        "Fatal error: response.completed usage.forestx_rollout_budget_units must be finite and non-negative"
    );
    assert_eq!(error.forestx_error_info, Some(ForestxErrorInfo::Other));
        ...
        wait_for_event(&test.forestx, |event| {
            matches!(
                ...
                EventMsg::Error(error)
                    if error.forestx_error_info == Some(ForestxErrorInfo::SessionBudgetExceeded)
            )
        })
        .await;
        wait_for_event(&test.forestx, |event| {
...
async fn compaction_budget_exhaustion_fails_without_retry(
    ...
    if provider_units {
        completed["response"]["usage"]["forestx_rollout_budget_units"] = json!(10.0);
    }

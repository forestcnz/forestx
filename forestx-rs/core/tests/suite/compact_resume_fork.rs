...
//! request payload that Forestx would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<ForestxThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.forestx)
}

async fn user_turn(conversation: &Arc<ForestxThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<ForestxThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<ForestxThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<ForestxThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = forestx_core::test_support::auth_manager_from_auth(

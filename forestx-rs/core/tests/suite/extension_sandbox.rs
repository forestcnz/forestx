...
use forestx_image_generation_extension::install as install_image_generation_extension;
...
async fn extension_tool_receives_turn_environment_sandbox() -> Result<()> {
    ...
    let extensions = image_generation_extensions(&auth, |config| Some(config.forestx_home.clone()));
...
async fn extension_tool_uses_granted_turn_permissions_without_local_persistence() -> Result<()> {
    ...
    assert!(!test.config.forestx_home.join("generated_images").exists());

...
async fn request_user_input_round_trip_for_mode(mode: ModeKind) -> anyhow::Result<()> {
    ...
    let request = wait_for_event_match(&forestx, |event| match event {
    ...
    wait_for_event(&forestx, |event| matches!(event, EventMsg::TokenCount(_))).await;
...
async fn request_user_input_interrupt_emits_deferred_token_count() -> anyhow::Result<()> {
    ...
    let token_count = wait_for_event_match(&forestx, |event| match event {

...
use forestx_models_manager::cache::ModelsCacheEntry;
use forestx_models_manager::cache::ModelsCacheError;
use forestx_models_manager::cache::ModelsCacheFuture;
...
async fn run_agent_with_model(models_manager: SharedModelsManager, model_slug: &str) -> Result<()> {
    ...
    loop {
        if matches!(
            wait_for_event(&test.forestx, |_| true).await,
...
async fn injected_cache_hit_drives_agent_model_selection() -> Result<()> {
    ...
    let cache = Arc::new(TestModelsCache {
        entry: Some(ModelsCacheEntry {
            fetched_at: Utc::now(),
            etag: None,
            client_version: Some(forestx_models_manager::client_version_to_whole()),

...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.forestx);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.forestx, "after resume").await?;
    shutdown_thread(&resumed.forestx).await?;
...
async fn submit_user_turn(forestx: &Arc<ForestxThread>, text: &str) -> Result<()> {
    forestx
        .submit(Op::UserInput {
...
async fn submit_compact_turn(forestx: &Arc<ForestxThread>) -> Result<()> {
    forestx.submit(Op::Compact).await?;
    let warning_event = wait_for_event(forestx, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(forestx: &Arc<ForestxThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        .header("x-forestx-window-id")
        .expect("missing x-forestx-window-id header");

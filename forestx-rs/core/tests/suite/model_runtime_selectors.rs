...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != forestx_code_mode::PUBLIC_TOOL_NAME
                && name != forestx_code_mode::WAIT_TOOL_NAME),
    ...
    assert_eq!(
        ...
        vec![
            ...
            forestx_code_mode::PUBLIC_TOOL_NAME.to_string(),
            forestx_code_mode::WAIT_TOOL_NAME.to_string(),
    ...
    assert!(
        tool_names(&unsupported_response.body)
            .iter()
            .any(|name| name == forestx_code_mode::PUBLIC_TOOL_NAME)
    );
    assert_eq!(
...
async fn remote_code_mode_only_selector_fails_closed_when_host_is_disabled() -> Result<()> {
    ...
    assert!(
        tools
            ...
            && tools
                .iter()
                .any(|name| name == forestx_code_mode::WAIT_TOOL_NAME),
...
async fn remote_multi_agent_selector_uses_model_selected_before_first_turn() -> Result<()> {
    ...
    assert_eq!(
        (
            ...
            test.forestx.multi_agent_version(),
        ),
        (1, None)
    );
    ...
    .await?;
    assert_eq!(test.forestx.multi_agent_version(), None);

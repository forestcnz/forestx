...
async fn windows_exec_server_runs_with_native_shell_and_cwd() -> Result<()> {
    ...
    const PATCH_FILE: &str = "forestx-apply-patch-smoke.txt";
    const COMMAND: &str = r#"if ((Get-Location).Path -ne 'C:\windows') { exit 1 }"#;
    const VERIFY_COMMAND: &str = r#"$path = Join-Path (Get-Location) 'forestx-apply-patch-smoke.txt'; if (-not (Test-Path $path)) { exit 1 }; if ([IO.File]::ReadAllText($path) -ne "patched through unified exec`n") { exit 2 }; Remove-Item $path; Write-Output 'PATCH_VERIFIED'"#;
    ...
    WineExecServer
        .scope(|exec_server_url, _wine_prefix| async move {
            ...
            let environments = TurnEnvironmentSelections::new(
                ...
                vec![{
                    let cwd = PathUri::parse("file:///C:/forestx-home")?;
            ...
            assert!(
                patch_end
                    ...
                    .contains_key(&std::path::PathBuf::from(format!(
                        r"C:\forestx-home\apply-patch-smoke\nested\{PATCH_FILE}"
                    ))),

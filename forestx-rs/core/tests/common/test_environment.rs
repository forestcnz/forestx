...
pub const TEST_ENVIRONMENT_ENV_VAR: &str = "FORESTX_TEST_ENVIRONMENT";
pub const LEGACY_REMOTE_ENV_ENV_VAR: &str = "FORESTX_TEST_REMOTE_ENV";
pub const DOCKER_CONTAINER_ENV_VAR: &str = "FORESTX_TEST_REMOTE_ENV_CONTAINER_NAME";
...
impl TestEnvironment {
    ...
    pub(crate) fn remote_cwd(&self, instance_id: &str) -> Result<Option<LegacyAppPathString>> {
        let path_uri = match self {
            ...
            Self::Docker { .. } => {
                PathUri::parse(&format!("file:///tmp/forestx-core-test-cwd-{instance_id}"))?
            }
            Self::WineExec => {
                ...
                PathUri::parse(&format!("file:///C:/forestx-core-test-cwd-{instance_id}"))?
            }
        };

...
use forestx_core::TimeProvider;
use forestx_core::config::Config;
use forestx_core::resolve_installation_id;
use forestx_core::shell::Shell;
use forestx_core::shell::get_shell_by_model_provided_path;
...
use forestx_exec_server::RemoveOptions;
...
use forestx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-forestx";
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "FORESTX_TEST_REMOTE_EXEC_SERVER_URL";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: forestx_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: forestx_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            forestx_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &forestx_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                forestx_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
impl TestForestxBuilder {
    ...
    pub fn with_auth(mut self, auth: ForestxAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestForestx> {
    ...
    ///
    /// With no remote test configuration, or with `FORESTX_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `FORESTX_TEST_ENVIRONMENT=docker` or
    /// `FORESTX_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `FORESTX_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `FORESTX_TEST_REMOTE_ENV` Docker-container
    ...
    pub async fn restart(
        ...
        previous.forestx.shutdown_and_wait().await?;
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let forestx_linux_sandbox_exe = Some(
            crate::find_forestx_linux_sandbox_exe()
                .context("should find binary for forestx-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let forestx_linux_sandbox_exe = None;
        let local_runtime_paths = forestx_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            forestx_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            forestx_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            forestx_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<forestx_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestForestx> {
        let auth = self.auth.clone();
        let state_db = forestx_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.forestx_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(ForestxHomeUserInstructionsProvider::new(
                    config.forestx_home.clone(),
                ))
            });
        let auth_manager = forestx_core::test_support::auth_manager_from_auth(auth.clone());
        let models_manager = self
            .models_manager
            .clone()
            .unwrap_or_else(|| forestx_core::build_models_manager(&config, auth_manager.clone()));
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            forestx_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let code_mode_host_program = self
            ...
            .take()
            .or_else(|| forestx_utils_cargo_bin::cargo_bin("forestx-code-mode-host").ok());
        let thread_manager = if config.features.enabled(Feature::CodeModeHost)
            && let Some(code_mode_host_program) = code_mode_host_program
        {
            forestx_core::test_support::with_code_mode_host_program(
                thread_manager,
                code_mode_host_program,
                &config,
            )
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    forestx_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    forestx_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
        ...
        Ok(TestForestx {
            ...
            cwd,
            config,
            forestx: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = forestx_utils_cargo_bin::cargo_bin("forestx") {
            config.forestx_self_exe = Some(path);
        } else if let Ok(path) = forestx_utils_cargo_bin::cargo_bin("forestx-exec") {
            // `forestx-exec` also supports `--forestx-run-as-apply-patch`, so use it
        ...
        {
            let forestx = bin_dir.join("forestx");
            let forestx_exec = bin_dir.join("forestx-exec");
            if forestx.is_file() {
                config.forestx_self_exe = Some(forestx);
            } else if forestx_exec.is_file() {
                config.forestx_self_exe = Some(forestx_exec);
            }
        }
...
pub struct TestForestx {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub forestx: Arc<ForestxThread>,
...
impl TestForestx {
    ...
    pub fn forestx_home_path(&self) -> &Path {
        self.config.forestx_home.as_path()
    }
    ...
    pub async fn submit_text_turn(&self, prompt: &str) -> Result<()> {
        ...
        wait_for_event(&self.forestx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
        Ok(())
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.forestx, |event| match event {
...
pub struct TestForestxHarness {
    server: MockServer,
    test: TestForestx,
}

impl TestForestxHarness {
    pub async fn with_builder(mut builder: TestForestxBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestForestxBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_forestx() -> TestForestxBuilder {
    TestForestxBuilder {
        ...
        auth: ForestxAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],

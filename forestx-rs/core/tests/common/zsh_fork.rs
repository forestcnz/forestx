...
use crate::test_forestx::TestForestx;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = forestx_utils_cargo_bin::cargo_bin("forestx-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `forestx-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_forestx().with_config(move |config| {

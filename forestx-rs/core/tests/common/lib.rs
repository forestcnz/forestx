...
use anyhow::ensure;
use forestx_arg0::Arg0PathEntryGuard;
use forestx_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub use forestx_core::test_support::TestForestxResponsesRequestKind;
pub use forestx_core::test_support::responses_metadata;
use forestx_utils_absolute_path::AbsolutePathBuf;
pub use forestx_utils_absolute_path::test_support::PathBufExt;
pub use forestx_utils_absolute_path::test_support::PathExt;
...
pub mod test_forestx;
pub mod test_forestx_exec;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    forestx_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    forestx_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(forestx_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = forestx_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("forestx-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\forestx\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.forestx`.
pub async fn load_default_config_for_test(forestx_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        forestx_linux_sandbox_exe: Some(
            find_forestx_linux_sandbox_exe().expect("should find binary for forestx-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_forestx_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    forestx_utils_cargo_bin::cargo_bin("forestx-linux-sandbox")
}

pub async fn wait_for_event<F>(
    forestx: &ForestxThread,
    predicate: F,
) -> forestx_protocol::protocol::EventMsg
where
    F: FnMut(&forestx_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(forestx, predicate, Duration::from_secs(1)).await
}

/// Waits for a configured MCP server to finish startup and requires it to be ready.
pub async fn wait_for_mcp_server(forestx: &ForestxThread, server_name: &str) -> anyhow::Result<()> {
...
pub async fn submit_thread_settings(
    ...
    let submission_id = forestx.submit(Op::ThreadSettings { thread_settings }).await?;
    loop {
        let ev = timeout(Duration::from_secs(10), forestx.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(forestx: &ForestxThread, matcher: F) -> T
where
    F: Fn(&forestx_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(forestx, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&forestx_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), forestx.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    forestx_core::spawn::FORESTX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    forestx_core::spawn::FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    forestx_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    forestx_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    forestx_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! forestx_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_forestx_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("forestx-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }

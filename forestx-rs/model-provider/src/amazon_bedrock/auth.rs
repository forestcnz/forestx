...
use forestx_api::AuthError;
use forestx_api::AuthProvider;
use forestx_api::SharedAuthProvider;
use forestx_aws_auth::AwsAuthContext;
use forestx_aws_auth::AwsAuthError;
use forestx_aws_auth::AwsRequestToSign;
use forestx_http_client::Request;
use forestx_http_client::RequestBody;
use forestx_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_forestx_error)?;
...
fn aws_auth_error_to_forestx_error(error: AwsAuthError) -> ForestxErr {
    ForestxErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer Forestx agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockMantleSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> forestx_api::AuthProviderFuture<'_> {

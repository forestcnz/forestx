...
impl AmazonBedrockModelProvider {
    ...
    async fn auth(&self) -> Option<ForestxAuth> {
        if self.info.has_command_auth() {
            ...
            self.managed_auth().map(ForestxAuth::BedrockApiKey)
        }
    }
    ...
    fn account_state(&self) -> ProviderAccountResult {
        Ok(ProviderAccountState {
            account: Some(ProviderAccount::AmazonBedrock {
                uses_forestx_managed_credentials: self.managed_auth().is_some(),
            }),
...
mod tests {
    ...
    async fn managed_auth_takes_precedence_over_aws_auth() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(ForestxAuth::BedrockApiKey(managed_auth.clone()));
        ...
        assert_eq!(
            provider.auth().await,
            Some(ForestxAuth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
            provider.account_state(),
            Ok(ProviderAccountState {

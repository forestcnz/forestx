...
use forestx_models_manager::cache::ModelsCache;
use forestx_models_manager::manager::OpenAiModelsManager;
use forestx_models_manager::manager::SharedModelsManager;
use forestx_models_manager::manager::StaticModelsManager;
use forestx_protocol::account::ProviderAccount;
...
/// Optional provider-backed features that Forestx may expose at runtime.
///
...
pub const DEFAULT_APPROVAL_REVIEW_PREFERRED_MODEL: &str = "forestx-auto-review";
...
pub trait ModelProvider: fmt::Debug + Send + Sync {
    ...
    /// to think through whether Forestx should have a unified provider-specific auth
    /// manager throughout the codebase; that is a larger refactor than this change.
    fn auth_manager(&self) -> Option<Arc<AuthManager>>;

    /// Returns the current provider-scoped auth value, if one is configured.
    fn auth(&self) -> ModelProviderFuture<'_, Option<ForestxAuth>>;

    /// Returns the current app-visible account state for this provider.
    fn account_state(&self) -> ProviderAccountResult;

    /// Maps an API client error into the provider's user-facing error representation.
    fn map_api_error(&self, error: ApiError) -> ForestxErr {
        forestx_api::map_api_error(error)
    }

    /// Returns provider configuration adapted for the API client.
    fn api_provider(&self) -> ModelProviderFuture<'_, forestx_protocol::error::Result<Provider>> {
        Box::pin(async move {
            let auth = self.auth().await;
            self.info()
                .to_api_provider(auth.as_ref().map(ForestxAuth::auth_mode))
        })
    }
    ...
    fn runtime_base_url(
        &self,
    ) -> ModelProviderFuture<'_, forestx_protocol::error::Result<Option<String>>> {
    ...
    fn api_auth(
        &self,
    ) -> ModelProviderFuture<'_, forestx_protocol::error::Result<SharedAuthProvider>> {
        Box::pin(async move {
            let auth = self.auth().await;
    ...
    /// Returns request credentials, optionally scoped to a Forestx session task.
    fn api_auth_for_scope(
        &self,
        scope: ProviderAuthScope,
    ) -> ModelProviderFuture<'_, forestx_protocol::error::Result<ResolvedProviderAuth>> {
        Box::pin(async move {
    ...
    fn models_manager_without_cache(
        ...
        let model_catalog = config_model_catalog
            .or_else(|| forestx_models_manager::bundled_models_response().ok())
            .unwrap_or_default();
        ...
        let account = if self.info.requires_openai_auth {
            self.auth_manager
                ...
                .and_then(|auth_manager| {
                    ...
                    if matches!(auth, ForestxAuth::Headers(_)) {
                        return None;
                    }
                    Some(auth)
                })
                .map(|auth| match &auth {
                    ForestxAuth::ApiKey(_) => Ok(ProviderAccount::ApiKey),
                    ForestxAuth::BedrockApiKey(_) => {
...
mod tests {
    ...
    use forestx_model_provider_info::ModelProviderAwsAuthInfo;
    ...
    fn test_forestx_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("forestx-model-provider-test-{}", std::process::id()))
    }
    ...
    fn bedrock_api_key_auth() -> ForestxAuth {
        ForestxAuth::BedrockApiKey(BedrockApiKeyAuth {
    ...
    fn create_model_provider_does_not_use_openai_auth_manager_for_amazon_bedrock_provider() {
        let provider = create_model_provider(
            ModelProviderInfo::create_amazon_bedrock_provider(Some(ModelProviderAwsAuthInfo {
                profile: Some("forestx-bedrock".to_string()),
                region: None,
            })),
    ...
    async fn amazon_bedrock_provider_creates_static_models_manager() {
        ...
        let manager =
            provider.models_manager(test_forestx_home(), /*config_model_catalog*/ None);
    ...
    async fn configured_bedrock_catalog_only_allows_default_service_tier() {
        let configured_model = forestx_models_manager::bundled_models_response()

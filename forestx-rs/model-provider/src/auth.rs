...
use forestx_agent_identity::AgentIdentityKey;
use forestx_agent_identity::authorization_header_for_agent_task;
...
use forestx_login::auth::AgentIdentityAuthError;
...
impl AuthProvider for AuthManagerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        let Some(auth) = self
            .auth_manager
            .auth_cached()
            .filter(ForestxAuth::uses_forestx_backend)
        else {
            return;
        };
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> forestx_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(ForestxAuth::BedrockApiKey(_))) {
        return Err(ForestxErr::UnsupportedOperation(
...
) -> forestx_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(ForestxAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(ForestxAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> forestx_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party Forestx auth snapshot.
pub fn auth_provider_from_auth(auth: &ForestxAuth) -> SharedAuthProvider {
    match auth {
        ForestxAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        ForestxAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        ForestxAuth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        ForestxAuth::ApiKey(_)
        | ForestxAuth::Chatgpt(_)
        | ForestxAuth::ChatgptAuthTokens(_)
        | ForestxAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
#[cfg(test)]
mod tests {
    use forestx_agent_identity::generate_agent_key_material;
    use forestx_login::AuthCredentialsStoreMode;
    ...
    use forestx_login::auth::login_with_chatgpt_auth_tokens;
    use forestx_model_provider_info::WireApi;
    use forestx_model_provider_info::create_oss_provider_with_base_url;
    ...
    fn test_forestx_home() -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "forestx-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp forestx home");
        path
    }

    fn write_chatgpt_auth_json(forestx_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, ForestxAuth) {
        let forestx_home = test_forestx_home();
        write_chatgpt_auth_json(&forestx_home);
        ...
        (forestx_home, auth_manager, auth)
    }
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = ForestxAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = ForestxAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&ForestxAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_forestx_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);

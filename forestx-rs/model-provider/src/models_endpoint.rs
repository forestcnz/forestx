...
use forestx_login::AuthEnvTelemetry;
use forestx_login::AuthManager;
use forestx_login::ForestxAuth;
use forestx_login::collect_auth_env_telemetry;
use forestx_login::default_client::create_client_for_route_async;
use forestx_model_provider_info::ModelProviderInfo;
use forestx_models_manager::manager::ModelsEndpointClient;
use forestx_models_manager::manager::ModelsEndpointFuture;
use forestx_otel::TelemetryAuthMode;
use forestx_protocol::error::ForestxErr;
use forestx_protocol::error::Result as CoreResult;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<ForestxAuth> {
    ...
    async fn list_models(
        ...
        let _timer =
            forestx_otel::start_global_timer("forestx.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(ForestxAuth::auth_mode);
        let api_provider = self.provider_info.to_api_provider(auth_mode)?;
        let api_auth = resolve_provider_auth(auth.as_ref(), &self.provider_info)?;
        let request_url =
            ModelsClient::<ReqwestTransport>::request_url(&api_provider, client_version);
        let auth_telemetry = auth_header_telemetry(api_auth.as_ref());
        let agent_identity_telemetry = if let Some(ForestxAuth::AgentIdentity(auth)) = auth.as_ref() {
        ...
        .await
        .map_err(|_| ForestxErr::Timeout)?
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let forestx_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.forestx_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, forestx_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_forestx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_forestx_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "forestx_otel.log_only",
            tracing::Level::INFO,
            event.name = "forestx.api_request",
            ...
            auth.env_forestx_api_key_present = self.auth_env.forestx_api_key_env_present,
            auth.env_forestx_api_key_enabled = self.auth_env.forestx_api_key_env_enabled,
        ...
        tracing::event!(
            target: "forestx_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "forestx.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,

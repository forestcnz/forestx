...
use forestx_backend_client as backend;
use forestx_backend_client::CodeTaskDetailsResponseExt;
use forestx_git_utils::ApplyGitRequest;
use forestx_git_utils::apply_git_patch;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/forestx/tasks/{}", self.base_url, id.0),
            };
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("FORESTX_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
    ...
    fn details_path(base_url: &str, id: &str) -> Option<String> {
        ...
        } else if base_url.contains("/api/forestx") {

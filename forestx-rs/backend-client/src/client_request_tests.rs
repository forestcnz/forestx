...
fn list_tasks_url_omits_empty_query_and_encodes_all_parameters() {
    ...
    assert_eq!(
        client
            ...
            .unwrap(),
        "https://example.test/api/forestx/tasks/list"
    );
    assert_eq!(
        client
            .list_tasks_url(
            ...
            .unwrap(),
        "https://example.test/api/forestx/tasks/list?limit=10&task_filter=mine+%2F+shared&cursor=next%3Dpage&environment_id=env%26one"
    );
}
...
async fn migrated_requests_preserve_query_auth_and_json_body() {
    ...
    .with_auth_provider(Arc::new(forestx_model_provider::BearerAuthProvider::new(
    ...
    assert!(requests[0].starts_with(
        "GET /api/forestx/tasks/list?limit=10&task_filter=mine+%2F+shared&cursor=next%3Dpage&environment_id=env%26one HTTP/1.1\r\n"
    ));
    assert!(
        requests[0]
            .to_ascii_lowercase()
    ...
    assert!(requests[1].starts_with("POST /api/forestx/tasks HTTP/1.1\r\n"));
    assert!(
        requests[1]
            .to_ascii_lowercase()
            .contains("authorization: bearer request-token\r\n")

...
impl Client {
    ...
    fn rate_limit_status_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/usage", self.base_url),
    ...
    fn rate_limit_reset_credits_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => {
                format!("{}/api/forestx/rate-limit-reset-credits", self.base_url)
            }
    ...
    fn consume_rate_limit_reset_credit_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => {
                format!(
                    "{}/api/forestx/rate-limit-reset-credits/consume",
                    self.base_url
                )
            }
            PathStyle::ChatGptApi => {

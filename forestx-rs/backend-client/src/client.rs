...
use crate::types::ForestxUserSettingsResponse;
use crate::types::ForestxWorkspaceMessagesResponse;
...
use forestx_http_client::RouteAwareRequestBuilder;
...
use forestx_protocol::protocol::RateLimitReachedType;
use forestx_protocol::protocol::RateLimitSnapshot;
use forestx_protocol::protocol::RateLimitWindow;
use forestx_protocol::protocol::SpendControlLimitSnapshot;
use http::Method;
use http::StatusCode;
...
impl Client {
    pub fn new(base_url: impl Into<String>, http_client_factory: HttpClientFactory) -> Self {
        ...
        Self {
            base_url,
            http,
            auth_provider: forestx_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(
        ...
        Self::new(base_url, http_client_factory)
            .with_user_agent(get_forestx_user_agent())
            .with_auth_provider(forestx_model_provider::auth_provider_from_auth(auth))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("forestx-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("forestx"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/profiles/me", self.base_url),
    ...
    fn list_tasks_url(
        ...
        let url = match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::ForestxApi => format!(
                "{}/api/forestx/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from forestx-backend.
    ///
    /// `GET /api/forestx/config/bundle` (Forestx API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/config/bundle", self.base_url),
    ...
    /// Fetch authenticated Forestx user settings from the active backend route.
    ///
    /// Uses `GET /api/forestx/settings/user` for Forestx API hosts and
    /// `GET /wham/settings/user` for ChatGPT `backend-api` hosts.
    pub async fn get_user_settings(
        &self,
    ) -> std::result::Result<ForestxUserSettingsResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<ForestxUserSettingsResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }

    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<ForestxWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<ForestxWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/tasks", self.base_url),
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<forestx_backend_openapi_models::models::SpendControlStatusDetails>,
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => format!(
                "{}/api/forestx/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/workspace-messages", self.base_url),
    ...
    fn user_settings_url(&self) -> String {
        match self.path_style {
            PathStyle::ForestxApi => format!("{}/api/forestx/settings/user", self.base_url),
...
mod tests {
    use super::*;
    use forestx_backend_openapi_models::models::AdditionalRateLimitDetails;
    use forestx_backend_openapi_models::models::RateLimitReachedKind;
    use forestx_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "forestx_other".to_string(),
                metered_feature: "forestx_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                forestx_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("forestx"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("forestx_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("forestx_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        let snapshots = [
            ...
            RateLimitSnapshot {
                ...
                limit_name: Some("forestx".to_string()),
                primary: Some(RateLimitWindow {
                    used_percent: 10.0,
                    window_minutes: Some(60),
                    resets_at: Some(2),
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("forestx"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let forestx_client = test_client("https://example.test", PathStyle::ForestxApi);
        assert_eq!(
            forestx_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/forestx/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let forestx_client = test_client("https://example.test", PathStyle::ForestxApi);
        assert_eq!(
            forestx_client.token_usage_profile_url(),
            "https://example.test/api/forestx/profiles/me"
        );
    ...
    fn workspace_messages_uses_expected_paths() {
        let forestx_client = test_client("https://example.test", PathStyle::ForestxApi);
        assert_eq!(
            forestx_client.workspace_messages_url(),
            "https://example.test/api/forestx/workspace-messages"
        );
    ...
    async fn user_settings_request_uses_expected_paths_and_revalidates_cached_responses() {
        ...
        for (request_path, commit_attribution_enabled) in [
            ("/api/forestx/settings/user", true),
        ...
        let forestx_response = Client::new(
            server.uri(),
            HttpClientFactory::new(forestx_http_client::OutboundProxyPolicy::ReqwestDefault),
        )
        .get_user_settings()
        .await
        .unwrap();
        ...
        assert_eq!(
            [forestx_response, chatgpt_response],
            [
                ForestxUserSettingsResponse {
    ...
    fn user_settings_missing_attribution_policy_defaults_to_disabled() {
        assert_eq!(
            serde_json::from_value::<ForestxUserSettingsResponse>(serde_json::json!({})).unwrap(),
            ForestxUserSettingsResponse {
                commit_attribution_enabled: false,
            }
        );

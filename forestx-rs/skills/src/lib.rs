...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".forestx-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills from an absolute FORESTX_HOME.
pub fn system_cache_root_dir(forestx_home: &AbsolutePathBuf) -> AbsolutePathBuf {
    forestx_home
        .join(SKILLS_DIR_NAME)
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `FORESTX_HOME/skills/.system`.
///
...
/// install is skipped.
pub fn install_system_skills(forestx_home: &AbsolutePathBuf) -> Result<(), SystemSkillsError> {
    let skills_root_dir = forestx_home.join(SKILLS_DIR_NAME);
    fs::create_dir_all(skills_root_dir.as_path())
        .map_err(|source| SystemSkillsError::io("create skills root dir", source))?;

    let dest_system = system_cache_root_dir(forestx_home);

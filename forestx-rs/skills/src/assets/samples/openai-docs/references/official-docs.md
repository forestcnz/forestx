...
Use this route for OpenAI product or API documentation, examples, citations, ChatGPT Work, learning content, mixed Chat/Work/Forestx comparisons, and narrow Forestx product documentation. Follow the root skill's official-source order and credential boundary.

An explicit OpenAI documentation question stays documentation-first even when embedded in a broader repository, Promptfoo, agent-evaluation, `PLANS.md`, frontend, tool-use, image, Realtime API, SDK installation, or streaming-debugging task. Search the exact requested official documentation and open or fetch its relevant page before inspecting local files, drafting a plan, running evals, reading bundled references, or invoking the Forestx manual. Then use the fetched official source to complete the requested work.
...
Preserve an explicitly requested model in the search and answer. Search the requested topic directly for model-specific frontend and tool-use prompting, image input or generation, Realtime voice or translation, official Agents SDK installation, Responses streaming errors, and Forestx Goals. A specific Forestx feature, error, setting, or requested citation is a narrow documentation lookup, not the broad manual-first exception. Current or latest model recommendations follow the model-selection route and the same official search-and-fetch order.

## ChatGPT Work and mixed surfaces

Treat a comparison between Chat, Work, and Forestx as ChatGPT Work documentation, not broad Forestx self-knowledge. Search `learn.chatgpt.com` and open or fetch the relevant official page. Useful starting pages are:

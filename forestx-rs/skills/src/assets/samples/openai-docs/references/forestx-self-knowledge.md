# Forestx self-knowledge

Use this manual-first route only for genuinely broad Forestx setup, orientation, customization, troubleshooting, local-state guidance, or system-map synthesis across skills, plugins, MCP, hooks, `AGENTS.md`, automations, and product surfaces. Mixed Chat/Work/Forestx comparisons belong to `official-docs.md` instead.

Narrow Forestx documentation questions require official documentation search first, then an actual page open or fetch using an available documentation or official-domain web capability. This includes a single feature such as Forestx Goals, a specific setting, documented behavior, exact error, or requested page citation. Search and fetch the exact official topic before inspecting local files or bundled references. Do not fetch the manual, read bundled references, inspect local configuration or caches, or turn a targeted documentation lookup into broad product synthesis. Current or latest model questions follow the model-selection route.
...
```bash
node <skill-dir>/scripts/fetch-forestx-manual.mjs
```
...
```bash
node <skill-dir>/scripts/fetch-forestx-manual.mjs --cache-dir <cache-dir>
```
...
The helper verifies the current source and returns a manual path, outline path, freshness status, and heading outline. Use that outline to locate relevant headings and line ranges, then read or search only the returned manual and outline paths. Do not inspect unrelated repositories, caches, source trees, or local state to establish a public Forestx product claim.
...
## Choose the smallest matching Forestx surface
...
- Project `.forestx/config.toml`: settings for a trusted repository, including sandbox, MCP, hooks, model, and reasoning defaults.

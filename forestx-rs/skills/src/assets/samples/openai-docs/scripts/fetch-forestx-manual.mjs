...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/forestx/forestx-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "forestx-manual.md";
const OUTLINE_FILE_NAME = "forestx-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "forestx-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-forestx-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# Forestx Manual Outline\n\n${outline.text}\n`;
...
const fetchForestxManual = async ({
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchForestxManual(args);
...
export { DEFAULT_MANUAL_URL, fetchForestxManual };

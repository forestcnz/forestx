...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate a local Forestx plugin.")
...
def validate_plugin(plugin_root: Path) -> list[str]:
    errors: list[str] = []
    manifest_path = plugin_root / ".forestx-plugin" / "plugin.json"
...
def load_json_object(path: Path, errors: list[str]) -> dict[str, Any] | None:
    if not path.is_file():
        errors.append("missing `.forestx-plugin/plugin.json`")
        return None
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError:
        errors.append("unable to read `.forestx-plugin/plugin.json`")
        return None
    except json.JSONDecodeError:
        errors.append("`.forestx-plugin/plugin.json` must be valid JSON")
        return None
    if not isinstance(payload, dict):
        errors.append("`.forestx-plugin/plugin.json` must contain a JSON object")
        return None
    return payload

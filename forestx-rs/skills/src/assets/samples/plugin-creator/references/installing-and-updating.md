...
- the user wants Forestx to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single Forestx cachebuster suffix:
...
outside Forestx depends on a specific token:
...
```bash
forestx plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `forestx plugin marketplace add` for that
path, and `forestx plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
forestx plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Forestx picks up new skills
   and tools.
...
```text
<base-version>+forestx.<cachebuster>
```

Examples:

- `0.1.0` → `0.1.0+forestx.local-20260519-184516`
- `0.1.0+forestx.old-token` → `0.1.0+forestx.local-20260519-184516`
- `1.2.3-beta.1+forestx.prev` → `1.2.3-beta.1+forestx.local-20260519-184516`
- `dev-build+other-tag` → `dev-build+forestx.local-20260519-184516`

Replace the existing Forestx cachebuster instead of appending another one. Do not keep incrementing
...
- Read the personal marketplace name with
  ...
  `forestx plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `forestx plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by Forestx.
...
- Use `forestx plugin list` when the plugin lives in a different configured marketplace and you need
...
- If a non-default local marketplace has not been configured yet, install it with
  `forestx plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `forestx plugin add <plugin-name>@<marketplace-name>`.

---
name: plugin-creator
description: Create and scaffold plugin directories for Forestx with a required `.forestx-plugin/plugin.json`, optional plugin folders/files, valid manifest defaults, and personal-marketplace entries by default. Use when Forestx needs to create a new personal plugin, add optional plugin structure, generate or update marketplace entries for plugin ordering and availability metadata, or update an existing local plugin during development with the CLI-driven cachebuster and reinstall flow.
---
...
2. Edit `<plugin-path>/.forestx-plugin/plugin.json` when the request gives specific metadata.
   The scaffold starts with valid defaults and must not contain `[TODO: ...]` placeholders.

3. Generate or update the personal marketplace entry when the plugin should appear in Forestx UI ordering:
...
- Always creates `/<parent-plugin-directory>/<plugin-name>/.forestx-plugin/plugin.json`.
...
- If the user specifies a different marketplace path, treat that marketplace as needing explicit installation via `forestx plugin marketplace add`.
...
- Treat plugin order in `plugins[]` as render order in Forestx. Append new entries unless a user explicitly asks to reorder the list.
...
- Do not remove required structure; keep `.forestx-plugin/plugin.json` present.
...
- If Forestx would need approval to write the marketplace file, ask for that approval before
...
- Do not tell the user to run `forestx plugin marketplace add` for the default personal-marketplace
...
- If the user provided a non-default `--marketplace-path`, make sure that marketplace is installed
  before giving reinstall instructions. Use `forestx plugin marketplace add <path-to-marketplace-root>`
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short Forestx app handoff. Say `To view this in the Forestx app:` and write
  `View <normalized plugin name>` and `Share <normalized plugin name>` as Markdown links, not raw
  URLs or code spans.
- The View deeplink uses `forestx://plugins/<normalized plugin name>?marketplacePath=<absolute marketplace.json path>`.
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. Forestx derives both after

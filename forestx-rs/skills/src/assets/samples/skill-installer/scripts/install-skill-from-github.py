#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $FORESTX_HOME/skills."""
...
def _forestx_home() -> str:
    return os.environ.get("FORESTX_HOME", os.path.expanduser("~/.forestx"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "forestx")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "forestx-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_forestx_home(), "skills")

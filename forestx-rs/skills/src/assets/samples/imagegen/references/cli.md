...
Set a stable path to the skill CLI (default `FORESTX_HOME` is `~/.forestx`):

```
export FORESTX_HOME="${FORESTX_HOME:-$HOME/.forestx}"
export IMAGE_GEN="$FORESTX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/forestx-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$FORESTX_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`

...
use forestx_connectors::ConnectorMetadata;
use forestx_connectors::ConnectorMetadataStore;
use forestx_connectors::ConnectorToolSummary;
use forestx_connectors::DirectoryListResponse;
use forestx_connectors::merge::merge_connectors;
use forestx_connectors::merge::merge_plugin_connectors;
use forestx_core::config::Config;
pub use forestx_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use forestx_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use forestx_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use forestx_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use forestx_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use forestx_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "forestx";
...
async fn connector_auth(config: &Config) -> anyhow::Result<ForestxAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_forestx_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        auth.uses_forestx_backend(),
        "ChatGPT connectors require Forestx backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = forestx_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = forestx_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_forestx_backend(),
        "ChatGPT backend requests require Forestx backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run forestx login"
    );
...
mod tests {
    use super::*;
    use forestx_connectors::metadata::connector_install_url;
    use forestx_plugin::AppConnectorId;

...
const FORESTX_PLUGINS_BETA_SETTING: &str = "enable_plugins";
...
struct CachedWorkspaceSettings {
    ...
    expires_at: Instant,
    forestx_plugins_enabled: bool,
}

impl WorkspaceSettingsCache {
    fn get_forestx_plugins_enabled(&self, key: &WorkspaceSettingsCacheKey) -> Option<bool> {
        {
            ...
            if let Some(cached) = entry.as_ref()
                ...
                && cached.key == *key
            {
                return Some(cached.forestx_plugins_enabled);
            }
        }
    ...
    fn set_forestx_plugins_enabled(&self, key: WorkspaceSettingsCacheKey, enabled: bool) {
        let mut entry = match self.entry.write() {
            Ok(entry) => entry,
            Err(err) => err.into_inner(),
        };
        *entry = Some(CachedWorkspaceSettings {
            ...
            forestx_plugins_enabled: enabled,
        });
    }
}

pub async fn forestx_plugins_enabled_for_workspace(
    config: &Config,
    ...
    if let Some(cache) = cache
        && let Some(enabled) = cache.get_forestx_plugins_enabled(&cache_key)
    {
        return Ok(enabled);
    }
    ...
    let forestx_plugins_enabled = settings
        .beta_settings
        .get(FORESTX_PLUGINS_BETA_SETTING)
        .copied()
        .unwrap_or(true);

    if let Some(cache) = cache {
        cache.set_forestx_plugins_enabled(cache_key, forestx_plugins_enabled);
    }

    Ok(forestx_plugins_enabled)
}

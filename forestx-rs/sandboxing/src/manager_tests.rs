...
fn transform_linux_seccomp_request(
    forestx_linux_sandbox_exe: &std::path::Path,
    ...
    manager
        .transform(SandboxTransformRequest {
            ...
            network: None,
            sandbox_policy_cwd: &cwd_uri,
            forestx_linux_sandbox_exe: Some(forestx_linux_sandbox_exe),
            use_legacy_landlock: false,
            windows_sandbox_level: WindowsSandboxLevel::Disabled,
            windows_sandbox_private_desktop: false,
        })
...
fn transform_linux_seccomp_preserves_helper_path_in_arg0_when_available() {
    let forestx_linux_sandbox_exe = std::path::PathBuf::from("/tmp/forestx-linux-sandbox");
    let exec_request = transform_linux_seccomp_request(&forestx_linux_sandbox_exe);

    assert_eq!(
        exec_request.arg0,
        Some(forestx_linux_sandbox_exe.to_string_lossy().into_owned())
    );
}
...
fn transform_linux_seccomp_uses_helper_alias_when_launcher_is_not_helper_path() {
    let forestx_linux_sandbox_exe = std::path::PathBuf::from("/tmp/forestx");
    let exec_request = transform_linux_seccomp_request(&forestx_linux_sandbox_exe);

    assert_eq!(exec_request.arg0, Some("forestx-linux-sandbox".to_string()));
}
...
fn transform_for_direct_spawn_windows_materializes_inner_helper() {
    let forestx_home = tempfile::TempDir::new().expect("forestx home");
    let helper_dir = tempfile::TempDir::new().expect("helper dir");
    let configured_helper = helper_dir.path().join("configured-forestx-helper.exe");
    ...
    let manager = SandboxManager::new();
    let exec_request = manager
        .transform_for_direct_spawn_with_forestx_home(
            SandboxDirectSpawnTransformRequest {
                workspace_roots: workspace_roots.as_slice(),
                windows_sandbox_proxy_settings_mode:
                    forestx_windows_sandbox::WindowsSandboxProxySettingsMode::Preserve,
                transform: SandboxTransformRequest {
                    command: SandboxCommand {
                        program: configured_helper.as_os_str().to_owned(),
                        args: vec!["--forestx-run-as-fs-helper".to_string()],
                        cwd: cwd_uri.clone(),
                        env: HashMap::from([(
                            "Path".to_string(),

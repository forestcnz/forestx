...
use crate::landlock::FORESTX_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub forestx_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode: forestx_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing forestx-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = forestx_linux_sandbox_exe
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let forestx_home = forestx_utils_home_dir::find_forestx_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_forestx_home(request, forestx_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_forestx_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        forestx_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    forestx_home: &Path,
    proxy_settings_mode: forestx_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = forestx_windows_sandbox::resolve_exe_for_launch(source.as_path(), forestx_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        forestx_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(FORESTX_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        FORESTX_LINUX_SANDBOX_ARG0.to_string()
    }
}

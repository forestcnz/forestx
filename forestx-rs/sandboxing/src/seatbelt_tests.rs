...
use forestx_network_proxy::ManagedNetworkSandboxContext;
...
use forestx_network_proxy::build_config_state;
...
use forestx_protocol::permissions::PROTECTED_METADATA_PATH_NAMES;
...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/forestx-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.forestx".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/forestx-readable");
    let unreadable = absolute_path("/tmp/forestx-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
...
fn create_seatbelt_args_allowlists_explicit_unix_socket_paths_without_proxy() {
    ...
    let extra_allow_unix_sockets = vec![absolute_path("/tmp/forestx-browser-use")];
    let args = create_seatbelt_command_args(CreateSeatbeltCommandArgsParams {
    ...
    let expected_socket_root = normalize_path_for_sandbox(Path::new("/tmp/forestx-browser-use"))
...
async fn create_seatbelt_args_merges_proxy_and_explicit_unix_socket_paths() -> anyhow::Result<()> {
    ...
    let network_socket = "/tmp/forestx-proxy-use";
    let explicit_socket = "/tmp/forestx-browser-use";
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_forestx_subpaths() {
    ...
    // write to .forestx/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected explicit writable root .git/.forestx carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_forestx_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .forestx/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_forestx_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .forestx is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_forestx_creation_with_metadata_name_regex() {
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_forestx = repo_root.join(".forestx");
    let config_toml = dot_forestx.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_forestx.to_string_lossy().as_ref(),
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    let expected_dot_forestx = format!(
        "-DWRITABLE_ROOT_0_EXCLUDED_1={}",
        dot_forestx_canonical.to_string_lossy()
    );
    assert!(
        args.contains(&expected_dot_forestx),
        "missing {expected_dot_forestx}: {args:#?}"
    );
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .forestx/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".forestx")).expect("create .forestx");
    fs::write(
        vulnerable_root.join(".forestx").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .forestx/config.toml");
    ...
    let dot_forestx_canonical = vulnerable_root_canonical.join(".forestx");

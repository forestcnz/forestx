...
pub async fn spawn_process(request: SpawnRequest<'_>) -> Result<SpawnedProcess> {
    if request.sandbox == SandboxType::WindowsRestrictedToken {
        #[cfg(target_os = "windows")]
        {
            ...
            let forestx_home = forestx_utils_home_dir::find_forestx_home()
                .context("windows sandbox: failed to resolve forestx_home")?;
            let empty_paths = &[];
            let overrides = windows.filesystem_overrides;

            return forestx_windows_sandbox::spawn_windows_sandbox_session_for_level(
                forestx_windows_sandbox::WindowsSandboxSessionRequest {
                    permission_profile: windows.permission_profile,
                    workspace_roots: windows.workspace_roots,
                    forestx_home: forestx_home.as_path(),
    ...
    if request.tty {
        forestx_utils_pty::pty::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        forestx_utils_pty::pipe::spawn_process(
            program,
            args,
            request.cwd,
            request.env,
        ...
        forestx_utils_pty::pipe::spawn_process_no_stdin(
            program,
            args,
            request.cwd,
            request.env,

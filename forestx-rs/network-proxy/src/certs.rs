...
fn managed_ca_dir() -> Result<PathBuf> {
    let forestx_home =
        find_forestx_home().context("failed to resolve FORESTX_HOME for managed MITM CA")?;
    Ok(forestx_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current Forestx-generated MITM CA bundle.

...
const NETWORK_POLICY_VIOLATION_PREFIX: &str = "FORESTX_NETWORK_POLICY_VIOLATION";
...
impl NetworkProxyState {
    ...
    pub async fn current_cfg(&self) -> Result<NetworkProxyConfig> {
        ...
        // `config.toml` (including Forestx-managed writes) take effect without a restart.
        self.reload_if_needed().await?;
        let guard = self.state.read().await;
...
mod tests {
    ...
    fn blocked_request_violation_log_line_serializes_payload() {
        ...
        assert_eq!(
            ...
            r#"FORESTX_NETWORK_POLICY_VIOLATION {"host":"google.com","reason":"not_allowed","client":"127.0.0.1","method":"GET","mode":"full","protocol":"http","decision":"ask","source":"decider","port":80,"timestamp":1735689600}"#
        );
    }

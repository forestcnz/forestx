...
const AUDIT_TARGET: &str = "forestx_otel.network_proxy";
const POLICY_DECISION_EVENT_NAME: &str = "forestx.network_proxy.policy_decision";
...
mod tests {
    ...
    const LEGACY_DOMAIN_POLICY_DECISION_EVENT_NAME: &str =
        "forestx.network_proxy.domain_policy_decision";
    const LEGACY_BLOCK_DECISION_EVENT_NAME: &str = "forestx.network_proxy.block_decision";
    ...
    async fn evaluate_host_policy_emits_domain_event_for_decider_allow_override() {
        ...
        assert!(event.target.starts_with("forestx_otel."));
        assert_eq!(
    ...
    async fn evaluate_host_policy_emits_metadata_fields() {
        let metadata = NetworkProxyAuditMetadata {
            ...
            originator: Some("forestx_cli_rs".to_string()),
            user_email: Some("test@example.com".to_string()),
            terminal_type: Some("iTerm.app/3.6.5".to_string()),
            model: Some("gpt-5.3-forestx".to_string()),
            slug: Some("gpt-5.3-forestx".to_string()),
        };
        ...
        assert_eq!(event.field("originator"), Some("forestx_cli_rs"));
        ...
        assert_eq!(event.field("model"), Some("gpt-5.3-forestx"));
        assert_eq!(event.field("slug"), Some("gpt-5.3-forestx"));
    }

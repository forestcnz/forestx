...
#[command(name = "forestx-network-proxy", about = "Forestx network sandbox proxy")]
pub struct Args {}
...
impl NetworkProxyBuilder {
    ...
    pub fn managed_by_forestx(mut self, managed_by_forestx: bool) -> Self {
        self.managed_by_forestx = managed_by_forestx;
        self
    }
...
pub const PROXY_ACTIVE_ENV_KEY: &str = "FORESTX_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "FORESTX_NETWORK_ALLOW_LOCAL_BINDING";
...
pub fn is_managed_proxy_env_var(key: &str, value: &str) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        key == PROXY_GIT_SSH_COMMAND_ENV_KEY
            && value.starts_with(FORESTX_PROXY_GIT_SSH_COMMAND_MARKER)
    }
...
#[cfg(target_os = "macos")]
pub const FORESTX_PROXY_GIT_SSH_COMMAND_MARKER: &str = "FORESTX_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const FORESTX_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "FORESTX_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const FORESTX_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
#[cfg(target_os = "macos")]
fn forestx_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{FORESTX_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{FORESTX_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_forestx_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(FORESTX_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(FORESTX_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
fn apply_proxy_env_overrides(
    ...
    if socks_enabled {
        ...
        // but refresh a previously injected Forestx fallback so it cannot point
        // at a stale proxy port after the proxy is restarted.
        match env.get(GIT_SSH_COMMAND_ENV_KEY) {
            Some(command) if !is_forestx_proxy_git_ssh_command(command) => {}
            _ => {
                env.insert(
                    GIT_SSH_COMMAND_ENV_KEY.to_string(),
                    forestx_proxy_git_ssh_command(socks_addr),
                );
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn non_forestx_managed_proxy_builder_uses_configured_ports() {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"FORESTX_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
        #[cfg(not(target_os = "macos"))]
    ...
    fn apply_proxy_env_overrides_sets_mitm_ca_trust_bundle_vars() {
        let mut env = HashMap::new();
        let mitm_ca_trust_bundle_path = Path::new("/tmp/forestx-proxy/ca-bundle.pem");
    ...
    #[cfg(target_os = "macos")]
    #[test]
    fn apply_proxy_env_overrides_refreshes_previous_forestx_proxy_git_ssh_command() {
        let mut env = HashMap::new();
        env.insert(
            GIT_SSH_COMMAND_ENV_KEY.to_string(),
            forestx_proxy_git_ssh_command(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), 8081)),
        );
        ...
        assert_eq!(
            env.get(GIT_SSH_COMMAND_ENV_KEY),
            Some(&forestx_proxy_git_ssh_command(SocketAddr::new(

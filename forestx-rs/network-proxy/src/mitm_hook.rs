...
mod tests {
        ...
        MitmHookConfig {
            ...
            actions: MitmHookActionsConfig {
                ...
                inject_request_headers: vec![InjectedHeaderConfig {
                    ...
                    secret_env_var: Some("FORESTX_GITHUB_TOKEN".to_string()),
                    secret_file: None,
    ...
    fn compile_resolves_env_backed_injected_headers() {
        ...
        let hooks = compile_mitm_hooks_with_resolvers(
            &config,
            |name| (name == "FORESTX_GITHUB_TOKEN").then(|| "ghp-secret".to_string()),
        ...
        assert_eq!(
            ...
            SecretSource::EnvVar("FORESTX_GITHUB_TOKEN".to_string())
        );
        assert_eq!(
    ...
    fn evaluate_returns_first_matching_hook() {
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/forestx/issues")
            .header("x-trace", "1")
            .body(Body::empty())
            .unwrap();
    ...
    fn evaluate_matches_query_and_header_constraints() {
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/forestx/issues?state=open&per_page=10")
    ...
    fn evaluate_matches_wildcard_path_query_and_header_constraints() {
        let mut config = base_config();
        let mut hook = github_hook();
        hook.matcher.path_prefixes = vec!["pattern:/repos/*/forestx/issues*".to_string()];
        hook.matcher.query =
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/forestx/issues?state=open")
    ...
    fn evaluate_path_wildcard_does_not_cross_segment_boundaries() {
        ...
        let nested_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/private/forestx/issues")
            .body(Body::empty())
            .unwrap();
    ...
    fn evaluate_treats_glob_metacharacters_as_literal_without_glob_prefix() {
        ...
        let exact_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/[draft]/forestx/issues?state=op*")
            .header("x-github-api-version", "2022-11-28[preview]")
            .body(Body::empty())
            .unwrap();
        let non_literal_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/draft/forestx/issues?state=open")
            .header("x-github-api-version", "2022-11-28-preview")
            .body(Body::empty())
            .unwrap();
    ...
    fn evaluate_allows_literal_values_with_reserved_prefixes() {
        ...
        let exact_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/forestx/issues?state=pattern%3A%2A")
            .header("x-github-api-version", "pattern:*")
            .body(Body::empty())
            .unwrap();
        let non_literal_req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/forestx/issues?state=pattern%3Aopen")
    ...
    fn evaluate_returns_hooked_host_no_match_when_query_constraint_fails() {
        ...
        let req = Request::builder()
            .method(Method::POST)
            .uri("/repos/openai/forestx/issues?state=closed")
            .body(Body::empty())
            .unwrap();

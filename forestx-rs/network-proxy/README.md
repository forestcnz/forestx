# forestx-network-proxy

`forestx-network-proxy` is Forestx's local network policy enforcement proxy. It runs:
...
`forestx-network-proxy` reads from Forestx's merged `config.toml` (via `forestx-core` config loading).
...
# bundle env vars pointing at immutable public files under $FORESTX_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p forestx-network-proxy --
```
...
`forestx-network-proxy` can be embedded as a library with a thin API:

```rust
use forestx_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `forestx-network-proxy` is embedded in managed Forestx runtime, policy decisions emit structured
OTEL-compatible events with `target=forestx_otel.network_proxy`.

Event name:

- `forestx.network_proxy.policy_decision`
...
This section documents the protections implemented by `forestx-network-proxy`, and the boundaries of

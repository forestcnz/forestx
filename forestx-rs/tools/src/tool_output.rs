...
impl ToolOutput for forestx_protocol::mcp::CallToolResult {
    fn log_preview(&self) -> String {
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    forestx_protocol::models::ContentItem::InputText { text }
                    | forestx_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    forestx_protocol::models::ContentItem::InputImage { image_url, detail } => {
                        FunctionCallOutputContentItem::InputImage {
                            image_url,
                            detail: detail.or(Some(DEFAULT_IMAGE_DETAIL)),
                        }
                    }
                    forestx_protocol::models::ContentItem::InputAudio { audio_url } => {

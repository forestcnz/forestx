            ...
            for tool in &mut namespace.tools {
                match tool {
                    ResponsesApiNamespaceTool::Function(tool) => {
                        ...
                        tool.description =
                            forestx_code_mode::augment_tool_definition(definition).description;
                    }
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    forestx_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| forestx_code_mode::augment_tool_definition(definition))
}
...
pub fn collect_code_mode_tool_definitions<'a>(
    ...
    let mut tool_definitions = specs
        ...
        .filter(|definition| forestx_code_mode::is_code_mode_nested_tool(&definition.name))
        .map(forestx_code_mode::augment_tool_definition)
        .collect::<Vec<_>>();

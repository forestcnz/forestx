...
`forestx-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `forestx-core`.
...
compatibility-sensitive orchestration in `forestx-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `forestx-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `forestx-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `forestx-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `forestx-core`.
3. Leave compatibility-sensitive adapters in `forestx-core` while downstream
...
`forestx-core`, that is a sign to revisit the extraction boundary before adding
more here.

...
use forestx_client::TransportError;
...
const X_FORESTX_TURN_STATE_HEADER: &str = "x-forestx-turn-state";
...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "forestx_api::responses_websocket_timing";
...
const WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str = "x-forestx-ws-stream-request-start-ms";
...
async fn connect_websocket(
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_FORESTX_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='forestx_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-forestx-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-forestx-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "forestx.response.metadata",
            "headers": {
                "x-forestx-safety-buffering-enabled": "true",
                "x-forestx-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "forestx.response.metadata",
            "headers": {
                "x-forestx-safety-buffering-enabled": "false",
                "x-forestx-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");

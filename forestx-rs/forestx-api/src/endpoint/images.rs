...
mod tests {
    ...
    fn provider() -> Provider {
        Provider {
            name: "test".to_string(),
            base_url: "https://example.com/api/forestx".to_string(),
            query_params: None,
            headers: HeaderMap::new(),
            retry: RetryConfig {
                max_attempts: 1,
    ...
    async fn generate_posts_typed_request_and_parses_image_response() {
        ...
        assert_eq!(
            request.url,
            "https://example.com/api/forestx/images/generations"
        );
        assert_eq!(
    ...
    async fn edit_posts_typed_request_and_parses_image_response() {
        ...
        let request = captured_request(&transport);
        assert_eq!(request.url, "https://example.com/api/forestx/images/edits");
        assert_eq!(
            request.body.as_ref().and_then(RequestBody::json),
            Some(&json!({

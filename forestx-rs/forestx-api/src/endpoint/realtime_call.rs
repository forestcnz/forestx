...
use bytes::Bytes;
use forestx_client::HttpTransport;
use forestx_client::Request;
use forestx_client::RequestBody;
use forestx_client::RequestTelemetry;
use http::HeaderMap;
...
const MULTIPART_BOUNDARY: &str = "forestx-realtime-call-boundary";
const MULTIPART_CONTENT_TYPE: &str = "multipart/form-data; boundary=forestx-realtime-call-boundary";
...
impl<T: HttpTransport> RealtimeCallClient<T> {
    ...
    pub async fn create_with_session_and_headers(
        ...
        trace!(target: "forestx_api::realtime_websocket::wire", "realtime call request SDP: {sdp}");
...
mod tests {
    ...
    use forestx_client::Request;
    use forestx_client::Response;
    use forestx_client::StreamResponse;
    ...
    async fn extracts_call_id_from_forwarded_backend_location() {
        ...
        let client = RealtimeCallClient::new(
            transport.clone(),
            provider("https://chatgpt.com/backend-api/forestx"),
            Arc::new(DummyAuth),
        );
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/forestx/realtime/calls"
        );
        assert_eq!(
            request.body,
            Some(RequestBody::Raw(Bytes::from_static(b"v=offer\r\n")))
    ...
    async fn sends_backend_session_call_as_json_body() {
        ...
        assert_eq!(
            request.url,
            "https://chatgpt.com/backend-api/forestx/realtime/calls?intent=quicksilver&architecture=avas"
        );

...
use chrono::DateTime;
use chrono::Utc;
use forestx_protocol::auth::PlanType;
use forestx_protocol::error::ForestxErr;
use forestx_protocol::error::ForestxErrorDetails;
use forestx_protocol::error::RetryLimitReachedError;
use forestx_protocol::error::UnexpectedResponseError;
use forestx_protocol::error::UsageLimitReachedError;
use http::HeaderMap;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> ForestxErr {
    match err {
        ApiError::ContextWindowExceeded => ForestxErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => ForestxErr::QuotaExceeded,
        ApiError::UsageNotIncluded => ForestxErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => {
            let error = ForestxErr::Stream(message);
            match delay {
                Some(delay) => error.with_retry_delay(delay),
                None => error,
            }
        }
        ApiError::Stream(msg) => ForestxErr::Stream(msg),
        ApiError::ServerOverloaded => ForestxErr::ServerOverloaded,
        ...
        ApiError::InvalidRequest { message } => ForestxErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => {
            ForestxErr::new(ForestxErrorDetails::CyberPolicy { message })
        }
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return ForestxErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                    ...
                    {
                        ForestxErr::InvalidImageRequest()
                    } else {
                        ForestxErr::InvalidRequest(body_text)
                    }
                } else if status == http::StatusCode::INTERNAL_SERVER_ERROR {
                    ForestxErr::InternalServerError
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return ForestxErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return ForestxErr::UsageNotIncluded;
                        }
                    }
            ...
            TransportError::RetryLimit => ForestxErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => ForestxErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => ForestxErr::Stream(msg),
        },
        ApiError::RateLimit(msg) => ForestxErr::Stream(msg),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-forestx-active-limit";

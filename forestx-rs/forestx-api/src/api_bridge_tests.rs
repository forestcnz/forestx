...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err.details(), ForestxErrorDetails::ServerOverloaded));
}
...
fn map_api_error_preserves_retry_delay() {
    ...
    assert!(matches!(
        err.details(),
        ForestxErrorDetails::Stream(message) if message == "retry later"
    ));
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let ForestxErrorDetails::UnexpectedStatus(err) = err.details() else {
        panic!("expected ForestxErrorDetails::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let ForestxErrorDetails::CyberPolicy { message } = err.details() else {
        panic!("expected ForestxErrorDetails::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let ForestxErrorDetails::InvalidRequest(message) = err.details() else {
        panic!("expected ForestxErrorDetails::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("forestx_other"),
    );
    headers.insert(
        "x-forestx-other-limit-name",
        http::HeaderValue::from_static("forestx_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let ForestxErrorDetails::UsageLimitReached(usage_limit) = err.details() else {
        panic!("expected ForestxErrorDetails::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_copies_rate_limit_reached_type_to_usage_limit_snapshot() {
    for (active_limit, expected_limit_id) in [(None, "forestx"), (Some("forestx_other"), "forestx_other")]
    {
        let mut headers = HeaderMap::new();
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-forestx-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",

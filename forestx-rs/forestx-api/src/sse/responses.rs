...
struct ResponseCompletedUsage {
    ...
    total_tokens: i64,
    #[serde(default)]
    forestx_rollout_budget_units: Option<serde_json::Number>,
}
...
impl From<ResponseCompletedUsage> for TokenUsage {
    fn from(val: ResponseCompletedUsage) -> Self {
        ...
        TokenUsage {
            ...
            forestx_rollout_budget_units: val.forestx_rollout_budget_units,
        }
    }
}
...
fn header_turn_state_value_from_json(value: &Value) -> Option<String> {
    let headers = value.as_object()?;
    headers.iter().find_map(|(name, value)| {
        if name.eq_ignore_ascii_case(X_FORESTX_TURN_STATE_HEADER) {
            json_value_as_string(value)
        } else {
            None
        }
...
mod tests {
    ...
    fn parses_cache_write_token_usage() {
        let usage: ResponseCompletedUsage = serde_json::from_value(json!({
            ...
            "total_tokens": 110,
            "forestx_rollout_budget_units": 2.5
        }))
        ...
        assert_eq!(
            ...
            TokenUsage {
                ...
                total_tokens: 110,
                forestx_rollout_budget_units: serde_json::Number::from_f64(2.5),
            }
        );
    }
    ...
    const CYBER_RESTRICTED_MODEL_FOR_TESTS: &str = "gpt-5.3-forestx";
}

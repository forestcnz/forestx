...
pub(crate) fn subagent_header(source: &Option<SessionSource>) -> Option<String> {
    ...
    match sub {
        forestx_protocol::protocol::SubAgentSource::Review => Some("review".to_string()),
        forestx_protocol::protocol::SubAgentSource::Compact => Some("compact".to_string()),
        forestx_protocol::protocol::SubAgentSource::MemoryConsolidation => {
            Some("memory_consolidation".to_string())
        }
        forestx_protocol::protocol::SubAgentSource::ThreadSpawn { .. } => {
            Some("collab_spawn".to_string())
        }
        forestx_protocol::protocol::SubAgentSource::Other(label) => Some(label.clone()),
    }
}

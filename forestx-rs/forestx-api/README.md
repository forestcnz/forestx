...
Typed clients for Forestx/OpenAI APIs built on top of the generic transport in `forestx-client`.
...
- Serves as the wire-level layer consumed by `forestx-core`; higher layers handle auth refresh and business logic.
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `forestx_api::CompactionInput`):
    - `model: &str`.
...
- **Memory summarize endpoint**
  - Input: `MemorySummarizeInput` (re-exported as `forestx_api::MemorySummarizeInput`):
    - `model: String`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `forestx-api` and `forestx-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.

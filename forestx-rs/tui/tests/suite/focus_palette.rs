...
fn focus_gained_with_unanswered_palette_queries_preserves_immediate_input() -> Result<()> {
    ...
    write_test_config(forestx_home.path(), &repo_root)?;

    let mut terminal = PtyForestx::start(&repo_root, forestx_home)?;
...
impl PtyForestx {
    fn start(repo_root: &Path, forestx_home: TempDir) -> Result<Self> {
        let mut master_fd = -1;
        let mut slave_fd = -1;
        ...
        let forestx = forestx_utils_cargo_bin::cargo_bin("forestx")
            .or_else(|_| forestx_utils_cargo_bin::cargo_bin("forestx-tui"))?;
        let child = Command::new(forestx)
            .arg("--no-alt-screen")
            .arg("-C")
            .arg(repo_root)
            .arg("-c")
            ...
            .stderr(slave)
            .spawn()
            .context("start Forestx in focus-test pseudo-terminal")?;
    ...
    fn wait_for_startup(&mut self) -> Result<()> {
        ...
        while Instant::now() < deadline {
            ...
            if self.palette_answered && self.screen_contains("OpenAI Forestx") {
                return Ok(());
            }

            if let Some(status) = self.child.try_wait()? {
                bail!(
                    "Forestx exited before the focus test started ({status}); screen:\n{}",
        ...
        bail!(
            "Forestx did not initialize within {:?}; screen:\n{}",
            STARTUP_TIMEOUT,
            self.screen_contents(),
        );
    }
...
fn write_test_config(forestx_home: &Path, repo_root: &Path) -> Result<()> {
    ...
    std::fs::write(forestx_home.join("config.toml"), config)
        .context("write focus-test Forestx configuration")?;
    std::fs::write(

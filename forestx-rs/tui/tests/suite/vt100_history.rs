...
struct TestScenario {
    term: forestx_tui::Terminal<VT100Backend>,
}
...
impl TestScenario {
    fn new(width: u16, height: u16, viewport: Rect) -> Self {
        ...
        let mut term =
            forestx_tui::Terminal::with_options(backend).expect("failed to construct terminal");
        term.set_viewport_area(viewport);
        Self { term }
    }

    fn run_insert(&mut self, lines: Vec<Line<'static>>) {
        forestx_tui::insert_history_lines(&mut self.term, lines)

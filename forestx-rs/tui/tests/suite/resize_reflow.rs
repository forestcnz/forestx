...
#[ignore = "requires tmux and a locally built forestx binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let forestx = forestx_binary(&repo_root)?;
    ...
    write_config(forestx_home.path(), &repo_root)?;
    write_auth(forestx_home.path())?;

    let session_name = format!("forestx-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("FORESTX_HOME={}", forestx_home.path().display()))
    ...
    let forestx_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!forestx_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&forestx_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&forestx_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("forestx-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&forestx_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("forestx-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn forestx_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = forestx_utils_cargo_bin::cargo_bin("forestx") {
        return Ok(path);
    }

    let fallback = repo_root.join("forestx-rs/target/debug/forestx");
    anyhow::ensure!(
        fallback.is_file(),
        "forestx binary is unavailable; run `cargo build -p forestx-cli` first"
    );
    Ok(fallback)
}

fn write_config(forestx_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(forestx_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(forestx_home: &Path) -> Result<()> {
    std::fs::write(

...
pub(crate) const DIRECTIVE_PREFIX: &str = "::forestx-inline-vis{";
...
impl InlineVisualizationContext {
    pub(crate) fn new(forestx_home: &Path, thread_id: ThreadId) -> Option<Self> {
        Self::new_with_writable_roots(forestx_home, thread_id, std::iter::empty())
    }
    ...
    fn new_with_writable_roots<'a>(
        ...
        let visualizations_dir = forestx_home.join("visualizations");
...
fn link_placeholder() -> String {
    ...
    format!("https://forestx.invalid/inline-visualization/{token}")
}

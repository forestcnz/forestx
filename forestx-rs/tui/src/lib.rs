...
// The standalone `forestx-tui` binary prints a short help message before the
...
use forestx_app_server_client::AppServerClient;
use forestx_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use forestx_app_server_client::InProcessAppServerClient;
use forestx_app_server_client::InProcessClientStartArgs;
use forestx_app_server_client::RemoteAppServerClient;
use forestx_app_server_client::RemoteAppServerConnectArgs;
pub use forestx_app_server_client::RemoteAppServerEndpoint;
use forestx_app_server_protocol::Account as AppServerAccount;
use forestx_app_server_protocol::AskForApproval;
use forestx_app_server_protocol::ConfigWarningNotification;
use forestx_app_server_protocol::Thread as AppServerThread;
use forestx_app_server_protocol::ThreadListCwdFilter;
use forestx_app_server_protocol::ThreadListParams;
use forestx_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use forestx_config::format_config_error_with_source;
...
use forestx_login::AuthConfig;
use forestx_login::AuthRouteConfig;
use forestx_login::default_client::originator;
use forestx_login::default_client::set_default_client_residency_requirement;
use forestx_login::enforce_login_restrictions;
...
use forestx_rollout::StateDbHandle;
use forestx_rollout::state_db;
use forestx_state::log_db;
use forestx_utils_absolute_path::AbsolutePathBuf;
use forestx_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use forestx_utils_home_dir::find_forestx_home;
use forestx_utils_oss::ensure_oss_provider_ready;
use forestx_utils_oss::get_default_model_for_oss_provider;
...
pub(crate) use forestx_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "forestx-tui.log";
...
async fn start_embedded_app_server(
    ...
    feedback: forestx_feedback::ForestxFeedback,
...
async fn init_state_db_for_app_server_target(
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            let database_path = forestx_state::runtime_db_path_for_corruption_error(&err)
...
fn remove_legacy_tui_log_file(forestx_home: &Path) {
    ...
    let _ = std::fs::remove_file(forestx_home.join("log").join(TUI_LOG_FILE_NAME));
}
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            let forestx_home = find_forestx_home().wrap_err("failed to resolve FORESTX_HOME")?;
            forestx_app_server_client::app_server_control_socket_path(&forestx_home)
...
#[cfg(unix)]
async fn maybe_probe_default_daemon_socket(forestx_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = forestx_app_server_client::app_server_control_socket_path(forestx_home).ok()?;
...
#[cfg(not(unix))]
async fn maybe_probe_default_daemon_socket(_forestx_home: &Path) -> Option<AbsolutePathBuf> {
    None
}
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        forestx_feedback::ForestxFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
async fn resolve_startup_resume_or_fork_cwd(
    tui: &mut Tui,
    config: &Config,
    state_db: Option<&forestx_state::StateRuntime>,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = forestx_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let forestx_home = match find_forestx_home() {
        Ok(forestx_home) => forestx_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding forestx home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Some(profile_v2) = cli.config_profile_v2.as_ref() {
        let user_config_path = resolve_profile_v2_config_path(&forestx_home, profile_v2);
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        maybe_probe_default_daemon_socket(&forestx_home).await
    } else {
        None
    };
    ...
    let local_runtime_paths = ExecServerRuntimePaths::from_optional_paths(
        arg0_paths.forestx_self_exe.clone(),
    ...
    let prepared_environment_manager =
        if should_load_configured_environments(&loader_overrides, &app_server_target) {
            EnvironmentManager::prepare_from_forestx_home(&forestx_home).await
        } else {
    ...
    let overrides = ConfigOverrides {
        ...
        forestx_self_exe: arg0_paths.forestx_self_exe.clone(),
        forestx_linux_sandbox_exe: arg0_paths.forestx_linux_sandbox_exe.clone(),
    ...
    remove_legacy_tui_log_file(config.forestx_home.as_path());

    let otel_originator = originator().value;
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        forestx_app_server_client::build_otel_provider(
            &config,
    ...
    if let Some(metrics) = otel.as_ref().and_then(forestx_otel::OtelProvider::metrics) {
        let _ = forestx_otel::record_process_start_once(metrics, otel_originator.as_str());
        let telemetry =
            forestx_rollout::sqlite_telemetry_recorder(metrics.clone(), otel_originator.as_str());
        let _ = forestx_state::install_process_db_telemetry(telemetry);
    }
    ...
    let (tui_file_layer, _tui_file_log_guard) = if config_toml_log_dir_configured {
        ...
        let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("forestx_core=info,forestx_tui=info,forestx_rmcp_client=info")
        });
    ...
    let feedback = forestx_feedback::ForestxFeedback::new();
...
async fn run_ratatui_app(
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !uses_remote_workspace {
            cloud_config_bundle = cloud_config_bundle_loader_for_storage(
                initial_config.forestx_home.to_path_buf(),
                /*enable_forestx_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `forestx {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    if let Some(w) = crate::render::highlight::set_theme_override(
        ...
        find_forestx_home().ok().map(AbsolutePathBuf::into_path_buf),
    ) {
    ...
    let required_elevated_sandbox_needs_setup = windows_sandbox_level
        ...
        && config
            ...
            .source
            .is_some()
        && !crate::windows_sandbox::sandbox_setup_is_complete(config.forestx_home.as_path());
    #[cfg(target_os = "windows")]
...
async fn load_bootstrap_config_or_exit(
    ...
    cli_kv_overrides: Vec<(String, forestx_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
    ...
    match load_config_toml_with_layer_stack(
        ...
        cwd,
        cli_kv_overrides,
        forestx_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .forestx_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
    ...
    fn write_session_rollout(
        ...
        let session_meta = forestx_protocol::protocol::SessionMeta {
            session_id: thread_id.into(),
            id: thread_id,
            timestamp: meta_rfc3339.to_string(),
            cwd: cwd.to_path_buf(),
            originator: "forestx".to_string(),
            cli_version: "0.0.0".to_string(),
            source: forestx_protocol::protocol::SessionSource::Cli,
            model_provider: Some(model_provider.to_string()),
            ..Default::default()
        };
        let session_meta = serde_json::to_value(forestx_protocol::protocol::SessionMetaLine {
            meta: session_meta,
            git: None,
        })?;
        let lines = [
    ...
    async fn startup_services_use_final_cloud_managed_http_policy() -> color_eyre::Result<()> {
            ...
            [(true, false), (false, true)]
        {
            ...
            let prepared_environment_manager =
                EnvironmentManager::prepare_from_forestx_home(forestx_home.path()).await?;
            ...
            let cloud_config_bundle =
                forestx_config::test_support::CloudConfigBundleFixture::loader_with_enterprise_requirement(
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: forestx_app_server_client::app_server_control_socket_path(&forestx_home)?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_relative_socket_path() -> color_eyre::Result<()> {
        assert_eq!(
            resolve_remote_addr("unix://forestx.sock")?,
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: AbsolutePathBuf::relative_to_current_dir("forestx.sock")?,
            }
        );
        Ok(())
    }
    ...
    fn resolve_remote_addr_accepts_absolute_socket_path() -> color_eyre::Result<()> {
        let temp_dir = TempDir::new()?;
        let socket_path = temp_dir.path().join("forestx.sock");
        assert_eq!(
    ...
    async fn default_daemon_auto_connect_skips_missing_socket() -> color_eyre::Result<()> {
        let forestx_home = TempDir::new()?;
        assert!(
            maybe_probe_default_daemon_socket(forestx_home.path())
                .await
                .is_none()
        );
        Ok(())
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let forestx_home = TempDir::new()?;
        let socket_path =
            forestx_app_server_client::app_server_control_socket_path(forestx_home.path())?;
        ...
        assert_eq!(
            maybe_probe_default_daemon_socket(forestx_home.path()).await,
            Some(socket_path)
        );
        Ok(())
    }
    ...
    {
        let socket_path = AbsolutePathBuf::relative_to_current_dir("forestx.sock")?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        ...
        let mut app_server = AppServerSession::new(
            forestx_app_server_client::AppServerClient::InProcess(
                ...
                assert!(
                    started
                        ...
                        .any(|item| {
                            matches!(
                                item,
                                forestx_app_server_protocol::ThreadItem::UserMessage { .. }
                            )
                        })
                );
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = forestx_state::StateRuntime::init(
                forestx_state::SqliteConfig::new_for_testing(config.forestx_home.as_path().abs()),
            ...
            let mut builder = forestx_state::ThreadMetadataBuilder::new(
                thread_id,
                rollout_path.clone(),
                created_at,
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        let sqlite =
            forestx_state::SqliteConfig::new_for_testing(occupied_sqlite_home.as_path().abs());
    ...
    -> color_eyre::Result<()> {
        ...
        let sqlite = forestx_state::SqliteConfig::new_for_testing(sqlite_home.as_path().abs());
        ...
        assert!(
            forestx_state::sqlite_error_detail_is_corruption(startup_error.detail()),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let forestx_home = temp_dir.path().to_path_buf();

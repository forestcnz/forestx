...
use forestx_config::NetworkUnixSocketPermissionToml;
use forestx_config::RequirementSource;
use forestx_config::ResidencyRequirement;
use forestx_config::SandboxModeRequirement;
use forestx_config::WebSearchModeRequirement;
use forestx_config::format_config_layer_source;
...
mod tests {
    ...
    use forestx_config::ConstraintError;
    use forestx_config::FeatureRequirementsToml;
    use forestx_config::FilesystemConstraints;
    use forestx_config::HookEventsToml;
    use forestx_config::HookHandlerConfig;
    use forestx_config::LoaderOverrides;
    use forestx_config::ManagedHooksRequirementsToml;
    use forestx_config::MatcherGroup;
    ...
    use forestx_config::NetworkUnixSocketPermissionToml;
    use forestx_config::NetworkUnixSocketPermissionsToml;
    use forestx_config::RequirementSource;
    use forestx_config::ResidencyRequirement;
    use forestx_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\forestx\\config.toml")
        } else {
            absolute_path("/etc/forestx/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.forestx")
        } else {
            absolute_path("/repo/.forestx")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_forestx_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Forestx\\requirements.toml")
        } else {
            absolute_path("/etc/forestx/requirements.toml")
        };
        ...
        let sqlite_home = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.forestx\\state")
        } else {
            absolute_path("/home/alice/.forestx/state")
        };
        let log_dir = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.forestx\\logs")
        } else {
            absolute_path("/home/alice/.forestx/logs")
        };
        let model_catalog_json = if cfg!(windows) {
            absolute_path("C:\\Users\\alice\\.forestx\\models.json")
        } else {
            absolute_path("/home/alice/.forestx/models.json")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.forestx\\config.toml")
        } else {
            absolute_path("/home/alice/.forestx/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/forestx.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\forestx")
        } else {
            absolute_path("/var/lib/forestx")
        };

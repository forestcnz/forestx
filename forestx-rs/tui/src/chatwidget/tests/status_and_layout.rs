...
use forestx_app_server_protocol::SpendControlLimitSnapshot;
...
async fn app_server_cyber_policy_error_renders_dedicated_notice() {
    ...
    handle_error(
        ...
        Some(ForestxErrorInfo::CyberPolicy),
    );
...
async fn status_line_git_summary_items_render_values() {
    ...
    chat.status_line_git_summary = Some(StatusLineGitSummary {
        pull_request: Some(crate::branch_summary::StatusLinePullRequest {
            number: 20_252,
            url: "https://github.com/openai/forestx/pull/20252".to_string(),
        }),
...
async fn completed_plan_table_tail_skips_provisional_history_insert() {
    ...
    controller.push("| --- | --- |\n");
    controller.push("| Verify | Forestx |\n");
    assert!(
        controller.has_live_tail(),
...
async fn configured_pet_load_is_deferred_until_after_construction() {
    ...
    crate::pets::write_test_pack(&cfg.forestx_home);
...
async fn rate_limit_snapshots_keep_separate_entries_per_limit_id() {
    ...
    let forestx = chat
        .rate_limit_snapshots_by_limit_id
        .get("forestx")
        .expect("forestx snapshot should exist");
    let other = chat
        .rate_limit_snapshots_by_limit_id
        .get("forestx_other")
        .expect("forestx_other snapshot should exist");

    assert_eq!(forestx.primary.as_ref().map(|w| w.used_percent), Some(20.0));
    assert_eq!(
        forestx
            .credits
            .as_ref()
...
#[tokio::test]
async fn rate_limit_switch_prompt_skips_non_forestx_limit() {
...
async fn rate_limit_usage_warnings_keep_workspace_limit_after_rolling_credits() {
    for rate_limit_reached_type in [
        ...
        for credits in [
            ...
            assert_eq!(
                chat.forestx_rate_limit_reached_type,
...
async fn rate_limit_usage_warnings_keep_newly_reached_workspace_limit() {
    for (limit_id, should_warn) in [("forestx", true), ("forestx_other", false)] {
        let (mut chat, mut rx, _) = make_chatwidget_manual(Some("gpt-5")).await;
        chat.has_chatgpt_account = true;
...
async fn rate_limit_usage_warnings_preserve_and_clear_spend_control_state() {
    ...
    blocked_snapshot.rate_limit_reached_type =
        Some(RateLimitReachedType::WorkspaceMemberUsageLimitReached);
    chat.on_rate_limit_snapshot(Some(blocked_snapshot));
    assert_eq!(chat.forestx_spend_control_reached, Some(true));
    ...
    assert_eq!(chat.forestx_spend_control_reached, Some(false));
    assert_eq!(
        chat.forestx_rate_limit_reached_type,
        Some(RateLimitReachedType::WorkspaceMemberUsageLimitReached)
    );
...
async fn rate_limit_usage_warnings_clear_workspace_limit_from_authoritative_snapshot() {
    ...
    assert_eq!(chat.forestx_rate_limit_reached_type, None);
}
...
async fn account_update_clears_derived_usage_limit_state_and_prompt() {
    ...
    assert!(chat.forestx_rate_limit_reached_type.is_some());
    assert_eq!(chat.forestx_spend_control_reached, Some(true));
    assert!(matches!(
        chat.rate_limit_switch_prompt,
        RateLimitSwitchPromptState::Shown
    ...
    chat.update_account_state(
        ...
        /*has_chatgpt_account*/ true, /*has_forestx_backend_auth*/ true,
    );

    assert_eq!(chat.rate_limit_warnings.primary_index, 0);
    assert_eq!(chat.rate_limit_warnings.secondary_index, 0);
    assert_eq!(chat.forestx_rate_limit_reached_type, None);
    assert_eq!(chat.forestx_spend_control_reached, None);
    assert!(matches!(
        chat.rate_limit_switch_prompt,
        RateLimitSwitchPromptState::Idle
    ));
...
async fn workspace_owner_limit_states_render_state_specific_messages() {
    let cases = [
        (
            RateLimitReachedType::WorkspaceOwnerCreditsDepleted,
            RateLimitErrorKind::Generic,
            "You're out of credits. Your workspace is out of credits. Add credits to continue using Forestx.",
        ),
        (
            RateLimitReachedType::WorkspaceOwnerUsageLimitReached,
            RateLimitErrorKind::UsageLimit,
            "Usage limit reached. You've reached your usage limit. Increase your limits to continue using forestx.",
        ),
    ];
...
async fn esc_interrupt_pauses_active_goal_turn() {
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::ForestxOp(Op::Interrupt)));
    assert_goal_paused_event(&mut rx, thread_id);
...
fn update_thread_goal(chat: &mut ChatWidget, thread_id: ThreadId, status: AppThreadGoalStatus) {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            forestx_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id,
                turn_id: None,
                goal,
            },
...
async fn ambient_pet_stays_hidden_until_a_pet_is_selected() {
    ...
    crate::pets::write_test_pack(&chat.config.forestx_home);
    chat.set_tui_pet(Some("forestx".to_string()));
...
async fn account_update_clears_workspace_headline_state() {
    ...
    chat.update_account_state(
        /*status_account_display*/ None, /*plan_type*/ None,
        /*has_chatgpt_account*/ false, /*has_forestx_backend_auth*/ false,
    );
...
async fn workspace_headline_fetch_allows_backend_auth_without_chatgpt_account() {
    ...
    chat.update_account_state(
        /*status_account_display*/ None, /*plan_type*/ None,
        /*has_chatgpt_account*/ false, /*has_forestx_backend_auth*/ true,
    );
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            forestx_app_server_protocol::HookEventName::PreToolUse,
...
async fn completed_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            forestx_app_server_protocol::HookEventName::PostToolUse,
            /*status_message*/ None,
        ),
    );
    reveal_running_hooks(&mut chat);
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            forestx_app_server_protocol::ThreadNameUpdatedNotification {
                thread_id: thread_id.to_string(),
        ...
        ServerNotification::ThreadGoalUpdated(
            forestx_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: test_thread_goal(
                    forestx_app_server_protocol::ThreadGoalStatus::Active,
                    /*token_budget*/ Some(50_000),
                    /*tokens_used*/ 40_000,
                ),
            },
...
async fn status_line_goal_complete_elapsed_footer_snapshot() {
    ...
    chat.refresh_status_line();
    let mut goal = test_thread_goal(
        forestx_app_server_protocol::ThreadGoalStatus::Complete,
        /*token_budget*/ None,
        /*tokens_used*/ 40_000,
    );
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        forestx_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            forestx_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            forestx_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
fn test_thread_goal(
    status: forestx_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> forestx_app_server_protocol::ThreadGoal {
    forestx_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
    ...
    chat.handle_server_notification(
        ServerNotification::HookStarted(AppServerHookStartedNotification {
            ...
            run: AppServerHookRunSummary {
                ...
                source: forestx_app_server_protocol::HookSource::User,
                display_order: 0,
...
async fn completed_hook_with_no_entries_stays_out_of_history() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "post-tool-use:0:/tmp/hooks.json",
            forestx_app_server_protocol::HookEventName::PostToolUse,
            forestx_app_server_protocol::HookRunStatus::Completed,
            Vec::new(),
        ),
    );
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            "pre-tool-use:0:/tmp/hooks.json",
            forestx_app_server_protocol::HookEventName::PreToolUse,
            forestx_app_server_protocol::HookRunStatus::Blocked,
            vec![forestx_app_server_protocol::HookOutputEntry {
                kind: forestx_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            forestx_app_server_protocol::HookEventName::PostToolUse,
            forestx_app_server_protocol::HookRunStatus::Failed,
            vec![forestx_app_server_protocol::HookOutputEntry {
                kind: forestx_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            forestx_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            hook_id,
            forestx_app_server_protocol::HookEventName::Stop,
            forestx_app_server_protocol::HookRunStatus::Stopped,
            vec![forestx_app_server_protocol::HookOutputEntry {
                kind: forestx_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            forestx_app_server_protocol::HookEventName::SessionStart,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            forestx_app_server_protocol::HookRunStatus::Completed,
            vec![forestx_app_server_protocol::HookOutputEntry {
                kind: forestx_app_server_protocol::HookOutputEntryKind::Context,
...
async fn long_hook_context_is_truncated_with_transcript_hint_snapshot() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            forestx_app_server_protocol::HookRunStatus::Stopped,
            vec![
                forestx_app_server_protocol::HookOutputEntry {
                    kind: forestx_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_started_run(
    id: &str,
    event_name: forestx_app_server_protocol::HookEventName,
    status_message: Option<&str>,
) -> forestx_app_server_protocol::HookRunSummary {
    hook_run_summary(
        id,
        event_name,
        forestx_app_server_protocol::HookRunStatus::Running,
        status_message,
        Vec::new(),
    )
}
...
fn hook_completed_run(
    id: &str,
    event_name: forestx_app_server_protocol::HookEventName,
    status: forestx_app_server_protocol::HookRunStatus,
    entries: Vec<forestx_app_server_protocol::HookOutputEntry>,
) -> forestx_app_server_protocol::HookRunSummary {
    hook_run_summary(
...
fn hook_run_summary(
    ...
    forestx_app_server_protocol::HookRunSummary {
        id: id.to_string(),
        event_name,
        handler_type: forestx_app_server_protocol::HookHandlerType::Command,
        execution_mode: forestx_app_server_protocol::HookExecutionMode::Sync,
        scope: forestx_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: forestx_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        started_at: 1,
        completed_at: (status != forestx_app_server_protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != forestx_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
...
async fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    handle_exec_begin(
        ...
        AppServerThreadItem::CommandExecution {
            id: "c1".into(),
            command: forestx_shell_command::parse_command::shlex_join(&command),
            cwd: cwd.clone().into(),
            process_id: None,
            plugin_id: None,
            script_path: None,

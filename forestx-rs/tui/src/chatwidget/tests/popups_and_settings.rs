...
use forestx_app_server_protocol::HookErrorInfo;
use forestx_app_server_protocol::HooksListEntry;
use forestx_app_server_protocol::HooksListResponse;
use forestx_app_server_protocol::MarketplaceLoadErrorInfo;
...
async fn plugin_detail_popup_snapshot_labels_personal_marketplace_as_local() {
    ...
    let mut plugin = plugins_test_detail(
        ...
        &[
            (forestx_app_server_protocol::HookEventName::PreToolUse, 1),
            (forestx_app_server_protocol::HookEventName::Stop, 2),
        ],
        &["Figma", "Slack"],
        &["figma-mcp", "docs-mcp"],
    );
...
async fn plugin_detail_popup_shows_local_share_context_as_read_only_snapshot() {
    ...
    let summary = PluginSummary {
        share_context: Some(PluginShareContext {
            ...
            share_url: Some("https://chatgpt.com/forestx/plugins/share/docs".to_string()),
...
async fn experimental_features_popup_snapshot() {
    ...
    let mut config = forestx_config::types::TuiKeymap::default();
    config.list.accept = Some(forestx_config::types::KeybindingsSpec::One(
        forestx_config::types::KeybindingSpec("ctrl-x enter".to_string()),
    ));
...
async fn server_overloaded_error_does_not_switch_models() {
    ...
    handle_error(
        &mut chat,
        "server overloaded",
        Some(ForestxErrorInfo::ServerOverloaded),
    );
...
async fn reasoning_up_shortcut_does_not_silently_enter_advanced_effort() {
    for (model, model_path) in [
        ...
        ("forestx-auto-test", "forestx-auto-test"),
    ] {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
        chat.thread_id = Some(ThreadId::new());
        let mut preset = get_available_model(&chat, "gpt-5.4");
...
async fn auto_model_advertising_advanced_effort_opens_reasoning_picker() {
    ...
    preset.id = "forestx-auto-test".to_string();
    preset.model = "forestx-auto-test".to_string();
    preset.display_name = "forestx-auto-test".to_string();
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &forestx_feedback::FeedbackDiagnostics::new(vec![forestx_feedback::FeedbackDiagnostic {

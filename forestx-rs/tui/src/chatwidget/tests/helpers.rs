...
pub(super) async fn test_config() -> Config {
    ...
    let forestx_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config =
        Config::load_default_with_cli_overrides_for_forestx_home(forestx_home.clone(), Vec::new())
            .await
            .expect("config");
    config.forestx_home = forestx_home.abs();
    config.sqlite = forestx_state::SqliteConfig::new_for_testing(forestx_home.as_path().abs());
    config.log_dir = forestx_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::ForestxOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask Forestx to do anything".to_string();
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_forestx_backend_auth = true;
...
fn token_usage_breakdown(usage: TokenUsage) -> forestx_app_server_protocol::TokenUsageBreakdown {
    forestx_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    forestx_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: forestx_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    forestx_error_info: Option<ForestxErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = forestx_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            forestx_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("forestx-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(forestx_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    forestx_app_server_protocol::PluginHookSummary {
        ...
        hook_run(
            ...
            vec![
                forestx_app_server_protocol::HookOutputEntry {
                    kind: forestx_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: forestx_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        forestx_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        forestx_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        forestx_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        forestx_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        forestx_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        forestx_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        forestx_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        forestx_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        forestx_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        forestx_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        forestx_app_server_protocol::HookEventName::Stop => "Stop",
    }
}

...
async fn slash_compact_eagerly_queues_follow_up_before_turn_start() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::ForestxOp(Op::Compact)) => {}
...
async fn queued_slash_compact_dispatches_after_active_turn() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, AppEvent::ForestxOp(Op::Compact))),
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::ForestxOp(Op::SetThreadName { name }) if name == "Queued rename"
        )),
...
async fn slash_init_does_not_depend_on_loaded_instruction_sources() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.instruction_source_paths = vec![forestx_utils_path_uri::PathUri::from_abs_path(
...
async fn goal_slash_command_emits_attached_images() {
    ...
    let local_image = chat.config.forestx_home.join("goal-local.png");
...
async fn queued_goal_slash_command_preserves_large_paste() {
    ...
    chat.thread_id = Some(thread_id);
    handle_turn_started(&mut chat, "turn-1");
    let paste = "x".repeat(forestx_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn interrupt_disambiguates_same_sized_goal_pastes() {
    ...
    chat.set_feature_enabled(Feature::Goals, /*enabled*/ true);
    handle_turn_started(&mut chat, "turn-1");
    let first = "a".repeat(forestx_protocol::user_input::MAX_USER_INPUT_TEXT_CHARS + 1);
...
async fn slash_rename_prefills_existing_thread_name() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::ForestxOp(Op::SetThreadName { name })) if name == "Current project title"
    );
}
...
async fn usage_command_runs_with_backend_auth_from_widget_init() {
    ...
    assert_matches!(rx.try_recv(), Ok(AppEvent::RefreshTokenActivity { .. }));
    assert!(!chat.has_chatgpt_account());
    assert!(chat.has_forestx_backend_auth());
}
...
async fn pending_token_activity_refresh_keeps_composer_visible_in_short_viewport() {
    ...
    assert!(
        term.backend()
            ...
            .screen()
            .contents()
            .contains("Ask Forestx to do anything")
    );
}
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(forestx_config::types::KeybindingsSpec::One(
        forestx_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            forestx_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                turn_id: None,
                goal: forestx_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: forestx_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let rollout_path = PathBuf::from("/tmp/forestx-test-rollout.jsonl");
...
async fn fast_slash_command_updates_and_persists_local_service_tier() {
    ...
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::ForestxOp(Op::OverrideTurnContext {
...
async fn compact_queues_user_messages_snapshot() {
    ...
    handle_error(
        ...
        Some(ForestxErrorInfo::ActiveTurnNotSteerable {

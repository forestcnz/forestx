...
async fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::ForestxOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::ForestxOp(Op::Review { target }) => {
            assert_eq!(
                target,
                ReviewTarget::Custom {
...
async fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::ForestxOp should be sent
...
async fn interrupted_turn_after_goal_budget_limited_uses_budget_message_snapshot() {
    ...
    chat.handle_server_notification(
        forestx_app_server_protocol::ServerNotification::TurnStarted(
            forestx_app_server_protocol::TurnStartedNotification {
                thread_id: "thread-1".to_string(),
                turn: forestx_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    items_view: forestx_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: forestx_app_server_protocol::TurnStatus::InProgress,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,
    ...
    chat.handle_server_notification(
        forestx_app_server_protocol::ServerNotification::ThreadGoalUpdated(
            forestx_app_server_protocol::ThreadGoalUpdatedNotification {
                ...
                goal: forestx_app_server_protocol::ThreadGoal {
                    ...
                    status: forestx_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        ...
        forestx_app_server_protocol::ServerNotification::TurnCompleted(
            forestx_app_server_protocol::TurnCompletedNotification {
                ...
                turn: forestx_app_server_protocol::Turn {
                    ...
                    items_view: forestx_app_server_protocol::TurnItemsView::Full,
                    items: Vec::new(),
                    status: forestx_app_server_protocol::TurnStatus::Interrupted,
                    error: None,
                    started_at: None,
                    completed_at: None,
                    duration_ms: None,

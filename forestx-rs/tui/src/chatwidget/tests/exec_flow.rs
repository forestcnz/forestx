...
async fn unified_exec_wait_after_final_agent_message_snapshot() {
    ...
    begin_unified_exec_startup(&mut chat, "call-wait", "proc-1", "cargo test -p forestx-core");
...
async fn unified_exec_wait_status_renders_command_in_single_details_row_snapshot() {
    ...
    begin_unified_exec_startup(
        ...
        "call-wait-ui",
        "proc-ui",
        "cargo test -p forestx-core -- --exact some::very::long::test::name",
    );
...
async fn apply_patch_approval_sends_op_with_call_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        ...
        } = app_ev
        {
            ...
            assert_matches!(
                decision,
                forestx_app_server_protocol::FileChangeApprovalDecision::Accept
            );
            found = true;
            break;
        }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto forestx_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to forestx channel");
    match forwarded {

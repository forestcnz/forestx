...
fn thread_settings_for_test(
    model: &str,
    thread_id: ThreadId,
) -> forestx_app_server_protocol::ThreadSettingsUpdatedNotification {
    forestx_app_server_protocol::ThreadSettingsUpdatedNotification {
        thread_id: thread_id.to_string(),
        thread_settings: forestx_app_server_protocol::ThreadSettings {
...
async fn invalid_url_elicitation_is_declined() {
    ...
    chat.handle_elicitation_request_now(
        forestx_app_server_protocol::RequestId::Integer(9),
        forestx_app_server_protocol::McpServerElicitationRequestParams {
            thread_id: request_thread_id.to_string(),
            turn_id: Some("turn-auth".to_string()),
            server_name: "payments".to_string(),
            request: forestx_app_server_protocol::McpServerElicitationRequest::Url {
                meta: None,
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: forestx_app_server_protocol::RequestId::Integer(9),
...
async fn thread_settings_updated_updates_visible_state_without_transcript() {
    ...
    assert_eq!(
        chat.config_ref()
            ...
            .id,
        forestx_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
    );
...
async fn live_app_server_turn_completion_repairs_dropped_message_deltas() {
    ...
    completed_turn.items_view = forestx_app_server_protocol::TurnItemsView::Summary;
    ...
    chat.handle_server_notification(
        ServerNotification::Error(ErrorNotification {
            error: AppServerTurnError {
                ...
                forestx_error_info: Some(ForestxErrorInfo::CyberPolicy),
                additional_details: None,
            },
            will_retry: false,
            thread_id: "thread-1".to_string(),
...
async fn app_server_safety_access_errors_render_dedicated_notice() {
    ...
    for (case, message) in cases {
        let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
        chat.handle_non_retry_error(message, /*forestx_error_info*/ None);

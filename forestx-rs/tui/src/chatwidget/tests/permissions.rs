...
fn windows_sandbox_requirements_stack(
    ...
    let requirements_toml = forestx_config::ConfigRequirementsToml {
        windows: Some(forestx_config::WindowsRequirementsToml {
...
fn requirements_stack(requirements_toml: forestx_config::ConfigRequirementsToml) -> ConfigLayerStack {
    let mut requirements_with_sources = forestx_config::ConfigRequirementsWithSources::default();
    requirements_with_sources
        .merge_unset_fields(RequirementSource::Unknown, requirements_toml.clone());
    let requirements = forestx_config::ConfigRequirements::try_from(requirements_with_sources)
...
async fn profile_permissions_selection_popup_with_disallowed_full_access_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.explicit_permission_profile_mode = true;
    chat.config.config_layer_stack = requirements_stack(forestx_config::ConfigRequirementsToml {
...
async fn startup_windows_sandbox_prompt_blocks_disallowed_unelevated_fallback() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(
        popup.contains("Your organization requires the default Forestx agent sandbox"),
...
async fn permissions_selection_sends_approvals_reviewer_in_override_turn_context() {
    ...
    let op = std::iter::from_fn(|| rx.try_recv().ok())
        .find_map(|event| match event {
            AppEvent::ForestxOp(op @ Op::OverrideTurnContext { .. }) => Some(op),
            _ => None,
        })

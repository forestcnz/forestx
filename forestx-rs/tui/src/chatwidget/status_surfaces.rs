...
impl ChatWidget {
    ...
    /// Clears the terminal title Forestx most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_forestx_folder } => {
                    dot_forestx_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::ForestxVersion => Some(FORESTX_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("forestx".to_string()),
            ...
            StatusSurfacePreviewItem::ForestxVersion => StatusLineItem::ForestxVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("forestx".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::ForestxVersion => self
                .status_line_value_for_item(StatusLineItem::ForestxVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_forestx_window(snapshot, "5h")
...
fn weekly_status_window(
    ...
    find_forestx_window(snapshot, "weekly")
        .or_else(|| snapshot.secondary.as_ref().map(|window| (window, true)))
}

fn find_forestx_window<'a>(
...
fn find_primary_forestx_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {

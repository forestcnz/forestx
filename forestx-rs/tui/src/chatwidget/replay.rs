            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: forestx_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            forestx_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        forestx_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            item @ ThreadItem::CommandExecution {
                status: forestx_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
            ...
            ThreadItem::WebSearch(item) => {
                ...
                self.on_web_search_end(
                    ...
                    item.query,
                    item.action
                        .unwrap_or(forestx_app_server_protocol::WebSearchAction::Other),
                );
            }

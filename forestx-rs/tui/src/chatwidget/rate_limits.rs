...
pub(super) fn app_server_rate_limit_error_kind(
    info: &AppServerForestxErrorInfo,
) -> Option<RateLimitErrorKind> {
    match info {
        AppServerForestxErrorInfo::ServerOverloaded => Some(RateLimitErrorKind::ServerOverloaded),
        AppServerForestxErrorInfo::UsageLimitExceeded => Some(RateLimitErrorKind::UsageLimit),
        AppServerForestxErrorInfo::ResponseTooManyFailedAttempts {
...
pub(super) fn is_app_server_cyber_policy_error(info: &AppServerForestxErrorInfo) -> bool {
    matches!(info, AppServerForestxErrorInfo::CyberPolicy)
}
...
impl ChatWidget {
    ...
    fn on_rate_limit_snapshot_from(
        ...
        if let Some(mut snapshot) = snapshot {
            ...
            let is_forestx_limit = limit_id.eq_ignore_ascii_case("forestx");
            if is_forestx_limit
                && (matches!(source, RateLimitSnapshotSource::AccountUsage)
                    || snapshot.spend_control_reached.is_some())
            {
                self.forestx_spend_control_reached = snapshot.spend_control_reached;
            }
            if (is_forestx_limit && matches!(source, RateLimitSnapshotSource::AccountUsage))
                || snapshot.rate_limit_reached_type.is_some()
            {
                self.forestx_rate_limit_reached_type = snapshot.rate_limit_reached_type;
            }
            let workspace_limit_reached = self.forestx_spend_control_reached == Some(true)
                || matches!(
                    self.forestx_rate_limit_reached_type,
                    Some(
            ...
            if is_forestx_limit && has_workspace_credits {
            ...
            let should_warn_about_rate_limit_usage = is_forestx_limit && !has_workspace_credits;
            ...
            let high_usage = is_forestx_limit
                && (snapshot
                    .secondary
                    .as_ref()
            ...
            self.forestx_rate_limit_reached_type = None;
            self.forestx_spend_control_reached = None;
        }
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::ForestxOp(AppCommand::override_turn_context(
                /*cwd*/ None,
    ...
    pub(super) fn open_workspace_owner_nudge_prompt(
        ...
        let (title, prompt) = match credit_type {
            ...
            AddCreditsNudgeCreditType::UsageLimit => (
                "Usage limit reached",
                "Request a limit increase from your owner to continue using forestx. Request increase?",
            ),
        };

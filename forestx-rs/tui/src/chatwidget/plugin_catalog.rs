...
impl ChatWidget {
    ...
    pub(super) fn marketplace_remove_confirmation_popup_params(
        ...
        header.push(Line::from(
            "This removes the configured marketplace from Forestx.".dim(),
        ));
                ...
                if let Some((location, plugin_name)) = plugin_detail_request {
                    vec![Box::new(move |tx| {
                        ...
                        tx.send(AppEvent::FetchPluginDetail {
                            cwd: cwd.clone(),
                            params: forestx_app_server_protocol::PluginReadParams {
                                marketplace_path,
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(forestx_app_server_protocol::HookEventName, usize)>::new();

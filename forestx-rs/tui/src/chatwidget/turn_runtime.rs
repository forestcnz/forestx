...
impl ChatWidget {
    ...
    pub(super) fn handle_app_server_steer_rejected_error(
        &mut self,
        forestx_error_info: &AppServerForestxErrorInfo,
    ) -> bool {
        matches!(
            forestx_error_info,
            AppServerForestxErrorInfo::ActiveTurnNotSteerable { .. }
    ...
    pub(super) fn on_server_overloaded_error(&mut self, message: String) {
        ...
        let message = if message.trim().is_empty() {
            "Forestx is currently experiencing high load.".to_string()
        } else {
            message
        };
    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        let rate_limit_reached_type = self.forestx_rate_limit_reached_type.map(|kind| {
        ...
        self.forestx_rate_limit_reached_type = rate_limit_reached_type;
    ...
    pub(super) fn handle_non_retry_error(
        &mut self,
        message: String,
        forestx_error_info: Option<AppServerForestxErrorInfo>,
    ) {
        if forestx_error_info
            .as_ref()
            .is_some_and(|info| self.handle_app_server_steer_rejected_error(info))
        {
        } else if forestx_error_info
            .as_ref()
            ...
            })
        {
            ...
            self.request_redraw();
            self.maybe_send_next_queued_input();
        } else if let Some(info) = forestx_error_info
            .as_ref()

...
impl ChatWidget {
    ...
    pub(super) fn elevated_windows_sandbox_setup_required(&self) -> bool {
        crate::windows_sandbox::level_from_config(&self.config) == WindowsSandboxLevel::Elevated
            && self
                ...
                .source
                .is_some()
            && !crate::windows_sandbox::sandbox_setup_is_complete(self.config.forestx_home.as_path())
    }
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match forestx_windows_sandbox::apply_world_writable_scan_and_denies_for_permissions(
            self.config.forestx_home.as_path(),
            cwd.as_path(),
            &env_map,
            &permissions,
            Some(self.config.forestx_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        self.session_telemetry.counter(
            "forestx.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        header.push(*Box::new(
            Paragraph::new(if allow_unelevated {
                vec![
                    line!["Set up the Forestx agent sandbox to protect your files and control network access. Learn more <https://developers.openai.com/forestx/windows>"],
                ]
            } else {
                vec![
                    line!["Your organization requires the default Forestx agent sandbox to continue. Set it up to protect your files and control network access."],
                    line!["Learn more <https://developers.openai.com/forestx/windows>"],
                ]
            })
        ...
        let mut items = vec![SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                accept_otel.counter(
                    "forestx.windows_sandbox.elevated_prompt_accept",
                    /*inc*/ 1,
                    &[],
                );
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "forestx.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                quit_otel.counter(
                    "forestx.windows_sandbox.elevated_prompt_quit",
                    /*inc*/ 1,
                    &[],
                );
    ...
    pub(crate) fn open_windows_sandbox_fallback_prompt(
        ...
        if allow_unelevated {
            lines.push(line![
                "You can still use Forestx in a non-admin sandbox. It carries greater risk if prompt injected."
            ]);
        } else {
            lines.push(line![
                "Your organization requires the default sandbox before Forestx can continue."
            ]);
        }
        lines.push(line![
            "Learn more <https://developers.openai.com/forestx/windows>"
        ]);
        ...
        let mut items = vec![SelectionItem {
            ...
            actions: vec![Box::new({
                ...
                move |tx| {
                    otel.counter(
                        "forestx.windows_sandbox.fallback_retry_elevated",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
        ...
        if allow_unelevated {
            items.push(SelectionItem {
                name: "Use Forestx with non-admin sandbox".to_string(),
                description: None,
                actions: vec![Box::new({
                    let otel = self.session_telemetry.clone();
                    ...
                    move |tx| {
                        otel.counter(
                            "forestx.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(move |tx| {
                quit_otel.counter(
                    "forestx.windows_sandbox.fallback_prompt_quit",
                    /*inc*/ 1,
                    &[],
                );
                tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));

...
impl App {
    ...
    pub(super) fn open_desktop_thread(&mut self, thread_id: ThreadId) {
        let url = format!("forestx://threads/{thread_id}");
    ...
    pub(super) fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, FORESTX_CLI_VERSION)
    }
...
$package = Get-AppxPackage -Name OpenAI.Forestx -ErrorAction SilentlyContinue
if ($null -eq $package) {{
...
$application = $manifest.Package.Applications.Application |
    Where-Object {{
        @($_.Extensions.Extension) | Where-Object {{
            $_.Category -eq 'windows.protocol' -and $_.Protocol.Name -eq 'forestx'
        }}
    }} |
    Select-Object -First 1
if ($null -eq $application -or [string]::IsNullOrWhiteSpace($application.Executable)) {{
    Write-Error 'Desktop app package does not declare a forestx protocol executable'
    exit 1
}}

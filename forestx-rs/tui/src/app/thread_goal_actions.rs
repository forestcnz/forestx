...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `forestx` to start a saved session, or `forestx resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let forestx_home = app_server.forestx_home_path(&self.config.forestx_home);
        match goal_files::objective_text_for_edit(app_server, forestx_home.as_ref(), &goal.objective)
            .await
        {

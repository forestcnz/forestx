...
impl App {
    ...
    pub(super) async fn apply_permission_profile_selection(
        ...
        self.sync_active_thread_permission_settings_to_cached_session()
            .await;
        self.app_event_tx
            .send(AppEvent::ForestxOp(AppCommand::override_turn_context(
                /*cwd*/ None,
                approval_policy,
                approvals_reviewer,
    ...
    pub(super) async fn read_effective_config_after_overridden_write(
        ...
        match crate::config_update::read_effective_config(app_server.request_handle(), cwd).await {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "{setting} were saved, but Forestx could not refresh the effective config: {err}"
                ));
                None
            }
        }
    ...
    pub(super) fn reasoning_label_for(
        ...
        (!model.starts_with("forestx-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }
    ...
    pub(super) fn restore_runtime_theme_from_config(&self) {
        if let Some(name) = self.config.tui_theme.as_deref()
            && let Some(theme) =
                crate::render::highlight::resolve_theme_by_name(name, Some(&self.config.forestx_home))
        {
...
fn windows_sandbox_mode_from_effective_config(
    effective_config: &ConfigReadResponse,
) -> Option<forestx_config::types::WindowsSandboxModeToml> {
...
mod tests {
    ...
    async fn resume_model_settings_preserves_only_explicit_model_overrides() {
        ...
        let profile = "work"
            .parse::<forestx_config::ProfileV2Name>()
    ...
    -> Result<()> {
        let mut app = make_test_app().await;
        let forestx_home = tempdir()?;
        let required_policy = forestx_protocol::protocol::AskForApproval::Never;
...
"#
    ...
    -> Result<()> {
        ...
        let forestx_home = tempdir()?;
        app.config.forestx_home = forestx_home.path().to_path_buf().abs();
        std::fs::write(forestx_home.path().join("config.toml"), "[broken")?;

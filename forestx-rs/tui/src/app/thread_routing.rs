...
impl App {
            ...
            ServerRequest::McpServerElicitationRequest { request_id, params } => {
                    ...
                    )
                {
                    ...
                    match &params.request {
                        forestx_app_server_protocol::McpServerElicitationRequest::Form {
                            message,
                            ..
                        ...
                        forestx_app_server_protocol::McpServerElicitationRequest::OpenAiForm {
                            ..
                        }
                        | forestx_app_server_protocol::McpServerElicitationRequest::Url { .. } => {
                            self.app_event_tx.resolve_elicitation(
                                ...
                                request_id.clone(),
                                forestx_app_server_protocol::McpServerElicitationAction::Decline,
                                /*content*/ None,
                                /*meta*/ None,
                            );
                            None
    ...
    pub(super) fn append_message_history_entry(&self, thread_id: ThreadId, text: String) {
        let history_config = forestx_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().forestx_home.clone(),
            &self.chat_widget.config_ref().history,
        );
        tokio::spawn(async move {
            if let Err(err) =
                forestx_message_history::append_entry(&text, thread_id, &history_config).await
            {
                tracing::warn!(
                    thread_id = %thread_id,
                    error = %err,
    ...
    pub(super) async fn lookup_message_history_entry(
        ...
        tokio::spawn(async move {
            let entry_opt = tokio::task::spawn_blocking(move || {
                forestx_message_history::lookup(log_id, offset, &history_config)
            })
            .await
            .unwrap_or_else(|err| {
    ...
    pub(super) async fn lookup_message_history_batch(
        &mut self,
        thread_id: ThreadId,
        cursor: forestx_message_history::HistoryBatchCursor,
        log_id: u64,
    ) -> Result<()> {
        let history_config = forestx_message_history::HistoryConfig::new(
            self.chat_widget.config_ref().forestx_home.clone(),
        ...
        tokio::spawn(async move {
            let event = match tokio::task::spawn_blocking(move || {
                forestx_message_history::lookup_batch(log_id, cursor, &history_config)
            })
            .await
            {
                Ok(Ok(batch)) => {
        ...
        match op {
            ...
            AppCommand::ListSkills { cwds, force_reload } => {
                self.handle_skills_list_result(
                    app_server
                        .skills_list(forestx_app_server_protocol::SkillsListParams {
                            cwds: cwds.clone(),

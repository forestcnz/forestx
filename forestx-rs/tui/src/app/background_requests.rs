...
pub(super) async fn fetch_account_token_activity(
    request_handle: AppServerRequestHandle,
) -> Result<forestx_app_server_protocol::GetAccountTokenUsageResponse> {
...
pub(super) async fn fetch_workspace_messages(
    request_handle: AppServerRequestHandle,
) -> Result<forestx_app_server_protocol::GetWorkspaceMessagesResponse> {
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<forestx_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: forestx_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
...
fn plugin_remote_section_error_next_step(label: &str, err: &str) -> &'static str {
    ...
    } else if err.contains("forestx plugins are disabled")
        || err.contains("plugin sharing is disabled")
        || err.contains("plugin sharing is not enabled")
        || err.contains("feature disabled")
    {
        "Ask a workspace admin to enable Forestx plugins or plugin sharing."
    ...
    } else if err.contains("old build") || err.contains("update forestx") || err.contains("stale") {
        "Update Forestx, then try opening the shared plugin again."
...
fn plugin_sharing_disabled_remote_section_error() -> PluginRemoteSectionError {
    PluginRemoteSectionError {
        ...
        message: "Plugin sharing is disabled for this Forestx session. Enable plugin sharing to load shared plugins.".to_string(),
    }
}
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![forestx_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, forestx_protocol::mcp::Tool>,
    HashMap<String, Vec<forestx_protocol::mcp::Resource>>,
    HashMap<String, Vec<forestx_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                forestx_app_server_protocol::McpAuthStatus::Unknown => McpAuthStatus::Unknown,
                forestx_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                forestx_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                forestx_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                forestx_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );

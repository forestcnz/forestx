...
use forestx_app_server_client::AppServerPath;
use forestx_app_server_protocol::AdditionalFileSystemPermissions;
use forestx_app_server_protocol::AdditionalNetworkPermissions;
use forestx_app_server_protocol::AdditionalPermissionProfile;
use forestx_app_server_protocol::AgentMessageDeltaNotification;
...
use forestx_app_server_protocol::FileUpdateChange;
use forestx_app_server_protocol::ItemStartedNotification;
use forestx_app_server_protocol::JSONRPCErrorError;
use forestx_app_server_protocol::McpServerElicitationRequest;
...
use forestx_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use forestx_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use forestx_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use forestx_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use forestx_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use forestx_app_server_protocol::PatchChangeKind;
use forestx_app_server_protocol::PermissionsRequestApprovalParams;
use forestx_app_server_protocol::RequestId as AppServerRequestId;
...
use forestx_app_server_protocol::UserInput as AppServerUserInput;
...
use forestx_protocol::models::NetworkPermissions;
use forestx_protocol::models::PermissionProfile;
use forestx_protocol::protocol::EventMsg;
use forestx_protocol::protocol::MAX_THREAD_GOAL_OBJECTIVE_CHARS;
...
use forestx_protocol::protocol::SessionSource as RolloutSessionSource;
...
use forestx_protocol::user_input::TextElement;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let forestx_app_server_client::AppServerEvent::ServerNotification(notification) = event
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: forestx_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_forestx_backend_auth: false,
        model_catalog: app.model_catalog.clone(),
        feedback: forestx_feedback::ForestxFeedback::new(),
        is_first_run: false,
    ...
    while let Ok(event) = app_event_rx.try_recv() {
        match event {
            ...
            AppEvent::ForestxOp(Op::UserTurn { items, .. }) => {
                assert!(
...
async fn history_lookup_response_is_routed_to_requesting_thread() -> Result<()> {
    ...
    let cursor = forestx_message_history::HistoryBatchCursor::new(/*end_offset*/ 10);
...
async fn active_history_batch_is_delivered_without_replay_buffering() -> Result<()> {
    ...
    let cursor = forestx_message_history::HistoryBatchCursor::new(/*end_offset*/ 1);
    let event = HistoryLookupResponse::Batch {
        ...
        next_older_cursor: Some(forestx_message_history::HistoryBatchCursor::new(
            /*end_offset*/ 0,
        )),
    };
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: forestx_app_server_protocol::CollabAgentTool::Wait,
                status: forestx_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(Box::new(
        ServerNotification::ItemCompleted(forestx_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: forestx_app_server_protocol::CollabAgentTool::SendInput,
                status: forestx_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    forestx_app_server_protocol::CollabAgentState {
                        status: forestx_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
    ...
    runtime.block_on(async {
            ...
            .enumerate()
        {
            ...
            let session_meta = SessionMeta {
                ...
                originator: "forestx-tui-test".to_string(),
        ...
        assert!(
            std::iter::from_fn(|| app_event_rx.try_recv().ok())
                .any(|event| matches!(event, AppEvent::ForestxOp(Op::UserTurn { .. })))
        );
...
async fn update_memory_settings_persists_and_updates_widget_config() -> Result<()> {
    ...
    app.config.forestx_home = forestx_home.path().to_path_buf().abs();
    ...
    let config = std::fs::read_to_string(forestx_home.path().join("config.toml"))?;
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let forestx_home = tempdir()?;
        app.config.forestx_home = forestx_home.path().to_path_buf().abs();
        app.config.sqlite = forestx_state::SqliteConfig::new_for_testing(forestx_home.path().abs());
        ...
        let state_db = forestx_state::StateRuntime::init(
            forestx_state::SqliteConfig::new_for_testing(forestx_home.path().abs()),
...
async fn reset_memories_clears_local_memory_directories() -> Result<()> {
    Box::pin(async {
        ...
        let memory_root = forestx_home.path().join("memories");
...
"#,
    ...
    let op = match app_event_rx.try_recv() {
        Ok(AppEvent::ForestxOp(op)) => op,
        other => panic!("expected ForestxOp event, got {other:?}"),
    };
    assert_eq!(
        op,
...
-> Result<()> {
    ...
    let forestx_home = tempdir()?;
    app.config.forestx_home = forestx_home.path().to_path_buf().abs();
    let config_toml_path = forestx_home.path().join("config.toml").abs();
...
async fn side_defers_subagent_approval_overlay_until_side_exits() -> Result<()> {
    ...
    app.enqueue_thread_request(
        ...
        ServerRequest::DynamicToolCall {
            ...
            params: forestx_app_server_protocol::DynamicToolCallParams {
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        ...
        vec![
            forestx_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            forestx_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            forestx_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            forestx_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: forestx_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: forestx_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: forestx_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                updated_at: 2,
                recency_at: Some(2),
                status: forestx_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: forestx_app_server_protocol::SessionSource::Unknown,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            forestx_app_server_protocol::ServerRequestResolvedNotification {
...
async fn primary_thread_ignores_child_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        forestx_app_server_client::AppServerEvent::ServerNotification(Box::new(
...
async fn background_side_cleanup_removes_local_state_and_ignores_late_events() -> Result<()> {
    ...
    app.handle_app_server_event(
        &app_server,
        forestx_app_server_client::AppServerEvent::ServerRequest(Box::new(exec_approval_request(
            side_thread_id,
            "turn-1",
            "item-1",
            Some("approval-1"),
    ...
    let resolution = app
        ...
        .take_resolution(Op::ExecApproval {
            ...
            turn_id: None,
            decision: forestx_app_server_protocol::CommandExecutionApprovalDecision::Accept,
        })
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            forestx_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        forestx_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
...
async fn set_thread_goal_draft_materializes_long_objective_and_confirms_before_paste() -> Result<()>
{
    ...
    let forestx_home = app_server
        .forestx_home_path(&app.chat_widget.config_ref().forestx_home)
        .expect("forestx home");
    assert!(goal_files::objective_file_path(&goal.objective, Some(&forestx_home)).is_some());
    assert_eq!(
        goal_files::objective_text_for_edit(&mut app_server, Some(&forestx_home), &goal.objective)
            .await
    ...
    for path in [
        format!("/tmp/{suffix}"),
        format!("{forestx_home}/../other/{suffix}"),
        format!("{forestx_home}/other/{suffix}"),
    ] {
        assert!(!is_managed(&forestx_home, &path));
    }
    assert!(!is_managed(
        &AppServerPath::from_app_server("/tmp/forestx\\home"),
        &format!("/tmp/forestx/home/{suffix}")
    ));
    let unix_path = AppServerPath::from_app_server("/tmp/forestx\\").join("a");
    assert_eq!(unix_path.as_str(), "/tmp/forestx\\/a");
    let attachments_dir = app.chat_widget.config_ref().forestx_home.join("attachments");
...
fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
    let turn_error = AppServerTurnError {
        ...
        forestx_error_info: Some(AppServerForestxErrorInfo::ActiveTurnNotSteerable {
...
fn session_start_error_surfaces_archived_guidance_without_rollout_path() {
    ...
    let target_session = SessionTarget {
        path: Some(std::path::PathBuf::from(
            "/Users/me/.forestx/archived_sessions/rollout.jsonl",
        )),
        thread_id,
    };
    let expected = format!(
        "session {thread_id} is archived. Run `forestx unarchive {thread_id}` to unarchive it first."
    );
...
async fn remote_resume_current_cwd_rejection_snapshot() -> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    std::fs::write(
        app.config.forestx_home.join("config.toml"),
        "[tui]\nresume_cwd = \"current\"\n",
    )?;
...
async fn remote_exec_resume_current_cwd_is_rejected() -> Result<()> {
    ...
    app.environment_manager = Arc::new(
        EnvironmentManager::create_for_tests(
            ...
            Some(forestx_exec_server::ExecServerRuntimePaths::new(
                std::env::current_exe()?,
                /*forestx_linux_sandbox_exe*/ None,
            )?),
        )
        .await,
    );
...
async fn remote_resume_keeps_server_only_cwd_out_of_local_config() -> Result<()> {
    ...
    let thread_id = app_test_support::create_fake_rollout(
        app.config.forestx_home.as_path(),
        filename_timestamp,
        "2025-01-05T12:00:00Z",
        "Saved user message",
...
async fn in_app_resume_uses_configured_or_explicit_cwd() -> Result<()> {
    for (configured_mode, has_explicit_cwd, has_remote_exec, expected_directory) in [
        ...
        let config = ConfigBuilder::default()
            .forestx_home(forestx_home.clone())
            .loader_overrides(LoaderOverrides::without_managed_config_for_tests())
        ...
        .expect("materialized rollout should be created");
        let rollout_path =
            app_test_support::rollout_path(&forestx_home, filename_timestamp, &thread_id);
...
async fn remembered_current_cwd_stays_at_launch_across_in_app_resumes() -> Result<()> {
    ...
    crate::legacy_core::config::edit::ConfigEditsBuilder::for_config(&config)
        .set_resume_cwd(forestx_config::types::ResumeCwdMode::Current)
        .apply()
        .await
...
async fn prompt_edit_forks_before_selected_prompt_and_preserves_source() -> Result<()> {
    ...
    .expect("materialized rollout should be created");
    let source_path =
        app_test_support::rollout_path(config.forestx_home.as_path(), filename_ts, &source_thread_id);
    ...
    for (turn_id, message, images, local_images) in [
        ...
        for item in [
            ...
            forestx_rollout::append_rollout_item_to_path(&source_path, &item).await?;
        }
    }
...
async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
    ...
    let replacement = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        has_forestx_backend_auth: app.chat_widget.has_forestx_backend_auth(),
        model_catalog: app.model_catalog.clone(),
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(Box::new(
                ServerNotification::ItemStarted(
                    forestx_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: forestx_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                forestx_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                forestx_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: forestx_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: forestx_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                forestx_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
...
async fn clear_only_ui_reset_allows_active_skill_warning_to_render_again() {
    ...
    let error = SkillErrorInfo {
        path: test_path_buf("/tmp/project/.forestx/skills/abc/SKILL.md"),

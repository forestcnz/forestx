...
pub(super) fn emit_system_bwrap_warning(app_event_tx: &AppEventSender, config: &Config) {
    let Some(message) =
        forestx_sandboxing::system_bwrap_warning(config.permissions.permission_profile())
    else {
        return;
    };
...
pub(super) fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_FORESTX_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_forestx_max_migration_prompt
            .unwrap_or(false),
...
mod tests {
    ...
    fn skill_load_warning_state_suppresses_repeated_active_errors() {
        ...
        let error = skill_error("/repo/.forestx/skills/abc/SKILL.md", "invalid description");
    ...
    fn skill_load_warning_state_displays_new_message_for_active_path() {
        let mut state = SkillLoadWarningState::default();
        let initial = skill_error("/repo/.forestx/skills/abc/SKILL.md", "invalid description");
        let changed = skill_error("/repo/.forestx/skills/abc/SKILL.md", "invalid frontmatter");
...
⚠ /repo/.forestx/skills/abc/SKILL.md: invalid description
");
    }
}

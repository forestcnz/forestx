...
impl HooksBrowserView {
    ...
    fn event_rows(&self) -> Vec<EventRow> {
        forestx_protocol::protocol::HookEventName::iter()
            .map(|event_name| {
    ...
    fn selected_event(&self) -> Option<HookEventName> {
        self.state
            .selected_idx
            .and_then(|idx| forestx_protocol::protocol::HookEventName::iter().nth(idx))
            .map(Into::into)
    }
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => forestx_protocol::protocol::HookEventName::iter().count(),
...
fn event_description(event_name: HookEventName) -> &'static str {
    match event_name {
        ...
        HookEventName::Stop => "Right before Forestx ends its turn",
    }
}
        ...
        HooksBrowserView::new(
            vec![
                hook(
                    ...
                    "${FORESTX_PLUGIN_ROOT}/hooks/pre-tool-use-check.sh",
                    /*enabled*/ true,
                    /*is_managed*/ false,
                    /*display_order*/ 0,
                ),

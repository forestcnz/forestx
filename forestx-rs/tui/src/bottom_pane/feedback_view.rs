...
use forestx_feedback::DOCTOR_REPORT_ATTACHMENT_FILENAME;
use forestx_feedback::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use forestx_feedback::FeedbackDiagnostics;
...
const BASE_CLI_BUG_ISSUE_URL: &str =
    "https://github.com/openai/forestx/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const FORESTX_FEEDBACK_INTERNAL_URL: &str = "http://go/forestx-feedback-internal";
...
pub(crate) fn feedback_success_cell(
    ...
    let mut lines = vec![Line::from(match issue_url.as_ref() {
        Some(_) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            format!("{prefix} Please report this in #forestx-feedback:")
        }
    ...
    match issue_url {
        Some(url) if feedback_audience == FeedbackAudience::OpenAiEmployee => {
            lines.extend([
                ...
                Line::from(vec![
                    "    ".into(),
                    format!("https://go/forestx-feedback/{thread_id}").bold(),
                ]),
            ]);
        }
        Some(url) => {
...
fn slack_feedback_url(_thread_id: &str) -> String {
    FORESTX_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "forestx-logs.log".into()]).into(),
        Line::from(vec![
            "  • ".into(),
        ...
        Line::from(vec![
            "  • ".into(),
            format!("{FORESTX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
        Line::from(vec![
            "  • ".into(),
            format!("{FORESTX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME} (if available)").into(),
        ])
        .into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Forestx session logs and diagnostics with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use forestx_feedback::FeedbackDiagnostic;
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_slack_url = "http://go/forestx-feedback-internal".to_string();
        ...
        let expected_external_url = "https://github.com/openai/forestx/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
    ...
    fn feedback_success_cell_matches_employee_bug_copy() {
        ...
        assert_eq!(
            rendered,
            "• Feedback uploaded. Please report this in #forestx-feedback:\n\n  http://go/forestx-feedback-internal\n\n  Share this and add some info about your problem:\n    https://go/forestx-feedback/thread-2"
        );
    }

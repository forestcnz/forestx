...
const MCP_FORESTX_APPS_SERVER_NAME: &str = "forestx_apps";
const MCP_TOOL_FORESTX_APPS_META_KEY: &str = "_forestx_apps";
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        request: &forestx_app_server_protocol::McpServerElicitationRequest,
    ) -> Option<Self> {
        let forestx_app_server_protocol::McpServerElicitationRequest::Url {
            meta,
            message,
            url,
            elicitation_id,
        ...
        if server_name == MCP_FORESTX_APPS_SERVER_NAME {
            let url = validate_external_url(url, /*require_chatgpt_host*/ true)?;
            return Self::from_forestx_apps_auth_url_parts(
                thread_id,
                server_name,
                request_id,
                meta.as_ref(),
    ...
    fn from_forestx_apps_auth_url_parts(
        thread_id: ThreadId,
        server_name: &str,
        request_id: AppServerRequestId,
        ...
        let auth_failure = meta?
            .as_object()?
            .get(MCP_TOOL_FORESTX_APPS_META_KEY)?
            .as_object()?
...
impl AppLinkView {
    ...
    fn complete_external_flow_and_close(&mut self) {
        let should_refresh_connectors = self
            .elicitation_target
            .as_ref()
            .is_none_or(|target| target.server_name == MCP_FORESTX_APPS_SERVER_NAME);
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let is_forestx_apps_auth = is_auth_suggestion
            && self
                .elicitation_target
                .as_ref()
                .is_some_and(|target| target.server_name == MCP_FORESTX_APPS_SERVER_NAME);
        lines.push(Line::from(
            if is_auth_suggestion {
                if is_forestx_apps_auth {
                    "Finish App Sign In"
                } else {
...
mod tests {
    ...
    fn auth_url_request(url: &str) -> forestx_app_server_protocol::McpServerElicitationRequest {
        forestx_app_server_protocol::McpServerElicitationRequest::Url {
            ...
            url: url.to_string(),
            elicitation_id: "forestx_apps_auth_call_123".to_string(),
        }
    }

    #[test]
    fn forestx_apps_auth_url_elicitation_builds_auth_app_link_params() {
    ...
    #[test]
    fn non_forestx_apps_url_elicitation_builds_generic_app_link_params() {
        let target = generic_url_target();
        let request = forestx_app_server_protocol::McpServerElicitationRequest::Url {
            meta: None,
    ...
    #[test]
    fn forestx_apps_auth_url_elicitation_rejects_untrusted_urls() {
        let target = suggestion_target();
        for url in [

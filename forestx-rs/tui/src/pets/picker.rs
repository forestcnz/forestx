...
/// has a sensible starting point without implying that Forestx is already the
...
pub(crate) fn build_pet_picker_params(
    ...
    let mut entries = available_pet_entries(forestx_home);
...
fn available_pet_entries(forestx_home: &Path) -> Vec<PetPickerEntry> {
    ...
    entries.extend(custom_pet_entries(forestx_home));
    entries
}

fn custom_pet_entries(forestx_home: &Path) -> Vec<PetPickerEntry> {
    let mut entries_by_selector = HashMap::new();
    for (directory_name, manifest_file) in [("avatars", "avatar.json"), ("pets", "pet.json")] {
        let Ok(children) = fs::read_dir(forestx_home.join(directory_name)) else {
            continue;
        };
        for child in children.flatten() {
            ...
            let Ok(pet) =
                Pet::load_with_forestx_home(&selector, /*forestx_home*/ Some(forestx_home))
            else {
                continue;
            };
...
mod tests {
    ...
    fn picker_lists_app_bundled_and_custom_pets() {
        ...
        write_pet(forestx_home.path(), "chefito", "Chefito");
    ...
    #[test]
    fn picker_preselects_forestx_without_marking_it_current_when_no_pet_is_configured() {
        let forestx_home = tempfile::tempdir().unwrap();
        let params = build_pet_picker_params(
            /*current_pet*/ None,
            forestx_home.path(),
        ...
        assert_eq!(params.initial_selected_idx, Some(2));
        assert_eq!(params.items[2].name, "Forestx");
    ...
    fn picker_imports_legacy_avatar_manifests() {
        let forestx_home = tempfile::tempdir().unwrap();
        write_legacy_avatar(forestx_home.path(), "legacy", "Legacy");

...
//! local spritesheets. The TUI resolves them from the public Forestx pets CDN on
//! first use, verifies that the downloaded file has the expected spritesheet
//! geometry, and installs it into a versioned cache under FORESTX_HOME.
//!
...
const PET_CDN_BASE_URL: &str = "https://persistent.oaistatic.com/forestx/pets/v1";
const PET_DOWNLOAD_TIMEOUT: Duration = Duration::from_secs(60);
const PET_MAX_DOWNLOAD_BYTES: u64 = 4 * 1024 * 1024;

pub(crate) fn builtin_spritesheet_path(forestx_home: &Path, file: &str) -> PathBuf {
    pack_dir(forestx_home).join("assets").join(file)
}
...
pub(crate) async fn ensure_builtin_pet(
    ...
    let destination = builtin_spritesheet_path(forestx_home, pet.spritesheet_file);
...
fn pack_dir(forestx_home: &Path) -> PathBuf {
    forestx_home.join(PET_PACK_DIR).join(PET_PACK_VERSION)
}
...
#[cfg(test)]
pub(crate) fn write_test_pack(forestx_home: &Path) {
    let assets_dir = pack_dir(forestx_home).join("assets");
...
mod tests {
    ...
    fn builtin_pet_url_uses_public_cdn_path() {
        ...
        assert_eq!(
            url,
            "https://persistent.oaistatic.com/forestx/pets/v1/dewey-spritesheet-v4.webp"
        );
    }

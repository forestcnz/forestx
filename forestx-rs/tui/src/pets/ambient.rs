//! Ambient terminal rendering for the Forestx companion.
//!
...
impl AmbientPet {
    ...
    /// FORESTX_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_forestx_home(
            ...
            /*forestx_home*/ Some(forestx_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = forestx_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_forestx_app_vocabulary() {

...
//! managed FORESTX_HOME cache, while custom pets remain entirely user-owned data
//! under `$FORESTX_HOME/pets/<pet-id>/pet.json` or legacy avatar directories.
//!
...
pub(crate) const DEFAULT_PET_ID: &str = "forestx";
...
async fn ensure_builtin_pack_for_pet(
    ...
    if let Some(pet) = catalog::builtin_pet(pet_id) {
        asset_pack::ensure_builtin_pet(forestx_home, pet, http_client).await?;
    }
    Ok(())
}
...
pub(crate) async fn load_pet_with_assets(
    ...
    ensure_builtin_pack_for_pet(&pet_id, &forestx_home, http_client).await?;

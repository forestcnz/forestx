...
//! in-memory `Pet`: built-in catalog ids, custom pet ids under FORESTX_HOME,
...
impl Pet {
    ...
    /// has already been materialized into FORESTX_HOME; if callers skip the
    /// asset-fetch step, they will get a missing-spritesheet error here on
    /// first use.
    pub(super) fn load_with_forestx_home(value: &str, forestx_home: Option<&Path>) -> Result<Self> {
        if path_like(value) {
            return load_pet_path(value);
        }

        if let Some(custom_id) = value.strip_prefix(CUSTOM_PET_PREFIX) {
            return load_custom_pet(custom_id, forestx_home);
        }

        if let Some(builtin) = catalog::builtin_pet(value) {
            return load_builtin_pet(builtin, forestx_home);
        }

        load_custom_pet(value, forestx_home)
    }
...
fn load_builtin_pet(pet: catalog::BuiltinPet, forestx_home: Option<&Path>) -> Result<Pet> {
    let forestx_home = forestx_home.context("FORESTX_HOME is not available")?;
    let spritesheet_path = super::builtin_spritesheet_path(forestx_home, pet.spritesheet_file);
...
fn load_custom_pet(value: &str, forestx_home: Option<&Path>) -> Result<Pet> {
    let forestx_home = forestx_home.context("FORESTX_HOME is not available")?;
    let pet_dir = forestx_home.join("pets").join(value);
    if pet_dir.join("pet.json").is_file() {
        return load_pet_manifest(&pet_dir, "pet.json", value, &custom_pet_cache_id(value));
    }

    let avatar_dir = forestx_home.join("avatars").join(value);
...
mod tests {
    ...
    fn load_pet_from_dir(dir: &tempfile::TempDir) -> Pet {
        Pet::load_with_forestx_home(dir.path().to_str().unwrap(), /*forestx_home*/ None).unwrap()
    }

    fn load_pet_error_from_dir(dir: &tempfile::TempDir) -> anyhow::Error {
        Pet::load_with_forestx_home(dir.path().to_str().unwrap(), /*forestx_home*/ None).unwrap_err()
    }

    #[test]
    fn load_builtin_pet_uses_app_catalog_storage() {
        let forestx_home = tempfile::tempdir().unwrap();
        super::super::asset_pack::write_test_pack(forestx_home.path());

        let pet =
            Pet::load_with_forestx_home("dewey", /*forestx_home*/ Some(forestx_home.path())).unwrap();
        ...
        assert_eq!(
            ...
            super::super::builtin_spritesheet_path(forestx_home.path(), "dewey-spritesheet-v4.webp")
        );
    ...
    fn load_pet_json_path_uses_containing_directory() {
        ...
        let pet = Pet::load_with_forestx_home(
    ...
    #[test]
    fn custom_pet_selector_loads_forestx_home_pet_manifest() {
        let dir = write_minimal_pet();
        let forestx_home = tempfile::tempdir().unwrap();
        let pet_dir = forestx_home.path().join("pets").join("chefito");
    ...
    fn custom_pet_selector_falls_back_to_legacy_avatar_manifest() {
        let dir = write_minimal_pet();
        let forestx_home = tempfile::tempdir().unwrap();
        let avatar_dir = forestx_home.path().join("avatars").join("legacy");
    ...
    fn custom_pet_rejects_spritesheet_path_escape() {
        let forestx_home = tempfile::tempdir().unwrap();
        let pet_dir = forestx_home.path().join("pets").join("escape");
        fs::create_dir_all(&pet_dir).unwrap();
        fs::write(
        ...
        let err = Pet::load_with_forestx_home(

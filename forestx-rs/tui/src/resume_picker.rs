...
use forestx_utils_path as path_utils;
...
async fn run_resume_picker_with_launch_context(
    ...
    run_session_picker_with_loader(
        ...
        spawn_app_server_page_loader(
            ...
            (!uses_remote_workspace).then(|| config.forestx_home.to_path_buf()),
            bg_tx,
        ),
        bg_rx,
    )
        ...
        while let Some(request) = request_rx.recv().await {
            match request {
                ...
                PickerLoadRequest::Preview { thread_id } => {
                    let preview =
                        load_transcript_preview(&mut app_server, thread_id, forestx_home.as_deref())
                            .await;
...
pub(crate) async fn load_transcript_preview(
    ...
    let inline_visualization_context = forestx_home.and_then(|forestx_home| {
        ThreadId::from_string(&thread.id)
            .ok()
            .and_then(|thread_id| InlineVisualizationContext::new(forestx_home, thread_id))
    });
        ...
        ThreadItem::UserMessage { content, .. } => TranscriptPreviewLine {
            ...
            text: content
                ...
                .filter_map(|input| match input {
                    forestx_app_server_protocol::UserInput::Text { text, .. } => Some(text.as_str()),
                    _ => None,
                })
                .collect::<Vec<_>>()
                .join(" "),
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.forestx_home)
...
mod tests {
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/forestx-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/forestx")),
            git_branch: Some(String::from("forestx/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      forestx/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/forestx", " main");
        assert_metadata_order(&created[0], "⌁ tmp/forestx", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/forestx"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/forestx", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/forestx.etraut-animations-false-improvements/forestx-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd = "~/code/forestx.owner-extremely-long-worktree-name-that-needs-truncating/forestx-rs";
        ...
        assert!(footer.contains("⌁ ~/code/forestx."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/forestx.owner-worktree/forestx-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let forestx_home_file = tmp.path().join("forestx-home-file");
        std::fs::write(&forestx_home_file, "not a directory").expect("write forestx home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        state.view_persistence = Some(SessionPickerViewPersistence {
            forestx_home: forestx_home_file,
        });
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/forestx.fcoury-session-picker/forestx-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
    ...
    fn expanded_session_snapshot() {
        ...
        let row = Row {
            ...
            created_at: parse_timestamp_str("2026-04-28T16:30:00Z"),
            updated_at: parse_timestamp_str("2026-04-28T17:45:00Z"),
            cwd: Some(PathBuf::from("/tmp/forestx")),
            git_branch: Some(String::from("fcoury/session-picker")),
        };
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![forestx_app_server_protocol::Turn {

...
#[cfg(any(unix, windows))]
const TUI_SOURCE_CLIENT_ID: &str = "forestx-tui";
#[cfg(any(unix, windows))]
const OPEN_IDE_HINT: &str =
    "Open this project in VS Code or Cursor with the Forestx extension active.";
#[cfg(any(unix, windows))]
const IDE_DID_NOT_PROVIDE_CONTEXT_HINT: &str = "The IDE extension did not provide context.";
#[cfg(any(unix, windows))]
const KEEP_TRYING_HINT: &str = "Forestx will keep trying on future messages.";
    ...
    pub(crate) fn user_facing_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Send(_) => {
                "Forestx could not request IDE context. Try /ide again.".to_string()
            }
            IdeContextError::Read(_) | IdeContextError::InvalidResponse(_) => {
                "Forestx could not read IDE context. Try /ide again.".to_string()
            }
        }
    }
    ...
    pub(crate) fn prompt_skip_hint(&self) -> String {
        match self {
            ...
            IdeContextError::Read(error) if error.kind() == std::io::ErrorKind::TimedOut => {
                "Forestx timed out waiting for IDE context. It will keep trying on future messages."
                    .to_string()
            }
            IdeContextError::RequestFailed(error) if error == "client-disconnected" => {
                hint_with_retry("The IDE connection changed while Forestx was requesting context.")
            }
            ...
            IdeContextError::Send(_) => {
                hint_with_retry("Forestx lost the IDE connection while requesting context.")
            }
            IdeContextError::InvalidResponse(_) => {
                hint_with_retry("Forestx received an unexpected IDE context response.")
            }
            IdeContextError::RequestFailed(_) => hint_with_retry(IDE_DID_NOT_PROVIDE_CONTEXT_HINT),
            IdeContextError::Read(_) => hint_with_retry("Forestx could not read IDE context."),
        }
    }
...
pub(crate) fn fetch_ide_context(
    ...
    let primary_socket_path = primary_ipc_socket_path(forestx_home);
...
#[cfg(unix)]
fn primary_ipc_socket_path(forestx_home: &Path) -> PathBuf {
    forestx_home.join("ipc").join("ipc.sock")
}

#[cfg(unix)]
fn legacy_ipc_socket_paths(temp_dir: &Path, uid: libc::uid_t) -> Vec<PathBuf> {
    let ipc_dir = temp_dir.join("forestx-ipc");
    if uid == 0 {
...
fn default_ipc_socket_path() -> PathBuf {
    PathBuf::from(r"\\.\pipe\forestx-ipc")
}
...
mod tests {
    ...
    #[test]
    fn primary_ipc_socket_path_uses_forestx_home() {
        let forestx_home = Path::new("/home/test/.forestx");

        assert_eq!(
            primary_ipc_socket_path(forestx_home),
            forestx_home.join("ipc").join("ipc.sock")
        );
    }
    ...
    fn validate_unix_socket_path_rejects_unsafe_parent_directory() {
        ...
        let socket_path = tempdir.path().join("forestx-ipc.sock");

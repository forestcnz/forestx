...
async fn state_runtime(config: &Config) -> std::io::Result<Arc<forestx_state::StateRuntime>> {
...
fn write_rollout(
    ...
    let session_meta = SessionMetaLine {
        meta: SessionMeta {
            ...
            cwd: config.forestx_home.join("project").to_path_buf(),
...
fn thread_metadata(
    ...
    builder.cwd = config.forestx_home.join("project").to_path_buf();
...
async fn deletes_valid_duplicate_after_stale_sqlite_hit() -> color_eyre::Result<()> {
    ...
    forestx_rollout::append_thread_name(config.forestx_home.as_path(), thread_id, "saved-session")
        .await?;
...
async fn trusts_sqlite_name_over_legacy_index_for_delete() -> color_eyre::Result<()> {
    ...
    runtime
        ...
        .await
        .map_err(std::io::Error::other)?;
    forestx_rollout::append_thread_name(config.forestx_home.as_path(), thread_id, "old-session")
        .await?;

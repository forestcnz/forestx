...
const CHATGPT_USAGE_URL: &str = "https://chatgpt.com/forestx/settings/usage";
...
impl HistoryCell for StatusHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        lines.push(Line::from(vec![
            ...
            Span::from("OpenAI Forestx").bold(),
            Span::from(" ").dim(),
            Span::from(format!("(v{FORESTX_CLI_VERSION})")).dim(),
        ]));
        ...
        let account_value = self.account.as_ref().map(|account| match account {
            ...
            StatusAccountDisplay::ApiKey => {
                "API key configured (run forestx login to use ChatGPT)".to_string()
            }
        });

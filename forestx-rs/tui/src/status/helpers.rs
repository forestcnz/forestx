...
mod tests {
    ...
    async fn test_config(forestx_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let forestx_home = TempDir::new().expect("temp forestx home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = forestx_home.path().join("global.md");
        let config = test_config(&forestx_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let forestx_home = TempDir::new().expect("temp forestx home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = forestx_home.path().join("override.md");
        let config = test_config(&forestx_home, &cwd).await;

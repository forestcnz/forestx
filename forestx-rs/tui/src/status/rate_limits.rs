...
use chrono::Local;
use chrono::Utc;
use forestx_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use forestx_app_server_protocol::RateLimitSnapshot;
use forestx_app_server_protocol::RateLimitWindow;
use forestx_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use forestx_protocol::num_format::format_with_separators;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `forestx` or `forestx_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "forestx".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("forestx");
        ...
        let combine_non_forestx_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_forestx_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_forestx_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_forestx_single_limit_renders_combined_row() {
        let now = Local::now();
        let forestx = RateLimitSnapshotDisplay {
            limit_name: "forestx".to_string(),
            captured_at: now,
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "forestx-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[forestx, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "forestx-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_forestx_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "forestx-other".to_string(),
            captured_at: now,

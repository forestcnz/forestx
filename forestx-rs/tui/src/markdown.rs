...
mod tests {
    ...
    fn append_markdown_preserves_full_text_line() {
        let src = "Hi! How can I help with forestx-rs today? Want me to explore the repo, run tests, or work on a specific change?\n";
        let mut out = Vec::new();
        append_markdown(src, /*width*/ None, /*cwd*/ None, &mut out);
        assert_eq!(
            out.len(),
        ...
        assert_eq!(
            rendered,
            "Hi! How can I help with forestx-rs today? Want me to explore the repo, run tests, or work on a specific change?"
        );
    }

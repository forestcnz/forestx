...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    if !config.check_for_update_on_startup || is_source_build_version(FORESTX_CLI_VERSION) {
        return None;
    }
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, FORESTX_CLI_VERSION).unwrap_or(false) {
...
const LATEST_RELEASE_URL: &str = "https://api.github.com/repos/openai/forestx/releases/latest";

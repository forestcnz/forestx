...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/openai/forestx".cyan().underlined(),
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{FORESTX_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
            "See full release notes:",
            "https://github.com/openai/forestx/releases/latest"
                .cyan()
                .underlined(),
        ];
    ...
    fn raw_lines(&self) -> Vec<Line<'static>> {
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            "See https://github.com/openai/forestx for installation options.".to_string()
        };
        vec![
            Line::from("Update available!"),
            Line::from(format!("{FORESTX_CLI_VERSION} -> {}", self.latest_version)),
            Line::from(update_instruction),
            Line::from(""),
            Line::from("See full release notes:"),
            Line::from("https://github.com/openai/forestx/releases/latest"),
        ]
    }

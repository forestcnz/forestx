...
use forestx_otel::RuntimeMetricTotals;
...
async fn test_config() -> Config {
    let forestx_home = std::env::temp_dir();
...
fn composite_cell_preserves_child_web_links() {
    let destination = "https://chatgpt.com/forestx/settings/usage";
...
fn empty_mcp_output_preserves_docs_hyperlink() {
    let destination = "https://developers.openai.com/forestx/mcp";
...
fn mcp_tools_output_from_statuses_renders_status_only_servers() {
    let statuses = vec![McpServerStatus {
        ...
        auth_status: forestx_app_server_protocol::McpAuthStatus::Unknown,
    }];
...
fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
    ...
    assert_eq!(
        rendered,
        vec![
            "✔ You approved forestx to".to_string(),
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p forestx-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),

...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<forestx_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<forestx_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(forestx_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`forestx-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `forestx mcp add`.
...
pub(crate) fn empty_mcp_output() -> WebHyperlinkHistoryCell {
    ...
    docs_line.push_span(
        "MCP docs".underlined(),
        Some("https://developers.openai.com/forestx/mcp"),
    );
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, forestx_protocol::mcp::Tool>,

...
impl AppEventSender {
    ...
    pub(crate) fn send(&self, event: AppEvent) {
        ...
        if !matches!(event, AppEvent::ForestxOp(_)) {
    ...
    pub(crate) fn interrupt(&self) {
        self.send(AppEvent::ForestxOp(AppCommand::interrupt()));
    }

    pub(crate) fn compact(&self) {
        self.send(AppEvent::ForestxOp(AppCommand::compact()));
    }

    pub(crate) fn set_thread_name(&self, name: String) {
        self.send(AppEvent::ForestxOp(AppCommand::set_thread_name(name)));
    }

    pub(crate) fn review(&self, target: ReviewTarget) {
        self.send(AppEvent::ForestxOp(AppCommand::review(target)));
    }

    pub(crate) fn list_skills(&self, cwds: Vec<PathBuf>, force_reload: bool) {
        self.send(AppEvent::ForestxOp(AppCommand::list_skills(
            cwds,
            force_reload,
        )));
    }
    ...
    pub(crate) fn user_input_answer(&self, id: String, response: ToolRequestUserInputResponse) {
        self.send(AppEvent::ForestxOp(AppCommand::user_input_answer(
            id, response,
        )));
    }

//! Forestx App directives embedded in assistant markdown.
    ...
    fn renders_code_comment_directives_as_markdown() {
        let parsed = parse_assistant_markdown(
            concat!(
                ...
                "\n\n",
                r#":::code-comment{title="[P1] Clamp the range" body="The line range should match the App." file="forestx/src/range.ts" start=8 end=2 priority=3}"#,
            ),
            Path::new("/repo"),
        );

//! Tracks when Forestx-owned transcript scrollback must be repaired after terminal resize.
//!
//! Terminal scrollback is not a retained widget tree: once Forestx writes wrapped lines into the
//! terminal, the terminal owns those rows. Width resize reflow treats the in-memory transcript cells
//! as the source of truth, clears Forestx-owned history, and re-emits the cells at the current width.
...
/// Forestx has already rebuilt scrollback. Keeping those widths distinct lets the next draw request a

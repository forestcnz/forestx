...
//! `forestx-cli`: it returns the diff for tracked changes as well as any
...
use forestx_git_utils::FsmonitorOverride;
use forestx_git_utils::FsmonitorProbeRunner;
use forestx_git_utils::detect_fsmonitor_override;
...
impl FsmonitorProbeRunner for WorkspaceFsmonitorProbeRunner<'_> {
    async fn run_probe(&mut self, args: &[&str]) -> Option<Vec<u8>> {
        let argv = ["git", "-c", forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG]
            .into_iter()
...
async fn run_git_command(
    ...
    let argv = [
        "git",
        "-c",
        forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG,
        "-c",
...
mod tests {
    ...
    fn git_probe_command(args: &[&str]) -> Vec<String> {
        ["git", "-c", forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG]
            .into_iter()
            .chain(args.iter().copied())
            .map(str::to_string)
            .collect()

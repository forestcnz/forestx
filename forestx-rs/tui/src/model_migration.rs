...
pub(crate) fn migration_copy_for_models(
    ...
    let heading_text = Span::from(format!(
        "Forestx just got an upgrade. Introducing {target_display_name}."
    ))
    .bold();
...
impl ModelMigrationScreen {
    ...
    fn render_menu(&self, column: &mut ColumnRenderable) {
        column.push(Line::from(""));
        column.push(
            Paragraph::new("Choose how you'd like Forestx to proceed.")
                .wrap(Wrap { trim: false })
                .inset(Insets::tlbr(
                    /*top*/ 0, /*left*/ 2, /*bottom*/ 0, /*right*/ 0,
                )),
...
mod tests {
    ...
    fn prompt_snapshot() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5.1-forestx-mini",
                "gpt-5.1-forestx-max",
                /*model_link*/ None,
                Some(
                    "Upgrade to gpt-5.2-forestx for the latest and greatest agentic coding model."
                        .to_string(),
                ),
                /*migration_markdown*/ None,
                "gpt-5.1-forestx-max".to_string(),
                Some("Forestx-optimized flagship for deep and fast reasoning.".to_string()),
                /*can_opt_out*/ true,
            ),
        );
    ...
    fn prompt_snapshot_gpt5_family() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5",
                "gpt-5.1",
                Some("https://www.forestx.com/models/gpt-5.1".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1".to_string(),
    ...
    #[test]
    fn prompt_snapshot_gpt5_forestx() {
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-forestx",
                "gpt-5.1-forestx-max",
                Some("https://www.forestx.com/models/gpt-5.1-forestx-max".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1-forestx-max".to_string(),
                Some("Forestx-optimized flagship for deep and fast reasoning.".to_string()),
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_forestx", terminal.backend());
    }

    #[test]
    fn prompt_snapshot_gpt5_forestx_mini() {
        let backend = VT100Backend::new(/*width*/ 60, /*height*/ 22);
        let mut terminal = Terminal::with_options(backend).expect("terminal");
        terminal.set_viewport_area(Rect::new(0, 0, 60, 22));
        ...
        let screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-5-forestx-mini",
                "gpt-5.1-forestx-mini",
                Some("https://www.forestx.com/models/gpt-5.1-forestx-mini".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-5.1-forestx-mini".to_string(),
                Some("Optimized for forestx. Cheaper, faster, but less capable.".to_string()),
                /*can_opt_out*/ false,
            ),
        );
        {
        ...
        terminal.flush().expect("flush");
        assert_snapshot!("model_migration_prompt_gpt5_forestx_mini", terminal.backend());
    }
    ...
    fn escape_key_accepts_prompt() {
        let mut screen = ModelMigrationScreen::new(
            ...
            migration_copy_for_models(
                "gpt-old",
                "gpt-new",
                Some("https://www.forestx.com/models/gpt-new".to_string()),
                /*migration_copy*/ None,
                /*migration_markdown*/ None,
                "gpt-new".to_string(),

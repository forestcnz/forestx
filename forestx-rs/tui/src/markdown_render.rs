...
use forestx_utils_string::normalize_markdown_hash_location_suffix;
use dirs::home_dir;
...
///
/// This accepts the path shapes Forestx emits today: `file://` URLs, absolute and relative paths,
...
mod tests {
    ...
    fn column_classification_token_heavy_for_local_path_lists() {
        ...
        let rows = vec![
            vec![make_cell(
                "forestx-rs/core/src/next_prompt_suggestion.rs:1, forestx-rs/core/src/next_prompt_suggestion_tests.rs:1",
            )],
            vec![make_cell(
                "forestx-rs/core/src/context/next_prompt_suggestion.rs:1, forestx-rs/core/src/context/contextual_user_message_tests.rs:1",
            )],
        ];
        let metrics = W::collect_table_column_metrics(&header, &rows, /*column_count*/ 1);
        assert_eq!(metrics[0].kind, TableColumnKind::TokenHeavy);

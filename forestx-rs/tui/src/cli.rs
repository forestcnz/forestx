...
use clap::Parser;
use forestx_utils_cli::ApprovalModeCliArg;
...
#[command(version)]
pub struct Cli {
    /// Process-only PSP routing selected by the parent Forestx CLI.
    #[clap(skip)]
    pub psp: bool,
    ...
    // Internal controls set by the top-level `forestx resume` subcommand.
    // These are not exposed as user flags on the base `forestx` command.
    #[clap(skip)]
    ...
    /// top-level `forestx resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `forestx fork` subcommand.
    // These are not exposed as user flags on the base `forestx` command.
    #[clap(skip)]
    ...
    /// top-level `forestx fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]

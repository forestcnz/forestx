...
const GOAL_FILE_PREFIX: &str = "Read the Forestx goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    forestx_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, forestx_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, forestx_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let forestx_home = forestx_home
        .context("App server did not report $FORESTX_HOME; cannot materialize goal files")?;

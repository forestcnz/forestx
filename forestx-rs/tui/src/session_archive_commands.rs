//! Shared implementation for `forestx archive`, `forestx delete`, and `forestx unarchive`.
//!
...
pub async fn run_session_archive_command(
    ...
    let forestx_home = find_forestx_home().wrap_err("failed to find Forestx home")?;
    let mut app_server =
        start_app_server_for_archive_command(options, forestx_home.to_path_buf()).await?;
...
async fn run_session_archive_action_with_app_server(
    ...
    let resolved = resolve_session_target(app_server, forestx_home, action, target).await?;
...
async fn resolve_session_target(
    ...
    for &archived in archived_values {
        if let Some(thread) =
            lookup_session_by_exact_name(app_server, forestx_home, target, archived).await?
        {
            ...
            loop {
                    ...
                    .filter(|thread| thread.name.as_deref() == Some(name))
                {
                    if !uses_remote_workspace {
                        ...
                        let expected_root = forestx_home.join(if archived {
                            forestx_rollout::ARCHIVED_SESSIONS_SUBDIR
                        } else {
                            forestx_rollout::SESSIONS_SUBDIR
                        });
                        let valid_rollout = if let Some(path) = thread.path.as_deref()
                            && let Some(path) = forestx_rollout::existing_rollout_path(path).await
                            && path.starts_with(expected_root)
                            && let Ok(session_meta) =
                                forestx_rollout::read_session_meta_line(path.as_path()).await
                        {
...
async fn start_app_server_for_archive_command(
    ...
    let default_daemon = if explicit_remote_endpoint.is_none() && reuse_implicit_local_daemon {
        super::maybe_probe_default_daemon_socket(forestx_home.as_path()).await
    } else {
        None
    };

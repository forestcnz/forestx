...
use forestx_app_server_client::AppServerEvent;
use forestx_app_server_client::AppServerPath;
use forestx_app_server_client::AppServerRequestHandle;
use forestx_app_server_client::TypedRequestError;
...
use forestx_app_server_protocol::ConfigBatchWriteParams;
use forestx_app_server_protocol::ConfigRequirementsReadResponse;
use forestx_app_server_protocol::ConfigWriteResponse;
use forestx_app_server_protocol::ExternalAgentConfigDetectParams;
use forestx_app_server_protocol::ExternalAgentConfigDetectResponse;
use forestx_app_server_protocol::ExternalAgentConfigImportParams;
use forestx_app_server_protocol::ExternalAgentConfigImportResponse;
use forestx_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use forestx_app_server_protocol::Model as ApiModel;
use forestx_app_server_protocol::ModelListParams;
use forestx_app_server_protocol::ModelListResponse;
use forestx_app_server_protocol::NewThreadModelDefaults;
use forestx_app_server_protocol::RateLimitSnapshot;
use forestx_app_server_protocol::RequestId;
use forestx_app_server_protocol::ReviewDelivery;
use forestx_app_server_protocol::ReviewStartParams;
use forestx_app_server_protocol::ReviewStartResponse;
use forestx_app_server_protocol::ReviewTarget;
use forestx_app_server_protocol::SessionSource;
...
use forestx_protocol::approvals::GuardianAssessmentEvent;
...
use forestx_protocol::openai_models::ModelAvailabilityNux;
use forestx_protocol::openai_models::ModelPreset;
use forestx_protocol::openai_models::ModelServiceTier;
use forestx_protocol::openai_models::ModelUpgrade;
use forestx_protocol::openai_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<forestx_protocol::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) fn forestx_home_path(
        &self,
        local_forestx_home: &AbsolutePathBuf,
    ) -> Option<AppServerPath> {
        self.client.forestx_home(local_forestx_home)
    }
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: forestx_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<forestx_protocol::openai_models::ReasoningEffort>,
        summary: Option<forestx_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<forestx_protocol::config_types::CollaborationMode>,
        personality: Option<forestx_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<forestx_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<forestx_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<forestx_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(forestx_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(forestx_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(forestx_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(forestx_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<forestx_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &forestx_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<forestx_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        forestx_message_history::HistoryConfig::new(config.forestx_home.clone(), &config.history);
    let (log_id, entry_count) = forestx_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(forestx_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn app_server_rate_limit_snapshots_deduplicates_top_level_limit_from_map() {
        let response = GetAccountRateLimitsResponse {
            rate_limits: rate_limit_snapshot("forestx"),
            rate_limits_by_limit_id: Some(HashMap::from([
                ("forestx".to_string(), rate_limit_snapshot("forestx")),
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(forestx_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn persisted_resume_does_not_forward_implicit_service_tier() -> Result<()> {
        let forestx_home = tempfile::tempdir().expect("tempdir");
        let mut config = build_config(&forestx_home).await;
    ...
    {
        let forestx_home = tempfile::tempdir().expect("tempdir");
        let config = build_config(&forestx_home).await;
    ...
    async fn side_fork_uses_one_request_for_long_paginated_history() -> Result<()> {
        ...
        let source_path =
            app_test_support::rollout_path(forestx_home.path(), filename_ts, source_id.as_str());
    ...
    async fn side_fork_excludes_turns_without_clearing_regular_ephemeral_fork() -> Result<()> {
        ...
        assert!(matches!(
            ...
            [forestx_app_server_protocol::ThreadItem::UserMessage { content, .. }]
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: forestx_app_server_protocol::Thread {
                ...
                source: forestx_app_server_protocol::SessionSource::Cli,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        forestx_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![forestx_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        forestx_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: forestx_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: forestx_app_server_protocol::ApprovalsReviewer::User,
        ...
        let embedded_config = ConfigBuilder::default()
            .forestx_home(temp_dir.path().join("embedded-forestx-home"))
            .harness_overrides(ConfigOverrides {
                default_permissions: Some(BUILT_IN_PERMISSION_PROFILE_WORKSPACE.to_string()),
                ..ConfigOverrides::default()
            })
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &forestx_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        forestx_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        forestx_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            forestx_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(forestx_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(forestx_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let business_prolite = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(forestx_protocol::account::PlanType::SelfServeBusinessProLite),
        );
        assert!(matches!(
            business_prolite,
            Some(StatusAccountDisplay::ChatGpt {

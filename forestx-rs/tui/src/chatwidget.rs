//! The main Forestx TUI chat surface.
//!
...
use crate::version::FORESTX_CLI_VERSION;
...
use forestx_app_server_protocol::ForestxErrorInfo as AppServerForestxErrorInfo;
...
use forestx_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use forestx_app_server_protocol::CreditsSnapshot;
...
use forestx_app_server_protocol::ModelVerification as AppServerModelVerification;
use forestx_app_server_protocol::RateLimitReachedType;
...
use forestx_app_server_protocol::SkillMetadata;
use forestx_app_server_protocol::SkillsListResponse;
use forestx_app_server_protocol::ThreadGoal as AppThreadGoal;
use forestx_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use forestx_config::ConstraintResult;
...
#[cfg(test)]
use forestx_git_utils::CommitLogEntry;
use forestx_git_utils::current_branch_name;
use forestx_git_utils::get_git_repo_root;
use forestx_git_utils::local_git_branches;
use forestx_git_utils::recent_commits;
use forestx_otel::RuntimeMetricsSummary;
use forestx_otel::SessionTelemetry;
use forestx_plugin::PluginCapabilitySummary;
use forestx_protocol::ThreadId;
use forestx_protocol::account::PlanType;
use forestx_protocol::approvals::GuardianAssessmentAction;
use forestx_protocol::approvals::GuardianAssessmentDecisionSource;
use forestx_protocol::approvals::GuardianAssessmentEvent;
use forestx_protocol::approvals::GuardianAssessmentStatus;
...
use forestx_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use forestx_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use forestx_utils_cli::resume_hint;
...
const MEMORIES_DOC_URL: &str = "https://developers.openai.com/forestx/memories";
...
use forestx_file_search::FileMatch;
...
use forestx_utils_approval_presets::ApprovalPreset;
use forestx_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_forestx_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: forestx_feedback::ForestxFeedback,
...
/// and by sending requests back to forestx-core.
///
...
pub(crate) struct ChatWidget {
    ...
    forestx_op_target: ForestxOpTarget,
    ...
    forestx_rate_limit_reached_type: Option<RateLimitReachedType>,
    forestx_spend_control_reached: Option<bool>,
    ...
    pub(crate) pet_http_client: forestx_http_client::RouteAwareClientPool,
...
fn request_permissions_from_params(
    params: forestx_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            forestx_windows_sandbox::current_log_file_path_for_forestx_home(&self.config.forestx_home)
                .is_file();
    ...
    /// Forward a command directly to forestx.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.forestx_op_target {
            ForestxOpTarget::Direct(forestx_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = forestx_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            ForestxOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::ForestxOp(op));
            }
        }
        true
    }

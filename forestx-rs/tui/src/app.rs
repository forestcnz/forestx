...
use forestx_ansi_escape::ansi_escape_line;
...
use forestx_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use forestx_app_server_protocol::SkillErrorInfo;
...
use forestx_app_server_protocol::WriteStatus;
...
use forestx_models_manager::model_presets::HIDE_GPT_5_1_FORESTX_MAX_MIGRATION_PROMPT_CONFIG;
use forestx_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use forestx_protocol::permissions::FileSystemSandboxKind;
...
use forestx_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        forestx_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&forestx_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&forestx_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[forestx_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&forestx_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<forestx_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    forestx_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.forestx_error_info,
        Some(AppServerForestxErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `forestx unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_forestx_backend_auth: self.chat_widget.has_forestx_backend_auth(),
    ...
    pub async fn run(
        ...
        let has_forestx_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            forestx_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("forestx.status_line", /*inc*/ 1, &[]);
        }
...
See the Forestx keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.forestx_home.clone();

...
pub(crate) async fn load_session_transcript(
    ...
    forestx_home: Option<&std::path::Path>,
...
pub(crate) fn thread_items_to_transcript_cells(
    ...
    let inline_visualization_context = forestx_home.and_then(|forestx_home| {
        thread_id.and_then(|thread_id| InlineVisualizationContext::new(forestx_home, thread_id))
    });
        ...
        match item {
            ThreadItem::UserMessage {
                ...
                let item = UserMessageItem {
                    ...
                    content: content
                        .into_iter()
                        .map(forestx_app_server_protocol::UserInput::into_core)
                        .collect(),
                };

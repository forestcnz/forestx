...
async fn run_git_command(
    ...
    argv.push(forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG.to_string());
...
mod tests {
    ...
    async fn open_pull_request_uses_current_branch_view_first() {
        let runner = FakeRunner::new(vec![response(
            ...
            /*exit_code*/ 0,
            r#"{"number":20252,"url":"https://github.com/openai/forestx/pull/20252","state":"OPEN"}"#,
        )]);
    ...
    async fn open_pull_request_falls_back_to_parent_repo_commit_lookup() {
        let runner = FakeRunner::new(vec![
            ...
            response(
                ...
                /*exit_code*/ 0,
                r#"{"nameWithOwner":"fcoury/forestx","parent":{"nameWithOwner":"openai/forestx"}}"#,
            ),
            response(
                &[
                    "gh",
                    ...
                    "-H",
                    "Accept: application/vnd.github+json",
                    "repos/openai/forestx/commits/head-sha/pulls",
                ],
                /*exit_code*/ 0,
                r#"[{"number":20252,"html_url":"https://github.com/openai/forestx/pull/20252","state":"open"}]"#,
            ),
        ]);
    ...
    fn status_line_pr_view_parser_requires_open_pr() {
        ...
        assert_eq!(
            pull_request_from_view_output(
                r#"{"number":20252,"url":"https://github.com/openai/forestx/pull/20252","state":"MERGED"}"#
            ),
            None
        );
    }
    ...
    fn status_line_pr_fallback_searches_parent_repo_first() {
        assert_eq!(
            ...
            Some(vec!["openai/forestx".to_string(), "fcoury/forestx".to_string()])
        );
    }
    ...
    fn response(argv: &[&str], exit_code: i32, stdout: &str) -> FakeResponse {
        ...
        if argv.first().map(String::as_str) == Some("git") {
            argv.splice(
                ...
                [
                    "-c".to_string(),
                    forestx_git_utils::SAFE_BARE_REPOSITORY_CONFIG.to_string(),
                ],
            );
        }
        FakeResponse {

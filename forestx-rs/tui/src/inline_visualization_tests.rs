...
fn context_with_fragment(fragment: &str) -> (TempDir, InlineVisualizationContext) {
    let forestx_home = tempfile::tempdir().expect("temp forestx home");
    let thread_id = ThreadId::new();
    let context = InlineVisualizationContext::new(forestx_home.path(), thread_id)
    ...
    (forestx_home, context)
}

#[test]
fn granted_visualization_root_overrides_thread_id_derived_root() {
    let forestx_home = tempfile::tempdir().expect("temp forestx home");
    let granted_context = InlineVisualizationContext::new(forestx_home.path(), ThreadId::new())
...
fn rewrites_complete_directive_to_trusted_static_file_placeholder() {
    let (_forestx_home, context) = context_with_fragment("<div>chart</div>");

    let rewritten = rewrite_inline_visualizations(
        "Before\n::forestx-inline-vis{file=\"chart.html\"}\nAfter",
        Some(&context),
    );
...
fn unavailable_artifact_has_explicit_fallback() {
    let forestx_home = tempfile::tempdir().expect("temp forestx home");
    let context = InlineVisualizationContext::new(forestx_home.path(), ThreadId::new())
        .expect("UUIDv7 thread id should provide a timestamp");

    assert_eq!(
        rewrite_inline_visualizations("::forestx-inline-vis{file=\"missing.html\"}", Some(&context),)
            .markdown,
    ...
    for file in ["../chart.html", "chart.svg"] {
        assert_eq!(
            rewrite_inline_visualizations(
                &format!("::forestx-inline-vis{{file=\"{file}\"}}"),
                Some(&context),
            )
            .markdown,
            "_Visualization unavailable on this device._"
...
fn rejects_oversized_fragment() {
    ...
    assert_eq!(
        rewrite_inline_visualizations("::forestx-inline-vis{file=\"chart.html\"}", Some(&context),)
            .markdown,
        "_Visualization unavailable on this device._"
    );
}
...
fn viewer_materializes_sandboxed_static_document() {
    let (_forestx_home, context) = context_with_fragment(
    ...
    assert_eq!(
        ...
        Some(std::ffi::OsStr::new(".forestx-viewers"))
    );
...
fn viewer_reuses_path_and_refreshes_static_document() {
    let (_forestx_home, context) = context_with_fragment("<div>first</div>");
...
fn transcript_overlay_remeasures_visualization_when_artifact_becomes_available() {
    ...
    let cell = AgentMarkdownCell::new_with_inline_visualizations(
        "::forestx-inline-vis{file=\"chart.html\"}".to_string(),
        Path::new("/workspace"),
        Some(context.clone()),
    );
...
fn visualization_link_uses_the_artifact_name() {
    ...
    let rewritten = rewrite_inline_visualizations(
        "::forestx-inline-vis{file=\"compound-interest-explorer.html\"}",
        Some(&context),
    );
...
fn user_markdown_keeps_directive_literal() {
    let rendered =
        crate::markdown_render::render_markdown_text("::forestx-inline-vis{file=\"chart.html\"}");
    let text = rendered
        .lines
        .iter()
    ...
    assert_eq!(text, "::forestx-inline-vis{file=\"chart.html\"}");
}

...
//! | `FORESTX_HOME` | `OnceLock<Option<PathBuf>>` | Root for custom `.tmTheme` discovery |
//!
...
static FORESTX_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
/// Set the user-configured syntax theme override and forestx home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `forestx_home` in
...
pub(crate) fn set_theme_override(
    ...
    let warning = validate_theme_name(name.as_deref(), forestx_home.as_deref());
    let override_set_ok = THEME_OVERRIDE.set(name.clone()).is_ok();
    let forestx_home_set_ok = FORESTX_HOME.set(forestx_home.clone()).is_ok();
    ...
    if !override_set_ok || !forestx_home_set_ok {
...
pub(crate) fn validate_theme_name(name: Option<&str>, forestx_home: Option<&Path>) -> Option<String> {
    let name = name?;
    let custom_theme_path_display = forestx_home
        .map(|home| custom_theme_path(name, home).display().to_string())
        .unwrap_or_else(|| format!("$FORESTX_HOME/themes/{name}.tmTheme"));
...
fn custom_theme_path(name: &str, forestx_home: &Path) -> PathBuf {
    forestx_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{forestx_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, forestx_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, forestx_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, forestx_home: Option<&Path>) -> Theme {
    ...
    if let Some(name) = name {
        ...
        // 2. Try loading {FORESTX_HOME}/themes/{name}.tmTheme from disk.
        if let Some(home) = forestx_home
...
fn build_default_theme() -> Theme {
    ...
    let forestx_home = FORESTX_HOME
        .get()
        .and_then(|forestx_home| forestx_home.as_deref());
    resolve_theme_with_override(name, forestx_home)
}
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = FORESTX_HOME.get()
...
pub(crate) fn resolve_theme_by_name(name: &str, forestx_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// `.tmTheme` file under `{FORESTX_HOME}/themes/`.
...
/// found in `{forestx_home}/themes/`.
pub(crate) fn list_available_themes(forestx_home: Option<&Path>) -> Vec<ThemeEntry> {
...
<key>background</key><string>{deleted_background}</string>
    ...
    fn unique_foreground_colors_for_theme(theme_name: &str) -> Vec<String> {
        let theme = resolve_theme_by_name(theme_name, /*forestx_home*/ None)
    ...
    fn bundled_theme_can_provide_diff_scope_backgrounds() {
        let theme = resolve_theme_by_name("github", /*forestx_home*/ None)
    ...
    fn validate_theme_name_none_for_bundled() {
        ...
        assert!(validate_theme_name(Some("dracula"), /*forestx_home*/ None).is_none());
        assert!(validate_theme_name(Some("nord"), Some(Path::new("/nonexistent"))).is_none());
    }

    #[test]
    fn validate_theme_name_none_when_no_override() {
        assert!(validate_theme_name(/*name*/ None, /*forestx_home*/ None).is_none());
    }

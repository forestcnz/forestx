...
pub(super) const ANALYTICS_CAPTURE_ENV_VAR: &str = "FORESTX_ANALYTICS_EVENTS_CAPTURE_FILE";
const TEST_USER_CONFIG_ENV_VAR: &str = "FORESTX_APP_SERVER_TEST_USER_CONFIG_FILE";
...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!("forestx-plugin-analytics-{}.jsonl", process::id()))
    });
    ...
    let mut client = ForestxClient::spawn_stdio_with_env(forestx_bin, &overrides, &child_environment)?;
...
fn run_plugin_turn(client: &mut ForestxClient, expected: &ExpectedPlugin) -> Result<String> {
...
fn wait_for_plugin_usage(
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "forestx_plugin_used"
...
fn plugin_installed(client: &mut ForestxClient) -> Result<PluginInstalledResponse> {
...
pub(super) fn wait_until_capture_is_ready(path: &Path) -> Result<()> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            bail!(
                "analytics capture did not become ready at {}; use a debug Forestx binary",
                path.display()
            );
        }
...
fn wait_for_turn_analytics(path: &Path, turn_id: &str) -> Result<Vec<Value>> {
    ...
    loop {
        ...
        if events.iter().any(|event| {
            event["event_type"] == "forestx_turn_event"
...
fn validate_plugin_events(events: Vec<Value>, expected: &ExpectedPlugin) -> Result<Vec<Value>> {
    ...
    for event_type in required_event_types() {
        ...
        if event_type == "forestx_plugin_used" {
...
fn required_event_types() -> [&'static str; 3] {
    [
        "forestx_plugin_disabled",
        "forestx_plugin_enabled",
        "forestx_plugin_used",
    ]
}
...
fn validate_used_metadata(event: &Value) -> Result<()> {
    ...
    for field in [
        ...
        if params.get(field).is_none_or(Value::is_null) {
            bail!("forestx_plugin_used event has null or missing `{field}`");
        }
    }
...
impl TemporaryConfigFile {
    fn create() -> Result<Self> {
        let path = std::env::temp_dir().join(format!(
            "forestx-plugin-analytics-config-{}.toml",
            process::id()
        ));
        fs::write(&path, "")

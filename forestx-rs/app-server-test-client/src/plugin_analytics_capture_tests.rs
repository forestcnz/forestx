...
fn reads_and_validates_remote_plugin_mutation_events() {
    ...
    let installed = mutation_event("forestx_plugin_installed");
    let uninstalled = mutation_event("forestx_plugin_uninstalled");
    let unrelated = json!({
...
fn rejects_missing_capability_metadata() {
    let mut installed = mutation_event("forestx_plugin_installed");
...
fn unique_capture_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "forestx-plugin-analytics-capture-{name}-{}-{nonce}.jsonl",
        process::id()
    ))
}

...
use forestx_app_server_protocol::JSONRPCMessage;
use forestx_app_server_protocol::JSONRPCNotification;
use forestx_app_server_protocol::JSONRPCRequest;
...
use forestx_app_server_protocol::SandboxPolicy;
...
use forestx_otel::OtelProvider;
use forestx_otel::current_span_w3c_trace_context;
use forestx_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "forestx-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $FORESTX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Forestx app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Forestx", version, about = "Bootstrap Forestx app-server", long_about = None)]
struct Cli {
    /// Path to the `forestx` CLI binary. When set, requests use stdio by
    /// spawning `forestx app-server` as a child process.
    #[arg(long, env = "FORESTX_BIN", global = true)]
    forestx_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--forestx-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "FORESTX_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `forestx` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `forestx app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/forestx-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `forestx app-server --listen`.
    ...
    /// Send a user message through the Forestx app-server.
    SendMessage {
        /// User message to send to Forestx.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a Forestx-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the Forestx app-server.
    GetAccountRateLimits,
    /// List the available models from the Forestx app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Forestx app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "FORESTX_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let forestx_bin = forestx_bin.unwrap_or_else(|| PathBuf::from("forestx"));
            serve(&forestx_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(forestx_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(forestx_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url = resolve_shared_websocket_url(forestx_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --forestx-bin and does not support --url");
            }
            let forestx_bin = forestx_bin.context("plugin-analytics-smoke requires --forestx-bin")?;
            plugin_analytics_smoke::run(&forestx_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --forestx-bin and does not support --url"
                );
            }
            let forestx_bin =
                forestx_bin.context("plugin-analytics-mutation-smoke requires --forestx-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --forestx-bin and does not support --url");
            }
            let forestx_bin = forestx_bin.context("plugin-remote-uninstall requires --forestx-bin")?;
...
fn resolve_endpoint(forestx_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if forestx_bin.is_some() && url.is_some() {
        bail!("--forestx-bin and --url are mutually exclusive");
    }
    if let Some(forestx_bin) = forestx_bin {
        return Ok(Endpoint::SpawnForestx(forestx_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if forestx_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --forestx-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(forestx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(forestx_bin);
        if let Some(forestx_bin_parent) = forestx_bin.parent() {
            let mut path = OsString::from(forestx_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", forestx_bin.display()))?;
...
fn serve(forestx_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/forestx-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,forestx_=trace {}",
        shell_quote(&forestx_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started forestx app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnForestx(forestx_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: forestx_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut ForestxClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = ForestxClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = ForestxClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (forestx_bin, url) {
        (Some(_), Some(_)) => bail!("--forestx-bin and --url are mutually exclusive"),
        (Some(forestx_bin), None) => {
            let server = BackgroundAppServer::spawn(&forestx_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve forestx-app-server-test-client binary path")?;
...
impl ForestxClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnForestx(forestx_bin) => Self::spawn_stdio(forestx_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(forestx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(forestx_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let forestx_bin_display = forestx_bin.display();
        let mut cmd = Command::new(forestx_bin);
        if let Some(forestx_bin_parent) = forestx_bin.parent() {
            let mut path = OsString::from(forestx_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut forestx_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{forestx_bin_display}` app-server"))?;

        let stdin = forestx_app_server
            .stdin
            .take()
            .context("forestx app-server stdin unavailable")?;
        let stdout = forestx_app_server
            .stdout
            .take()
            .context("forestx app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: forestx_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `forestx-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "forestx-toy-app-server".to_string(),
                    title: Some("Forestx Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: forestx_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                forestx_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: forestx_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to forestx app-server")?;
                    return Ok(());
                }
                bail!("forestx app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from forestx app-server")?;
                if bytes == 0 {
                    bail!("forestx app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = forestx_core::otel_init::build_provider(
            &config,
...
impl Drop for ForestxClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[forestx app-server exited: {status}]");
            return;
        }

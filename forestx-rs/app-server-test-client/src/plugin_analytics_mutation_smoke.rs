...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "forestx-plugin-analytics-mutation-{}.jsonl",
            process::id()
        ))
    });
    prepare_capture_file(&capture_path)?;
    let mut client = spawn_client(forestx_bin, config_overrides, &capture_path)?;
    ...
    match (sequence_result, restoration) {
        ...
        (sequence_result, RestorationStatus::Dirty(cleanup_err)) => {
            ...
            print_dirty_recovery(forestx_bin, config_overrides, remote_plugin_id, &cleanup_err);
            Err(cleanup_err)
        }
        (sequence_result, RestorationStatus::Unknown(cleanup_err)) => {
            ...
            print_recovery_command(forestx_bin, config_overrides, remote_plugin_id);
            Err(cleanup_err)
        }
    }
}
...
pub(super) fn run_cleanup(
    ...
    let mut client = ForestxClient::spawn_stdio(forestx_bin, &overrides)?;
    client.initialize()?;
    ...
    match restore_uninstalled_state(&mut client, remote_plugin_id) {
        ...
        RestorationStatus::Dirty(err) => {
            print_dirty_recovery(forestx_bin, config_overrides, remote_plugin_id, &err);
            Err(err)
        }
...
fn spawn_client(
    ...
    ForestxClient::spawn_stdio_with_env(forestx_bin, &overrides, &environment)
}
...
fn install_remote_plugin(client: &mut ForestxClient, plugin: &RemotePluginExpectation) -> Result<()> {
    let request_id = client.request_id();
...
fn uninstall_remote_plugin(client: &mut ForestxClient, remote_plugin_id: &str) -> Result<()> {
    let request_id = client.request_id();
...
fn print_recovery_command(forestx_bin: &Path, config_overrides: &[String], remote_plugin_id: &str) {
    let test_client = std::env::current_exe()
        .map(|path| path.display().to_string())
        .unwrap_or_else(|_| "forestx-app-server-test-client".to_string());

...
Quickstart for running and hitting `forestx app-server`.

## Quickstart

Run from `<reporoot>/forestx-rs`.

```bash
# 1) Build debug forestx binary
cargo build -p forestx-cli --bin forestx

# 2) Start websocket app-server in background
cargo run -p forestx-app-server-test-client -- \
  --forestx-bin ./target/debug/forestx \
  serve --listen ws://127.0.0.1:4222 --kill

# 3) Call app-server (defaults to ws://127.0.0.1:4222)
cargo run -p forestx-app-server-test-client -- model-list
```

`send-message` and `send-message-v2` handle `request_user_input` server requests interactively.
When Forestx asks a question, choose a numbered option (or `o` for a free-form answer when offered)
and the client will send the response and continue streaming the same turn.

## Testing Forestx-managed Amazon Bedrock login
...
credential and sets `model_provider = "amazon-bedrock"`, so use an isolated `FORESTX_HOME` when
testing.

```bash
export FORESTX_HOME="$(mktemp -d)"
printf 'cli_auth_credentials_store = "file"\n' > "$FORESTX_HOME/config.toml"
...
The test client redacts `apiKey` from its outbound request log. After login, start a fresh Forestx
process with the same `FORESTX_HOME` to verify that it uses the persisted managed credential.

## Testing logout

`test-logout` initializes the app-server, sends an `account/logout` request, and waits for the
resulting `account/updated` notification. It uses the active `FORESTX_HOME`, so point it at an
...
active Forestx profile must be authenticated. On a fresh local cache, the command
retries ephemeral turns while the installed remote bundle finishes syncing.

```bash
# Build a debug Forestx binary; analytics capture is unavailable in release builds.
cargo build -p forestx-cli --bin forestx
...
The command validates one `forestx_plugin_disabled`, `forestx_plugin_enabled`, and
`forestx_plugin_used` event with the expected local and remote plugin identities
...
installed, installs it, validates `forestx_plugin_installed`, uninstalls it, and
validates `forestx_plugin_uninstalled`, and verifies that the original
uninstalled state was restored.

The mutation events include the local Forestx ID in `plugin_id` and the backend ID
in `remote_plugin_id`.
...
Cleanup does not require analytics capture or a debug Forestx binary. When the
...
```bash
cargo run -p forestx-app-server-test-client -- watch
```

## Testing Thread Rejoin Behavior

Build and start an app server using commands above. The app-server log is written to `/tmp/forestx-app-server-test-client/app-server.log`
...
```bash
cargo run -p forestx-app-server-test-client -- send-message-v2 "seed thread for rejoin test"
cargo run -p forestx-app-server-test-client -- thread-list --limit 5
```
...
```bash
cargo run --bin forestx-app-server-test-client -- \
...
```bash
cargo run --bin forestx-app-server-test-client -- thread-resume <THREAD_ID>
```

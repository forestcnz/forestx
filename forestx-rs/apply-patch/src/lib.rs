...
use forestx_utils_path_uri::PathUriParseError;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the Forestx executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `forestx-apply-patch` (to avoid forcing
/// `forestx-arg0` to depend on `forestx-core`), it remains part of the "forestx core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const FORESTX_CORE_APPLY_PATCH_ARG1: &str = "--forestx-run-as-apply-patch";

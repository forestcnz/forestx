...
impl McpProcess {
    pub async fn new(forestx_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(forestx_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = forestx_utils_cargo_bin::cargo_bin("forestx-mcp-server")
            .context("should find binary for forestx-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = forestx_login::default_client::originator().value;
    ...
    pub async fn send_forestx_tool_call(
        &mut self,
        params: ForestxToolCallParam,
    ) -> anyhow::Result<i64> {
        let forestx_tool_call_params = CallToolRequestParams::new("forestx").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(forestx_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "forestx/event" with params.msg.type == "task_complete".
    pub async fn read_stream_until_legacy_task_complete_notification(
        ...
        self.read_stream_until_forestx_event("task_complete").await
    }

    pub async fn read_stream_until_forestx_event(
        &mut self,
        event_type: &str,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        self.read_stream_until_forestx_event_matching(event_type, |_| true)
            .await
    }

    pub async fn read_stream_until_forestx_event_matching(
        &mut self,
        event_type: &str,
        predicate: impl Fn(&serde_json::Value) -> bool,
    ) -> anyhow::Result<JsonRpcNotification<CustomNotification>> {
        eprintln!("in read_stream_until_forestx_event({event_type})");

        loop {
            let message = self.read_jsonrpc_message().await?;
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = notification.notification.method == "forestx/event"
                        && notification
                            .notification
                            .params
                            .as_ref()
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `forestx-mcp-server` child process.
        //

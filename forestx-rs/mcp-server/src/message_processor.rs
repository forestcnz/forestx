...
use forestx_login::default_client::USER_AGENT_SUFFIX;
...
use crate::forestx_tool_config::ForestxToolCallParam;
use crate::forestx_tool_config::ForestxToolCallReplyParam;
use crate::forestx_tool_config::create_tool_for_forestx_tool_call_param;
use crate::forestx_tool_config::create_tool_for_forestx_tool_call_reply_param;
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        forestx_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.forestx_home.clone()),
        );
        let skill_providers = forestx_skills_extension::SkillProviders::new()
            .with_host_provider(Arc::new(forestx_skills_extension::HostSkillProvider::new()));
        forestx_skills_extension::install_with_providers_and_metrics(
            &mut extensions,
            skill_providers,
            forestx_otel::global(),
            |config: &Config| forestx_skills_extension::SkillsExtensionConfig {
        ...
        let thread_manager = Arc::new(ThreadManager::new(
            ...
            forestx_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            forestx_core::thread_store_from_config(config.as_ref(), state_db.clone()),
    ...
    async fn handle_initialize(
        ...
        let server_info =
            Implementation::new("forestx-mcp-server", env!("CARGO_PKG_VERSION")).with_title("Forestx");

        // Preserve Forestx's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_forestx_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult::with_all_items(vec![
            create_tool_for_forestx_tool_call_param(),
            create_tool_for_forestx_tool_call_reply_param(),
        ]);
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "forestx" => self.handle_tool_call_forestx(id, arguments).await,
            "forestx-reply" => {
                self.handle_tool_call_forestx_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_forestx(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<ForestxToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                            format!("Failed to load Forestx configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result);
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for Forestx tool: {e}"),
                    )]);
                    self.outgoing.send_response(id, result);
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for forestx tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result);
                return;
            }
        ...
        // Spawn an async task to handle the Forestx session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Forestx session and stream events back to the client.
            crate::forestx_tool_runner::run_forestx_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_forestx_session_reply(
        &self,
        ...
        // parse arguments
        let forestx_tool_call_reply_param: ForestxToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<ForestxToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Forestx tool call reply parameters: {e}");
                    let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                        format!("Failed to parse configuration for Forestx tool: {e}"),
                    )]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for forestx-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::ContentBlock::text(
                    "Missing arguments for forestx-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match forestx_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let forestx = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::forestx_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = forestx_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let active_turns = Arc::clone(&active_turns);

            async move {
                crate::forestx_tool_runner::run_forestx_tool_session_reply(
                    thread_id,
                    forestx,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the Forestx thread from the server.
        let forestx_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Forestx.
        if let Err(e) = forestx_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: forestx_protocol::protocol::Op::Interrupt,
            ...
            .await
        {
            tracing::error!("Failed to submit interrupt to Forestx: {e}");
            return;
        }

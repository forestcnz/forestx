//! Asynchronous worker that executes a **Forestx** tool-call inside a spawned
...
use forestx_core::config::Config as ForestxConfig;
...
/// To adhere to MCP `tools/call` response format, include the Forestx
...
/// Run a complete Forestx session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_forestx_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: ForestxConfig,
        ...
        .await
    {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![ContentBlock::text(format!(
                "Failed to start Forestx session: {e}"
            ))]);
    ...
    // Preserve the legacy event ID for initial `forestx` calls. Each call starts
    ...
    run_forestx_tool_session_inner(thread_id, thread, outgoing, id, active_turns).await;
}

pub async fn run_forestx_tool_session_reply(
    thread_id: ThreadId,
    ...
    run_forestx_tool_session_inner(thread_id, thread, outgoing, request_id, active_turns).await;
}

async fn run_forestx_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<ForestxThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(forestx_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("Forestx runtime error: {e}"),
                    Some(true),
                );
                active_turns.finish(&request_id, || {
                    outgoing.send_response(request_id.clone(), result);

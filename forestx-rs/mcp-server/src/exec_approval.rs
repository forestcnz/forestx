...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the forestx tool call.
    ...
    pub forestx_command: Vec<String>,
    pub forestx_cwd: PathBuf,
    pub forestx_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    forestx_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Forestx to run `{escaped_command}` in `{cwd}`?",
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, forestx).await;
        });
    }
}
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `forestx`.

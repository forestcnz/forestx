...
fn disabled_thread_context_accepts_trace_calls_without_writing() -> anyhow::Result<()> {
    ...
    thread_trace.record_forestx_turn_event("turn-1", &EventMsg::ShutdownComplete);
    ...
    let token_usage: Option<forestx_protocol::protocol::TokenUsage> = None;
...
fn compaction_contexts_share_identity_across_models() -> anyhow::Result<()> {
    ...
    thread_trace.record_forestx_turn_started("turn-1");

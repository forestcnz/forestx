...
impl TraceWriter {
    ...
    pub fn append_with_context(
        ...
        let event = RawTraceEvent {
            ...
            forestx_turn_id: context.forestx_turn_id,
            payload,
        };
        inner.next_seq += 1;
...
mod tests {
    ...
    fn writer_records_payload_refs_and_replays_rollout_status() -> anyhow::Result<()> {
        ...
        writer.append(RawTraceEventPayload::ForestxTurnEnded {
            forestx_turn_id: "turn-1".to_string(),
        ...
        assert_eq!(rollout.forestx_turns["turn-1"].thread_id, "thread-root");
        assert_eq!(
            rollout.forestx_turns["turn-1"].execution.status,

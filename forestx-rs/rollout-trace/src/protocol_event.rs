//! Mapping from Forestx protocol events into raw rollout-trace events.
//!
...
//! `forestx-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `forestx-rollout-trace`.
//!
...
use forestx_protocol::protocol::PatchApplyStatus;
use forestx_protocol::protocol::SubAgentActivityEvent;
use forestx_protocol::protocol::TurnAbortReason;
use serde::Serialize;
use std::time::Duration;

use crate::AgentThreadId;
use crate::ForestxTurnId;
use crate::ExecutionStatus;
use crate::RawTraceEventPayload;

pub(crate) struct ForestxTurnTraceEvent {
    pub context_turn_id: ForestxTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn forestx_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<ForestxTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let forestx_turn_id = event.turn_id.clone();
            Some(ForestxTurnTraceEvent {
                context_turn_id: forestx_turn_id.clone(),
                payload: RawTraceEventPayload::ForestxTurnStarted {
                    forestx_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(ForestxTurnTraceEvent {
                context_turn_id: forestx_turn_id.clone(),
                payload: RawTraceEventPayload::ForestxTurnEnded {
                    forestx_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a forestx_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a forestx_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a forestx_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a forestx_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a forestx_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a forestx_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a forestx_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a forestx_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [forestx_protocol::parse_command::ParsedCommand],

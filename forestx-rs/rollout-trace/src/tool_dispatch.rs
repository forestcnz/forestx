...
use forestx_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical Forestx tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub forestx_turn_id: ForestxTurnId,
...
/// Raw invocation payload for the canonical Forestx tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            forestx_turn_id: invocation.forestx_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == forestx_code_mode::PUBLIC_TOOL_NAME
}

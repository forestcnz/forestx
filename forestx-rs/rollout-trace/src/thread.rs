...
use crate::protocol_event::forestx_turn_trace_event;
...
pub const FORESTX_ROLLOUT_TRACE_ROOT_ENV: &str = "FORESTX_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `FORESTX_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the Forestx
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(FORESTX_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed Forestx turn lifecycle events from protocol lifecycle events.
    pub fn record_forestx_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            forestx_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what Forestx did while executing that boundary.
    pub fn record_tool_call_event(&self, forestx_turn_id: impl Into<ForestxTurnId>, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        ...
        context.append_with_context_best_effort(forestx_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_forestx_turn_id: impl Into<ForestxTurnId>,
    ...
    pub fn record_forestx_turn_started(&self, forestx_turn_id: impl Into<ForestxTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let forestx_turn_id = forestx_turn_id.into();
        context.append_with_context_best_effort(
            forestx_turn_id.clone(),
            RawTraceEventPayload::ForestxTurnStarted {
                forestx_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    /// Builds reusable inference trace context for one Forestx turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        forestx_turn_id: ForestxTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            forestx_turn_id: Some(forestx_turn_id),
        };

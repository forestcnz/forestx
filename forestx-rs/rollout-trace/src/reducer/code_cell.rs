...
//! - runtime work starts as soon as Forestx dispatches the tool.
//!
...
pub(super) struct PendingCodeCellStart {
    ...
    pub(super) forestx_turn_id: Option<String>,
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        let Some(forestx_turn_id) = forestx_turn_id else {
            bail!(
                "code cell start {} did not include a Forestx turn id",
                started.code_cell_id
            );
        };
        self.validate_code_cell_turn(&thread_id, &forestx_turn_id)?;
    ...
    pub(super) fn terminate_running_code_cells_for_turn_end(
        ...
        let code_cell_ids: Vec<_> = self
            ...
            .values()
            .filter(|cell| {
                cell.forestx_turn_id == forestx_turn_id
    ...
    pub(super) fn code_cell_event_thread_id(
        ...
        self.rollout
            ...
            .with_context(|| {
                format!(
                    "{event_name} {runtime_cell_id} referenced unknown Forestx turn {forestx_turn_id}"
                )
            })
    }
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, forestx_turn_id: &str) -> Result<()> {
        ...
        let Some(turn) = self.rollout.forestx_turns.get(forestx_turn_id) else {
            bail!("code cell start referenced unknown Forestx turn {forestx_turn_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but Forestx turn {forestx_turn_id} belongs \
                 to {}",
                turn.thread_id
            );
        }

...
impl TraceReducer {
    ...
    /// Starts a Forestx turn inside an existing thread.
    pub(super) fn start_forestx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.forestx_turns.contains_key(&forestx_turn_id) {
            bail!("duplicate forestx turn start for {forestx_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.forestx_turns.insert(
    ...
    /// Marks a Forestx turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_forestx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.forestx_turns.get(&forestx_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "forestx turn end for {forestx_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.forestx_turns.get_mut(&forestx_turn_id) else {
            bail!("forestx turn end referenced unknown turn {forestx_turn_id}");
        };

...
impl TraceReducer {
    ...
    pub(super) fn start_tool_call(
        ...
        let thread_id = self.tool_thread_id(thread_id, forestx_turn_id.as_deref())?;
        self.validate_tool_turn(&thread_id, forestx_turn_id.as_deref())?;
        ...
        self.rollout.tool_calls.insert(
            ...
            ToolCall {
                ...
                thread_id,
                started_by_forestx_turn_id: forestx_turn_id,
    ...
    fn tool_thread_id(
        &self,
        thread_id: Option<String>,
        forestx_turn_id: Option<&str>,
    ) -> Result<String> {
        if let Some(thread_id) = thread_id {
            return Ok(thread_id);
        }
        let Some(forestx_turn_id) = forestx_turn_id else {
            bail!("tool call start did not include thread or Forestx turn context");
        };
        self.rollout
            .forestx_turns
            .get(forestx_turn_id)
            ...
            .with_context(|| {
                format!("tool call start referenced unknown Forestx turn {forestx_turn_id}")
            })
    }

    fn validate_tool_turn(&self, thread_id: &str, forestx_turn_id: Option<&str>) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("tool call start referenced unknown thread {thread_id}");
        }
        if let Some(forestx_turn_id) = forestx_turn_id {
            let Some(turn) = self.rollout.forestx_turns.get(forestx_turn_id) else {
                bail!("tool call start referenced unknown Forestx turn {forestx_turn_id}");
            };
            if turn.thread_id != thread_id {
                bail!(
                    "tool call start used thread {thread_id}, but Forestx turn {forestx_turn_id} \
                     belongs to {}",
                    turn.thread_id
                );
            }

...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self.rollout.forestx_turns.get(&started.forestx_turn_id) else {
            bail!(
                "compaction request {} referenced unknown forestx turn {}",
                started.compaction_request_id,
                started.forestx_turn_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but forestx turn {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.forestx_turn_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                forestx_turn_id: started.forestx_turn_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.forestx_turns.get(&forestx_turn_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown forestx turn {forestx_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but forestx turn {forestx_turn_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) forestx_turn_id: String,

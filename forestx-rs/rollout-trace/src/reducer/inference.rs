...
impl TraceReducer {
    ...
    /// If a call is still `Running`, Forestx stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.forestx_turn_id == forestx_turn_id

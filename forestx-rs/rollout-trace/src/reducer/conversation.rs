...
impl TraceReducer {
    ...
    pub(super) fn reduce_inference_request(
        ...
        // history, not the pre-compaction prompt. Any repeated developer/context prefix that Forestx
    ...
    pub(super) fn reduce_inference_response(
        ...
        let Some((thread_id, forestx_turn_id)) = self
            .rollout
            .inference_calls
            .get(inference_call_id)
            .map(|inference| (inference.thread_id.clone(), inference.forestx_turn_id.clone()))
        else {
        ...
        let response_item_ids = self.reconcile_conversation_items(
            ...
            ReconcileItems {
                thread_id: &thread_id,
                forestx_turn_id: &forestx_turn_id,
                wall_time_unix_ms,
        ...
        for (offset, item) in items.into_iter().enumerate() {
            ...
            let item_id = if let Some(previous_item_id) = previous_snapshot.get(index) {
                if self.item_matches(previous_item_id, &item) {
                    ...
                    self.find_matching_snapshot_item(&previous_snapshot, &item_ids, &item)
                        .unwrap_or_else(|| {
                            self.create_conversation_item(
                                context.thread_id,
                                Some(context.forestx_turn_id.to_string()),
                    ...
                    let forestx_turn_id = context.forestx_turn_id;
                    let thread_id = context.thread_id;
                    bail!(
                        "model conversation mismatch while reducing turn {forestx_turn_id} for \
    ...
    pub(super) fn reduce_compaction_checkpoint(
        ...
        let marker_item_id = self.create_conversation_item(
            thread_id,
            Some(forestx_turn_id.to_string()),
            wall_time_unix_ms,

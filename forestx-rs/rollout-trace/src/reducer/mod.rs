        ...
        match event.payload {
            ...
            RawTraceEventPayload::CodeCellStarted {
                ...
                let thread_id = self.code_cell_event_thread_id(
                    event.thread_id,
                    event.forestx_turn_id.as_deref(),
                    &runtime_cell_id,
                    "code cell start",
                )?;
                ...
                self.start_or_queue_code_cell(PendingCodeCellStart {
                    ...
                    thread_id,
                    forestx_turn_id: event.forestx_turn_id,
            ...
            RawTraceEventPayload::CompactionInstalled {
                ...
                let Some(forestx_turn_id) = event.forestx_turn_id else {
                    bail!(
                        "compaction installed event {compaction_id} did not include a forestx turn id"
                    );
                };

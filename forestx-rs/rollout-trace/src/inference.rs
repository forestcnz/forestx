...
const INFERENCE_CALL_ID_HEADER: &str = "x-forestx-inference-call-id";
...
///
/// A Forestx turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one Forestx turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        forestx_turn_id: ForestxTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                forestx_turn_id: attempt.context.forestx_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps forestx-core focused on
    ...
    /// Records a provider stream that Forestx intentionally stopped consuming.
    ///
...
/// Forestx received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        forestx_turn_id: Some(context.forestx_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.forestx_turn_id, "turn-1");

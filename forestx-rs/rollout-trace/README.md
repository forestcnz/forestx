...
> **Privacy:** Rollout tracing is not telemetry. Forestx does **not** upload or
> report these traces; it writes local bundles only when
> `FORESTX_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a Forestx session. It records raw runtime evidence into a local bundle on
...
Hot-path Forestx code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how Forestx got
there.
...
```mermaid
flowchart TD
    subgraph Runtime["forestx-core runtime"]
...
one from `FORESTX_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
Forestx session fail just because diagnostic recording failed. Core emits raw
...
- `state.json`: optional reducer output written by `forestx debug trace-reduce`.

`trace_id` identifies this diagnostic artifact. `rollout_id` identifies the
Forestx rollout/session being observed. Keeping those separate lets us reason about
the stored trace without confusing it with the product-level session identity.

To reduce a bundle:

```bash
forestx debug trace-reduce <trace-bundle>
```

By default this writes `<trace-bundle>/state.json`. Rust callers can also call
`forestx_rollout_trace::replay_bundle` directly.

...
use forestx_utils_template::Template;
use rand::RngCore;
...
// Keep in sync with the Forestx CLI Hydra redirect URI allow-list.
...
pub struct ServerOptions {
    ...
    pub forestx_streamlined_login: bool,
    ...
    match path.as_str() {
            ...
            .await
            {
                Ok(tokens) => {
                    ...
                    let redirect = compose_success_url(
                        ...
                        &tokens.id_token,
                        &tokens.access_token,
                        opts.forestx_streamlined_login,
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Forestx failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "forestx_streamlined_login" && value == "true");
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        ("forestx_cli_simplified_flow".to_string(), "true".to_string()),
...
fn redact_sensitive_error_url(
    mut err: forestx_http_client::HttpError,
) -> forestx_http_client::HttpError {
...
pub(crate) async fn persist_tokens_async(
    ...
    let forestx_home = forestx_home.to_path_buf();
...
/// Returns true when the OAuth callback represents a missing Forestx entitlement.
fn is_missing_forestx_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_forestx_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_forestx_entitlement_error(error_code, error_description) {
        return "Forestx is not enabled for your workspace. Contact your workspace administrator to request access to Forestx.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_forestx_entitlement_error(code, error_description) {
            (
                "You do not have access to Forestx".to_string(),
                "This account is not currently authorized to use Forestx in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Forestx.".to_string(),
                "Contact your workspace administrator to get access to Forestx, then return to Forestx and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Forestx to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_forestx_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_forestx_entitlement");
        assert!(is_missing_forestx_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to Forestx"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_forestx_entitlement"));
    }
}

//! Default Forestx HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! `HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`forestx_login::default_client`] from other crates in this
//! workspace.

use forestx_http_client::BuildRouteAwareHttpClientError;
...
/// See https://github.com/openai/forestx/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Forestx. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "forestx_cli_rs";
pub const FORESTX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "FORESTX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-forestx-residency";

pub use forestx_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(FORESTX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(FORESTX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "forestx-tui"
        || originator_value == "forestx_vscode"
        || originator_value.starts_with("Forestx ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "forestx_atlas" || originator_value == "forestx_chatgpt_desktop"
}

pub fn get_forestx_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Forestx user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Forestx user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Forestx originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// Forestx's route-aware system/PAC resolution.
...
/// Builds the default Forestx HTTP client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the Forestx
...
/// Builds the default Forestx HTTP client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without Forestx default headers.
...
/// Builds the default Forestx HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_forestx_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("FORESTX_SANDBOX").as_deref() == Ok("seatbelt")
}

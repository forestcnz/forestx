...
use forestx_protocol::auth::KnownPlan as InternalKnownPlan;
...
async fn refresh_without_id_token() {
    let forestx_home = tempdir().unwrap();
...
async fn stored_agent_identity_jwt_keeps_auth_json_unchanged() -> anyhow::Result<()> {
    ...
    let ForestxAuth::AgentIdentity(agent_identity_auth) = auth else {
        panic!("stored JWT should load as agent identity auth");
    };
    assert_eq!(agent_identity_auth.run_task_id(), "task-id");
    let storage = FileAuthStorage::new(forestx_home.path().to_path_buf());
    let auth = storage
        .try_read_auth_json(&get_auth_file(forestx_home.path()))
        .expect("auth.json should parse");
    assert_eq!(
        auth.agent_identity,
        Some(AgentIdentityStorage::Jwt(agent_identity))
...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("FORESTX_AUTHAPI_BASE_URL", &server.uri());
...
async fn chatgpt_auth_task_registration_retry_exhaustion_is_fallback_eligible() -> anyhow::Result<()>
{
    ...
    let storage = FileAuthStorage::new(forestx_home.path().to_path_buf());
    let auth_path = get_auth_file(forestx_home.path());
...
async fn agent_identity_jwt_task_registration_retry_exhaustion_is_strict() -> anyhow::Result<()> {
    ...
    let err = ForestxAuth::from_agent_identity_jwt_with_authapi_base_url(
        &agent_identity,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),
...
async fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = ForestxAuth::from_auth_dot_json(
        forestx_home.path(),
...
#[derive(Clone)]
struct StaticExternalAuth(ForestxAuth);
...
impl ExternalAuth for StaticExternalAuth {
    ...
    fn refresh(&self, _context: ExternalAuthRefreshContext) -> ExternalAuthFuture<'_, ForestxAuth> {
        Box::pin(async { Ok(self.0.clone()) })
    }
}
...
async fn external_auth_provider_can_install_headers() {
    ...
    let auth = ForestxAuth::Headers(AuthHeaders::new(headers));
    ...
    assert!(
        manager
            .auth_cached()
            .is_some_and(|auth| auth.uses_forestx_backend())
    );
    assert!(
        !manager
            .auth_cached()
...
fn write_auth_file(params: AuthFileParams, forestx_home: &Path) -> std::io::Result<String> {
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(FORESTX_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(FORESTX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    ...
    let ForestxAuth::AgentIdentity(agent_identity) = auth else {
    ...
    assert!(
        !get_auth_file(forestx_home.path()).exists(),
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("FORESTX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(FORESTX_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(FORESTX_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("FORESTX_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(FORESTX_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            forestx_home.path().to_path_buf(),
            /*enable_forestx_api_key_env*/ false,
...
#[tokio::test]
#[serial(forestx_auth_env)]
async fn load_auth_keeps_forestx_api_key_env_precedence() {
    let forestx_home = tempdir().unwrap();
    let record = agent_identity_record(WORKSPACE_ID_ALLOWED);
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(FORESTX_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(FORESTX_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(FORESTX_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let forestx_home = tempdir().unwrap();
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material =
        forestx_agent_identity::generate_agent_key_material().expect("generate agent key material");
...
fn fake_agent_identity_jwt_with_plan_type(
    ...
    let payload = json!({
        "iss": "https://chatgpt.com/forestx-backend/agent-identity",
        "aud": "forestx-app-server",
...
async fn assert_agent_identity_plan_alias(
    ...
    let authapi_base_url = server.uri();
    let chatgpt_base_url = format!("{authapi_base_url}/backend-api");
    let auth = ForestxAuth::from_agent_identity_jwt_with_authapi_base_url(
        &jwt,
        Some(&chatgpt_base_url),
        &authapi_base_url,
        &crate::test_support::transport_default_auth_route_config(),

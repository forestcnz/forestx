...
#[test]
fn test_get_forestx_user_agent() {
    let user_agent = get_forestx_user_agent();
...
fn is_first_party_originator_matches_known_values() {
    ...
    assert_eq!(is_first_party_originator("forestx-tui"), true);
    assert_eq!(is_first_party_originator("forestx_vscode"), true);
    assert_eq!(is_first_party_originator("Forestx Something Else"), true);
    assert_eq!(is_first_party_originator("forestx_cli"), false);
    assert_eq!(is_first_party_originator("Other"), false);
}

#[test]
fn is_first_party_chat_originator_matches_known_values() {
    assert_eq!(is_first_party_chat_originator("forestx_atlas"), true);
    assert_eq!(
        is_first_party_chat_originator("forestx_chatgpt_desktop"),
        true
    );
    assert_eq!(is_first_party_chat_originator(DEFAULT_ORIGINATOR), false);
    assert_eq!(is_first_party_chat_originator("forestx_vscode"), false);
}
...
async fn test_create_client_sets_default_headers() {
    ...
    // User-Agent matches the computed Forestx UA for that originator
    let expected_ua = get_forestx_user_agent();
...
fn test_invalid_suffix_is_sanitized() {
    let prefix = "forestx_cli_rs/0.0.0";
    let suffix = "bad\rsuffix";

    assert_eq!(
        sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
        "forestx_cli_rs/0.0.0 (bad_suffix)"
    );
}

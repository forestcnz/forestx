...
use forestx_agent_identity::AgentIdentityJwtClaims;
use forestx_agent_identity::decode_agent_identity_jwt;
use forestx_config::types::AuthCredentialsStoreMode;
pub use forestx_config::types::AuthKeyringBackendKind;
use forestx_keyring_store::DefaultKeyringStore;
use forestx_keyring_store::KeyringStore;
use forestx_protocol::account::PlanType as AccountPlanType;
use forestx_protocol::auth::AuthMode;
use forestx_secrets::LocalSecretsNamespace;
use forestx_secrets::SecretName;
use forestx_secrets::SecretScope;
use forestx_secrets::SecretsBackendKind;
use forestx_secrets::SecretsManager;
use once_cell::sync::Lazy;

/// Expected structure for $FORESTX_HOME/auth.json.
...
pub(super) fn get_auth_file(forestx_home: &Path) -> PathBuf {
    forestx_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(forestx_home: &Path) -> std::io::Result<bool> {
    let auth_file = get_auth_file(forestx_home);
...
impl FileAuthStorage {
    pub(super) fn new(forestx_home: PathBuf) -> Self {
        Self { forestx_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `FORESTX_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.forestx_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.forestx_home)
    }
}

static FORESTX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("FORESTX_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("FORESTX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "Forestx Auth";

// turns forestx_home path into a stable, short key string
fn compute_store_key(forestx_home: &Path) -> std::io::Result<String> {
    let canonical = forestx_home
        .canonicalize()
        .unwrap_or_else(|_| forestx_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(forestx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            forestx_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.forestx_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.forestx_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.forestx_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(forestx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(forestx_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            forestx_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::ForestxAuth,
        );
        Self {
            forestx_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &FORESTX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &FORESTX_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &FORESTX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(forestx_home)),
        }
    }
}
...
// A global in-memory store for mapping forestx_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(forestx_home: PathBuf) -> Self {
        Self { forestx_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(forestx_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(forestx_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(forestx_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(forestx_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(forestx_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(forestx_home, keyring_store))
        }
    }
}

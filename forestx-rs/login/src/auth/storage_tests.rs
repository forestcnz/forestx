...
use forestx_secrets::compute_keyring_account;
use pretty_assertions::assert_eq;
use serde_json::json;
use tempfile::tempdir;

use forestx_keyring_store::tests::MockKeyringStore;
...
async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
    ...
    let file = get_auth_file(forestx_home.path());
    storage
        .save(&auth_dot_json)
        .context("failed to save auth file")?;
...
async fn file_storage_loads_empty_agent_identity_email_as_none() -> anyhow::Result<()> {
    let forestx_home = tempdir()?;
    let storage = FileAuthStorage::new(forestx_home.path().to_path_buf());
    let auth_file = get_auth_file(forestx_home.path());
    std::fs::write(
        &auth_file,
...
fn encrypted_auth_file(forestx_home: &Path) -> PathBuf {
    forestx_home.join("secrets").join("forestx_auth.age")
}
...
fn secrets_keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_secrets_backend_with_auth(&mock_keyring, forestx_home.path(), &expected)?;
...
fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
    let forestx_home = PathBuf::from("~/.forestx");

    let key = compute_store_key(forestx_home.as_path())?;
...
fn direct_keyring_auth_storage_delete_removes_keyring_and_file() -> anyhow::Result<()> {
    ...
    assert!(
        mock_keyring
            .saved_value(&compute_store_key(forestx_home.path())?)
            .is_none(),
...
fn secrets_keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
    ...
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, forestx_home.path(), &auth)?;
    Ok(())
}
...
fn auto_auth_storage_load_prefers_keyring_value() -> anyhow::Result<()> {
    ...
    seed_secrets_backend_with_auth(&mock_keyring, forestx_home.path(), &keyring_auth)?;
...
fn auto_auth_storage_load_falls_back_when_keyring_errors() -> anyhow::Result<()> {
    ...
    let key = compute_keyring_account(forestx_home.path());

    let encrypted = auth_with_prefix("encrypted");
    seed_secrets_backend_with_auth(&mock_keyring, forestx_home.path(), &encrypted)?;
...
fn auto_auth_storage_save_prefers_keyring() -> anyhow::Result<()> {
    ...
    assert_keyring_saved_auth_and_removed_fallback(&mock_keyring, forestx_home.path(), &expected)?;
    Ok(())
}

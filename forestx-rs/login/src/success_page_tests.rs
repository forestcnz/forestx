...
fn compose_success_url_uses_local_page_by_default() {
    ...
    assert_eq!(
        url.query_pairs()
            .find(|(key, _)| key == "forestx_streamlined_login"),
        None
    );
}
    ...
    assert_eq!(
        compose_success_url(
            ...
            &LoginSuccessPage::Hosted {
                url: Url::parse(FORESTX_OPEN_APP_URL).expect("open app URL should parse"),
                app_brand: LoginSuccessPageBrand::Chatgpt,
            },
        ),
        LoginSuccessRedirect::Hosted(
            "https://chatgpt.com/forestx/open-app?source=login&app_brand=chatgpt".to_string()
        )
    );
}
...
fn compose_success_url_keeps_setup_on_local_page() {
    ...
    let LoginSuccessRedirect::Local(url) = compose_success_url(
        ...
        &LoginSuccessPage::Hosted {
            url: Url::parse(FORESTX_OPEN_APP_URL).expect("open app URL should parse"),
            app_brand: LoginSuccessPageBrand::Forestx,
        },
    ) else {
        panic!("expected local success redirect");
    };

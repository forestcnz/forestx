    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this Forestx server. Use the browser login or verify the server URL.",
            ));
        }
...
fn device_code_prompt(verification_url: &str, code: &str) -> String {
    ...
    format!(
        "\nWelcome to Forestx [v{ANSI_GRAY}{version}{ANSI_RESET}]\n{ANSI_GRAY}OpenAI's command-line coding agent{ANSI_RESET}\n\
\nFollow these steps to sign in with ChatGPT using device code authorization:\n\
\n1. Open this link in your browser and sign in to your account\n   {ANSI_BLUE}{verification_url}{ANSI_RESET}\n\
\n2. Enter this one-time code {ANSI_GRAY}(expires in 15 minutes){ANSI_RESET}\n   {ANSI_BLUE}{code}{ANSI_RESET}\n\
\n{ANSI_GRAY}Continue only if you started this login in Forestx. If a website or another person gave you this code, cancel.{ANSI_RESET}\n",
    )
}
...
pub async fn request_device_code(opts: &ServerOptions) -> std::io::Result<DeviceCode> {
    ...
    Ok(DeviceCode {
        verification_url: format!("{base_url}/forestx/device"),

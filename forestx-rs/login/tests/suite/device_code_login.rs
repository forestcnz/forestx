...
use forestx_login::auth::load_auth_dot_json;
...
async fn device_code_login_integration_succeeds() -> anyhow::Result<()> {
    ...
    let opts = server_opts(&forestx_home, issuer, AuthCredentialsStoreMode::File);
...
async fn device_code_login_rejects_workspace_mismatch() -> anyhow::Result<()> {
    ...
    let issuer = mock_server.uri();
    let mut opts = server_opts(&forestx_home, issuer, AuthCredentialsStoreMode::File);

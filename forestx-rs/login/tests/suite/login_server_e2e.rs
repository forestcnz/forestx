...
use forestx_login::LoginCallbackResult;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = forestx_home.clone();

    let opts = ServerOptions {
        forestx_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: forestx_login::test_support::transport_default_auth_route_config(),
        client_id: forestx_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = forestx_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/forestx/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_forestx_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let forestx_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = HttpClientBuilder::new().build_direct()?;
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_forestx_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Forestx"),
        "error body should clearly explain the Forestx access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_forestx_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Forestx to retry"),
    ...
    assert!(
        !body.contains("get access to Forestx"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not forestx"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        forestx_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_forestx_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        forestx_home: first_forestx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: forestx_login::test_support::transport_default_auth_route_config(),
        client_id: forestx_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_forestx_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        forestx_home: second_forestx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: forestx_login::test_support::transport_default_auth_route_config(),
        client_id: forestx_login::CLIENT_ID.to_string(),
        issuer,

...
```
forestx --config mcp_servers.example={command="forestx-stdio-to-uds",args=["/tmp/mcp.sock"]}
```
...
As a workaround, this crate uses `forestx-uds`, which provides a cross-platform async UDS API backed by https://crates.io/crates/uds_windows on Windows.

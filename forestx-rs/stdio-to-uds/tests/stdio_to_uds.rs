...
async fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let child_task = tokio::task::spawn_blocking(move || -> anyhow::Result<ChildOutput> {
        ...
        let mut child = Command::new(forestx_utils_cargo_bin::cargo_bin("forestx-stdio-to-uds")?)
            .arg(&socket_path)
            .stdin(Stdio::from(stdin))
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .context("failed to spawn forestx-stdio-to-uds")?;
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                anyhow::bail!(
                    "forestx-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                    server_events,
    ...
    assert!(
        ...
        "forestx-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",

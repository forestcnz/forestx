...
const CODE_MODE_HOST_EXECUTABLE_NAME: &str = if cfg!(windows) {
    "forestx-code-mode-host.exe"
} else {
    "forestx-code-mode-host"
};
const PACKAGE_METADATA_FILENAME: &str = "forestx-package.json";
const PATH_DIRNAME: &str = "forestx-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "forestx-resources";
...
pub struct InstallContext {
    ...
    pub package_layout: Option<ForestxPackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.forestx/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `forestx-resources/`, and `forestx-path/`.
    ...
    /// A Forestx binary launched through the npm-managed `forestx.js` shim.
    Npm,
    /// A Forestx binary launched through the bun-managed `forestx.js` shim.
    Bun,
    /// A Forestx binary launched through the pnpm-managed `forestx.js` shim.
    Pnpm,
    /// A Forestx binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Forestx binaries, custom
    /// internal launchers, and tests that execute Forestx from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let forestx_home = forestx_utils_home_dir::find_forestx_home().ok();
        Self::from_exe_with_forestx_home(
            is_macos,
            current_exe,
            method_override,
            forestx_home.as_deref(),
        )
    }

    fn from_exe_with_forestx_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        method_override: Option<InstallMethod>,
        forestx_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(ForestxPackageLayout::from_exe);
        let method = if let Some(method) = method_override {
            method
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, forestx_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let method_override = if std::env::var_os("FORESTX_MANAGED_BY_PNPM").is_some() {
                Some(InstallMethod::Pnpm)
            } else if std::env::var_os("FORESTX_MANAGED_BY_NPM").is_some() {
                Some(InstallMethod::Npm)
            } else if std::env::var_os("FORESTX_MANAGED_BY_BUN").is_some() {
    ...
    pub fn code_mode_host_program(&self) -> PathBuf {
        // prefer the one packed under forestx-resources
...
fn install_method_from_exe(
    ...
    if let Some(standalone_method) = standalone_install_method(exe_path, forestx_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_forestx_home = canonical_absolute_path(forestx_home?)?;
    ...
    let releases_root = canonical_forestx_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "forestx-test-helper";
    ...
    fn code_mode_host_program_prefers_package_resource_over_legacy_binary() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "forestx.exe" } else { "forestx" });
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let release_dir = forestx_home
            .path()
            .join("packages/standalone/releases/1.2.3-x86_64-unknown-linux-musl");
        let resources_dir = release_dir.join(RESOURCES_DIRNAME);
        fs::create_dir_all(&resources_dir)?;
        let exe_path = release_dir.join(if cfg!(windows) { "forestx.exe" } else { "forestx" });
        fs::write(&exe_path, "")?;
        ...
        let context = InstallContext::from_exe_with_forestx_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*method_override*/ None,
            /*forestx_home*/ Some(forestx_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let package_layout = ForestxPackageLayout {
    ...
    fn code_mode_host_program_accepts_symlinks_to_files() -> std::io::Result<()> {
        ...
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
        let exe_path = bin_dir.join("forestx");
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_forestx_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn bundled_file_lookups_ignore_directories() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "forestx.exe" } else { "forestx" });
        fs::write(&exe_path, "")?;
        let fallback_exe_path = package_dir.path().join("fallback-forestx");
    ...
    fn code_mode_host_program_is_next_to_the_executable_even_when_missing() {
        ...
        assert_eq!(
            context.code_mode_host_program_from_exe(Some(Path::new("/opt/forestx/bin/forestx"))),
            PathBuf::from("/opt/forestx/bin").join(CODE_MODE_HOST_EXECUTABLE_NAME)
        );
    }
    ...
    fn package_manager_method_overrides_take_precedence() {
        let pnpm_context = InstallContext::from_exe(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/forestx")),
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_forestx_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/forestx")),
            /*method_override*/ None,
            /*forestx_home*/ None,
        );
        assert_eq!(

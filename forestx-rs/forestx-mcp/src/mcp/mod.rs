...
pub const FORESTX_APPS_MCP_SERVER_NAME: &str = "forestx_apps";
const DEFAULT_FORESTX_APPS_MCP_PRODUCT_SKU: &str = "forestx";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const FORESTX_CONNECTORS_TOKEN_ENV_VAR: &str = "FORESTX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `forestx_core::config::Config`.
///
...
pub struct McpConfig {
    ...
    /// Forestx home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `forestx-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_forestx_apps_enabled(config: &McpConfig, auth: Option<&ForestxAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(ForestxAuth::uses_forestx_backend)
}
...
fn is_trusted_chatgpt_mcp_server(
    ...
    if url::Url::parse(CHATGPT_FORESTX_BASE_URL)
        .ok()
...
pub fn effective_mcp_servers_from_configured(
    ...
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_forestx_apps_enabled(config, auth) {
        servers.remove(FORESTX_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
pub async fn read_mcp_resource(
    ...
    forestx_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            tool_catalog_cache,
            forestx_apps_tools_cache_key: connector_runtime_context_key(auth),
...
fn forestx_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(FORESTX_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(FORESTX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(FORESTX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_forestx_apps_base_url(base_url: &str) -> String {
...
fn forestx_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_forestx_apps_base_url(base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/forestx") {
        base_url
    } else {
        format!("{base_url}/api/forestx")
    };
    format!("{base_url}/ps/mcp")
}

pub fn forestx_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        forestx_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    forestx_apps_mcp_server_config(chatgpt_base_url, apps_mcp_product_sku, originator)
}
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_FORESTX_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: forestx_apps_mcp_bearer_token_env_var(),

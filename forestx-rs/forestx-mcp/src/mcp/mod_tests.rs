...
pub(crate) fn test_mcp_config(forestx_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        config_layer_stack: forestx_config::ConfigLayerStack::default(),
        approvals_reviewer: forestx_config::types::ApprovalsReviewer::default(),
        ...
        connector_snapshot: forestx_connectors::ConnectorSnapshot::default(),
    }
}
...
fn tool_plugin_provenance_collects_app_and_mcp_sources() {
    ...
    catalog.register(McpServerRegistration::from_plugin(
        ...
        /*plugin_order*/ 0,
        forestx_apps_mcp_server_config(
            "https://alpha.example",
            /*apps_mcp_product_sku*/ None,
            /*originator*/ None,
        ),
...
#[test]
fn forestx_apps_mcp_url_for_base_url_uses_plugin_service_paths() {
    assert_eq!(
        forestx_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/ps/mcp"
    );
    assert_eq!(
        forestx_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/ps/mcp"
    );
    assert_eq!(
        forestx_apps_mcp_url_for_base_url("http://localhost:8080/api/forestx"),
        "http://localhost:8080/api/forestx/ps/mcp"
    );
    assert_eq!(
        forestx_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/forestx/ps/mcp"
    );
}

#[test]
fn forestx_apps_server_config_uses_plugin_service_path() {
    let config = forestx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for forestx apps"),
    };
...
#[test]
fn forestx_apps_server_config_forwards_thread_originator_header() {
    let config = forestx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "forestx".to_string()),
                ]))
            );
...
#[test]
fn forestx_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "forestx"), (Some("tpp"), "tpp")] {
        let config = forestx_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn forestx_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = forestx_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
fn effective_mcp_servers_preserve_chatgpt_auth_for_staging() {
    for url in [
        ...
        let server = forestx_apps_mcp_server_config(
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(forestx_home.path().to_path_buf());
    ...
    let forestx_apps = effective
        .get(FORESTX_APPS_MCP_SERVER_NAME)
        .expect("forestx apps server should exist");

    let sample = sample.config();
    let docs = docs.config();
    let forestx_apps = forestx_apps.config();

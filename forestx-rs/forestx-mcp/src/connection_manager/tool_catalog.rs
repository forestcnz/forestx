...
use crate::rmcp_client::FORESTX_APPS_REFRESH_DURATION_METRIC;
...
impl McpConnectionSet {
    ...
    pub async fn list_all_tools(&self) -> Vec<ToolInfo> {
        ...
        let server_results = join_all(self.servers.iter().map(|(server_name, view)| async move {
            ...
            let catalog_override = if server_name == FORESTX_APPS_MCP_SERVER_NAME {
                self.forestx_apps_tools_override.read().await.clone()
            } else {
                None
            };
            let server_tools = async {
                match catalog_override {
                    Some(tools) => {
                        ...
                        Some(prepare_forestx_apps_tools_for_model(
                            tools,
    ...
    pub(crate) async fn capture_binding_with_metadata(
        ...
        join_all(self.servers.iter().map(|(server_name, view)| async move {
                ...
                .load(Ordering::Acquire)
            {
                ...
                let must_wait_for_startup = required
                    ...
                    || (server_name == FORESTX_APPS_MCP_SERVER_NAME && !has_cached_tools);
        ...
        let server_results = join_all(self.servers.iter().map(|(server_name, view)| async move {
            ...
            let server_tools = if server_name == FORESTX_APPS_MCP_SERVER_NAME {
                prepare_forestx_apps_tools_for_model(server_tools, &self.tool_plugin_provenance)
            } else {
    ...
    /// Force-refresh Forestx Apps tools and publish one new exact catalog revision.
    #[expect(
        clippy::await_holding_invalid_type,
        reason = "catalog publication must remain serialized with captured tool calls"
    )]
    pub async fn hard_refresh_forestx_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let _refresh = self.forestx_apps_refresh_lock.lock().await;
        let refresh_start = Instant::now();
        let view = self
            .servers
            .get(FORESTX_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{FORESTX_APPS_MCP_SERVER_NAME}'"))?;
        ...
        let client_tools = list_tools_for_client_uncached(
            ...
            /*forestx_apps_refresh_trigger*/ "explicit",
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{FORESTX_APPS_MCP_SERVER_NAME}'")
        })?;

        let mut tool_catalog_revision = self.tool_catalog_revision.write().await;
        let tools = match (
            managed_client.forestx_apps_tools_cache_context.as_ref(),
            fetch_ticket,
        ) {
        ...
        *self.forestx_apps_tools_override.write().await = Some(client_tools);
        *tool_catalog_revision += 1;
        ...
        let tools = prepare_forestx_apps_tools_for_model(

...
use crate::rmcp_client::FORESTX_APPS_RECONNECT_INITIAL_BACKOFF;
use crate::rmcp_client::ForestxAppsStartupReconnect;
...
use forestx_config::AppToolApproval;
use forestx_config::Constrained;
use forestx_config::McpServerAuth;
use forestx_config::McpServerConfig;
use forestx_config::McpServerEnvVar;
use forestx_config::McpServerToolConfig;
use forestx_config::types::AuthKeyringBackendKind;
use forestx_config::types::OAuthCredentialsStoreMode;
use forestx_connectors::ConnectorRuntimeContext;
use forestx_connectors::ConnectorRuntimeContextKey;
use forestx_connectors::ConnectorRuntimeFetchSource;
use forestx_connectors::ConnectorRuntimeManager;
use forestx_exec_server_test_support::environment_manager_without_environments;
use forestx_login::AuthHeaders;
use forestx_login::ForestxAuth;
use forestx_protocol::ToolName;
use forestx_protocol::mcp::ClientMcpExtensions;
use forestx_protocol::mcp::McpServerInfo;
use forestx_protocol::models::PermissionProfile;
use forestx_protocol::protocol::GranularApprovalConfig;
use forestx_protocol::protocol::McpStartupFailureReason;
use forestx_rmcp_client::ElicitationResponse;
use forestx_rmcp_client::InProcessTransportFactory;
use forestx_rmcp_client::McpAuthState;
use forestx_rmcp_client::McpLoginRequirement;
use forestx_rmcp_client::RmcpClient;
...
impl McpConnectionSet {
    fn new_uninitialized(
        ...
        Self {
            ...
            forestx_apps_tools_override: RwLock::new(None),
            forestx_apps_refresh_lock: Mutex::new(()),
...
fn create_forestx_apps_tools_cache_context(
...
fn store_current_tools(cache_context: &ConnectorRuntimeContext<ToolInfo>, tools: Vec<ToolInfo>) {
    let _ = cache_context.publish_if_newest_accepted(
        ...
        &create_test_server_info("Forestx Apps"),
        tools,
    );
}
...
async fn legacy_tool_catalog_does_not_follow_pagination_cursor() -> anyhow::Result<()> {
    ...
    client
        .initialize(
            InitializeRequestParams::new(
                ...
                Implementation::new("forestx-test", "0.0.0-test"),
            )
    ...
    let tools = list_tools_for_client_uncached(
        "legacy",
        /*is_forestx_apps_mcp_server*/ false,
        "test",
        &client,
        Some(Duration::from_secs(5)),
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        forestx_apps_tools_cache_context: None,
    }
}
...
async fn create_test_manager_with_ready_apps_client(
    ...
    let tool = create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, tool_name);
    let client = Arc::new(
        RmcpClient::new_in_process_client(Arc::new(RefreshTestTransportFactory {
    ...
    let managed_client = ManagedClient {
        client,
        server_info: create_test_server_info("Forestx Apps"),
        tools: vec![tool],
        tool_timeout: Some(Duration::from_secs(5)),
        server_instructions: None,
        server_supports_sandbox_state_meta_capability: false,
        forestx_apps_tools_cache_context: Some(cache_context.clone()),
    };
    ...
    manager.insert_test_client(
        FORESTX_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
            client: futures::future::ready::<Result<ManagedClient, StartupOutcomeError>>(Ok(
                managed_client,
            ))
            .boxed()
            .shared(),
            is_forestx_apps_mcp_server: true,
            cached_server_info: Some(create_test_server_info("Forestx Apps")),
            forestx_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: None,
            cancel_token: CancellationToken::new(),
...
fn create_test_manager_with_failed_apps_startup(
    ...
    .boxed()
    .shared();
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
    ...
    manager.insert_test_client(
        ...
        AsyncManagedClient {
            ...
            tool_catalog_cache_context: None,
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: Some(Arc::new(ForestxAppsStartupReconnect::new(reconnect_factory))),
            cancel_token: CancellationToken::new(),
        },
    );
    manager
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        forestx_rmcp_client::Elicitation::Mcp(ElicitRequestParams::FormElicitationParams {
            meta: None,
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let (
        forestx_protocol::mcp::RequestId::String(request_a_id),
        forestx_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected Forestx-owned string request IDs");
    };
...
async fn cancelled_elicitation_is_removed_without_affecting_other_pending_requests() {
    ...
    let (
        forestx_protocol::mcp::RequestId::String(cancelled_id),
        forestx_protocol::mcp::RequestId::String(pending_id),
...
fn test_normalize_tools_selects_raw_server_name() {
    let mut tool = create_test_tool("forestx_apps", "search");
    tool.callable_namespace = "forestx_apps__calendar".to_string();
    ...
    assert_eq!(
        model_tool_names(&model_tools),
        HashSet::from([ToolName::namespaced("forestx_apps__calendar", "search")])
    );
}
...
#[test]
fn forestx_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_forestx_apps_tools_cache(
        FORESTX_APPS_MCP_SERVER_NAME,
...
#[tokio::test]
async fn forestx_apps_extension_does_not_share_host_owned_tools_cache() -> anyhow::Result<()> {
    let forestx_home = tempdir()?;
    let cache_key = ConnectorRuntimeContextKey::personal(
        /*account_id*/ None, /*chatgpt_user_id*/ None,
    );
    let forestx_apps_tools_cache = ConnectorRuntimeManager::<ToolInfo>::default();
    let cache_context =
        forestx_apps_tools_cache.context(forestx_home.path().to_path_buf(), cache_key.clone());
    store_current_tools(
        &cache_context,
    ...
    let mut config = crate::mcp::tests::test_mcp_config(forestx_home.path().to_path_buf());
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            forestx_apps_tools_cache,
            tool_catalog_cache: McpToolCatalogCache::default(),
            forestx_apps_tools_cache_key: cache_key,
            client_mcp_extensions: ClientMcpExtensions::default(),
            auth: None,
            forestx_apps_auth_manager: None,
    ...
    let client = manager.test_client(FORESTX_APPS_MCP_SERVER_NAME);
    assert!(
        client.forestx_apps_tools_cache_context.is_none(),
...
#[tokio::test]
async fn list_all_tools_uses_shared_forestx_apps_cache_while_client_is_pending() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__forestx_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, FORESTX_APPS_MCP_SERVER_NAME);
...
async fn capture_binding_uses_the_ready_clients_own_tools() {
    ...
    let mut ready_client = create_test_managed_client(vec![
        create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "client_local_tool"),
        create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "client_local_blocked"),
    ])
    .await;
    ...
    ready_client.forestx_apps_tools_cache_context = Some(cache_context.clone());
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionSet::new_uninitialized(
        &approval_policy,
    ...
    manager
        .servers
        .get_mut(FORESTX_APPS_MCP_SERVER_NAME)
        .expect("test server exists")
    ...
    assert!(
        step.prepare_call(FORESTX_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        step.prepare_call(FORESTX_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    assert!(
        step.prepare_call(FORESTX_APPS_MCP_SERVER_NAME, "client_local_blocked")
            .is_none()
    );
}
...
-> anyhow::Result<()> {
    ...
    let cache_context_a = shared_cache.context(forestx_home.path().to_path_buf(), cache_key.clone());
    let cache_context_b = shared_cache.context(forestx_home.path().to_path_buf(), cache_key);
    ...
    let refresh_a = tokio::spawn(async move {
        manager_a_for_refresh
            .hard_refresh_forestx_apps_tools_cache()
            .await
    });
    list_started.notified().await;
    let tools_b = manager_b.hard_refresh_forestx_apps_tools_cache().await?;
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<forestx_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    let server_info = create_test_server_info("Forestx Apps");
    manager.insert_test_client(
        FORESTX_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
            client: pending_client,
    ...
    assert_eq!(
        server_infos.get(FORESTX_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn capture_binding_exposes_cached_tools_before_startup() {
    ...
    let mut cached_tool = create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "shared_cached_tool");
    ...
    assert!(
        cached_binding
            .prepare_call(FORESTX_APPS_MCP_SERVER_NAME, "shared_cached_tool")
            .is_none()
    );
    ...
    let startup = tokio::spawn(async move {
        manager_for_startup
            .wait_for_server_startup(FORESTX_APPS_MCP_SERVER_NAME)
            .await
    });
...
async fn capture_binding_resolves_concurrently_and_rechecks_cached_clients() {
    ...
    apps_client.is_forestx_apps_mcp_server = true;
    apps_client.forestx_apps_tools_cache_context = Some(cache_context);
    let first_client =
    ...
    manager.insert_test_client(FORESTX_APPS_MCP_SERVER_NAME, apps_client);
    ...
    tokio::time::timeout(Duration::from_secs(1), apps_started)
        .await
        .expect("Forestx Apps startup should begin")
        .expect("signal Forestx Apps startup");
    ...
    release_apps.send(()).expect("release Forestx Apps startup");
    assert!(startup.await.expect("Forestx Apps startup task"));
    ...
    assert!(
        binding
            .prepare_call(FORESTX_APPS_MCP_SERVER_NAME, "client_local_tool")
            .is_some()
    );
    assert!(
        binding
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_forestx_apps_cache_is_empty() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_forestx_apps_cache_when_client_startup_fails() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_forestx_apps_startup_and_reuses_client() {
    ...
    let step = capture_binding(&manager).await;
    let prepared = step
        .prepare_call(FORESTX_APPS_MCP_SERVER_NAME, "drive_search")
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(FORESTX_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_forestx_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        FORESTX_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::forestx_apps_mcp_server_config(
        "https://docs.example",
...
fn hosted_actor_credentials_are_only_available_to_host_owned_mcp_servers() {
    let bootstrap_auth = ForestxAuth::create_dummy_chatgpt_auth_for_testing();
    let mut actor_headers = Default::default();
    forestx_model_provider::auth_provider_from_auth(&bootstrap_auth)
    ...
    let hosted_auth = ForestxAuth::Headers(AuthHeaders::new(actor_headers));
    let provider = forestx_model_provider::auth_provider_from_auth(&hosted_auth);
    let mut local_config = crate::forestx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    ...
    let local_provider =
        chatgpt_auth_provider_for_server(&local_server, Some(Arc::clone(&provider)))
            .expect("host-owned Forestx Apps must retain hosted authentication");
    assert_eq!(
        local_provider
            .to_auth_headers()
...
async fn executor_owned_chatgpt_mcp_accepts_only_safe_explicit_authorization() -> anyhow::Result<()>
{
    ...
    let hosted_auth = ForestxAuth::Headers(AuthHeaders::new(actor_headers));
    let runtime_config = crate::mcp::tests::test_mcp_config(forestx_home.path().to_path_buf());
    let cases = [
        ...
        (
            ...
            None,
            Some(("aUtHoRiZaTiOn", "FORESTX_TEST_HOSTED_SECRET")),
            false,
        ),
        (
            "environment bearer",
            Some(("Authorization", "Bearer executor-secret")),
            Some("FORESTX_TEST_HOSTED_SECRET"),
            None,
            false,
        ),
        (
    ...
    for (case, static_header, bearer_env_var, env_header, allows_executor_auth) in cases {
        ...
        assert!(
            chatgpt_auth_provider_for_server(
                remote_server,
                Some(forestx_model_provider::auth_provider_from_auth(&hosted_auth)),
            )
            .is_none(),
        ...
        let connection_identity = |keyring_backend_kind| {
            McpServerConnectionIdentity::new(
                ...
                Some(&hosted_auth),
                /*forestx_apps_cache_identity*/ None,
                ElicitationCapability::default(),
                ClientMcpExtensions::default(),
            )
        };
        ...
        let manager = McpConnectionSet::new(
            ...
            McpRuntimeInput {
                ...
                forestx_apps_tools_cache: ConnectorRuntimeManager::default(),
                tool_catalog_cache: McpToolCatalogCache::default(),
                forestx_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                    /*account_id*/ None, /*chatgpt_user_id*/ None,
                ),
                client_mcp_extensions: ClientMcpExtensions::default(),
...
async fn no_local_runtime_fails_local_stdio_but_keeps_local_http_server() {
    ...
    let manager = McpConnectionSet::new(
        ...
        McpRuntimeInput {
            ...
            forestx_apps_tools_cache: ConnectorRuntimeManager::<ToolInfo>::default(),
            tool_catalog_cache: McpToolCatalogCache::default(),
            forestx_apps_tools_cache_key: ConnectorRuntimeContextKey::personal(
                /*account_id*/ None, /*chatgpt_user_id*/ None,
            ),
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = FORESTX_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
...
fn mcp_init_error_display_prompts_for_login_when_auth_required() {
    ...
    let expected = format!(
        "The {server_name} MCP server is not logged in. Run `forestx mcp login {server_name}`."
    );
...
fn reusable_server_config(url: &str) -> McpServerConfig {
    McpServerConfig {
        ...
        transport: McpServerTransportConfig::StreamableHttp {
            url: url.to_string(),
            bearer_token_env_var: Some("FORESTX_MCP_REUSE_TEST_TOKEN".to_string()),
            http_headers: None,
            env_http_headers: None,
        },
        environment_id: forestx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
...
async fn reconciliation_reuses_connection_without_relisting_regular_tools() -> anyhow::Result<()> {
    ...
    let initial_tools = list_tools_for_client_uncached(
        "docs",
        /*is_forestx_apps_mcp_server*/ false,
        /*forestx_apps_refresh_trigger*/ "test",
        &client,
        /*timeout*/ None,
        crate::pagination::MAX_MCP_CATALOG_ITEMS,
...
async fn reconciliation_reuses_legacy_stdio_server_with_existing_protocol_marker() {
    ...
    config.transport = McpServerTransportConfig::Stdio {
        ...
        env: Some(HashMap::from([(
            "FORESTX_MCP_PROTOCOL_VERSION".to_string(),
...
async fn reconciliation_replaces_connection_when_protocol_mode_changes() {
    ...
    .await;
    let forestx_home = tempdir().expect("tempdir");
    let mut mcp_config = crate::mcp::tests::test_mcp_config(forestx_home.path().to_path_buf());
    mcp_config.protocol_mode = forestx_rmcp_client::McpProtocolMode::V20260728;
...
async fn connection_identity_distinguishes_accounts_with_the_same_token() -> anyhow::Result<()> {
    ...
    let previous_auth = ForestxAuth::from_external_chatgpt_tokens(
        access_token,
        "account-a",
        /*chatgpt_plan_type*/ None,
    )?;
    let changed_auth = ForestxAuth::from_external_chatgpt_tokens(
        access_token,
        "account-b",
        /*chatgpt_plan_type*/ None,
    )?;
    let connection_identity = |auth: &ForestxAuth| {
        let provider = forestx_model_provider::auth_provider_from_auth(auth);
        McpServerConnectionIdentity::new(
            "docs",
            &server,
            OAuthCredentialsStoreMode::default(),
...
async fn connection_identity_distinguishes_agent_account_runtime_and_task() -> anyhow::Result<()> {
    ...
    let config = reusable_server_config("http://127.0.0.1:1");
    let server = EffectiveMcpServer::configured(config);
    let record = forestx_login::auth::AgentIdentityAuthRecord {
        ...
        plan_type: forestx_protocol::account::PlanType::Plus,
        chatgpt_account_is_fedramp: false,
        task_id: Some("task-a".to_string()),
    };
    let auth_route_config = forestx_login::test_support::transport_default_auth_route_config();
    let previous_auth = ForestxAuth::AgentIdentity(
        forestx_login::auth::AgentIdentityAuth::from_record(
            record.clone(),
    ...
    for changed_record in [
        forestx_login::auth::AgentIdentityAuthRecord {
        ...
        let changed_auth = ForestxAuth::AgentIdentity(
            forestx_login::auth::AgentIdentityAuth::from_record(
                changed_record,
                "https://auth.openai.com/api/accounts",
                &auth_route_config,

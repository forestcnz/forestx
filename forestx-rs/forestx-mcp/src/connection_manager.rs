//! Aggregates MCP server connections for Forestx.
//!
...
use startup::should_share_forestx_apps_tools_cache;
...
use crate::pagination::MAX_FORESTX_APPS_TOOL_CATALOG_ITEMS;
use crate::pagination::MAX_MCP_CATALOG_ITEMS;
use crate::rmcp_client::AsyncManagedClient;
use crate::rmcp_client::DEFAULT_TOOL_TIMEOUT;
use crate::rmcp_client::ManagedClient;
use crate::rmcp_client::StartupOutcomeError;
use crate::rmcp_client::prepare_forestx_apps_tools_for_model;
...
use forestx_protocol::protocol::McpStartupCompleteEvent;
use forestx_protocol::protocol::McpStartupFailure;
use forestx_protocol::protocol::McpStartupStatus;
use forestx_protocol::protocol::McpStartupUpdateEvent;
use forestx_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpServerView {
    async fn listed_tools(
        ...
        Some(if self.connection.client.is_forestx_apps_mcp_server {
            prepare_forestx_apps_tools_for_model(tools, tool_plugin_provenance)
        } else {
...
pub(crate) struct McpConnectionSet {
    ...
    forestx_apps_tools_override: RwLock<Option<Vec<ToolInfo>>>,
    forestx_apps_refresh_lock: Mutex<()>,
...
impl McpConnectionSet {
    ...
    pub async fn new(
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_forestx_backend())
            .map(forestx_model_provider::auth_provider_from_auth);
        let forestx_apps_auth_provider = forestx_apps_auth_manager.and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_forestx_backend()).map(|auth| {
                forestx_model_provider::auth_provider_from_auth_manager(auth_manager, auth)
            })
        });
        ...
        {
            let is_host_owned_forestx_apps = server_name == FORESTX_APPS_MCP_SERVER_NAME
            ...
            let catalog_item_limit = if is_host_owned_forestx_apps {
                MAX_FORESTX_APPS_TOOL_CATALOG_ITEMS
            } else {
            ...
            // For built-in Forestx Apps, `FORESTX_CONNECTORS_TOKEN` is a debug
            ...
            let shares_forestx_apps_tools_cache = is_host_owned_forestx_apps
                && should_share_forestx_apps_tools_cache(&server_name, uses_env_bearer_token);
            let forestx_apps_tools_cache_context = shares_forestx_apps_tools_cache.then(|| {
                forestx_apps_tools_cache
                    .context(forestx_home.clone(), forestx_apps_tools_cache_key.clone())
            });
            // The reserved Forestx Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == FORESTX_APPS_MCP_SERVER_NAME {
                forestx_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If Forestx Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient ForestxAuth provider.
            let runtime_auth_provider =
                if server_name == FORESTX_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
            ...
            let connection_identity = McpServerConnectionIdentity::new(
                ...
                auth,
                shares_forestx_apps_tools_cache
                    .then(|| (forestx_home.clone(), forestx_apps_tools_cache_key.clone())),
            ...
            let expected_protocol_mode = match &configured_config.transport {
                ...
                McpServerTransportConfig::Stdio { env, .. } => match env
                    .as_ref()
                    .and_then(|variables| variables.get("FORESTX_MCP_PROTOCOL_VERSION"))
                {
            ...
            let tool_catalog_cache_context = if server_name == FORESTX_APPS_MCP_SERVER_NAME {
                None
    ...
    /// Forestx Apps metadata may come from its existing cache; regular MCP server information is

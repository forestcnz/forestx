...
//! and exposing cached Forestx Apps tools while a client is still connecting.
...
use crate::forestx_apps::normalize_forestx_apps_callable_name;
use crate::forestx_apps::normalize_forestx_apps_callable_namespace;
use crate::forestx_apps::normalize_forestx_apps_tool_title;
use crate::forestx_apps::prepare_openai_file_params_for_model;
use crate::elicitation::ElicitationRequestManager;
use crate::mcp::FORESTX_APPS_MCP_SERVER_NAME;
...
use forestx_protocol::protocol::McpStartupStatus;
use forestx_protocol::protocol::McpStartupUpdateEvent;
use forestx_rmcp_client::ExecutorStdioServerLauncher;
use forestx_rmcp_client::LocalStdioServerLauncher;
use forestx_rmcp_client::McpProtocolMode;
use forestx_rmcp_client::RmcpClient;
use forestx_rmcp_client::StdioServerLauncher;
use forestx_rmcp_client::StreamableHttpRedirectMode;
use forestx_rmcp_client::ToolWithConnectorId;
use forestx_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that Forestx should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "forestx/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "forestx/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "forestx.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "forestx.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const FORESTX_APPS_REFRESH_DURATION_METRIC: &str = "forestx.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const FORESTX_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const FORESTX_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) forestx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .forestx_apps_tools_cache_context
            .as_ref()
        ...
        if self.forestx_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct ForestxAppsStartupReconnectState {
...
#[derive(Clone)]
struct ForestxAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct ForestxAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<ForestxAppsStartupReconnectState>,
    startup_status_context: Option<ForestxAppsStartupStatusContext>,
}

impl ForestxAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(ForestxAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = tx_event.map(|tx_event| ForestxAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after = forestx_apps_reconnect_backoff(state.consecutive_failures);
...
fn forestx_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    FORESTX_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(FORESTX_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    forestx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_forestx_apps_mcp_server = server_name == FORESTX_APPS_MCP_SERVER_NAME;
        ...
        async move {
            let refresh_start = is_forestx_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    FORESTX_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_forestx_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<ForestxAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let cached_server_info = if is_forestx_apps_mcp_server {
            forestx_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            forestx_apps_tools_cache_context: forestx_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_forestx_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                ForestxAppsStartupReconnect::new(Arc::new(move || startup.start()))
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.forestx_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_forestx_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }
    }
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_forestx_apps_mcp_server: bool,
    forestx_apps_refresh_trigger: &'static str,
    ...
    .collect();
    if is_forestx_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", forestx_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared Forestx Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
pub(crate) fn prepare_forestx_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_forestx_apps_mcp_server {
        forestx_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a Forestx Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn forestx_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_forestx_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_forestx_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_forestx_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = forestx_apps_tools_cache_context
        .as_ref()
    ...
    let client_tools = list_tools_for_client_uncached(
        &server_name,
        is_forestx_apps_mcp_server,
        /*forestx_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
        catalog_item_limit,
    ...
    let shared_tools = match (forestx_apps_tools_cache_context.as_ref(), fetch_ticket) {
        ...
        _ => unreachable!("Forestx Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog = is_forestx_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("forestx-mcp-client", env!("CARGO_PKG_VERSION")).with_title("Forestx"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(forestx_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    use super::*;
    use forestx_protocol::mcp::MCP_APP_UI_EXTENSION_ID;
    ...
    #[test]
    fn forestx_apps_connector_metadata_is_preserved() {
        ...
        let expected = ToolInfo {
            server_name: FORESTX_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "forestx_apps__gmail".to_string(),

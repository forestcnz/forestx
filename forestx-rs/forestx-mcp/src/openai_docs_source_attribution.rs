...
const OPENAI_DEVELOPER_DOCS_MCP_FORESTX_URL: &str = "https://developers.openai.com/mcp?source=forestx";
...
impl OpenAiDocsHttpClient {
    fn attribute_mcp_request(&self, params: &mut HttpRequestParams) {
        if params.url == OPENAI_DEVELOPER_DOCS_MCP_URL {
            params.url = OPENAI_DEVELOPER_DOCS_MCP_FORESTX_URL.to_string();
        }
    }
}

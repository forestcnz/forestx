...
//! Session orchestration stays in `forestx-core`.

use forestx_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_FORESTX_APPS_META_KEY: &str = "_forestx_apps";
...
pub struct ForestxAppsConnectorAuthFailure {
...
pub struct ForestxAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct ForestxAppsAuthElicitationPlan {
    pub auth_failure: ForestxAppsConnectorAuthFailure,
    pub elicitation: ForestxAppsAuthElicitation,
}

#[derive(Serialize)]
struct ForestxAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<ForestxAppsConnectorAuthFailure> {
    ...
    Some(ForestxAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<ForestxAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(ForestxAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &ForestxAppsConnectorAuthFailure,
) -> ForestxAppsAuthElicitation {
    ForestxAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_FORESTX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: ForestxAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("forestx_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &ForestxAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in Forestx.",
            auth_failure.connector_name
        ),
        _ => format!(
...
mod tests {
    ...
    fn builds_auth_elicitation_plan() {
        ...
        assert_eq!(plan.elicitation.elicitation_id, "forestx_apps_auth_call_123");
    }
}

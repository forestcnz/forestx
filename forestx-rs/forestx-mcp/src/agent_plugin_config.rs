...
/// Translates an Agent Plugins `mcp.json` into Forestx MCP configuration.
...
fn normalize_agent_plugin_mcp_server(
    ...
    let object = match server {
        ...
        AgentPluginMcpServer::Sse { .. } => {
            return Err("Agent Plugins legacy SSE transport is not supported by Forestx".to_string());
        }
    };

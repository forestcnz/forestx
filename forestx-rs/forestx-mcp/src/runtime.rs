...
pub struct McpRuntimeInput {
    ...
    pub forestx_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
    pub tool_catalog_cache: McpToolCatalogCache,
    pub forestx_apps_tools_cache_key: ConnectorRuntimeContextKey,
    pub client_mcp_extensions: ClientMcpExtensions,
    pub auth: Option<ForestxAuth>,
    pub forestx_apps_auth_manager: Option<Arc<AuthManager>>,
    pub elicitation_reviewer: Option<ElicitationReviewerHandle>,
    pub elicitation_lifecycle: Option<ElicitationLifecycle>,
}

/// Owns all mutable MCP state for one Forestx thread.
///
...
impl McpRuntime {
    ...
    pub async fn replace_fresh(&self, input: McpRuntimeInput) -> anyhow::Result<Vec<ToolInfo>> {
        ...
        self.latest_hard_refresh_forestx_apps_tools_cache().await
    }
    ...
    pub fn current_auth_matches(&self, auth: Option<&ForestxAuth>) -> bool {
    ...
    pub async fn latest_hard_refresh_forestx_apps_tools_cache(
        &self,
...
impl McpRuntimeContext {
    ...
    pub(crate) fn resolve_server_environment(
        &self,
        server_name: &str,
        config: &forestx_config::McpServerConfig,
        ...
        if config.is_local_environment() {
            return match config.transport {
                forestx_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                forestx_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }

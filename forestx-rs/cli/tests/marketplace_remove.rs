...
fn write_installed_marketplace(forestx_home: &Path, marketplace_name: &str) -> Result<()> {
    let root = marketplace_install_root(forestx_home).join(marketplace_name);
...
async fn marketplace_remove_deletes_config_and_installed_root() -> Result<()> {
    ...
    record_user_marketplace(forestx_home.path(), "debug", &configured_marketplace_update())?;
    write_installed_marketplace(forestx_home.path(), "debug")?;

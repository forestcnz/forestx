...
fn write_file_auth_config(forestx_home: &Path) -> Result<()> {
    std::fs::write(
...
fn read_auth_json(forestx_home: &Path) -> Result<Value> {
    let auth_json = std::fs::read_to_string(forestx_home.join("auth.json"))?;
...
fn login_with_api_key_reads_stdin_and_writes_auth_json() -> Result<()> {
    ...
    write_file_auth_config(forestx_home.path())?;
    ...
    let auth = read_auth_json(forestx_home.path())?;
...
async fn debug_prompt_input_follows_authenticated_attribution_setting() -> Result<()> {
    ...
    for enabled in [true, false] {
        let output = forestx_command(forestx_home.path())?
        ...
        assert_eq!(
            prompt.contains("Co-authored-by: Forestx <noreply@openai.com>"),
            enabled
        );

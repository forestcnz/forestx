...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = forestx_command(forestx_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = forestx_command(forestx_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = forestx_command(forestx_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
async fn add_and_login_discover_oauth_through_configured_http_proxy() -> Result<()> {
    ...
    assert!(
        load_global_mcp_servers(forestx_home.path())
            .await?
            .contains_key("oauth")
    );
    ...
    let mut login = forestx_command(forestx_home.path())?;
    login
        .env("HTTP_PROXY", proxy.uri())
        .env("http_proxy", proxy.uri())
        .env_remove("HTTPS_PROXY")
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!forestx_home.path().join(".credentials.json").exists());
    assert!(!forestx_home.path().join(".env").exists());

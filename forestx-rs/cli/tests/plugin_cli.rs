...
use forestx_config::MarketplaceConfigUpdate;
use forestx_config::record_user_marketplace;
...
fn forestx_command(forestx_home: &Path) -> Result<assert_cmd::Command> {
    ...
    cmd.env("HOME", forestx_home);
    Ok(cmd)
}

fn forestx_command_in(forestx_home: &Path, current_dir: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = forestx_command(forestx_home)?;
...
fn write_plugins_enabled_config(forestx_home: &Path) -> Result<()> {
    std::fs::write(
...
fn write_marketplace_source_with_manifest(source: &Path, marketplace_manifest: &str) -> Result<()> {
    ...
    std::fs::create_dir_all(source.join("plugins").join("sample").join(".forestx-plugin"))?;
    std::fs::write(
        source
            .join(".agents")
            .join("plugins")
...
fn setup_local_marketplace_with_implicit_system_roots() -> Result<(TempDir, TempDir, TempDir)> {
    let (forestx_home, source) = setup_local_marketplace()?;

    let bundled_root = forestx_home
        .path()
        .join(".tmp")
    ...
    let runtime_root = cache_home
        .path()
        .join(".cache")
        .join("forestx-runtimes")
        .join("forestx-primary-runtime")
        .join("plugins")
    ...
    Ok((forestx_home, source, cache_home))
}
...
fn setup_custom_marketplace_under_implicit_system_root() -> Result<(TempDir, std::path::PathBuf)> {
    ...
    Ok((forestx_home, custom_root))
}

fn remove_installed_plugin_config(forestx_home: &Path, plugin_key: &str) -> Result<()> {
...
async fn marketplace_list_json_includes_configured_git_marketplace_source() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let marketplace_root = forestx_home
        .path()
        .join(".tmp")
        .join("marketplaces")
        .join("debug");
    ...
    record_user_marketplace(forestx_home.path(), "debug", &update)?;
...
async fn marketplace_list_includes_root_when_plugins_are_filtered_out() -> Result<()> {
    let (forestx_home, source) = setup_local_marketplace_with_explicit_empty_products()?;
    let expected_row = marketplace_list_row("debug", source.path());
...
async fn marketplace_list_fails_when_configured_marketplace_snapshot_is_missing() -> Result<()> {
    let (forestx_home, source) = setup_configured_marketplace_without_manifest()?;
...
async fn marketplace_list_fails_when_configured_marketplace_name_is_invalid() -> Result<()> {
    let forestx_home = setup_configured_local_marketplace_with_invalid_name()?;
...
{
    let forestx_home = setup_configured_local_marketplace_with_missing_source()?;
...
async fn marketplace_list_fails_when_configured_marketplace_snapshot_is_malformed() -> Result<()> {
    let (forestx_home, source) = setup_configured_marketplace_with_malformed_manifest()?;
...
async fn plugin_list_available_requires_json() -> Result<()> {
    let (forestx_home, _source) = setup_local_marketplace()?;
...
async fn plugin_list_excludes_unconfigured_repo_local_marketplaces() -> Result<()> {
    let (forestx_home, source) = setup_unconfigured_local_marketplace()?;

    forestx_command_in(forestx_home.path(), source.path())?
...
async fn plugin_list_ignores_implicit_system_marketplace_roots_without_manifests() -> Result<()> {
    let (forestx_home, source, cache_home) = setup_local_marketplace_with_implicit_system_roots()?;
...
async fn plugin_list_fails_for_custom_marketplace_under_system_root() -> Result<()> {
    let (forestx_home, custom_root) = setup_custom_marketplace_under_implicit_system_root()?;
...
async fn plugin_list_hides_version_for_cached_but_unconfigured_plugin() -> Result<()> {
    ...
    remove_installed_plugin_config(forestx_home.path(), "sample@debug")?;
...
async fn plugin_add_and_remove_updates_installed_plugin_config() -> Result<()> {
    ...
    let config = std::fs::read_to_string(forestx_home.path().join(CONFIG_TOML_FILE))?;
...
async fn plugin_add_json_prints_install_outcome() -> Result<()> {
    ...
    let stdout = assert.get_output().stdout.as_slice();
    let actual: serde_json::Value = serde_json::from_slice(stdout)?;
    let installed_path = forestx_home.path().join("plugins/cache/debug/sample/1.2.3");
...
async fn plugin_add_reinstalls_from_configured_marketplace_snapshot() -> Result<()> {
    ...
    assert!(
        forestx_home
            .path()
            .join("plugins/cache/debug/sample/1.2.3/.forestx-plugin/plugin.json")
            .is_file()
    );

...
impl CloudManagedMcpFixture {
    async fn new() -> Result<Option<Self>> {
        ...
        std::fs::write(forestx_home.path().join("config.toml"), &user_config)?;

        let bootstrap_config = load_config_toml_with_layer_stack(
            forestx_home.path(),
            Some(&AbsolutePathBuf::from_absolute_path(forestx_home.path())?),
            Vec::new(),
    ...
    fn command(&self, args: &[&str]) -> Result<Command> {
        let mut command = Command::new(forestx_utils_cargo_bin::cargo_bin("forestx")?);
        command
            .kill_on_drop(true)
            .current_dir(self.forestx_home.path())
            .env("FORESTX_HOME", self.forestx_home.path())
            .env("NO_PROXY", "127.0.0.1,localhost")
            .env("no_proxy", "127.0.0.1,localhost")
            .env_remove("FORESTX_ACCESS_TOKEN")
    ...
    async fn output(&self, args: &[&str]) -> Result<Output> {
        let output = self.command(args)?.output().await?;
            ...
            "forestx {} failed with status {}: stdout={}; stderr={}",
            args.join(" "),
            output.status,
    ...
    fn assert_user_config_unchanged(&self) -> Result<()> {
        assert_eq!(
            std::fs::read_to_string(self.forestx_home.path().join("config.toml"))?,
            self.user_config
        );
        Ok(())
    }
...
async fn login_and_logout_persist_only_cloud_managed_mcp_oauth_credentials() -> Result<()> {
    ...
    let credentials_path = fixture.forestx_home.path().join(".credentials.json");
...
async fn add_and_remove_do_not_copy_or_delete_cloud_managed_mcp_servers() -> Result<()> {
    ...
    fixture
        ...
        .await?;
    let local_servers = load_global_mcp_servers(fixture.forestx_home.path()).await?;
    ...
    assert!(
        load_global_mcp_servers(fixture.forestx_home.path())
            .await?
            .is_empty()
    );
    fixture

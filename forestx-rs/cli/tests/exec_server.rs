...
fn local_exec_server_ignores_invalid_config_without_strict_config() -> Result<()> {
    let forestx_home = TempDir::new()?;
    std::fs::write(forestx_home.path().join("config.toml"), "not valid toml = [")?;
...
fn local_exec_server_allows_disabled_parent_lifetime_environment_variable() -> Result<()> {
    ...
    let mut cmd = forestx_command(forestx_home.path())?;
    cmd.env(
        forestx_exec_server::FORESTX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
        "false",
    )
...
"#
    ...
    let mut command = tokio::process::Command::new(forestx_utils_cargo_bin::cargo_bin("forestx")?);
    command
        .env("FORESTX_HOME", forestx_home.path())
        .env("FORESTX_API_KEY", "test-api-key")
        .env(
            forestx_exec_server::FORESTX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
            "true",
        )
    ...
    let argv = vec![
        "cmd.exe",
        "/C",
        "if defined FORESTX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE (exit /b 1) else ping -n 61 127.0.0.1",
    ];
    #[cfg(not(windows))]
    let argv = vec![
        "/bin/sh",
        "-c",
        "[ -z \"${FORESTX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE+present}\" ] && exec /bin/sleep 60",
    ];
    ...
    client
        .exec(ExecParams {
            ...
            env_policy: Some(forestx_exec_server::ExecEnvPolicy {
                inherit: forestx_protocol::config_types::ShellEnvironmentPolicyInherit::All,
...
"#
    ...
    let forestx_bin = forestx_utils_cargo_bin::cargo_bin("forestx")?;
    let forestx_home = forestx_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(forestx_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let forestx_home = TempDir::new()?;
    let mut child = std::process::Command::new(forestx_utils_cargo_bin::cargo_bin("forestx")?)
        .env("FORESTX_HOME", forestx_home.path())

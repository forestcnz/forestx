...
async fn configure_http_oauth_server(forestx_home: &Path, url: &str) -> Result<()> {
    let mut servers = load_global_mcp_servers(forestx_home).await?;
    servers.insert(
        "oauth".to_string(),
...
"#,
  ...
  "mcpServers": {
    ...
    let mut list_cmd = forestx_command(forestx_home.path())?;
    list_cmd
        .env(FORESTX_API_KEY_ENV_VAR, "sk-test")
        .args(["mcp", "list", "--json"])
        .assert()
        .success()
        .stdout(contains(r#""name": "api-docs""#));

    let mut get_cmd = forestx_command(forestx_home.path())?;
    get_cmd
        .env(FORESTX_API_KEY_ENV_VAR, "sk-test")
...
async fn list_discovers_local_oauth_server_through_environment_proxy() -> Result<()> {
    let forestx_home = TempDir::new()?;
    configure_http_oauth_server(forestx_home.path(), "http://mcp-proxy.invalid/mcp").await?;
    std::fs::write(forestx_home.path().join("environments.toml"), "invalid = [")?;
    ...
    let mut command = forestx_command(forestx_home.path())?;
    command
...
async fn list_reports_unknown_auth_status_when_oauth_discovery_is_rate_limited() -> Result<()> {
    ...
    wiremock::Mock::given(wiremock::matchers::method("GET"))
        ...
        .mount(&server)
        .await;
    configure_http_oauth_server(forestx_home.path(), &format!("{}/mcp", server.uri())).await?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = forestx_command(forestx_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(forestx_home.path()).await?;
    ...
    let mut list_json_cmd = forestx_command(forestx_home.path())?;
    ...
    assert!(stdout.contains("remove: forestx mcp remove docs"));

    let mut get_json_cmd = forestx_command(forestx_home.path())?;
    get_json_cmd

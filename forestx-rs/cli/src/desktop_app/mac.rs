...
const FORESTX_BUNDLE_IDENTIFIER: &str = "com.openai.forestx";
const FORESTX_DMG_URL_ARM64: &str = "https://persistent.oaistatic.com/forestx-app-prod/Forestx.dmg";
const FORESTX_DMG_URL_X64: &str =
    "https://persistent.oaistatic.com/forestx-app-prod/Forestx-latest-x64.dmg";

pub async fn run_mac_app_open_or_install(
    workspace: PathBuf,
    download_url_override: Option<String>,
) -> anyhow::Result<()> {
    if let Some(app_path) = find_existing_forestx_app_path(&forestx_app_search_dirs()) {
        eprintln!(
            "Opening Desktop app at {app_path}...",
            app_path = app_path.display()
        );
        open_forestx_app(&app_path, &workspace).await?;
        return Ok(());
    }
    ...
    let download_url = download_url_override.unwrap_or_else(|| {
        let default_url = if is_apple_silicon_mac() {
            FORESTX_DMG_URL_ARM64
        } else {
            FORESTX_DMG_URL_X64
        };
        default_url.to_string()
    });
    let installed_app = download_and_install_forestx_to_user_applications(&download_url)
        .await
    ...
    open_forestx_app(&installed_app, &workspace).await?;
    Ok(())
}
...
fn find_existing_forestx_app_path(applications_dirs: &[PathBuf]) -> Option<PathBuf> {
    applications_dirs
        .iter()
        .flat_map(|dir| ["ChatGPT.app", "Forestx.app"].map(|app_name| dir.join(app_name)))
        .find(|candidate| is_forestx_app_bundle(candidate))
}

fn forestx_app_search_dirs() -> Vec<PathBuf> {
...
fn is_forestx_app_bundle(app_path: &Path) -> bool {
    if !app_path.is_dir() {
        return false;
    }
    ...
    std::process::Command::new("/usr/bin/plutil")
        ...
        .is_ok_and(|output| {
            output.status.success()
                && String::from_utf8_lossy(&output.stdout).trim() == FORESTX_BUNDLE_IDENTIFIER
        })
}

async fn open_forestx_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
        "Opening workspace {workspace}...",
        workspace = workspace.display()
    );
    let url = forestx_new_thread_url(workspace);
...
fn forestx_new_thread_url(workspace: &Path) -> String {
    let workspace = workspace.as_os_str().to_string_lossy();
    let mut serializer = url::form_urlencoded::Serializer::new(String::new());
    serializer.append_pair("path", workspace.as_ref());
    let query = serializer.finish();
    format!("forestx://threads/new?{query}")
}

async fn download_and_install_forestx_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("forestx-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Forestx.dmg");
    ...
    let result = async {
        let app_in_volume = find_forestx_app_in_mount(&mount_point)
            .context("failed to locate Forestx.app in mounted dmg")?;
        install_forestx_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_forestx_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        ...
        let dest_app = applications_dir.join("Forestx.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Forestx.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Forestx.app to any applications directory");
}
...
fn find_forestx_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Forestx.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
#[cfg(test)]
mod tests {
    use super::forestx_new_thread_url;
    use super::find_existing_forestx_app_path;
    ...
    #[test]
    fn finds_chatgpt_app_with_forestx_bundle_identifier() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let app_path = temp_dir.path().join("ChatGPT.app");
        write_app_bundle(&app_path, "com.openai.forestx");

        assert_eq!(
            find_existing_forestx_app_path(&[temp_dir.path().to_path_buf()]),
            Some(app_path)
        );
    }
    ...
    fn ignores_classic_chatgpt_app() {
        ...
        let forestx_app_path = temp_dir.path().join("Forestx.app");
        write_app_bundle(&forestx_app_path, "com.openai.forestx");

        assert_eq!(
            find_existing_forestx_app_path(&[temp_dir.path().to_path_buf()]),
            Some(forestx_app_path)
        );
    }

    #[test]
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tForestx\t/Volumes/Forestx\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Forestx")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tForestx Installer\t/Volumes/Forestx Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Forestx Installer")
        );
    }

    #[test]
    fn forestx_new_thread_url_encodes_workspace_path() {
        let url = url::Url::parse(&forestx_new_thread_url(Path::new("/tmp/forestx workspace/#1")))
        ...
        assert_eq!(
            ...
            (
                ...
                "/new".to_string(),
                vec![("path".to_string(), "/tmp/forestx workspace/#1".to_string())],
            )
        );
    }
}

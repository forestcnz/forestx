...
use forestx_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use forestx_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use forestx_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use forestx_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use forestx_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = forestx_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = forestx_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = forestx_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let runtime_options = AppServerRuntimeOptions {
        remote_control_startup_mode: forestx_app_server::RemoteControlStartupMode::EnabledEphemeral,
        install_shutdown_signal_handler: false,
        psp,
        ..Default::default()
    };
    let (stop_rx, stop_signal_task) = foreground_stop_signal();
    let mut app_server_task = tokio::spawn(forestx_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    forestx_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_forestx_path, managed_forestx_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_forestx_path.display()),
        format!("  version: {}", managed_forestx_version.unwrap_or("unknown")),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_forestx_path,
            output.managed_forestx_version.as_deref(),
        ),
...
mod tests {
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_forestx_path: PathBuf::from("/opt/forestx/bin/forestx"),
                managed_forestx_version: Some("1.0.0".to_string()),
    ...
    fn daemon_app_server_human_lines_include_path_and_version() {
        assert_eq!(
            ...
            vec![
                "Daemon used app-server:".to_string(),
                "  path: /opt/forestx/bin/forestx".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedForestxPath": "/opt/forestx/bin/forestx",
                    "managedForestxVersion": "1.0.0",

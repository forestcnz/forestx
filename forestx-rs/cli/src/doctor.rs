//! Implements the `forestx doctor` diagnostic report.
//!
...
use forestx_api::ResponsesWebsocketClient;
use forestx_api::is_azure_responses_provider;
...
use forestx_install_context::ForestxPackageLayout;
use forestx_install_context::InstallContext;
use forestx_install_context::InstallMethod;
use forestx_install_context::StandalonePlatform;
use forestx_login::AuthDotJson;
use forestx_login::AuthManager;
use forestx_login::FORESTX_ACCESS_TOKEN_ENV_VAR;
use forestx_login::FORESTX_API_KEY_ENV_VAR;
use forestx_login::ForestxAuth;
use forestx_login::OPENAI_API_KEY_ENV_VAR;
use forestx_login::default_client::create_client_without_request_logging;
use forestx_login::default_client::default_headers;
use forestx_login::load_auth_dot_json;
...
use forestx_terminal_detection::Multiplexer;
use forestx_terminal_detection::TerminalInfo;
use forestx_terminal_detection::TerminalName;
use forestx_terminal_detection::terminal_info;
...
/// Options for building a local Forestx diagnostic report.
///
...
///
/// This is the CLI entry point for forestx doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun forestx doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        forestx_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load Forestx config")
}
...
/// JSON support report emitted by `forestx doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        forestx_version: report.forestx_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("FORESTX_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("FORESTX_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "FORESTX_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = forestx_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH forestx entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH forestx #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @openai/forestx would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Forestx so the JS shim provides FORESTX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("FORESTX_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("FORESTX_MANAGED_BY_NPM").is_none()
        && env::var_os("FORESTX_MANAGED_BY_BUN").is_none()
        && env::var_os("FORESTX_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&ForestxPackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("FORESTX_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("forestx");
...
fn forestx_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["forestx"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "forestx"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("FORESTX_HOME: {}", config.forestx_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.forestx_home.join(forestx_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.forestx_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        FORESTX_API_KEY_ENV_VAR,
        FORESTX_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run forestx login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no Forestx credentials were found",
        )
        .details(details)
        .remediation("Run forestx login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run forestx login again."),
    }
}
...
fn stored_auth_mode(auth: &forestx_login::AuthDotJson) -> &'static str {
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(FORESTX_API_KEY_ENV_VAR);
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["FORESTX_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "forestx-linux-sandbox helper",
        arg0_paths.forestx_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.forestx_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "FORESTX_HOME", &config.forestx_home);
    ...
    rollout_stats_details(&mut details, &config.forestx_home);
...
async fn sqlite_integrity_detail(
    sqlite: &forestx_state::SqliteConfig,
    details: &mut Vec<String>,
    ...
    match forestx_state::sqlite_integrity_check(sqlite, path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, forestx_home: &Path) {
    let active = collect_rollout_stats(&forestx_home.join("sessions"));
    let archived = collect_rollout_stats(&forestx_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &ForestxAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match forestx_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "FORESTX_HOME was resolved without config",
        )
        .detail(format!("FORESTX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "FORESTX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(FORESTX_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(FORESTX_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/forestx");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("forestx")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/forestx");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("forestx"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].forestx_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            forestx_self_exe: Some(PathBuf::from("/bin/forestx")),
            forestx_linux_sandbox_exe: Some(PathBuf::from("/bin/forestx-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/forestx-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.forestx_self_exe, arg0_paths.forestx_self_exe);
        assert_eq!(
            overrides.forestx_linux_sandbox_exe,
            arg0_paths.forestx_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            forestx_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "system.environment",
                    "system",
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "FORESTX_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("FORESTX_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Forestx."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Forestx.".to_string())
        );
    }
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-forestx-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-forestx-doctor-mcp\" is not resolvable",
            )
        }));
    }

...
const OTEL_SERVICE_NAME: &str = "forestx-exec-server";
...
pub(crate) fn init(
    config: Option<&forestx_core::config::Config>,
) -> (impl Send + Sync, forestx_exec_server::ExecServerTelemetry) {
    let fmt_layer = tracing_subscriber::fmt::layer()
        .with_writer(std::io::stderr)
        .with_filter(stderr_env_filter());
    let otel = match config {
        Some(config) => forestx_core::otel_init::build_provider(
            config,
    ...
    forestx_core::otel_init::record_process_start(provider, OTEL_SERVICE_NAME);
    ...
    let telemetry = provider
        ...
        .cloned()
        .map(forestx_exec_server::ExecServerTelemetry::new)
        .unwrap_or_default();

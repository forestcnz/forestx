...
use forestx_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use forestx_core_plugins::marketplace::marketplace_root_dir;
use forestx_core_plugins::marketplace_add::MarketplaceAddOutcome;
use forestx_core_plugins::marketplace_add::MarketplaceAddRequest;
use forestx_core_plugins::marketplace_add::add_marketplace;
use forestx_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use forestx_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "forestx plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces Forestx is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "forestx plugin marketplace add",
    after_help = "Examples:\n  forestx plugin marketplace add ./path/to/marketplace\n  forestx plugin marketplace add owner/repo --ref main\n  forestx plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "forestx plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "forestx plugin marketplace upgrade",
    after_help = "Examples:\n  forestx plugin marketplace upgrade\n  forestx plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "forestx plugin marketplace remove",
    after_help = "Example:\n  forestx plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let manager = PluginsManager::new(config.forestx_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.forestx_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<forestx_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, forestx_home);

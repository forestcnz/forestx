...
use forestx_core::config::load_global_mcp_servers;
use forestx_core_plugins::PluginsManager;
use forestx_exec_server::EnvironmentManager;
use forestx_exec_server::HttpClient;
use forestx_exec_server::RouteAwareHttpClient;
use forestx_login::AuthManager;
use forestx_mcp::McpOAuthLoginSupport;
use forestx_mcp::McpRuntimeContext;
use forestx_mcp::ResolvedMcpOAuthScopes;
use forestx_mcp::compute_auth_statuses;
...
use forestx_rmcp_client::delete_oauth_tokens;
use forestx_rmcp_client::perform_oauth_login;
use forestx_utils_cli::CliConfigOverrides;
use forestx_utils_cli::format_env_display;
...
/// - `add`    — add a server launcher entry to `~/.forestx/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "forestx mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: forestx_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: forestx_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&forestx_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", forestx_home.display()))?;
    ...
    ConfigEditsBuilder::new(&forestx_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", forestx_home.display()))?;
    ...
    match login_support {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `forestx mcp login {name}` to login."
        ),
    }
...
async fn run_list(config: &Config, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `forestx mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config: &Config, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: forestx mcp remove {}", get_args.name);

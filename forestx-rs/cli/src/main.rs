...
use forestx_app_server_daemon::BootstrapOptions as AppServerBootstrapOptions;
use forestx_app_server_daemon::LifecycleCommand as AppServerLifecycleCommand;
use forestx_app_server_daemon::RemoteControlMode as AppServerRemoteControlMode;
use forestx_arg0::Arg0DispatchPaths;
use forestx_arg0::arg0_dispatch_or_else;
use forestx_chatgpt::apply_command::ApplyCommand;
use forestx_chatgpt::apply_command::run_apply_command;
use forestx_cli::read_access_token_from_stdin;
use forestx_cli::read_api_key_from_stdin;
use forestx_cli::run_login_status;
use forestx_cli::run_login_with_access_token;
use forestx_cli::run_login_with_api_key;
use forestx_cli::run_login_with_chatgpt;
use forestx_cli::run_login_with_device_code;
use forestx_cli::run_logout;
use forestx_cloud_tasks::Cli as CloudTasksCli;
use forestx_exec::Cli as ExecCli;
use forestx_exec::Command as ExecCommand;
use forestx_exec::ReviewArgs;
use forestx_execpolicy::ExecPolicyCheckCommand;
use forestx_responses_api_proxy::Args as ResponsesApiProxyArgs;
use forestx_rollout_trace::REDUCED_STATE_FILE_NAME;
use forestx_rollout_trace::replay_bundle;
use forestx_state::StateRuntime;
use forestx_tui::AppExitInfo;
use forestx_tui::Cli as TuiCli;
use forestx_tui::ExitReason;
use forestx_tui::UpdateAction;
use forestx_utils_absolute_path::AbsolutePathBuf;
use forestx_utils_cli::CliConfigOverrides;
use forestx_utils_cli::ProfileV2Name;
use forestx_utils_cli::SharedCliOptions;
...
use forestx_core::build_models_manager;
...
#[clap(
    ...
    // `forestx-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `forestx` command name that users run.
    bin_name = "forestx",
    override_usage = "forestx [OPTIONS] [PROMPT]\n       forestx [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Forestx.
    Mcp(McpCli),

    /// Manage Forestx plugins.
    Plugin(PluginCli),

    /// Start Forestx as an MCP server (stdio).
    ...
    /// Update Forestx to the latest version.
    Update,

    /// Diagnose local Forestx installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a Forestx-provided sandbox.
    ...
    /// Apply the latest diff produced by Forestx agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Forestx Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of Forestx.
...
#[cfg(target_os = "macos")]
type HostSandboxArgs = forestx_cli::SeatbeltCommand;
#[cfg(target_os = "linux")]
type HostSandboxArgs = forestx_cli::LandlockCommand;
#[cfg(target_os = "windows")]
type HostSandboxArgs = forestx_cli::WindowsCommand;
...
struct UnsupportedSandboxArgs {
    /// Layer $FORESTX_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | forestx login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv FORESTX_ACCESS_TOKEN | forestx login --with-access-token`)"
    )]
...
struct AppServerCommand {
    ...
    #[command(flatten)]
    code_mode_host: forestx_app_server::AppServerCodeModeHostArgs,
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = forestx_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: forestx_app_server::AppServerTransport,
    ...
    /// ```
    ///
    /// See https://developers.openai.com/forestx/config-advanced/#metrics for more details.
    #[arg(long = "analytics-default-enabled")]
    analytics_default_enabled: bool,

    #[command(flatten)]
    auth: forestx_app_server::AppServerWebsocketAuthArgs,
}
...
struct ExecServerCommand {
    ...
    request_dispatch_mode: forestx_exec_server::RequestDispatchMode,
    ...
    /// Use Agent Identity auth from FORESTX_ACCESS_TOKEN for remote registration.
    ...
    #[arg(
        ...
        env = forestx_exec_server::FORESTX_EXEC_SERVER_EXIT_ON_STDIN_CLOSE_ENV_VAR,
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Forestx tooling.
    #[clap(hide = true)]
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Forestx via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Forestx.");
    Ok(())
}
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`forestx update` is not available in debug builds. Install a release build of Forestx to use this command."
        );
    }

    #[cfg(not(debug_assertions))]
    {
        let Some(action) = forestx_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the Forestx installation method. Please update manually: https://developers.openai.com/forestx/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn run_session_archive_cli_command(
    action: forestx_tui::SessionArchiveAction,
    ...
    forestx_tui::run_session_archive_command(
        action,
        target,
        forestx_tui::SessionArchiveCommandOptions {
            cli: interactive,
            arg0_paths,
...
fn delete_action(target: &str, force: bool) -> anyhow::Result<forestx_tui::SessionArchiveAction> {
    if force && forestx_protocol::ThreadId::from_string(target).is_err() {
        anyhow::bail!("--force requires a session UUID; names must be confirmed interactively");
    }
    let confirmation = match force {
        true => forestx_tui::DeleteConfirmation::Skip,
        false => forestx_tui::DeleteConfirmation::Prompt,
    };
    Ok(forestx_tui::SessionArchiveAction::Delete(confirmation))
}

async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let forestx_bin = std::env::current_exe()?;
            forestx_app_server_test_client::send_message_v2(&forestx_bin, &[], cmd.user_message, &None)
                .await
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    let remote_control_disabled = forestx_app_server::take_remote_control_disabled_env();
...
async fn cli_main(
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            forestx_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["forestx", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            exec_cli.psp = psp;
            ...
            match subcommand {
                None => {
                    let transport = if stdio {
                        forestx_app_server::AppServerTransport::Stdio
                    } else {
                        listen
                    };
                    let auth = auth.try_into_settings()?;
                    let runtime_options = forestx_app_server::AppServerRuntimeOptions {
                        code_mode_host_transport: code_mode_host.into(),
                        remote_control_startup_mode: match (remote_control, remote_control_disabled)
                        {
                            (true, _) => {
                                forestx_app_server::RemoteControlStartupMode::EnabledEphemeral
                            }
                            (false, true) => {
                                forestx_app_server::RemoteControlStartupMode::DisabledEphemeral
                            }
                            (false, false) => {
                                forestx_app_server::RemoteControlStartupMode::ResolvePersisted
                            }
                        },
                        psp,
                        ..Default::default()
                    };
                    forestx_app_server::run_main_with_transport_options(
                        arg0_paths.clone(),
                        root_config_overrides,
                        ...
                        analytics_default_enabled,
                        transport,
                        forestx_protocol::protocol::SessionSource::VSCode,
                        auth,
                        runtime_options,
                    )
                    .await?;
                ...
                Some(AppServerSubcommand::Daemon(daemon_cli)) => match daemon_cli.subcommand {
                    ...
                    AppServerDaemonSubcommand::Bootstrap(bootstrap_cli) => {
                        let output =
                            forestx_app_server_daemon::bootstrap(AppServerBootstrapOptions {
                    ...
                    AppServerDaemonSubcommand::PidUpdateLoop => {
                        ...
                        forestx_app_server_daemon::run_pid_update_loop(http_client_factory).await?;
                    }
                },
                Some(AppServerSubcommand::Proxy(proxy_cli)) => {
                    let socket_path = match proxy_cli.socket_path {
                        ...
                        None => {
                            let forestx_home = find_forestx_home()?;
                            forestx_app_server::app_server_control_socket_path(&forestx_home)?
                        }
                    };
                    forestx_stdio_to_uds::run(socket_path.as_path()).await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = forestx_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    forestx_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    forestx_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    forestx_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        ...
        Some(Subcommand::Archive(cmd)) => {
            let output = run_session_archive_cli_command(
                forestx_tui::SessionArchiveAction::Archive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
        ...
        Some(Subcommand::Unarchive(cmd)) => {
            let output = run_session_archive_cli_command(
                forestx_tui::SessionArchiveAction::Unarchive,
                cmd,
                interactive,
                root_config_overrides.clone(),
                root_remote.clone(),
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | forestx login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            forestx_cloud_tasks::run_main(cloud_cli, arg0_paths.forestx_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(target_os = "macos")]
            forestx_cli::run_command_under_seatbelt(
                sandbox_cli,
                arg0_paths.forestx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "linux")]
            forestx_cli::run_command_under_landlock(
                sandbox_cli,
                arg0_paths.forestx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            .await?;
            #[cfg(target_os = "windows")]
            forestx_cli::run_command_under_windows_sandbox(
                sandbox_cli,
                arg0_paths.forestx_linux_sandbox_exe.clone(),
                loader_overrides,
            )
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`forestx sandbox` is not supported on this operating system");
            }
        }
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || forestx_responses_api_proxy::run_main(args))
                .await??;
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `forestx mcp`: `forestx`, `forestx exec`, `forestx review`, `forestx resume`, `forestx archive`, `forestx delete`, `forestx unarchive`, `forestx fork`, `forestx mcp`, `forestx sandbox`, and `forestx debug prompt-input`."
        ),
    }
}
...
async fn run_exec_server_command(
    ...
    let forestx_self_exe = arg0_paths
        .forestx_self_exe
        .clone()
        .ok_or_else(|| anyhow::anyhow!("Forestx executable path is not configured"))?;
    let runtime_paths = forestx_exec_server::ExecServerRuntimePaths::new(
        forestx_self_exe,
        arg0_paths.forestx_linux_sandbox_exe.clone(),
    )?;
    if let Some(base_url) = cmd.remote {
        ...
        let auth_provider =
            load_exec_server_remote_auth_provider(&config, &base_url, cmd.use_agent_identity_auth)
                .await?;
        let mut remote_config = forestx_exec_server::RemoteEnvironmentConfig::new(
            base_url,
            environment_id,
            auth_provider,
        ...
        exec_server_telemetry::run_until_shutdown(
            async move {
                forestx_exec_server::run_remote_environment_until_shutdown(
                    remote_config,
                    runtime_paths,
                    async move {
        ...
        let http_client_factory = config
            .as_ref()
            .map(forestx_core::config::Config::http_client_factory)
            .unwrap_or_else(|| {
                forestx_http_client::HttpClientFactory::new(
                    forestx_http_client::OutboundProxyPolicy::ReqwestDefault,
                )
            });
        let listen_url = cmd
            .listen
            .unwrap_or_else(|| forestx_exec_server::DEFAULT_LISTEN_URL.to_string());
        exec_server_telemetry::run_until_shutdown(
            async move {
                forestx_exec_server::run_main_with_telemetry(
                    &listen_url,
                    runtime_paths,
                    telemetry,
                    http_client_factory,
...
async fn load_exec_server_remote_auth_provider(
    config: &forestx_core::config::Config,
    base_url: &str,
    use_agent_identity_auth: bool,
) -> anyhow::Result<forestx_api::SharedAuthProvider> {
    if use_agent_identity_auth {
        let agent_identity_jwt = read_forestx_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("FORESTX_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
        let auth_route_config = config.auth_route_config();
        let auth = ForestxAuth::from_agent_identity_jwt(
            &agent_identity_jwt,
            Some(&config.chatgpt_base_url),
            &auth_route_config,
        )
        .await?;
        return Ok(forestx_model_provider::auth_provider_from_auth(&auth));
    }

    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `forestx login` or set FORESTX_API_KEY",
    )
    .await?;
    ...
    Ok(forestx_model_provider::auth_provider_from_auth(&auth))
}

fn is_supported_exec_server_remote_auth(auth: &ForestxAuth) -> bool {
...
async fn load_exec_server_config(
    root_config_overrides: &CliConfigOverrides,
    strict_config: bool,
) -> anyhow::Result<forestx_core::config::Config> {
...
) -> anyhow::Result<forestx_login::ForestxAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_forestx_api_key_env*/ true).await;
...
async fn enable_feature_in_config(feature: &str) -> anyhow::Result<()> {
    ...
    let forestx_home = find_forestx_home()?;
    ConfigEditsBuilder::new(&forestx_home)
        .set_feature_enabled(feature, /*enabled*/ true)
        .apply()
        .await?;
    println!("Enabled feature `{feature}` in config.toml.");
    maybe_print_under_development_feature_warning(&forestx_home, feature);
    Ok(())
}
...
fn loader_overrides_for_profile(
    ...
    match profile_v2 {
        Some(profile_v2) => {
            let forestx_home = find_forestx_home()?;
            Ok(loader_overrides_for_profile_at_forestx_home(
                Some(profile_v2),
                &forestx_home,
            ))
        }
...
fn loader_overrides_for_profile_at_forestx_home(
    profile_v2: Option<&ProfileV2Name>,
    forestx_home: &std::path::Path,
) -> LoaderOverrides {
    match profile_v2 {
        Some(profile_v2) => LoaderOverrides {
            user_config_path: Some(resolve_profile_v2_config_path(forestx_home, profile_v2)),
...
fn maybe_print_under_development_feature_warning(forestx_home: &std::path::Path, feature: &str) {
    ...
    let config_path = forestx_home.join(forestx_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_prompt_input_command(
    ...
    let sandbox_mode = if shared.dangerously_bypass_approvals_and_sandbox {
        Some(forestx_protocol::config_types::SandboxMode::DangerFullAccess)
    } else {
    ...
    let overrides = ConfigOverrides {
        ...
        sandbox_mode,
        cwd: shared.cwd,
        forestx_self_exe: arg0_paths.forestx_self_exe,
        forestx_linux_sandbox_exe: arg0_paths.forestx_linux_sandbox_exe,
    ...
    let auth_manager =
        AuthManager::shared_from_config(&config, /*enable_forestx_api_key_env*/ false).await;
    let mut extensions = forestx_extension_api::ExtensionRegistryBuilder::new();
    forestx_git_attribution::install(
        &mut extensions,
        auth_manager,
        config.chatgpt_base_url.clone(),
        config.http_client_factory(),
    );
    forestx_skills_extension::install(&mut extensions, |config: &Config| {
        forestx_skills_extension::SkillsExtensionConfig {
            include_instructions: config.include_skill_instructions,
            bundled_skills_enabled: config.bundled_skills_enabled(),
            orchestrator_skills_enabled: config.orchestrator_skills_enabled,
            shadow_selection_enabled: config
                .features
                .enabled(forestx_features::Feature::SkillSearch),
        }
    });
    let prompt_input = forestx_core::build_prompt_input(
        config,
        input,
        /*state_db*/ None,
...
async fn run_debug_clear_memories_command(
    ...
    clear_memory_roots_contents(&config.forestx_home).await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `forestx {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `forestx {subcommand}`"
        );
    }
    Ok(())
}
...
/// `forestx --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `forestx {subcommand}`");
    }
    Ok(())
}
...
async fn print_app_server_daemon_output(command: AppServerLifecycleCommand) -> anyhow::Result<()> {
    let output = forestx_app_server_daemon::run(command).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}

fn updater_http_client_factory(
    config: anyhow::Result<forestx_core::config::Config>,
) -> forestx_http_client::HttpClientFactory {
    match config {
...
async fn print_app_server_remote_control_output(
    ...
    let output = forestx_app_server_daemon::set_remote_control(mode).await?;
    println!("{}", serde_json::to_string(&output)?);
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = forestx_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Forestx's interactive TUI may not work in this terminal."
        );
    ...
    let start_tui = || {
        forestx_tui::run_main(
            interactive.clone(),
            arg0_paths.clone(),
            forestx_config::LoaderOverrides::default(),
        ...
        match local_state_db::backup_files_for_fresh_start(startup_error).await {
            ...
            Err(backup_err) => {
                ...
                return Ok(AppExitInfo::fatal(format!(
                    "failed to move damaged Forestx local database files into a backup folder automatically: {backup_err}"
                )));
            }
        }
    }
...
fn resolve_remote_endpoint(
    remote: Option<String>,
    remote_auth_token_env: Option<String>,
) -> std::io::Result<Option<forestx_tui::RemoteAppServerEndpoint>> {
    let mut remote_endpoint = remote
        .as_deref()
        .map(forestx_tui::resolve_remote_addr)
        .transpose()
    ...
    if let Some(remote_auth_token_env) = remote_auth_token_env {
        ...
        if !forestx_tui::remote_addr_supports_auth_token(endpoint) {
            return Err(std::io::Error::other(
        ...
        let auth_token = read_remote_auth_token_from_env_var(&remote_auth_token_env)
            .map_err(std::io::Error::other)?;
        let forestx_tui::RemoteAppServerEndpoint::WebSocket {
            auth_token: slot, ..
        } = endpoint
        else {
            return Err(std::io::Error::other(
...
/// Build the final `TuiCli` for a `forestx resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `forestx` without additional flags.
...
/// Build the final `TuiCli` for a `forestx fork` invocation.
...
mod tests {
    ...
    use forestx_tui::TokenUsage;
    use pretty_assertions::assert_eq;

    #[tokio::test]
    async fn updater_http_client_factory_honors_respect_system_proxy() {
        let forestx_home = tempfile::tempdir().expect("temporary Forestx home");
        let config = ConfigBuilder::default()
        ...
        assert_eq!(
            ...
            forestx_http_client::OutboundProxyPolicy::RespectSystemProxy
        );
    }
    ...
    fn exec_server_remote_auth_accepts_api_key_auth() {
        let auth = ForestxAuth::from_api_key("sk-test");
    ...
    #[test]
    fn profile_loader_overrides_use_explicit_forestx_home() -> anyhow::Result<()> {
        let forestx_home = tempfile::tempdir()?;
        let profile: ProfileV2Name = "work".parse()?;

        let overrides =
            loader_overrides_for_profile_at_forestx_home(Some(&profile), forestx_home.path());

        assert_eq!(
            overrides.user_config_path,
            Some(resolve_profile_v2_config_path(forestx_home.path(), &profile))
        );
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["forestx", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["forestx", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["forestx", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["forestx", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["forestx", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["forestx", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["forestx", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(forestx_exec::Command::Resume(args)) = exec.command else {
    ...
    fn approve_for_me_configures_interactive_mode() {
        for flag in ["--approve-for-me", "--not-so-yolo"] {
            let mut cli = MultitoolCli::try_parse_from(["forestx", flag]).expect("parse flag");
    ...
    fn not_so_yolo_alias_is_hidden_from_help() {
        for args in [&["forestx", "--help"][..], &["forestx", "exec", "--help"][..]] {
    ...
    fn approve_for_me_defaults_propagate_from_root_to_exec() {
        let exec = finalize_exec_from_args(&["forestx", "--approve-for-me", "exec", "summarize"]);
    ...
    fn later_exec_sandbox_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            exec.sandbox_mode,
            Some(forestx_utils_cli::SandboxModeCliArg::ReadOnly)
        );
        assert_eq!(
            exec.config_overrides.raw_overrides,
            vec![
    ...
    fn later_resume_approval_policy_partially_overrides_approve_for_me() {
        ...
        assert_matches!(
            ...
            Some(forestx_utils_cli::ApprovalModeCliArg::Never)
        );
        assert_eq!(
    ...
    fn approve_for_me_conflicts_with_explicit_interactive_permissions() {
        for conflicting_args in [
            ...
            let mut args = vec!["forestx", "--approve-for-me"];
    ...
    fn default_app_server_socket_path() -> AbsolutePathBuf {
        let forestx_home = find_forestx_home().expect("forestx home");
        forestx_app_server::app_server_control_socket_path(&forestx_home)
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["forestx", "plugin", "marketplace", "--help"]);
        assert!(
            help.contains("Usage: forestx plugin marketplace [OPTIONS] <COMMAND>"),
            "{help}"
        );

        for (subcommand, usage) in [
            ("add", "Usage: forestx plugin marketplace add"),
            ("list", "Usage: forestx plugin marketplace list"),
            ("upgrade", "Usage: forestx plugin marketplace upgrade"),
            ("remove", "Usage: forestx plugin marketplace remove"),
        ] {
            let help = help_from_args(&["forestx", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["forestx", "update"]).expect("parse");
    ...
    fn sandbox_help_only_shows_singular_permission_profile() {
        let help = help_from_args(&["forestx", "sandbox", "--help"]);
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["forestx", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["forestx", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["forestx", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["forestx", "marketplace", "remove", "debug"]);
    ...
    fn sample_exit_info(conversation_id: Option<&str>, thread_name: Option<&str>) -> AppExitInfo {
        ...
        AppExitInfo {
            token_usage,
            thread_id,
            resume_hint: forestx_utils_cli::resume_hint(thread_name, thread_id),
            update_action: None,
    ...
    fn format_exit_messages_includes_resume_hint_for_fatal_exit() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run forestx resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_names_picker_item_when_thread_has_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run forestx resume, then select my-thread (123e4567-e89b-12d3-a456-426614174000)".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["forestx", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["forestx", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["forestx", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["forestx", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["forestx", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["forestx", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["forestx", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["forestx", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["forestx", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags() {
        ...
        assert_matches!(
            ...
            Some(forestx_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(forestx_utils_cli::ApprovalModeCliArg::OnRequest)
        );
        assert_eq!(
            interactive.cwd.as_deref(),
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["forestx", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["forestx", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["forestx", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["forestx", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["forestx", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["forestx", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["forestx", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["forestx", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["forestx", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["forestx", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["forestx", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["forestx", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["forestx", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli =
            MultitoolCli::try_parse_from(["forestx", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["forestx", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["forestx", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["forestx", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `forestx mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["forestx", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `forestx remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["forestx", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `forestx app-server proxy`"
        );
    }

    #[test]
    fn reject_remote_flag_for_remote_control() {
        let cli = MultitoolCli::try_parse_from(["forestx", "--remote", "unix://", "remote-control"])
            .expect("parse");
    ...
    fn remote_control_pair_parses() {
        let cli = MultitoolCli::try_parse_from(["forestx", "remote-control", "pair"]).expect("parse");
        let Some(Subcommand::RemoteControl(remote_control)) = &cli.subcommand else {
            panic!("expected remote-control subcommand");
        };
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["forestx", "--remote", "unix://forestx.sock"])
            .expect("parse");
        assert_eq!(cli.remote.remote.as_deref(), Some("unix://forestx.sock"));
    }
    ...
    fn remote_auth_token_env_flag_parses_for_interactive_root() {
        ...
        assert_eq!(
            cli.remote.remote_auth_token_env.as_deref(),
            Some("FORESTX_REMOTE_AUTH_TOKEN")
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["forestx", "resume", "--remote", "unix://forestx.sock"])
                .expect("parse");
        ...
        assert_eq!(remote.remote.as_deref(), Some("unix://forestx.sock"));
    }
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("FORESTX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("FORESTX_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn psp_is_a_global_runtime_argument() {
        for args in [
            ["forestx", "--psp"].as_slice(),
            ["forestx", "app-server", "--psp"].as_slice(),
            ["forestx", "remote-control", "--psp"].as_slice(),
        ] {
    ...
    fn app_server_code_mode_host_url_parses_independently_of_listen_transport() {
        ...
        assert_eq!(
            app_server.listen,
            forestx_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_rejects_invalid_code_mode_host_urls() {
        for endpoint in [
            ...
            let error =
                MultitoolCli::try_parse_from(["forestx", "app-server", "--code-mode-host", endpoint])
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["forestx", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            forestx_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["forestx", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            forestx_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["forestx", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["forestx", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
            forestx_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["forestx", "app-server", "--listen", "unix:///tmp/forestx.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            forestx_app_server::AppServerTransport::UnixSocket {
                socket_path: AbsolutePathBuf::from_absolute_path("/tmp/forestx.sock")
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["forestx", "app-server", "--listen", "off"].as_ref());
        assert_eq!(app_server.listen, forestx_app_server::AppServerTransport::Off);
    }

    #[test]
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["forestx", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["forestx", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["forestx", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["forestx", "app-server", "daemon", "restart"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["forestx", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["forestx", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["forestx", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["forestx", "app-server", "daemon", "version"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["forestx", "app-server", "proxy", "--sock", "forestx.sock"].as_ref());
        ...
        assert_eq!(
            proxy.socket_path,
            Some(
                AbsolutePathBuf::relative_to_current_dir("forestx.sock")
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(forestx_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/forestx-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(forestx_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/forestx-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["forestx", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["forestx", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("forestx")

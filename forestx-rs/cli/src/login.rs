...
//! interactive-session layers. Direct `forestx login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `forestx-login.log` artifact that
...
use forestx_login::CLIENT_ID;
use forestx_login::ForestxAuth;
use forestx_login::ServerOptions;
use forestx_login::login_with_access_token;
use forestx_login::login_with_api_key;
use forestx_login::logout_with_revoke;
use forestx_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `forestx login` flows.
///
...
/// command produce a durable `forestx-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match forestx_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("forestx-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("forestx_cli=info,forestx_core=info,forestx_login=info"));
    ...
    // Direct `forestx login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `forestx login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | forestx login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv FORESTX_ACCESS_TOKEN | forestx login --with-access-token`.",
...
/// `forestx login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match ForestxAuth::from_auth_storage(
        &config.forestx_home,
        config.cli_auth_credentials_store_mode,
        Some(&config.chatgpt_base_url),
        config.auth_keyring_backend_kind(),
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        let forestx_home = tempdir().expect("create temporary Forestx home");
        login_with_api_key(
            forestx_home.path(),
            "sk-existing",

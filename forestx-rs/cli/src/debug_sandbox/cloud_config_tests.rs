...
use super::super::load_debug_sandbox_config_with_forestx_home;
...
async fn debug_sandbox_bootstraps_cloud_managed_permission_profile_from_backend() -> Result<()> {
    ...
    let cloud_config_bundle = bootstrap_cloud_config_bundle(
        &[],
        &options,
        || AbsolutePathBuf::from_absolute_path(forestx_home.path()),
        /*strict_config*/ false,
    )
    .await?;
    ...
    let cache: Value = serde_json::from_slice(&std::fs::read(
        forestx_home.path().join("cloud-config-bundle-cache.json"),
    )?)?;
    assert_eq!(
        json!({

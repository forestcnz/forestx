...
use forestx_core_plugins::ConfiguredMarketplace;
use forestx_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use forestx_core_plugins::PluginInstallOutcome;
...
use forestx_core_plugins::allowed_configured_marketplace_names;
use forestx_core_plugins::installed_marketplaces::marketplace_install_root;
use forestx_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use forestx_core_plugins::marketplace::MarketplaceListError;
use forestx_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use forestx_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use forestx_core_plugins::marketplace::MarketplacePluginSource;
use forestx_core_plugins::marketplace::find_marketplace_manifest_path;
use forestx_login::ForestxAuth;
use forestx_login::auth::read_forestx_api_key_from_env;
use forestx_plugin::PluginId;
use forestx_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(bin_name = "forestx plugin")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "forestx plugin add",
    after_help = "Examples:\n  forestx plugin add sample@debug\n  forestx plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "forestx plugin list",
    after_help = "Examples:\n  forestx plugin list\n  forestx plugin list --marketplace debug\n  forestx plugin list --json\n  forestx plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "forestx plugin remove",
    after_help = "Examples:\n  forestx plugin remove sample@debug\n  forestx plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplaces = outcome
        ...
        .collect::<Vec<_>>();
    let marketplace_sources = configured_marketplace_sources(&plugins_input, forestx_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    forestx_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    forestx_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    forestx_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<forestx_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, forestx_home);
...
async fn load_plugin_command_context(
    ...
    let manager = PluginsManager::new(forestx_home.to_path_buf());
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    if let Some(api_key) = read_forestx_api_key_from_env() {
        return Some(ForestxAuth::from_api_key(&api_key).api_auth_mode());
    }
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(forestx_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, forestx_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),

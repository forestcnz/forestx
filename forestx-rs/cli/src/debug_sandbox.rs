...
#[cfg(target_os = "macos")]
use forestx_core::spawn::FORESTX_SANDBOX_ENV_VAR;
...
use forestx_sandboxing::landlock::allow_network_for_proxy;
use forestx_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use forestx_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use forestx_sandboxing::seatbelt::create_seatbelt_command_args;
use forestx_sandboxing::with_managed_mitm_ca_readable_root;
...
pub async fn run_command_under_seatbelt(
    ...
    _forestx_linux_sandbox_exe: Option<PathBuf>,
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<forestx_mcp::SandboxState>)
        .transpose()
    ...
    let forestx_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .forestx_linux_sandbox_exe
                .clone()
                .or(forestx_linux_sandbox_exe)
        }
        None => forestx_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(forestx_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(FORESTX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let forestx_linux_sandbox_exe = config
                .forestx_linux_sandbox_exe
                .expect("forestx-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use forestx_windows_sandbox::WindowsSandboxProxySettingsMode;
    use forestx_windows_sandbox::WindowsSandboxSessionRequest;
    use forestx_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        forestx_home: config.forestx_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = forestx_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(FORESTX_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_forestx_home(
        cli_overrides,
        forestx_linux_sandbox_exe,
        options,
        /*forestx_home*/ None,
...
async fn load_debug_sandbox_config_with_forestx_home(
    cli_overrides: Vec<(String, TomlValue)>,
    forestx_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `forestx sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            forestx_linux_sandbox_exe: forestx_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        forestx_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(forestx_home) = forestx_home {
        builder = builder
            .forestx_home(forestx_home.clone())
            .fallback_cwd(Some(forestx_home));
    }
    builder.build().await
}
...
"#;
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        write_permissions_profile_config(&forestx_home, &docs, &private)?;
        let forestx_home_path = forestx_home.path().to_path_buf();

        let profile_config = build_debug_sandbox_config(
            Vec::new(),
            ConfigOverrides::default(),
            Some(forestx_home_path.clone()),
        ...
        let config = load_debug_sandbox_config_with_forestx_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = forestx_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_forestx_home(
            ...
            Some(forestx_home.path().to_path_buf()),
            CloudConfigBundleLoader::default(),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = forestx_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries

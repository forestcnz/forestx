...
use forestx_state::LogQuery;
use forestx_state::LogRow;
...
#[derive(Debug, Parser)]
#[command(name = "forestx-state-logs")]
#[command(about = "Tail Forestx logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to FORESTX_HOME. Defaults to $FORESTX_HOME or ~/.forestx.
    #[arg(long, env = "FORESTX_HOME")]
    forestx_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --forestx-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn resolve_sqlite_config(args: &Args) -> anyhow::Result<SqliteConfig> {
    ...
    if let Some(forestx_home) = args.forestx_home.as_ref() {
        config_builder = config_builder.forestx_home(forestx_home.clone());
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["forestx-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["forestx-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["forestx-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["forestx-state-logs", "--level", "WARN"])
            .expect("parse uppercase log level");

        assert_eq!(args.level, Some(LogLevelThreshold::Warn));
    }

    /// Explicit database selection must not parse an overridden Forestx home.
    #[tokio::test]
    async fn direct_db_skips_forestx_home_config() {
        let forestx_home = tempfile::tempdir().expect("create Forestx home");
        std::fs::write(forestx_home.path().join("config.toml"), "model = [")
            .expect("write invalid config");
        let sqlite_home = tempfile::tempdir().expect("create SQLite home");
        let db_path = sqlite_home.path().join("logs_2.sqlite");
        let args = Args::try_parse_from([
            OsString::from("forestx-state-logs"),
            OsString::from("--forestx-home"),
            forestx_home.path().as_os_str().to_owned(),
            OsString::from("--db"),

...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run Forestx after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
        conflicts_with = "current_user",
        requires = "forestx_home"
    )]
    user: Option<String>,

    /// Use the current Windows user as the Forestx user.
    #[arg(
        long = "current-user",
    ...
    /// FORESTX_HOME for the Forestx user. Required with --user.
    #[arg(long = "forestx-home", value_name = "DIR")]
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`forestx sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        .forestx_home(identity.forestx_home.clone())
        .fallback_cwd(Some(identity.forestx_home.clone()))
        .loader_overrides(super::loader_overrides_for_profile_at_forestx_home(
            config_profile.as_ref(),
            &identity.forestx_home,
        ))
        .cli_overrides(cli_overrides)
        .build()
        .await
        .context("failed to load target user's Forestx config for sandbox provisioning")?;

    forestx_core::windows_sandbox::run_elevated_provisioning_setup(
        identity.forestx_home.as_path(),
        identity.real_user.as_str(),
        config.permissions.network.as_ref(),
    )?;
    ConfigEditsBuilder::new(identity.forestx_home.as_path())
...
fn resolve_sandbox_setup_identity(
    ...
    if cmd.current_user {
        ...
        let forestx_home = match cmd.forestx_home.clone() {
            Some(forestx_home) => forestx_home,
            None => find_forestx_home()?.to_path_buf(),
        };
    ...
    let forestx_home = cmd
        .forestx_home
        .clone()
        .ok_or_else(|| anyhow::anyhow!("--forestx-home is required with --user"))?;
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            command.forestx_home.as_deref(),
            Some(std::path::Path::new(r"C:\Users\alice\.forestx"))
        );
    }
    ...
    #[test]
    fn requires_forestx_home_for_managed_user() {
        let err =
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            "--forestx-home".to_string(),
            r"C:\Users\alice\.forestx".to_string(),
        ])
        .expect("parse")

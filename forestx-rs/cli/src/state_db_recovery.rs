...
use forestx_state::RuntimeDbBackup;
use forestx_tui::LocalStateDbStartupError;
...
pub(crate) fn is_locked(detail: &str) -> bool {
    forestx_state::sqlite_error_detail_is_lock(detail)
}

pub(crate) fn is_corruption(detail: &str) -> bool {
    forestx_state::sqlite_error_detail_is_corruption(detail)
}
...
pub(crate) fn print_auto_backup_start(startup_error: &LocalStateDbStartupError) {
    eprintln!("Forestx couldn't start because its local database appears to be damaged.");
    eprintln!("Moving the damaged local database aside so Forestx can rebuild it from saved data.");
...
pub(crate) async fn backup_files_for_fresh_start(
    ...
    forestx_state::backup_runtime_db_for_fresh_start(startup_error.database_path()).await
}
...
pub(crate) fn confirm_fresh_start_rebuild(
    ...
    eprintln!("Forestx rebuilt its local database.");
    eprintln!(
        "Forestx detected a damaged local database, moved it into a backup folder, and will continue startup with a fresh database."
    );
...
pub(crate) fn print_diagnostic_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Forestx couldn't start because its local database appears to be damaged.");
    eprintln!("Run `forestx doctor` to check your setup and get next-step guidance.");
...
pub(crate) fn print_locked_guidance(startup_error: &LocalStateDbStartupError) {
    eprintln!("Forestx couldn't start because another Forestx process is using its local data.");
    eprintln!("Quit any other copies of Forestx that may still be running, then try again.");
    print_technical_details(startup_error);
}
...
mod tests {
    ...
    async fn backup_backs_up_only_failed_database_file() -> std::io::Result<()> {
        ...
        let sqlite = forestx_state::SqliteConfig::new_for_testing(temp_dir.path().abs());

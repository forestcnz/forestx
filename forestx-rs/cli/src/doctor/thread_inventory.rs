...
use forestx_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let scan = scan_rollout_files(forestx_home).await;
    ...
    let rows = match forestx_state::read_thread_state_audit_rows(sqlite).await {
        Ok(rows) => rows,
        Err(err) => {
    ...
    parity_check_from_scan_and_rows(forestx_home, scan, rows, details)
}
...
fn missing_state_db_check(scan: RolloutScan, details: Vec<String>) -> DoctorCheck {
    ...
    if !scan.files.is_empty() {
        check = check
            .issue(
                ...
                .remedy("Start Forestx with no state DB present so startup backfill can create it from rollout files."),
        )
            .remediation(
                "Start Forestx with no state DB present so startup backfill can create it from rollout files.",
            );
    }
    if !scan.scan_errors.is_empty() || !scan.malformed_names.is_empty() || scan.reached_scan_cap {
        check = check.issue(
            ...
            .remedy("Check file permissions and unexpected files under FORESTX_HOME sessions."),
        );
    }
    check
}
                ...
                let expected_archived = rollout_by_key
                    ...
                    .or_else(|| {
                        (scan
                            ...
                            .contains(&rollout_path_key(&row.rollout_path))
                            || row.rollout_path.is_file())
                        .then(|| archived_from_rollout_path(forestx_home, &row.rollout_path))
                        .flatten()
                    })?;
...
async fn scan_rollout_files(forestx_home: &Path) -> RolloutScan {
    let mut scan = RolloutScan::default();
    scan_rollout_root(
        &forestx_home.join("sessions"),
        /*archived*/ false,
        &mut scan,
    )
    .await;
    scan_rollout_root(
        &forestx_home.join("archived_sessions"),
        /*archived*/ true,
        &mut scan,
    )
    .await;
...
async fn scan_rollout_root(root: &Path, archived: bool, scan: &mut RolloutScan) {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            let logical_path = forestx_rollout::plain_rollout_path(&path);
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    let mut lines = match forestx_rollout::open_rollout_line_reader(path).await {
        Ok(lines) => lines,
    ...
    let logical_path = forestx_rollout::plain_rollout_path(path);
    forestx_rollout::builder_from_items(&[], &logical_path)
...
fn rollout_path_key(path: &Path) -> PathBuf {
    path_key(&forestx_rollout::plain_rollout_path(path))
}

fn archived_from_rollout_path(forestx_home: &Path, path: &Path) -> Option<bool> {
    let key = path_key(path);
    if key.starts_with(path_key(&forestx_home.join("archived_sessions"))) {
        return Some(true);
    }
    if key.starts_with(path_key(&forestx_home.join("sessions"))) {
        return Some(false);
    }
    None
}
...
mod tests {
    ...
    async fn thread_inventory_check_warns_for_missing_stale_and_mismatched_rows() {
        ...
        assert!(check.issues.iter().all(|issue| {
            !issue
                .remedy
                .as_deref()
                .is_some_and(|remedy| remedy.starts_with("Restart Forestx"))
        }));
    ...
    async fn thread_inventory_check_uses_compressed_metadata_id_and_legacy_filename_fallback() {
        ...
        let legacy_path = fixture.forestx_home.path().join(format!(
    ...
    async fn thread_inventory_check_ignores_compression_temp_files() {
        let fixture = Fixture::new().await;
        let temp_path = fixture.forestx_home.path().join(
    ...
    impl Fixture {
        async fn new() -> Self {
            let forestx_home = TempDir::new().expect("forestx home");
            let sqlite_home = TempDir::new().expect("sqlite home");
            let _runtime = forestx_state::StateRuntime::init(
        ...
        fn sqlite(&self) -> forestx_state::SqliteConfig {
            forestx_state::SqliteConfig::new_for_testing(self.sqlite_home.path().abs())
        }

        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            let root = if archived {
                self.forestx_home.path().join("archived_sessions")
            } else {
                self.forestx_home.path().join("sessions/2025/01/02")
            };
            ...
            let rollout_line = RolloutLine {
                ...
                ordinal: None,
                item: RolloutItem::SessionMeta(forestx_protocol::protocol::SessionMetaLine {
                    meta: forestx_protocol::protocol::SessionMeta {
                        session_id: parsed_thread_id.into(),
                        id: parsed_thread_id,
                        timestamp: timestamp.to_string(),
                        cwd: self.forestx_home.path().to_path_buf(),
...
INSERT INTO threads (
    ...
    archived_at
            ...
            .bind("cli")
            .bind("test-provider")
            .bind(self.forestx_home.path().display().to_string())
            .bind("test title")
            .bind("read-only")
            .bind("on-request")

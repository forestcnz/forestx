...
pub(super) async fn background_server_check(config: &Config) -> DoctorCheck {
    ...
    let state_dir = config.forestx_home.join(STATE_DIR_NAME);
    ...
    let socket_path = match forestx_app_server::app_server_control_socket_path(&config.forestx_home) {
    ...
    if status.check_status() == CheckStatus::Warning {
        check = check.remediation("Run forestx app-server daemon version for more details.");
    }
    check
}
...
async fn socket_status(socket_path: &Path) -> SocketStatus {
    ...
    match forestx_app_server_daemon::probe_app_server_version(socket_path).await {
...
mod tests {
    ...
    async fn test_config(forestx_home: PathBuf) -> Config {
    ...
    fn create_socket_placeholder(config: &Config) {
        let socket_path = forestx_app_server::app_server_control_socket_path(&config.forestx_home)
            .expect("socket path");

...
fn install_details(parsed: &[ParsedDetail], options: HumanOutputOptions) -> Vec<HumanDetail> {
    ...
    let path_entries = numbered_values(parsed, "PATH forestx #");
...
fn state_details(parsed: &[ParsedDetail]) -> Vec<HumanDetail> {
    let mut out = Vec::new();
    push_row_if_present(&mut out, parsed, "FORESTX_HOME", "FORESTX_HOME");
...
fn display_label(label: &str) -> String {
    match label {
        "forestx-linux-sandbox helper" => "linux helper",

...
fn write_footer(out: &mut String, options: HumanOutputOptions) {
    if options.show_details {
        ...
        let _ = writeln!(
            ...
            "{}",
            dim(
                "Run forestx doctor without --summary for detailed diagnostics.",
                options
            )
        );
        let _ = writeln!(
...
fn header_suffix(report: &DoctorReport) -> String {
    let version = format!("v{}", report.forestx_version);
    report
        .checks
        .iter()
...
fn update_note(check: &DoctorCheck, report: &DoctorReport) -> Option<DoctorNote> {
    ...
    let mut parenthetical = format!("current {}", report.forestx_version);
...
mod tests {
    ...
    fn sample_report() -> DoctorReport {
        let checks = vec![
            ...
            .detail("terminal title project value: forestx"),
            DoctorCheck::new(
                "state.paths",
                "state",
                CheckStatus::Ok,
    ...
    fn render_human_report_includes_details_by_default_without_color() {
        ...
        let expected = format!(
            "\
Forestx Doctor v0.0.0

Notes
   ⚠ terminal     narrow terminal
   ✗ auth         token expired - Run `forestx login`.
─────────────────────────────────────────────────────────────
...
Notes
   [!!] terminal     narrow terminal
   [XX] auth         token expired - Run `forestx login`.
-------------------------------------------------------------
...
Environment
  ...
  [!!] terminal     narrow terminal
  [ok] title        default | project forestx
...
",
    ...
    fn detail_value_colors_inline_statuses_and_low_signal_values() {
        let rendered = detail_value(
            "npm: no · commit unknown · integrity ok · ~/code/forestx/target/debug/forestx · <redacted>",
        ...
        assert!(rendered.contains("\u{1b}[38;5;117m~/code/forestx/target/debug/forestx"));
    ...
    fn redact_detail_preserves_env_var_names() {
        assert_eq!(
            redact_detail("auth env vars present: OPENAI_API_KEY, FORESTX_API_KEY"),
            "auth env vars present: OPENAI_API_KEY, FORESTX_API_KEY"
        );
    }

...
fn completed_imports_do_not_read_source_files() {
    let root = TempDir::new().expect("tempdir");
    let forestx_home = root.path().join("forestx-home");
    ...
    let ledger = super::load_import_ledger(&forestx_home).expect("ledger");
...
fn checkpoint_is_compare_and_swap_and_preserves_record_metadata() {
    ...
    let ledger_before = ledger_bytes(&forestx_home);
    ...
    assert_eq!(ledger_bytes(&forestx_home), ledger_before);
    ...
    let ledger = super::load_import_ledger(&forestx_home).expect("updated ledger");
...
fn connector_candidates_accumulate_imports_for_each_source() {
    ...
    assert_eq!(
        read_imported_connector_candidates(&forestx_home).expect("read connector candidates"),
        vec![
...
fn connector_candidates_accumulate_detected_and_imported_session_clues() {
    ...
    assert_eq!(
        read_imported_connector_candidates(&forestx_home).expect("read detected connectors"),
    ...
    assert_eq!(
        read_imported_connector_candidates(&forestx_home).expect("read imported connectors"),
        vec![
            ImportedConnectorCandidate {
                name: "Figma".to_string(),
                session_count: 1,
    ...
    assert_eq!(
        read_imported_connector_candidates(&forestx_home).expect("read appended connectors"),
        vec![
            ImportedConnectorCandidate {
                name: "Figma".to_string(),
                session_count: 1,
...
fn ledger_bytes(forestx_home: &Path) -> Vec<u8> {
    std::fs::read(super::import_ledger_path(forestx_home)).expect("ledger")
}

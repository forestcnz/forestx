...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(forestx_home)?.contains_current_source(source_path)
}
...
pub(crate) fn find_existing_session_import(
    ...
    let source_path = canonical_source_path(source_path)?;
    let ledger = load_import_ledger(forestx_home)?;
...
pub(super) fn checkpoint_existing_session_import(
    ...
    let mut ledger = load_import_ledger(forestx_home)?;
    ...
    save_import_ledger(forestx_home, &ledger)?;
    Ok(true)
}
...
pub(crate) fn load_import_ledger(
    ...
    let path = import_ledger_path(forestx_home);
...
fn import_ledger_path(forestx_home: &Path) -> PathBuf {
    forestx_home.join(SESSION_IMPORT_LEDGER_FILE)
}

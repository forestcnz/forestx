...
pub fn prepare_validated_session_import_with_metadata_mode(
    ...
    pending.target = match ledger::find_existing_session_import(forestx_home, &pending.source_path)? {
...
mod tests {
    ...
    fn skips_session_that_was_already_imported() {
        ...
        std::fs::write(&source_path, session_record(root.path(), "first request"))
            .expect("session");
        ledger::record_imported_session(&forestx_home, &source_path, ThreadId::new())
            .expect("record import");

        let pending =
            prepare_validated_session_import(&forestx_home, session_migration(&source_path))
    ...
    fn prepares_changed_session_for_its_unique_import() {
        ...
        ledger::record_imported_session(&forestx_home, &source_path, imported_thread_id)
            .expect("record import");

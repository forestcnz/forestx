...
fn migrated_quoted_hook_command(script_name: &str) -> String {
    let hook_path = Path::new("/repo/.forestx")
...
#[test]
fn subagent_requires_minimum_forestx_agent_fields() {
...
fn subagent_target_preserves_dotted_file_stem() {
    let target_agents = Path::new("/repo/.forestx/agents");
    let source_file = source_path("agents/security.audit.md");

    assert_eq!(
        subagent_target_file(&source_file, target_agents),
        Some(PathBuf::from("/repo/.forestx/agents/security.audit.toml"))
    );
}
...
fn hook_command_paths_rewrite_to_target_hook_dir() {
    ...
    assert_eq!(
        rewrite_hook_command_cla(&plugin_script_command, Some(Path::new("/repo/.forestx")),),
...
fn hook_script_copy_keeps_existing_target_scripts() {
    ...
    let target_config_dir = root.path().join(".forestx");

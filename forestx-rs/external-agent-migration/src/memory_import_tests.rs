...
fn copies_only_selected_projects_and_recopies_changed_content() {
    ...
    let forestx_home = root.path().join(".forestx");
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &all_files).expect("detect new memories"),
        BTreeSet::from(["project-a".to_string(), "project-b".to_string()])
    );
    let selected_memory = BTreeSet::from(["project-a"]);

    let outcome =
        copy_resources(&forestx_home, &all_files, &selected_memory).expect("copy project A");
    assert_eq!(outcome.synchronized_projects, vec!["project-a"]);
    assert_eq!(outcome.failures, Vec::new());
    assert_eq!(
        fs::read(resources_root(&forestx_home).join("project-a/MEMORY.md"))
            .expect("read project A memory"),
        b"project A memory".to_vec()
    );
    assert!(!resources_root(&forestx_home).join("project-b").exists());
    assert_eq!(
        projects_needing_import(&forestx_home, &all_files).expect("detect exact imported content"),
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &updated_files).expect("detect project file changes"),
        BTreeSet::from(["project-a".to_string(), "project-b".to_string()])
    );
    fs::write(project_a_memory.join("updated.md"), b"updated memory")
        .expect("write updated project A topic");
    let updated_files = discover_external_memory_files(&source_home).expect("rediscover memories");
    copy_resources(&forestx_home, &updated_files, &selected_memory)
    ...
    assert_eq!(
        fs::read(resources_root(&forestx_home).join("project-a/updated.md"))
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &changed_files).expect("detect changed memory"),
        BTreeSet::from(["project-a".to_string(), "project-b".to_string()])
    );
    let outcome =
        copy_resources(&forestx_home, &changed_files, &selected_memory).expect("recopy project A");
    assert_eq!(outcome.synchronized_projects, vec!["project-a"]);
    assert_eq!(outcome.failures, Vec::new());
    assert_eq!(
        fs::read(resources_root(&forestx_home).join("project-a/MEMORY.md"))
...
fn preserves_project_successes_and_reports_each_failed_selection() {
    ...
    let outcome = copy_resources(&forestx_home, &memory_files, &selected_memory)
...
fn removes_project_resources_when_the_source_project_disappears() {
    ...
    let selected_memory = BTreeSet::from(["project-a"]);
    let memory_files = discover_external_memory_files(&source_home).expect("discover memories");
    copy_resources(&forestx_home, &memory_files, &selected_memory).expect("copy project");
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &memory_files).expect("detect removed project"),
        BTreeSet::from(["project-a".to_string()])
    );
    assert_eq!(
        copy_resources(&forestx_home, &memory_files, &selected_memory)
    ...
    assert!(!resources_root(&forestx_home).join("project-a").exists());
}
...
fn uses_scope_file_to_identify_owned_projects() {
    let root = TempDir::new().expect("create tempdir");
    let forestx_home = root.path().join(".forestx");
    let resources_root = resources_root(&forestx_home);
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &[]).expect("detect removed projects"),
...
fn project_rename_removes_the_old_target_and_imports_the_new_target() {
    ...
    let memory_files = discover_external_memory_files(&source_home).expect("discover memories");
    copy_resources(&forestx_home, &memory_files, &project_a_selection).expect("copy project");
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &memory_files).expect("detect renamed project"),
        BTreeSet::from(["project-a".to_string(), "project-b".to_string()])
    );
    assert_eq!(
        copy_resources(&forestx_home, &memory_files, &selected_memory)
...
fn does_not_import_a_new_project_without_a_reliable_cwd() {
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &memory_files).expect("detect memories"),
        BTreeSet::new()
    );
    let outcome =
        copy_resources(&forestx_home, &memory_files, &selected_memory).expect("attempt copy");
    assert_eq!(
        outcome,
        MemoryImportOutcome {
...
fn missing_cwd_does_not_make_an_existing_scoped_project_look_deleted() {
    ...
    assert!(resources_root(&forestx_home).join("project-a").is_dir());
}
...
fn removes_an_existing_unscoped_target_when_cwd_is_unavailable() {
    ...
    let target_root = resources_root(&forestx_home).join("project-a");
    ...
    assert_eq!(
        projects_needing_import(&forestx_home, &memory_files).expect("detect unscoped target"),
        BTreeSet::from(["project-a".to_string()])
    );
    assert_eq!(
        copy_resources(&forestx_home, &memory_files, &selected_memory)

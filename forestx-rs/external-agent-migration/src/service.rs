...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "forestx.external_agent_config.import";

#[derive(Clone)]
pub struct ExternalAgentConfigService {
    pub(super) forestx_home: PathBuf,
...
impl ExternalAgentConfigService {
    ...
    pub fn forestx_home(&self) -> &Path {
        &self.forestx_home
    }

    #[cfg(test)]
    fn new_for_test(forestx_home: PathBuf, external_agent_home: PathBuf) -> Self {
        let source = ExternalAgentSource::default();
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            forestx_home,
            ...
            let import_result = match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Memory if self.source.supports_memory() => {
                    async {
                        ...
                        let target_path = memory_import::resources_root(&self.forestx_home);
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let target_config = match &scope {
            MigrationScope::Home => self.forestx_home.join("config.toml"),
            MigrationScope::Repository { root } => root.join(".forestx").join("config.toml"),
        };
    ...
    fn import_hooks(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        let target_hooks = match &scope {
            MigrationScope::Home => self.forestx_home.join("hooks.json"),
            MigrationScope::Repository { root } => root.join(".forestx").join("hooks.json"),
        };
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            ...
            MigrationScope::Home => {
                ...
                (source_agents_md, self.forestx_home.join("AGENTS.md"))
            }
        };
        ...
        let plugins = marketplace
            ...
            .filter(|plugin| {
                plugin
                    ...
                    .products
                    .as_deref()
                    .is_none_or(|products| Product::Forestx.matches_product_restriction(products))
            })

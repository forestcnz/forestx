...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "forestx.external_agent_config.detect";
...
impl ExternalAgentConfigService {
    pub async fn detect(
        ...
        if params.include_home
            ...
            && let Some(item) = memory::detect(&self.forestx_home, &self.external_agent_home)?
        {
            items.push(item);
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.forestx_home.join("config.toml"),
            |repo_root| repo_root.join(".forestx").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.forestx_home.join("hooks.json"),
            |repo_root| repo_root.join(".forestx").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            ...
            |repo_root| repo_root.join(".forestx").join("agents"),
        );
        ...
        if self.source.supports_plugin_migration(settings.as_ref()) {
            match ConfigBuilder::default()
                ...
                .fallback_cwd(Some(self.forestx_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.forestx_home.clone()),
                    )?;

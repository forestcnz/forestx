...
async fn import_repo_mcp_preserves_existing_same_named_server() {
    ...
    .expect("write mcp");
    fs::create_dir_all(repo_root.join(".forestx")).expect("create forestx dir");
    let existing_config = r#"[mcp_servers.mixedTransport]
url = "https://example.com/mixed-transport"
"#;
    fs::write(
        repo_root.join(".forestx").join("config.toml"),
        existing_config,
    )
    .expect("write config");
    ...
    assert_eq!(
        fs::read_to_string(repo_root.join(".forestx").join("config.toml")).expect("read config"),
        existing_config
    );
}
    ...
    assert_eq!(
        ...
        vec![ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                repo_root.display(),
                repo_root.join(".forestx").join("config.toml").display()
            ),
            cwd: Some(repo_root),
...
async fn import_home_migrates_supported_config_fields_skills_and_agents_md() {
    let (_root, external_agent_home, forestx_home) = fixture_paths();
    let agents_skills = forestx_home
        .parent()
    ...
    fs::write(
            ...
            format!(r#"{{"model":"{SOURCE_EXTERNAL_AGENT_NAME}","permissions":{{"ask":["git push"]}},"env":{{"FOO":"bar","CI":false,"MAX_RETRIES":3,"MY_TEAM":"forestx","IGNORED":null,"LIST":["a","b"],"MAP":{{"x":1}}}},"sandbox":{{"enabled":true,"network":{{"allowLocalBinding":true}}}}}}"#),
        )
    ...
    service_for_paths(external_agent_home, forestx_home.clone())
        .import(vec![
    ...
    assert_eq!(
        fs::read_to_string(forestx_home.join("AGENTS.md")).expect("read agents"),
        "Forestx guidance"
    );

    let config: TomlValue =
        toml::from_str(&fs::read_to_string(forestx_home.join("config.toml")).expect("read config"))
            .expect("parse config");
...
"#,
    ...
    assert_eq!(
        ...
        "Use Forestx and Forestx utilities."
    );
}
...
async fn import_home_config_ignores_invalid_local_settings() {
    ...
    assert_eq!(
        fs::read_to_string(forestx_home.join("config.toml")).expect("read config"),
        "[shell_environment_policy]\ninherit = \"core\"\n\n[shell_environment_policy.set]\nFOO = \"project\"\n"
    );
}
...
async fn import_home_skips_empty_config_migration() {
    ...
    let outcome = service_for_paths(external_agent_home, forestx_home.clone())
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::Config,
            description: String::new(),
            cwd: None,
    ...
    assert!(!forestx_home.join("config.toml").exists());
}
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("create plugin manifest dir");
    ...
    fs::write(
        plugin_root.join(".forestx-plugin").join("plugin.json"),
    ...
    let config = fs::read_to_string(forestx_home.join("config.toml")).expect("read config");
...
async fn detect_home_skips_config_when_target_already_has_supported_fields() {
    ...
    let items = service_for_paths(external_agent_home, forestx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            include_memory: false,
            cwds: None,

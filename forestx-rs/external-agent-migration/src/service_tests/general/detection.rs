...
async fn detect_home_lists_config_skills_and_agents_md() {
    ...
    let items = service_for_paths(external_agent_home.clone(), forestx_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                forestx_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                forestx_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_cursor_home_lists_user_and_managed_skills() {
    ...
    let target_skills = forestx_home
        .parent()
        .map(|parent| parent.join(".agents").join("skills"))
        .unwrap_or_else(|| PathBuf::from(".agents").join("skills"));
    ...
    let mut service = service_for_paths(external_agent_home, forestx_home);
...
async fn detect_cursor_repo_lists_skills_from_skills() {
    ...
    let mut service = service_for_paths(root.path().join(".cursor"), forestx_home);
    service.source = ExternalAgentSource::Cur;
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, forestx_home) = fixture_paths();
    ...
    let items = service_for_paths(external_agent_home.clone(), forestx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: true,
            include_memory: false,
            cwds: None,
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    fs::write(forestx_home.join("config.toml"), "this is not valid = [toml")
        .expect("write invalid forestx config");
    fs::write(
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), forestx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
            cwds: Some(vec![repo_root.clone()]),
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".forestx").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".forestx").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        forestx_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".forestx").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: forestx_config::HooksFile =
...
Research with Forestx carefully."""
"#,
    )

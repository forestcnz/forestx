...
fn marketplace_import_sources_prefers_scoped_source_over_registry_name_collision() {
    let (root, external_agent_home, _forestx_home) = fixture_paths();
...
fn marketplace_import_sources_prefers_supported_declaration_over_materialization() {
    let (_root, external_agent_home, _forestx_home) = fixture_paths();
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_already_configured_in_forestx() {
...
#[tokio::test]
async fn detect_repo_skips_plugins_that_are_disabled_in_forestx() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let forestx_home = root.path().join(".forestx");
    let repo_root = root.path().join("repo");
...
#[tokio::test]
async fn detect_repo_skips_plugins_without_explicit_enabled_in_forestx() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let forestx_home = root.path().join(".forestx");
    let repo_root = root.path().join("repo");
...
async fn import_plugins_requires_details() {
    ...
    let err = service_for_paths(external_agent_home, forestx_home)
...
#[tokio::test]
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_forestx() {
    let root = TempDir::new().expect("create tempdir");
    let external_agent_home = root.path().join(EXTERNAL_AGENT_DIR);
    let forestx_home = root.path().join(".forestx");
    let repo_root = root.path().join("repo");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".forestx")).expect("create repo forestx dir");
    fs::create_dir_all(&forestx_home).expect("create forestx home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
...
"#,
    ...
    .expect("write project forestx config");
...
async fn detect_repo_filters_plugins_against_installed_marketplace() {
    ...
    let forestx_home = root.path().join(".forestx");
    let repo_root = root.path().join("repo");
    let marketplace_root = forestx_home.join(".tmp").join("marketplaces").join("debug");
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");

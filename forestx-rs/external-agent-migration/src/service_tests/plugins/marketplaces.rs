...
async fn import_plugins_requires_source_marketplace_details() {
    ...
    let outcome = service_for_paths(external_agent_home, forestx_home)
        .import_plugins(
            /*cwd*/ None,
...
async fn import_plugins_reuses_configured_marketplace_with_different_source() {
    ...
    fs::create_dir_all(configured_plugin_root.join(".forestx-plugin"))
    ...
    fs::create_dir_all(source_plugin_root.join(".forestx-plugin"))
...
"#
    ...
    fs::write(
        source_plugin_root.join(".forestx-plugin/plugin.json"),
...
fn import_skills_returns_only_new_skill_directory_names() {
    ...
    let copied_names = service_for_paths(external_agent_home, forestx_home)

...
- For each project, always read its source `MEMORY.md` first when it exists. Use it to seed or update that project's scoped entry in Forestx `MEMORY.md`, and add only the smallest broadly useful route to `memory_summary.md`.
...
- Keep source-specific frontmatter in the imported resource. Do not reinterpret fields such as `metadata.originSessionId` as a Forestx `thread_id`, `rollout_path`, or `updated_at`.
- Treat every other source `*.md` file as detailed supporting evidence analogous to a rollout summary. Do not flatten its full contents into Forestx `MEMORY.md` or `memory_summary.md`. Keep the detail in the imported resource, add a concise pointer from the scoped `MEMORY.md` entry when useful, and read the resource progressively when a later task needs that topic.
- Preserve this hierarchy after migration: Forestx `MEMORY.md` is the searchable routing layer, `memory_summary.md` is the compact global index, and non-`MEMORY.md` imported resources are progressive-disclosure detail.
...
pub(super) async fn import(
    ...
    let state_db = state_db.ok_or_else(|| {
        io::Error::new(
            ...
            "memory import requires the Forestx state database",
        )
    })?;
    let memory_root = forestx_home.join("memories");
    forestx_memories_write::workspace::prepare_memory_workspace(&memory_root)
        .await
        .map_err(io::Error::other)?;
    let memory_files = discover_external_memory_files(external_agent_home)?;
    let copy_outcome = copy_resources(forestx_home, &memory_files, &selected_memory)?;
...
pub(crate) fn projects_needing_import(
    ...
    for (&project_key, project_files) in &files_by_project {
        let Some(project_cwd) = project_cwd(project_files) else {
            if project_has_unscoped_target(forestx_home, project_key)? {
                projects.insert(project_key.to_string());
            }
            continue;
        };
        if project_needs_import(forestx_home, project_key, project_cwd, project_files)? {
            projects.insert(project_key.to_string());
        }
    }
    projects.extend(
        owned_project_keys(forestx_home)?
...
fn copy_resources(
        ...
        let sync_result = match files_by_project.get(project_key) {
            Some(project_files) => {
                if let Some(project_cwd) = project_cwd(project_files) {
                    replace_project_resources(forestx_home, project_key, project_cwd, project_files)
                        .map(|()| true)
                } else if project_has_unscoped_target(forestx_home, project_key)? {
                    remove_project_resources(forestx_home, project_key)
                } else {
            ...
            None => remove_project_resources(forestx_home, project_key),
        };
        match sync_result {
            Ok(true) => {
    ...
    if !synchronized_projects.is_empty() {
        let instructions_path = extension_root(forestx_home).join("instructions.md");
...
fn owned_project_keys(forestx_home: &Path) -> io::Result<BTreeSet<String>> {
    let entries = match fs::read_dir(resources_root(forestx_home)) {
        Ok(entries) => entries,
...
fn project_has_unscoped_target(forestx_home: &Path, project_key: &str) -> io::Result<bool> {
    let target_root = resources_root(forestx_home).join(project_key);
...
fn project_needs_import(
    ...
    for memory_file in memory_files {
        ...
        if fs::read(resource_path(forestx_home, memory_file))
            .ok()
            .as_deref()
    ...
    if fs::read(project_scope_path(forestx_home, project_key))
        .ok()
        .as_deref()
...
fn remove_project_resources(forestx_home: &Path, project_key: &str) -> io::Result<bool> {
    let target_root = resources_root(forestx_home).join(project_key);
...
fn resource_path(forestx_home: &Path, memory_file: &ExternalMemoryFile) -> PathBuf {
    resources_root(forestx_home)
        .join(&memory_file.project_key)
        .join(&memory_file.relative_path)
}

fn project_scope_path(forestx_home: &Path, project_key: &str) -> PathBuf {
    resources_root(forestx_home)
        .join(project_key)
...
fn extension_root(forestx_home: &Path) -> PathBuf {
    forestx_home
        .join("memories")
        .join("extensions")
        .join(EXTENSION_NAME)
...
pub(super) fn resources_root(forestx_home: &Path) -> PathBuf {
    extension_root(forestx_home).join("resources")
}

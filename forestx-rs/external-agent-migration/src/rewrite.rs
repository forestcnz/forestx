...
impl RewriteProfile {
    ...
    /// Rewrites source-specific documentation names and product terms to their Forestx forms.
    pub fn rewrite(self, content: &str) -> String {
        ...
        for from in self.term_variants {
            rewritten = replace_case_insensitive_with_boundaries(&rewritten, from, "Forestx");
        }
        for from in self.case_sensitive_term_variants {
            rewritten = replace_with_boundaries(&rewritten, from, "Forestx");
        }
        rewritten
    }
}

...
pub enum Feature {
    ...
    Chronicle,
    /// Compress request bodies (zstd) when sending streaming requests to forestx-backend.
    EnableRequestCompression,
    /// Start the managed network proxy for sandboxed sessions.
    NetworkProxy,
    /// Respect host system proxy settings for Forestx-owned network clients.
    RespectSystemProxy,
    /// Enable collab tools.
    Collab,
    ...
    /// Prompt Forestx Apps connector auth failures through MCP URL elicitations.
    AuthElicitation,
    ...
    match feature {
        ...
        _ => {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://developers.openai.com/forestx/config-basic#feature-flags for details."
                ))
            };
            (summary, details)
        }
...
pub const FEATURES: &[FeatureSpec] = &[
    ...
    FeatureSpec {
        id: Feature::ForestxGitCommit,
        key: "forestx_git_commit",
        stage: Stage::Removed,
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: Stage::Experimental {
            ...
            announcement: "NEW: Network proxy can now be enabled from /experimental. Restart Forestx after enabling it.",
        },
        default_enabled: false,
    },
    FeatureSpec {
    ...
    FeatureSpec {
        ...
        stage: if cfg!(any(
            ...
            Stage::Experimental {
                ...
                menu_description: "Keep your computer awake while Forestx is running a thread.",

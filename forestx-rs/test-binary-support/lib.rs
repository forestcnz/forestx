    ...
    F: FnOnce(&str, Option<&str>) -> TestBinaryDispatchMode,
{
    ...
    match classify(exe_name, argv1.as_deref().and_then(|arg| arg.to_str())) {
        ...
        TestBinaryDispatchMode::InstallAliases => {
            let forestx_home = match tempfile::Builder::new().prefix(forestx_home_prefix).tempdir() {
                Ok(forestx_home) => forestx_home,
                Err(error) => panic!("failed to create test FORESTX_HOME: {error}"),
            };
            let previous_forestx_home = std::env::var_os("FORESTX_HOME");
            // Safety: this runs from a test ctor before test threads begin.
            unsafe {
                std::env::set_var("FORESTX_HOME", forestx_home.path());
            }
            ...
            match previous_forestx_home.as_ref() {
                Some(value) => unsafe {
                    std::env::set_var("FORESTX_HOME", value);
                },
                None => unsafe {
            ...
            Some(TestBinaryDispatchGuard {
                ...
                arg0,
                _previous_forestx_home: previous_forestx_home,
            })
        }
    }
}

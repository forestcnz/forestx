...
/// The easiest way to iterate is to add unit tests and have Forestx fix the implementation.
...
/// Tests are at the top to encourage using TDD + Forestx to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "FORESTX_SANDBOX_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R FORESTX_SANDBOX_ENV_VAR -n .".to_string(),
                query: Some("FORESTX_SANDBOX_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            &vec_str(&[
                "grep",
                "-R",
                "FORESTX_SANDBOX_ENV_VAR",
                "-n",
                "core/src/spawn.rs",
            ]),
            vec![ParsedCommand::Search {
                cmd: "grep -R FORESTX_SANDBOX_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("FORESTX_SANDBOX_ENV_VAR".to_string()),
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd forestx-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner = r#"cd /Users/pakrym/code/forestx && rg -n "forestx_api" forestx-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n forestx_api forestx-rs -S".to_string(),
                query: Some("forestx_api".to_string()),
                path: Some("forestx-rs".to_string()),
            }],
        );
    }

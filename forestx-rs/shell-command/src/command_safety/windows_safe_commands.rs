...
mod tests {
    ...
    fn rejects_git_subcommand_options_with_side_effects() {
        let results: Vec<(&str, bool)> = [
            "git diff --output forestx_poc.txt",
            "git diff --ext-diff HEAD",
            "git log --textconv -1",
            "git show --output=forestx_poc.txt HEAD",
        ...
        assert_eq!(
            vec![
                ("git diff --output forestx_poc.txt", false),
                ("git diff --ext-diff HEAD", false),
                ("git log --textconv -1", false),
                ("git show --output=forestx_poc.txt HEAD", false),
    ...
    fn rejects_stop_parsing_git_forms() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "git log --% HEAD --output=forestx_poc.txt",
        ])));
    }
    ...
    fn rejects_powershell_named_blocks() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "begin { Set-Content forestx_poc.txt pwned } end { Get-Content Cargo.toml }",
        ])));
    }
    ...
    fn rejects_powershell_trap_blocks() {
        assert!(!is_safe_command_windows(&vec_str(&[
            ...
            "-NoProfile",
            "-Command",
            "trap { Set-Content forestx_poc.txt pwned; continue } Get-Content missing -ErrorAction Stop",
        ])));
    }

...
mod tests {
    ...
    fn cache_context(forestx_home: &TempDir, id: &str) -> ConnectorDirectoryCacheContext {
        ConnectorDirectoryCacheContext::new(forestx_home.path().to_path_buf(), cache_key(id))
    }
    ...
    async fn list_all_connectors_uses_shared_directory_cache() -> anyhow::Result<()> {
        ...
        let cache_context = cache_context(&forestx_home, "shared");
    ...
    async fn list_all_connectors_merges_and_normalizes_directory_apps() -> anyhow::Result<()> {
        ...
        let forestx_home = TempDir::new()?;
        let cache_context = cache_context(&forestx_home, "merged");
        let calls = Arc::new(AtomicUsize::new(0));
        let call_counter = Arc::clone(&calls);
    ...
    async fn list_all_connectors_overlaps_workspace_and_directory_requests() -> anyhow::Result<()> {
        ...
        let forestx_home = TempDir::new()?;
        let cache_context = cache_context(&forestx_home, "overlap");
    ...
    async fn cached_directory_connectors_reads_directory_disk_cache() -> anyhow::Result<()> {
        ...
        let forestx_home = TempDir::new()?;
        let cache_context = cache_context(&forestx_home, "disk");
        let calls = Arc::new(AtomicUsize::new(0));
        let call_counter = Arc::clone(&calls);
    ...
    -> anyhow::Result<()> {
        ...
        let forestx_home = TempDir::new()?;
        let cache_context = cache_context(&forestx_home, "disk-refresh");
        let calls = Arc::new(AtomicUsize::new(0));
        let call_counter = Arc::clone(&calls);
    ...
    async fn cached_directory_connectors_drops_stale_disk_schema() -> anyhow::Result<()> {
        ...
        clear_directory_memory_cache();
        let forestx_home = TempDir::new()?;
        let cache_context = cache_context(&forestx_home, "stale-schema");

use super::persistence::FORESTX_APPS_TOOLS_CACHE_MAX_BYTES;
use super::persistence::FORESTX_APPS_TOOLS_CACHE_SCHEMA_VERSION;
use super::persistence::read_cached_forestx_apps_tools;
use super::persistence::write_cached_forestx_apps_tools;
use super::persistence::write_cached_forestx_apps_tools_for_test;
use super::*;
...
const FORESTX_APPS_MCP_SERVER_NAME: &str = "forestx_apps";
...
#[test]
fn forestx_apps_tools_cache_is_overwritten_by_last_write() {
    ...
    let tools_gateway_1 = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "two")];

    write_cached_forestx_apps_tools(&cache_context, &tools_gateway_1).expect("write first cache");
    let cached_gateway_1 =
        read_cached_forestx_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].callable_name, "one");

    write_cached_forestx_apps_tools(&cache_context, &tools_gateway_2).expect("write second cache");
    let cached_gateway_2 =
        read_cached_forestx_apps_tools(&cache_context).expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].callable_name, "two");
}

#[test]
fn forestx_apps_tools_cache_is_scoped_per_user() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "two")];

    write_cached_forestx_apps_tools(&cache_context_user_1, &tools_user_1)
        .expect("write user one cache");
    write_cached_forestx_apps_tools(&cache_context_user_2, &tools_user_2)
        .expect("write user two cache");

    let read_user_1 =
        read_cached_forestx_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_forestx_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn forestx_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_forestx_apps_tools(&cache_context, &tools).expect("write cache");
    let cached = read_cached_forestx_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn forestx_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": FORESTX_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_forestx_apps_tools(&cache_context).is_none());
}

#[test]
fn forestx_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_forestx_apps_tools_loads_from_disk_cache() {
    let forestx_home = tempdir().expect("tempdir");
    let writer_cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    write_cached_forestx_apps_tools_for_test(&writer_cache_context, &server_info, &cached_tools);
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    ...
    assert_eq!(startup_tools[0].server_name, FORESTX_APPS_MCP_SERVER_NAME);
    assert_eq!(startup_tools[0].callable_name, "calendar_search");
    assert_eq!(cached_server_info, Some(server_info));
}

#[test]
fn startup_cached_forestx_apps_tools_loads_without_server_info_cache() {
    let forestx_home = tempdir().expect("tempdir");
    let writer_cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": FORESTX_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        "tools": [create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
    let cache_context = create_forestx_apps_tools_cache_context(
...
#[test]
fn forestx_apps_server_info_cache_survives_legacy_tools_cache_write() {
    let forestx_home = tempdir().expect("tempdir");
    let cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let server_info = create_test_server_info("Forestx Apps");
    write_cached_forestx_apps_tools_for_test(
        &cache_context,
        &server_info,
        &[create_test_tool(
            FORESTX_APPS_MCP_SERVER_NAME,
    ...
    let startup_cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
#[test]
fn forestx_apps_tools_cache_context_does_not_reread_disk_after_creation() {
    let forestx_home = tempdir().expect("tempdir");
    let writer_cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached_tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "cached")];
    write_cached_forestx_apps_tools(&writer_cache_context, &cached_tools).expect("write cache");
    let reader_cache_context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let updated_tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "updated")];
    write_cached_forestx_apps_tools(&writer_cache_context, &updated_tools).expect("rewrite cache");
    ...
    assert_eq!(
        read_cached_forestx_apps_tools(&writer_cache_context).expect("disk tools")[0].callable_name,
        "updated"
    );
}

#[test]
fn forestx_apps_tools_cache_publishes_newest_shared_snapshot() {
    let forestx_home = tempdir().expect("tempdir");
    ...
    let server_info = create_test_server_info("Forestx Apps");
    let newer_tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "newer")];
    let older_tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "older")];
    ...
    assert_eq!(
        read_cached_forestx_apps_tools(&cache_context_1).expect("persisted snapshot")[0]
            .callable_name,
        "newer"
    );
}
...
#[test]
fn forestx_apps_tools_cache_keeps_live_publish_when_disk_persistence_fails() {
    let forestx_home = tempdir().expect("tempdir");
    let forestx_home_file = forestx_home.path().join("not-a-directory");
    std::fs::write(&forestx_home_file, b"occupied").expect("create forestx home file");
    ...
    let tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "live")];
...
fn connector_runtime_without_cache_ignores_disk_state() {
    let forestx_home = tempdir().expect("tempdir");
    let writer = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "cached")];
    let server_info = create_test_server_info("Forestx Apps");
    write_cached_forestx_apps_tools_for_test(&writer, &server_info, &tools);
...
fn connector_runtime_without_cache_publishes_without_writing() {
    ...
    assert!(!forestx_home.exists());
}

#[cfg(unix)]
#[test]
fn forestx_apps_tools_cache_scopes_non_utf8_home_disk_paths() {
    let forestx_home = PathBuf::from(std::ffi::OsString::from_vec(
        b"/tmp/forestx-home-\xff".to_vec(),
    ));
    let cache = ConnectorRuntimeManager::<TestTool>::default();
...
fn contexts_for_different_identities_keep_isolated_snapshots() {
    ...
    let tools_a = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "tool-a")];
    ...
    let tools_b = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "tool-b")];
    let snapshot_b = context_b.publish_runtime_if_newest_accepted(
        context_b.begin_fetch(ConnectorRuntimeFetchSource::HardRefresh),
        &create_test_server_info("Forestx Apps"),
        tools_b.clone(),
    );
    let newer_tools_a = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "newer-a")];
    ...
    let stale_snapshot_a = context_a.publish_runtime_if_newest_accepted(
        older_ticket_a,
        &create_test_server_info("Forestx Apps"),
        vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "stale-a")],
    );
...
fn oversized_tools_cache_is_ignored_during_initial_load() {
    let forestx_home = tempdir().expect("tempdir");
    let context = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    ...
    file.set_len(FORESTX_APPS_TOOLS_CACHE_MAX_BYTES + 1)
        .expect("size oversized cache");

    let reloaded = create_forestx_apps_tools_cache_context(
        forestx_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
...
fn cold_loaded_snapshot_uses_cache_modification_time() {
    ...
    let tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "cached")];
    write_cached_forestx_apps_tools(&writer, &tools).expect("write tools cache");
...
fn accepted_generations_finish_persistence_in_order() {
    ...
    let older_publish = std::thread::spawn(move || {
        older_context.publish_runtime_if_newest_accepted_with(
            older_ticket,
            &create_test_server_info("Forestx Apps"),
            vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "older")],
            move |_, _, _| {
                older_persisting_tx
                    .send(())
    ...
    let newer_publish = std::thread::spawn(move || {
        newer_context.publish_runtime_if_newest_accepted_with(
            newer_ticket,
            &create_test_server_info("Forestx Apps"),
            vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "newer")],
            move |_, _, _| {
                newer_persisting_tx
                    .send(())
...
fn personal_and_workspace_contexts_are_distinct_even_with_matching_ids() {
    ...
    let personal_tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "personal")];
    ...
    let workspace_tools = vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "workspace")];
...
fn live_publish_sets_timestamp_and_stale_publish_preserves_it() {
    ...
    let current = context.publish_runtime_if_newest_accepted(
        current_ticket,
        &create_test_server_info("Forestx Apps"),
        vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "current")],
    );
    ...
    let stale = context.publish_runtime_if_newest_accepted(
        stale_ticket,
        &create_test_server_info("Forestx Apps"),
        vec![create_test_tool(FORESTX_APPS_MCP_SERVER_NAME, "stale")],
    );

# forestx-responses-api-proxy

#### tl;dr:

```
# Launch the proxy, dump request/response pairs to /tmp/proxy
cd path/to/forestx/forestx-rs
cargo build
echo $OPENAI_API_KEY | ./target/debug/forestx-responses-api-proxy \
    --port 60001 \
    --dump-dir /tmp/proxy


# Add this to ~/.forestx/config.toml:

[model_providers.forestx-responses-api-proxy]
name = 'forestx-responses-api-proxy'
base_url = 'http://127.0.0.1:60001/v1'
wire_api='responses'

[profiles.proxy]
model_provider = "forestx-responses-api-proxy"
...
**IMPORTANT:** `forestx-responses-api-proxy` is designed to be run by a privileged user with access to `OPENAI_API_KEY` so that an unprivileged user cannot inspect or tamper with the process. Though if `--http-shutdown` is specified, an unprivileged user _can_ make a `GET` request to `/shutdown` to shutdown the server, as an unprivileged user could not send `SIGTERM` to kill the process.

A privileged user (i.e., `root` or a user with `sudo`) who has access to `OPENAI_API_KEY` would run the following to start the server, as `forestx-responses-api-proxy` reads the auth token from `stdin`:

```shell
printenv OPENAI_API_KEY | env -u OPENAI_API_KEY forestx-responses-api-proxy --http-shutdown --server-info /tmp/server-info.json
```

A non-privileged user would then run Forestx as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
forestx exec -c "model_providers.openai-proxy={ name = 'OpenAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Reads the API key from `stdin`. All callers should pipe the key in (for example, `printenv OPENAI_API_KEY | forestx-responses-api-proxy`).
...
```
forestx-responses-api-proxy [--port <PORT>] [--server-info <FILE>] [--http-shutdown] [--upstream-url <URL>] [--dump-dir <DIR>]
```
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the Forestx CLI expectations.

For Azure, for example (ensure your deployment accepts `Authorization: Bearer <key>`):

```shell
printenv AZURE_OPENAI_API_KEY | env -u AZURE_OPENAI_API_KEY forestx-responses-api-proxy \
  --http-shutdown \
...
- We leverage [`forestx_process_hardening`](https://github.com/openai/forestx/blob/main/forestx-rs/process-hardening/README.md) so `forestx-responses-api-proxy` is run with standard process-hardening techniques.

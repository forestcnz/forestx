...
`forestx-http-client` is the low-level HTTP transport shared by Forestx crates. It is the intended
...
- custom CA handling through `FORESTX_CA_CERTIFICATE` and `SSL_CERT_FILE`;
...
feature requires more than enabling reqwest's default proxy behavior: Forestx must resolve platform
...
Higher-level retry, SSE, and request-attempt telemetry policy remains in `forestx-client`.
...
would skip Forestx's route selection for the redirect target. In `RespectSystemProxy` mode the pool
...
```rust
use forestx_http_client::ClientRouteClass;
use forestx_http_client::ReqwestTransport;

let client = http_client_factory.build_client(api_url, ClientRouteClass::Api)?;
let transport = ReqwestTransport::from_http_client(client);
```

If the existing wrapper surface cannot support a use case, extend `forestx-http-client` rather than

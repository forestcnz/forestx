...
//!
//! The shared reqwest client honors `FORESTX_CA_CERTIFICATE` and `SSL_CERT_FILE`, but those
...
//!
//! The detailed explanation of what "hermetic" means here lives in `forestx_http_client::custom_ca`.
//! This binary exists so the tests can exercise
//! [`forestx_http_client::build_reqwest_client_for_subprocess_tests`] in a separate process without
...
fn build_probe_client(
    ...
    if let Some(proxy_url) = proxy_url {
        ...
        return forestx_http_client::build_reqwest_client_with_custom_ca(builder.proxy(proxy))
            .map_err(|error| error.to_string());
    }

    forestx_http_client::build_reqwest_client_for_subprocess_tests(builder)
        .map_err(|error| error.to_string())
}

...
/// Adds the process-local ChatGPT Cloudflare cookie jar used by Forestx HTTP clients.
///
...
mod tests {
    ...
    fn stores_and_returns_cloudflare_cookies_for_chatgpt_hosts() {
        ...
        let url = reqwest::Url::parse("https://chatgpt.com/backend-api/forestx/responses").unwrap();
    ...
    fn does_not_return_chatgpt_cloudflare_cookies_for_other_hosts() {
        let store = ChatGptCloudflareCookieStore::default();
        let chatgpt_url =
            reqwest::Url::parse("https://chatgpt.com/backend-api/forestx/responses").unwrap();
    ...
    fn rejects_plain_http_chatgpt_cookie_urls() {
        let store = ChatGptCloudflareCookieStore::default();
        let http_url = reqwest::Url::parse("http://chatgpt.com/backend-api/forestx/responses")
            .expect("URL should parse");
        let https_url = reqwest::Url::parse("https://chatgpt.com/backend-api/forestx/responses")
            .expect("URL should parse");
        let set_cookie = HeaderValue::from_static("_cfuvid=visitor; Path=/; Secure; HttpOnly");
    ...
    fn only_allows_https_urls() {
        let url = reqwest::Url::parse("http://chatgpt.com/backend-api/forestx/responses").unwrap();

        assert!(!is_chatgpt_cookie_url(&url));

        let url = reqwest::Url::parse("wss://chatgpt.com/backend-api/forestx/responses").unwrap();

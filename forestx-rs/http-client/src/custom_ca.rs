//! Custom CA handling shared by Forestx outbound HTTP and websocket clients.
//!
//! Forestx constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `FORESTX_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Forestx itself commonly runs spawned test processes under seatbelt, so this is not just
...
use forestx_utils_rustls_provider::ensure_rustls_crypto_provider;
...
pub const FORESTX_CA_CERT_ENV: &str = "FORESTX_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set FORESTX_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Forestx custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `FORESTX_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Forestx HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Forestx custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `FORESTX_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
/// Builds a rustls client config using native roots and any configured Forestx custom CA bundle.
///
...
fn build_rustls_client_config(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Forestx custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        forestx_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `FORESTX_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Forestx-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(FORESTX_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: FORESTX_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Forestx CA overrides.
    ///
    /// This accepts a few common real-world variants so Forestx behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Forestx only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_forestx_env() {
        let env = map_env(&[
            (FORESTX_CA_CERT_ENV, "/tmp/forestx.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/forestx.pem"))
        );
    }
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(FORESTX_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);

...
//! detailed explanation of what "hermetic" means here lives in `forestx_http_client::custom_ca`; these
...
const FORESTX_CA_CERT_ENV: &str = "FORESTX_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "FORESTX_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "FORESTX_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "FORESTX_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(FORESTX_CA_CERT_ENV);
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    forestx_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls13_material() -> Tls13Material {
    ...
    ca_distinguished_name.push(DnType::CommonName, "forestx test CA");
...
#[test]
fn uses_forestx_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(FORESTX_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_forestx_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (FORESTX_CA_CERT_ENV, cert_path.as_path()),
...
fn posts_to_tls13_server_using_custom_ca_bundle() {
    ...
    let output =
        run_probe_posting_to_tls13_server(&[(FORESTX_CA_CERT_ENV, cert_path.as_path())], &server.url);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("FORESTX_CA_CERTIFICATE"));

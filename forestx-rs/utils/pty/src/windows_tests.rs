...
const READY_MARKER: &str = "__FORESTX_CHILD_READY__";
const VALUE_MARKER: &str = "__FORESTX_CHILD_VALUE__";
...
async fn assert_terminate_kills_descendant(
    ...
    let marker = std::env::temp_dir().join(format!(
        "forestx-job-descendant-{backend}-{}-{}",
        std::process::id(),
...
async fn assert_normal_exit_preserves_descendant(
    ...
    let marker_base = std::env::temp_dir().join(format!(
        "forestx-job-natural-exit-{backend}-{}-{}",
        std::process::id(),
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)?
            .as_nanos()
...
async fn conpty_delivers_input_to_foreground_children() -> anyhow::Result<()> {
    ...
    let code = format!(
        "print('__FORESTX_CHILD_'+'READY__', flush=True); value=input(); print('{VALUE_MARKER}'+value.encode('utf-8').hex(), flush=True)"
    );

...
mod tests {
    ...
    fn from_absolute_path_does_not_read_current_dir_when_path_is_absolute() {
        let status = Command::new(std::env::current_exe().expect("current test binary"))
            ...
            .arg("--ignored")
            .env("FORESTX_ABSOLUTE_PATH_REMOVED_CWD_CHILD", "1")
            .status()
    ...
    fn from_absolute_path_with_removed_current_dir_child() {
        if std::env::var_os("FORESTX_ABSOLUTE_PATH_REMOVED_CWD_CHILD").is_none() {
            return;
        }
        ...
        let path = AbsolutePathBuf::from_absolute_path(test_path_buf(
            "/tmp/forestx/../forestx-home/plugins/cache",
        ))
        ...
        assert_eq!(
            path.as_path(),
            test_path_buf("/tmp/forestx-home/plugins/cache")
        );
    }

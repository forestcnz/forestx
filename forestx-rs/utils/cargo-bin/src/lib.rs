...
/// site.)
///
/// This is expected to be used exclusively in test code because Forestx CLI is a
...
pub fn repo_root() -> io::Result<PathBuf> {
    let marker = if runfiles_available() {
        ...
        let marker_path = option_env!("FORESTX_REPO_ROOT_MARKER")
            .map(PathBuf::from)
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::NotFound,
                    "FORESTX_REPO_ROOT_MARKER was not set at compile time",
                )
            })?;

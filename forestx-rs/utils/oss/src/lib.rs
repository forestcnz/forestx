...
pub fn get_default_model_for_oss_provider(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => Some(forestx_lmstudio::DEFAULT_OSS_MODEL),
        OLLAMA_OSS_PROVIDER_ID => Some(forestx_ollama::DEFAULT_OSS_MODEL),
        _ => None,
    }
}
...
pub async fn ensure_oss_provider_ready(
    ...
    match provider_id {
        LMSTUDIO_OSS_PROVIDER_ID => {
            forestx_lmstudio::ensure_oss_ready(config)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        OLLAMA_OSS_PROVIDER_ID => {
            let client = forestx_ollama::OllamaClient::try_from_oss_provider(config).await?;
            forestx_ollama::ensure_responses_supported(&client).await?;
            forestx_ollama::ensure_oss_ready(config, &client)
                .await
                .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
        }
        _ => {

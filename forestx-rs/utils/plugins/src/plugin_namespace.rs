...
/// plugin manifest (same `name` rules as full manifest loading in forestx-core).
...
mod tests {
    ...
    async fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".forestx-plugin")).expect("mkdir manifest");
        fs::write(
    ...
    fn ignores_unrelated_root_plugin_manifest_before_legacy_fallback() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let legacy_path = plugin_root.join(".forestx-plugin/plugin.json");
    ...
    #[test]
    fn preserves_forestx_claude_cursor_legacy_precedence() {
        let tmp = tempdir().expect("tempdir");
        let plugin_root = tmp.path().join("plugins/sample");
        let forestx_path = plugin_root.join(".forestx-plugin/plugin.json");
        let claude_path = plugin_root.join(".claude-plugin/plugin.json");
        let cursor_path = plugin_root.join(".cursor-plugin/plugin.json");
        for path in [&forestx_path, &claude_path, &cursor_path] {
        ...
        assert_eq!(
            ...
            Some(forestx_path.clone())
        );
        fs::remove_file(forestx_path).expect("remove Forestx manifest");

//! Registry of model providers supported by Forestx.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Forestx works out-of-the-box.
//!   2. User-defined entries inside `~/.forestx/config.toml` under the `model_providers`
//!      key. These override or extend the defaults at runtime.

use forestx_api::Provider as ApiProvider;
use forestx_api::RetryConfig as ApiRetryConfig;
use forestx_protocol::auth::AuthMode;
use forestx_protocol::config_types::ModelProviderAuthInfo;
use forestx_protocol::error::EnvVarError;
...
pub const CHATGPT_FORESTX_BASE_URL: &str = "https://chatgpt.com/backend-api/forestx";
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "forestx";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/openai/forestx/discussions/7782";
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> ForestxResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> ForestxResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> ForestxResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
    ...
    ///
    /// DeepSeek's API is OpenAI-compatible and, as of the V4-Flash release,
    /// speaks the Responses API at `/v1/responses`, so it plugs directly into
    /// Forestx's responses-only wire layer. Auth is a bearer token read from
    /// the `DEEPSEEK_API_KEY` environment variable (no OpenAI login flow).
    pub fn create_deepseek_provider(base_url: Option<String>) -> ModelProviderInfo {
        ModelProviderInfo {
            name: DEEPSEEK_PROVIDER_NAME.into(),
            base_url: Some(base_url.unwrap_or_else(|| DEEPSEEK_DEFAULT_BASE_URL.to_string())),
            env_key: Some(DEEPSEEK_API_KEY_ENV.into()),
            env_key_instructions: Some(
                "Create a key at https://platform.deepseek.com and export DEEPSEEK_API_KEY.".into(),
            ),
            experimental_bearer_token: None,
            auth: None,
            aws: None,
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with Forestx CLI, so we only include the OpenAI and
    // open source ("oss") providers by default. Users are encouraged to add to
    // `model_providers` in config.toml to add their own providers.
    [
        (OPENAI_PROVIDER_ID, openai_provider),
        (DEEPSEEK_PROVIDER_ID, P::create_deepseek_provider(None)),
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These FORESTX_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_forestx_oss_base_url = format!(
        "http://localhost:{forestx_oss_port}/v1",
        forestx_oss_port = std::env::var("FORESTX_OSS_PORT")
            .ok()
    ...
    let forestx_oss_base_url = std::env::var("FORESTX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_forestx_oss_base_url);
    create_oss_provider_with_base_url(&forestx_oss_base_url, wire_api)
}

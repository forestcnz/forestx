...
#[command(
    version,
    override_usage = "forestx exec [OPTIONS] [PROMPT]\n       forestx exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running Forestx outside a Git repository.
    ...
    /// Do not load `$FORESTX_HOME/config.toml`; auth still uses `FORESTX_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "forestx resume --last <prompt>" treats the positional as a prompt, not a session id.

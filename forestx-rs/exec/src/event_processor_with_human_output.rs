...
use forestx_utils_sandbox_summary::summarize_permission_profile;
use owo_colors::OwoColorize;
use owo_colors::Style;

use crate::event_processor::ForestxStatus;
    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!(
                    "{}\n{}",
                    "forestx".style(self.italic).style(self.magenta),
                    text
                );
...
impl EventProcessor for EventProcessorWithHumanOutput {
    fn print_config_summary(
        ...
        eprintln!("OpenAI Forestx v{VERSION}\n--------");
    ...
    fn process_server_notification(&mut self, notification: ServerNotification) -> ForestxStatus {
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        ...
                        forestx_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        forestx_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => ForestxStatus::Running,

...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("forestx-exec-tests");
...
fn exec_default_stderr_filter_suppresses_otel_self_diagnostics() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        tracing::error!(target: "forestx_exec_test", "real exec error");
    });
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = forestx_protocol::protocol::W3cTraceContext {
...
fn runtime_warnings_are_filtered_to_the_primary_thread() {
    ...
    let turn_id = "turn-1";
    let outcomes = [
        forestx_app_server_protocol::WarningNotification {
            thread_id: None,
...
async fn resume_lookup_model_providers_filters_only_last_lookup() {
    let forestx_home = tempdir().expect("create temp forestx home");
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: forestx_app_server_protocol::SessionSource::Exec,
        ...
        name: None,
        turns: vec![
            forestx_app_server_protocol::Turn {
                id: "turn-1".to_string(),
    ...
    let notification =
        ServerNotification::TurnCompleted(forestx_app_server_protocol::TurnCompletedNotification {
            ...
            turn: forestx_app_server_protocol::Turn {
                id: "turn-1".to_string(),
                items_view: forestx_app_server_protocol::TurnItemsView::Summary,
                items: Vec::new(),
                status: forestx_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(forestx_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(forestx_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
"#,
    ...
    .expect("write config");
    let requirements_path = forestx_home.path().join("requirements.toml");
    std::fs::write(
        &requirements_path,
        r#"
...
async fn thread_start_params_include_user_thread_source() {
    ...
    assert_eq!(
        ...
        Some(forestx_app_server_protocol::ThreadSource::User)
    );
}
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(forestx_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        thread: forestx_app_server_protocol::Thread {
            ...
            can_accept_direct_input: None,
            thread_source: Some(forestx_app_server_protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            git_info: None,
        ...
        approval_policy: forestx_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: forestx_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: forestx_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],

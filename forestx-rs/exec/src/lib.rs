...
use forestx_app_server_client::EnvironmentManager;
use forestx_app_server_client::ExecServerRuntimePaths;
use forestx_app_server_client::InProcessAppServerClient;
use forestx_app_server_client::InProcessClientStartArgs;
use forestx_app_server_client::InProcessServerEvent;
...
use forestx_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use forestx_core::StateDbHandle;
use forestx_core::check_execpolicy_for_warnings;
use forestx_core::config::Config;
use forestx_core::config::ConfigBuilder;
use forestx_core::config::ConfigOverrides;
use forestx_core::config::ConfigTomlLoadResult;
...
use forestx_core::config::resolve_oss_provider;
use forestx_core::config::resolve_profile_v2_config_path;
use forestx_core::find_thread_meta_by_name_str;
use forestx_core::format_exec_policy_error_with_source;
use forestx_core::path_utils;
use forestx_core::read_session_meta_line;
...
use forestx_login::default_client::set_default_originator;
...
use forestx_otel::set_parent_from_context;
use forestx_otel::traceparent_context_from_env;
...
pub use event_processor_with_jsonl_output::ForestxStatus;
...
enum StdinPromptBehavior {
    ...
    /// `forestx exec` behavior for `forestx exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `forestx exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<forestx_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("forestx_exec".to_string()) {
        tracing::warn!(?err, "Failed to set forestx exec originator override {err:?}");
    }
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&forestx_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        forestx_core::otel_init::build_provider(
            &config,
    ...
    forestx_core::otel_init::record_process_start(otel.as_ref(), "forestx_exec");
    forestx_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "forestx_exec");
    ...
    let state_db = forestx_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_forestx_home(
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "forestx_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = forestx_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Forestx
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            forestx_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("Forestx initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(notification) => {
                ...
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        forestx_app_server_protocol::TurnStatus::Failed
                            | forestx_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        ForestxStatus::Running => {}
                        ForestxStatus::InitiateShutdown => {
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<forestx_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<forestx_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<forestx_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
fn should_backfill_turn_completed_items(
    ...
    !thread_ephemeral && payload.turn.items_view != forestx_app_server_protocol::TurnItemsView::Full
}
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.forestx_home, session_id, Some(state_db.as_ref()))
                .await?

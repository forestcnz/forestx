#![cfg(unix)]
use forestx_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use forestx_core::exec::build_exec_request;
    ...
    let forestx_linux_sandbox_exe = Some(
        core_test_support::find_forestx_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_forestx_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .forestx && echo 'sandbox_mode = \"danger-full-access\"' > .forestx/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &permission_profile,
    ...
    .await
    .expect("should spawn command creating .forestx");

    let status = child.wait().await.expect("should wait for .forestx command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .forestx creation: {status:?}"
    );
    let dot_forestx_metadata = tokio::fs::symlink_metadata(&dot_forestx).await;
    if let Ok(metadata) = dot_forestx_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_forestx.display()
        );
    } else if let Err(err) = &dot_forestx_metadata {
        assert_eq!(
            err.kind(),

...
/// While we may add an `apply-patch` subcommand to the `forestx` CLI multitool
/// at some point, we must ensure that the smaller `forestx-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(forestx_utils_cargo_bin::cargo_bin("forestx-exec")?)
        .arg(FORESTX_CORE_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
...
async fn shutdown_flushes_completed_turn_and_file_diff() -> anyhow::Result<()> {
    ...
    test.cmd_with_server(&server)
        .env_remove("FORESTX_API_KEY")
        .env_remove("FORESTX_ANALYTICS_EVENTS_CAPTURE_FILE")
        .arg("--skip-git-repo-check")
        .arg("-s")
        .arg("danger-full-access")
        .arg("-c")
    ...
    assert!(
        event_types.iter().any(|event| event == "forestx_turn_event"),

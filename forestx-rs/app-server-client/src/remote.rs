...
impl RemoteAppServerClient {
    ...
    pub fn forestx_home(&self) -> Option<&str> {
        self.forestx_home.as_deref()
    }
        ...
        S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let (pending_events, server_version, forestx_home) = initialize_remote_connection(
            &mut stream,
            &endpoint,
            initialize_params,
            INITIALIZE_TIMEOUT,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
                ...
                Some(Ok(Message::Text(text))) => {
                    ...
                    match message {
                        JSONRPCMessage::Response(response) if response.id == initialize_request_id => {
                            ...
                            forestx_home = response
                                .result
                                .get("forestxHome")
                                .and_then(serde_json::Value::as_str)
                                .filter(|forestx_home| !forestx_home.is_empty())
                                .map(str::to_string);
                            break Ok(());
                        }
    ...
    Ok((pending_events, server_version, forestx_home))
}

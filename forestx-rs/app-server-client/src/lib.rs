...
//!
//! This crate wraps [`forestx_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](forestx_app_server::in_process::InProcessClientHandle),
...
pub use forestx_app_server::app_server_control_socket_path;
pub use forestx_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use forestx_app_server::in_process::InProcessServerEvent;
use forestx_app_server::in_process::InProcessStartArgs;
use forestx_app_server::in_process::LogDbLayer;
pub use forestx_app_server::in_process::StateDbHandle;
use forestx_app_server_protocol::ClientInfo;
use forestx_app_server_protocol::ClientNotification;
...
use forestx_app_server_protocol::Result as JsonRpcResult;
...
use forestx_config::NoopThreadConfigLoader;
use forestx_config::RemoteThreadConfigLoader;
use forestx_config::ThreadConfigLoader;
use forestx_core::config::Config;
pub use forestx_core::otel_init::build_provider as build_otel_provider;
...
/// module exists so clients can remove a direct `forestx-core` dependency
/// while legacy startup/config paths are migrated to RPCs.
pub mod legacy_core {
    pub mod config {
        pub use forestx_core::config::*;

        pub mod edit {
            pub use forestx_core::config::edit::*;
        }
    }
}
...
pub struct InProcessClientStartArgs {
    ...
    /// Whether auth loading should honor the `FORESTX_API_KEY` environment variable.
    pub enable_forestx_api_key_env: bool,
...
impl InProcessClientStartArgs {
    ...
    fn into_runtime_start_args(self) -> InProcessStartArgs {
        ...
        InProcessStartArgs {
            ...
            enable_forestx_api_key_env: self.enable_forestx_api_key_env,
            initialize,
...
/// - [`forestx_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            forestx_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
impl AppServerClient {
    pub fn forestx_home(&self, local_forestx_home: &AbsolutePathBuf) -> Option<AppServerPath> {
        match self {
            Self::InProcess(_) => Some(AppServerPath::from_app_server(
                local_forestx_home.display().to_string(),
            )),
            Self::Remote(client) => client.forestx_home().map(AppServerPath::from_app_server),
        }
    }
...
mod tests {
    ...
    use forestx_app_server_protocol::ServerNotification;
    use forestx_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use forestx_core::init_state_db;
    use forestx_uds::UnixListener;
    use forestx_utils_absolute_path::AbsolutePathBuf;
    use futures::SinkExt;
    ...
    async fn build_test_config_for_forestx_home(forestx_home: &Path) -> Config {
        ...
        {
            Ok(config) => config,
            Err(_) => Config::load_default_with_cli_overrides_for_forestx_home(
    ...
    async fn start_test_client_with_capacity(
        ...
        let config = Arc::new(build_test_config_for_forestx_home(forestx_home.path()).await);
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "forestx-app-server-client-test".to_string(),
        ...
        write_websocket_message(
            ...
            JSONRPCMessage::Response(JSONRPCResponse {
                ...
                result: serde_json::json!({
                    "userAgent": "forestx_cli_rs/9.8.7-test (Test OS; x86_64) rust",
                    "forestxHome": "/server/.forestx",
                }),
            }),
        )
        .await;
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            forestx_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            forestx_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(forestx_app_server_protocol::ItemCompletedNotification {
            ...
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: forestx_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(forestx_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: forestx_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: forestx_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: forestx_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: forestx_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<forestx_app_server_protocol::ThreadReadResponse>(
            ...
            InProcessServerEvent::ServerNotification(notification)
                if matches!(
                    ...
                    ServerNotification::ItemCompleted(notification)
                        if matches!(
                            &notification.item,
                            forestx_app_server_protocol::ThreadItem::AgentMessage { text, .. }
                                if text == "hello"
                        )
                )
        ));
                ...
                if matches!(
                    ...
                    ServerNotification::TurnCompleted(notification)
                        if notification.turn.status
                            == forestx_app_server_protocol::TurnStatus::Completed
                )
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        assert_eq!(client.forestx_home(), Some("/server/.forestx"));
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: forestx_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
    ...
    async fn remote_unix_socket_typed_request_roundtrip_works() {
        ...
        let socket_path = AbsolutePathBuf::from_absolute_path(socket_dir.path().join("forestx.sock"))
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                forestx_app_server_protocol::ServerNotification::TurnCompleted(
                    forestx_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: forestx_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: forestx_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                forestx_app_server_protocol::ServerNotification::AgentMessageDelta(
                    forestx_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                forestx_app_server_protocol::ServerNotification::ItemCompleted(
                    forestx_app_server_protocol::ItemCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        completed_at_ms: 0,
                        item: forestx_app_server_protocol::ThreadItem::AgentMessage {
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                forestx_app_server_protocol::ServerNotification::ExternalAgentConfigImportCompleted(
                    forestx_app_server_protocol::ExternalAgentConfigImportCompletedNotification {
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(Box::new(
                forestx_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    forestx_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            forestx_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }

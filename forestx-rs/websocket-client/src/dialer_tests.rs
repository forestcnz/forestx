...
use forestx_http_client::OutboundProxyRoute;
...
async fn tungstenite_default_tls_mode_ignores_invalid_custom_ca_in_a_subprocess() {
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        command
            .env(
                "FORESTX_CA_CERTIFICATE",
                "/forestx-websocket-client-nonexistent-custom-ca.pem",
            )
            .env("FORESTX_WEBSOCKET_DEFAULT_TLS_PROBE_URL", target_url)
            .output()
...
async fn tungstenite_default_tls_mode_subprocess_probe() {
    let Ok(url) = std::env::var("FORESTX_WEBSOCKET_DEFAULT_TLS_PROBE_URL") else {
        return;
    };
    let factory = HttpClientFactory::new(OutboundProxyPolicy::ReqwestDefault);
    assert!(
        WebSocketConnector::new(&factory).is_err(),
        "explicit Forestx TLS should reject the invalid custom CA"
    );
    let connector =
    ...
    let (mut websocket, _) = connector
        .connect(request, WebSocketConfig::default())
        .await
        .expect("WebSocket should connect without constructing Forestx TLS");
...
async fn no_proxy_subprocess_probe() {
    let Ok(url) = std::env::var("FORESTX_WEBSOCKET_NO_PROXY_PROBE_URL") else {
        return;
    };
    let proxy_url = std::env::var("FORESTX_WEBSOCKET_NO_PROXY_PROBE_PROXY")
    ...
    let request = url
        ...
        .expect("websocket request should build");
    let tls_config =
        if let Ok(certificate_hex) = std::env::var("FORESTX_WEBSOCKET_NO_PROXY_PROBE_CA_DER") {
...
async fn assert_no_proxy_subprocess(no_proxy: &str, expect_proxy: bool, proxy_tls: bool) {
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        command
            ...
            .env("FORESTX_WEBSOCKET_NO_PROXY_PROBE_URL", target_url)
            .env("FORESTX_WEBSOCKET_NO_PROXY_PROBE_PROXY", proxy_url);
        command.env_remove("FORESTX_WEBSOCKET_NO_PROXY_PROBE_CA_DER");
        if let Some(certificate) = certificate {
            let certificate_hex = certificate
                ...
                .collect::<String>();
            command.env("FORESTX_WEBSOCKET_NO_PROXY_PROBE_CA_DER", certificate_hex);
        }
        command
            .output()

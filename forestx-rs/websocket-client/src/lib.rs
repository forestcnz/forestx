//! Proxy-aware WebSocket connection setup shared by Forestx API clients.
...
use forestx_http_client::BuildCustomCaTransportError;
use forestx_http_client::HttpClientFactory;
use forestx_http_client::build_rustls_client_config_with_custom_ca;
use futures::Sink;
use futures::Stream;
...
/// Selects whether WebSocket TLS follows Forestx custom-CA policy or Tungstenite defaults.
#[derive(Clone, Copy, PartialEq, Eq)]
pub enum WebSocketTlsMode {
    /// Build an explicit TLS configuration from native roots and configured Forestx custom CAs.
    ExplicitForestxTls,
...
impl WebSocketConnector {
    /// Creates a connector using native roots and any configured Forestx custom CA bundle.
    pub fn new(
        http_client_factory: &HttpClientFactory,
    ) -> Result<Self, BuildCustomCaTransportError> {
        Self::new_with_tls_mode(http_client_factory, WebSocketTlsMode::ExplicitForestxTls)
    }

    /// Creates a connector with explicit Forestx TLS or the transport's existing TLS defaults.
    ///
    /// HTTPS proxy connections still build Forestx TLS configuration when they establish their
    ...
    pub fn new_with_tls_mode(
        ...
        let tls_config = match tls_mode {
            WebSocketTlsMode::ExplicitForestxTls => {

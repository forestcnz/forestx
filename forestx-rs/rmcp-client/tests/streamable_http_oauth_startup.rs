...
use forestx_rmcp_client::WrappedOAuthTokenResponse;
use forestx_rmcp_client::determine_streamable_http_auth_status;
use forestx_rmcp_client::is_authentication_required_error;
use forestx_rmcp_client::save_oauth_tokens;
...
async fn refreshes_expired_persisted_token_before_initialize() -> anyhow::Result<()> {
    ...
    Mock::given(method("GET"))
        ...
        .and(header(
            "user-agent",
            concat!("forestx-mcp-client/", env!("CARGO_PKG_VERSION")),
        ))
    ...
    // Credential storage resolves FORESTX_HOME from the process environment.

...
-> anyhow::Result<()> {
    let server = forestx_utils_cargo_bin::cargo_bin("test_mcp_2026_discovery_stdio_server")?;
...
async fn legacy_stdio_preserves_existing_protocol_marker_environment() -> anyhow::Result<()> {
    let server = forestx_utils_cargo_bin::cargo_bin("test_stdio_server")?;
            ...
            let result = client
                .call_tool(
                    ...
                    Some(json!({
                        ...
                        "env_var": "FORESTX_MCP_PROTOCOL_VERSION",
                    })),
                    /*meta*/ None,
                    Some(Duration::from_secs(10)),
                )

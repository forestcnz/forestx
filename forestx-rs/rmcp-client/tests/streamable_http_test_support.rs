...
use forestx_exec_server::RemoteExecServerConnectArgs;
...
fn streamable_http_server_bin() -> Result<PathBuf, CargoBinError> {
    forestx_utils_cargo_bin::cargo_bin("test_streamable_http_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("forestx-test", "0.0.0-test").with_title("Forestx rmcp recovery test"),
    )
...
fn control_client() -> anyhow::Result<forestx_http_client::HttpClient> {
    Ok(forestx_http_client::HttpClientBuilder::new().build_direct()?)
}
...
/// Reads the websocket URL printed by `forestx exec-server --listen`.

...
const RESOURCE_URI: &str = "memo://forestx/example-note";

fn stdio_server_bin() -> Result<PathBuf, CargoBinError> {
    forestx_utils_cargo_bin::cargo_bin("test_stdio_server")
}
...
fn init_params() -> InitializeRequestParams {
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("forestx-test", "0.0.0-test").with_title("Forestx rmcp resource test"),
    )
...
async fn rmcp_client_can_list_and_read_resources() -> anyhow::Result<()> {
    ...
    let mut expected_templates = ListResourceTemplatesResult::with_all_items(vec![
        rmcp::model::ResourceTemplate::new("memo://forestx/{slug}", "forestx-memo")

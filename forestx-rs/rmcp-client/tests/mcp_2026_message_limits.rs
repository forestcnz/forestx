...
fn initialize_params() -> InitializeRequestParams {
    InitializeRequestParams::new(
        ...
        Implementation::new("forestx-message-limit-test", "0.0.0-test"),
    )
...
async fn modern_local_and_executor_stdio_reject_oversized_lines() -> anyhow::Result<()> {
    let server = forestx_utils_cargo_bin::cargo_bin("test_mcp_2026_stdio_server")?;
    ...
    for (modern, executor) in [(false, false), (false, true), (true, false), (true, true)] {
        ...
        let env = modern.then(|| {
            HashMap::from([(
                OsString::from("FORESTX_MCP_PROTOCOL_VERSION"),

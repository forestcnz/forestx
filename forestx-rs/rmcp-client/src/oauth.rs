...
//!
//! If the keyring is not available or fails, we fall back to FORESTX_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Forestx MCP Credentials";
...
fn load_oauth_tokens_from_secrets_keyring<K: KeyringStore + Clone + 'static>(
    ...
    let forestx_home = find_forestx_home().map_err(anyhow::Error::from)?;
...
fn fallback_file_path() -> Result<PathBuf> {
    Ok(find_forestx_home()?.join(FALLBACK_FILENAME).to_path_buf())
}

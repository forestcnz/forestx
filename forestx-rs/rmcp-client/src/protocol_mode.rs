...
/// MCP compatibility policy selected once when a Forestx session is created.
    ...
    pub(crate) fn stdio_mode(self, requested_version: Option<&OsStr>) -> io::Result<Self> {
        match (self, requested_version) {
            ...
            (_, Some(version)) => Err(io::Error::new(
                ...
                format!(
                    "unsupported FORESTX_MCP_PROTOCOL_VERSION `{}` for stdio MCP server; expected `2026-07-28`",

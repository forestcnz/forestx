...
const LOCK_CONTENTION_EVENT_TARGET: &str = "forestx_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are FORESTX_HOME-backed today: if FORESTX_HOME is unset they use
        // the default home (`~/.forestx`), and if an embedder has no local home/filesystem authority
        ...
        let forestx_home = find_forestx_home()
            .map_err(|source| OAuthStoreLockFailure::ForestxHome { store, source })?;
        Self::acquire_in(&forestx_home, store, STORE_LOCK_ACQUIRE_TIMEOUT)
    }
    ...
    pub(super) fn acquire_in(
        ...
        let path = oauth_store_lock_path(forestx_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve FORESTX_HOME for MCP OAuth {store} aggregate-store lock")]
    ForestxHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(forestx_home: &Path, store: OAuthStore) -> PathBuf {
    forestx_home.join(OAUTH_LOCK_DIR).join(store.lock_filename())
}

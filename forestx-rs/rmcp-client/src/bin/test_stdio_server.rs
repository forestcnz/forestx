...
const MEMO_URI: &str = "memo://forestx/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "forestx/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Forestx TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Forestx TUI):
    /// - Build this binary: `cargo build -p forestx-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `forestx mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Forestx TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        ResourceTemplate::new("memo://forestx/{slug}", "forestx-memo")
            .with_title("Forestx Memo")
            .with_description("Template for memo://forestx/{slug} resources used in tests.")
            .with_mime_type("text/plain")
    }
...
/// Scenarios for `image_scenario`, intended to exercise Forestx TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        let server_info = match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("forestx-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "encrypted_output" => {
                ...
                meta.insert("forestx/encryptedContent".to_string(), json!(true));

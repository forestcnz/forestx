...
pub trait ModelsEndpointClient: fmt::Debug + Send + Sync {
    ...
    /// Returns whether the currently resolved auth can use Forestx backend-only models.
...
pub trait ModelsManager: fmt::Debug + Send + Sync {
    ...
    fn build_available_models(&self, mut remote_models: Vec<ModelInfo>) -> Vec<ModelPreset> {
        ...
        presets = ModelPreset::filter_by_auth(presets, uses_forestx_backend);
...
impl OpenAiModelsManager {
    ...
    pub fn new(
        ...
        let cache_path = forestx_home.join(MODEL_CACHE_FILE);
...
impl OpenAiModelsManager {
    ...
    async fn should_refresh_models(&self) -> bool {
        self.endpoint_client.uses_forestx_backend().await || self.endpoint_client.has_command_auth()
    }
    ...
    async fn try_load_cache(&self) -> bool {
        ...
        let _timer =
            forestx_otel::start_global_timer("forestx.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
...
pub(crate) fn construct_model_info_from_candidates(
    ...
    // retry for namespaced slugs like `custom/gpt-5.3-forestx`.

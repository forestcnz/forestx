...
use forestx_login::ExternalAuth;
use forestx_login::ExternalAuthRefreshContext;
use forestx_login::TokenData;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> forestx_login::ExternalAuthFuture<'_, ForestxAuth> {
        Box::pin(async { Ok(ForestxAuth::from_api_key("test-external-api-key")) })
    }

    fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> forestx_login::ExternalAuthFuture<'_, ForestxAuth> {
        Box::pin(async { Ok(ForestxAuth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_forestx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_forestx_backend })
    }
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(forestx_home, endpoint_client, auth_manager)
}

async fn mutate_file_cache_for_test<F>(forestx_home: &Path, f: F)
where
    F: FnOnce(&mut ModelsCacheEntry),
{
    let client_version = crate::client_version_to_whole();
    let cache = FileModelsCache::new(forestx_home.join(MODEL_CACHE_FILE), DEFAULT_MODEL_CACHE_TTL);
    let mut entry = cache
        .load(&client_version)
        .await
...
async fn file_cache_implements_models_cache_contract() {
    let forestx_home = tempdir().expect("temp dir");
    let cache = FileModelsCache::new(
        forestx_home.path().join(MODEL_CACHE_FILE),
...
async fn chatgpt_auth_tokens_for_tests(forestx_home: &Path) -> ForestxAuth {
    let auth_dot_json = forestx_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: forestx_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(forestx_home).expect("forestx home should be created");
    std::fs::write(
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    ...
    let manager = openai_manager_for_tests(forestx_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-forestx/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(forestx_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(forestx_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(forestx_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_refetches_when_cache_stale() {
    ...
    mutate_file_cache_for_test(forestx_home.path(), |cache| {
...
impl TestAuthAwareModelsEndpoint {
    ...
    async fn uses_forestx_backend(&self) -> bool {
        match self.auth_manager.as_ref() {
            Some(auth_manager) => auth_manager
                .auth()
                .await
                .as_ref()
                .is_some_and(ForestxAuth::uses_forestx_backend),
            None => false,
        }
    }
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_forestx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_forestx_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(forestx_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        forestx_home.path().to_path_buf(),
        endpoint.clone(),

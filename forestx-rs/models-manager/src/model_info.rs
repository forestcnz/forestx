...
use forestx_protocol::openai_models::TruncationMode;
...
const DEFAULT_PERSONALITY_HEADER: &str = "You are Forestx, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
pub fn with_config_overrides(mut model: ModelInfo, config: &ModelsManagerConfig) -> ModelInfo {
    ...
    if let Some(base_instructions) = &config.base_instructions {
        ...
        let uses_local_personality_template = model.used_fallback_model_metadata
            && matches!(
                model.slug.as_str(),
                "gpt-5.2-forestx" | "exp-forestx-personality"
            );
        if !config.personality_enabled
...
fn local_model_messages_for_slug(slug: &str) -> ModelMessages {
    match slug {
        "gpt-5.2-forestx" | "exp-forestx-personality" => ModelMessages {

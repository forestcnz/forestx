# forestx-shell-escalation

This crate contains the Unix shell-escalation protocol implementation and the
`forestx-execve-wrapper` executable.

`forestx-execve-wrapper` receives the arguments to an intercepted `execve(2)` call and delegates the
decision to the shell-escalation protocol over a shared file descriptor (specified by the
`FORESTX_ESCALATE_SOCKET` environment variable). The server on the other side replies with one of:

- `Run`: `forestx-execve-wrapper` should invoke `execve(2)` on itself to run the original command
...
- `Escalate`: forward the file descriptors of the current process so the command can be run
  ...
  back to `forestx-execve-wrapper`.
- `Deny`: the server has declared the proposed command to be forbidden, so
  `forestx-execve-wrapper` prints an error to `stderr` and exits with `1`.
...
`forestx-zsh-vX.Y.Z` tag is pushed. When the zsh commit or patch changes, publish

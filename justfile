set working-directory := "forestx-rs"
...
alias c := forestx
forestx *args:
    cargo run --bin forestx -- {args}

# `forestx exec`
exec *args:
    cargo run --bin forestx -- exec {args}

# Start `forestx exec-server` and run forestx-tui.
[no-cd]
...
file-search *args:
    cargo run --bin forestx-file-search -- {args}

# Run the standalone code-mode host from source.
code-mode-host *args:
    cargo run --bin forestx-code-mode-host -- {args}

# Build the CLI and run the app-server test client
app-server-test-client *args:
    cargo build -p forestx-cli
    cargo run -p forestx-app-server-test-client -- --forestx-bin ./target/debug/forestx {args}
...
bench-e2e:
    ...
    bazel test --compilation_mode=opt --cache_test_results=no --test_output=streamed //forestx-rs:e2e-benchmarks
...
bench-e2e-smoke:
    ...
    bazel test --compilation_mode=fastbuild --@rules_rust//rust/settings:extra_rustc_flag=-Cdebug-assertions=no --@rules_rust//rust/settings:extra_exec_rustc_flag=-Cdebug-assertions=no --cache_test_results=no --test_output=streamed --test_arg=--test //forestx-rs:e2e-benchmarks

# Build and run Forestx from source using Bazel.
# On Unix, use `[no-cd]` and `--run_under="cd $PWD &&"` to ensure Bazel runs
# the command in the current working directory.
[no-cd]
[unix]
bazel-forestx *args:
    bazel run //forestx-rs/cli:forestx --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-forestx *args:
    bazel run //forestx-rs/cli:forestx --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)

# Build and run the standalone code-mode host from source using Bazel.
[no-cd]
[unix]
bazel-code-mode-host *args:
    bazel run //forestx-rs/code-mode-host:forestx-code-mode-host --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-code-mode-host *args:
    bazel run //forestx-rs/code-mode-host:forestx-code-mode-host --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)
...
build-for-release:
    bazel build //forestx-rs/cli:release_binaries

# Run the MCP server
mcp-server-run *args:
    cargo run -p forestx-mcp-server -- {args}

# Regenerate the json schema for config.toml from the current config types.
write-config-schema:
    cargo run -p forestx-core --bin forestx-write-config-schema

# Regenerate vendored app-server protocol schema artifacts.
write-app-server-schema *args:
    cargo run -p forestx-app-server-protocol --bin write_schema_fixtures -- {args}

[no-cd]
write-hooks-schema:
    cargo run --manifest-path {{ justfile_directory() }}/forestx-rs/Cargo.toml -p forestx-hooks --bin write_hooks_schema_fixtures

# Run the argument-comment Dylint checks across forestx-rs.
[no-cd]
[unix]
...
[unix]
log *args:
    if [ "${1:-}" = "--" ]; then shift; fi; cargo run -p forestx-cli --bin logs_client -- "$@"

[windows]
log *args:
    $forwarded_args = @($args | Select-Object -Skip 1); if ($forwarded_args.Count -gt 0 -and $forwarded_args[0] -eq "--") { $forwarded_args = @($forwarded_args | Select-Object -Skip 1) }; cargo run -p forestx-cli --bin logs_client -- @forwarded_args

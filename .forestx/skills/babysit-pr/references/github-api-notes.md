...
- `gh api repos/{owner}/{repo}/actions/jobs/{job_id}/logs > /tmp/forestx-gh-job-{job_id}-logs.zip`
- `gh run view <run-id> --log-failed`

Used by Forestx to classify branch-related vs flaky/unrelated failures. Prefer the direct job log endpoint as soon as a job has failed because `gh run view --log-failed` may not produce failed-job logs until the overall workflow run completes.

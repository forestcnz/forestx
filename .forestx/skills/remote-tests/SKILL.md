...
Use `TestForestxBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$FORESTX_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `forestx_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap forestx_remote_env_cleanup EXIT

  cd forestx-rs
  just test -p forestx-core --test all
'
```
...
bash -c '
  ...
  cd forestx-rs
  just test -p forestx-app-server --test all
'
```
...
```sh
bazel test //forestx-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `forestx` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of forestx in `~/code/forestx`. Reset files if needed. Multiple checkouts take longer to build and take up more space.

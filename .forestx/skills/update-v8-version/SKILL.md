...
description: Update Forestx's pinned `v8` / `rusty_v8` versions, validate the release-candidate path, and investigate failed V8 canary or artifact builds. Use when asked to bump V8, update `rusty_v8` artifacts, prepare or validate a V8 release candidate, check `v8-canary`, or diagnose why a V8 version update no longer builds.
---
...
2. Inspect and update the concrete repo surfaces that carry the pin:
   - `forestx-rs/Cargo.toml`
   - `forestx-rs/Cargo.lock`
   - `MODULE.bazel`
...
4. Trace each failing delta back into Forestx's build graph:
   - `MODULE.bazel`
   - `third_party/v8/BUILD.bazel`
...
- When blocked, report the upstream delta that matters, the Forestx file it hits,

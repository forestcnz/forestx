#!/usr/bin/env python3
"""Collect recent openai/forestx issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a Forestx owner digest."
    )
    parser.add_argument(
        "--repo", default="openai/forestx", help="OWNER/REPO, default openai/forestx"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(forestx(?: desktop| app|\.app| cli)?|desktop|windows forestx app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )

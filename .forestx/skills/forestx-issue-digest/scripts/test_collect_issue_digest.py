...
def test_build_search_queries_uses_each_owner_and_kind_label():
    ...
    queries = collect_issue_digest.build_search_queries(
        "openai/forestx", ["tui", "exec"], since
    )

    assert queries == [
        "repo:openai/forestx is:issue updated:>=2026-04-25 label:tui label:bug",
        "repo:openai/forestx is:issue updated:>=2026-04-25 label:tui label:enhancement",
        "repo:openai/forestx is:issue updated:>=2026-04-25 label:exec label:bug",
        "repo:openai/forestx is:issue updated:>=2026-04-25 label:exec label:enhancement",
    ]
...
def test_build_search_queries_can_scan_all_labels():
    ...
    queries = collect_issue_digest.build_search_queries(
        "openai/forestx", [], since, all_labels=True
    )

    assert queries == [
        "repo:openai/forestx is:issue updated:>=2026-04-25 label:bug",
        "repo:openai/forestx is:issue updated:>=2026-04-25 label:enhancement",
    ]
...
def test_summarize_issue_keeps_new_comments_and_reaction_signals():
    ...
    issue = {
        ...
        "html_url": "https://github.com/openai/forestx/issues/123",
        "state": "open",
    ...
    comments = [
        {
            ...
            "html_url": "https://github.com/openai/forestx/issues/123#issuecomment-1",
            "user": {"login": "bob"},
        ...
        },
        {
            ...
            "html_url": "https://github.com/openai/forestx/issues/123#issuecomment-2",
            "user": {"login": "carol"},
            "author_association": "NONE",
            "body": "Older comment.",
    ...
    assert summary == {
        ...
        "url": "https://github.com/openai/forestx/issues/123",
        "state": "open",
        "author": "alice",
        "author_association": "NONE",
        "created_at": "2026-04-24T20:00:00Z",
        ...
        "new_comments": [
            {
                ...
                "created_at": "2026-04-25T11:00:00Z",
                "updated_at": "2026-04-25T11:00:00Z",
                "url": "https://github.com/openai/forestx/issues/123#issuecomment-1",
...
def test_fetch_comments_uses_since_filter_and_page_cap(monkeypatch):
    ...
    payload = collect_issue_digest.fetch_comments(
        "openai/forestx", 123, since=since, max_pages=1
    )
    ...
    assert calls == [
        [
            "api",
            "repos/openai/forestx/issues/123/comments?since=2026-04-25T00%3A00%3A00Z&per_page=100&page=1",
        ]
    ]
...
def test_issue_description_prefers_title_over_body_noise():
    issue = {
        "title": "Forestx.app GUI: MCP child processes not reaped after task completion",
...
def test_attention_markers_count_human_user_interactions():
    ...
    issue = {
        ...
        "html_url": "https://github.com/openai/forestx/issues/456",
        "state": "open",
        "created_at": "2026-04-25T01:00:00Z",
...
def test_reactions_count_toward_attention_markers():
    ...
    issue = {
        ...
        "html_url": "https://github.com/openai/forestx/issues/789",
        "state": "open",
...
def test_user_interactions_are_deduped_by_human_login():
    ...
    issue = {
        ...
        "html_url": "https://github.com/openai/forestx/issues/790",
        "state": "open",
        "created_at": "2026-04-25T01:00:00Z",
        "updated_at": "2026-04-25T12:00:00Z",
        "user": {"login": "Alice"},
...
def test_digest_rows_are_table_ready_with_concise_descriptions():
        ...
        [
            {
                ...
                "url": "https://github.com/openai/forestx/issues/1",
            ...
            },
            {
                ...
                "url": "https://github.com/openai/forestx/issues/2",
                "owner_labels": ["agent"],
                "kind_labels": ["bug"],
                "state": "open",
                "attention": True,
    ...
    assert rows[0] == {
        "ref": 1,
        "ref_markdown": "[1](https://github.com/openai/forestx/issues/2)",
        "marker": "🔥",
        "attention_marker": "🔥",
        "number": 2,
        "description": "High-volume bug report",
...
def test_summary_inputs_are_model_ready_without_preclustering():
    issues = [
        {
            ...
            "url": "https://github.com/openai/forestx/issues/20",
            "labels": ["app", "bug"],
            "owner_labels": ["app"],
            "kind_labels": ["bug"],
            "attention": False,
        ...
        },
        {
            ...
            "url": "https://github.com/openai/forestx/issues/21",
            "labels": ["app", "bug"],
            "owner_labels": ["app"],
            "kind_labels": ["bug"],
            "attention": False,
        ...
        },
        {
            ...
            "url": "https://github.com/openai/forestx/issues/22",
            "labels": ["app", "bug"],
            "owner_labels": ["app"],
            "kind_labels": ["bug"],
            "attention": False,
    ...
    assert rows == [
        {
            "ref": 1,
            "ref_markdown": "[1](https://github.com/openai/forestx/issues/20)",
            "number": 20,
            "title": "Windows app Browser Use external navigation fails",
            "description": "Browser Use navigation or app-server failure",
            "url": "https://github.com/openai/forestx/issues/20",
        ...
        {
            "ref": 2,
            "ref_markdown": "[2](https://github.com/openai/forestx/issues/21)",
            "number": 21,
            "title": "On Windows, cmake output waits until timeout",
            "description": "Windows command timeout/capture problem",
            "url": "https://github.com/openai/forestx/issues/21",
        ...
        {
            "ref": 3,
            "ref_markdown": "[3](https://github.com/openai/forestx/issues/22)",
            "number": 22,
            "title": "Windows computer use tool fails to click buttons",
            "description": "Computer-use workflow failure",
            "url": "https://github.com/openai/forestx/issues/22",

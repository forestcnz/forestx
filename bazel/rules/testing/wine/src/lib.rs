...
impl WineRuntimePaths {
    fn from_runfiles() -> Result<Self> {
        let wine = forestx_utils_cargo_bin::cargo_bin("wine")?;
        let runtime_marker = forestx_utils_cargo_bin::cargo_bin("wine-runtime-marker")?;
        let dll_path = runtime_marker
            .parent()
            .context("locate Wine runtime directory")?
            .to_path_buf();
        let wineserver = forestx_utils_cargo_bin::cargo_bin("wineserver")?;
        let powershell_runtime = forestx_utils_cargo_bin::cargo_bin("pwsh-runtime-marker")?
            .parent()

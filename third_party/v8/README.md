...
`RUSTY_V8_ARCHIVE`/`RUSTY_V8_SRC_BINDING_PATH` with Forestx release assets. Bazel
...
1. Bump the `v8` crate version and refresh `forestx-rs/Cargo.lock`.
...
by Forestx's Bazel Windows GNU toolchain.
...
downloaded `RUSTY_V8_SRC_BINDING_PATH` to point at those `openai/forestx` release
...
the exact resolved `v8` crate version in `forestx-rs/Cargo.lock`.

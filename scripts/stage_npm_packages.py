#!/usr/bin/env python3
"""Stage one or more Forestx npm packages for release."""
...
BUILD_SCRIPT = REPO_ROOT / "forestx-cli" / "scripts" / "build_npm_package.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
GITHUB_REPO = "openai/forestx"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("forestx_build_npm_package", BUILD_SCRIPT)
...
FORESTX_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "FORESTX_PLATFORM_PACKAGES", {})
FORESTX_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "FORESTX_PACKAGE_COMPONENT", "forestx-package"
)
...
BINARY_COMPONENTS = {
    "forestx-responses-api-proxy": BinaryComponent(
        artifact_prefix="forestx-responses-api-proxy",
        dest_dir="forestx-responses-api-proxy",
        binary_basename="forestx-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if FORESTX_PACKAGE_COMPONENT in components:
        install_forestx_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = FORESTX_PACKAGE_COMPONENT in components or any(
...
def install_forestx_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing Forestx package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_forestx_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_forestx_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"forestx-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in FORESTX_PLATFORM_PACKAGES:
        platform = package.removeprefix("forestx-")
        return f"forestx-npm-{platform}-{version}.tgz"

...
from forestx_package.zsh import resolve_zsh_bin
...
class ResolveZshBinTest(unittest.TestCase):
    def test_uses_prebuilt_executable_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("forestx_package.zsh.fetch_dotslash_executable") as fetch:
    ...
    def test_uses_manifest_override(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive = root / "forestx-zsh.tar.gz"
            source = root / "zsh"
            source.write_bytes(b"standalone zsh")
            with tarfile.open(archive, "w:gz") as tar:
                tar.add(source, arcname="forestx-zsh/bin/zsh")

            manifest = root / "forestx-zsh"
            manifest.write_text(
                json.dumps(
                    {
                        "platforms": {
            ...
            with patch(
                "forestx_package.dotslash.default_cache_root",

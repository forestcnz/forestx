"""Canonical Forestx package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "forestx-resources"
    path_dir = package_dir / "forestx-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"forestx-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.forestx_command_runner_bin is not None:
        copy_executable(
            inputs.forestx_command_runner_bin,
            resources_dir / "forestx-command-runner.exe",
            is_windows=True,
        )

    if inputs.forestx_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.forestx_windows_sandbox_setup_bin,
            resources_dir / "forestx-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    metadata = {
        ...
        "resourcesDir": "forestx-resources",
        "pathDir": "forestx-path",
    }
    write_json(package_dir / "forestx-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "forestx-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: forestx-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"forestx-code-mode-host{spec.exe_suffix}",
        Path("forestx-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("forestx-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("forestx-resources") / "bwrap")
        executable_files.append(Path("forestx-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("forestx-resources") / "forestx-command-runner.exe",
                Path("forestx-resources") / "forestx-windows-sandbox-setup.exe",
            ]
        )

# Forestx package builder

This package contains the implementation behind `scripts/build_forestx_package.py`.
...
The builder creates a canonical Forestx package directory:
...
│   ├── forestx-command-runner.exe          # Windows only
│   └── forestx-windows-sandbox-setup.exe   # Windows only
└── forestx-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match Forestx release
...
`forestx` and `forestx-app-server`. The `version` field in `forestx-package.json` is
read from `[workspace.package].version` in `forestx-rs/Cargo.toml`.
...
- all targets: `forestx-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `forestx-command-runner` and `forestx-windows-sandbox-setup`,
...
`--forestx-command-runner-bin` plus `--forestx-windows-sandbox-setup-bin` for
...
downloads and verifies the matching Forestx-built V8 release pair before invoking
...
DotSlash manifest at `scripts/forestx_package/rg`. Downloaded archives are cached
under `$TMPDIR/forestx-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/forestx_package/forestx-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/forestx-package/<target>-zsh` and installed at
`forestx-resources/zsh/bin/zsh`. Pass `--zsh-bin` to package a prebuilt, signed

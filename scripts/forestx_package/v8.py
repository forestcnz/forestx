"""Forestx-built V8 artifact overrides for package Cargo builds."""
...
def resolve_forestx_v8_cargo_env(
    spec: TargetSpec,
    *,
    ...
    artifacts = fetch_forestx_v8_artifacts(spec, cache_root=cache_root)
    return {
...
def fetch_forestx_v8_artifacts(
    spec: TargetSpec,
    *,
    ...
    release_url = (
        f"https://github.com/openai/forestx/releases/download/rusty-v8-v{version}"
    )
    target = spec.target
...
def resolved_v8_crate_version() -> str:
    ...
    cargo_lock = tomllib.loads((REPO_ROOT / "forestx-rs" / "Cargo.lock").read_text())
    versions = sorted(
        {
            package["version"]
...
def default_cache_root() -> Path:
    return Path(tempfile.gettempdir()) / "forestx-package"
...
def ensure_valid_artifact(artifact: Path, checksum: str, url: str) -> None:
    ...
    artifact.unlink(missing_ok=True)
    raise RuntimeError(
        f"Forestx-built V8 artifact {artifact} failed checksum validation."
    )

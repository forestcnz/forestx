"""Command-line interface for building Forestx package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical Forestx package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt forestx-code-mode-host executable. If omitted, "
    ...
    zsh_source.add_argument(
        ...
        help=(
            ...
            "scripts/forestx_package/forestx-zsh."
        ),
    )
    zsh_source.add_argument(
        "--zsh-bin",
    ...
    parser.add_argument(
        "--forestx-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows forestx-command-runner.exe executable. "
            "If omitted for Windows targets, forestx-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--forestx-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows forestx-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "forestx-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="forestx-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        forestx_command_runner_bin=resolve_optional_input_path(
            args.forestx_command_runner_bin,
            "prebuilt Windows forestx-command-runner.exe executable",
            "--forestx-command-runner-bin",
        ),
        forestx_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.forestx_windows_sandbox_setup_bin,
            "prebuilt Windows forestx-windows-sandbox-setup.exe executable",
            "--forestx-windows-sandbox-setup-bin",
        ),
    )
    ...
    inputs = PackageInputs(
        ...
        forestx_command_runner_bin=source_outputs.forestx_command_runner_bin,
        forestx_windows_sandbox_setup_bin=source_outputs.forestx_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built Forestx package archive at {archive_path}")

    print(f"Built Forestx package directory at {package_dir}")
    return 0

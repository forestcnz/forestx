...
from forestx_package.layout import build_package_dir
from forestx_package.layout import validate_package_dir
from forestx_package.targets import PACKAGE_VARIANTS
from forestx_package.targets import PackageInputs
...
class PackageLayoutTest(unittest.TestCase):
    def test_macos_package_preserves_prebuilt_resource_binaries(self) -> None:
        for variant_name in ("forestx", "forestx-app-server"):
                    ...
                    with tempfile.TemporaryDirectory() as temp_dir:
                        ...
                        inputs = PackageInputs(
                            ...
                            code_mode_host_bin=touch_executable(
                                root / "forestx-code-mode-host"
                            ),
                            rg_bin=rg_bin,
                            zsh_bin=zsh_bin,
                            bwrap_bin=None,
                            forestx_command_runner_bin=None,
                            forestx_windows_sandbox_setup_bin=None,
                        )
                        ...
                        self.assertEqual(
                            {
                                "rg": (package_dir / "forestx-path" / "rg").read_bytes(),
                                "zsh": (
                                    package_dir
                                    / "forestx-resources"
                                    / "zsh"
    ...
    def test_app_server_package_places_code_mode_host_beside_entrypoint(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            package_dir.mkdir()
            inputs = PackageInputs(
                entrypoint_bin=touch_executable(root / "forestx-app-server"),
                code_mode_host_bin=touch_executable(root / "forestx-code-mode-host"),
            ...
            self.assertTrue((package_dir / "bin" / "forestx-code-mode-host").is_file())

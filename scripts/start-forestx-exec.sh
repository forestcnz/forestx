...
remote_path='~/code/forestx-sync'
local_exec_server_port="${FORESTX_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${FORESTX_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/forestx-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/forestx-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/forestx-sync"
remote_forestx_rs="$remote_repo_root/forestx-rs"

cd "${remote_forestx_rs}"
cargo build -p forestx-cli --bin forestx

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/forestx exec-server --listen ws://127.0.0.1:0 \
...
echo "Start forestx via: "
printf '  FORESTX_EXEC_SERVER_URL=ws://127.0.0.1:%s forestx -C %q\n' \

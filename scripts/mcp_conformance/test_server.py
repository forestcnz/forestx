...
def test_review_protocol_env_is_available_to_a_legacy_client() -> None:
    ...
    with patch.dict(
        "os.environ", {"FORESTX_MCP_PROTOCOL_VERSION": "legacy-review-sentinel"}
    ):
        plan = server.handle(
            request(
                "tools/call",
...
def test_review_protocol_env_is_available_to_the_shipping_legacy_client() -> None:
    ...
    with patch.dict(
        "os.environ", {"FORESTX_MCP_PROTOCOL_VERSION": "shipping-legacy-sentinel"}
    ):
        plan = server.handle(
            request(
                "tools/call",
...
def test_review_protocol_env_is_available_to_a_modern_client() -> None:
    ...
    with patch.dict("os.environ", {"FORESTX_MCP_PROTOCOL_VERSION": MODERN_VERSION}):
        plan = server.handle(
            request(
                "tools/call",
                params={

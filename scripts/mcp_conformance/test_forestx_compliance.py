...
import pytest
from run_forestx_compliance import (
...
def test_registration_commands_select_stdio_then_http_shapes() -> None:
    ...
    assert stdio == [
        ...
        TEST_SERVER_NAME,
        "--env",
        f"FORESTX_MCP_PROTOCOL_VERSION={MODERN_VERSION}",
        "--",
        sys.executable,
        "/src/server.py",
        "--mode",
    ...
    valid, _ = _validate_registration(
        {
            ...
            "transport": {
                ...
                "env": {"FORESTX_MCP_PROTOCOL_VERSION": MODERN_VERSION},
            },
        },
        transport="stdio",
        mode=MODERN_VERSION,
...
def test_isolated_environment_drops_model_credentials(
    ...
    monkeypatch.setenv("FORESTX_API_KEY", "secret")
    monkeypatch.setenv("FORESTX_ACCESS_TOKEN", "secret")
    monkeypatch.setenv("OPENAI_API_KEY", "secret")

    env = _isolated_environment(tmp_path)

    assert env["FORESTX_HOME"] == str(tmp_path)
    assert "FORESTX_API_KEY" not in env
    assert "FORESTX_ACCESS_TOKEN" not in env
...
def test_scenario_ratio_includes_adapter_failures() -> None:
    checks = [
        ...
        CheckResult(
            "harness/auth/example/forestx-adapter",
            False,
            "adapter failed",
            source="harness",
            scenario="auth/example",
...
def test_official_case_preserves_adapter_check_identity_on_failure_and_success(
    ...
    monkeypatch.setattr(
        "run_forestx_compliance.run_official_mode",
        lambda **_kwargs: [result],
    )

    case = _run_official_case(
        Path("/opt/forestx"),
        Path("/src/forestx_conformance_adapter.py"),
    ...
    assert case.checks == [
        CheckResult(
            name=f"harness/{scenario}/forestx-adapter",
            success=adapter_success,
    ...
    assert gate["fixedChecks"] == [
        {
            ...
            "source": "harness",
            "scenario": scenario,
            "check_id": f"harness/{scenario}/forestx-adapter",
        },
        {
            "mode": MODERN_VERSION,
            "transport": "official-http",
...
def test_regression_cli_accepts_a_baseline_report() -> None:
    args = _parse_args(
        ["/opt/forestx", "--baseline-report", "/tmp/forestx-mcp-baseline.json"]
    )

    assert args.baseline_report == Path("/tmp/forestx-mcp-baseline.json")
...
def test_regression_cli_extracts_compact_baseline_without_starting_conformance(
    ...
    forestx = tmp_path / "forestx"
    forestx.write_text("#!/bin/sh\nexit 99\n", encoding="utf-8")
    forestx.chmod(0o755)
...
def test_regression_cli_rejects_extraction_without_a_source_report(
    ...
    assert main([str(forestx), "--extract-baseline", str(tmp_path / "compact.json")]) == 2
...
def test_regression_cli_rejects_malformed_baselines(
    ...
    assert main([str(forestx), "--baseline-report", str(baseline)]) == 2
...
def test_regression_cli_rejects_missing_baseline_before_starting_conformance(
    ...
    assert main([str(forestx), "--baseline-report", str(tmp_path / "missing.json")]) == 2

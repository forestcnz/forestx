#!/usr/bin/env python3
"""Black-box MCP compliance runner for a supplied Forestx binary."""
...
class AppServerClient:
    """Minimal JSONL client for the Forestx app-server protocol."""
    ...
    def __init__(
        ...
        *,
        ...
        self._stdout_thread = threading.Thread(
            ...
            name="forestx-app-server-stdout",
            daemon=True,
        )
        self._stderr_thread = threading.Thread(
            target=self._read_stderr,
            name="forestx-app-server-stderr",
            daemon=True,
        )
    ...
    def _send(self, message: Mapping[str, object]) -> None:
        if self.process.poll() is not None:
            raise AppServerError(
                f"Forestx app-server exited with code {self.process.returncode}"
            )
    ...
    def request(
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Forestx app-server closed stdout while handling {method}"
                )
    ...
    def wait_for_notification(
        ...
        *,
        ...
        while True:
            ...
            if message is None:
                raise AppServerError(
                    f"Forestx app-server closed stdout while waiting for {method}"
                )
            if "id" in message and isinstance(message.get("method"), str):
                self._handle_server_request(message)
                continue
...
def _isolated_environment(forestx_home: Path) -> dict[str, str]:
    env = dict(os.environ)
    env["FORESTX_HOME"] = str(forestx_home)
    # Direct app-server MCP calls do not need a model or credentials.
    for name in ("FORESTX_API_KEY", "FORESTX_ACCESS_TOKEN", "OPENAI_API_KEY"):
        env.pop(name, None)
    return env
...
def _registration_command(
    ...
    *,
    ...
    command = [str(forestx_binary), "mcp", "add", TEST_SERVER_NAME]
    if transport == "stdio":
        if mode == MODERN_VERSION:
            command.extend(["--env", f"FORESTX_MCP_PROTOCOL_VERSION={MODERN_VERSION}"])
        return [
            *command,
            "--",
            sys.executable,
...
def _validate_registration(
    ...
    *,
    ...
    if transport == "stdio":
        ...
        modern_opt_in = (
            isinstance(env, dict)
            and env.get("FORESTX_MCP_PROTOCOL_VERSION") == MODERN_VERSION
        )
...
def _exercise_app_server(
    ...
    *,
    ...
    sentinel = f"forestx-mcp-{transport}-{mode}"
...
def _run_case(
    ...
    *,
    ...
    try:
        ...
        if registered:
            remove = _run_command(
                [str(forestx_binary), "mcp", "remove", TEST_SERVER_NAME],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
            ...
            listed = _run_command(
                [str(forestx_binary), "mcp", "list", "--json"],
                env=env,
                cwd=workspace,
                timeout_seconds=timeout_seconds,
            )
...
def _run_official_case(
    ...
    *,
    ...
    case_home.mkdir(parents=True)
    env = _isolated_environment(case_home)
    env["FORESTX_CONFORMANCE_TIMEOUT"] = str(timeout_seconds)
    env["FORESTX_CONFORMANCE_ENABLE_MODERN_FEATURE"] = (
        "1" if enable_modern_feature else "0"
    )
    env["FORESTX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH"] = (
        "1" if require_automatic_auth else "0"
    )
    results = run_official_mode(
        conformance_command=conformance_command,
        adapter_script=adapter_script,
        forestx_binary=forestx_binary,
        mode=mode,
        scenarios=scenarios,
    ...
    for result in results:
        ...
        case.checks.append(
            CheckResult(
                name=f"harness/{result.scenario}/forestx-adapter",
...
def run_compliance(
    ...
    *,
    ...
    report: dict[str, object] = {
        ...
        "forestxBinary": str(forestx_binary),
        "forestxVersion": version.stdout or None,
...
def _required_regression_checks(
    ...
    *,
    ...
    if not isinstance(version_check, dict) or version_check.get("success") is not True:
        errors.append(f"{label} does not contain a successful Forestx version check")
...
def _print_human_report(report: Mapping[str, object]) -> None:
    ...
    print(f"Forestx MCP compliance: {'PASS' if success else 'FAIL'}")
    print(f"Binary: {report.get('forestxBinary')}")
    print(f"Version: {report.get('forestxVersion') or 'unknown'}")
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "supplied Forestx binary, plus the supplemental local stdio suite."
        )
    )
    parser.add_argument("forestx_binary", type=Path)
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--adapter-script",
        type=Path,
        default=Path(__file__).resolve().with_name("forestx_conformance_adapter.py"),
        help="Path to the Forestx adapter used by the official client suite.",
    )
    parser.add_argument(
        "--conformance-cli",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Require Forestx to recover from OAuth scope escalation and "
    ...
    parser.add_argument(
        "--artifact-parent",
        type=Path,
        help="Parent directory for isolated temporary Forestx homes.",
    )
    parser.add_argument(
        "--keep-artifacts",
        action="store_true",
        help="Keep isolated Forestx homes and include their path in the report.",
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    forestx_binary = args.forestx_binary.expanduser().resolve()
    ...
    if not forestx_binary.is_file() or not os.access(forestx_binary, os.X_OK):
        print(f"error: Forestx binary is not executable: {forestx_binary}", file=sys.stderr)
        return 2

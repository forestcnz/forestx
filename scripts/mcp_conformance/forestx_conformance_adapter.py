#!/usr/bin/env python3
"""Adapt one official MCP client scenario to Forestx app-server requests."""
...
from run_forestx_compliance import (  # noqa: E402 - direct scripts must first add their sibling directory.
    MODERN_VERSION,
    TEST_SERVER_NAME,
    AppServerClient,
    AppServerError,
...
def _elicitation_content(
    ...
    if scenario == "elicitation-sep1034-client-defaults":
        # The official scenario deliberately supplies no values. Forestx, as the
...
def _validated_callback_url(authorization_url: str, location: str) -> str:
    ...
    if (
        ...
        raise AdapterFailure(
            "authorization server redirect did not target Forestx's exact loopback callback"
        )
...
def _drive_headless_authorization(
    ...
    *,
    ...
    if not 200 <= callback_status < 500:
        raise AdapterFailure("Forestx OAuth callback endpoint returned an invalid status")
...
def _validate_oauth_secret_not_persisted(forestx_home: Path, client_secret: str) -> None:
    if not client_secret:
        return

    candidates = [forestx_home / "config.toml", *forestx_home.rglob(".credentials.json")]
    ...
    for candidate in dict.fromkeys(candidates):
        ...
        if secret_bytes in candidate.read_bytes():
            raise AdapterFailure(
                ...
                f"{candidate.relative_to(forestx_home)}"
            )
    ...
    *,
    ...
    if scenario == "auth/scope-step-up":
        ...
        if require_automatic_auth:
            _auth_tool_call(client, workspace)
            return "Forestx automatically recovered from the challenged OAuth scope"
        try:
            _auth_tool_call(client, workspace)
    ...
    if scenario == "auth/scope-retry-limit":
        if require_automatic_auth:
            ...
            try:
                ...
                return "observed Forestx's production OAuth retry-limit behavior"
            raise AdapterFailure(
    ...
    if scenario == "auth/authorization-server-migration":
        ...
        if require_automatic_auth:
            _auth_tool_call(client, workspace)
            return (
                "Forestx automatically registered with the migrated authorization server"
            )
        try:
            _auth_tool_call(client, workspace)
        except AdapterFailure:
...
def run_adapter(server_url: str) -> dict[str, object]:
    forestx_binary = _required_path("FORESTX_CONFORMANCE_BINARY")
    forestx_home = _required_path("FORESTX_CONFORMANCE_HOME")
    scenario = _required_env("MCP_CONFORMANCE_SCENARIO")
    protocol_version = _required_env("MCP_CONFORMANCE_PROTOCOL_VERSION")
    timeout_seconds = float(os.environ.get("FORESTX_CONFORMANCE_TIMEOUT", "30"))
    enable_modern_feature = (
        os.environ.get("FORESTX_CONFORMANCE_ENABLE_MODERN_FEATURE", "1") != "0"
    )
    require_automatic_auth = (
        os.environ.get("FORESTX_CONFORMANCE_REQUIRE_AUTOMATIC_AUTH", "0") == "1"
    )
    context = _conformance_context()

    forestx_home.mkdir(parents=True, exist_ok=True)
    workspace = forestx_home / "workspace"
    workspace.mkdir()
    env = _isolated_environment(forestx_home)
    # Scenario context can contain ephemeral OAuth client secrets. The adapter
    # consumes it directly and does not expose the full blob to Forestx.
    ...
    try:
        if scenario.startswith("auth/"):
            config_path = forestx_home / "config.toml"
        ...
        if scenario.startswith("auth/"):
            ...
            if not registered:
                raise AdapterFailure("forestx mcp add failed")
        ...
        if not registration_ok:
            raise AdapterFailure("Forestx registration did not preserve the scenario URL")
            ...
            if scenario.startswith("auth/"):
                ...
                if scenario == "auth/pre-registration" and isinstance(
                    ...
                    _validate_oauth_secret_not_persisted(
                        forestx_home, oauth_client_secret
                    )
                    steps.append(
                        Step(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report_path_value = os.environ.get("FORESTX_CONFORMANCE_ADAPTER_REPORT")

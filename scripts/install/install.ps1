[CmdletBinding()]
param(
    [string]$Release = $env:FORESTX_RELEASE
)
...
$NonInteractive = $env:FORESTX_NON_INTERACTIVE -match "^(?i:1|true|yes)$"
$DefaultPreferReleasesOpenAICom = $true
$PreferReleasesOpenAICom = if ([string]::IsNullOrWhiteSpace($env:FORESTX_INSTALLER_USE_RELEASES_OPENAI_COM)) {
    $DefaultPreferReleasesOpenAICom
} else {
    $env:FORESTX_INSTALLER_USE_RELEASES_OPENAI_COM -match "^(?i:1|true|yes)$"
}
$ReleasesBaseUri = "https://releases.openai.com/forestx"
...
function Assert-ValidReleaseVersion {
    ...
    if ($Version -cne "latest" -and $Version -cnotmatch "^[0-9]+\.[0-9]+\.[0-9]+(?:-alpha(?:\.[0-9]+){0,2}|-beta(?:\.[0-9]+)?)?$") {
        throw "Invalid Forestx release version: $Version. Expected latest or x.y.z[-alpha[.N[.M]]|-beta[.N]]."
    }
}
...
function Resolve-ReleaseAssetSelection {
    ...
    $packageAsset = "forestx-package-$Target.tar.gz"
    $checksumAsset = "forestx-package_SHA256SUMS"
    $packageUrl = $null
    ...
    if ($ResolvedRelease.Source -eq "ReleasesOpenAICom") {
        ...
        $packageFallbackUrl = "https://github.com/openai/forestx/releases/download/rust-v$version/$packageAsset"
        $checksumUrl = "$ReleasesBaseUri/releases/$version/$checksumAsset"
        $checksumFallbackUrl = "https://github.com/openai/forestx/releases/download/rust-v$version/$checksumAsset"
    }
    ...
    $packageAsset = "forestx-npm-$NpmTag-$version.tgz"
    $packageUrl = $null
    $packageFallbackUrl = $null
    if ($ResolvedRelease.Source -eq "ReleasesOpenAICom") {
        $packageUrl = "$ReleasesBaseUri/releases/$version/$packageAsset"
    ...
    if ($null -eq $packageMetadata) {
        throw "Could not find Forestx package or platform npm release assets for Forestx $version."
    }
...
function Test-ArchiveDigest {
    ...
    if ($actualDigest -ne $ExpectedDigest) {
        throw "Downloaded Forestx archive checksum did not match expected digest. Expected $ExpectedDigest but got $actualDigest."
    }
}
...
function Get-PackageArchiveDigest {
    ...
    throw "Could not find SHA-256 digest for $AssetName in forestx-package_SHA256SUMS."
}
...
function Resolve-VersionFromReleaseMetadata {
    ...
    if (-not $ReleaseMetadata.tag_name) {
        throw "Failed to resolve the latest Forestx release version."
    }
...
function Resolve-ReleaseFromGitHub {
    ...
    if ($NormalizedVersion -eq "latest") {
        ...
        $metadataUri = "https://api.github.com/repos/openai/forestx/releases/latest"
    } else {
        $resolvedVersion = $NormalizedVersion
        $requestedRelease = $resolvedVersion
        $metadataUri = "https://api.github.com/repos/openai/forestx/releases/tags/rust-v$resolvedVersion"
    }

    try {
        $releaseMetadata = Invoke-RestMethod -Uri $metadataUri
    } catch {
        throw "Could not fetch GitHub release metadata for Forestx $requestedRelease. GitHub API may be unavailable or rate limited. $($_.Exception.Message)"
    }
...
function Resolve-ReleaseFromReleases {
    ...
    try {
        ...
        if ($NormalizedVersion -ne "latest" -and $resolvedVersion -cne $NormalizedVersion) {
            throw "Release metadata version did not match requested Forestx version $NormalizedVersion."
        }
...
function Get-VersionFromBinary {
    ...
    if (-not (Test-Path -LiteralPath $ForestxPath -PathType Leaf)) {
        return $null
    }

    try {
        $versionOutput = & $ForestxPath --version 2>$null
    } catch {
        return $null
    }
...
function Get-CurrentInstalledVersion {
    ...
    $standaloneVersion = Get-VersionFromBinary -ForestxPath (Join-Path $StandaloneCurrentDir "bin\forestx.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }

    $standaloneVersion = Get-VersionFromBinary -ForestxPath (Join-Path $StandaloneCurrentDir "forestx.exe")
    if (-not [string]::IsNullOrWhiteSpace($standaloneVersion)) {
        return $standaloneVersion
    }
...
function Test-OldStandaloneBinLayout {
    ...
    $requiredFiles = @("forestx.exe", "rg.exe")
    ...
    $knownFiles = @(
        "forestx.exe",
        "rg.exe",
        "forestx-command-runner.exe",
        "forestx-windows-sandbox.exe",
        "forestx-windows-sandbox-setup.exe"
    )
...
function Move-OldStandaloneBinIfApproved {
    ...
    Write-Step "We found an older Forestx install at $VisibleBinDir"
    Write-WarningStep "To continue, Forestx needs to update the install at this path."
    if (-not (Prompt-YesNo "Replace it with the current Forestx setup now?")) {
...
function Add-JunctionSupportType {
    if (([System.Management.Automation.PSTypeName]'ForestxInstaller.Junction').Type) {
        return
    }
...
function Set-JunctionTarget {
    ...
    [ForestxInstaller.Junction]::SetTarget($LinkPath, $TargetPath)
}
...
function Test-PackageContentsAreComplete {
    ...
    $expectedFiles = @(
        "forestx-package.json",
        "bin\forestx.exe",
        "bin\forestx-code-mode-host.exe",
        "forestx-path\rg.exe",
        "forestx-resources\forestx-command-runner.exe",
        "forestx-resources\forestx-windows-sandbox-setup.exe"
    )
...
function Test-ReleaseIsComplete {
    ...
    switch ($Layout) {
        "Package" {
            ...
            $forestxPath = Join-Path $ReleaseDir "bin\forestx.exe"
        }
        "LegacyPlatformNpm" {
            if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $ReleaseDir)) {
                return $false
            }
            $forestxPath = Join-Path $ReleaseDir "forestx.exe"
        }
        default {
            throw "Unknown Forestx installer layout: $Layout"
        }
    }

    return (Split-Path -Leaf $ReleaseDir) -eq "$ExpectedVersion-$ExpectedTarget" -and
        (Get-VersionFromBinary -ForestxPath $forestxPath) -ceq $ExpectedVersion
}

function Get-ExistingForestxCommand {
    $existing = Get-Command forestx -ErrorAction SilentlyContinue
    if ($null -eq $existing) {
        return $null
    }
...
function Get-ExistingForestxManager {
    param(
        [string]$ExistingPath,
        [string]$VisibleBinDir
    )
...
function Get-ConflictingInstall {
    ...
    $existingPath = Get-ExistingForestxCommand
    $manager = Get-ExistingForestxManager -ExistingPath $existingPath -VisibleBinDir $VisibleBinDir
    if ($null -eq $manager) {
        return $null
    }

    Write-Step "Detected existing $manager-managed Forestx at $existingPath"
    Write-WarningStep "Multiple managed Forestx installs can be ambiguous because PATH order decides which one runs."
...
function Maybe-HandleConflictingInstall {
    ...
    $uninstallArgs = if ($manager -eq "bun") {
        @("remove", "-g", "@openai/forestx")
    } else {
        @("uninstall", "-g", "@openai/forestx")
    }
    $uninstallCommand = if ($manager -eq "bun") { "bun" } else { "npm" }

    if (Prompt-YesNo "Uninstall the existing $manager-managed Forestx now?") {
        Write-Step "Running: $uninstallCommand $($uninstallArgs -join ' ')"
        try {
            & $uninstallCommand @uninstallArgs
        } catch {
            Write-WarningStep "Failed to uninstall the existing $manager-managed Forestx. Continuing with the standalone install."
        }
    } else {
        Write-WarningStep "Leaving the existing $manager-managed Forestx installed. PATH order will determine which forestx runs."
    }
}

function Test-VisibleForestxCommand {
    param(
        [string]$VisibleBinDir
    )

    $forestxCommand = Join-Path $VisibleBinDir "forestx.exe"
    & $forestxCommand --version *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "Installed Forestx command failed verification: $forestxCommand --version"
    }
}
...
if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Error "Forestx requires a 64-bit version of Windows."
    exit 1
}
...
$forestxHome = if ([string]::IsNullOrWhiteSpace($env:FORESTX_HOME)) {
    Join-Path $env:USERPROFILE ".forestx"
} else {
    $env:FORESTX_HOME
}
$standaloneRoot = Join-Path $forestxHome "packages\standalone"
$releasesDir = Join-Path $standaloneRoot "releases"
$currentDir = Join-Path $standaloneRoot "current"
$lockPath = Join-Path $standaloneRoot "install.lock"

$defaultVisibleBinDir = Join-Path $env:LOCALAPPDATA "Programs\OpenAI\Forestx\bin"
if ([string]::IsNullOrWhiteSpace($env:FORESTX_INSTALL_DIR)) {
    $visibleBinDir = $defaultVisibleBinDir
} else {
    $visibleBinDir = $env:FORESTX_INSTALL_DIR
}
...
if (-not [string]::IsNullOrWhiteSpace($currentVersion) -and $currentVersion -ne $resolvedVersion) {
    Write-Step "Updating Forestx CLI from $currentVersion to $resolvedVersion"
} elseif (-not [string]::IsNullOrWhiteSpace($currentVersion)) {
    Write-Step "Updating Forestx CLI"
} else {
    Write-Step "Installing Forestx CLI"
}
...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("forestx-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
    Invoke-WithInstallLock -LockPath $lockPath -Script {
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            ...
            Write-Step "Downloading Forestx CLI"
            ...
            if ($installLayout -eq "Package") {
                ...
                if (-not (Test-PackageContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Forestx package archive did not contain the expected package layout."
                }
            } else {
                ...
                $resourcesDir = Join-Path $stagingDir "forestx-resources"
                New-Item -ItemType Directory -Force -Path $resourcesDir | Out-Null
                $copyMap = @{
                    "forestx/forestx.exe" = "forestx.exe"
                    "forestx/forestx-command-runner.exe" = "forestx-resources\forestx-command-runner.exe"
                    "forestx/forestx-windows-sandbox-setup.exe" = "forestx-resources\forestx-windows-sandbox-setup.exe"
                    "path/rg.exe" = "forestx-resources\rg.exe"
                }
                ...
                if (-not (Test-LegacyPlatformNpmContentsAreComplete -PackageDir $stagingDir)) {
                    throw "Downloaded Forestx npm archive did not contain the expected legacy platform package layout."
                }
            }
        ...
        if (-not (Test-ReleaseIsComplete -ReleaseDir $releaseDir -ExpectedVersion $resolvedVersion -ExpectedTarget $target -Layout $installLayout)) {
            throw "Installed Forestx command did not report expected version $resolvedVersion."
        }
        ...
        try {
            ...
            Test-VisibleForestxCommand -VisibleBinDir $visibleBinDir
        } catch {
...
Write-Step "Current PowerShell session: forestx"
Write-Step "Future PowerShell windows: open a new PowerShell window and run: forestx"
Write-Host "Forestx CLI $resolvedVersion installed successfully."

$forestxCommand = Join-Path $visibleBinDir "forestx.exe"
if (Prompt-YesNo "Start Forestx now?") {
    Write-Step "Launching Forestx"
    & $forestxCommand
}

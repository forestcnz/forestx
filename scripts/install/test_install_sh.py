...
class InstallShTest(unittest.TestCase):
    def test_metadata_fetch_failure_is_not_reported_as_missing_assets(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/openai/forestx/releases/tags/"
                f"rust-v{VERSION}"
            ],
        )
        self.assertIn(
            f"Could not fetch GitHub release metadata for Forestx {VERSION}",
            result.stderr,
        )
        self.assertNotIn("Could not find Forestx package", result.stderr)
    ...
    def test_exact_release_opt_out_uses_github_metadata_once(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "https://api.github.com/repos/openai/forestx/releases/tags/"
                f"rust-v{VERSION}",
                "https://github.com/openai/forestx/releases/download/"
                f"rust-v{VERSION}/forestx-package_SHA256SUMS",
            ],
        )
    ...
    def test_latest_release_reuses_version_metadata(self) -> None:
        ...
        self.assertEqual(
            requests,
            [
                "https://api.github.com/repos/openai/forestx/releases/latest",
                "https://github.com/openai/forestx/releases/download/"
                f"rust-v{VERSION}/forestx-package_SHA256SUMS",
            ],
        )
    ...
    def test_json_like_strings_and_nested_fields_do_not_define_assets(self) -> None:
        ...
        self.assertIn("/forestx-npm-", requests[1])
        self.assertNotIn("forestx-package_SHA256SUMS", requests[1])

    def test_macos_install_exposes_code_mode_host_beside_forestx(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            current = root / "forestx-home" / "packages" / "standalone" / "current"
            forestx_path = install_bin / "forestx"
            host_path = install_bin / "forestx-code-mode-host"
            self.assertEqual(os.readlink(forestx_path), str(current / "bin" / "forestx"))
            self.assertEqual(
                os.readlink(host_path),
                str(current / "bin" / "forestx-code-mode-host"),
            )
    ...
    def test_releases_latest_installs_verified_package_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                requests,
                [
                    "https://releases.openai.com/forestx/channels/latest",
                    f"https://releases.openai.com/forestx/releases/{VERSION}/forestx-package_SHA256SUMS",
                    f"https://releases.openai.com/forestx/releases/{VERSION}/forestx-package-aarch64-apple-darwin.tar.gz",
                ],
            )
            ...
            "invalid_checksum_digest": json.dumps(
                {
                    ...
                    "assets": [
                        {
                            "name": "forestx-package-aarch64-apple-darwin.tar.gz",
                            "digest": "sha256:" + "a" * 64,
                        },
                        {
                            "name": "forestx-package_SHA256SUMS",
                            "digest": "sha256:" + "z" * 64,
                        },
                    ],
                }
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    self.assertEqual(
                        ...
                        [
                            ...
                            f"rust-v{VERSION}/forestx-package_SHA256SUMS",
                            "https://github.com/openai/forestx/releases/download/"
                            f"rust-v{VERSION}/forestx-package-aarch64-apple-darwin.tar.gz",
                        ],
                    )
    ...
    def test_releases_exact_metadata_version_mismatch_falls_back_to_github(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                requests,
                [
                    f"https://releases.openai.com/forestx/releases/{VERSION}/release.json",
                    "https://api.github.com/repos/openai/forestx/releases/tags/"
                    f"rust-v{VERSION}",
                    "https://github.com/openai/forestx/releases/download/"
                    f"rust-v{VERSION}/forestx-package_SHA256SUMS",
    ...
    def test_releases_wrong_checksum_digest_uses_github_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for release_asset in mirror_metadata["assets"]:
                if release_asset["name"] == "forestx-package_SHA256SUMS":
    ...
    def test_releases_incomplete_checksum_manifest_falls_back_to_github(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mirror_checksum_path.write_text(
                f"{'a' * 64}  forestx-package-other-platform.tar.gz\n",
                encoding="utf-8",
            )
            mirror_metadata = json.loads(metadata_json)
            for release_asset in mirror_metadata["assets"]:
    ...
    def test_releases_exact_rejects_wrong_binary_version(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                requests,
                [
                    f"https://releases.openai.com/forestx/releases/{MISMATCH_VERSION}/release.json",
                    f"https://releases.openai.com/forestx/releases/{MISMATCH_VERSION}/forestx-package_SHA256SUMS",
                    f"https://releases.openai.com/forestx/releases/{MISMATCH_VERSION}/forestx-package-aarch64-apple-darwin.tar.gz",
                ],
            )
            self.assertIn(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                ...
                [
                    ...
                    f"rust-v{VERSION}",
                    "https://github.com/openai/forestx/releases/download/"
                    f"rust-v{VERSION}/forestx-npm-darwin-arm64-{VERSION}.tgz",
                ],
            )
            ...
            self.assertNotIn("Downloading Forestx CLI", second_result.stdout)
    ...
    *,
    ...
    fake_curl.write_text(
        textwrap.dedent(
            ...
            printf '%s\n' "$url" >>"$FORESTX_TEST_REQUEST_LOG"

            case "$url" in
              https://api.github.com/*)
                if [ "$FORESTX_TEST_METADATA_FAILURE" = "1" ]; then
                  echo "curl: (22) The requested URL returned error: 403" >&2
                  exit 22
                fi
                printf '%s\n' "$FORESTX_TEST_METADATA_JSON"
                ;;
              https://releases.openai.com/forestx/channels/latest|https://releases.openai.com/forestx/releases/*/release.json)
                if [ "$FORESTX_TEST_RELEASES_MODE" = "channel_failure" ]; then
                  exit 22
                fi
                printf '%s\n' "$FORESTX_TEST_RELEASES_METADATA_JSON"
                ;;
              https://releases.openai.com/forestx/releases/*/forestx-package_SHA256SUMS)
                if [ "$FORESTX_TEST_RELEASES_MODE" = "asset_fallback" ]; then
                  exit 22
                fi
                if [ "$FORESTX_TEST_RELEASES_MODE" = "corrupt_assets" ] ||
                  [ "$FORESTX_TEST_RELEASES_MODE" = "corrupt_checksum_and_github" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$FORESTX_TEST_RELEASES_CHECKSUM_PATH" ]; then
                  cp "$FORESTX_TEST_RELEASES_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://releases.openai.com/forestx/releases/*/forestx-package-*.tar.gz)
                if [ "$FORESTX_TEST_RELEASES_MODE" = "asset_fallback" ]; then
                  exit 22
                fi
                if [ "$FORESTX_TEST_RELEASES_MODE" = "corrupt_assets" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$FORESTX_TEST_ARCHIVE_PATH" ]; then
                  cp "$FORESTX_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://github.com/openai/forestx/releases/download/*/forestx-package_SHA256SUMS)
                if [ "$FORESTX_TEST_RELEASES_MODE" = "corrupt_checksum_and_github" ]; then
                  printf '<html>proxy error</html>\n' >"$output"
                  exit 0
                fi
                if [ -n "$FORESTX_TEST_CHECKSUM_PATH" ]; then
                  cp "$FORESTX_TEST_CHECKSUM_PATH" "$output"
                else
                  exit 22
                fi
                ;;
              https://github.com/openai/forestx/releases/download/*/forestx-package-*.tar.gz)
                if [ -n "$FORESTX_TEST_ARCHIVE_PATH" ]; then
                  cp "$FORESTX_TEST_ARCHIVE_PATH" "$output"
                else
                  exit 22
                ...
                ;;
              https://github.com/openai/forestx/releases/download/*/forestx-npm-*.tgz)
                if [ -n "$FORESTX_TEST_LEGACY_ARCHIVE_PATH" ]; then
                  cp "$FORESTX_TEST_LEGACY_ARCHIVE_PATH" "$output"
                else
                  exit 22
                fi
                ;;
    ...
    env.update(
        {
            "FORESTX_HOME": str(root / "forestx-home"),
            "FORESTX_INSTALL_DIR": str(root / "install-bin"),
            "FORESTX_NON_INTERACTIVE": "1",
            "FORESTX_RELEASE": release,
            "FORESTX_TEST_ARCHIVE_PATH": str(archive_path or ""),
            "FORESTX_TEST_CHECKSUM_PATH": str(checksum_path or ""),
            "FORESTX_TEST_RELEASES_CHECKSUM_PATH": str(
                releases_checksum_path or checksum_path or ""
            ),
            "FORESTX_TEST_LEGACY_ARCHIVE_PATH": str(legacy_archive_path or ""),
            "FORESTX_TEST_METADATA_FAILURE": "1" if metadata_failure else "0",
            "FORESTX_TEST_METADATA_JSON": (
            ...
            "FORESTX_TEST_RELEASES_MODE": releases_mode,
            "FORESTX_TEST_REQUEST_LOG": str(request_log),
            "HOME": str(home),
    ...
    if use_mirror is None:
        env.pop("FORESTX_INSTALLER_USE_RELEASES_OPENAI_COM", None)
    else:
        env["FORESTX_INSTALLER_USE_RELEASES_OPENAI_COM"] = (
...
def create_package_release(
    ...
    *,
    ...
    (package_dir / "forestx-package.json").write_text("{}\n", encoding="utf-8")
    write_executable(
        package_dir / "bin" / "forestx",
        f"#!/bin/sh\nprintf 'forestx-cli {VERSION}\\n'\n",
    )
    write_executable(
        package_dir / "bin" / "forestx-code-mode-host",
        "#!/bin/sh\nexit 0\n",
    )
    write_executable(package_dir / "forestx-path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = "forestx-package-aarch64-apple-darwin.tar.gz"
    ...
    checksum_path = root / "forestx-package_SHA256SUMS"
...
def create_legacy_release(root: Path) -> tuple[Path, str]:
    ...
    (vendor_dir / "forestx").mkdir(parents=True)
    (vendor_dir / "path").mkdir()
    write_executable(
        vendor_dir / "forestx" / "forestx",
        f"#!/bin/sh\nprintf 'forestx-cli {VERSION}\\n'\n",
    )
    write_executable(vendor_dir / "path" / "rg", "#!/bin/sh\nexit 0\n")

    asset = f"forestx-npm-darwin-arm64-{VERSION}.tgz"
    archive_path = root / asset
    with tarfile.open(archive_path, "w:gz") as archive:
...
def release_metadata(*, compact: bool = False, reorder: bool = False) -> str:
    assets = [
        asset_metadata(
            f"forestx-package-{target}.tar.gz",
            f"sha256:{'a' * 64}",
            reorder=reorder,
        )
        for target in (
...
def legacy_release_metadata_with_decoys() -> str:
    ...
    assets = [
        {
            "metadata": {
                "name": "forestx-package-x86_64-unknown-linux-musl.tar.gz",
                "digest": fake_digest,
            },
            "digest": f"sha256:{'c' * 64}",
            "name": f"forestx-npm-{target}-{VERSION}.tgz",
        }
    ...
    return json.dumps(
        {
            "body": (
                f'fake: {{"name":"forestx-package_SHA256SUMS","digest":"{fake_digest}"}}'
            ),
            "assets": assets,
            "tag_name": f"rust-v{VERSION}",
        },

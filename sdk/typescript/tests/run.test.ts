...
import { forestxExecSpy } from "./forestxExecSpy";
...
import { createMockClient, createTestClient } from "./testForestx";
...
describe("Forestx", () => {
  ...
  it("passes turn options to exec", async () => {
    ...
    const { args: spawnArgs, restore } = forestxExecSpy();
    const { client, cleanup } = createMockClient(url);
  ...
  it("passes ForestxOptions config overrides as TOML --config flags", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("lets thread options override ForestxOptions config overrides", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("forwards images to exec", async () => {
    ...
    const { args: spawnArgs, restore } = forestxExecSpy();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "forestx-images-"));
  ...
  it("runs in provided working directory", async () => {
    ...
    const { args: spawnArgs, restore } = forestxExecSpy();
    const workingDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "forestx-working-dir-"));
    const { client, cleanup } = createTestClient({
      baseUrl: url,
      apiKey: "test",
    });
  ...
  it("sets the forestx sdk originator header", async () => {
    const { url, close, requests } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [sse(responseStarted(), assistantMessage("Hi!"), responseCompleted())],
    });
    ...
    try {
      ...
      if (Array.isArray(originatorHeader)) {
        expect(originatorHeader).toContain("forestx_sdk_ts");
      } else {
        expect(originatorHeader).toBe("forestx_sdk_ts");
      }
    } finally {
      cleanup();
      await close();
...
/**
 * Given a list of args to `forestx` and a `key`, collects all `--config`

...
describe("ForestxExec", () => {
  it("rejects when exit happens before stdout closes", async () => {
    ...
    if (result.status === "rejected") {
      ...
      expect(result.error.message).toMatch(/Forestx Exec exited/);
    }
  });
  ...
  it("allows overriding the env passed to the Forestx CLI", async () => {
    const { ForestxExec } = await import("../src/exec");
    spawnMock.mockClear();
    const child = new FakeChildProcess();
    spawnMock.mockReturnValue(child as unknown as child_process.ChildProcess);
    ...
    process.env.FORESTX_ENV_SHOULD_NOT_LEAK = "leak";

    try {
      const exec = new ForestxExec("forestx", {
        FORESTX_HOME: "/tmp/forestx-home",
        CUSTOM_ENV: "custom",
      });
      ...
      expect(spawnEnv.FORESTX_HOME).toBe("/tmp/forestx-home");
      expect(spawnEnv.CUSTOM_ENV).toBe("custom");
      expect(spawnEnv.FORESTX_ENV_SHOULD_NOT_LEAK).toBeUndefined();
      expect(spawnEnv.FORESTX_API_KEY).toBe("test");
      expect(spawnEnv.FORESTX_INTERNAL_ORIGINATOR_OVERRIDE).toBeDefined();
      expect(commandArgs).toContain("--config");
      expect(commandArgs).toContain(`openai_base_url=${JSON.stringify("https://example.test")}`);
    } finally {
      delete process.env.FORESTX_ENV_SHOULD_NOT_LEAK;
    }
  });

  it("resolves the package-layout binary and PATH directory", async () => {
    const { resolveNativePackage } = await import("../src/exec");
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "forestx-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "bin");
    const pathDir = path.join(packageRoot, "forestx-path");
    mkdirSync(binDir, { recursive: true });
    mkdirSync(pathDir, { recursive: true });
    writeFileSync(path.join(packageRoot, "forestx-package.json"), "{}");
    writeFileSync(path.join(binDir, "forestx"), "");

    expect(resolveNativePackage(vendorRoot, "x86_64-unknown-linux-musl", "forestx")).toEqual({
      executablePath: path.join(binDir, "forestx"),
      pathDirs: [pathDir],
    });
  });
  ...
  it("falls back to the legacy binary layout", async () => {
    ...
    const vendorRoot = mkdtempSync(path.join(tmpdir(), "forestx-sdk-vendor-"));
    const packageRoot = path.join(vendorRoot, "x86_64-unknown-linux-musl");
    const binDir = path.join(packageRoot, "forestx");
  ...
  it("prepends package PATH entries without duplicating them", async () => {
    ...
    const pathDir = path.join(tmpdir(), "forestx-path");

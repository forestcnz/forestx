...
_NORMALIZED_FORESTX_VERSION_PATTERN = re.compile(
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Resolve a Python runtime package version to its Forestx release tag."
    )
...
def resolve_python_runtime_release(python_version: str) -> tuple[str, str]:
    ...
    return python_version, forestx_release_tag(python_version)


def forestx_release_tag(version: str) -> str:
    return f"rust-v{forestx_release_version(version)}"


def forestx_release_version(version: str) -> str:
    normalized = normalize_forestx_version(version)
...
def normalize_forestx_version(version: str) -> str:
    ...
    if _NORMALIZED_FORESTX_VERSION_PATTERN.fullmatch(normalized) is None:
        raise RuntimeError(f"Could not normalize Forestx version {version!r} to a PEP 440 version")
    return normalized

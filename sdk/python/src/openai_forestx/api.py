...
from .async_client import AsyncForestxClient
from .client import ForestxClient, ForestxConfig
...
class Forestx:
    """Synchronous client for creating threads and running Forestx turns.

    The client starts its runtime connection during construction. Use it as a
    context manager so resources are closed promptly.
    """

    def __init__(self, config: ForestxConfig | None = None) -> None:
        self._client = ForestxClient(config=config)
        try:
            self._client.start()
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate Forestx with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current Forestx account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current Forestx account session."""
        self._client.account_logout()

    # BEGIN GENERATED: Forestx.flat_methods
    def thread_start(
        self,
        *,
...
class AsyncForestx:
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncForestx is not initialized yet. Prefer `async with AsyncForestx()`; "
    ...
    # BEGIN GENERATED: AsyncForestx.flat_methods
    ...
    # END GENERATED: AsyncForestx.flat_methods
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        """Read this thread, optionally including its turn history."""
        await self._forestx._ensure_initialized()
        return await self._forestx._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._forestx._ensure_initialized()
        return await self._forestx._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._forestx._ensure_initialized()
        return await self._forestx._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        """Send additional input to this active turn."""
        await self._forestx._ensure_initialized()
        return await self._forestx._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        """Request interruption of this active turn."""
        await self._forestx._ensure_initialized()
        return await self._forestx._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._forestx._ensure_initialized()
        self._forestx._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._forestx._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._forestx._client.unregister_turn_notifications(self.id)

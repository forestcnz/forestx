...
class AsyncForestxClient:
    """Async wrapper around ForestxClient using thread offloading."""

    def __init__(self, config: ForestxConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = ForestxClient(config=config)

    async def __aenter__(self) -> "AsyncForestxClient":
        """Start the Forestx process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the Forestx process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the Forestx session."""
        ...
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="forestx-goal-start-cleanup",
                    daemon=True,
                ).start()

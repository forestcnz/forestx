...
from .client import ForestxClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncForestx needed by async login handles."""

    _client: AsyncForestxClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Forestx connection."""
        ...


def start_chatgpt_login(client: ForestxClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: ForestxClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Forestx.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Forestx.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncForestx.login_chatgpt()`."""

    _forestx: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._forestx._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._forestx._ensure_initialized()
        return await self._forestx._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncForestx.login_chatgpt_device_code()`."""

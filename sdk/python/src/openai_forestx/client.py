...
from .errors import ForestxError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "openai-forestx-cli-bin"
...
def _installed_forestx_path() -> Path:
    try:
        from forestx_cli_bin import bundled_forestx_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Forestx runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set ForestxConfig.forestx_bin "
            "explicitly."
        ) from exc

    return bundled_forestx_path()


def _installed_forestx_path_dirs() -> tuple[Path, ...]:
    try:
        from forestx_cli_bin import bundled_path_dir
...
class ForestxBinResolverOps:
    installed_forestx_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_forestx_bin_resolver_ops() -> ForestxBinResolverOps:
    return ForestxBinResolverOps(
        installed_forestx_path=_installed_forestx_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_forestx_bin(config: "ForestxConfig", ops: ForestxBinResolverOps) -> Path:
    if config.forestx_bin is not None:
        forestx_bin = Path(config.forestx_bin)
        if not ops.path_exists(forestx_bin):
            raise FileNotFoundError(
                f"Forestx binary not found at {forestx_bin}. Set ForestxConfig.forestx_bin "
                "to a valid binary path."
            )
        return forestx_bin

    return ops.installed_forestx_path()


def _resolve_forestx_bin(config: "ForestxConfig") -> Path:
    return resolve_forestx_bin(config, _default_forestx_bin_resolver_ops())


@dataclass(slots=True)
class ForestxConfig:
    """Configuration for launching and identifying the local Forestx runtime.

    Most callers can use ``Forestx()`` without configuration. Set ``forestx_bin``
    only when intentionally using a specific local Forestx executable.
    """

    forestx_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "forestx_python_sdk"
    client_title: str = "Forestx Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class ForestxClient:
    """Synchronous typed JSON-RPC client for `forestx app-server` over stdio."""

    def __init__(
        self,
        config: ForestxConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or ForestxConfig()
    ...
    def __enter__(self) -> "ForestxClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            forestx_bin = _resolve_forestx_bin(self.config)
            if self.config.forestx_bin is None:
                path_dirs = _installed_forestx_path_dirs()
            args = [str(forestx_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise ForestxError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("Forestx process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"Forestx process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ForestxError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise ForestxError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_forestx_home() -> str:
    return str(Path.home() / ".forestx")

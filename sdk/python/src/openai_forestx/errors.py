...
class ForestxError(Exception):
    """Base exception for SDK errors."""


class JsonRpcError(ForestxError):
...
class TransportClosedError(ForestxError):
    """Raised when the Forestx transport closes unexpectedly."""


class ForestxRpcError(JsonRpcError):
    """Base typed error for JSON-RPC failures."""


class ParseError(ForestxRpcError):
    """Raised when a request or response cannot be parsed."""


class InvalidRequestError(ForestxRpcError):
    """Raised when the runtime rejects the request shape."""


class MethodNotFoundError(ForestxRpcError):
    """Raised when the requested operation is unavailable."""


class InvalidParamsError(ForestxRpcError):
    """Raised when an operation receives invalid parameters."""


class InternalRpcError(ForestxRpcError):
    """Raised when the runtime reports an internal RPC failure."""


class ServerBusyError(ForestxRpcError):
...
def _is_server_overloaded(data: Any) -> bool:
    ...
    if isinstance(data, dict):
        direct = data.get("forestx_error_info") or data.get("forestxErrorInfo") or data.get("errorInfo")
...
def map_jsonrpc_error(code: int, message: str, data: Any = None) -> JsonRpcError:
    ...
    if -32099 <= code <= -32000:
        ...
        if _contains_retry_limit_text(message):
            return RetryLimitExceededError(code, message, data)
        return ForestxRpcError(code, message, data)

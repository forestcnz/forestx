# OpenAI Forestx Python SDK - API Reference

Public surface of `openai_forestx` for Forestx workflows.
...
- Version: `openai_forestx.__version__`
- Requires Python >= 3.10
- Public Forestx protocol value and event types live in `openai_forestx.types`

## Forestx (sync)

```python
Forestx(config: ForestxConfig | None = None)
```
...
## AsyncForestx (async parity)

```python
AsyncForestx(config: ForestxConfig | None = None)
```
...
`AsyncForestx` initializes lazily. Context entry is the standard path because it
...
When `sandbox=` is omitted, Forestx uses its configured default. A sandbox
...
- one `Forestx` instance can stream multiple active turns concurrently
...
- `stream()` and `run()` consume only notifications for their own turn ID
- one `AsyncForestx` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public Forestx protocol models wherever possible:

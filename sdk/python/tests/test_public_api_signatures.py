...
import openai_forestx.types as public_types
...
from openai_forestx._initialize_metadata import validate_initialize_metadata
from openai_forestx.types import InitializeResponse
...
def test_root_exports_forestx_config() -> None:
    """The root package should expose the process configuration object."""
    assert ForestxConfig.__name__ == "ForestxConfig"
...
def test_package_and_default_client_versions_follow_project_version() -> None:
    ...
    assert openai_forestx.__version__ == pyproject["project"]["version"]
    assert ForestxConfig().client_version == openai_forestx.__version__
...
def test_curated_public_api_has_builtin_help_documentation() -> None:
    ...
    documented = {
        "module": openai_forestx,
        "Forestx": Forestx,
        "AsyncForestx": AsyncForestx,
        "ForestxConfig": ForestxConfig,
        "Thread": Thread,
        ...
        "Sandbox": Sandbox,
        "thread_start": Forestx.thread_start,
        "thread_resume": Forestx.thread_resume,
...
def test_package_includes_py_typed_marker() -> None:
    ...
    marker = resources.files("openai_forestx").joinpath("py.typed")
...
def test_package_root_exports_only_public_api() -> None:
    ...
    assert openai_forestx.__all__ == EXPECTED_ROOT_EXPORTS
    assert {name: hasattr(openai_forestx, name) for name in EXPECTED_ROOT_EXPORTS} == dict.fromkeys(
        EXPECTED_ROOT_EXPORTS, True
    )
    assert {
        "ForestxClient": hasattr(openai_forestx, "ForestxClient"),
        "AsyncForestxClient": hasattr(openai_forestx, "AsyncForestxClient"),
        "InitializeResponse": hasattr(openai_forestx, "InitializeResponse"),
        "ThreadStartParams": hasattr(openai_forestx, "ThreadStartParams"),
        "TurnStartParams": hasattr(openai_forestx, "TurnStartParams"),
        "TurnCompletedNotification": hasattr(openai_forestx, "TurnCompletedNotification"),
        "TurnStatus": hasattr(openai_forestx, "TurnStatus"),
    } == {
        "ForestxClient": False,
        "AsyncForestxClient": False,
...
def test_package_star_import_matches_public_api() -> None:
    ...
    exec("from openai_forestx import *", namespace)
...
def test_types_module_exports_curated_public_types() -> None:
    """The public type module should expose Forestx protocol models."""
...
def test_types_star_import_matches_public_types() -> None:
    ...
    namespace: dict[str, object] = {}
    exec("from openai_forestx.types import *", namespace)
...
def test_examples_use_public_import_surfaces() -> None:
    ...
    private_import_markers = [
        "openai_forestx.api",
        "openai_forestx.client",
        "openai_forestx.generated",
        "openai_forestx.models",
        "openai_forestx.retry",
    ]
...
def test_lifecycle_methods_are_forestx_scoped() -> None:
    """Lifecycle operations should hang off the client rather than thread objects."""
    assert hasattr(Forestx, "thread_resume")
    assert hasattr(Forestx, "thread_fork")
    assert hasattr(Forestx, "thread_archive")
    assert hasattr(Forestx, "thread_unarchive")
    assert hasattr(AsyncForestx, "thread_resume")
    assert hasattr(AsyncForestx, "thread_fork")
    assert hasattr(AsyncForestx, "thread_archive")
    assert hasattr(AsyncForestx, "thread_unarchive")
    assert not hasattr(Forestx, "thread")
    assert not hasattr(AsyncForestx, "thread")
    ...
    for fn in (
        Forestx.thread_archive,
        Forestx.thread_unarchive,
        AsyncForestx.thread_archive,
        AsyncForestx.thread_unarchive,
    ):
        _assert_no_any_annotations(fn)
...
def test_initialize_metadata_parses_user_agent_shape() -> None:
    ...
    payload = InitializeResponse.model_validate({"userAgent": "forestx-cli/1.2.3"})
    parsed = validate_initialize_metadata(payload)
    assert parsed is payload
    assert parsed.userAgent == "forestx-cli/1.2.3"
    assert parsed.serverInfo is not None
    assert parsed.serverInfo.name == "forestx-cli"

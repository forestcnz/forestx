...
import openai_forestx.api as public_api_module
from openai_forestx.api import (
    ApprovalMode,
    AsyncForestx,
    Forestx,
    Sandbox,
)
from openai_forestx.generated.v2_all import TurnStartParams
from openai_forestx.models import InitializeResponse
...
def test_forestx_init_failure_closes_client(monkeypatch: pytest.MonkeyPatch) -> None:
    closed: list[bool] = []
    ...
    monkeypatch.setattr(public_api_module, "ForestxClient", FakeClient)
...
def test_async_forestx_init_failure_closes_client() -> None:
    async def scenario() -> None:
        forestx = AsyncForestx()
        close_calls = 0
        ...
        forestx._client.start = fake_start  # type: ignore[method-assign]
        forestx._client.initialize = fake_initialize  # type: ignore[method-assign]
        forestx._client.close = fake_close  # type: ignore[method-assign]

        with pytest.raises(RuntimeError, match="missing required metadata"):
            await forestx.models()

        assert close_calls == 1
        assert forestx._initialized is False
        assert forestx._init is None

    asyncio.run(scenario())


def test_async_forestx_initializes_only_once_under_concurrency() -> None:
    async def scenario() -> None:
        forestx = AsyncForestx()
        start_calls = 0
        initialize_calls = 0
        ...
        async def fake_initialize() -> InitializeResponse:
            ...
            return InitializeResponse.model_validate(
                {
                    "userAgent": "forestx-cli/1.2.3",
                    "serverInfo": {"name": "forestx-cli", "version": "1.2.3"},
                }
            )
        ...
        forestx._client.start = fake_start  # type: ignore[method-assign]
        forestx._client.initialize = fake_initialize  # type: ignore[method-assign]
        forestx._client.model_list = fake_model_list  # type: ignore[method-assign]

        await asyncio.gather(forestx.models(), forestx.models())

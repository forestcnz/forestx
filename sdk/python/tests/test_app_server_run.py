...
from openai_forestx.generated.v2_all import MessagePhase
...
def test_sync_turn_result_uses_last_unknown_phase_message(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            result = forestx.thread_start().run("case: last unknown phase wins")
...
def test_sync_turn_result_preserves_empty_last_message(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            result = forestx.thread_start().run("case: empty last message")
...
def test_sync_turn_result_does_not_promote_commentary_only_to_final(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            result = forestx.thread_start().run("case: commentary only")
...
def test_async_turn_result_uses_last_unknown_phase_message(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncForestx(config=harness.app_server_config()) as forestx:
                result = await (await forestx.thread_start()).run("case: async last unknown phase")
...
def test_async_turn_result_does_not_promote_commentary_only_to_final(
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncForestx(config=harness.app_server_config()) as forestx:
                result = await (await forestx.thread_start()).run("case: async commentary only")
...
def test_final_answer_phase_survives_real_app_server_mapping(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            result = forestx.thread_start().run("choose final answer")

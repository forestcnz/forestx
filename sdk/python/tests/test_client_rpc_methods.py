...
from openai_forestx.client import ForestxClient, _params_dict
...
from openai_forestx.models import Notification, UnknownNotification
from openai_forestx.types import ThreadSource
...
def test_generated_v2_bundle_has_single_shared_plan_type_definition() -> None:
    source = (ROOT / "src" / "openai_forestx" / "generated" / "v2_all.py").read_text()
    assert source.count("class PlanType(") == 1


def test_plan_type_accepts_business_prolite_from_newer_runtime() -> None:
    """New runtime plan values should remain typed when using a forestx_bin override."""

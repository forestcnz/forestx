...
from openai_forestx.generated.v2_all import (
...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """ForestxClient.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            thread = forestx.thread_start()
            chunks = list(forestx._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncForestx(config=harness.app_server_config()) as forestx:
                thread = await forestx.thread_start()
                stream = forestx._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(forestx.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            first_thread = forestx.thread_start()
            second_thread = forestx.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncForestx(config=harness.app_server_config()) as forestx:
                first_thread = await forestx.thread_start()
                second_thread = await forestx.thread_start()

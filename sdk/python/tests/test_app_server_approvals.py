...
from openai_forestx import ApprovalMode, AsyncForestx, Forestx
from openai_forestx.generated.v2_all import AskForApprovalValue, ThreadResumeParams
...
def test_thread_resume_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            source = forestx.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            resumed = forestx.thread_resume(source.id)
            resumed_state = forestx._client.thread_resume(  # noqa: SLF001
                resumed.id,
...
def test_thread_fork_inherits_deny_all_approval_mode(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            source = forestx.thread_start(approval_mode=ApprovalMode.deny_all)
            result = source.run("seed the source rollout")
            forked = forestx.thread_fork(source.id)
            forked_state = forestx._client.thread_resume(  # noqa: SLF001
                forked.id,
...
def test_turn_approval_mode_persists_until_next_turn(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            ...
            after_turn_override = forestx._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            second_result = thread.run("inherit previous approval mode")
            after_omitted_turn = forestx._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_thread_run_approval_mode_persists_until_explicit_override(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            thread = forestx.thread_start(approval_mode=ApprovalMode.deny_all)

            first_result = thread.run("keep approvals denied")
            after_default_run = forestx._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
            ...
            after_override_run = forestx._client.thread_resume(  # noqa: SLF001
                thread.id,
                ThreadResumeParams(thread_id=thread.id),
            )
...
def test_async_thread_run_approval_mode_persists_until_explicit_override(
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncForestx(config=harness.app_server_config()) as forestx:
                thread = await forestx.thread_start(approval_mode=ApprovalMode.deny_all)
                first_result = await thread.run("keep async approvals denied")
                after_default_run = await forestx._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )
                ...
                after_override_run = await forestx._client.thread_resume(  # noqa: SLF001
                    thread.id,
                    ThreadResumeParams(thread_id=thread.id),
                )

...
def _app_server_config(harness: AppServerHarness) -> ForestxConfig:
...
def test_api_key_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with Forestx(config=_app_server_config(harness)) as forestx:
            forestx.login_api_key("sk-sdk-login-test")
            result = forestx.thread_start().run("prove api key auth")
...
def test_chatgpt_token_login_authenticates_follow_up_model_requests(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path, requires_openai_auth=True) as harness:
        ...
        with Forestx(config=_app_server_config(harness)) as forestx:
            login = forestx._client.account_login_start(
                LoginAccountParams(
            ...
            result = forestx.thread_start().run("prove chatgpt token auth")
            request = harness.responses.single_request()

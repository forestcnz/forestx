...
from openai_forestx import AsyncForestx, Forestx
...
def test_sync_and_async_initialization_round_trip_metadata(tmp_path) -> None:
    ...
    async def async_scenario(harness: AppServerHarness) -> dict[str, object]:
        async with AsyncForestx(config=harness.app_server_config()) as forestx:
            ...
            server = forestx.metadata.serverInfo
            return {
                "thread_id": thread.id,
                "user_agent": forestx.metadata.userAgent,
...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            active_thread = forestx.thread_start()
            archived_thread = forestx.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            forestx.thread_archive(archived_thread.id)
            active_list = forestx.thread_list(archived=False)
            archived_list = forestx.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncForestx(config=harness.app_server_config()) as forestx:
                ...
                resumed = await forestx.thread_resume(thread.id)
                forked = await forestx.thread_fork(thread.id)
                archive_response = await forestx.thread_archive(thread.id)
                unarchived = await forestx.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            ...
            forked = forestx.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            ...
            archived = forestx.thread_archive(thread.id)
            unarchived = forestx.thread_unarchive(thread.id)
...
def test_models_rpc(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        with Forestx(config=harness.app_server_config()) as forestx:
            models = forestx.models(include_hidden=True)

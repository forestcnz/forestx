...
from openai_forestx._goal import _GoalNotificationStream
from openai_forestx._run import _collect_turn_result
from openai_forestx.generated.notification_registry import notification_turn_id
from openai_forestx.generated.v2_all import TurnStatus
...
def test_private_goal_operation_coalesces_runtime_continuations(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with Forestx(config=harness.app_server_config()) as forestx:
            ...
            state, turn_id = forestx._client.start_goal_operation(  # noqa: SLF001
                thread.id,
            ...
            stream = _GoalNotificationStream(
                ...
                lambda: forestx._client.unregister_goal_operation(state),  # noqa: SLF001
                lambda: forestx._client.cancel_goal_operation(state),  # noqa: SLF001
            )
            events = list(stream)
            result = _collect_turn_result(iter(events), turn_id=turn_id)
            routes = forestx._client._router._goal_operations.copy()  # noqa: SLF001

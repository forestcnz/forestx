...
from openai_forestx import AsyncForestx
...
async def main() -> None:
    async with AsyncForestx(config=runtime_config()) as forestx:
        ...
        reopened = await forestx.thread_resume(thread.id)
        listing_active = await forestx.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await forestx.thread_archive(reopened.id)
        listing_archived = await forestx.thread_list(limit=20, archived=True)
        unarchived = await forestx.thread_unarchive(reopened.id)

        resumed = await forestx.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await forestx.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (

...
with Forestx(config=runtime_config()) as forestx:
    ...
    reopened = forestx.thread_resume(thread.id)
    listing_active = forestx.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = forestx.thread_archive(reopened.id)
    listing_archived = forestx.thread_list(limit=20, archived=True)
    unarchived = forestx.thread_unarchive(reopened.id)

    resumed = forestx.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = forestx.thread_fork(unarchived.id, model="gpt-5.4")

...
from release_version import normalize_forestx_version  # noqa: E402

SDK_DISTRIBUTION_NAME = "openai-forestx"
RUNTIME_DISTRIBUTION_NAME = "openai-forestx-cli-bin"
RUNTIME_PACKAGE_ROOT = Path("src") / "forestx_cli_bin"
FORESTX_PACKAGE_METADATA = "forestx-package.json"
...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "forestx_app_server_protocol.v2.schemas.json"
...
def runtime_binary_name() -> str:
    return "forestx.exe" if _is_windows() else "forestx"


def runtime_code_mode_host_name() -> str:
    return "forestx-code-mode-host.exe" if _is_windows() else "forestx-code-mode-host"
...
def pinned_runtime_version() -> str:
    ...
    return normalize_forestx_version(pins[0])


def pinned_runtime_forestx_path() -> Path:
    """Return the bundled Forestx binary from the installed pinned runtime wheel."""
    ...
    normalized_installed_version = normalize_forestx_version(installed_version)
    ...
    try:
        from forestx_cli_bin import bundled_forestx_path
    except ImportError as exc:
        raise RuntimeError(
            f"Installed {RUNTIME_DISTRIBUTION_NAME} package does not expose bundled_forestx_path."
        ) from exc

    forestx_path = bundled_forestx_path()
    if not forestx_path.exists():
        raise RuntimeError(f"Pinned Forestx runtime binary not found at {forestx_path}.")
    return forestx_path
...
def stage_python_sdk_package(staging_dir: Path, sdk_version: str) -> Path:
    package_version = normalize_forestx_version(sdk_version)
    _copy_package_tree(sdk_root(), staging_dir)
    sdk_bin_dir = staging_dir / "src" / "openai_forestx" / "bin"
...
def stage_python_runtime_package(
    ...
    package_version = normalize_forestx_version(forestx_version)
    ...
    _extract_forestx_package_archive(package_archive, staged_runtime_package_root(staging_dir))
    return staging_dir


def _extract_forestx_package_archive(package_archive: Path, runtime_package_root: Path) -> None:
    if not package_archive.name.endswith(".tar.gz"):
        raise RuntimeError(f"Expected a .tar.gz Forestx package archive: {package_archive}")
    ...
    _validate_forestx_package_layout(runtime_package_root, package_archive)


def _validate_forestx_package_layout(package_dir: Path, package_archive: Path) -> None:
    missing_entries = []
    if not (package_dir / FORESTX_PACKAGE_METADATA).is_file():
        missing_entries.append(FORESTX_PACKAGE_METADATA)
    for entry in ("bin", "forestx-resources", "forestx-path"):
    ...
    if missing_entries:
        ...
        raise RuntimeError(f"Missing Forestx package layout entries in {package_archive}: {missing}")
...
def generate_schema_from_pinned_runtime(schema_dir: Path) -> Path:
    ...
    forestx_path = pinned_runtime_forestx_path()
...
def generate_v2_all(schema_dir: Path) -> None:
    ...
    out_path = sdk_root() / "src" / "openai_forestx" / "generated" / "v2_all.py"
...
def _notification_specs(schema_dir: Path) -> list[tuple[str, str]]:
    ...
    generated_source = (sdk_root() / "src" / "openai_forestx" / "generated" / "v2_all.py").read_text()
...
def generate_notification_registry(schema_dir: Path) -> None:
    ...
    out = sdk_root() / "src" / "openai_forestx" / "generated" / "notification_registry.py"
...
def _load_public_fields(
    ...
    if module_name == "openai_forestx.generated.v2_all":
...
def _load_generated_v2_all_module() -> types.ModuleType:
    ...
    module_name = "_openai_forestx_generated_v2_all_for_artifacts"
    sys.modules.pop(module_name, None)
    module_path = sdk_root() / "src" / "openai_forestx" / "generated" / "v2_all.py"
...
def _render_forestx_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(thread_start_fields),
        "    ) -> Thread:",
        '        """Create a new Forestx conversation thread."""',
...
def _render_async_forestx_block(
    thread_start_fields: list[PublicFieldSpec],
    thread_list_fields: list[PublicFieldSpec],
    resume_fields: list[PublicFieldSpec],
    fork_fields: list[PublicFieldSpec],
...
def _render_async_thread_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(turn_fields),
        "    ) -> AsyncTurnHandle:",
        '        """Start a turn and return a handle for streaming or control."""',
        "        await self._forestx._ensure_initialized()",
        "        wire_input = _to_wire_input(_normalize_run_input(input))",
        _approval_mode_assignment_line("_approval_mode_override_settings"),
        "        params = TurnStartParams(",
        "            thread_id=self.id,",
        ...
        *_model_arg_lines(turn_fields),
        "        )",
        "        turn = await self._forestx._client.turn_start(",
        "            self.id,",
        "            wire_input,",
        "            params=params,",
        "        )",
        "        return AsyncTurnHandle(self._forestx, self.id, turn.turn.id)",
    ]
    return "\n".join(lines)
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "openai_forestx" / "api.py"
    ...
    thread_start_fields = _load_public_fields(
        "openai_forestx.generated.v2_all",
        "ThreadStartParams",
    ...
    source = _replace_generated_block(
        source,
        "Forestx.flat_methods",
        _render_forestx_block(
            thread_start_fields,
            thread_list_fields,
    ...
    source = _replace_generated_block(
        source,
        "AsyncForestx.flat_methods",
        _render_async_forestx_block(
            thread_start_fields,
            thread_list_fields,
            thread_resume_fields,
            thread_fork_fields,
...
def generate_types() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="forestx-python-schema-") as td:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_runtime_parser.add_argument(
        "package_archive",
        type=Path,
        help="Path to a Forestx package .tar.gz archive for this platform.",
    )
    stage_runtime_parser.add_argument(
        "--forestx-version",
        required=True,
        help=(
            "Forestx release version to write into the staged runtime package. "
...
def run_command(args: argparse.Namespace, ops: CliOps) -> None:
    if args.command == "generate-types":
        ...
        ops.stage_python_sdk_package(
            args.staging_dir,
            normalize_forestx_version(args.sdk_version),
        )
    elif args.command == "stage-runtime":
        ops.stage_python_runtime_package(
            args.staging_dir,
            normalize_forestx_version(args.forestx_version),

...
CONTAINER_NAME="forestx_$(echo "$WORK_DIR" | sed 's/\//_/g' | sed 's/[^a-zA-Z0-9_-]//g')"
...
docker exec --user root "$CONTAINER_NAME" bash -c "mkdir -p /etc/forestx"
for domain in $OPENAI_ALLOWED_DOMAINS; do
  ...
  echo "$domain" | docker exec --user root -i "$CONTAINER_NAME" bash -c "cat >> /etc/forestx/allowed_domains.txt"
done

# Set proper permissions on the domains file
docker exec --user root "$CONTAINER_NAME" bash -c "chmod 444 /etc/forestx/allowed_domains.txt && chown root:root /etc/forestx/allowed_domains.txt"
...
docker exec -it "$CONTAINER_NAME" bash -c "cd \"/app$WORK_DIR\" && forestx --sandbox workspace-write --ask-for-approval on-request ${quoted_args}"

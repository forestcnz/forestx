...
./scripts/stage_npm_packages.py \
  ...
  --package forestx \
  --package forestx-responses-api-proxy \
  --package forestx-sdk
```
...
When `--package forestx` is provided, the staging helper builds the lightweight
`@openai/forestx` meta package plus all platform-native `@openai/forestx` variants
